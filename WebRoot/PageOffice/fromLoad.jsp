...
<html>
  ...
  <script type="text/javascript">
  	...
  	function init(){
		...
		if(para.isPreview){
			//预览打印
			window.location.href = "printFormPre.do?isCheck=false"+paraStr;
		}else{
			//直接打印
			window.location.href = "printForm.do?isCheck=false"+paraStr;
		}
		
		/* $.ajax({
			type: "post",
			url: "printFormData.do?isCheck=false",
			...
			success: function (responseData) {
				...
				var resPara="&temp_file_name="+responseData.temp_file_name+"&business_no="+responseData.business_no;
				resPara=resPara+"&group_id="+responseData.group_id+"&hos_id="+responseData.hos_id;
				resPara=resPara+"&hos_copy="+responseData.copy_code+"&template_code="+responseData.template_code;
				
				if(responseData.isPreview){
					//预览打印
					resPara=resPara+"&para011="+responseData.para011;
					window.location.href = "printFormPre.do?isCheck=false"+resPara;
				}else{
					//直接打印
					window.location.href = "printForm.do?isCheck=false"+resPara;
				}
