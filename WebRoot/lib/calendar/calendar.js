    ...
    $.fn.calendar = function (options) {
        ...
        var yearSelect = '';
    	for (var i = 1900; i < 2101; i++) yearSelect += "<option value=\"" + i + "\">" + i + "</option>";
    	$('.SY').html(yearSelect);
        ...
        $(".saveChange").click(function () {
            ...
            for(var i =0; i<dateArr.length; i++){
               if(!obj[dateArr[i].attend_date]){
                  result.push(dateArr[i]);
                  obj[dateArr[i].attend_date] = true;
               }
            };

        	ajaxPostData({
                url: 'addOfficialHoliday.do',
                data: {
                	paramVo : JSON.stringify(result),
                },
    ...
    function setconfigDay(options, count, _configDayM) {
        ...
        if (_configDayM) {
            $.each(_configDayM, function (k, v) {
                ...
                if (v == 0) {
                    $("#days" + D).append("<div class=\"xbgj\" xbgj=\"0\" style=\"position: absolute;top:0;right:-1px;border-top: 25px solid #53a253; border-left: 25px solid transparent\"><div class=\"rest\" style=\"position: absolute;top:-37px;right:10px\">休</div></div>");
                }
                if (v == 1) {
                    $("#days" + D).append("<div class=\"xbgj\" xbgj=\"1\" style=\"position: absolute;top:0;right:-1px;border-top: 25px solid #e15e5e; border-left: 25px solid transparent\"><div class=\"rest\" style=\"position: absolute;top:-37px;right:10px\">班</div></div>");
                }
            });
        }
    }
    ...
    DateTable.prototype.create = function (M) {
        this.html = "<div class=\"calendar\ calendar" +M + "\">";
    ...
    DateTable.prototype.leftHead = function () {
        ...
        /*this.html += "<div class=\"text\">" + this.Y + "年" + this.M + "月</div>";*/
        this.html += "<div class=\"text\">" + this.M + "月</div>";
        this.html += "</div>";
    };
    ...
    DateTable.prototype.leftDay = function () {
        ...
        for (i = 0; i < this.count * 7; i += 1) {
            if (i < this.W - 1 || i >= this.days + this.W - 1) this.html += "<div class=\"days1\"></div>";
                ...
                this.html += "<div class=\"days\" id=\"days" + day + "\" title=\"请右键设置\"><div id=\"num" + day +
                    "\" class=\"num\" style=\"color:" + color + ";\">" +
                    (day > 9 ? day : ("0" + day)) + "</div><div class=\"lunar lunar" +
                    day + "\"></div></div>";
                W += 1;
                if (W == 8) W = 1;
            }
    ...
    DateTable.prototype.lunarDay = function () {
        ...
        for (var i = 0; i < this.days; i += 1) {
            ...
            $(".calendar" +(this.M) +' '+ ".lunar" + (i + 1)).append(temp);
            // 设置不同的显示字体颜色
            if (calendar.lunarFestival[lunar[2] + "-" + lunar[3]] || calendar.gregorianFestival[this.M + "-" + (i + 1)]) {
                $(".calendar" +(this.M) +' '+ ".lunar" + (i + 1)).css("color", "red");
            } else if (solarTerms !== "") {
                $(".calendar" +(this.M) +' '+ ".lunar" + (i + 1)).css("color", "green");
            }
            // 计算下个日期的农历
    ...
    function contextmenuClick(options, count, e) {
        ...
        var mouth = e.parentNode.firstChild.textContent;
        mouth = parseInt(mouth.substr(0,mouth.length -1));
        var MM = (mouth > 9 ? mouth : ("0" + mouth));
        var DD = (D > 9 ? D : ("0" + D));
        
        // 加班
        $("#menu-overtime").on('click',function () {
            var temp = $(e).find(".xbgj");
            if (temp.length > 0) {
                if ($(temp[0]).attr("xbgj") == "1") {
                	$.etDialog.warn('请不要重复选择!');
                	$(".dayRightClick").remove();
                    return;
                }else{
                	if(dateArr.length > 0){
                		$.each(dateArr,function(i,k){
                    		if(k.attend_date.indexOf(Y +"-" +MM +"-"+ DD)>-1){
                    			k.attend_date_state = "1";	
                    		}else{
                    			dateArr.push({"attend_date":Y +"-" +MM +"-"+ DD,"attend_date_state":"1"});
                    		}
                    	});
                	}else{
                		dateArr.push({"attend_date":Y +"-" +MM +"-"+ DD,"attend_date_state":"1"});
                	}
                	$(e).append("<div class=\"xbgj\" xbgj=\"1\" style=\"position: absolute;top:0;right:-1px;border-top: 25px solid #e15e5e; border-left: 25px solid transparent\"><div class=\"rest\" style=\"position: absolute;top:-37px;right:10px;\">班</div></div>");
                	$(".dayRightClick").remove();
                }
                temp.remove();
        ...
        // 休假
        $("#menu-vacation").on('click',function () {
            var temp = $(e).find(".xbgj");
            if (temp.length > 0) {
                if ($(temp[0]).attr("xbgj") == "0") {
                	...
                	if(dateArr.length > 0){
                		$.each(dateArr,function(i,k){
                    		if(k.attend_date.indexOf(Y +"-" +MM +"-"+ DD)>-1){
                    			k.attend_date_state = "0";	
                    		}else{
                    			dateArr.push({"attend_date":Y +"-" +MM +"-"+ DD,"attend_date_state":"0"})
                    		}
                    	});
                	...
                	$(e).append("<div class=\"xbgj\" xbgj=\"0\" style=\"position: absolute;top:0;right:-1px;border-top: 25px solid #53a253; border-left: 25px solid transparent\"><div class=\"rest\" style=\"position: absolute;top:-37px;right:10px;\">休</div></div>");
                	$(".dayRightClick").remove();
                }
                temp.remove();
                delete configDayM["D" + D];
                ...
                if(dateArr.length > 0){
                	$.each(dateArr,function(i,k){
                		if(k.attend_date.indexOf(Y +"-" +MM +"-"+ DD)>-1){
                			...
                			dateArr.push({"attend_date":Y +"-" +MM +"-"+ DD,"attend_date_state":""})
                		}
                	});
                }else{
                	dateArr.push({"attend_date":Y +"-" +MM +"-"+ DD,"attend_date_state":""})
    ...
    function btnClick(options, e, count) {
        ...
        if (isclick) {
            $(".days").contextmenu(function (e) {
                ...
                //body盒子的宽高
                var calendarW = $("#calendar").width()- 80;
                var calendarH = $(window).height()- $(".main").height()-80;
                if(mouseX > calendarW) {
                	mouseX = calendarW;
                }
                if(mouseY > calendarH) {
                ...
                html += "<div class=\"menu-item\" id=\"menu-vacation\"><div class=\"menu-ico\"><div class=\"icoText\" style=\"background-color: #53a253\">休</div></div><div class=\"menu-text\">休假</div></div>";
                html += "<div class=\"menu-item\" id=\"menu-overtime\"><div class=\"menu-ico\"><div class=\"icoText\" style=\"background-color: #e15e5e\">班</div></div><div class=\"menu-text\">加班</div></div>";
                html += "<div class=\"menu-line\"></div>";
                html += "<div class=\"menu-item\" id=\"menu-cancel\"><div class=\"menu-text\" style=\"padding-left: 19px\">取消</div></div>";  
                html += "</div>";
                $("body").append(html);
    ...
    function setLayer(options, count) {
        ...
        $(".calendar").css("width","24%").css("height","332px").css('float','left');
        /*$(".leftArea").css("width", parseInt(width * rate - 16) + "px").css("height", height - 16 + "px");*/
        $(".head").css("width","100%").css("height","23px").css("line-height", "23px");
        $(".week").css("width", "14.28%").css("height", parseInt((height - 16) / (count + 1) * 0.4) + "px");
        $(".days").css("width", "13.5%").css("height", parseInt((height - 16) / (count + 1) - 2.5) + "px").css('line-height','33px').css('position','relative');
        $(".days1").css("width","13.5%").css("height", parseInt((height - 16) / (count + 1) - 2.5) + "px");
        ...
        success: function (responseData) {
            ...
            if (responseData) {
            	for(var i=0; i<responseData.Rows.length; i++){
            		Y = responseData.Rows[i].attend_date.split("-")[0];
            		M = responseData.Rows[i].attend_date.split("-")[1].replace(/^0/, '');
            		D = responseData.Rows[i].attend_date.split("-")[2].replace(/^0/, '');

            		if(YYYY == Y){
            			var temp = $(".calendar" + M +" " + "#days" + D).find(".xbgj");
                        if (temp.length > 0) {
                            $(".calendar" + M +" " + "#days" + D).find(".xbgj").remove();
                        }
                        if (responseData.Rows[i].attend_date_state == 0) {
                            $(".calendar" + M +" " + "#days" + D).append("<div class=\"xbgj\" xbgj=\"0\" style=\"position: absolute;top:0;right:-1px;border-top: 25px solid #53a253; border-left: 25px solid transparent\"><div class=\"rest\" style=\"position: absolute;top:-37px;right:10px\">休</div></div>");
                        }
                        if (responseData.Rows[i].attend_date_state == 1) {
                            $(".calendar" + M +" " + "#days" + D).append("<div class=\"xbgj\" xbgj=\"1\" style=\"position: absolute;top:0;right:-1px;border-top: 25px solid #e15e5e; border-left: 25px solid transparent\"><div class=\"rest\" style=\"position: absolute;top:-37px;right:10px\">班</div></div>");
                        }
            		}
            	}
            }
