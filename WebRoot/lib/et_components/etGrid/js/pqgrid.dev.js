					...
					DM.data = responseObj.Rows;               //   之前是data 数据格式给为Rows
 					if (PM.type && PM.type == "remote") {
						...
						if (responseObj.Total) {
							PM.totalRecords = responseObj.Total
						}
					}
				}
				that.hideLoading();
				...
				if (type == "checkbox") {
					...
					if (type == "separator") {
						...
						if (type == "button") {
							...
							$ctrl = $("<button type='button' class='" + cls + "' " + attr+"  toolbarid='"+itemid + "' " + style + ">" + text + "</button>").button(options).appendTo(element);
						} else {
							if (type == "select") {
								var options = item.options ? item.options : [];
								if (typeof options === "function") {
...
/*========================重写方法================================== */
	...
	var _PR = $.paramquery.cCells.prototype;
	var _PC = $.paramquery.cRows.prototype;
