...
(function ($) {
    ...
    $.ligerDefaults.ComboBox = {
        ...
        onCheckBoxSelected: null, //复选框选择事件
        onUnCheckBoxSelected: null, //
        initValue: null,
        value: null,
        initText: null,
        valueField: 'id',
        ...
        selectBoxTableCss: null,    //   选项框table样式 object形式的   { ‘fontSize’：‘12px’,.....}
        keySupport: false, //按键支持： 上、下、回车 支
        initIsTriggerEvent: false, //初始化时是否触发选择事件
        conditionSearchClick: null, //下拉框表格搜索按钮自定义函数
        onChangeValueImmediately: null,  //  input框输入值时触发事件
        onChangeValue: null,
        delayLoadGrid: true, //是否在按下显示下拉框的时候才 加载 grid
        setTextBySource: true, //设置文本框值时是否从数据源中加载
        async: true ,//默认异步//pj20161019
        spaceSearch:false,  //空格代替 键盘检索input autoComplete
        autocompletelocal: false,//pj20191128,是否本地检索
        pageSize: 50//pj20191128,配合参数autocompletelocal使用，默认只显示50条
    };
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function () {
            var g = this,
                p = this.options;
            g.data = p.data;
            if(p.autocompletelocal)g.initData=p.data;//pj20191128参数autocompletelocal
            g.inputText = null;
            g.select = null;
            g.textFieldID = "";
            g.valueFieldID = "";
            ...
            if (p.inputlabelAlign) {
                g.inputText.css('textAlign', p.inputlabelAlign);
            }
            ...
            if (p.maxWidth) {
                g.selectBox.css('maxWidth', p.maxWidth);
            }
            if (p.grid) {
                if (p.delayLoadGrid) {
        ...
        _setCancelable: function (value) {
            ...
            g.wrapper.hover(function () {
                if (g.unselect != null) g.unselect.show(); //pj20161019
            }, function () {
                if (g.unselect != null) g.unselect.hide(); //pj20161019
            })
        ...
        insertItem: function (data, index) {
            var g = this,
                p = this.options;
            g.data = g.data || [];
            if(p.autocompletelocal){
            	g.data.splice(0, 1, data);
            }else{
        ...
        _setUrl: function (url, callback) {
            ...
            if (p.delayLoad && !g.isAccessDelay && !g.triggerLoaded) {
                //只要delayLoad=true每次都延时加载
                //g.isAccessDelay = true;//已经有一次延时加载了
                return;
            }
            if(p.autocompletelocal && g.data && g.data.length>0){
            	//pj20191128有数据不加载,缓存机制,配置data参数,组件会执行g.setData(g.data)方法
            	return;
            }
            ...
            var ajaxOp = {
                ...
                success: function (result) {
                    ...
                    /************pj20191128pj20191128只加载默认的条数*********/
                    if(p.autocompletelocal)g.initData=data;
                	g.setData(data);
                    /************pj20191128pj20191128只加载默认的条数*********/
                    g.trigger('success', [data]);
        ...
        setData: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
            	//pj20191128autocompletelocal
                if(p.autocompletelocal){
              		if(p.initValue && p.initValue!=null && p.initValue!=""){
              			if(data[i][p.valueField]==p.initValue){
              				this._setValue(p.initValue);
              			}else{
              				continue;
              			}
              		}else if(i>p.pageSize){
              			break;
              		}
            ...
            if(p.autocompletelocal)g.data=pageData;//pj20191128autocompletelocal
            if (!p.columns) {
                if (p.isShowCheckBox) {
                ...
                if (p.selectBoxTableCss && typeof p.selectBoxTableCss == "object") {
                    g.selectBox.table.css(p.selectBoxTableCss);
                }
            } else {
            ...
            $(".l-table-checkbox input:checkbox", g.selectBox).change(function () {
                if (this.checked) g.trigger('CheckBoxSelected', [$(this).parent().parent().attr("value")]);
                if (!this.checked) g.trigger('UnCheckBoxSelected', [$(this).parent().parent().attr("value")]);
        ...
        setGrid: function (grid) {
            ...
            var selecteds = [],
                onGridSelect = function () {
                    ...
                    var selected1 = g.grid.getSelected();
                    if (selected1)
                        g.grid.gridbody.scrollTop(selected1["__index"] * 29);
        ...
        upFocus: function () {
            ...
            if (g.grid) {
                ...
                var selected1 = g.grid.getSelected();
                // if(selected1 != null)
                // g._gridScrollAdjust(selected1["__index"]);
        ...
        _gridScrollAdjust: function (currentIndex) { //下拉框(grid)滚动条控制
            var g = this,
                p = this.options;
            if (currentIndex * 29 > g.grid.gridview2.height() * .6) {
                g.grid.gridbody.scrollTop(g.grid.gridbody.scrollTop() + 2 * 29);

            } else if (currentIndex * 29 - g.grid.gridbody.scrollTop() < 29) {
                g.grid.gridbody.scrollTop(currentIndex * 29)
            };
        },
        ...
        updateSelectBoxPosition: function () {
            ...
            if (p && p.absolute) {
                ...
                var selfheight = g.selectBox.height() + (textHeight || 0) + 4;
                if (topheight + selfheight > $(window).height() && topheight > selfheight) {
                    g.selectBox.css("marginTop", -1 * (g.selectBox.height() + (textHeight || 0) + 5) + (p.selectBoxPosYDiff || 0));
                }
            }
        },
        _toggleSelectBox: function (isHide) {
            ...
            if (isHide) {
                ...
                if (p.slide) {
                    ...
                    } else if (g.grid) { //下拉框(grid)初始化时滚动条滚到选中位置
                        var selected1 = g.grid.getSelected();
                        if (selected1)
                            g.grid.gridbody.scrollTop(selected1["__index"] * 29);
                    }
        ...
        _setAutocomplete: function (value) {
            ...
            g.inputText.keyup(function (event) {
                ...
                //if (!g.inputText.val().replace(/\s/g, "")) { return;}  // 文本框为空则跳出有bug，当输错时，删除后不检索
                if(event.keyCode!== 32 && p.spaceSearch){
                    return;
                }
                var onoff = 'keyup';
                if (g.hasBind("changeValueImmediately")) {
                    g.trigger('changeValueImmediately', [onoff])
                }
               
                if (event.keyCode == 37 || event.keyCode == 38 || event.keyCode == 39 || event.keyCode == 40 || event.keyCode == 13) //left 、up 、right 、down、enter
                {
                    return;
                }
                var currentKey = g.inputText.val();
            	if(currentKey.indexOf(" ")!=-1){currentKey=currentKey.split(" ")[0];} //包含空格
                currentKey=currentKey.toUpperCase().replace(/\s/g, "").replace(/\'/g,"");
               /*********本地检索pj20191128******************/
                if(p.autocompletelocal){
                	p.initValue="";
                	var substrRegex,likeReg,vegReg;
                	try{
    	 	           substrRegex = new RegExp($.trim(currentKey), 'i');//左右模糊匹配
    	 	           likeReg = new RegExp("^"+$.trim(currentKey));//右模糊匹配
    	 	           //vegReg = new RegExp("^(0|[1-9]\d{0,9})$"+$.trim(currentKey));
     	            }catch(e){}
     	            var data=[];
     	            if(g.initData){
     	            	 $.each(g.initData,function(i,o){
          	            	if(data.length>p.pageSize){
          	            		return false;
          	            	}
          	            	if(likeReg.test(o[p.textField]) || substrRegex.test(o[g.options.textField])){
          	            		data.push(o);
          	            	}else if(o.spell_code && (likeReg.test(o.spell_code) || substrRegex.test(o.spell_code))){
          	            		data.push(o);
          	            	}else if(o.wbx_code && (likeReg.test(o.wbx_code) || substrRegex.test(o.spell_code))){
          	            		data.push(o);
          	            	}
          	            });
     	            }
        ...
        $(document).bind('keydown.ligercombobox', function (event) {
            function down() {
                if (!combobox.selectBox.is(":visible")) {
                    combobox.updateSelectBoxPosition();  // 定位不准   ----(cl)
            ...
            if (curInput.length && curInput.attr("data-comboboxid")) {
                ...
                {
                    if (combobox.hasBind('textBoxKeySpace')) {
                    	//if(combobox.inputText["0"].value == "") {
                    		combobox.trigger('textBoxKeySpace', [{
                                callback: function () {
                                	space();
                                }
                            }]);
...
(function ($) {
    ...
    $.ligerDefaults.DateEditor = {
        ...
        cancelable: true, //可取消选择
        readonly: false, //是否只读
        showSelectBox: false      //  控制表格里日期控件在获焦时是否展现下拉框
    };
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function () {
            ...
            dateeditorHTML += "        <div class='l-button l-button-cancel'></div>";
            ...
            g.buttons = {
                ...
                btnCancel: $(".l-button-cancel", g.toolbar)
            };
            ...
            g.buttons.btnCancel.html(p.cancelMessage);
            //设置时间
            if (p.showTime) {
                g.toolbar.time.show();
            ...
            g.buttons.btnCancel.click(function () {
                g.clear();
            })
            ...
            g.inputText.bind('keydown.dateeditor', function (e) {
                var key = e.which;
                if (key == 13) {
                    g.onTextChange();
                    if (!g.inputText.val() && g.dateeditor.is(":visible")) {
                        g.showDate();
                    }
        ...
        _bulidDate: function (dateStr) {
            ...
            var rel2 = /^(\d{1,2})(\/)(\d{1,2})(\/)(\d{1,4})$/;
            if (!r) return null;
            if (/^\d{8}$/.test(dateStr)) {
                var dateStrArr = dateStr.split("");
                dateStrArr.splice(4, 0, "-");
                dateStrArr.splice(-2, 0, "-");
                dateStr = dateStrArr.join("");
            } else if (rel2.test(dateStr)) {
                var dateStrArr = dateStr.split("/");
                dateStrArr.reverse();
                dateStrArr[1] = dateStrArr.splice(2, 1, dateStrArr[1])[0]; //数组元素换位
                dateStr = dateStrArr.join("-");
            }
...
(function ($) {
    ...
    $.ligerDefaults.Dialog = {
        ...
        id: null, //给dialog附加id
        buttons: null, //按钮集合
        initFocusOnButton: false,     // 按钮是否聚焦
        isDrag: true, //是否拖动
        width: 280, //宽度
        ...
        showMax: false, //是否显示最大化按钮  同时 可以控制初始化最大化
        initShowMax: true, //是否初始化最大化
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function () {
            ...
            $('body').append(dialog);
            g.dialog = dialog;
            if (!p.showClose) g.dialog.find(".l-dialog-close").css("display", "none");
            ...
            if (p.buttons) {
                $(p.buttons).each(function (i, item) {
                    var btn = $('<div tabindex = "0" class="l-dialog-btn"><div class="l-dialog-btn-l"></div><div class="l-dialog-btn-r"></div><div class="l-dialog-btn-inner"></div></div>');
            ...
            if (p.initFocusOnButton) {
                $(".l-dialog-btn-ok", g.dialog.buttons).focus();
            };
        },
        _borderX: 12,
        _borderY: 32,
        ...
        active: function () {
            ...
            if (g.minimize) {
                ...
                if (p.slide) {
                    g._borderX = 0; //CHD20160824激活宽度会变小12，所以设置为0
                    g.dialog.body.animate({
                        width: width - g._borderX
                    }, p.slide);
                    g.dialog.animate({
    ...
    $.ligerDialog.confirm = function (content, title, callback, options) {
        if (typeof (title) == "function") {
            callback = title;
            type = null;
        }
...
(function ($) {
    var l = $.ligerui;
    var _self;   //  指向grid   在右键菜单参数有用到
    var menuData=[];
    var is_flag = false; //由于页面中有多个grid点击按钮会执行多次click方法，故加上判断限制
	menuData.push({
        text: '保存查询列', id: 'saveColumns',icon: 'view', click: function () {
            //************************************************************************************************************************
            if(is_flag){return false;}
            is_flag = true;
            //页面路径
            var pathName = window.location.pathname;//(/CHD-HRP/hrp/mat/storage/in/mainPage.do)
            var split = pathName.split("/").length - 4;//(后台请求路径：hrp/sys/saveStyleByUrl)
            var urlStr = "";
            for (var i = 0; i < split; i++) {
                urlStr = urlStr + "../"; //计算请求路径
            }

            pathName = pathName.replace(".do", "").replace(/\//g, '[').replace("[CHD-HRP[hrp[", "");
            if (_self.options.gid) {
                pathName = pathName + "[" + _self.options.gid;
            }
            if (pathName.leng > 230) {
                alert(pathName + "，路径长度超过230！")
                return;
            }

            var para = {
                type: "grid",
                mod_code: getPageModCode(),
                page_url: pathName,
                page_json: JSON.stringify(_self.getColumns(1))
            };
						
            $.ajax({
                type: 'POST',
                url: urlStr + 'sys/saveStyleByUrl.do?isCheck=false',
                dataType: 'json',
                ...
                success: function (responseData) {
                    if (responseData.error) {
                        ...
                        //存浏览器
                        Local.set(pathName, JSON.stringify(_self.getColumns(1)));
                    }
                    is_flag = false;
	...
	if (typeof lodopExportExcel === "function") {
		menuData.push({
        text: '导出Excel', icon: 'down', click: function () {
            //************************************************************************************************************************
            
            if (typeof lodopExcel === "function") {
                lodopExcel();
            }

            if (_self.options.usePager) {
                //分页，导出全部
                var selPara = {};
                $.each(_self.options.parms, function (i, obj) {
                    selPara[obj.name] = obj.value;
                });
                //selPara["usePager"]=false;
                selPara["pagesize"] = 999999;
                selPara["page"] = 0;
                ajaxJsonObjectByUrl(_self.options.url, selPara, function (responseData) {
                    lodopExportExcel({ column: _self.getColumns(1), data: responseData.Rows, isFastExport: _self.options.lodop.isFastExport, fn: _self.options.lodop.fn });
                });
            } else {
                //不分页，导出查询结果
                lodopExportExcel({ column: _self.getColumns(1), data: _self.getData(), isFastExport: _self.options.lodop.isFastExport, fn: _self.options.lodop.fn });
            }
...
/* {
    text: '保存打印列', icon: 'pager', click: function () {
        //************************************************************************************************************************
        //页面路径
        var pathName = window.location.pathname;//(/CHD-HRP/hrp/mat/storage/in/mainPage.do)
        var split = pathName.split("/").length - 4;//(后台请求路径：hrp/sys/saveStyleByUrl)
        ...
        if (_self.options.title) {
            pathName = pathName + "{" + _self.options.title;
        }
        if (pathName.leng > 230) {
            alert(pathName + "，路径长度超过230！")
            return;
...
{
    text: '还原默认格式', icon: 'back', click: function () {
        //************************************************************************************************************************
        //页面路径
        var pathName = window.location.pathname;//(/CHD-HRP/hrp/mat/storage/in/mainPage.do)
        var split = pathName.split("/").length - 4;//(后台请求路径：hrp/sys/saveStyleByUrl)
        ...
        $.ajax({
            type: 'POST',
            url: urlStr + 'sys/deleteStyleByUrl.do?isCheck=false',
            dataType: 'json',
            contentType: 'application/json',
            data: JSON.stringify(para),
            success: function (responseData) {
...
{
    text: '打印预览', icon: 'print', click: function () {
        //***********************************************************************************************************************
        if (!(typeof lodopTable === "function")) {
            alert("没有引入LodopFuncs.js")
            return;
        }

        var type = _self.options.lodop.type;
        if (_self.options.usePager) {
            //分页，打印全部
            var selPara = {};
            $.each(_self.options.parms, function (i, obj) {
            ...
            ajaxJsonObjectByUrl(_self.options.url, selPara, function (responseData) {
                lodopGridTable(_self, responseData.Rows, type);
            });

        } else {
            lodopGridTable(_self, null, type);
        }
...
{
    text: '直接打印', icon: 'print', click: function () {
        //***********************************************************************************************************************
        if (!(typeof lodopTable === "function")) {
            alert("没有引入LodopFuncs.js")
            return;
        ...
        //分页，打印全部
            ...
            ajaxJsonObjectByUrl(_self.options.url, selPara, function (responseData) {   
                lodopGridTable(_self, responseData.Rows,4);
            });

        }else{
        lodopGridTable(_self, null,4);
        }
        //***********************************************************************************************************************
    }
},
{
    text: '打印维护', icon: 'settings', click: function () {
        //************************************************************************************************************************
        if (!(typeof lodopTable === "function")) {
            alert("没有引入LodopFuncs.js")
            return;
        ...
        if (_self.options.usePager) {
            ...
            selPara["page"] = 0;
            ajaxJsonObjectByUrl(_self.options.url, selPara, function (responseData) {
                lodopGridTable(_self, responseData.Rows, 2);
            });

        } else {
            lodopGridTable(_self, null, 2);
        }
        //***********************************************************************************************************************
    }
}*/
	
    var contextmenuEidtorObj = {             // 参数是对象  ligerMenu的参数一样  已设默认值
        top: 100,
        left: 100,
        width: 150,
        items: menuData
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [10, 20, 30, 40, 50, 100, 200, 500], //可选择设定的每页结果数
        parms: [], //提交到服务器的参数
        columns: [], //数据源
        ...
        unSelectedRow: [], //复选框不选择行数
        checkBoxDisplay: null, //  复选框是否出现函数
        ...
        isAddRow: true, //是否允许grid增加行
        isApplyEditorWhenChangeRow:true,  //  在键盘回车换行时添加行是否有编辑状态  
        isSaveColumn: true,   //是否显示保存查询列
        editingWhenAddingRow: false,  //在添加行时最近编辑的单元格是否处于编辑状态 
        onDragCol: null, //拖动列事件
        onToggleCol: null, //切换列事件
        onChangeSort: null, //改变排序事件
        onSuccess: null, //成功获取服务器数据的事件
        onDblClickRow: null, //双击行事件
        onClickRow: null, //单击行事件（表格不可编辑时[enabledEdit : false]可用）
        onGridClick: null, //   任意单击事件
        ...
        onContextmenu: function (parm, e) {
        	//合并右键菜单列
            if (JSON.stringify(this.options.contextmenuEidtor .items) !== '[]') {
                this.contextmenu.show({ top: e.pageY, left: e.pageX });
                this.contextmenu.hideAllSubMenu();
                _self = this;
            };
            
            //给右键菜单列增加点击事件
            if (this.contextmenu.options.items.length > 0) {
	        	$(this.contextmenu.options.items).each(function (i, item) {
	        		 $('#l-menu-'+item.id).unbind('click');
        			$(document).off('click', '#l-menu-'+item.id).on('click', '#l-menu-'+item.id, function(){
        				item.click(parm.data,e)
    				})
	        	});
            };
            return false;
        },                      //右击事件(默认)
        contextmenuEidtor: contextmenuEidtorObj,    //增加右击事件的列
        /*contextmenuEidtor:{     //增加右键菜单列的写法(line:true表示加分割线)
	        items:[
                {id:'add',text: "增加", icon: "add",line:true, click: addItem},
                {id:'search',text: "查询", icon: "search",click: addItem}
            ]                   
       },*/
        ...
        notCellEditByColName: null, // 是否通过列名条件来判断单元格编辑  值为列名
        onChangeRow: function () { },  // 键盘控制换行事件
        onKeyDownWhenEnter:function () {}, // 键盘回车时事件 返回值false时 停留在当前单元格
        lodop: {         //lodop对象	
            group: null,//grid分组//{name:"subj_name",display:"科目"}
            title: null,//打印内容的标题
            head: null,//打印表头
            foot: null,//表尾user:parent.sessionJson.user_name,date:true
            type: 3,//1：打印设计，2：打印维护，3：打印预览，4：直接打印
            isFastExport: true,//true、false，快速生成（无表格样式,数据量较大时或许用到)
            percent: "Auto-Width",//百分比,Full-Width|Full-Height|Full-Page|Auto-Width|Auto-Height|Width:200%|Height:200%|Width:200%;Height:200%|55%
            fn: {}
        }
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function () {
            ...
            gridhtmlarr.push("        <div class='l-panel-topbar' style='display:none'><div class='l-panel-topbarinner'></div><div class='l-tab-links-left' style='display:none' ><span></span></div><div class='l-tab-links-right' style='display:none'><span></span></div></div><div class='l-clear'></div>");
            ...
            gridhtmlarr.push("                    <div class='l-grid-popup' style='max-height:150px;background-color:#fff;width:150px'><table cellpadding='0' cellspacing='0'><tbody></tbody></table></div>");
            ...
            //顶部工具栏翻页按钮
            g.topbar_prevBtn = $(".l-tab-links-left", g.grid);

            g.topbar_nextBtn = $(".l-tab-links-right", g.grid);
            ...
            //隐藏保存查询列
            if(p.isSaveColumn == false) {
            	contextmenuEidtorObj = {};
             	contextmenuEidtorObj.items = [];
            }
            
            if(p.contextmenuEidtor.items[0].text =="保存查询列" && p.isSaveColumn == false) {
              	contextmenuEidtorObj = {};
              	contextmenuEidtorObj.items = [];
            	p.contextmenuEidtor = contextmenuEidtorObj;	
            }

            var itemsArr = p.contextmenuEidtor.items,
                opts,
                result, 
                repeat_index;

            if(p.contextmenuEidtor !== contextmenuEidtorObj) {
                opts = $.extend(false,{},contextmenuEidtorObj, p.contextmenuEidtor);   
                for( var i = 0, length = itemsArr.length; i< length; i++){    //  合并items数组属性
                    repeat_index = 0;
                    if(itemsArr[i].line){
                        contextmenuEidtorObj.items.push(itemsArr[i]);
                        continue;
                    }
                    result = contextmenuEidtorObj.items.some(function(item,index, array){
                        repeat_index = index; 
                        return item['text'] == itemsArr[i]['text'];
                    })
                    if(result) {
                        contextmenuEidtorObj.items.splice[repeat_index, 1, itemsArr[i] ]
                    }else {
                        contextmenuEidtorObj.items.push(itemsArr[i]);
                    }
                }
                    opts.items = contextmenuEidtorObj.items;  
                    g.contextmenu = $.ligerMenu(opts);        
                
            }else {
        			g.contextmenu = $.ligerMenu(contextmenuEidtorObj); 
            }
            //处理autoFilter
            if (p.autoFilter) {
                var item = {
            ...
            //根据格式数据 渲染列样式 xjx 2017/5/17
            //===========================================
            var pathName = window.location.pathname.replace(".do", "").replace(/\//g, '[').replace("[CHD-HRP[hrp[", "");
            if (g.options.gid) {
                pathName = pathName + "[" + g.options.gid;
            }
            var columnStyle = JSON.parse(Local.get(pathName));
            if (columnStyle) {
                setTimeout(function () {//找不到渲染结束为止 只能用异步执行逻辑...
                    for (var i = 0; i < columnStyle.length; i++) {
                        var columnInfo = columnStyle[i];
                        var column = g.getColumnByName(columnInfo.name);
                        if(column){
                             g.toggleCol(column, !columnInfo._hide);
                        }
                    }
                }, 120);
            }
        ...
        _setToolbar: function (value) {
            ...
            if (value && $.fn.ligerToolBar) {
                ...
                var ditems = $(g.toolbarManager.element).find(".l-toolbar-item");
                var sumWidth = 0;
                for (var i = 0, length = ditems.length; i < length; i++) {
                    sumWidth += ditems.eq(i).outerWidth() + 10
                }
                if (sumWidth > g.grid.width()) {
                    g.topbar.css({
                        "position": "absolute",
                        "left": "17px"
                    })
                    g.topbar_prevBtn.show();
                    g.topbar_nextBtn.show();
                    g.topbar_prevBtn.hover(function () {
                        $(this).addClass("l-tab-links-left-over");
                    }, function () {
                        $(this).removeClass("l-tab-links-left-over");
                    }).click(function () {
                        g.moveToPrevTopBarItem();
                    });
                    g.topbar_nextBtn.hover(function () {
                        $(this).addClass("l-tab-links-right-over");
                    }, function () {
                        $(this).removeClass("l-tab-links-right-over");
                    }).click(function () {
                        g.moveToNextTopBarItem();
                    });
                };
                $(g.toolbarManager.element).width(sumWidth);
                if (value.title) {
        ...
        //切换到上一个topbaritem
        moveToPrevTopBarItem: function () {
            var g = this,
                p = this.options;
            var dItems = $(g.toolbarManager.element).find(".l-toolbar-item"),
                nextBtn = g.topbar_nextBtn,
                prevBtn = g.topbar_prevBtn;
            ...
            var moveToTopitem = null,
                currentWidth = 0;
            var prevBtnLeft = prevBtnOffset.left + prevBtn.outerWidth();
            for (var i = 0, l = dItems.length; i < l; i++) {
                var Topitem = $(dItems[i]);
                var offset = Topitem.offset();
                var start = offset.left,
                    end = offset.left + Math.round(Topitem.outerWidth()) + 10;
                if (start < prevBtnLeft && end >= prevBtnLeft) {
                    moveToTopitem = Topitem;
                    break;
                }
                currentWidth += Math.round(Topitem.outerWidth()) + 6;
            }
            if (moveToTopitem == null) return false;
            //计算出正确的移动位置
        ...
        moveToNextTopBarItem: function () {
            ...
            var moveToTopBarItem = null,
                currentWidth = 0;
            for (var i = 0, l = dItems.length; i < l; i++) {
                var dItem = $(dItems[i]);
                currentWidth += Math.round(dItem.outerWidth()) + 10;
                var offset = dItem.offset();
                var start = offset.left,
                    end = offset.left + Math.round(dItem.outerWidth()) + 10;
                if (start <= nextBtnOffset.left && end > nextBtnOffset.left) {
                    moveToTopBarItem = dItem;
                    break;
                }
            }
            if (moveToTopBarItem == null) return false;
            //计算出正确的移动位置
            var left = currentWidth - (nextBtnOffset.left - prevBtnOffset.left) + 18;
            g.topbar.animate({
                left: -1 * left
            });
            return true;
        ...
        loadData: function (loadDataParm, sourceType) {
            ...
            if (p.dataAction == "server") {
                ...
                //  判断重新加载时是否由点击查询触发  若是则清空sortOrder,sortName
                if(typeof loadDataParm == "undefined"){   
                    p.sortOrder = null;
                    p.sortName && (p.sortName = "");
                }
            }
            var param = [];
            if (p.parms) {
            ...
            g.loading = false;
            if (g.f) g.f.gridheader.find(".l-grid-hd-row").attr('class', 'l-grid-hd-row');  //  表头checkbox 重置值
        },
        ...
        loadServerData: function (param, clause, sourceType) {
            ...
            var ajaxOptions = {
                ...
                success: function (data) {
                    //当grid请求，返回json对象里面包含error属性时，提示后台程序处理的错误信息
                    ...
                    if (g.trigger('success', [data, g]) == false) {
                        return false;
                    }
                ...
                complete: function () {
                    ...
                    if (g.hasBind('loaded')) {
                        g.trigger('loaded', [g]);
                        g.toggleLoading(false); //自加的语句，若执行loaded事件即隐藏“加载中...."GIF图
                    } else {
        ...
        */
        ...
        addEditRowChd: function (rowdata, containerBulider) {
            rowdata = this.add(rowdata);
            this.beginEdit(rowdata, containerBulider);
        },
        ...
        _enabledEditByCell: function (cell) {
            ...
            var Rowdata = g._getRowByDomId(cell.id);
            if (!!Rowdata.isNotEdit || Rowdata.notEidtColNames.some(function (item, index, array) { return (item == column.columnname) }))
                return false;
         ...
         * xjx重构 2016 11 11
         */
        endEditToNext2: function () {
        //   表格回车事件当单元格里的下拉框有选择值时 回车会回充值 不会立马跳到下一个单元格 再按下回车直接跳过
             if ($('.l-box-select-inner').not(':hidden').length != 0) {          
               if($('.l-box-select-inner').find('td').hasClass('l-over')) {
                   return ;
               }
               if($('.l-box-select-inner').find('tr.l-grid-row').hasClass('l-selected')) {
                   return ;
               }
            } 
            var g = this, //grid
                p = this.options, //grid参数
                editor = g.editor, //grid 编辑参数
                curCell = null,
                curRow = null, //當前行
                curDom = null, //當前dom
                $editCell = null,
            ...
            if (editor && editor.editing) { //编辑状态
                editParm = editor.editParm;
                curCell = g.getCellObj(editParm.record, editParm.column); //当前单元格
                curDom = document.getElementById(curCell.id);
                curRow = $(curDom).parent();
                $editCell = getEditCell($(curCell));

            if( g.trigger('keyDownWhenEnter', [editParm]) == false) return false;               
                //编辑结束验证
                // var validate = true;
                // if (p.onBeforeSubmitEdit) {
                //     validate = p.onBeforeSubmitEdit(editParm);
                // }
                //通过跳转下一个单元格
                if (validate) {
                    g.endEdit(); //结束编辑
                    if ($editCell) { //编辑列进入状态
                        //当前行的单元格进入编辑状态
                        // g._applyEditor($editCell.get(0));  将设置单元格编辑状态全部放到  “改变滚动条函数”里面
                    } else { //下一行第一列进入状态

                        if (typeof p.onChangeRow === "function") {
                            p.onChangeRow(editParm);
                        }

                        if (curRow.nextAll(".l-grid-row").eq(0).length === 0) { //下一行爲空
                            if (!p.isAddRow) return;
                            g.addRow();
                            var onoff = true;
                        }
                         if(!p.isApplyEditorWhenChangeRow) return false; 
                        curDom = document.getElementById(curCell.id); //需要重新獲取
                        curRow = $(curDom).parent();
                        var Rowdata = g._getRowByDomId(curCell.id); //获取当前行的index值，然后通过getNextEditRowObj方法遍历出下面的编辑行
                        var index = Rowdata["__index"];
                        var nextEidtRow = getNextEditRowObj(index);
                        $editCell = $(nextEidtRow).find("td:first");

                        if (!g._enabledEditByCell($editCell.get(0))) { //判断第一个单元格是否可编辑
                            $editCell = getEditCell($editCell);
                        }
                    }
                }

                if (g.enabledDetail) { //收起明細
                    curRow.find(".l-grid-row-cell-detailbtn").removeClass("l-open");
                    g.collapseDetail(curRow);
                }
                autoScrollByEdit($editCell); //自动改变滚动条
            }
             ...
             * return 下一个可编辑单元格
             */
            function getEditCell($cell) {
                var $nextCell = $cell.next();
                //如果下一个单元格length等于0 说明一行结束
                if ($nextCell && $nextCell.length == 0) {
                    return false;
                }
                //判断是否可编辑,是否隐藏
                if (g._enabledEditByCell($nextCell.get(0)) && !$nextCell.is(":hidden")) {
                    return $nextCell;
                } else {
                    return getEditCell($nextCell);
                }
            }

            function getNextEditRowObj(rowdataIndex) { //通过index值获取下一个可编辑的行的节点
                var index = rowdataIndex;
                index++;
                if (index == g.rows.length)
                    return false;
                if (!g.rows[index].isNotEdit)
                    return g.getRowObj(g.rows[index]["__id"]);
                else
                    return getNextEditRowObj(index);
            }
             ...
             *
             */
            function autoScrollByEdit(cell) {
                var scrollWidth = g.gridbody.width();
                var cellLeft = cell.position().left; //单元格距当前打开浏览器left
                var cellWidth = cell.width(); //单元格宽度
                var rowLeft = cell.parent().position().left; //行距当前打开浏览器left
                var left = cellLeft - rowLeft - scrollWidth + cellWidth + 30; //滚动条横向位置
                var top = cell.position().top; //滚动条竖向位置
                if (top >= g.gridview2.height() * .6) //当光标距离底部一段距离时触发滚动条滚动
                {
                    g.gridbody.scrollTop(cell.parent().height() * cell.parent()[0].rowIndex);
                }

                //横向滚动条不滚动
                if (cellLeft + cellWidth < scrollWidth) {
                    //换行
                    if (cellLeft < 0) {
                        g.gridbody.scrollLeft(0);
                    }
                    setTimeout(function () { //异步添加编辑
                        g._applyEditor(cell.get(0));
                    }, 50);
                    return;
                }
                g.gridbody.animate({
        ...
        getEditCell: function ($cell) { //获取编辑单元格
            var g = this,
                p = this.options;
            var $nextCell = $cell.next();
            //如果下一个单元格length等于0 说明一行结束
            ...
            //判断是否可编辑
            if (g._enabledEditByCell($nextCell.get(0))) {
                return $nextCell;
            } else {
                return g.getEditCell($nextCell);
            }
        },
        /**
         * 从当前跳到目标单元格
        ...
        endEditToCell: function (target) {
            ...
            if (editor && editor.editing) {
                ...
                curDom = document.getElementById(curCell.id);
                curRow = $(curDom).parent();
                $editCell = getCellByTarget(target, $(curCell));
            ...
            //获取指定单元格
            function getCellByTarget(target, $cell) {
                var $nextCell;
                var $curRow = $cell.parent();
                var index = $curRow.find("td").index($cell);
                var nextRow;
                var $editorInput = editor.container.find('input');
                var selectionIndex = $editorInput[0].selectionStart;


                switch (target) {
                    case 'right':
                        if (selectionIndex !== $editorInput.val().length) { //如果光标没在最后一个 则不跳转
                            return false;
                        }

                        $nextCell = $cell.next();

                        if ($nextCell && $nextCell.length == 0) { //如果下一个单元格length等于0 说明一行结束
                            return false;
                        }
                        //判断是否可编辑
                        if (g._enabledEditByCell($nextCell.get(0)) && !$nextCell.is(":hidden")) {
                            ...
                            return getCellByTarget('right', $nextCell);
                        };

                    case 'top':
                        // $nextRow = $curRow.prevAll(".l-grid-row").eq(0);
                        // $nextCell = $nextRow.find("td").eq(index);
                        // $curRow.parent()
                        $nextRow = $(getPrevEditRowObj($curRow.get(0).rowIndex));
                        $nextCell = $nextRow.find("td").eq(index);
                        if ($nextRow.length === 0) { //上一行爲空
                            return false;
                        }
                        if (typeof p.onChangeRow === "function") {
                            p.onChangeRow(editParm);
                    ...
                    case 'left':
                        ...
                        if (g._enabledEditByCell($nextCell.get(0)) && !$nextCell.is(":hidden")) {
                            ...
                            return getCellByTarget('left', $nextCell);
                        };

                    case 'bottom':
                        $nextRow = $(getNextEditRowObj($curRow.get(0).rowIndex));
                        $nextCell = $nextRow.find("td").eq(index);
                        // $nextRow = $curRow.nextAll(".l-grid-row").eq(0);
                        // $nextCell = $nextRow.find("td").eq(index);

                        if ($nextRow.length === 0) { //下一行爲空
                            return false;
                        }
                        if (typeof p.onChangeRow === "function") {
                            p.onChangeRow(editParm);
            ...
            function getPrevEditRowObj(rowdataIndex) {
                var index = rowdataIndex; //通过index值获取上一个可编辑的行的节点
                index--;
                if (index < 0) {
                    return false;
                }
                if (!g.rows[index].isNotEdit) {
                    ...
                    return getPrevEditRowObj(index);
                }
            }
            ...
            function autoScrollByEdit(cell) {
                var scrollWidth = g.gridbody.width();
                var scrollHeight = g.gridbody.height();
                var cellLeft = cell.position().left; //单元格距当前打开浏览器left
                var cellWidth = cell.width(); //单元格宽度
                ...
                var top = cell.position().top; //滚动条竖向位置
                /**cjc
                 * 财务工资录入 不想上下左右滚动条置顶的效果
                 * */
                if (top + cellHeight > scrollHeight || top < 0) //判断上下距离来控制滚动条
                {
                    g.gridbody.scrollTop(cell.parent().height() * cell.parent()[0].rowIndex);
                }
                
                //横向滚动条不滚动
                if (cellLeft + cellWidth < scrollWidth && cellLeft > 10) { // && cellLeft > 10  为负数时  不换页滚动条滚动 (cjc)
                    //换行
                    if (cellLeft < 0) {
                        g.gridbody.scrollLeft(0);
                    }
        ...
        endEditVouchToNext: function () {
            ...
            if (editor) {
                var editParm = editor.editParm;
                // if(editor.editParm==undefined)return;
                var column = editParm.column;
                var columnIndex = $.inArray(column, g.columns);
                var cell = g.getCellObj(editParm.record, editParm.column);
                jprev = $(cell);
                ...
                if (!jnext.length) {
                    ...
                    columnIndex = $.inArray(column, g.columns);
                    cell = g.getCellObj(editParm.record, editParm.column);
                    jprev = $(cell);
                    //  jnext = jprev.next();
                    jnext = getNextRowCell();
                }
                /*   if (jnext.length)
                   {
                       ...
                       while (!g._enabledEditByCell(jnext.get(0)))
                       {
                           ...
                           if (!jnext.length)//已经是当前行最后一个单元格了
                           {
                               ...
                               if (!jnext.length) {
                                     g.addRow();
                                     editParm =g.editor.editor.editParm;
                                     column = editParm.column;
                                     columnIndex = $.inArray(column, g.columns);
                                     cell =g.getCellObj(editParm.record, editParm.column);
                                     jprev =$(cell);
        ...
        endEditToMatNext: function () {
            ...
            if (editor.editing) {
                ...
                if (!jnext.length) {
                    ...
                    //横向光标移动到最左侧
                    //g.gridbody.scrollLeft(0);
                    var editParm = g.editor.editParm;
                    var column = editParm.column;
                    var columnIndex = $.inArray(column, g.columns);
                    cell = g.getCellObj(editParm.record, editParm.column);
                    jprev = $(cell);
                ...
                //gird宽度
                var gridWidth = g.gridbody.width();
                //下一个编辑框距左侧距离
                var scrollWidth = 0;
                for (var i = 0; i < g.columns.length; i++) {
                    //获取单元格属性
                    var col = g.getColumn(i);
                    //alert(JSON.stringify(g.getColumn(col)));
                    if (columnIndex == g.columns.length - 1) {
                        ...
                        ////隐藏列不计算宽度
                        if (!col._hide) {
                            scrollWidth = scrollWidth + col._width;
                            if (i > columnIndex && (col.editor != null && col.editor != 'undefined' && col.editor != '')) {
                                break;
                            }
                        }
                    }
                }
                //如果编辑框到左侧的宽度大于表格宽度则光标位移
                if (scrollWidth > gridWidth) {
                    g.gridbody.scrollLeft(scrollWidth - gridWidth);
                } else {
                    g.gridbody.scrollLeft(0);
                }
                //获取下一行第一个列对象
            ...
            if (editor) {
                ...
                for (var i = 0; i < g.columns.length; i++) {
                    ...
                    if (columnIndex == g.columns.length - 1) {
                        ...
                        scrollWidth = scrollWidth + g._getColumnWidth(column);
                        if (i > columnIndex && (column.editor != null && column.editor != 'undefined' && column.editor != '')) {
                            break;
                        }
                    }
        ...
        endEdit: function (rowParm) {
            ...
            if (e && e.which == 1) {
                var editParm = g.editor.editParm;
                if (editParm) {
                    var curCell = g.getCellObj(editParm.record, editParm.column); //当前单元格
                    // if(curCell)
                    //  {
                    var curDom = document.getElementById(curCell.id);
                    var curRow = $(curDom).parent();

                    if (curRow.nextAll(".l-grid-row").eq(0).length === 0 && p.isAddRow) {
                        setTimeout(function () {
                            g.addRow();
                            if (g.gridview2.height() <= g.gridbody.find(".l-grid-body-inner:first").height()) { //当添加一行处于最底部时,滚动条滚动到最底部

                                g.gridbody.scrollTop(g.gridbody.find(".l-grid-body-inner:first").height());
                            }
                        }, 100)
                    }
                    // }
            ...
            if (g.editor.editing) {
                ...
                /**** */
                g.lastCellObj = g.getCellObj(g.editor.editParm.record, g.editor.editParm.column);
                /******** */
                g.editor = {
                    editing: false
                };
        ...
        deleteSelectedRow: function () {
            ...
            this.reRender.ligerDefer(this, 20);
						...
						//编辑表格如果所有行则需新增一个空行用于编辑
						if(this.options.enabledEdit && this.options.isAddRow){
	            var that = this
	            setTimeout(function () {
	                var len = that.getData().length;
	                if (len === 0) {
	                    that.addRow();
	                }
	            }, 50)
        ...
        deleteSelectedRow2: function () { // 删除选择行后不新增行
            if (!this.selected) return;
            for (var i in this.selected) {
                var o = this.selected[i];
                if (o['__id'] in this.records)
        ...
        deleteAllRows: function () {
            var g = this,
                p = this.options;
            if (g.rows && JSON.stringify(g.rows) != '[]') {
                for (var i = 0, l = g.rows.length; i < l; i++) {
                    var o = g.getRow(i);
                    if (o['__id'] in g.records)
                        g._deleteData.ligerDefer(g, 10, [o]);
                }
            }
        ...
        */
        updateCell: function (arg, value, rowParm) {
            ...
            if (value != null && column.name) {
                ...
                if(g.editor.editParm && value != g.editor.editParm['oldvalue']) {  //  在编辑前事件传入的参数中增加oldvalue，储存之前的值
                         g.isDataChanged = true;
                }else if(g.editor.editParm && value == g.editor.editParm['oldvalue']){
        ...
        addRows: function (rowdataArr, neardata, isBefore, parentRowData) {
            ...
            g.trigger('afterAddRows', rowdataArr);
        },
        ...
        _addNewRecord: function (o, previd, pid) {
            var g = this,
                p = this.options;
            g.recordNumber++;
            o['notEidtColNames'] = [];    //   自建的属性  向里面传入列名 即根据里面列名判断单元格是否编辑
        ...
        addRow: function (rowdata, neardata, isBefore, parentRowData) {
            ...
            /****** */
            //在编辑后给单元格赋编辑状态   lastCellObj是储存单元格 editorStatus_onoff是编辑状态开关与单击和键盘互锁
            if (p.editingWhenAddingRow && g.lastCellObj && g.editorStatus_onoff) {
                g.editorStatus_onoff = false;
                setTimeout(function () {
                    g._applyEditor(g.lastCellObj)
                }, 0)
            }
            /******* */
            return rowdata;
        },
        addRowEdited: function (rowdata,column) { // 增加带编辑的行  此函数若不是用在初始化表格时建议不要加参数
            var g = this,
                p = this.options,
                 curCell;
            if (!!rowdata) {
            ...
            setTimeout(function () { // 因为初始化表格时要等结构加载完，所以加延时
                var rowindex = g.rows.length - 1;
                var colname = g.getColumn(2).columnname;
                if (g.rows[rowindex][colname])
                    g.addRow();
                rowindex = g.rows.length - 1; //确定添加行的index值
                var col = g.columns[1]; //从第二列开始查询编辑框
                if (g.options.checkbox) //若有复选框从第三列开始
                {
                    col = g.columns[2];
                }
                if(column) {
                	var col2 = g.getColumnByName(column);
                	curCell = g.getCellObj(rowindex, col2);
                }else{
                	
                	curCell = g.getCellObj(rowindex, col);
                }
                var $nextcell = g.getEditCell($(curCell));
                var nextcell = $nextcell.get(0);
                if (g._enabledEditByCell(curCell)) //判断单元格是否能编辑
                    setTimeout(function () {
                        g._applyEditor(curCell); //赋与编辑状态
                    }, 50)
                else {
                    setTimeout(function () {
                        g._applyEditor(nextcell);
                    }, 50)
                }
                if (g.gridview2.height() <= g.gridbody.find(".l-grid-body-inner:first").height()) //当添加一行处于最底部时,滚动条滚动到最底部
                    g.gridbody.scrollTop(g.gridbody.find(".l-grid-body-inner:first").height());
        ...
        getPrintColumns:function () {
            ...
            function _filterCols(cols) {
                var result = cols.map(function(item,index,array){
                    item = $.extend(true,{},item);
                    item.colSpan = item.__colSpan;
                    item.rowSpan = item.__rowSpan; 
                    item.level = item.__level;
                    item.hide = item._hide;
                    item.columnindex = colIndx;

                    maxLevel = maxLevel < item.level ? item.level : maxLevel;
                    //过滤掉不需要展示的列
                    if(item.isrownumber || item.ischeckbox || item.hide || (!item.columns && item.name == undefined) ) {
                        return false;
                    }
                    if(item.columns && item.columns.length) {
                        item.columns = _filterCols(item.columns);
                    ...
                    // 必要属性集合
                    var fKeys = ['name','display','type','align','width','colSpan','formatter','rowSpan','columnindex','level','columns','reg'];
                    for(var key in item) { // 删除其他不必要的属性
                        ...
                        } else if(!item[key] && (key == 'rowSpan' || key == 'colSpan')){  // 删除rowSpan、colSpan属性值为1
                            delete item[key];
                        }
                    }
        ...
        changeSort: function (columnName, sortOrder) {
            ...
            if (p.dataAction == "server" && g.rows.length) {   // 判断表格是否有数据 
                g.loadData(p.where);
            }
        },
        //改变分页
        changePage: function (ctype) {
            var g = this,
                p = this.options;
            if (g.loading) return true;
            if (p.enabledEdit && p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged))
                return false;
        ...
        selectedDataRender: function (selectArr, oldPageSize, newPageSize, pageNum) {
            var g = this,
                p = this.options;
        ...
        changePages: function (newpage) { // 根据输入页数改变页面
            var g = this,
                p = this.options;
            $('.pcontrol input', g.toolbar).val(newpage);
            g.changePage("input");
        },
        ...
        getSelectedRows: function () {
            ...
            for (var i in this.selected) {
                var o = this.selected[i];
                if (typeof this.options.checkBoxDisplay === 'function' && this.options.checkBoxDisplay(o) == false) {
                    continue;
                }
                if (o['__id'] in this.records)
                    arr.push(o);
        ...
        getRowObj: function (rowParm, frozen) {
            ...
            if (typeof (rowParm) == "string") {
                if (g._isRowId(rowParm))
                    ...
                    var result = g.rows[parseInt(rowParm)] ? document.getElementById(g.id + (frozen ? "|1|" : "|2|") + g.rows[parseInt(rowParm)]['__id']) : null;
                    return result;
                }
        ...
        _createHeaderCell: function (column) {
            ...
            /*if (!column['__rowSpan'] && cellHeight > 10) {   //  影响表头文本垂直居中  若解释则文本偏上
        ...
        _initFootbar: function () {
            ...
            if (p.usePager) {
                ...
                $('.l-bar-selectpagesize select', g.toolbar).keydown(function (e) {
                        ...
                        case 33://pageup
                            {
                                ...
                                g.detailPageChange("up"); //   控制子表格翻页
                                break;
                            }
                        case 34://pagedown
                            {

                                g.detailPageChange("down");
                                break;
                            }
                    }
                ...
                if (p.switchPageSizeApplyComboBox && $.fn.ligerComboBox) {
                    $(".l-bar-selectpagesize select", g.toolbar).ligerComboBox({
                        ...
                        onBeforeSelect: function () {
                            if (p.enabledEdit && p.url && g.isDataChanged && !confirm(p.isContinueByDataChanged))
                                return false;
                            return true;
                        },
                        width: 45
        ...
        _showData: function (sourceType) {
            ...
            if (p.mouseoverRowCssClass) {
                for (var i in g.rows) {
                    ...
                    (function (i) {
                        rowobj.bind('mouseenter.gridrow', function () {
                            if (p.onInRow) {
                                p.onInRow(g.rows[i]);
                            }
                        }).bind('mouseleave.gridrow', function () {
                            if (p.onOutRow) {
                                p.onOutRow(g.rows[i]);
                            }
                        })
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                if (!!item.isNotEdit) {
                    gridhtmlarr.push(' l-grid-row-notedit'); //添加不编辑样式
                };
                //自定义css class
                if (p.rowClsRender) {
                ...
                $(g.columns).each(function (columnindex, column) {
                    ...
                    if (this.ischeckbox) {
                        ...
                        if ($.inArray(i + 1, p.unSelectedRow) != -1 || item.checkboxstatus == 1 || typeof p.checkBoxDisplay === 'function' && p.checkBoxDisplay(item) == false) gridhtmlarr.push('<span class="l-grid-row-cell-btn-checkbox cbdisabled"></span>');
                        else gridhtmlarr.push('<span class="l-grid-row-cell-btn-checkbox"></span>');
                    ...
                    if (item.__status != "add" && column.columnname == p.notCellEditByColName && item[p.notCellEditByColName]) gridhtmlarr.push(' l-grid-row-notedit '); // 不编辑列的样式
                    ...
                    //  //   通过notEidtColNames属性数组是否有相对应的列名，有则加至灰的样式
                    // if(item.notEidtColNames && item.notEidtColNames.some(function(item, index, array){ return (item == column.columnname)})){
                    //     var find_index = gridhtmlarr.lastIndexOf(' class="l-grid-row-cell ');
                    //      gridhtmlarr[find_index] = 'class=" l-grid-row-cell l-grid-row-notedit ';
                    // }
                    gridhtmlarr.push('</td>');
                });
                gridhtmlarr.push('</tr>');
        ...
        _getCellHtml: function (rowdata, column) {
            ...
            var htmlarr = [];
            var content = g._getCellContent(rowdata, column).toString();
            var rel = /<\/?[^>]*>/g;
            var title_content = content.replace(rel, '');
            // 若字符串含有>,< 则需再次清除  同时清除字符串间的空格
            title_content = title_content.replace(/<|>|\s/g, '');
            htmlarr.push('<div class="l-grid-row-cell-inner" title=\"' + title_content + '\"');  // title属性
        ...
        _applyEditor: function (obj) {
            ...
            var row = $(rowcell).parent(),
                ...
                rowid = rowdata['__id'],
                rowindex = rowdata['__index'];
            //  判断notEidtColNames数组中是否有值且与当前的column.columnname 相等，若相等及跳出
            if (rowdata.notEidtColNames && rowdata.notEidtColNames.some(function (item, index, array) { return (item == column.columnname) })) return;
            if (rowdata.__status != "add" && column.columnname == p.notCellEditByColName && rowdata[p.notCellEditByColName]) return; // 不编辑列的条件
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
              ...
              currentdata = currentdata == undefined ? '':currentdata;
                ...
                //编辑框自动获取焦点事件
                if (editorInput.element) {
                    editorInput.element.onfocus = function () {
                        //   让日期控件的下拉框在编辑的时候不自动出现
                        if (editorInput.type == "DateEditor" && !editorInput.options.showSelectBox) return;
                        //这里写编辑框获取焦点事件
                        if (editorInput.link) {
                            editorInput.link.click();
                        }
                    }
                }
                g.unbind('sysEndEdit');
        ...
        _getTotalSummaryHtml: function (data, classCssName, frozen) {
            ...
            $(g.columns).each(function (columnindex, column) {
                ...
                if (column._hide) { //如果是隐藏列
                    totalsummaryArr.push('<td style="display:none" class="l-grid-totalsummary-cell');
                } else {
        ...
        _setEvent: function () {
            ...
            g.grid.bind("keydown.grid", function (e) {
                ...
                if (g.editor.editing) {
                    switch (event.keyCode) {
                        case 13: //回车
                            g.endEditToNext2();
                            if (p.editingWhenAddingRow)
                                g.editorStatus_onoff = false;   // 键盘操作时锁住，否则会有编辑框会悬浮在左上角
                            break;
                        case 37: //左
                            g.endEditToCell("left");
                            break;
                        case 38: //上
                            {
                                g.endEditToCell("top");
                        ...
                        case 39: //右
                            g.endEditToCell("right");
                            break;
                        case 40: //下
                            {
                                g.endEditToCell("bottom");
                                break;
                            }
                        case 33://pageup
                            {
            ...
            //表体 - 滚动联动事件
            g.gridbody.bind('scroll.grid', function (e) {
            ...
            $('select', g.toolbar).change(function () {
                if (p.enabledEdit && g.isDataChanged && p.dataAction != "local" && !confirm(p.isContinueByDataChanged))
                    return false;
                p.newPage = 1;
                ...
                g.gridbody.add(g.f.gridbody).ligerDrag({
                    ...
                    onStopDrag: function (current, e) {
                        ...
                        if (g.rowDropIn != -1) {
                            ...
                            //====================
                                for(var i = 0, length = rows.length; i < length; i++) {     //   拖动行时 选中该拖动的行 滚动条滚动到该位置
                                    g.select(rows[i].__index);
                                }
                                g.gridbody.scrollTop(p.rowHeight * rows[0].__index);
                            //===================
                                g.trigger('rowDragDrop', {
                                    rows: rows,
                                    parent: prow,
        ...
        detailPageChange: function (target) {
            var g = this,
                p = this.options;
            if (!g.editor.editing) {   // 判断是否处于编辑 若不编辑则翻页当前grid
                if (target == "up") {
                    $(".l-bar-btnprev span", g.toolbar).click();
                } else {
                    $(".l-bar-btnnext span", g.toolbar).click();
                }
            } else if (g.editor.input.grid) {  //  若编辑 则再判断是否选项框是grid
                if (target == "up") {
                    $(".l-bar-btnprev span", g.editor.input.grid.toolbar).click();
                } else {
                    $(".l-bar-btnnext span", g.editor.input.grid.toolbar).click();
                }
            }
        },
        ...
        _onClick: function (e) {
            ...
            var g = this,
                p = this.options;
            if (p.editingWhenAddingRow)
                g.editorStatus_onoff = false;   //单击时锁住 为了在点击单元格时放开
            var src = g._getSrcElementByEvent(e);
            if (g.hasBind('gridClick')) g.trigger('gridClick', [src]);
            if (src.out) {
            ...
            if (src.checkboxall) //复选框全选
            {
                ...
                for (var rowid in g.records) {
                    if (uncheck)
                        ...
                        if (g.records[rowid].checkboxstatus == 0 || typeof this.options.checkBoxDisplay === 'function' && p.checkBoxDisplay(g.records[rowid]) == false)
                            g.unselect(g.records[rowid]);
                ...
                if (p.unSelectedRow) {
                    ...
                    for (var i in p.unSelectedRow) {
                        g.unselect(g.records["r100" + p.unSelectedRow[i]])
                    };
                }
            } else if (src.hcelltext) //排序
            {
                ...
                if (p.enabledEdit && p.url && p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged)) return;
            ...
            //明细
                ...
                if ($(src.detailbtn).hasClass("l-open")) {
                    ...
                    //收起所有明细
                    if (p.detail.single) {
                        $(g.rows).each(function (i, v) {
                            g.collapseDetail(v);
                        });
                    }
                    var nextrow = p.detail.reload ? "" : row.next("tr.l-grid-detailpanel");
                    if (nextrow.length > 0) {
                        nextrow.show();
            ...
            {
                if($(obj).is('a[href]')) {       //  给点击元素是带有href的a标签添加visited样式 
                    $(obj).css('color','#ce8c9b');
                }
                //表格不可编辑时添加行单击事件
                if (p.onClickRow && !p.enabledEdit) {
                ...
                if (!src.checkbox && src.cell && p.enabledEdit && p.clickToEdit && !src.data.isNotEdit) {
                ...
                if (src.data.checkboxstatus == 0 || typeof this.options.checkBoxDisplay === 'function' && p.checkBoxDisplay(g.records[src.data['__id']]) == false)

                    g.unselect(g.records[src.data['__id']]);

            } else if (src.row && !g.enabledCheckbox() && p.selectable != false) {
                if (src.cell && p.enabledEdit && p.clickToEdit && !src.data.isNotEdit) {
                    g._applyEditor(src.cell);
        ...
        getTotalInfoSum: function (columnName) {
            var g = this,
                p = this.options;
            try {
                var totalsummaryArr = [];
        ...
        _onResize: function () {
            ...
            if (g.enabledFrozen()) {
                ...
                // if ($(".l-grid-body-inner").width() <= $(window).width()-60)
                //  g._f_gridLayout_autoAdapt(gridViewWidth - gridView1Width);//将这行代码拷贝粘贴即可
            }
        ...
        _f_gridLayout_autoAdapt: function (ChangedWidth) {
            ...
            for (var level = 1; level <= g._columnMaxLevel; level++) {
                ...
                jQuery(".l-grid-header2 .l-grid-hd-cell").css({
                    width: columnWidth
                }); //修改标题宽度（排除序号列）
                ...
                if (g.gridview2 != undefined) {
                    ...
                    if (gridHeaderInner != undefined) {
                        jQuery("." + gridHeaderInner.className).css({
                            width: ChangedWidth
                        });
                    }
                }
                    ...
                    if (bodyTable != undefined) {
                        ...
                        if (gridRows != undefined) {
                            ...
                            if (rowCellClassName != null && rowCellClassName != "") {
                                ...
                                jQuery(".l-grid-body2 ." + rowCellClassName).css({
                                    width: columnWidth
                                });
                            }
        ...
        _updateCenterBottom: function (isHeightResize) {
            ...
            if (g.centerBottom) {
                if (isHeightResize) {
                    ...
                    if (g.center.header) {
                        g.center.content.height(g.center.height() - g.center.header.height());    //  调整.l-layout-center的高度  自加的
                    } else g.center.content.height(g.center.height());
                }
...
(function ($) {
    ...
    $.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item, menu) {
            ...
            item.id && ditem.attr("id", "l-menu-"+item.id);
            ...
            ditem.hover(function () {
                ...
                var itemleft = $(this).offset().left + $(this).width() - 5;
                ...
                if (item.children) {
                    ...
                    if (g.menus[ligeruimenutemid]) {
                        g.show({ top: itemtop, left: itemleft }, g.menus[ligeruimenutemid]);
    ...
    $.ligerui.controls.MenuBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item) {
            ...
            if (item.img) {
                ...
                ditem.addClass("l-menubar-item-hasicon");
            }
            else if (item.icon) {
                ...
                ditem.append("<div class='l-menubar-item-color' style='background:" + item.color + "'></div>");
                ditem.addClass("l-menubar-item-hasicon");
            }
...
(function ($) {
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        moveTabItem: function (fromTabItemID, toTabItemID) {
            ...
            var repeat_index = g.tab.links.ul.find(">li").index(from);
            var index2 = g.tab.links.ul.find(">li").index(to);
            if (repeat_index < index2) {
                to.after(from);
            } else {
                to.before(from);
            }
        ...
        addTabItem: function (options) {
            ...
            if (url) {
            	...
            	if(p.is_select){
            		...
            		iframe.attr("name", tabid + "tab").attr("id", tabid);
            	}
            }
            if (content) {
                contentitem.html(content);
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        updateStyle: function () {
            ...
            if (g.inputText.attr('readonly')) {
                ...
                g.inputText.prop('disabled','disabled');
                p.disabled = true;
            } else {
...
(function ($) {
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setItems: function (items) {
            var g = this;
            g.toolBar.html("");
            // g.toolBar.append("<div class='l-tab-links-left'><span></span></div><div class='l-tab-links-right'><span></span></div>")
            $(items).each(function (i, item) {
                if (!!item && !item.hide) {
                    g.addItem(item);
                }
        ...
        setHide: function (itemid) {
            ...
            var nextBar = $("> .l-toolbar-item[toolbarid=" + itemid + "]", g.toolBar).next();
            if(nextBar && nextBar.hasClass("l-bar-separator")){
            	nextBar.hide();
            }
        },
        setShow: function (itemid) {
            var g = this,
                p = this.options;
            $("> .l-toolbar-item[toolbarid=" + itemid + "]", g.toolBar).show();
            var nextBar = $("> .l-toolbar-item[toolbarid=" + itemid + "]", g.toolBar).next();
            if(nextBar && nextBar.hasClass("l-bar-separator")){
            	nextBar.show();
            }
...
(function ($) {
    ...
    /*************下拉框、文本框快捷键Enter支持Tab键功能  ---(cl)*** */
      /*  $('.l-text-field').keydown(function (event) {
         var curInput = $(":focus");
         if (event.keyCode == 13) {
             focusable = $(document).find('input,select,button,a,textarea').filter(':visible').filter('.l-text-field');
             var next = focusable.eq(focusable.index(curInput) + 1);
             if (next.length) {
                 next.focus();
             }
             if (event.shiftKey) {
                 prev = focusable.eq(focusable.index(curInput) - 1);
                 if (prev.length) {
                     prev.focus();
                 }
             }
...
//lodop打印
function lodopGridTable(_self, _rows, type) {

    if (typeof lodopPrint === "function") {
        lodopPrint();
    }

    var mod_code, user_name;
    if (parent.sessionJson) {
        mod_code = parent.sessionJson.mod_code;
        user_name = parent.sessionJson.user_name;
    } else if (parent.parent.sessionJson) {
        mod_code = parent.parent.sessionJson.mod_code;
        user_name = parent.parent.sessionJson.user_name;
    } else if (parent.parent.parent.sessionJson) {
        mod_code = parent.parent.parent.sessionJson.mod_code;
        user_name = parent.parent.parent.sessionJson.user_name;
    }

    //页脚
    var foot = {};
    if (_self.options.lodop.foot) {
        foot = _self.options.lodop.foot;
    } else {
        foot = { user: parent.sessionJson.user_name, date: true };
    }

    //标题
    if (!_self.options.lodop.title && _self.options.title) {
        _self.options.lodop.title = _self.options.title;
    }
    ...
    if (pathName.leng > 230) {
        //PrintSetups_*，文件名最大支持长度260个字符
        alert(pathName + "，路径长度超过230！")
        return;
    }

    //查询用户打印格式
    var columns = _self.getColumns(1);
    ajaxTextByUrl(urlStr + "sys/queryPrintByPageUrl.do?isCheck=false", { mod_code: mod_code, page_url: pathName }, function (responseData) {
        if (responseData.length > 0) {
            columns = JSON.parse(responseData);
        }
    }, false);
    ...
    //分组
    if (_self.options.groupColumnName && _self.options.groupColumnDisplay) {
        _self.options.lodop.group = { name: _self.options.groupColumnName, display: _self.options.groupColumnDisplay };
    }

    lodopTable({
        column: columns,
        data: _rows,
        group: _self.options.lodop.group,//_self分组
        taskName: pathName,
        title: _self.options.lodop.title,
        head: _self.options.lodop.head,
        foot: foot,
        type: type,//1：打印设计，2：打印维护，3：打印预览，4：直接打印
        percent: _self.options.lodop.percent,
        fn: _self.options.lodop.fn
    });
}

//取当前页面模块编码
function getPageModCode() {
    var modCode;
    if (parent.sessionJson) {
        modCode = parent.sessionJson.mod_code;
    } else if (parent.parent.sessionJson) {
        modCode = parent.parent.sessionJson.mod_code;
    } else if (parent.parent.parent.sessionJson) {
        modCode = parent.parent.parent.sessionJson.mod_code;
    }
    return modCode;
}