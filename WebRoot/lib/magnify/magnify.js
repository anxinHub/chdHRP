...
(function (factory) {
    ...
    Magnify.prototype = {
        ...
        creatDOM: function () {
            ...
            var btnsTpl = {
                print: '<button class="magnify-button magnify-button-print" title="' + this.options.i18n.print + '">\
                            <i class="' + this.options.icons.print + '" aria-hidden="true"></i>\
                        </button>',
        ...
        build: function () {
            ...
            this.$print = $magnify.find('.magnify-button-print');

            // 改成轮播图形式
            $('#images-view').append($magnify);
        ...
        print: function (self) {
            if (this.options.print && typeof this.options.print === 'function') {
                this.options.print(self)
            } else {
                return false;
            }
        ...
        setModalSize: function (img) {
            ...
            this.$magnify.css({
                width: minWidth + 'px',
                height: minHeight + 'px',
                /* left: (winWidth - minWidth) / 2 + scrollLeft + 'px',  /* 不要绝对定位 */
        ...
        loadImg: function (imgSrc) {
            ...
            preloadImg(imgSrc, function (img) {
                ...
                // 初始化填满容器
                // 添加初始化最大化参数
                if (self.options.initMaximized && !self.isMaximized) {
                    self.maximize();
                };
            }, function () {
                // loading end
                self.$magnify.find('.magnify-loading').remove();
                // self.addEvent();
                self.$close.off('click').on('click', function (e) {
                    self.close();
                });
        ...
        addEvent: function () {
            ...
            this.$print.off('click').on('click', function () {
                self.print(self);
            });
            ...
            // $W.on('resize', self.resize()); 注释 以免报错 2018/1/17
    ...
    $.fn.magnify = function (options) {
        ...
        if (typeof options === 'string') {
            ...
            jqEl.off('click.magnify').on('click.magnify', function (e) {
                ...
                /* 当只需一个图片展示框 有magnify对象生成时无需再生成一次 */
                if (!$.magnify.item) {
                    $(this).data('magnify', new Magnify(this, options));
                } else {
                    var index = $(this).index();
                    $('.magnify').removeClass('magnify-active');
                    $(this).addClass('magnify-active');
                    $.magnify.item.data('magnify').jumpTo(index);
                }
            });
