...
(function ($) {
	...
	$.fn.smartMenu = function (data, options) {
					...
					$.each(dataMenu, function (i, arr) {
						...
						if ($.isArray(arr)) {
							$.each(arr, function (j, obj) {
								...
								if (text) {
									if(text == "render"){         //当数据中的text值为render时，即为动态渲染   在beforeShow事件中动态赋菜单栏项目值
										if (textChange.length > params.textLimit) {
											strTitle = ' title="' + textChange + '"';
											textChange = textChange.slice(0, params.textLimit) + "…";
											
										}
										if ($.isArray(obj.data) && obj.data.length) {
											htmlMenuLi = '<li class="' + clKey + 'li" data-hover="true">' + htmlCreateMenu(obj.data) +
												'<a href="javascript:" class="' + clKey + 'a"' + strTitle + ' data-key="' + rand + '"><i class="' + clKey + 'triangle"></i>' + textChange + '</a>' +
												'</li>';
										} else {
											htmlMenuLi = '<li class="' + clKey + 'li">' +
												'<a href="javascript:" class="' + clKey + 'a"' + strTitle + ' data-key="' + rand + '">' +  textChange + '</a>' +
												'</li>';
										}
									} else {    
				...
				if (!jqueryMenu.size()) {
					...
					$(idKey + params.name + " a").bind("click", function () {
						...
						if ($.isFunction(callback)) {
							callback.call(D.data("trigger"), event);
						}
						$.smartMenu.remove();
						return false;
					});
		...
		$(this).each(function () {
			...
			windowsObj=this;
			if(options.windowsObj=="frameObj"){
				windowsObj=frameObj;
			}
			
			
			windowsObj.oncontextmenu  = function(e){ 
				e = e || window.event;
				
				if((typeof (frameObj) != 'undefined' && typeof (frameObj.grid) != 'undefined' && frameObj.grid.isEditing)
						|| (typeof (grid) != 'undefined') &&  (typeof (grid.isEditing) != 'undefined') && grid.isEditing){
					
					return true;
				}else{
					//回调
					if ($.isFunction(params.beforeShow)) {
					textChange = params.beforeShow.call(this, e, textChange);
					}
					//阻止冒泡
					e.cancelBubble = true;
					...
					$.smartMenu.remove();
					var st = D.scrollTop();
					var stleft = D.scrollLeft();
					var jqueryMenu = funSmartMenu(e);
					if (jqueryMenu) {
						jqueryMenu.css({
							display: "block",
							left: e.clientX + stleft + params.offsetX,
							top: e.clientY + st + params.offsetY+options.topsetX
						});
						D.data("target", jqueryMenu);
						D.data("trigger", this);
						//回调
						if ($.isFunction(params.afterShow)) {
							params.afterShow.call(this,e);
						}
						return false;
					}
				}
		...
		/*if (B.data("bind")) {    // 键盘触发或其他地方点击触发其消失
			B.bind("click", $.smartMenu.hide).bind("keydown", $.smartMenu.hide).data("bind", true);
		}*/
		windowsObj=window;
		if(options.windowsObj=="frameObj"){
			//凭证主页面
		...
		windowsObj.onclick = function (){
			$.smartMenu.hide();
		}
		windowsObj.onkeydown = function (){
			$.smartMenu.hide();
		}
	
	};
	
	$.extend($.smartMenu, {               //     扩展方法
		hide: function () {
