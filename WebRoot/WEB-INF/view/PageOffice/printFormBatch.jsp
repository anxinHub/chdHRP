...
<script type="text/javascript">	
	...
	function loadHead(){
		grid = $("#maingrid").ligerGrid({
			columns: [ { 
				display: 'ID', name: 'print_id', align: 'left', width: '90', hide: true
			}, { 
				...
				render: function(rowdata, rowindex, value){
					return "<a href='#' onclick='showFile(\""+rowdata.print_id+"\")'>预览</a>"; 
				}
			}, { 
	...
	function showFile(p_id){
		...
		if(files_name[p_id]){
			...
			paraStr = "group_id=" + retMap.group_id + "&hos_id=" + retMap.hos_id 
				+ "&hos_copy=" + retMap.hos_copy + "&temp_file_name=" + files_name[p_id];
			
			$("#myOffice").attr("src", "printFormBatchPre.do?isCheck=false&" + paraStr);
			
		}else{
			//请求数据
			var resMsg = ajaxJsonObjectByUrl("printFormDataBatch.do?isCheck=false", param, function (responseData) {
				//记录文件地址
				files_name[p_id] = responseData.toFilePath;
		...
		if(is_first){
			var data = grid.getData();
		   	$.each(data, function(i, obj){
		   		print_ids[i] = obj.print_id;
		   		row_ids[obj.print_id] = i;
		   	});
		}
		//开启轮循任务30秒执行一次 
	...
	function printData(){
		...
		if(!is_print){
			if(print_ids.length > 0){
				is_print = true;
				row_id = row_ids[print_ids[0]];
				grid.updateCell("print_state", "打印中", row_id);
				
				if(files_name[print_ids[0]]){
					//打印数据
					fileName = files_name[print_ids[0]];
					openFile();
				}else{
					//请求数据
					var param = retMap;
					param.print_id = print_ids[0]; 
					
					var resMsg = ajaxJsonObjectByUrl("printFormDataBatch.do?isCheck=false", param, function (res) {
						//记录文件地址
						files_name[print_ids[0]] = res.toFilePath;
