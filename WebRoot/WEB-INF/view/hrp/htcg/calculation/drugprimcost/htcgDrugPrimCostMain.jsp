...
<script type="text/javascript">
	...
	function init() {
		...
		ajaxJsonObjectByUrl("initHtcgDrugPrimCost.do?isCheck=false", formPara,
	...
	function updateBatchMarkupPercent() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定批量填充加成率?',function(yes) {
		       if (yes) {ajaxJsonObjectByUrl("updateBatchMarkupPercentDrugPrimCost.do?isCheck=false",{
	...
	function calculate_cost() {
		...
		ajaxJsonObjectByUrl("calHtcgDrugPrimCost.do?isCheck=false",
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgDrugPrimCost.do", {
						ParamVo :  ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
