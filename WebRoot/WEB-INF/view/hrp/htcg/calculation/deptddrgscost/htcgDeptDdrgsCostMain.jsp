...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcgDeptDdrgsCost.do',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : true,
			selectRowButtonOnly : true,
			delayLoad : true, //heightDiff: -10,
			toolbar : {
				items : [ {
					text : '查询',
					id : 'search',
	...
	function init() {
		...
		ajaxJsonObjectByUrl("initHtcgDeptDdrgsCost.do?isCheck=false", formPara,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgDeptDdrgsCost.do", {
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit">
         <tr>
			...
			<td align="left" class="l-table-edit-td"><input class="Wdate"
				style="width: 160px;" name="acc_year" type="text" id="acc_year"
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td"><input name=period_code
				type="text" id="period_code" ltype="text"
				validate="{required:true,maxlength:20}" /></td>
			<td align="left"></td>
		</tr>
	</table>
