...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					columns : [{
						...
						render: function(rowdata, rowindex, value) {
							...
							+ rowdata.mate_code + "');\" >" + rowdata.mr_no + "</a>"
						}
					}, {
						display: '住院号',
						name: 'in_hos_no',
					...
					dataType : 'server',
					usePager : true,
					url : 'queryHtcgMaterialAdvice.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function imp() {
		...
		importSpreadView("hrp/htcg/collect/materialAdvice/importHtcgMaterialAdvice.do?isCheck=false", para);

	}
	
	function add_open(){
		$.ligerDialog.open({
			url : 'htcgMaterialAdviceAddPage.do?isCheck=false',
			height : 440,
			width : 470,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveMaterialAdvice();
				},
    ...
    function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgMaterialAdvice.do", {
	...
	function openUpdate(obj) {
		...
		//实际代码中&temp替换主键
		$.ligerDialog.open({
			url : 'htcgMaterialAdviceUpdatePage.do?isCheck=false&' + parm,
			data : {},
			height : 420,
			width : 500,
			title : '修改',
			...
			buttons : [ {
				text : '确定',
				onclick : function(item, dialog) {
					dialog.frame.upateMaterialAdvice();
				},
				cls : 'l-dialog-btn-highlight'
			}, {
				text : '取消',
