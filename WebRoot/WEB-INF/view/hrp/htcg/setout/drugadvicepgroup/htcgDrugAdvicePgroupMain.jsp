...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcgDrugAdvicePgroup.do',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : true,
	...
	function init() {
		...
		ajaxJsonObjectByUrl("initHtcgDrugAdvicePgroup.do?isCheck=false", formPara,
	...
	function calculate() {
		...
		ajaxJsonObjectByUrl("calculateHtcgDrugAdvicePgroup.do?isCheck=false",
	...
	function admittance() {
		...
		ajaxJsonObjectByUrl("admittanceHtcgDrugAdvicePgroup.do?isCheck=false",
				formPara, function(WorkponseData) {
					if (WorkponseData.state == "true") {
						query();
					}
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgDrugAdvicePgroup.do", {
