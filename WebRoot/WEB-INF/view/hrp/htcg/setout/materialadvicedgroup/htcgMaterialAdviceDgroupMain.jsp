...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [ {
				...
				minWidth : 80,
				render : function(rowdata, rowindex, value) {
					if (rowdata.recipe_d_group == 0) {
						return '未准入'
					} else if (rowdata.recipe_d_group == 1) {
						return '已准入'
					}
				}
			} ],
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcgMaterialAdviceDgroup.do',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : true,
	...
	function init() {
		...
		ajaxJsonObjectByUrl("initHtcgHtcgMaterialAdviceDgroup.do?isCheck=false",
	...
	function calculate() {
		...
		ajaxJsonObjectByUrl("calculateHtcgMaterialAdviceDgroup.do?isCheck=false",
				formPara, function(WorkponseData) {
					if (WorkponseData.state == "true") {
						query();
					}
	...
	function admittance() {
		...
		ajaxJsonObjectByUrl("admittanceHtcgMaterialAdviceDgroup.do?isCheck=false",
				formPara, function(WorkponseData) {
					if (WorkponseData.state == "true") {
						query();
					}
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgMaterialAdviceDgroup.do", {
...
<body style="padding: 0px; overflow: hidden;">
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit">
        ...
        <tr>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">材料项目：</td>
            <td align="left" class="l-table-edit-td"><input name="mate_name" style="width:160px;" type="text" id="mate_name" ltype="text" validate="{required:true,maxlength:20}" /></td>
            <td align="left"></td>
        </tr> 
	</table>
	<div id="maingrid"></div>
