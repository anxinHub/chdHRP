...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcgMedicalAdviceDmerge.do',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : true,
	...
	function init() {
		...
		ajaxJsonObjectByUrl("initHtcgMedicalAdviceDmerge.do?isCheck=false",
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgMedicalAdviceDmerge.do",
							{
