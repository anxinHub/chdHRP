...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcgMedicalAdviceDgroup.do',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : true,
	...
	function init() {
		...
		ajaxJsonObjectByUrl("initHtcgMedicalAdviceDgroup.do?isCheck=false",
	...
	function calculate() {
		...
		ajaxJsonObjectByUrl("calculateHtcgMedicalAdviceDgroup.do?isCheck=false",
				formPara, function(WorkponseData) {
					if (WorkponseData.state == "true") {
						query();
					}
	...
	function admittance() {
		...
		ajaxJsonObjectByUrl("admittanceHtcgMedicalAdviceDgroup.do?isCheck=false",
				formPara, function(WorkponseData) {
					if (WorkponseData.state == "true") {
						query();
					}
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgMedicalAdviceDgroup.do",
							{
