...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcgMaterialAdvicePgroup.do',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : true,
	...
	function init() {
		...
		ajaxJsonObjectByUrl("initHtcgMaterialAdvicePgroup.do?isCheck=false",
	...
	function calculate() {
		...
		ajaxJsonObjectByUrl("calculateHtcgMaterialAdvicePgroup.do?isCheck=false",
				formPara, function(WorkponseData) {
					if (WorkponseData.state == "true") {
						query();
					}
	...
	function admittance() {
		...
		ajaxJsonObjectByUrl("admittanceHtcgMaterialAdvicePgroup.do?isCheck=false",
				formPara, function(WorkponseData) {
					if (WorkponseData.state == "true") {
						query();
					}
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgMaterialAdvicePgroup.do", {
