...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcgDrugAdvicePmerge.do',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : true,
	...
	function init() {
		...
		ajaxJsonObjectByUrl("initHtcgDrugAdvicePmerge.do?isCheck=false", formPara,
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgDrugAdvicePmerge.do", {
