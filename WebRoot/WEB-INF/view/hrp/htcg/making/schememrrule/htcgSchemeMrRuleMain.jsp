...
<script type="text/javascript">
	//var mrRuleDataList = mr_rule_dict.Rows;
	var grid;
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [ {
				...
				name : 'mr_rule_code',
				align : 'left',
				textField : 'mr_rule_name',
				editor : {
					type : 'select',
					valueField : 'id',
					textField : 'text',
					url : '../../base/queryHtcgMrRuleDict.do?isCheck=false',
					keySupport : true,
					autocomplete : true,
	...
	function init(){
		...
		ajaxJsonObjectByUrl("initHtcgSchemeMrRule.do?isCheck=false",
			...
			$(data).each(
					...
					if(this.group_id != null || this.hos_id != null || this.copy_code != null)
						ParamVo.push(
								  ...
								  +this.drgs_code + "@"
								  +this.mr_rule_code + "@"
								  +this.mr_rule_count);//实际代码中temp替换主键
					});
			$.ligerDialog.confirm('确定要保存更新?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("saveHtcgSchemeMrRule.do?isCheck=false",{
	   ...
	   var data = gridManager.getCheckedRows();
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcgSchemeMrRule.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
