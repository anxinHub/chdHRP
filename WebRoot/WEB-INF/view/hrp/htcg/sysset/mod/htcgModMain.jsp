...
<script type="text/javascript">
				...
				.ligerGrid(
						{
							columns : [
										...
										render : function(rowdata, rowindex,
											...
											if (value != null) {
												return "<input class='Wdate'"
														+ "name='start_year"
														+ rowindex
														+ "' type='text' id='start_year"
														+ rowindex
														+ "' ltype='text'"
														+ "validate='{required:true,maxlength:20}'"
														+ "onFocus='year();' style='margin-top:5px;' value='"
														+ value
														+ "' disabled='disabled'/>";
											}
											return "<input class='Wdate'"
										...
										render : function(rowdata, rowindex,
											...
											if (value != null) {
												return "<input class='Wdate'"
														+ "name='start_month"
														+ rowindex
														+ "' type='text' id='start_month"
														+ rowindex
														+ "' ltype='text'"
														+ "validate='{required:true,maxlength:20}'"
														+ "onFocus='month();' style='margin-top:5px;' value='"
														+ value
														+ "' disabled='disabled'/>";
											}
									...
									{
										display : '启用人',
										name : 'modStart.create_user',
										align : 'center'
									},
									{
										display : '操作时间',
										name : 'modStart.create_date',
										align : 'center'
									},
									{
										display : '操作',
	...
	function save(mod_code, index) {
		...
		ajaxJsonObjectByUrl("saveHtcgMod.do", formPara, function(responseData) {
