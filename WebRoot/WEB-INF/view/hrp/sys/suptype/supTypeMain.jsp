...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'querySupType.do',
    				...
    				{
						openUpdate(
								rowdata.group_id   + "|" + rowdata.hos_id   + "|" + rowdata.type_code 
							);
    				} 
                   });
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'supTypeAddPage.do?isCheck=false', height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveSupType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteSupType.do",{ParamVo : ParamVo},function (responseData){
    			...
    			case "downTemplate":location.href = "downTemplate.do?isCheck=false" ;
    				 return;
    			case "import":
                   	$.ligerDialog.open({url: 'supTypeImportPage.do?isCheck=false', height: 430,width: 760, isResize:true, title:'导入'});
                   	return;
                case "Excel":
                case "Word":
                case "PDF":
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'supTypeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveSupType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
	...
	function print(){
    	...
    	var colspan_num = grid.getColumns(1).length-1;
   		...
   		ajaxJsonObjectByUrl("querySupType.do?isCheck=false", selPara, function (responseData) {
