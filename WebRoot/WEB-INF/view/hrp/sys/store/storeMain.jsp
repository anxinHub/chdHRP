...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'../storedict/queryStoreDictList.do?isCheck=false',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'storeAddPage.do?isCheck=false', height: 280,width: 640, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveStore(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                /*  
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteStore.do",{ParamVo : ParamVo},function (responseData){
            	...
            	case "import":
                   	$.ligerDialog.open({url: 'sysStoreImportPage.do?isCheck=false', height: 430,width: 760, isResize:true, title:'导入'});
                   	return;
            }   
        }
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'storeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 700, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
    			buttons: [ 
