...
$(function(){
	...
	function plupload(){  
		$("#uploader").pluploadQueue({           
	       	...
	       	url : "readSysStoreFiles.do?isCheck=false",    
	       	chunk_size : '2mb',           
	       	filters : {                
	           mime_types: [  
...
function loadHead() {
	grid = $("#maingrid").ligerGrid(
					{
				columns : [
							 { display: '库房编码', name: 'store_code', align: 'left',editor:{type:'text'}},
							 { display: '库房名称', name: 'store_name', align: 'left',editor:{type:'text'}},
							 { display: '库房分类编码', name: 'type_code', align: 'left',editor:{type:'text'}},
							 { display: '库房分类名称', name: 'type_name', align: 'left',editor:{type:'text'}},
        ...
        switch (item.id)
        {
            case "save":
            	var data = gridManager.getData();
           	 	ajaxJsonObjectByUrl("addBatchSysStore.do?isCheck=false",{ParamVo : JSON.stringify(data)},function (responseData){
