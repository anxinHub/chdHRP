...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	grid.options.parms.push({name:'hos_no',value:$("#hos_no").val()}); 
    	...
    	grid.options.parms.push({name:'hos_simple',value:$("#hos_simple").val()}); 
    ...
    function f_getWhere(){
        ...
        var clause = function (rowdata, rowindex){
                	if($("#hos_no").val()!=""){
                		return rowdata.hos_no.indexOf($("#hos_no").val()) > -1;	
                	}
                	if($("#group_id").val()!=""){
                	...
                	if($("#hos_code").val()!=""){
                		return rowdata.hos_code.indexOf($("#hos_code").val()) > -1;	
                	}
                	if($("#hos_name").val()!=""){
                		return rowdata.hos_name.indexOf($("#hos_name").val()) > -1;	
                	}
                	if($("#hos_simple").val()!=""){
                		return rowdata.hos_simple.indexOf($("#hos_simple").val()) > -1;	
                	}
                	if($("#user_code").val()!=""){
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: 'hos_no', name: 'hos_no', align: 'left'
					 },
                     { display: 'group_id', name: 'group_id', align: 'left'
					 },
                     { display: 'hos_id', name: 'hos_id', align: 'left'
					 },
                     { display: 'hos_code', name: 'hos_code', align: 'left'
					 },
                     { display: 'hos_name', name: 'hos_name', align: 'left'
					 },
                     { display: 'hos_simple', name: 'hos_simple', align: 'left'
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryInfoDict.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'infoDictAddPage.do?isCheck=false', height: 500,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveInfoDict(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteInfoDict.do",{ParamVo : ParamVo},function (responseData){
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'deptIncomeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveInfoDict(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        ...
        <tr>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">hos_no：</td>
            <td align="left" class="l-table-edit-td"><input name="hos_no" type="text" id="hos_no" ltype="text" validate="{required:true,maxlength:20}" /></td>
            <td align="left"></td>
