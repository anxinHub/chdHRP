...
<script type="text/javascript">
    ...
    function f_getWhere(){
        ...
        var clause = function (rowdata, rowindex){
                	...
                	if($("#dept_name").val()!=""){
                		return rowdata.dept_name.indexOf($("#dept_name").val()) > -1;	
                	}
                	if($("#user_code").val()!=""){
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: 'dept_no', name: 'dept_no', align: 'left'
					 },
                     { display: 'group_id', name: 'group_id', align: 'left'
					 },
                     { display: 'hos_id', name: 'hos_id', align: 'left'
					 },
                     { display: 'dept_id', name: 'dept_id', align: 'left'
					 },
                     { display: 'dept_code', name: 'dept_code', align: 'left'
					 },
                     { display: 'dept_name', name: 'dept_name', align: 'left'
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryDeptDict.do?isCheck=false',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'deptDictAddPage.do?isCheck=false', height: 500,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveDeptDict(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteDeptDict.do",{ParamVo : ParamVo},function (responseData){
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'deptIncomeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveDeptDict(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
