...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPermQuery.do',
    ...
    function alertRole(role_id){
    	$.ligerDialog.open({ url : 'roleUserMainPage.do?isCheck=false&role_id='+role_id,data:{}, height: 400,width: 500, title:'查看',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [  { text: '关闭', onclick: function (item, dialog) { dialog.close(); } } ] });
    }
    ...
    function addPerm(){
		...
		var copy_code;var is_copy = obj.split(".")[7];
		
    	if(is_copy == 0){copy_code = '0';}
    	
    	else{copy_code =liger.get("copy_code").getValue();}

    	var acc_year;var is_acc_year = obj.split(".")[8];
    	
    	if(is_acc_year == 0){acc_year = '0';}
    	
    	else{acc_year = $("#acc_year").val()}

    	
    	var param = [];
    	$.each(data,function(i,v){
    		if(tip==1){
				param.push(user_id+"@"+table_code+"@"+v.obj_id+"@"+mod_code+"@"+$("#is_read"+v.obj_id+"").attr("checked")+"@"+$("#is_write"+v.obj_id+"").attr("checked")+"@"+copy_code+"@"+acc_year);
			}else{
				param.push(role_id+"@"+table_code+"@"+v.obj_id+"@"+mod_code+"@"+$("#is_read"+v.obj_id+"").attr("checked")+"@"+$("#is_write"+v.obj_id+"").attr("checked")+"@"+copy_code+"@"+acc_year);
			}
    	});

    	if(tip==1){
			$.post("addUserPermQuery.do?isCheck=false",{ParamVo : param},function(data){
				if(data != null){
			...
			$.post("addRolePermQuery.do?isCheck=false",{ParamVo : param},function(data){
				if(data != null){
					$.ligerDialog.success('保存成功');
					query();
				}
