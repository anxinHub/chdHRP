...
<script type="text/javascript">
    ...
    function  query(){
    	...
    	grid.options.parms.push({name:'target_table_code',value:$("#target_table_code").val()}); 
    	grid.options.parms.push({name:'target_table_name',value:$("#target_table_name").val()}); 
    	grid.options.parms.push({name:'target_column',value:$("#target_column").val()}); 
    	grid.options.parms.push({name:'exec_proc',value:$("#exec_proc").val()}); 
    ...
    function f_getWhere(){
        ...
        var clause = function (rowdata, rowindex){
                	...
                	if($("#target_table_code").val()!=""){
                		return rowdata.target_table_code.indexOf($("#target_table_code").val()) > -1;	
                	}
                	if($("#target_table_name").val()!=""){
                		return rowdata.target_table_name.indexOf($("#target_table_name").val()) > -1;	
                	}
                	if($("#target_column").val()!=""){
                		return rowdata.target_column.indexOf($("#target_column").val()) > -1;	
                	}
                	if($("#exec_proc").val()!=""){
                		return rowdata.exec_proc.indexOf($("#exec_proc").val()) > -1;	
                	}
        };
        return clause; 
    }
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: 'target_table_code', name: 'target_table_code', align: 'left'
					 },
                     { display: 'target_table_name', name: 'target_table_name', align: 'left'
					 },
                     { display: 'target_column', name: 'target_column', align: 'left'
					 },
                     { display: 'exec_proc', name: 'exec_proc', align: 'left'
					 }
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'querySchSet.do',
    				...
    				onDblClickRow : function (rowdata, rowindex, value)
    				{
						openUpdate(
								rowdata.table_code   + "|" + 
								rowdata.target_table_code   + "|" + 
								rowdata.target_column 
							);
    				} 
                   });
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'schSetAddPage.do?isCheck=false', height: 500,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveSchSet(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $(data).each(function (){					
							...
							//表的主键
							this.table_code   +"@"+ 
							this.target_table_code   +"@"+ 
							this.target_column 
							)
                        });
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteSchSet.do",{ParamVo : ParamVo},function (responseData){
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'deptIncomeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveSchSet(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
