...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	grid.options.parms.push({name:'store_no',value:$("#store_no").val()}); 
    	...
    	grid.options.parms.push({name:'store_code',value:$("#store_code").val()}); 
    ...
    function f_getWhere(){
        ...
        var clause = function (rowdata, rowindex){
                	if($("#store_no").val()!=""){
                		return rowdata.store_no.indexOf($("#store_no").val()) > -1;	
                	}
                	if($("#group_id").val()!=""){
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: 'store_no', name: 'store_no', align: 'left'
					 },
                     { display: 'group_id', name: 'group_id', align: 'left'
					 },
                     { display: 'hos_id', name: 'hos_id', align: 'left'
					 },
                     { display: 'store_id', name: 'store_id', align: 'left'
					 },
                     { display: 'store_code', name: 'store_code', align: 'left'
					 },
                     { display: 'store_name', name: 'store_name', align: 'left'
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryStoreDict.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'storeDictAddPage.do?isCheck=false', height: 500,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveStoreDict(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteStoreDict.do",{ParamVo : ParamVo},function (responseData){
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'deptIncomeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveStoreDict(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        ...
        <tr>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">store_no：</td>
            <td align="left" class="l-table-edit-td"><input name="store_no" type="text" id="store_no" ltype="text" validate="{required:true,maxlength:20}" /></td>
            <td align="left"></td>
        ...
        <tr>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">store_id：</td>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">store_code：</td>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">store_name：</td>
