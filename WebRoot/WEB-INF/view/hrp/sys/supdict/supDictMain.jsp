...
<script type="text/javascript">
    ...
    function f_getWhere(){
        ...
        var clause = function (rowdata, rowindex){
                	if($("#sup_no").val()!=""){
                		return rowdata.sup_no.indexOf($("#sup_no").val()) > -1;	
                	}
                	if($("#group_id").val()!=""){
                	...
                	if($("#sup_name").val()!=""){
                		return rowdata.sup_name.indexOf($("#sup_name").val()) > -1;	
                	}
                	if($("#user_code").val()!=""){
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: 'sup_no', name: 'sup_no', align: 'left'
					 },
                     { display: 'group_id', name: 'group_id', align: 'left'
					 },
                     { display: 'hos_id', name: 'hos_id', align: 'left'
					 },
                     { display: 'sup_id', name: 'sup_id', align: 'left'
					 },
                     { display: 'sup_code', name: 'sup_code', align: 'left'
					 },
                     { display: 'sup_name', name: 'sup_name', align: 'left'
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'querySupDict.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'supDictAddPage.do?isCheck=false', height: 500,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveSupDict(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteSupDict.do",{ParamVo : ParamVo},function (responseData){
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'deptIncomeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveSupDict(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        ...
        <tr>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">sup_no：</td>
            <td align="left" class="l-table-edit-td"><input name="sup_no" type="text" id="sup_no" ltype="text" validate="{required:true,maxlength:20}" /></td>
            <td align="left"></td>
        ...
        <tr>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">sup_id：</td>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">sup_code：</td>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">sup_name：</td>
