...
var sourceSoues = [];   
	var save = function() {
		...
		if(!formValidate.test()){
		 ...
		 $(data).each(function () {
             var rowdata = this.rowData;
       		 ...
       		 money1+=parseFloat(rowdata.plan_money);
             param.push(obj);
         });
		 ...
		 $(payGrid.getAllData()).each(function (i,v) {
            	 ...
            	 if(j>=i+1)
       				{
       					...
       					{
       						error+='[收款详情第'+ (i+1) + '行与第'+(j+1)+'行资金来源重复]  ';
       					}
       				}
             });
             if(!rowdata.pay_way){error = "[收款详情第"+ (i+1) + "行支付方式不能为空]  ";return ;}
       		 ...
       		 obj.hos_id = ${hos_id };
       		 obj.copy_code = '${copy_code}';
       		 obj.detail_id = rowdata.plan_detail_id;
       		 obj.rec_money = rowdata.rec_money;
		...
		ajaxPostData({
			url : 'addPactRecMainSKHT.do?isCheck=false',
			data : {
			  ...
			  success: function (result) {
				  pact_code = $("#pact_code").etSelect({
					 ...
					 onChange:function(value, $item){
		            	...
		            	palnGrid.loadData(params,'queryPactPlanSKHT.do?isCheck=false');
			      	 }  
				 });
			  },
		});
    ...
    var initPactPlanGrid = function () {
        var columns = [
        	 { display: '收款期号', name: 'plan_detail_id',hidden:true,editable: false},
        ...
        var paramObj = {
        	...
        	load :function(){
        		 $("#rec_money").val(palnGrid.getTotalSummary("rec_money"));	
        	},
        	/* selectChange:function(){$("#rec_money").val(sum());}, */
        	change:function(){$("#rec_money").val(sum());}
        };
    ...
    var initPayDetailGrid = function () {
        ...
        var paramObj = {
        	...
        	usePager: false,
          	...
          	cellClick: cellClick,
          	editorEnd: editorEnd,
          	addRowByKey:false,selectChange:selectChange,
          	toolbar: {
                items: [
    ...
    var selectChange = function(event, ui){
    	var pay = payGrid.getAllData();
    	$(pay).each(function () {
    ...
    var cellClick = function(event,ui){
    	...
    	if(ui.dataIndx=="pay_name"){
    		payGrid.getColumns()[1].editor.url ='../../../basicset/select/queryPayTypeDictBySourceSelect.do?isCheck=false';//queryPayTypeDictBySourceSelect.do?isCheck=false&source_id='+ui.rowData.source_id;
    	}
    	return true;
    }
	...
	<div>
		<table class="table-layout">
			<tr>
				...
				<td class="ipt"><input id="rec_code" type="text" disabled="disabled" value="系统生成" style="background-color: #eaeaea"/></td>
			...
			<tr>
				...
				<td class="ipt"><input id="rec_money" type="text" disabled="disabled" style="background-color: #eaeaea"/></td>
			</tr>
			<tr>
				...
				<td class="ipt" colspan="3"><input id="note" type="text" style="width: 508px"/></td>
			</tr>
		</table>
	</div>
