...
<script type="text/javascript">
	...
	function query() {
		...
		grid.loadData(params, '../../pactdoc/queryPactFileTypeSKHT.do?isCheck=false')
	}
	...
	function save() {
		...
		ajaxPostData({
			url : '../../pactdoc/saveFileDocSKHT.do?isCheck=false',
			data : {
	 ...
	 function toResource(data) {
			parent.$.etDialog.open({
						url : 'hrp/pac/skht/pactdoc/toPactSKHTSelectDocPage.do?isCheck=false&pact_code=${pact_code }&pact_type_code=${pact_type_code}&file_type=' + data.type_code +
								"&_rowIndx=" + data._rowIndx + "&type_code="+data.type_code,
						width : $(window).width(),
						height : '450px',
						title : '文件选择',
						modal : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.etDialog.confirm('确定删除?', function() {
				ajaxPostData({
					url : '../../pactdoc/deletePactExecSKHTFile.do?isCheck=false',
					data : {
						listVo : JSON.stringify(param)
					},
					success : function() {
	...
	function addFile() {
		parent.$.etDialog
				.open({
					url : 'hrp/pac/skht/pactdoc/toPactSKHTFileManagerPage.do?isCheck=false&pact_code=${pact_code }&pact_type_code=${pact_type_code}',
					width : '450px',
					height : '250px',
					title : '归档管理',
					modal : true,
	...
	function docFileManager() {
		parent.$.etDialog.open({
					url : 'hrp/pac/skht/pactdoc/pactDocSKHTMainPage.do?isCheck=false&pact_code=${pact_code }&pact_type_code=${pact_type_code}',
					width : $(window).width(),
					height : '450px',
					title : '文档管理',
					modal : true,
