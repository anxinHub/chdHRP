    ...
    <script>
        ...
        var initGrid = function () {
            ...
            var paramObj = {
            	...
            	checkbox: true,
                dataModel: {
                	url: '../pactinit/queryPactMainSKHT.do?isCheck=false&is_init=0'
                },
        ...
        var check = function(data){
             ...
             if (data.length == 0) {
                 ...
                 if(param.length == 0){
                   ...
                   ajaxPostData({
                       url: '../pactinit/checkPactMainSKHT.do?isCheck=false',
                       data: {
        ...
        var save = function (data) {
        	 parent.$.etDialog.open({
                 url: 'hrp/pac/skht/pactinfo/pactinit/pactInitSKHTAdd.do?isCheck=false&is_init=0',
                 //width: $(window).width(),
        ...
        var edit = function (data) {
        	...
        	if(s != '01' || s != 1){
        		parent.$.etDialog.open({
                    url: 'hrp/pac/skht/pactinfo/pactexec/pactExecSKHTEdit.do?isCheck=false&pact_code='+data.pact_code,
                    width: $(window).width(),
                    height: $(window).height(),
                    title: '查看',
                    modal: true,
	        	...
	        	parent.$.etDialog.open({
	                 url: 'hrp/pac/skht/pactinfo/pactinit/pactInitSKHTEdit.do?isCheck=false&pact_code='+data.pact_code+'&state='+data.state,
	                 //width: $(window).width(),
	                 //height: $(window).height(),
	                 isMax: true, isResize: true,
	                 title: '修改',
        ...
        var remove = function () {
             ...
             if (data.length == 0) {
                 ...
                 $.etDialog.confirm('确定删除?', function () {
                     ajaxPostData({
                         url: '../pactinit/deletePactMainSKHT.do?isChekc=false',
                         data: {
                        	 mapVo: JSON.stringify(param)
                         },
                         success: function () {
