...
var pact_code;
	 ...
	 var query = function (id) {
         ...
         docGrid.loadData(params,'queryPactDocSKHT.do?isCheck=false');
     };
    ...
    var initSelect=  function(){
       	...
       	pact_type_code = $("#pact_type_code").etSelect({
   			...
   			defaultValue: pact,
   			 onChange:function(value, $item){
   				...
   				pact_code.reload({url: '../../basicset/select/queryPactSKHTSelect.do?isCheck=false&pact_type_code=' + pact_type_code.getValue()})
   			 }, 
   		 });
       	var pact = '${pact_code}';if (!pact) {pact = "none";}
       	pact_code = $("#pact_code").etSelect({
         		url: '../../basicset/select/queryPactSKHTSelect.do?isCheck=false&pact_type_code=' + pact_type_code.getValue(),
         		defaultValue: pact,
         		onChange:function(value, $item){
         			query(value);
	 ...
	 var initPactDocGrid = function () {
         ...
         var paramObj = {
         	...
         	checkbox: true,
         	dataModel: {
	         	url: 'queryPactDocSKHT.do?isCheck=false&pact_code='+$("#pact_code").val(),
             },
         	usePager: false,
             columns: docColumns,
             toolbar: {
	...
	function addDoc() {
		if($("#pact_type_code").val()==""){
			$.etDialog.msg("请选择合同类型");
			return;
		}else if($("#pact_code").val()==""){
			$.etDialog.msg("请选择合同");
			return;
		}
		docGrid.addRow();
	...
	function saveDoc(){
		...
		ajaxPostData({
		 	url: 'addPactDocSKHT.do?isCheck=false',
		 	 data: {
