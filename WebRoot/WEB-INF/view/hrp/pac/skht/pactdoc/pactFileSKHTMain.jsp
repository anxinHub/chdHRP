...
<script type="text/javascript">
	...
	function query(pact_type_code,pact_code) {
		...
		grid.loadData(params, 'queryPactFileTypeSKHT.do?isCheck=false')
	}
   ...
   jQuery.fn.rowspan = function (){
		...
		pact_type = $("#pact_type_code").etSelect({
			...
			defaultValue: "none",
			 onChange:function(value, $item){
				 ...
				 pact.reload({
					 url: '../../basicset/select/queryPactSKHTSelect.do?isCheck=false&pact_type_code=' + pact_type.getValue(),
	    			})
			 }, 
		 });
    	pact = $("#pact_code").etSelect({
      		...
      		onChange:function(value, $item){
      			...
      			if (!pact_type.getValue()) {
	      			if (value == 0) {return;}
	           		ajaxPostData({
	          		 	url: 'queryPactSKHTType.do?isCheck=false',
	...
	function toResource(data) {
		parent.$.etDialog.open({
					url : 'hrp/pac/skht/pactdoc/toPactSKHTSelectDocPage.do?isCheck=false&pact_code='
							+ pact.getValue()
							+ '&pact_type_code='
							+ pact_type.getValue()
							+ "&file_type="
	...
	function save() {
		...
		ajaxPostData({
			url : 'saveFileDocSKHT.do?isCheck=false',
			data : {
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.etDialog.confirm('确定删除?', function() {
				ajaxPostData({
					url : 'deletePactExecSKHTFile.do?isCheck=false',
					data : {
						listVo : JSON.stringify(param)
					},
					success : function() {
	...
	function addFile() {
		...
		if (!pact_type_code) {
			$.etDialog.msg("请选择合同类型");return;
		}
		var pact_code = pact.getValue();
		if (!pact_code) {
			$.etDialog.msg("请选择合同");return;
		}
		parent.$.etDialog
				.open({
					url : 'hrp/pac/skht/pactdoc/toPactSKHTFileManagerPage.do?isCheck=false&pact_code='
						+ pact.getValue()
						+ '&pact_type_code='
						+ pact_type.getValue(),
					width : '450px',
	...
	function docFileManager() {
		...
		parent.$.etDialog.open({
					url : 'hrp/pac/skht/pactdoc/pactDocSKHTMainPage.do?isCheck=false&pact_code='
						+ pact.getValue()
						+ '&pact_type_code='
						+ pact_type.getValue(),
					width : $(window).width(),
