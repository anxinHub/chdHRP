...
<script type="text/javascript">
	...
	function query() {
		...
		grid.loadData(params, '../../pactdoc/queryPactFileTypeFKXY.do?isCheck=false')
	}
	...
	var initPactDocGrid = function() {
		var docColumns = [
				...
				{display : '选择',name : 'select',width : 120,editable : true,
					render : function(data) {
	...
	function save(){
		...
		if(err!=""){$.etDialog.error("第"+err+"行协议文档为空，请先选择协议文档");return;}
		ajaxPostData({
			url : '../../pactdoc/saveFileDocFKXY.do?isCheck=false',
			data : {
	...
	function toResource(data) {
		parent.$.etDialog.open({
			url : 'hrp/pac/fkxy/pactdoc/toPactFKXYSelectDocPage.do?isCheck=false&pact_code=${pact_code}&pact_type_code=${pact_type_code}&file_type='
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.etDialog.confirm('确定删除?', function() {
				ajaxPostData({
					url : '../../pactdoc/deletePactExecFKXYFile.do?isCheck=false',
					data : {
						listVo : JSON.stringify(param)
					},
					success : function() {
	...
	function addFile() {
		parent.$.etDialog
				.open({
					url : 'hrp/pac/fkxy/pactdoc/toPactFKXYFileManagerPage.do?isCheck=false&pact_code=${pact_code}&pact_type_code=${pact_type_code}&pact_name=${pact_name}',
					width : '450px',
					height : '250px',
					title : '归档管理',
					modal : true,
	...
	function docFileManager() {
		parent.$.etDialog
				.open({
					url : 'hrp/pac/fkxy/pactdoc/pactDocFKXYMainPage.do?isCheck=false&pact_code=${pact_code}&pact_type_code=${pact_type_code}&pact_name=${pact_name}',
					width : $(window).width(),
					height : '450px',
					title : '文档管理',
					modal : true,
