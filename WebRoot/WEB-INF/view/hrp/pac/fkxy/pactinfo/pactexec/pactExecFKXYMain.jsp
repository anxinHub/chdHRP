    ...
    <script>
        ...
        var initGrid = function () {
            var columns = [
                 ...
                 { display: '协议档案', name: 'state', align: 'center', width: '80px' ,
                	 render:function(data){
               		  return '<a class="getSouse" rowIndex = "'+data.rowIndx+'">协议档案</a>'
               	  	}	 
                 },
            ];
            var paramObj = {
            	...
            	checkbox: true,
                dataModel: {
                    url: 'queryPactExecFKXY.do?isCheck=false&FKXY_Attr='+"01"
                },
                ...
                toolbar: {
                    items: [
                        ...
                        { type: 'button', label: '归档',   listeners: [{ click: file }],  icon: 'audit', disabled:true },
                        { type: 'button', label: '取消归档',   listeners: [{ click: unfile }],  icon: 'back', disabled:true },
                        { type: 'button', label: '作废',   listeners: [{ click: unuse }],  icon: 'cancel' },
                        { type: 'button', label: '恢复',  listeners: [{ click: recover }],  icon: 'back' },
                        { type: 'button', label: '中止',   listeners: [{ click: stop }],  icon: 'cancal' },
        ...
        function toResource(rowData){
                ...
                url: 'hrp/pac/fkxy/pactinfo/pactexec/toPactExecFKXYFilePage.do?isCheck=false&pact_code='+rowData.pact_code,
                width: $(window).width(),
        ...
        var edit = function (data) {
        	parent.$.etDialog.open({
                 url: 'hrp/pac/fkxy/pactinfo/pactexec/pactExecFKXYEdit.do?isCheck=false&pact_code='+data.pact_code,
                 width: $(window).width(),
                 height: $(window).height(),
                 title: '查看',
                 modal: true,
