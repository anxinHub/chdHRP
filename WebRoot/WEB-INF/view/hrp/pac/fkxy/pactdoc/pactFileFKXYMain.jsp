...
<script type="text/javascript">
	...
	function query(pact_type_code,pact_code) {
		...
		grid.loadData(params, 'queryPactFileTypeFKXY.do?isCheck=false')
	}
		...
		pact_type = $("#pact_type_code").etSelect({
			...
			defaultValue: "none",
			 onChange:function(value, $item){
				...
				pact.reload({url: '../../basicset/select/queryPactFKXYSelect.do?isCheck=false&pact_type_code=' + pact_type.getValue()})
			 }, 
		 });
    	pact = $("#pact_code").etSelect({
      		url: '../../basicset/select/queryPactFKXYSelect.do?isCheck=false&pact_type_code=' + pact_type.getValue(),
      		defaultValue: "none",
      		onChange:function(value, $item){
      			...
      			if (!pact_type.getValue()) {
   	      			if (value == 0) {return;}
   	           		ajaxPostData({
   	          		 	url: 'queryPactFKXYType.do?isCheck=false',
	...
	function toResource(data) {
		parent.$.etDialog.open({
					url : 'hrp/pac/fkxy/pactdoc/toPactFKXYSelectDocPage.do?isCheck=false&pact_code='
							+ pact.getValue()
							+ '&pact_type_code='
							+ pact_type.getValue()
							+ "&file_type="
	...
	function save() {
		...
		ajaxPostData({
			url : 'saveFileDocFKXY.do?isCheck=false',
			data : {
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.etDialog.confirm('确定删除?', function() {
				ajaxPostData({
					url : 'deletePactExecFKXYFile.do?isCheck=false',
					data : {
						listVo : JSON.stringify(param)
					},
					success : function() {
	...
	function addFile() {
		...
		parent.$.etDialog
				.open({
					url : 'hrp/pac/fkxy/pactdoc/toPactFKXYFileManagerPage.do?isCheck=false&pact_code='
						+ pact.getValue()
						+ '&pact_type_code='
						+ pact_type.getValue(),
					width : '450px',
	...
	function docFileManager() {
		...
		parent.$.etDialog.open({
					url : 'hrp/pac/fkxy/pactdoc/pactDocFKXYMainPage.do?isCheck=false&pact_code='
						+ pact.getValue()
						+ '&pact_type_code='
						+ pact_type.getValue(),
					width : $(window).width(),
