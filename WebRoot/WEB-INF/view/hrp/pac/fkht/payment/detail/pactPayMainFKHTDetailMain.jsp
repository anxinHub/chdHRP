    ...
    <script>
      ...
      //查询
        function  query(){
            ...
            grid.options.parms.push({name:'sup_no',value: liger.get("sup_no").getValue()}); 
        ...
        var initSelect=  function(){
            //字典下拉框
            autocomplete("#pact_type_code",'../../../basicset/select/queryPactTypeFKHTSelect.do?isCheck=false',"id","text",true,true,'',false);
            autocomplete("#sup_no",'../../../basicset/select/queryHosSupDictSelect.do?isCheck=false',"id","text",true,true,'',false);
            autocomplete("#dept_no",'../../../basicset/select/queryDeptSelect.do?isCheck=false' ,"id","text",true,true,'',false);
            autocomplete("#pact_nature","../../../basicset/select/queryTypeFKHTNatureSelect.do?isCheck=false" ,"id","text",true,true,'',false);
            $("#pact_type_code").ligerComboBox({ width:180});
        	$("#sup_no").ligerComboBox({ width:180});
        ...
        function loadHead(){
    		grid = $("#mainGrid").ligerGrid({
    			columns: [ 
                    ...
                    { display: '供应商', name: 'sup_name', align: 'left',width:100},
                    ...
                    { display: '应付凭证', name: '', align: 'left',width:100},
                    { display: '计划付款',
                    	columns:[
                    		 { display: '期号', name: 'p_pay_id', align: 'left',width:100},
                    		 { display: '付款日期', name: 'p_pay_date', align: 'left',width:100},
                    		 /* { display: '资金来源', name: 'p_source_name', align: 'left',width:100}, */
                    		 { display: '付款方式', name: 'p_cond_name', align: 'left',width:100},
                    		 { display: '计划金额', name: 'p_plan_money', align: 'left',width:100},
                    		 { display: '实际付款', name: 'p_pay_money', align: 'left',width:100},
                    		 { display: '未付金额', name: 'p_unpay', align: 'left',width:100},
                    	]	
                    },
                    { display: '实际付款',
                    	columns:[
                    		 { display: '付款单号', name: 'r_pay_code', align: 'left',width:100},
                    		 { display: '付款日期', name: 'r_pay_date', align: 'left',width:100},
                    		 { display: '资金来源', name: 'r_source_name', align: 'left',width:100},
                    		 { display: '付款方式', name: 'r_pay_name', align: 'left',width:100},
                    		 { display: '付款金额', name: 'r_pay_money', align: 'left',width:100},
                    		 { display: '付款凭证', name: '', align: 'left',width:100}
                    	]	
                    },
                    
                ],
                dataAction: 'server',dataType: 'server',usePager:false,url:'queryPactPayMainFKHTDetail.do',/* enabledEdit : true, */
        ...
        function onAfterShowData(){
        	 $('#mainGrid .l-grid-body-table tbody').rowspan('pact_code', grid , 6)
        	 $('#mainGrid .l-grid-body-table tbody').rowspan('p_pay_id', grid , 7)
        }
...
<body>
    <table style="margin-bottom: 12px">
        <tr>
            <td class="label" style="padding-left: 20px;padding-top: 20px">签订日期：</td>
           ...
           <td align="left" class="l-table-edit-td" ><input name="end_date" class="Wdate" type="text" id="end_date" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" /></td>
            ...
            <td class="label" style="padding-left: 40px;padding-top: 20px">供应商：</td>
            <td class="ipt" style="padding-top: 20px"> <input id="sup_no" > </td>
        </tr>
        <tr>
            ...
            <td class="label" style="padding-left: 40px;padding-top: 12px">签订科室：</td>
