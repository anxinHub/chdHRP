	...
	var save = function() {
		...
		if(subGrid){
	         ...
	         if (data != null && data.length != 0) {
	              ...
	              $(data).each(function () {
	                  ...
	                  if((typeof(rowdata.dept_name) != "undefined")&&(rowdata.dept_name.indexOf("@") != -1)){ ////Modified By QW20200403
		...
		ajaxPostData({
			...
			data : {
				...
				is_change : is_change.checked ? 1 : 0,
    ...
    var initSelect=  function(){
                 	...
                 	onChange:function(value){
                 		...
                 		ajaxPostData({
                			...
                			success: function (result) {
								...
								signpicker.setValue(result.start_date);
								signpicker.setValue(result.end_date);
     ...
     var initSubGrid = function () {
         var columns = [
				...
				{ display: '生产厂家', name: 'fac_name', width: '120',textField : 'label',
					editor: {
						type: 'select',
						valueField:'id',
						textField : 'text',
				...
				{ display: '金额', name: 'money',width: '120',align: "right",editor: {type: 'text'},
                  	 render:function(rowData){
         ...
         var paramObj = {
             ...
             usePager: false,
             columns: columns,
             url:'queryPactDetFKHT.do?isCheck=false&pact_code='+$("#pact_code").val()+'&change_code='+change_code,
             enabledEdit : true,
     ...
     function loadHeadItem() {
 		gridItem = $("#sourceGrid").ligerGrid({
 			columns : [{ display : '资金来源',	name : 'source_id',width : 100,textField : 'source_name',
	 			...
	 			{ display: '备注', name: 'note',width: '80',editor: { type: 'string' }} ],
 			usePager : false,
  ...
  // 用于动态生成标的物中的金额
 	...
 	function addAssNoDict(){
 	 ...
 	 var initPactPlanGrid = function () {
         ...
         var paramObj = {
         	...
         	usePager: false,
             dataModel: {
	             url: 'queryPactPlanFKHT.do?isCheck=false&pact_code='+$("#pact_code").val()+'&change_code='+change_code,
             },
             columns: columns,
	...
	function toolBarSet(){// 引用定标 引用资产购置申请  按钮 启用/禁用设置
		if(state == 1){
			...
			subGrid.toolbarManager.setDisabled("add");
			subGrid.toolbarManager.setDisabled("delete");
			subGrid.toolbarManager.setDisabled("addElse");
			sourceGrid.toolbarManager.setDisabled("saveItem");
			sourceGrid.toolbarManager.setDisabled("saveItem");
			sourceGrid.toolbarManager.setDisabled("deleteItem");
			subGrid.set({ enabledEdit: false });
			sourceGrid.set({ enabledEdit: false });
		}
...
<body style="overflow: scroll; ">
	<table class="table-layout">
		...
		<tr>
			...
			<td class="ipt"><input id="pact_money_w" type="text" disabled="disabled" value="${entity.pact_money_w }" style="background-color: #EAEAEA" onchange="formatMoney(this.id);"/></td>
		...
		<tr>
			...
			<td class="ipt"><input id="deposit_money" type="text" value="${entity.deposit_money }" disabled="disabled" style="background-color: #EAEAEA" onchange="formatMoney(this.id);"/></td>
		</tr>
		<tr>
