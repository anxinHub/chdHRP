...
var state = '${state}';
	$(function(){
		...
		$("#pactInfoCopy").on("click", function () {
			openPactCopy();
		})
		if(state == 0){
    ...
    var initSelect=  function(){
	 ...
	 var initSubGrid = function () {
		 grid = $("#subGrid").ligerGrid({
             ...
             url:'queryPactChangeFKHTAfterDetUpdate.do?isCheck=false&change_code='+'${change_code}'+'&pact_code='+'${pact_code}',
     ...
     function editorEnd(e){
    	 if(e.column.name == 'change_amount' || e.column.name == 'change_money'){
    		...
    		if(!data.change_money){
    			...
    			e.record.change_price = change_price;
    			e.record.amount =amount;
    			e.record.price =price;
  ...
  // 计算  金额
    ...
    function save() {
		...
		ajaxPostData({
			url : 'updatePactChangeFKHTAfter.do?isCheck=false',
			data : {
				chang_code : "${change_code}" ,
	...
	//合同数据查看 (备份表数据)
    function openPactCopy(){
    	var pactCode = pact_code.getValue();
    	if(pactCode == ""){
    		pactCode = '${pact_code}';
    	}
    	var change_code = "${change_code}";
   	 	parent.$.etDialog.open({
   	 	 url: 'hrp/pac/fkht/change/pactFKHTEdit.do?isCheck=false&pact_code='+pactCode+'&change_code='+change_code+'&change_state='+state,
            width: $(window).width(),
...
<body style="overflow: scroll; ">
	<table class="table-layout">
		<tr>
			...
			<td class="ipt"><input id="change_code" type="text" disabled="disabled" value="${change_code}"/></td>
			...
			<td class="ipt" ><input id="pact_code" type="text" style="width: 180px;" disabled="disabled" /></td>
		...
		<tr>
			...
			<td class="ipt" colspan="7"><textarea id="change_reason" style="resize:none;width: 95.5%;">${change_reason}</textarea></td>
		</tr>
		<tr>
			<td class="label"><input id="is_money_c" type="checkbox" disabled="disabled" />金额变动</td>
		</tr>
	</table>
	<div id ="planMod" style="height: 130px;display: none">
		<table  class="table-layout">
			<tr>
				...
				<td class="ipt"><input id="pay_id" type="text" style="width: 180px;" disabled="disabled" value='${pay_id}'/></td>
				...
				<td class="ipt"><input id="summary" type="text" value='${summary}'/></td>
			</tr>
			<tr>
				...
				<td class="ipt"><input id="plan_money" type="text" disabled="disabled" value='${plan_money}'/></td>
			</tr>
			<tr>
				<td class="label no-empty" style="width: 100px;">付款条件：</td>
				<td class="ipt" colspan="7"><textarea id="pay_cond" style="resize:none;width: 95.5%;">${pay_cond}</textarea></td>
			</tr>
	...
	<div class="button-group">
	  ...
	  <button id="cancle">取消</button>
	   <button id="pactInfoCopy">合同信息</button>
	</div>
