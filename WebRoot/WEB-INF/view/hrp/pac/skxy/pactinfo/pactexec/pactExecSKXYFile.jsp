...
<script type="text/javascript">
	...
	function query() {
		...
		grid.loadData(params, '../../pactdoc/queryPactFileTypeSKXY.do?isCheck=false')
	}
	...
	function save(){
		...
		ajaxPostData({
			url : '../../pactdoc/saveFileDocSKXY.do?isCheck=false',
			data : {
	...
	function toResource(data) {
		parent.$.etDialog.open({
			url : 'hrp/pac/skxy/pactdoc/toPactSKXYSelectDocPage.do?isCheck=false&pact_code=${pact_code}&pact_type_code=${pact_type_code}&file_type='
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.etDialog.confirm('确定删除?', function() {
				ajaxPostData({
					url : '../../pactdoc/deletePactExecSKXYFile.do?isCheck=false',
					data : {
						listVo : JSON.stringify(param)
					},
					success : function() {
	...
	function addFile() {
		parent.$.etDialog
				.open({
					url : 'hrp/pac/skxy/pactdoc/toPactSKXYFileManagerPage.do?isCheck=false&pact_code=${pact_code}&pact_type_code=${pact_type_code}&pact_name=${pact_name}',
					width : '450px',
					height : '250px',
					title : '归档管理',
					modal : true,
	...
	function docFileManager() {
		parent.$.etDialog
				.open({
					url : 'hrp/pac/skxy/pactdoc/pactDocSKXYMainPage.do?isCheck=false&pact_code=${pact_code}&pact_type_code=${pact_type_code}&pact_name=${pact_name}',
					width : $(window).width(),
					height : '450px',
					title : '文档管理',
					modal : true,
