...
<head>
    ...
    <script>
        ...
        var query = function () {
            params = [
                { name: 'sub_name', value: $("#sub_name").val() },
        ...
        var initGrid = function () {
            var columns = [
            	 { display: '标的物编码', name: 'sub_code',width: '15%'},
                 { display: '标的物名称', name: 'sub_name',width: '19%'},
        ...
        var openUpdate = function (rowData) {
            $.etDialog.open({
            	 url: 'pactElseSubEdit.do?isCheck=false&sub_code=' + rowData.sub_code+"&sub_id="+rowData.sub_id+"&sub_no="+rowData.sub_no,
                 width: 320,
                 height: 280,
                 title: '修改',
                 btn: ['保存', '取消'],
                 ...
                 btn1: function (index, el) {
                	 $.ajax({
                         type: 'POST',
                         url: 'updatePactElseSub.do',
                         data : {
                        	 sub_code : rowData.sub_code,
              				check : 'false',
              			},
                         dataType: 'json',
        ...
        var save = function (data) {
        	 $.etDialog.open({
                 url: 'pactElseSubAdd.do?isCheck=false',
                 width: 320,
                 height: 280,
                 title: '添加',
                 btn: ['保存', '取消'],
        ...
        var remove = function () {
             ...
             if (data.length == 0) {
                 ...
                 $.etDialog.confirm('确定删除?', function () {
                     ajaxPostData({
                         url: 'deletePactElseSub.do?isCheck=false',
                         data: {
...
<body>
    <table class="table-layout">
        <tr>
            <td class="label" style="width: 100px;">标的物名称：</td>
            <td class="ipt">
