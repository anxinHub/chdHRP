...
<script type="text/javascript">
			...
			columns : [
					{display : '数据源编码',name : 'ds_code',width: 100,align : 'left',
						render: function (item) {
	                        ...
	                        return "<a onClick=openUpdate('"+item.copy_code+"','"+item.ds_code+"','"+item.hip_med_code+"','"+item.hrp_med_code+"') >"+item.ds_code+"</a>";
					...
					{display : '平台药品编码',name : 'hip_med_code',width: 100,align : 'left'},
					
					{display : '平台药品名称',name : 'hip_med_name',width: 150,align : 'left'},
					
					{display : 'HRP药品编码',name : 'hrp_med_code',align : 'left',width: 100,align : 'left'},
					
					{display : 'HRP药品名称',name : 'hrp_med_name',align : 'left',width: 150,align : 'left'}
					],
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryHipMedRef.do',
    ...
    function templateDown() {
		...
		location.href = "downTemplateHipMedRef.do?isCheck=false"
    ...
    function importData() {
		...
		$.ligerDialog.open({
			url : 'HipMedRefImportPage.do?isCheck=false',
			height : 430,
			width : 760,
			isResize : true,
			title : '导入'
    ...
    function delCheck(){
		...
		if (data.length == 0) {
			...
			$(data).each(function() {
				...
				ParamVo.push(this.group_id + "@" +this.hos_id + "@"+this.copy_code + "@" +this.ds_code + "@" + this.hip_med_code + "@" + this.hrp_med_code)
				
			});
			
			$.ligerDialog.confirm('确定删除选择?', function (yes){if(yes){
				
				ajaxJsonObjectByUrl("delCheckHipMedRef.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function delAll(){
		...
		$.ligerDialog.confirm('确定清空所有数据?', function (yes){if(yes){
			ajaxJsonObjectByUrl("deleteHipMedRef.do", "", function(responseData) {
    ...
    function openUpdate(obj,obj1,obj2,obj3){
		...
		ajaxJsonObjectByUrl("updateHipMedRefPage.do?isCheck=false", formPara, function(responseData) {

			liger.get("ds_code").setValue(responseData.ds_code);
			
			liger.get("ds_code").setText(responseData.ds_name);
			
			liger.get("hip_med_code").setValue(responseData.hip_med_code);
			
			liger.get("hip_med_code").setText(responseData.hip_med_name);
			
			liger.get("hrp_med_code").setValue(responseData.hrp_med_code);
			
			liger.get("hrp_med_code").setText(responseData.hrp_med_name);
			
			$("#copy_code_del").val(obj);
			
			$("#old_data").val(responseData.copy_code+"@"+responseData.ds_code+"@"+responseData.hip_med_code+"@"+responseData.hrp_med_code);
    ...
    function loadDict(){
    	...
    	$("#ds_code").ligerComboBox({
    		...
    		onSelected: function (selectValue){
    			...
    			autocomplete("#hip_med_code","../queryHipMedDict.do?isCheck=false","id","text",true,true,hip_para,true,false,'180');	
           }
    	 });
    	
    	$("#hip_med_code").ligerComboBox({width:180 });
    	
    	var hrp_para = {copy_code : liger.get("copy_code").getValue()};
    	
    	autocomplete("#hrp_med_code","../queryMedInv.do?isCheck=false","id","text",true,true,hrp_para,true,false,'180');	
    ...
    function btnDel(){
    	...
    	var hip_med_code = liger.get("hip_med_code").getValue();
    	
    	var hrp_med_code = liger.get("hrp_med_code").getValue();
		...
		var formPara = {
				...
				hip_med_code:hip_med_code,
				
				hrp_med_code:hrp_med_code
				
		};
		$.ligerDialog.confirm('确定删除?', function (yes){if(yes){
		ajaxJsonObjectByUrl("deleteHipMedRef.do", formPara, function(responseData) {
    ...
    function btnSave(){
		...
		ajaxJsonObjectByUrl("addHipMedRef.do", formPara, function(responseData) {
    ...
    <div id="layout1">
            ...
            <div position="right" title="明细">
            	...
            	<table cellpadding="0" cellspacing="0" class="l-table-edit" >
			        ...
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">平台药品：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hip_med_code" type="text" id="hip_med_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">HRP药品：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hrp_med_code" type="text" id="hrp_med_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			    </table>
            </div>  
