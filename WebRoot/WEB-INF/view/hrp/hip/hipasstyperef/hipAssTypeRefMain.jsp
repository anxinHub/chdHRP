...
<script type="text/javascript">
			...
			columns : [
					{display : '数 据 源编码',name : 'ds_code',width: 100,align : 'left',
						render: function (item) {
	                        ...
	                        return "<a onClick=openUpdate('"+item.copy_code+"','"+item.ds_code+"','"+item.hip_ass_type_code+"','"+item.hrp_ass_type_code+"') >"+item.ds_code+"</a>";
					...
					{display : '平台资产分类编码',name : 'hip_ass_type_code',width: 100,align : 'left'},
					
					{display : '平台资产分类名称',name : 'hip_ass_type_name',width: 150,align : 'left'},
					
					{display : 'HRP资产分类编码',name : 'hrp_ass_type_code',align : 'left',width: 100,align : 'left'},
					
					{display : 'HRP资产分类名称',name : 'hrp_ass_type_name',align : 'left',width: 150,align : 'left'}
					],
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryHipAssTypeRef.do',
    ...
    function templateDown() {
		...
		location.href = "downTemplateHipAssTypeRef.do?isCheck=false"
    ...
    function importData() {
		...
		$.ligerDialog.open({
			url : 'HipAssTypeRefImportPage.do?isCheck=false',
			height : 430,
			width : 760,
			isResize : true,
			title : '导入'
    ...
    function delCheck(){
		...
		if (data.length == 0) {
			...
			$(data).each(function() {
				...
				ParamVo.push(this.group_id + "@" +this.hos_id + "@"+this.copy_code + "@" +this.ds_code + "@" + this.hip_ass_type_code + "@" + this.hrp_ass_type_code)
				
			});
			
			$.ligerDialog.confirm('确定删除选择?', function (yes){if(yes){
				
				ajaxJsonObjectByUrl("delCheckHipAssTypeRef.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData) {
					...
					if (responseData.state == "true") {
				    	...
				    	$("#hip_ass_type_code").val('');
				    	
				    	$("#hrp_ass_type_code").val('');
	...
	function delAll(){
		...
		$.ligerDialog.confirm('确定清空所有数据?', function (yes){if(yes){
			ajaxJsonObjectByUrl("deleteHipAssTypeRef.do", "", function(responseData) {
    ...
    function openUpdate(obj,obj1,obj2,obj3){
    	...
    	var formPara = {
    			...
    			hip_ass_type_code : obj2,
    			
    			hrp_ass_type_code : obj3

		};
		
		ajaxJsonObjectByUrl("updateHipAssTypeRefPage.do?isCheck=false", formPara, function(responseData) {

			liger.get("ds_code").setValue(responseData.ds_code);
			
			liger.get("ds_code").setText(responseData.ds_name);
			
			$("#hip_ass_type_code").val(responseData.hip_ass_type_code);
			
			$("#hrp_ass_type_code").val(responseData.hrp_ass_type_code);
			
			$("#old_data").val(responseData.copy_code+"@"+responseData.ds_code+"@"+responseData.hip_ass_type_code+"@"+responseData.hrp_ass_type_code);
    ...
    function loadDict(){
    	...
    	$("#ds_code").ligerComboBox({
    		...
    		onSelected: function (selectValue){
    			...
    			autocomplete("#hip_ass_type_code","../queryHipAssTypeDict.do?isCheck=false","id","text",true,true,hip_para,true,false,'180');	
           }
    	 });
    	...
    	autocomplete("#hrp_ass_type_code","../queryAssTypeDict.do?isCheck=false","id","text",true,true,hrp_para,true,false,'180');
    ...
    function btnDel(){
    	...
    	var hip_ass_type_code = liger.get("hip_ass_type_code").getValue();
    	
    	var hrp_ass_type_code = liger.get("hrp_ass_type_code").getValue();
		...
		var formPara = {
				...
				hip_ass_type_code:hip_ass_type_code,
				
				hrp_ass_type_code:hrp_ass_type_code
				
		};
		$.ligerDialog.confirm('确定删除?', function (yes){if(yes){
		ajaxJsonObjectByUrl("deleteHipAssTypeRef.do", formPara, function(responseData) {
    ...
    function btnSave(){
		...
		ajaxJsonObjectByUrl("addHipAssTypeRef.do", formPara, function(responseData) {
    ...
    <div id="layout1">
            ...
            <div position="right" title="明细">
            	...
            	<table cellpadding="0" cellspacing="0" class="l-table-edit" >
			        ...
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">平台资产分类：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hip_ass_type_code" type="text" id="hip_ass_type_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">HRP资产分类：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hrp_ass_type_code" type="text" id="hrp_ass_type_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			    </table>
            </div>  
