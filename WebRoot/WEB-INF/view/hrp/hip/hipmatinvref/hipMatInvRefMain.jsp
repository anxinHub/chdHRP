...
<script type="text/javascript">
			...
			columns : [
					{display : '数据源编码',name : 'ds_code',width: 100,align : 'left',
						render: function (item) {
	                        ...
	                        return "<a onClick=openUpdate('"+item.copy_code+"','"+item.ds_code+"','"+item.hip_inv_code+"','"+item.hrp_inv_code+"') >"+item.ds_code+"</a>";
					...
					{display : '平台材料编码',name : 'hip_inv_code',width: 100,align : 'left'},
					
					{display : '平台材料名称',name : 'hip_inv_name',width: 150,align : 'left'},
					
					{display : 'HRP材料编码',name : 'hrp_inv_code',align : 'left',width: 100,align : 'left'},
					
					{display : 'HRP材料名称',name : 'hrp_inv_name',align : 'left',width: 150,align : 'left'},
					],
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryHipMatInvRef.do',
    ...
    function templateDown() {
		...
		location.href = "downTemplateHipMatInvRef.do?isCheck=false"
    ...
    function importData() {
		...
		$.ligerDialog.open({
			url : 'HipMatInvRefImportPage.do?isCheck=false',
			height : 430,
			width : 760,
			isResize : true,
			title : '导入'
    ...
    function delCheck(){
		...
		if (data.length == 0) {
			...
			$(data).each(function() {
				...
				ParamVo.push(this.group_id + "@" +this.hos_id + "@"+this.copy_code + "@" +this.ds_code + "@" + this.hip_inv_code + "@" + this.hrp_inv_code)
				
			});
			
			$.ligerDialog.confirm('确定删除选择?', function (yes){if(yes){
				
				ajaxJsonObjectByUrl("delCheckHipMatInvRef.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function delAll(){
		...
		$.ligerDialog.confirm('确定清空所有数据?', function (yes){if(yes){
			ajaxJsonObjectByUrl("deleteHipMatInvRef.do", "", function(responseData) {
    ...
    function openUpdate(obj,obj1,obj2,obj3){
		...
		ajaxJsonObjectByUrl("updateHipMatInvRefPage.do?isCheck=false", formPara, function(responseData) {

			liger.get("ds_code").setValue(responseData.ds_code);
			
			liger.get("ds_code").setText(responseData.ds_name);
			
			liger.get("hip_inv_code").setValue(responseData.hip_inv_code);
			
			liger.get("hip_inv_code").setText(responseData.hip_inv_name);
			
			liger.get("hrp_inv_code").setValue(responseData.hrp_inv_code);
			
			liger.get("hrp_inv_code").setText(responseData.hrp_inv_name);
			
			$("#copy_code_del").val(obj);
			
			$("#old_data").val(responseData.copy_code+"@"+responseData.ds_code+"@"+responseData.hip_inv_code+"@"+responseData.hrp_inv_code);
    ...
    function loadDict(){
    	...
    	$("#ds_code").ligerComboBox({
    		...
    		onSelected: function (selectValue){
    			...
    			autocomplete("#hip_inv_code","../queryHipMatInvDict.do?isCheck=false","id","text",true,true,hip_para,true,false,'180');	
           }
    	 });
    	
    	$("#hip_inv_code").ligerComboBox({width:180 });
    	
    	var hrp_para = {copy_code : liger.get("copy_code").getValue()};
    	
    	autocomplete("#hrp_inv_code","../queryMatInvDict.do?isCheck=false","id","text",true,true,hrp_para,true,false,'180');
    ...
    function btnDel(){
    	...
    	var hip_inv_code = liger.get("hip_inv_code").getValue();
    	
    	var hrp_inv_code = liger.get("hrp_inv_code").getValue();
		...
		var formPara = {
				...
				hip_inv_code:hip_inv_code,
				
				hrp_inv_code:hrp_inv_code
				
		};
		$.ligerDialog.confirm('确定删除?', function (yes){if(yes){
		ajaxJsonObjectByUrl("deleteHipMatInvRef.do", formPara, function(responseData) {
    ...
    function btnSave(){
		...
		ajaxJsonObjectByUrl("addHipMatInvRef.do", formPara, function(responseData) {
    ...
    <div id="layout1">
            ...
            <div position="right" title="明细">
            	...
            	<table cellpadding="0" cellspacing="0" class="l-table-edit" >
			        ...
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">平台材料：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hip_inv_code" type="text" id="hip_inv_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">HRP材料：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hrp_inv_code" type="text" id="hrp_inv_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			    </table>
            </div>  
