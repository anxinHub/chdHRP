...
<script type="text/javascript">
			...
			columns : [
					{display : '数据源编码',name : 'ds_code',width: 100,align : 'left',
						render: function (item) {
	                        ...
	                        return "<a onClick=openUpdate('"+item.copy_code+"','"+item.ds_code+"','"+item.hip_mat_type_code+"','"+item.hrp_mat_type_code+"') >"+item.ds_code+"</a>";
					...
					{display : '平台材料分类编码',name : 'hip_mat_type_code',width: 100,align : 'left'},
					
					{display : '平台材料分类名称',name : 'hip_mat_type_name',width: 150,align : 'left'},
					
					{display : 'HIP材料分类编码',name : 'hrp_mat_type_code',align : 'left',width: 100,align : 'left'},
					
					{display : 'HIP材料分类名称',name : 'hrp_mat_type_name',align : 'left',width: 150,align : 'left'}
					],
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryHipMatTypeRef.do',
    ...
    function templateDown() {
		...
		location.href = "downTemplateHipMatTypeRef.do?isCheck=false"
    ...
    function importData() {
		...
		$.ligerDialog.open({
			url : 'HipMatTypeRefImportPage.do?isCheck=false',
			height : 430,
			width : 760,
			isResize : true,
			title : '导入'
    ...
    function delCheck(){
		...
		if (data.length == 0) {
			...
			$(data).each(function() {
				...
				ParamVo.push(this.group_id + "@" +this.hos_id + "@"+this.copy_code + "@" +this.ds_code + "@" + this.hip_mat_type_code + "@" + this.hrp_mat_type_code)
				
			});
			
			$.ligerDialog.confirm('确定删除选择?', function (yes){if(yes){
				
				ajaxJsonObjectByUrl("delCheckHipMatTypeRef.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData) {
					...
					if (responseData.state == "true") {
				    	...
				    	$("#hip_mat_type_code").val('');
				    	
				    	$("#hrp_mat_type_code").val('');
	...
	function delAll(){
		...
		$.ligerDialog.confirm('确定清空所有数据?', function (yes){if(yes){
			ajaxJsonObjectByUrl("deleteHipMatTypeRef.do", "", function(responseData) {
    ...
    function openUpdate(obj,obj1,obj2,obj3){
    	...
    	var formPara = {
    			...
    			hip_mat_type_code : obj2,
    			
    			hrp_mat_type_code : obj3

		};
		
		ajaxJsonObjectByUrl("updateHipMatTypeRefPage.do?isCheck=false", formPara, function(responseData) {

			liger.get("ds_code").setValue(responseData.ds_code);
			
			liger.get("ds_code").setText(responseData.ds_name);
			
			liger.get("hip_mat_type_code").setValue(responseData.hip_mat_type_code);
			
			liger.get("hip_mat_type_code").setText(responseData.hip_mat_type_name);
			
			liger.get("hrp_mat_type_code").setValue(responseData.hrp_mat_type_code);
			
			liger.get("hrp_mat_type_code").setText(responseData.hrp_mat_type_name);
			
			$("#copy_code_del").val(obj);
			
			$("#old_data").val(responseData.copy_code+"@"+responseData.ds_code+"@"+responseData.hip_mat_type_code+"@"+responseData.hrp_mat_type_code);
    ...
    function loadDict(){
    	...
    	$("#ds_code").ligerComboBox({
    		...
    		onSelected: function (selectValue){
    			...
    			autocomplete("#hip_mat_type_code","../queryHipMatTypeDict.do?isCheck=false","id","text",true,true,hip_para,true,false,'180');	
           }
    	 });
    	...
    	autocomplete("#hrp_mat_type_code","../queryMatTypeDict.do?isCheck=false","id","text",true,true,hrp_para,true,false,'180');	
    ...
    function btnDel(){
    	...
    	var hip_mat_type_code = liger.get("hip_mat_type_code").getValue();
    	
    	var hrp_mat_type_code = liger.get("hrp_mat_type_code").getValue();
		...
		var formPara = {
				...
				hip_mat_type_code:hip_mat_type_code,
				
				hrp_mat_type_code:hrp_mat_type_code
				
		};
		$.ligerDialog.confirm('确定删除?', function (yes){if(yes){
		ajaxJsonObjectByUrl("deleteHipMatTypeRef.do", formPara, function(responseData) {
    ...
    function btnSave(){
		...
		ajaxJsonObjectByUrl("addHipMatTypeRef.do", formPara, function(responseData) {
    ...
    <div id="layout1">
            ...
            <div position="right" title="明细">
            	...
            	<table cellpadding="0" cellspacing="0" class="l-table-edit" >
			        ...
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">平台材料分类：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hip_mat_type_code" type="text" id="hip_mat_type_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">HIP材料分类：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hrp_mat_type_code" type="text" id="hrp_mat_type_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			    </table>
            </div>  
