...
<script type="text/javascript">
    ...
    function loadHead(){
		...
		grid = $("#maingrid").ligerGrid({
			...
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryHipDeptRefCost.do',
			width : '100%',height : '100%',checkbox : true,selectRowButtonOnly : true,alternatingRow: false,//heightDiff: -10,
			toolbar : {
				items : [
				         {text : '查询',id : 'search',click : query,icon : ''},
				         {line : true},
    ...
    function importData() {
    	...
    	var para = {
			...
			importSpreadView(
					"hrp/hip/hipdeptref/readFiles.do?isCheck=false",
					para);
	}
			...
			$.ligerDialog.confirm('确定删除选择?', function (yes){if(yes){
				...
				ajaxJsonObjectByUrl("delCheckHipDeptRef.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData) {
					...
					if (responseData.state == "true") {
						...
						liger.get("ds_code").setValue('');
						liger.get("hip_dept_code").setValue('');
						liger.get("hrp_dept_code").setValue('');
	...
	function delAll(){
		...
		$.ligerDialog.confirm('确定清空所有数据?', function (yes){if(yes){
			ajaxJsonObjectByUrl("deleteHipDeptRefCost.do", "", function(responseData) {
    ...
    function openUpdate(obj,obj1,obj2, obj3){
		...
		ajaxJsonObjectByUrl("updateHipDeptRefCostPage.do?isCheck=false", formPara, function(responseData) {
    ...
    function btnDel(){
		...
		ajaxJsonObjectByUrl("deleteHipDeptRefCost.do", formPara, function(responseData) {

			if (responseData.state == "true") {
				
				query();
				
				liger.get("ds_code").getValue();
    ...
    function btnSave(){
		...
		ajaxJsonObjectByUrl("addHipDeptRefCost.do", formPara, function(responseData) {
