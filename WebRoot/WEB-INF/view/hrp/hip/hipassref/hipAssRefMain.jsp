...
<script type="text/javascript">
			...
			columns : [
					{display : '数 据 源编码',name : 'ds_code',width: 100,align : 'left',
						render: function (item) {
	                        ...
	                        return "<a onClick=openUpdate('"+item.copy_code+"','"+item.ds_code+"','"+item.hip_ass_code+"','"+item.hrp_ass_code+"') >"+item.ds_code+"</a>";
					...
					{display : '平台资产编码',name : 'hip_ass_code',width: 100,align : 'left'},
					
					{display : '平台资产名称',name : 'hip_ass_name',width: 150,align : 'left'},
					
					{display : 'HRP资产编码',name : 'hrp_ass_code',align : 'left',width: 100,align : 'left'},
					
					{display : 'HRP资产名称',name : 'hrp_ass_name',align : 'left',width: 150,align : 'left'}
					],
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryHipAssRef.do',
    ...
    function templateDown() {
		...
		location.href = "downTemplateHipAssRef.do?isCheck=false"
    ...
    function importData() {
		...
		$.ligerDialog.open({
			url : 'HipAssRefImportPage.do?isCheck=false',
			height : 430,
			width : 760,
			isResize : true,
			title : '导入'
    ...
    function delCheck(){
		...
		if (data.length == 0) {
			...
			$(data).each(function() {
				...
				ParamVo.push(this.group_id + "@" +this.hos_id + "@"+this.copy_code + "@" +this.ds_code + "@" + this.hip_ass_code + "@" + this.hrp_ass_code)
				
			});
			
			$.ligerDialog.confirm('确定删除选择?', function (yes){if(yes){
				
				ajaxJsonObjectByUrl("delCheckHipAssRef.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function delAll(){
		...
		$.ligerDialog.confirm('确定清空所有数据?', function (yes){if(yes){
			ajaxJsonObjectByUrl("deleteHipAssRef.do", "", function(responseData) {
    ...
    function openUpdate(obj,obj1,obj2,obj3){
		...
		ajaxJsonObjectByUrl("updateHipAssRefPage.do?isCheck=false", formPara, function(responseData) {

			liger.get("ds_code").setValue(responseData.ds_code);
			
			liger.get("ds_code").setText(responseData.ds_name);
			
			liger.get("hip_ass_code").setValue(responseData.hip_ass_code);
			
			liger.get("hip_ass_code").setText(responseData.hip_ass_name);
			
			liger.get("hrp_ass_code").setValue(responseData.hrp_ass_code);
			
			liger.get("hrp_ass_code").setText(responseData.hrp_ass_name);
			
			$("#copy_code_del").val(obj);
			
			$("#old_data").val(responseData.copy_code+"@"+responseData.ds_code+"@"+responseData.hip_ass_code+"@"+responseData.hrp_ass_code);
    ...
    function loadDict(){
    	...
    	$("#ds_code").ligerComboBox({
    		...
    		onSelected: function (selectValue){
    			...
    			autocomplete("#hip_ass_code","../queryHipAssDict.do?isCheck=false","id","text",true,true,hip_para,true,false,'180');	
           }
    	 });
    	...
    	autocomplete("#hrp_ass_code","../queryAssDict.do?isCheck=false","id","text",true,true,hrp_para,true,false,'180');
    ...
    function btnDel(){
    	...
    	var hip_ass_code = liger.get("hip_ass_code").getValue();
    	
    	var hrp_ass_code = liger.get("hrp_ass_code").getValue();
		...
		var formPara = {
				...
				hip_ass_code:hip_ass_code,
				
				hrp_ass_code:hrp_ass_code
				
		};
		$.ligerDialog.confirm('确定删除?', function (yes){if(yes){
		ajaxJsonObjectByUrl("deleteHipAssRef.do", formPara, function(responseData) {
    ...
    function btnSave(){
		...
		ajaxJsonObjectByUrl("addHipAssRef.do", formPara, function(responseData) {
    ...
    <div id="layout1">
            ...
            <div position="right" title="明细">
            	...
            	<table cellpadding="0" cellspacing="0" class="l-table-edit" >
			        ...
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">平台资产：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hip_ass_code" type="text" id="hip_ass_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">HRP资产：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hrp_ass_code" type="text" id="hrp_ass_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			    </table>
            </div>  
