  ...
  <head>
		...
		if($("#is_copy_code").val() == 0){
  		...
  		ajaxJsonObjectByUrl("saveHipDataType.do?isCheck=false", para, function(
			...
			if (responseData.state == "true") {
				...
				$("#type_id").val(responseData.type_id);
    ...
    function loadDict(){
    	...
    	if(user_type == '0' || user_type == '4'){
    		autocomplete("#dgroup_id", "../../acc/queryGroupDictDataType.do?isCheck=false&group_id=${sessionScope.group_id}","id", "text",true,true,null,true,null,"280");
        	
        	autoCompleteByData("#dhos_id", no_info.Rows, "id", "text", false, true, true,false,null,280);
        	
        	autoCompleteByData("#dcopy_code", no_info.Rows, "id", "text", false, true, true,false,null,280);
        	
    		$("#dgroup_id").change(function(){
    			autocomplete("#dhos_id", "../../acc/queryHosInfoDataType.do?isCheck=false","id", "text",true,true,{group_id:liger.get("dgroup_id").getValue(),hos_id:'${sessionScope.hos_id}'},true,null,"280");
    		});
    		
    		$("#dhos_id").change(function(){
    			autocomplete("#dcopy_code", "../../acc/queryHosCopyDataType.do?isCheck=false","id", "text",true,true,{group_id:liger.get("dgroup_id").getValue(),hos_id:liger.get("dhos_id").getValue()},true,null,"280");
    		});
    		...
    		$("#dgroup_id").change(function(){
    			autocomplete("#dhos_id", "../../acc/queryHosInfoDataType.do?isCheck=false","id", "text",true,true,{group_id:liger.get("dgroup_id").getValue()},true,null,"280");
    		});
	...
	function open_decodeSet(){
		$.ligerDialog.open({ 
			url : 'decodeSetPage.do?isCheck=false',
			data:{}, 
   ...
   <input id="is_group" type="hidden"   />
   <input id="is_hos" type="hidden"   />
   <input id="is_copy_code" type="hidden"   />
   	<div id="layout1" style="height: 100%;">
		    ...
		    <tr>
                ...
                <td align="left" class="l-table-edit-td"><input name="type_code" type="text" id="type_code" /></td>
                <td align="left"></td>
            </tr> 
            <tr>
                <td align="right" class="l-table-edit-td"  style="padding-left:20px;">业务名称：</td>
                <td align="left" class="l-table-edit-td"><input name="type_name" type="text" id="type_name" /></td>
                <td align="left"></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td align="left" class="l-table-edit-td"><input name="table_name" type="text" id="table_name"  /></td>
            ...
            <div position="center" title="查询SQL"  style="left:width:100%; height: 100%;">
				...
				<table style="float: left;" width="100%"  >
					<tr >	
		        	 ...
		        	 <tr>
		                <td align="left" class="l-table-edit-td">
		    				<textarea class="comments" id="q_sql" rows="24" cols="100" placeholder="select的别名必须与目标表的字段保持一致；如遇到目标表有id的字段需要做字段转换，可以不需要查询；group_id,hos_id,copy_code字段可以省略，系统自动处理；" onpropertychange= "this.style.posHeight=this.scrollHeight"></textarea>
		                </td>
		            </tr>
		            <tr>
		                <td align="left" class="l-table-edit-td">
		                	同步条件表达式（HRP）：
		    				<textarea class="comments" id="pk_col" rows="5" placeholder="如根据type_code和inv_code增量同步：type_code,inv_code...；如根据时间删除同步：date >= @begin_date and date <=@end_date" cols="160" onpropertychange= "this.style.posHeight=this.scrollHeight">${pk_col }</textarea>
		                </td>
		            </tr> 
		        	<tr>
						<td align="left" class="l-table-edit-td" >
