...
<script type="text/javascript">
			...
			columns : [
					{display : '数 据 源编码',name : 'ds_code',width: 100,align : 'left',
						render: function (item) {
	                        ...
	                        return "<a onClick=openUpdate('"+item.copy_code+"','"+item.ds_code+"','"+item.hip_charge_detail_code+"','"+item.hrp_charge_detail_code+"') >"+item.ds_code+"</a>";
					...
					{display : '平台收费项目编码',name : 'hip_charge_detail_code',width: 100,align : 'left'},
					
					{display : '平台收费项目名称',name : 'hip_charge_detail_name',width: 150,align : 'left'},
					
					{display : '收费项目编码',name : 'hrp_charge_detail_code',align : 'left',width: 100,align : 'left'},
					
					{display : '收费项目名称',name : 'hrp_charge_detail_name',align : 'left',width: 150,align : 'left'}
					],
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryHipChargeDetailRef.do',
    ...
    function templateDown() {
		...
		location.href = "downTemplateHipChargeDetailRef.do?isCheck=false"
    ...
    function importData() {
		...
		$.ligerDialog.open({
			url : 'HipChargeDetailRefImportPage.do?isCheck=false',
			height : 430,
			width : 760,
			isResize : true,
			title : '导入'
    ...
    function delCheck(){
		...
		if (data.length == 0) {
			...
			$(data).each(function() {
				...
				ParamVo.push(this.group_id + "@" +this.hos_id + "@"+this.copy_code + "@" +this.ds_code + "@" + this.hip_charge_detail_code + "@" + this.hrp_charge_detail_code)
				
			});
			
			$.ligerDialog.confirm('确定删除选择?', function (yes){if(yes){
				
				ajaxJsonObjectByUrl("delCheckHipChargeDetailRef.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData) {
					...
					if (responseData.state == "true") {
				    	...
				    	$("#hip_charge_detail_code").val('');
				    	
				    	$("#hrp_charge_detail_code").val('');
	...
	function delAll(){
		...
		$.ligerDialog.confirm('确定清空所有数据?', function (yes){if(yes){
			ajaxJsonObjectByUrl("deleteHipChargeDetailRef.do", "", function(responseData) {
    ...
    function openUpdate(obj,obj1,obj2,obj3){
    	...
    	var formPara = {
    			...
    			hip_charge_detail_code : obj2,
    			
    			hrp_charge_detail_code : obj3

		};
		
		ajaxJsonObjectByUrl("updateHipChargeDetailRefPage.do?isCheck=false", formPara, function(responseData) {

			liger.get("ds_code").setValue(responseData.ds_code);
			
			liger.get("ds_code").setText(responseData.ds_name);
			
			liger.get("hip_charge_detail_code").setValue(responseData.hip_charge_detail_code);
			
			liger.get("hip_charge_detail_code").setText(responseData.hip_charge_detail_name);
			
			liger.get("hrp_charge_detail_code").setValue(responseData.hrp_charge_detail_code);
			
			liger.get("hrp_charge_detail_code").setText(responseData.hrp_charge_detail_name);
			
			$("#copy_code_del").val(obj);
			
			$("#old_data").val(responseData.copy_code+"@"+responseData.ds_code+"@"+responseData.hip_charge_detail_code+"@"+responseData.hrp_charge_detail_code);
    ...
    function loadDict(){
    	...
    	$("#ds_code").ligerComboBox({
    		...
    		onSelected: function (selectValue){
    			...
    			autocomplete("#hip_charge_detail_code","../queryHipChargeDetailDict.do?isCheck=false","id","text",true,true,hip_para,true,false,'180');	
           }
    	 });
    	
    	$("#hip_charge_detail_code").ligerComboBox({width:180 });
    	
    	var hrp_para = {copy_code : liger.get("copy_code").getValue()};
    	
    	autocomplete("#hrp_charge_detail_code","../queryCostChargeItemArrt.do?isCheck=false","id","text",true,true,hrp_para,true,false,'180');	
    ...
    function btnDel(){
    	...
    	var hip_charge_detail_code = liger.get("hip_charge_detail_code").getValue();
    	
    	var hrp_charge_detail_code = liger.get("hrp_charge_detail_code").getValue();
		...
		var formPara = {
				...
				hip_charge_detail_code:hip_charge_detail_code,
				
				hrp_charge_detail_code:hrp_charge_detail_code
				
		};
		$.ligerDialog.confirm('确定删除?', function (yes){if(yes){
		ajaxJsonObjectByUrl("deleteHipChargeDetailRef.do", formPara, function(responseData) {
    ...
    function btnSave(){
		...
		ajaxJsonObjectByUrl("addHipChargeDetailRef.do", formPara, function(responseData) {
    ...
    <div id="layout1">
            ...
            <div position="right" title="明细">
            	...
            	<table cellpadding="0" cellspacing="0" class="l-table-edit" >
			        ...
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">平台收费项目：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hip_charge_detail_code" type="text" id="hip_charge_detail_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			        <tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:20px;padding-top:20px;">收费项目：</td>
			            <td align="left" class="l-table-edit-td" style="padding-top:20px;"><input name="hrp_charge_detail_code" type="text" id="hrp_charge_detail_code" ltype="text" validate="{required:true,maxlength:20}"/></td>
			            <td align="left"></td>
			        </tr>
			    </table>
            </div>  
