...
<script type="text/javascript">
    ...
    var renderFunc = {
    		/* state:function(value){//状态
				...
				return formatNumber(value, '${sessionScope.med_para_map["08005"] }', 1);
			}, */
	...
	function loadHead() {//加载grid；定义按钮及其方法名
		grid = $("#maingrid").ligerGrid({
		       columns: [ { display: '单据标记', name: 'bill_flag',  hide:true},
		                 ...
		                 dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedUnconfirmBill.do',
	...
	function checkout(btn){//结转下月
		...
		if(data.length==0){
			...
			$.ligerDialog.confirm('将要把全部未确认单据结转到下个月，是否继续？', function (yes){
				if(yes){//确定要结转下月。
					//结转月份必须是物流当前所在期间
			    	if('${med_year}'==liger.get("year").getValue()  &&  '${med_month}'== liger.get("month").getValue()){
			    		var paramStr ="";
    	...
    	var colspan_num = grid.getColumns(1).length-1;
   		...
   		var printPara={
   			...
   			head:[
				...
				{"cell":0,"value":"统计日期: " + $("#year").val() +"-"+ $("#month").val(),"colspan":colspan_num,"br":true}
   			],
   			foot:[
   		...
   		ajaxJsonObjectByUrl("queryMedUnconfirmBill.do?isCheck=false", selPara, function (responseData) {
