    ...
    <script type="text/javascript">
		...
		var priceMsg = "";
 		...
 		if(store_inv.length > 0){
			...
			ajaxJsonObjectByUrl("../../existsMedStoreIncludeInv.do?isCheck=false", para, function (responseData){
				if(responseData.state=="false"){
					msg += "以下药品不在该仓库中！<br>"+responseData.inv_ids+"<br>";
				}
			}, false);
	 			...
	 			if(liger.get("sup_code").getValue()){
		 			para = {
		 	 		...
		 	 		ajaxJsonObjectByUrl("../../existsMedSupIncludeInv.do?isCheck=false", para, function (responseData){
		 	 			if(responseData.state=="false"){
		 	 				msg += "以下药品不属于该供应商！<br>"+responseData.inv_ids+"<br>";
		 	 			}
		 	 		}, false);
	 			}
			}
     ...
     function  save(){
    	if(validateGrid()){
	        ...
	        ajaxJsonObjectByUrl("addMedInitIn.do",formPara,function(responseData){
			...
			columns : [{
				...
				editor : {
					...
					grid : {
						...
						url : '../../queryMedInvListInitial.do?isCheck=false&is_com=0&store_id=' 
