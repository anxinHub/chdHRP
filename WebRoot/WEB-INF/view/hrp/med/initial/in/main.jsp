...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedInitIn.do',
    ...
    function lodopPrint(){
    	var head="<table class='head' width='100%'><tr><td>单位：${sessionScope.hos_name}</td></tr>";
 		...
 		grid.options.lodop.title="常备药品期初入库";
    }
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/med/initial/in/addPage.do?isCheck=false',
    ...
    function update_open(obj){		
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/med/initial/in/updatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});   
    }
    ...
    function remove(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("deleteMedInitIn.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function confirmData(confirmDate){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定入库确认?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("confirmMedInitInBatch.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    /* 	parent.$.ligerDialog.open({url : 'hrp/med/initial/in/storageInPrintSetPageQc.do?template_code=08008&use_id='+useId,
...
// 			} 
	    	...
	    	printTemplate("hrp/med/initial/in/queryMedInByPrintTemlateQc.do?isCheck=false",para); */
