...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedAdjust.do',
    ...
    function itemclick(item){ 
        if(item.id){
            switch (item.id){
                case "add":
              		$.ligerDialog.open({url: 'medAdjustAddPage.do?isCheck=false',height: 500,top:1,width: 1100, title:'添加',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,buttons: [ { text: '保存', onclick: function (item, dialog) { dialog.frame.saveMedAdjust(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                       ...
                       $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteBatchMedAdjust.do?paramVo="+Param,{},function (responseData){
                ...
                case "check":
                    ...
                    if (data.length == 0){
                       ...
                       $.ligerDialog.confirm('审核将修改药品零售价和计划价,确定修改吗?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("updateMedAdjustState.do?paramVo="+Param,{},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
                    ...
                    return;
                case "import":
                	$.ligerDialog.open({url: 'medAdjustImportPage.do?isCheck=false', height: 500,width: 1100, title:'导入',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,top:10 });
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({ 
			url : 'hrp/med/adjustMan/changePriceBill/updateMedAdjustPage.do?isCheck=false&adjust_id='+adjust_id+'&state='+state,
			data:{},  title:'修改',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,
			buttons: [ { text: '保存', onclick: function (item, dialog) { dialog.frame.saveMedAdjust(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    
    }
   
	 function loadDict(){
         //字典下拉框
		 autoCompleteByData("#state", medAdjust_state.Rows, "id", "text", true, true);//状态下拉框
