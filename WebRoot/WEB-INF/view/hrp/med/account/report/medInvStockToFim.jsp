...
<script type="text/javascript">
    ...
    function loadHead(){
		...
		//获取本期增加、减少查询字段
    	ajaxJsonObjectByUrl("queryMedAccountReportInvStockToFimColumns.do?isCheck=false", "", function (responseData){
    		if(responseData.Rows.length > 0){
    			...
    			$.each(responseData.Rows, function(v_index, v_data){ 
    				if(v_data.direction_flag == 1){//本期增加
    					add_columns.append("{display: '"+v_data.show_name+"', name: '"+v_data.show_code+"', align: 'right', minWidth: 100, "+render+"}, ")	
    				}else if(v_data.direction_flag == 2){//本期减少
    					reduct_columns.append("{display: '"+v_data.show_name+"', name: '"+v_data.show_code+"', align: 'right', minWidth: 100, "+render+"}, ")	
    				}
    				renderFunc[v_data.show_code] = function(value){
    					return formatNumber(value ==null ? 0 : value, '${p08005 }', 1);
    				}
    			});
    			add_columns.append("{display: '合计金额', name: 'add_sum_money', align: 'right', minWidth: 100, "+render+"} ]");
    			reduct_columns.append("{display: '合计金额', name: 'reduct_sum_money', align: 'right', minWidth: 100, "+render+"} ]");
			}
    	}, false);
        

		var columns = [{
				display: '财务分类编码', name: 'fim_type_code', align: 'left', minWidth: '100', frozen: true
			}, { 
	 			display: '财务分类名称', name: 'fim_type_name', align: 'left', minWidth: '150', frozen: true
	 		}, { 
    	...
    	grid = $("#maingrid").ligerGrid({
    		columns : columns, 
			dataAction: 'server', dataType: 'server', usePager: false, url: 'queryMedAccountReportInvStockToFim.do', 
    ...
    function lodopPrint(){
    	...
    	var head="<table class='head' width='100%'><tr><td>单位：${sessionScope.hos_name}</td></tr>";
 		...
 		grid.options.lodop.title=liger.get("store_code").getText().split(" ")[1]+"药品库存(财务)汇总表";
    }
	...
	function print(){
    	...
    	var colspan_num = grid.getColumns(1).length-1;
   		...
   		ajaxJsonObjectByUrl("queryMedAccountReportInvStockToFim.do?isCheck=false", selPara, function (responseData) {
