...
<script type="text/javascript">
    ...
    $(function ()
    {
		...
		$("#store_type_td").find(":radio").click(function(){
			...
			if(store_type == 1){
				$("#store_label").text("虚仓");
				autocompleteAsync("#store_code", "../../queryMedVirStore.do?isCheck=false", "id", "text", true, true, "", true);
			}else{
				$("#store_label").text("仓库");
				autocompleteAsync("#store_code", "../../queryMedStoreByRead.do?isCheck=false", "id", "text", true, true, "", true);
			}
		});
    });
    ...
    function loadHead(){
		...
		//获取本期增加、减少查询字段
    	ajaxJsonObjectByUrl("queryMedAccountReportInvStockColumns.do?isCheck=false", "", function (responseData){
    		if(responseData.Rows.length > 0){
    			//存在本期增加减少字段
    			var render = "render : function(rowdata, rowindex, value) { return formatNumber(value ==null ? 0 : value, " + '${p08005 }' + ", 1);}";
		...
		var columns = [{
				display: '药品分类编码', name: 'med_type_code', align: 'left', minWidth: '100', frozen: true
			}, { 
	 			display: '药品分类名称', name: 'med_type_name', align: 'left', minWidth: '150', frozen: true
	 		}, { 
    	...
    	grid = $("#maingrid").ligerGrid({
    		columns : columns, 
			dataAction: 'server', dataType: 'server', usePager: false, url: 'queryMedAccountReportInvStock.do', 
	    		...
	    		//title:"药品库存汇总表",
	    		 /* fn:{
	    			begin_money:function(value){//期初金额
	    				if(value == 0){return "";}
	           			else{return formatNumber(value ==null ? 0 : value, '${p08005 }', 1);}
	    			},
	    			bal_money:function(value){//进销误差
	    				if(value == 0){return "";}
	          			else{return formatNumber(value ==null ? 0 : value, '${p08005 }', 1);}
	...
	function changeColumns(){ 
		...
		var reduct_columns = new StringBuffer();
		//获取本期增加、减少查询字段
    	ajaxJsonObjectByUrl("queryMedAccountReportInvStockColumns.do?isCheck=false", paras, function (responseData){
    		if(responseData.Rows.length > 0){
    			//存在本期增加减少字段
    			var render = "render : function(rowdata, rowindex, value) { return formatNumber(value ==null ? 0 : value, " + '${p08005 }' + ", 1);}";
    			$.each(responseData.Rows, function(v_index, v_data){ 
    				...
    				if(v_data.direction_flag == 1){//本期增加
    					if(v_index == 1){
    						add_columns.append("[ {display: '"+v_data.show_name+"', name: '"+v_data.show_code+"', align: 'right', width: 120, "+render+"}")
    					}else{
    						add_columns.append(", {display: '"+v_data.show_name+"', name: '"+v_data.show_code+"', align: 'right', width: 120, "+render+"}")	
    					}
    				}else if(v_data.direction_flag == 2){//本期减少
    					if(v_index == 1){
    						reduct_columns.append("[ {display: '"+v_data.show_name+"', name: '"+v_data.show_code+"', align: 'right', width: 120, "+render+"}")
    					}else{
    						reduct_columns.append(", {display: '"+v_data.show_name+"', name: '"+v_data.show_code+"', align: 'right', width: 120, "+render+"}")	
    					}
    				}
        ...
        var columns = [{
				display: '药品分类编码', name: 'med_type_code', align: 'left', width: '100',
	 			...
	 			display: '药品分类名称', name: 'med_type_name', align: 'left', width: '150'
	 		}, { 
	 			...
	 			display: '本期增加',
	 			columns: eval(add_columns)
	 		}, { 
	 			display: '本期减少', 
	 			columns: eval(reduct_columns)
	 		}, { 
	 			display: '进销误差', name: 'stocker_name', align: 'left', width: '80',
	 			render : function(rowdata, rowindex, value) {
					return formatNumber(value ==null ? 0 : value, '${p08005 }', 1);
				}
	 		}, { 
	 			display: '期末结存', name: 'amount_money', align: 'right', width: '100',
	 			render : function(rowdata, rowindex, value) {
					return formatNumber(value ==null ? 0 : value, '${p08005 }', 1);
				}
	 		} ];
    ...
    */
	...
	function print(){
    	...
    	var colspan_num = grid.getColumns(1).length-1;
   		...
   		ajaxJsonObjectByUrl("queryMedAccountReportInvStock.do?isCheck=false", selPara, function (responseData) {
    ...
    function loadDict(){//字典下拉框
		...
		autocompleteAsync("#type_level", "../../queryMedTypeLevel.do?isCheck=false", "id", "text", true, true, "", true);
