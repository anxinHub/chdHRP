...
<script type="text/javascript">
     ...
     function  save(){
        ...
        ajaxJsonObjectByUrl("addMedAffiTranMain.do?isCheck=true",formPara,function(responseData){            
 ...
 function loadForm(){
    ...
    function loadDict(){
		...
		autoCompleteByData("#bus_type", medTranMain_busType.Rows, "id", "text",true, true,'',true,false,'180');//业务类型
		...
		//字典下拉框
		autocompleteAsync("#out_hos_id", "../../queryMedHosInfo.do?isCheck=false", "id","text", true, true, "", true, false, '180');
		var paraO;
		//alert('${hos_id}')
		...
		autocompleteAsync("#out_store_id", "../../queryMedStore.do?isCheck=false", "id","text", true, true, paraO, true, false, '180');
		//联动事件
		$("#out_hos_id").bind("change",function(){
			...
			//autocomplete("#in_store_id", "../../queryMedStore.do?isCheck=false", "id","text", true, true, para, true, false, '180');	    	
		})
		autocompleteAsync("#in_hos_id", "../../queryMedHosInfo.do?isCheck=false", "id","text", true, true, "", true, false, '180');
		...
		autocompleteAsync("#in_store_id", "../../queryMedStoreAll.do?isCheck=false", "id","text", true, true, paraI, true, false, '180');
		//联动事件
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
				...
				toolbar: { 
					items: [
						{ text: '保存', id:'add', click: saveMedOutMain,icon:'search' },
						{ line:true },
    ...
    function choice_inv(){
		...
		$.ligerDialog.open({url: "medAffiTranMainChoiceInvPage.do?isCheck=false&store_id="+store_id+"&store_text="+store_text,
    ...
    function deleteDetailRow(){ 
	 ...
	 function medTranBySingleImport(){//整单出库
		 ...
		 var store_id = liger.get("out_store_id").getValue()?liger.get("out_store_id").getValue():'null';
			var paras = store_id;			
			$.ligerDialog.open({
				url: "medAffiTranMainBySinglePage.do?isCheck=false&paras="+paras, height: 500,width: 900, 
