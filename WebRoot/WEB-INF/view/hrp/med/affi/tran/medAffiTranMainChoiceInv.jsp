    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
			...
			dataAction: 'local', dataType: 'server', usePager:true, url:'queryAffiTranInvBatchList.do?isCheck=false',
	...
	function addNew(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定生成调拨单?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("queryAffiTranInvListByChoiceInv.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
                		//清空父页面表格数据
