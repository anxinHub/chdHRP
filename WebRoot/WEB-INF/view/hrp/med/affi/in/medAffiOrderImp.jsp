...
<head>
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
			            {display : '药品变更号',name : 'inv_no',width:80,hide : true},
					    {display : '订单ID',name : 'order_id',width:80,hide : true},
					    {display : '订单明细ID',name : 'order_detail_id',width:80,hide : true},
					    {display : '药品ID',name : 'inv_id',width:80,hide : true},
					    {display : '药品NO',name : 'inv_no',width:80,hide : true},
						...
						{display : '入库数量(E)',name : 'amount',minWidth : 100,type : 'float',align : 'right',editor : {type : 'float',},
							totalSummary : {
								align : 'right',
								render : function(suminf, column,cell) {
										return '<div>'+ formatNumber(suminf.sum == null ? 0: suminf.sum,0, 0)+ '</div>';
						...
						}, {display : '生产批号(E)',name : 'batch_no',minWidth : 250,align : 'left',editor : {type : 'text',}
					...
					url : 'queryMedAffiInOrderN.do?isCheck=false',
					...
					//groupColumnName : 'inv_name',groupColumnDisplay : '药品名称    ',
					toolbar: {
	...
	function closeInvInfo(){
		...
		$.ligerDialog.open({
			url: "medAffiOrderImpCloseInvPage.do?isCheck=false&paras="+paras, 
	...
	function closeInv(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要关闭选中的药品？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("updateMedStorageAffiInCloseInv.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData){
	...
	function f_onBeforeEdit(e) {
		...
		if(column_name=='cert_id'){
			...
			certId.editor.url='../../queryMedInvCertIdWithDate.do?isCheck=false&inv_id='+e.record.inv_id;
		}
	}
	// 编辑单元格提交编辑状态之前作判断限制
	...
	function save() {
		if (validateGrid()) {
			...
			ajaxJsonObjectByUrl("addMedAffiInByOrder.do?isCheck=true", formPara, function (responseData) {
				if (responseData.state == "true") {
	...
	function loadDict() {
        ...
        autocomplete("#bus_type_code", "../../queryMedBusType.do?isCheck=false", "id", "text", true, true,{codes:'27,31'},false,"27");//业务类型
		autocompleteAsync("#store_code", "../../queryMedStoreStocker.do?isCheck=false", "id", "text", true, true, {is_com : 1}, true);
