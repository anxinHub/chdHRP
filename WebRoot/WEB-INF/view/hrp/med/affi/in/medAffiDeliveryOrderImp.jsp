...
// 			});
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					detail: { onShowDetail: showBatchSn, reload: true,height:'auto', single: true},//药品批次明细
	...
	function showBatchSn(row, detailPanel,callback){
		...
		detailGrid =$(batchSn).css({'margin-top':10, 'margin-left':60}).ligerGrid({
			columns : [
						{
							display : '药品变更号',name : 'inv_no',align : 'left',hide : true
						},
						{
						...
						{
							display : '药品名称',name : 'inv_name',align : 'left',width : '220'
						},
						{
						...
						},
						{
							...
							render : function(rowdata, rowindex,value) {
								rowdata.price = value == null ? ""
										: formatNumber(value,'${p08006 }',0);
								return value == null ? ""
										: formatNumber(value,'${p08006 }',1);
							}
						},
						{
							...
							render : function(rowdata, rowindex,value) {
								rowdata.amount_money = value == null ? ""
										: formatNumber(value,'${p08005 }',0);
								return value == null ? ""
										: formatNumber(value,'${p08005 }',1);
							},
							totalSummary : {
								align : 'right',
								render : function(suminf, column,cell) {
...
// 		liger.get("sup_code").setText("${sup_text}");
		...
		autoCompleteByData("#bill_type", medOrderMain_orderType.Rows,  "id", "text", true, true,'',true,false,'160');//订单类型
		...
		autocompleteAsync("#bus_type_code", "../../queryMedBusType.do?isCheck=false", "id", "text", true, true,{codes:'27,31'},false,"27");//业务类型
		autocompleteAsync("#store_code", "../../queryMedStore.do?isCheck=false", "id", "text", true, true, {is_com : 1}, true);
		autocomplete("#stocker", "../../queryMedStockEmp.do?isCheck=false", "id", "text", true, true, "", false);
