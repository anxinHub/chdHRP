...
<html style="overflow:hidden;">
  <head>
    ...
    //查询明细
 	function queryDetail(){
 		...
 		grid.options.parms.push({
 			name : 'group_id',
 			value : '${medAffiInMain.group_id}'
 		});
 		grid.options.parms.push({
 			name : 'hos_id',
 			value : '${medAffiInMain.hos_id}'
 		});
 		grid.options.parms.push({
 			name : 'copy_code',
 			value : '${medAffiInMain.copy_code}'
 		});
 		grid.options.parms.push({
 			name : 'in_id',
 			value : '${medAffiInMain.in_id}'
 		});
 		grid.options.parms.push({
 			name : 'store_id',
 			value : '${medAffiInMain.store_id}'
 		});
	...
	function  save(){
		...
		//判断仓库药品关系
 		if(store_inv.length > 0){
			...
			ajaxJsonObjectByUrl("../../existsMedAffiSupIsInv.do?isCheck=false", para, function (responseData){
				if(responseData.state=="false"){
		...
		ajaxJsonObjectByUrl("updateMedAffiBack.do", formPara, function(responseData){
	...
	function loadDict(){
    	//字典下拉框
    	autocomplete("#bus_type_code", "../../queryMedBusType.do?isCheck=false", "id", "text", true, true, {codes : '29'}, false, "${medAffiInMain.bus_type_code}",'180');
		autocomplete("#store_code", "../../queryMedStore.do?isCheck=false", "id", "text", true, true, {is_com : '1'}, false, false,'180',"",200);
		liger.get("store_code").setValue("${medAffiInMain.store_id},${medAffiInMain.store_no}");
		liger.get("store_code").setText("${medAffiInMain.store_code} ${medAffiInMain.store_name}");
		
		autocomplete("#stocker", "../../queryMedStockEmp.do?isCheck=false", "id", "text", true, true, "", false, "${medAffiInMain.stocker}",'180');
		liger.get("stocker").setValue("${medAffiInMain.stocker}");
		liger.get("stocker").setText("${medAffiInMain.stocker_code} ${medAffiInMain.stocker_name}");
		
		autocomplete("#sup_code", "../../queryHosSupDictDisable.do?isCheck=false", "id", "text", true, true, "", false, false,'180',"",300);
		liger.get("sup_code").setValue("${medAffiInMain.sup_id},${medAffiInMain.sup_no}");
		liger.get("sup_code").setText("${medAffiInMain.sup_code} ${medAffiInMain.sup_name}");
		
		autocomplete("#stock_type_code", "../../queryMedStockType.do?isCheck=false", "id", "text", true, true, "", false, "${medAffiInMain.stock_type_code}",'180');
		
		autocomplete("#app_dept", "../../queryMedDept.do?isCheck=false", "id", "text", true, true, {is_last : 1}, false, false,'180',"",300);
		liger.get("app_dept").setValue("${medAffiInMain.app_dept}");
		liger.get("app_dept").setText("${medAffiInMain.app_dept_code} ${medAffiInMain.app_dept_name}");
		
		autocomplete("#protocol_code", "../../queryMedProtocolMain.do?isCheck=false", "id", "text", true, true, "", false, false,'180');
		liger.get("protocol_code").setValue("${medAffiInMain.protocol_code}");
		liger.get("protocol_code").setText("${medAffiInMain.protocol_code} ${medAffiInMain.protocol_name}");
		
		autocomplete("#proj_code", "../../queryMedProj.do?isCheck=false", "id", "text", true, true, "", false, false,'',"",510);
		liger.get("proj_code").setValue("${medAffiInMain.proj_id}");
		liger.get("proj_code").setText("${medAffiInMain.proj_code} ${medAffiInMain.proj_name}");
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
					...
					type : 'select',
	         		...
	         		grid : {
	         			columns : [ {
	         				...
	         				display : '药品类别', name : 'med_type_name', minWidth : 140, align : 'left'
	         			}, {
	         			...
	         			url : '${p08021}' == 0 ? '../../queryMedAffiInvList.do?isCheck=false&store_id='+liger.get("store_code").getValue().split(",")[0]
							: '../../queryMedAffiInvList.do?isCheck=false&store_id='+liger.get("store_code").getValue().split(",")[0]+'&sup_id='+liger.get("sup_code").getValue().split(",")[0],
	         			//url : '../../queryMedAffiOutInvList.do?isCheck=false&store_id='+ liger.get("store_code").getValue().split(",")[0]+'&in_id=${in_id}',
	         			pageSize : 500,
			...
			heightDiff:-20,url : 'queryMedAffiBackDetail.do?isCheck=false&in_id=${medAffiInMain.in_id}&store_id=${medAffiInMain.store_id}',
			...
			toolbar: { items: [
				...
				{ line:true },
				{ text: '退货确认', id:'confirm', click: confirm,icon:'account' , disabled: state == 2 ? false : true},
				{ line:true },
				...
				editor : {
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : '../../queryMedAffiOutDetailInvList.do?isCheck=false&is_com=1&store_id=' 
    ...
    function in_imp(){
    	...
    	$.ligerDialog.open({
			...
			height: 500,
			width: 900,
			url: 'medAffiBackInImpPage.do?isCheck=false&'+para,
	...
	function audit(){
		...
		$.ligerDialog.confirm('确定审核?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("auditMedAffiBack.do", formPara, function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 2;
						change_button();
	...
	function unAudit(){
		...
		$.ligerDialog.confirm('确定消审?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("unAuditMedAffiBack.do", formPara, function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 1;
						change_button();
    ...
    function confirm(){
		...
		$.ligerDialog.confirm('确定退货确认?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("confirmMedAffiBack.do", formPara, function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 3;
						change_button();
	...
	function printDate(){
		...
		printTemplate("queryMedAffiBackByPrintTemlate.do",para); */
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
			<tr>
	            <td style="display: none;">
	            	<input name="group_id" type="text" id="group_id" value="${medAffiInMain.group_id}" ltype="text" />
	            	<input name="hos_id" type="text" id="hos_id" value="${medAffiInMain.hos_id}" ltype="text" />
	            	<input name="copy_code" type="text" id="copy_code" value="${medAffiInMain.copy_code}" ltype="text" />
	            	<input name="in_id" type="text" id="in_id" value="${medAffiInMain.in_id}" ltype="text" />
	            	<%-- <input name="money_sum" type="text" id="money_sum" value="${medAffiInMain.money_sum}" ltype="text" /> --%>
	            </td>
			</tr>
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="in_no" type="text" id="in_no" value="${medAffiInMain.in_no}" ltype="text" disabled="disabled" required="true" validate="{required:true}" />
	            </td>
	            <td align="right" class="l-table-edit-td" >
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="in_date" id="in_date" value="${medAffiInMain.in_date}" type="text" required="true" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" validate="{required:true}" />
	            </td>
	            <td align="right" class="l-table-edit-td"  >
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td" colspan="5">
	            	<input name="brief" type="text" id="brief" ltype="text" value="${medAffiInMain.brief}" validate="{required:false,maxlength:50}" />
	            </td>
			</tr>
	    </table>
    </form>
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%"  style="margin-top: 5px;">
			<tr>	
				<td align="center" class="l-table-edit-td" >
					制单人：${medAffiInMain.maker_name}
				</td>
				<td align="center" class="l-table-edit-td" >
					审核人：${medAffiInMain.checker_name}
				</td>
				<td align="center" class="l-table-edit-td" >
					确认人：${medAffiInMain.confirmer_name}
				</td>
			</tr>
				<tr>	
