...
/* 	  		grid.options.parms.push({name:'inv_code',value:liger.get("inv_code").getText().split(" ")[0]}); */
    	...
    	if(show_detail=="1"){
    		grid = $("#maingrid").ligerGrid({
    			columns: [{
                    ...
                    /* { display: '状态', name: 'state', align: 'left',minWidth: '80',editor: { type: 'select',data: medOutMain_state.Rows},render: fieldTypeRender} */
    		 	],
    			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedAffiOutCommon.do?isCheck=true&show_detail=1',
    			...
    			toolbar: { items: [
    				...
    				{ line:true },
    				{ text: '出库确认（<u>R</u>）', id:'confirm', click: confirm,icon:'account' },
    		...
    		grid = $("#maingrid").ligerGrid({
    			...
    			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedAffiOutCommon.do?isCheck=true&show_detail=0',
	...
	function loadHotkeys() {
		...
		hotkeys('C', copy);//复制
		hotkeys('V', offSet);//冲销
		hotkeys('R', confirm);//出库确认
		//hotkeys('P', printDate);//模板打印
	}
    function loadDict(){
		//字典下拉框
		autocomplete("#store_id", "../../queryMedStore.do?isCheck=false", "id", "text", true, true,{is_com : '1'},false,null,"180");
		autocomplete("#dept_code", "../../queryMedDept.do?isCheck=false", "id", "text", true, true,{is_last : '1'}, false,null,"200");
		autocompleteAsync("#bus_type_code", "../../queryMedBusType.do?isCheck=false", "id", "text", true, true, {codes:'5,7,9,28,30,32,33,34'},false,'28',"180");
		autocomplete("#sup_code", "../../queryHosSupDict.do?isCheck=false", "id", "text", true, true);//供应商

		autoCompleteByData("#state", medOutMain_state.Rows, "id", "text", true, true,null,null,null,"180");
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/med/affi/out/medAffiOutCommonAddPage.do?isCheck=false',
    ...
    function remove(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("deleteMedAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function audit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定审核吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditMedAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function unAudit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定销审?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("unAuditMedAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function copy(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要复制单据吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("copyMedAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    	...
    	if (data.length == 0){
			...
			if(data.length > 1){
				...
				$.ligerDialog.confirm('确定要冲销吗?', function (yes){
					if(yes){
						ajaxJsonObjectByUrl("offsetMedAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
							if(responseData.state=="true"){
								query();
							}
						});
	...
	function printSet(){
		...
		officeFormTemplate({template_code:"08019",use_id:useId});
		
		/* parent.parent.$.ligerDialog.open({url : 'hrp/med/affi/out/affiOutPrintSetPage.do?template_code=08020&use_id='+useId,
    ...
    function print(){
		...
		if (data.length == 0){
	    	...
	    	printTemplate("hrp/med/affi/out/queryMedAffiOutByPrintTemlate.do?isCheck=false",para); */
	    	
			 var para={
		    			paraId :in_id.substring(0,in_id.length-1) ,
		    			template_code:'08019',
		    			class_name:"com.chd.hrp.med.serviceImpl.affi.out.MedAffiOutCommonServiceImpl",
		    			method_name:"medAffiOutCommonTemplate",
		    			isPrintCount:false,//更新打印次数
		    			isPreview:true,//预览窗口，传绝对路径
		    			use_id:useId,
		    			p_num:1
