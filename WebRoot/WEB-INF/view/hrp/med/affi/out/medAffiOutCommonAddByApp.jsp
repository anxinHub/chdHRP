...
<script src="<%=path%>/lib/hrp/med/med.js"	type="text/javascript"></script>
    <script type="text/javascript">
     ...
     function validateGrid() {  
  		...
  		$.each(data,function(i, v){
 			...
 			if(targetMap.get(key)== null || targetMap.get(key) == 'undefined' || targetMap.get(key) == ""){
 				...
 				msg += targetMap.get(key)+"与"+value+"药品编码、生成批号、条形码不能重复" + "\n\r";
 			}
  		});
  		if(msg != ""){
     ...
     function  save(){
    	if(validateGrid()){
	        ...
	        ajaxJsonObjectByUrl("addMedAffiOutCommon.do",formPara,function(responseData){
    ...
    function loadDict(){
    	//字典下拉框
    	autocomplete("#bus_type_code", "../../queryMedBusType.do?isCheck=false", "id", "text", true, true, {codes:'9,28,30,33,34'},false, "${medAffiOut.bus_type_code}");
		autocomplete("#store_code", "../../queryMedStore.do?isCheck=false", "id", "text", true, true,"", false, "${medAffiOut.store_id},${medAffiOut.store_no}");
    ...
    function loadHead() {
    	var detail_data = ${medAffiOutDetail};
			...
			columns : [{
				display : '药品编码',name : 'inv_code',minWidth : 100,align : 'left',
					...
					editor : {
						...
						valueField: 'id', 
						textField: 'text',
						url : '../../queryMedInvDict.do?isCheck=false&is_com=1',
					    keySupport:true,
						selectBoxWidth: 660,
						selectBoxHeight: 240,
				      	autocomplete: true,
					...
					render : function(rowdata, rowindex, value) {
						return rowdata.price == null ? "" : formatNumber(rowdata.price, '${sessionScope.med_para_map["08006"]}', 1);
					}
				}, {display : '金额', name : 'amount_money', minWidth : 80, type : 'number', align : 'right',
					render : function(rowdata, rowindex, value) {
						return rowdata.amount_money == null ? "" : formatNumber(rowdata.amount_money, '${sessionScope.med_para_map["08005"]}', 1);
					}
					...
					render : function(rowdata, rowindex, value) {
						return rowdata.sell_price == null ? "" : formatNumber(rowdata.sell_price, '${sessionScope.med_para_map["08006"]}', 1);
					}
				},{display : '零售金额', name : 'sell_money', type : 'number', minWidth : 80, align : 'right',
					render : function(rowdata, rowindex, value) {
						return rowdata.sell_money == null ? "" : formatNumber(rowdata.sell_money, '${sessionScope.med_para_map["08005"]}', 1);
					}
				},{
					display : '货位名称(E)',
					name : 'location_id',
    ...
    function f_onAfterEdit(e) {
		if(e.column.name=='inv_name' && e.value != ''){	
			...
			//alert(paras);
			$.ligerDialog.open({
				url: "medMedAffiOutInvFifoPage.do?isCheck=false&paras="+paras, height: 450,width: 1000, 
