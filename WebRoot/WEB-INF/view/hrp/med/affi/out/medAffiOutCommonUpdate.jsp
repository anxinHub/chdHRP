  ...
  <head>
   ...
   //键盘事件
	...
	function loadDict(){
    	...
    	autocomplete("#bus_type_code", "../../queryMedBusType.do?isCheck=false", "id", "text", true, true,{codes:'5,7,9,28,30,32,33,34'},false,'${bus_type_code}','180');
    	$("#dept_id").ligerComboBox({
    		...
    		onSelected: function (selectValue){   			
    			if(selectValue !=null ){   				
    				...
    				autocomplete("#dept_emp", "../../queryMedEmpDict.do?isCheck=false", "id", "text", true, true,dept_emp_paras,false,false,180);
    			}
			}
    	 });
    	
    	autocomplete("#dept_emp", "../../queryMedEmpDict.do?isCheck=false", "id", "text", true, true,"",false,'',180);
    	liger.get("dept_emp").setValue('${dept_emp}');
		liger.get("dept_emp").setText('${dept_emp_name}');
    	autocomplete("#use_code", "../../queryMedOutUse.do?isCheck=false", "id", "text", true, true, "", true, "${use_code}", '180');
    	autocomplete("#proj_id", "../../queryMedProj.do?isCheck=false", "id", "text", true, true, "", false, "${proj_id}","","",515);
	...
	function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
					...
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : '../../queryMedAffiOutInvList.do?isCheck=false&store_id=' 
         					...
         					+ liger.get("bus_type_code").getValue() +'&out_id=${out_id}',
			...
			enabledEdit : state == 1 ? true : false,fixedCellHeight:true,heightDiff:-20,
			onBeforeEdit : f_onBeforeEdit, 
			onBeforeSubmitEdit : f_onBeforeSubmitEdit, 
			onAfterEdit : f_onAfterEdit,
			url:'queryMedAffiOutDetaill.do?isCheck=false&out_id=${out_id}&store_id='+store_id,
			onsuccess:function(){
				//is_addRow();
			},		
			...
			toolbar: { items: [
				...
				{ line:true },
				{text : '添加出库单（<u>A</u>）',id : 'newAnother',click : newAnother,icon : 'add', disabled: state == 1 ? false : true},
				{line : true},
	...
	//出库确认
 	...
 	function upOpen(){
 		if('${out_idUp}' == $("#out_id").val()){
 			...
 			var obj = '${group_id}'+","+'${hos_id}'+","+'${copy_code}'+"," + '${out_idUp}';
 	...
 	function nextOpen(){
 		if('${out_idNext}' == $("#out_id").val()){
			$.ligerDialog.confirm('本单据为最后一张单据,是否跳转添加单据', function (yes){
 			...
 			var obj = '${group_id}'+","+'${hos_id}'+","+'${copy_code}'+"," + '${out_idNext}';
 			parentFrameUse().openUpdate(obj);
 			this_close();
 		}
 	}
	...
	function printDate(){
		...
		printTemplate("queryMedAffiOutByPrintTemlate.do",para); */
