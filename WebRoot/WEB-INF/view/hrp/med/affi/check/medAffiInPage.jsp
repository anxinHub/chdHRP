...
<jsp:include page="${path}/inc_jquery_1.9.0.jsp" />
     ...
     function validateGrid() {  
    	//主表
 		...
 		//alert(JSON.stringify(data));
  		...
  		$.each(data,function(i, v){
 			...
 			if (v.inv_id == "" || v.inv_id == null || v.inv_id == 'undefined') {
 				rowm+="[药品]、";
 				//$.ligerDialog.warn("第"+(i+1)+"行[药品]不能为空");  
 				//return false;  
 			}else{ 
				...
				var key=v.inv_id +"|"+v.batch_no+"|"+v.sn+"|"+v.location_id+"|"+v.bar_code;
	 			...
	 			if(targetMap.get(key)== null || targetMap.get(key) == 'undefined' || targetMap.get(key) == ""){
					...
					msg += targetMap.get(key)+"与"+value+"药品编码、生产批号、条形码、货位、院内码不能重复" + "<br>";
	 			}
	 			rows = rows + 1;
				store_inv += v.inv_id + ",";
 		...
 		if(store_inv.length > 0){
			...
			is_flag = ajaxJsonObjectByUrl("../../existsMedAffiSupIncludeInv.do?isCheck=false", para, function (responseData){
		...
		if(sup_inv.length > 0){
			...
			var is_flag = ajaxJsonObjectByUrl("../../existsMedAffiSupIncludeInv.do?isCheck=false", para, function (responseData){
				if(responseData.state=="false"){
    ...
    function loadDict(){
    	//字典下拉框
    	autocomplete("#bus_type_code", "../../queryMedBusType.do?isCheck=false", "id", "text", true, true, {codes:'27,8'}, false, "${medAffiIn.bus_type_code}");
		...
		autocomplete("#proj_code", "../../queryMedProj.do?isCheck=false", "id", "text", true, true, "", false, false,"","",530);
					...
					editor : {
						...
						grid : {
							columns : [ 
								...
								{display : '药品类别', name : 'med_type_code', width : 100, align : 'left'}, 
    ...
    function before_no(){
		...
		ajaxJsonObjectByUrl("queryMedAffiInBeforeNo?isCheck=false",formPara,function(responseData){
			if(responseData.state=="true"){
	...
	function next_no(){
		...
		ajaxJsonObjectByUrl("queryMedAffiInNextNo.do?isCheck=false",formPara,function(responseData){
			if(responseData.state=="true"){
				parent.update_open(responseData.update_para);
				this_close();
			}else{
	...
	<form name="form1" method="post"  id="form1" >
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="in_date" id="in_date" type="text" readOnly value="${medAffiIn.in_date}" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
	            </td>
