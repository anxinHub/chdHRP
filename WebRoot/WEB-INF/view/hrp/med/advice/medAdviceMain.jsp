...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedAdvice.do',
                     width: '100%', height: '94%', checkbox: true,rownumbers:true,delayLoad :true,
                     selectRowButtonOnly:true,//heightDiff: -10,
                     toolbar: { items: [
                    	 {text : '查询',id : 'search',click : query,icon : 'search'},
                    	 {line : true},
                     	 { text: '生成', id:'init', click: initMedAdvice,icon:'save' },
                     	 {line : true}, 
	...
	function printSet() {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/med/advice/medAdviceInPrintSetPage.do?template_code=08016&use_id=' + useId,
			data : {},
	...
	function print() {
		...
		$(data).each(function() {
			if (this.state != 2) {
				receive_ids = receive_ids + this.receive_id + "<br>";
			}
		...
		var para = {
			template_code : '08016',
			class_name : "com.chd.hrp.med.serviceImpl.advice.MedAdviceServiceImpl",
			method_name : "queryMedAdviceByPrintTemlateNewPrint",
			isSetPrint : false,//是否套打，默认非套打
			isPreview : true,//是否预览，默认直接打印

			paraId : receive_id.substring(0, receive_id.length - 1),
			use_id : useId,
			p_num : 1
		};
    ...
    function initMedAdvice(){
		...
		if(data.length == 0) {
			$.ligerDialog.error('请选择行');
			 ...
			 ajaxJsonObjectByUrl("initMedAdvice.do",formPara,function(responseData){
    ...
    function loadDict(){
		...
		autocomplete("#inv_id",
				"../queryMedInv.do?isCheck=false","id",
			    "text",true,true,null,false);
		autocomplete("#charge_item_id",
				"../../cost/queryChargeItemArrt.do?isCheck=false","id",
			    "text",true,true,null,false);
		autocomplete("#dept_id", "../queryMedDept.do?isCheck=false", "id", "text", true, true);
