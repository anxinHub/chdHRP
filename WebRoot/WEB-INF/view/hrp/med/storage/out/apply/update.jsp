     ...
     function  save(){
		grid.endEdit();
    	 if(validateGrid()){
 	        ...
 	        ajaxJsonObjectByUrl("updateMedCommonOutApply.do",formPara,function(responseData){
 	            if(responseData.state=="true"){
 	            	queryDetail();
...
function loadForm(){
    ...
    function loadDict(){
		...
		autocompleteAsync("#apply_dept", "../../../queryMedDeptDictDate.do?isCheck=false", "id", "text", true, true, {is_last: 1,is_write:'1'});
		...
		//autocompleteAsync("#app_emp", "../../../queryMedEmp.do?isCheck=false", "id", "text", true, true, {dept_id : "${medApplyMain.dept_id}"});
		autocompleteAsync("#app_emp", "../../../queryMedEmp.do?isCheck=false", "id", "text", true, true);
		liger.get("app_emp").setValue("${medApplyMain.app_emp}");
    	liger.get("app_emp").setText("${medApplyMain.emp_code} ${medApplyMain.emp_name}");
    	
		autocomplete("#response_emp", "../../../queryMedEmp.do?isCheck=false", "id", "text", true, true);
		liger.get("response_emp").setValue("${medApplyMain.response_emp}");
    	liger.get("response_emp").setText("${medApplyMain.response_code} ${medApplyMain.response_name}");
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [ {
				...
				editor : {
					...
					grid : {
						columns : [{
							...
							display : '是否灭菌药品', name : 'is_disinfect', width : 90, align : 'left',
							render : function(rowdata, rowindex, value) {
								return value == 1 ? '是' : '否';
							}
						}, {
				...
				display : '出库数量', name : 'out_amount', width : 90, type : 'number', align : 'left',
				render : function(rowdata, rowindex, value) {
					return value == null ? "" : formatNumber(value, 2, 1);
				}
			}, {
				display : '采购数量', name : 'pur_amount', width : 90, type : 'number', align : 'left',
				render : function(rowdata, rowindex, value) {
					return value == null ? "" : formatNumber(value, 2, 1);
				}
			}, {
			...
			url : 'queryMedCommonOutApplyDetail.do?isCheck=false&store_id='+liger.get("store_code").getValue().split(",")[0],
			checkbox : true, 
			...
			toolbar : {
				items : [ {
					...
					text : '消审（<u>X</u>）', id : 'unAudit', click : unAudit, icon : 'unaudit', disabled: state == 2 || state == 4  ? false : true
				}, {
					line : true
				}, */ {
					text : '发送（<u>F</u>）', id : 'send', click : send, icon : 'logout', disabled: state == 1 ? false : true
				}, {
					line : true
				},{
					text : '选择药品（<u>C</u>）', id : 'choice_inv', click : choice_inv, icon : 'add'
				}]
			}
		});
	...
	function choice_inv(){
		...
		$.ligerDialog.open({url: "medCommonOutApplyChoiceInvPage.do?isCheck=false&store_id="+store_id+"&store_no="+store_no,
  ...
  //发送
	function send(){
		...
		var ParamVo =[];
		ParamVo.push(
			"${medApplyMain.group_id}"   +"@"+ 
			"${medApplyMain.hos_id}"  +"@"+ 
			"${medApplyMain.copy_code}"   +"@"+ 
			"${medApplyMain.apply_id}"
		) 
		$.ligerDialog.confirm('确定发送?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("sendMedCommonOutApply.do",{ParamVo : ParamVo.toString()},function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 3;
						changeButton();
	...
	function audit(){
		...
		$.ligerDialog.confirm('确定审核?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("auditMedCommonOutApply.do",{ParamVo : ParamVo.toString()},function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 2;
						changeButton();
	...
	function unAudit(){
		...
		$.ligerDialog.confirm('确定消审?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("unAuditMedCommonOutApply.do",{ParamVo : ParamVo.toString()},function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 1;
						changeButton();
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="apply_no" type="text" id="apply_no" value="${medApplyMain.apply_no}" ltype="text" disabled="disabled" required="true" validate="{required:true}" />
	            </td>
	            <td align="right" class="l-table-edit-td" > 
	            	<span style="color:red">*</span>申请日期：
	            </td>
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="app_date" id="app_date" type="text" value="${medApplyMain.app_date}" required="true" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd HH:mm:ss'})" validate="{required:true}" />
	            </td>
	            <td align="right" class="l-table-edit-td"  >
			...
			<tr>
	            ...
	            <td align="left" class="l-table-edit-td" colspan="5">
	            	<input name="brief" type="text" id="brief" ltype="text" value="${medApplyMain.brief}" validate="{required:false,maxlength:50}" />
	            </td>
			</tr>
	    </table>
    </form>
