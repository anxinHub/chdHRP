...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedCommonOutApplyN.do',
    ...
    function loadDetailHead(){
    	detailGrid = $("#detailgrid").ligerGrid({
			columns : [ {
				...
				display : '药品名称', name : 'inv_id', textField : 'inv_name', minWidth : 240, align : 'left',
			}, {
				display : '规格型号', name : 'inv_model', minWidth : 180, align : 'left'
			}, {
				display : '生成厂商', name : 'fac_name', minWidth : 200, align : 'left'
			}, {
				display : '计量单位', name : 'unit_name', minWidth : 80, align : 'left'
			}, {
				display : '请领数量', name : 'app_amount', minWidth : 80, type : 'number', align : 'left',
				render : function(rowdata, rowindex, value) {
				...
				display : '出库数量', name : 'out_amount', minWidth : 80, type : 'number', align : 'left',
				render : function(rowdata, rowindex, value) {
					return value == null ? "" : formatNumber(value, 2, 1);
				}
			}, {
				display : '采购数量', name : 'pur_amount', minWidth : 80, type : 'number', align : 'left',
				render : function(rowdata, rowindex, value) {
					return value == null ? "" : formatNumber(value, 2, 1);
				}
			}, {
				...
				display : '是否代销药品', name : 'is_com', minWidth : 100, align : 'left',
				render : function(rowdata, rowindex, value) {
					return value == 1 ? '是' : '否';
				}
			}, {
				display : '是否科室库管理', name : 'is_sec_whg', minWidth : 100, align : 'left',
				render : function(rowdata, rowindex, value) {
					return value == 1 ? '是' : '否';
				}
			}, {
				display : '是否关闭', name : 'is_closed', minWidth : 100, align : 'left',
				render : function(rowdata, rowindex, value) {
					return value == 1 ? '是' : '否';
				}
			}, {
				display : '备注(E)', name : 'note', minWidth : 120, align : 'left',
			} ],
	...
	function qxsend(){
		...
		if (data.length == 0){
			...
			ajaxJsonObjectByUrl("backsendMedCommonOutApplyN.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function loadDict(){
		...
		autoCompleteByData("#check_state", medApplyMain_checkState.Rows, "id", "text", true, true,'',true);
