...
<script type="text/javascript">
 ...
 function loadForm(){
	...
	function checkMedOutFifo(){
        ...
        ajaxJsonObjectByUrl("queryMedOutDetailByMatch.do?isCheck=false",para,function(responseData){
    ...
    function loadDict(){
    	...
    	autocomplete("#dept_match_id", "../../../queryMedDeptMatch.do?isCheck=false", "id", "text", true, true,"",true,false,'180');
    ...
    function loadHead(){
		...
		grid = $("#maingrid").ligerGrid({
           columns: [
                ...
                { display: '药品名称', name: 'inv_name', align: 'left', minWidth: 140},
                { display: '规格型号', name: 'inv_model', align: 'left', minWidth: 160},
                { display: '计量单位', name: 'unit_name', align: 'left', minWidth: 50},
                { display: '数量', name: 'amount', align: 'left', minWidth: 80},
                { display: '存放仓库名称', name: 'store_name', minWidth: 80},
				],
				usePager : false,fixedCellHeight:true,
				dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedDeptMatchByMatchd.do?isCheck=false',parms :para,
				isScroll : true,selectRowButtonOnly:true,heightDiff: 10,width: '100%', height: '100%',delayLoad:true,
				toolbar: { items: [
