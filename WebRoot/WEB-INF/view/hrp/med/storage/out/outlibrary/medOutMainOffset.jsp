...
<script type="text/javascript">
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	$("#dept_id").ligerComboBox({width:160,disabled:true,cancelable: false});
	...
	function loadHead(){
		...
		grid = $("#maingrid").ligerGrid({
			...
			usePager : false,width : '100%',height : '100%',enabledEdit : true,fixedCellHeight:true,heightDiff:-20,data:medOutDetail,
    ...
    function showBatchSn(row, detailPanel,callback){
		...
		detailGrid =$(batchSn).css({'margin-top':10, 'margin-left':60}).ligerGrid({
    		columns: [{ 
    			...
    			display: '批次', name: 'batch_sn', align : 'left' ,width : 70
    		}, { 
     			display : '库存', name : 'cur_amount', minWidth : 70, align : 'left'
     		}, { 
     			display : '即时库存', name : 'imme_amount', minWidth : 70, align : 'left', 
     		}, { 
    			...
    			render : function(rowdata, rowindex, value) {
    				rowdata.price = value == null ? "" : value;
    				return value == null ? "" : formatNumber(value, '${p08006 }', 1);
    			}
    		}, { 
    			display: '金额', name: 'amount_money', align: 'right', width:80,
    			...
    			render : function(rowdata, rowindex, value) {
    				rowdata.sale_price = value == null ? "" : value;
    				return value == null ? "" : formatNumber(value, '${p08006 }', 1);
    			}
    		}, { 
    			display: '批发金额', name: 'sale_money', align: 'right', width:80,
    			...
    			render : function(rowdata, rowindex, value) {
    				rowdata.sell_price = value == null ? "" : value;
    				return value == null ? "" : formatNumber(value, '${p08006 }', 1);
    			} 
    		}, { 
    			display: '零售金额', name: 'sell_money', align: 'right', width:80,
