...
<script type="text/javascript">
     ...
     $(function (){
		...
		$("#dept_id").bind("change",function(){
			if(isEmpByDept && liger.get("dept_id").getValue()){
		    	...
		    	autocomplete("#dept_emp", "../../../queryMedEmpDict.do?isCheck=false", "id", "text", true, true, para, false);
			}
		});
     ...
     function  save(){
        ...
        var formPara={
        		...
        		is_dir:$("#is_dir").val(),
        		app_date : $("#app_date").val() == null ? "" : $("#app_date").val() ,//2017/08/10  即墨  申领日期
        		out_detail_data:JSON.stringify(out_detail_data)
		};
        ajaxJsonObjectByUrl("addMedOutMain.do",formPara,function(responseData){
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	autocompleteAsync("#bus_type_code", "../../../queryMedBusType.do?isCheck=false", "id", "text", true, true,bus_type_code_paras,true);
    	
    	//autocompleteAsync("#store_id", "../../../queryMedStore.do?isCheck=false", "id", "text", true, true, {is_com : 0}, true);
       
    	autocompleteAsync("#store_id", "../../../queryMedStoreDictDate.do?isCheck=false", "id", "text", true, true, isUseAffiStore ? "" : {is_com : 0,is_write:'1'}, true);
    	
    	loadDictDeptId();

    	autocomplete("#dept_emp", "../../../queryMedEmpDict.do?isCheck=false", "id", "text", true, true, isEmpByDept ? {dept_id: liger.get("dept_id").getValue().split(",")[0]} : "", false); 
    ...
    function loadDictDeptId (value) {
    	autocompleteAsync("#dept_id", "../../../queryMedDeptDictDate.do?isCheck=false", "id", "text", true, true, {is_last: 1,is_write:"1"}, false, value);
    }
	...
	function loadHead(){
		...
		grid = $("#maingrid").ligerGrid({
			...
			toolbar: { items: [
				...
				/* { text: '选择药品', id:'add', click: choice_invOld,icon:'add' },
				{ line:true } */
	...
	function choice_inv(){
		...
		$.ligerDialog.open({url: "medOutMainChoiceInvPage.do?isCheck=false&store_id="+store_id+"&store_text="+store_text,
	...
	function choice_invOld(){
		...
		$.ligerDialog.open({url: "medOutMainFifoPage.do?isCheck=false&paras="+paras,
				height: 500,width: 900, title:'选择药品',modal:true,showToggle:false,showMax:true,
				showMin: false,isResize:true,
				buttons: [ 
				    { text: '确定',onclick: function (item, dialog) { 
							dialog.frame.checkMedOutFifo(); 
						},cls:'l-dialog-btn-highlight' }, 
  ...
  //删除行集合
    ...
    function showBatchSn(row, detailPanel,callback){
		...
		detailGrid =$(batchSn).css({'margin-top':10, 'margin-left':60}).ligerGrid({
    		...
    		width: '65%',height: '80%',columnWidth:80, data : f_getInvDetailData(row),
    		toolbar: { items: [
    ...
    function medOutMatchedImport(){//配套导入
		...
		$.ligerDialog.open({
			url: "medOutMainMatchedPage.do?isCheck=false&paras="+paras, 
			height: 500,width: 900, title:'配套导入',
			modal:true,showToggle:false,showMax:true,showMin: false,isResize:true
			//,buttons: [ { text: '选择', onclick: function (item, dialog) { dialog.frame.checkMedOutFifo(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] 
		});
