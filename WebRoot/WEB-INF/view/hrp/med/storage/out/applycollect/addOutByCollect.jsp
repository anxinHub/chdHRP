...
<script type="text/javascript">
     ...
     function  save(){
    	...
    	if(validateGrid()){
            ...
            if(is_com == "0"){
            	pageTitle = "出库单修改";
            	saveUrl = "../applycheck/addMedOutMainByApp.do?isCheck=false";
            	pageUrl = "hrp/med/storage/out/outlibrary/medOutMainUpdatePage.do?isCheck=false";
            }else{
            	pageTitle = "代销出库单修改";
            	saveUrl = "../applycheck/addMedAffiOutByApp.do?isCheck=false";
            	pageUrl = "hrp/med/affi/out/medAffiOutCommonUpdatePage.do?isCheck=false";
            }
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	autocompleteAsync("#store_id", "../../../queryMedStore.do?isCheck=false", "id", "text", true, true, {is_com : is_com});
    	...
    	//autocomplete("#dept_emp", "../../../queryMedEmpDict.do?isCheck=false", "id", "text", true, true, {dept_id: '${medOutMain.dept_id}'},false,"${medOutMain.dept_emp},${medOutMain.emp_no}"); 
	...
	function loadHead(){
		...
		grid = $("#maingrid").ligerGrid({
				...
				editor : {
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : (is_com == "0" ? '../../../queryMedStockInvList.do?isCheck=false&is_com=0' : '../../../queryMedAffiOutInvList.do?isCheck=false&is_com=1')
	         					+'&store_id=' + liger.get("store_id").getValue().split(",")[0] 
	         					+ '&bus_type_code=' + liger.get("bus_type_code").getValue(),
	         			pageSize : 200,
			...
			toolbar: { items: [
				...
				{ line:true },
				/* { text: '定向出库（<u>I</u>）', id:'dirImp', click: medOutDirImport,icon:'up' },
				{ line:true },
				{ text: '配套导入（<u>M</u>）', id:'matchImp', click: medOutMatchedImport,icon:'up' },
				{ line:true },
				{ text: '历史使用导入（<u>H</u>）', id:'hisImp', click: medOutHistoryImport,icon:'up' },
				{ line:true } */
			]}
		});
	...
	function loadHotkeys() {
		hotkeys('D', deleteRow);
		hotkeys('I', medOutDirImport);
		hotkeys('M', medOutMatchedImport);
		hotkeys('H', medOutHistoryImport);
	}
		...
		detailGrid =$(batchSn).css({'margin-top':10, 'margin-left':60}).ligerGrid({
    		columns: [{ 
				...
				editor : {
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : (is_com == "0" ? '../../../queryMedStockInvDetailList.do?isCheck=false&is_com=0' : '../../../queryMedAffiOutDetailInvList.do?isCheck=false&is_com=1')
    			...
    			display: '批次', name: 'batch_sn', align : 'left' ,width : 80
     			...
     			display : '库存', name : 'cur_amount', minWidth : 80, align : 'left'
     		}, { 
     			display : '即时库存', name : 'imme_amount', minWidth : 80, align : 'left', 
     		}, { 
