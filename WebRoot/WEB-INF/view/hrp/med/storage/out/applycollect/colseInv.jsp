...
<head>
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
					...
					url : 'queryMedApplyCloseInvInfo.do?isCheck=false',
	...
	function cancleCloseInv(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要取消关闭的药品？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("updateMedApplyCancleCloseInv.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData){
	...
	function loadDict() {
		...
		autocomplete("#store_code", "../../../queryMedStore.do?isCheck=false", "id", "text", true, true, "", false, false, 180);
