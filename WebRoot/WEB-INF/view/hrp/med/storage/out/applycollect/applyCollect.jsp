...
<script type="text/javascript">
    ...
    function  query(){
    	...
    	is_com = $("#is_com input:radio:checked").val();
		grid.options.parms.push({
			name : 'is_com',
			value : is_com
		}); 
		...
		grid.options.parms.push({
			name : 'is_enough',
			value : $("#is_enough input:radio:checked").val()
		}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			columns: [{
					display: '申请单号', name: 'apply_no', align: 'left', width: 90,
					...
					render : function(rowdata, rowindex, value) {
		 			...
		 			display: '药品名称', name: 'inv_name', align: 'left', width: 140,
		 		},{ 
					...
					display : '备注(E)', name : 'note', width : 80, align : 'left',
		 			...
		 			display: '处理状态', name: 'do_state', align: 'left', minWidth: 80,
		 			render: function(rowdata, rowindex, value){
		 				if(value == 1){
		 					return "未完成";	
		 				}else if(value == 2){
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedCommonOutApplyCollect.do?isCheck=false',
			...
			toolbar: { items: [
				...
				{ line:true },
				{ text: '查看关闭药品（<u>V</u>）', id:'queryColseInv', click: queryColseInv, icon:'query' }
			]},  
			...
			onBeforeCheckRow : function(checked, rowdata, rowindex, rowobj){
				if(checked){
					/* if(rowdata.store_id == '0'){
						$.ligerDialog.warn("药品【"+rowdata.inv_name+"】没有维护默认申领库房");
						return false;
					} */
					/* var para = rowdata.store_id+","+rowdata.dept_id;
					if(store_dept != ""){
						if(store_dept != para){
							$.ligerDialog.warn("所选药品响应库房与领用科室需一致");
							return false;
						}
					}else{
						store_dept = para;
    ...
    function queryColseInv(){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/med/storage/out/applycollect/colseInvPage.do?isCheck=false',
		...
		if (data.length == 0){
			...
			$.ligerDialog.prompt('退回原因', true, function (yes, value){
				if(yes){
					...
					ajaxJsonObjectByUrl("backMedCommonOutApplyCollect.do?",paras,function (responseData){
	...
	function closeInv(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要关闭单据中的药品？', function (yes){
				if(yes){
	...
	function out(){
		...
		if (data.length == 0){
			...
			//验证库存
			ajaxJsonObjectByUrl("checkMedCommonOutApplyCollectForAdd.do?isCheck=false", paras, function(responseData){
	            if(responseData.state=="true"){
					$.ligerDialog.confirm(is_com == '1' ? '确定生成代销出库单？' : '确定生成出库单？', function (yes){
						if(yes){
							parent.$.ligerDialog.open({
								...
								height: $(window).height(),
								width: $(window).width(),
								url: 'hrp/med/storage/out/applycollect/addOutPage.do?isCheck=false' + paras.toString(),
	...
	function tran(){
		...
		if (data.length == 0){
			...
			paras += "&apply_ids=" + apply_ids + "&detail_ids=" + detail_ids;
			//验证仓库
	        ajaxJsonObjectByUrl("existsMedCommonOutApplyCollectStoreManage.do?isCheck=false", paras, function(responseData){
				if(responseData.state=="true"){
					//验证库存
					ajaxJsonObjectByUrl("checkMedCommonOutApplyCollectForAdd.do?isCheck=false", paras, function(responseData){
			            if(responseData.state=="true"){
			    			$.ligerDialog.confirm(is_com == '1' ? '确定生成代销调拨单？' : '确定生成调拨单？', function (yes){
			    				if(yes){
			    					parent.$.ligerDialog.open({
			    						...
			    						url: 'hrp/med/storage/out/applycollect/addTranPage.do?isCheck=false' + paras.toString(),
			    						modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, 
				    					parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
			    					});
			    				}
			...
			$.ligerDialog.error('请选择行');
				...
				paras += "&apply_ids=" + apply_ids + "&detail_ids=" + detail_ids;
						...
						parent.$.ligerDialog.open({
							...
							height: $(window).height()-50,
							width: $(window).width()-100,
							url: 'hrp/med/storage/out/applycollect/addReqPage.do?isCheck=false' + paras.toString(),
							modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, 
	    					parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
						});
	...
	function batchReq(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定汇总生成科室需求计划？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("addMedReqByApplyCollect.do?isCheck=false", paras, function(responseData){
			            if(responseData.state=="true"){
							query();
			            }
					});
	...
	function batchPur(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定汇总生成采购计划？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("addMedPurByApplyCollect.do?isCheck=false", paras, function(responseData){
			            if(responseData.state=="true"){
							query();
			            }
					});
    ...
    function loadDict(){
		...
		autoCompleteByData("#do_state", medApplyDetail_doState.Rows, "id", "text", true, true,null,null,'1');
		autocomplete("#store_code", "../../../queryMedStoreDictDate.do?isCheck=false", "id", "text", true, true,{is_write:'1'});
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" >
        ...
        <tr>
			...
			<td id="is_com" align="left" class="l-table-edit-td">
    			...
    			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    			<input name="is_bar" type="checkbox" id="is_bar" ltype="text" />只显示条码药品
			</td>
        	<td align="right" class="l-table-edit-td" >
				库存状态：
			</td>
