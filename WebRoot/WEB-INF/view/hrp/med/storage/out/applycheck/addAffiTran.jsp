...
<script type="text/javascript">
	...
	var apply_id = "${medAffiTran.apply_id}";
	var is_add = "${medAffiTran.is_add}";
	...
	$(function (){
     ...
     function  save(){
        ...
        //ajaxJsonObjectByUrl("../../../affi/out/addMedAffiTranMain.do?isCheck=true",formPara,function(responseData){
        ajaxJsonObjectByUrl("addMedAffiTranMainByApp.do?isCheck=true",formPara,function(responseData){
            if(responseData.state=="true"){	
            	...
            	parent.parent.$.ligerDialog.open({
					...
					url: 'hrp/med/affi/tran/medAffiTranMainUpdatePage.do?isCheck=false&'+paras.toString(),
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	if("${medAffiTran.tran_no}"){
			$("#tran_no").val("${medAffiTran.tran_no}");
		}
    	autoCompleteByData("#tran_type", medTranMain_tranType.Rows, "id", "text",true, true,'',false,"${medAffiTran.tran_type}",'180');
    	autoCompleteByData("#bus_type", medTranMain_busType.Rows, "id", "text",true, true,'',false,"${medAffiTran.bus_type}",'180');
        ...
        liger.get("tran_method").setValue("${medAffiTran.tran_method}");
        if("${medAffiTran.tran_method}" == 1){
		...
		if("${medAffiTran.out_hos_id}"){
			liger.get("out_hos_id").setValue('${medAffiTran.out_hos_id}');
			liger.get("out_hos_id").setText('${medAffiTran.out_hos_code} ${medAffiTran.out_hos_name}');
    	}
		autocompleteAsync("#out_store_id", "../../../queryMedStore.do?isCheck=false", "id","text", true, true, {hos_id : "${medAffiTran.out_hos_id}"}, false, false, '180');
		if("${medAffiTran.out_store_id}"){
			liger.get("out_store_id").setValue('${medAffiTran.out_store_id},${medAffiTran.out_store_no}');
			liger.get("out_store_id").setText('${medAffiTran.out_store_code} ${medAffiTran.out_store_name}');
		}
		autocompleteAsync("#in_hos_id", "../../../queryMedHosInfo.do?isCheck=false", "id","text", true, true, "", false, false, '180');
		if("${medAffiTran.in_hos_id}"){
			liger.get("in_hos_id").setValue('${medAffiTran.in_hos_id}');
			liger.get("in_hos_id").setText('${medAffiTran.in_hos_code} ${medAffiTran.in_hos_name}');
		}
		autocompleteAsync("#in_store_id", "../../../queryMedStore.do?isCheck=false", "id","text", true, true, {hos_id : "${medAffiTran.in_hos_id}"}, false, false, '180');
		if("${medAffiTran.in_store_id}"){
			liger.get("in_store_id").setValue('${medAffiTran.in_store_id},${medAffiTran.in_store_no}');
			liger.get("in_store_id").setText('${medAffiTran.in_store_code} ${medAffiTran.in_store_name}');
		}
		$("#tran_date").ligerTextBox({width : 180});
		if("${medAffiTran.tran_date}"){
			$("#tran_date").val("${medAffiTran.tran_date}");
		}else{
			autodate("#tran_date");
		}
	} 
    ...
    function loadHead(){
		var medTranDetail = ${medAffiTranDetail};
				...
				editor : {
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : '../../queryMedAffiInvDetailList.do?isCheck=false&store_id=' 
				...
				<form name="form1" method="post"  id="form1" >
			       ...
			       <tr>
			        ...
			        <tr>
			            ...
			            <td align="left" class="l-table-edit-td" colspan="7"><input name="brief" type="text" id="brief" ltype="text"  value="${medAffiTran.brief}" validate="{maxlength:50}" /></td>
			            <td align="left"></td>
			        </tr>
			    </table>
			    </form>
