...
<script type="text/javascript">
    ...
    var apply_id = "${medReqMain.apply_id}";
	var is_add = "${medReqMain.is_add}";
	...
	function save(){
		...
		if(validateGrid()){
			...
			//ajaxJsonObjectByUrl("${path}/WEB-INF/view/hrp/med/departrequried/plan/addMedDeptRequriedPlan.do?isCheck=true", formPara, function(responseData) {
			ajaxJsonObjectByUrl("addMedRequriedMainByApp.do?isCheck=true", formPara, function(responseData) {
				if(responseData.state=="true"){
					...
					parent.parent.$.ligerDialog.open({
						...
						url: 'hrp/med/require/dept/plan/medDeptRequriedPlanUpdatePage.do?isCheck=false&req_code='+responseData.req_code,
	...
	function loadHead() {
		...
		grid = $("#maingrid").ligerGrid({
		       columns: [ 
						 ...
						 { display: '药品名称(E)', name: 'inv_name', align: 'left',width:'240',
							 editor : {
									type : 'select',
									valueField : 'inv_name',
									textField : 'inv_name',
									...
									grid : {
										...
										url : '../../../queryMedInvListDept.do?isCheck=false&dept_id=${medReqMain.dept_id}'+
												'&dept_no=${medReqMain.dept_no}&store_id=${medReqMain.store_id}',
										pageSize : 200
									},
									keySupport : true,
									rownumbers : true,
						...
						// { display: '需求日期(E)', name: 'rdate', type: 'date', format: 'yyyy-MM-dd', width: 100, editor: { type: 'date'},width:'100' },
						 ...
						 { display: '供应商', name : 'sup_code', textField : 'sup_name', minWidth : 160, align : 'left',
								editor : {
									type : 'select',
									valueField : 'id',
									textField : 'text',
									url : '../../queryMedSupByInvId.do?isCheck=false',
									keySupport : true,
									autocomplete : true,
								},
								render : function(rowdata, rowindex, value) {
	...
	//字典加载
	function loadDict() {
        if("${medReqMain.req_code}"){
        	$("#req_code").val("${medReqMain.req_code}");
        }
		autocomplete("#dept_code", "../../../queryMedDept.do?isCheck=false", "id", "text", true, true, {is_last : '1'});//主管部门
		if("${medReqMain.dept_id}"){
			liger.get("dept_code").setValue("${medReqMain.dept_id},${medReqMain.dept_no}");
	        liger.get("dept_code").setText("${medReqMain.dept_code} ${medReqMain.dept_name}");
		}
		$("#store_code").ligerComboBox({width:160,disabled:true,cancelable: false});
		if("${medReqMain.store_id}"){
			liger.get("store_code").setValue("${medReqMain.store_id},${medReqMain.store_no}");
	        liger.get("store_code").setText("${medReqMain.store_code} ${medReqMain.store_name}");
		}
        $("#make_date").ligerTextBox({width:160});
        if("${medReqMain.make_date}"){
        	$("#make_date").val("${medReqMain.make_date}");
        }else{
	        autodate("#make_date");
        }
