...
<head>
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
						...
						}, {display : '生产批号(E)',name : 'batch_no',minWidth : 200,align : 'left',editor : {type : 'text',}
					...
					url : 'queryMedStorageInOrderN.do?isCheck=false',
	...
	function closeInvInfo(){
		...
		$.ligerDialog.open({
			url: "medOrderImpCloseInvPage.do?isCheck=false&paras="+paras, 
	...
	function closeInv(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要关闭选中的药品？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("updateMedStorageInCloseInv.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData){
	...
	function save() {
		if (validateGrid()) {
			...
			ajaxJsonObjectByUrl("addMedStorageInOrder.do?isCheck=true", formPara, function (responseData) {
				if (responseData.state == "true") {
