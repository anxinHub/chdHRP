    	...
    	grid = $("#maingrid").ligerGrid({
			columns: [{ 
		 			...
		 			render : function(rowdata, rowindex, value) {
						rowdata.price = value == null ? "" : formatNumber(value, '${sessionScope.med_para_map["08006"] }', 0);
						return value == null ? "" : formatNumber(value, '${sessionScope.med_para_map["08006"] }', 1);
					}
		 		} ],
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedStorageInInv.do?isCheck=false',
    ...
    function imp(){
		...
		if (data.length == 0){
			...
			$.each(data, function(index, data){
				...
				detail_rows.append('"price":').append(formatNumber(data.price, '${sessionScope.med_para_map["08006"] }', 0)).append(',');
				detail_rows.append('"amount_money":').append(formatNumber(data.price*data.amount*num, '${sessionScope.med_para_map["08005"] }', 0)).append('}');
			});
			detail_rows.append("]");
			//添加药品
