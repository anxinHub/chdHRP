...
<html style="overflow:hidden;">
  <head>
	...
	function  save(){
		...
		//ajaxJsonObjectByUrl("offsetMedStorageIn.do", formPara, function(responseData){
		ajaxJsonObjectByUrl("../back/addMedStorageBack.do", formPara, function(responseData){
	...
	function loadDict(){
		...
		if("${medInMain.app_dept_code}"){
			liger.get("app_dept").setValue("${medInMain.app_dept}");
			liger.get("app_dept").setText("${medInMain.dept_code} ${medInMain.dept_name}");
		}
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
				...
				editor : {
					type : 'select',
	         		...
	         		grid : {
						...
						url : liger.get("sup_code").getValue() ? '../../queryMedStockInvListBackNew.do?isCheck=false&store_id='+liger.get("store_code").getValue().split(",")[0] + '&sup_id=' + liger.get("sup_code").getValue().split(",")[0]+'&out_id=${out_id}'
			...
			url : 'queryMedStorageMegerBackDetail.do?isCheck=false&in_ids='+'${in_ids}'+'&store_id='+'${medInMain.store_id}',
