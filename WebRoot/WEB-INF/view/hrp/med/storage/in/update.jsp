...
// 			grid.reRender();
			...
			console.log(grid);
			grid.columns[6].editor.grid.url = '../../queryMedInvListIn.do?isCheck=false&store_id=' + liger.get("store_code").getValue().split(",")[0]+'&sup_id=' + liger.get("sup_code").getValue().split(",")[0];
			grid.reRender();
		});
...
// 			grid.reRender();
	...
	function  save(){
		grid.endEdit();
     	if(validateGrid()){
 	        ...
 	        ajaxJsonObjectByUrl("updateMedStorageIn.do", formPara, function(responseData){
	...
	function loadDict(){
    	//字典下拉框
    	autocompleteAsync("#bus_type_code", "../../queryMedBusType.do?isCheck=false", "id", "text", true, true, {sel_flag : 'in'}, false, "${medInMain.bus_type_code}");
		autocompleteAsync("#store_code", "../../queryMedStoreDictDate.do?isCheck=false", "id", "text", true, true, isUseAffiStore ? "" : {is_com : 0,is_write:'1'});
		...
		autocompleteAsync("#protocol_code", "../../queryMedProtocolMain.do?isCheck=false", "id", "text", true, true, {sup_id: "${medInMain.sup_id}"}, false, false, 160, false, 240);
    ...
    function initBus(){
    	if("${medInMain.bus_type_code}" != 2 && "${medInMain.bus_type_code}" != 22){
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [{
				...
				display : '明细ID', name : 'in_detail_id', minWidth : 100, align : 'left', isAllowHide: false, hide: true
			}, {
				...
				display : '药品名称(E)', name : 'inv_id', textField : 'inv_name', minWidth : 240, align : 'left',				
				editor : flag != 0 ? {} :{
					type : 'select',
					valueField : 'inv_id',
					textField : 'inv_name',
				...
				render : function(rowdata, rowindex, value) {
					...
					if(rowdata.is_bar&&rowdata.sn!=null&&rowdata.sn!=""){
				...
				display : '备注(E)', name : 'note', minWidth : 240, align : 'left',
				editor : {
					type : 'text',
				}
			},{
			...
			dataAction : 'server', dataType : 'server', usePager : false, url : 'queryMedStorageInDetail.do?isCheck=false', 
    ...
    function copy () {
		...
		if (data_copys.length == 0){
			...
			for(var i = 0, length = data_copys.length; i<length; i++){         // 把数组的每个data都复制一遍，即深复制
				...
				data_copys[i].in_detail_id='';  //新加药品明细ID应为空
				data_copys[i].batch_sn='';  //新加药品批次应为空
			}
	...
	function audit(){
		...
		$.ligerDialog.confirm('确定审核?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("auditMedStorageIn.do", formPara, function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 2;
						change_button();
	...
	function unAudit(){
		...
		$.ligerDialog.confirm('确定消审?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("unAuditMedStorageIn.do", formPara, function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 1;
						change_button();
    ...
    function confirmData(confirmDate){
		...
		$.ligerDialog.confirm('确定入库确认?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("verifyMedClosingDate.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData){
					if (responseData.state == "true") {
						ajaxJsonObjectByUrl("confirmMedStorageIn.do", formPara, function (responseData){
							if(responseData.state=="true"){
								parentFrameUse().query();
								state = 3;
								change_button();
    ...
    function create_invoice(){
    	...
    	$.ligerDialog.confirm('确定生成发票?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("addMedBillByInvoice.do?isCheck=false&"+para, "", function (responseData){});
			}
		}); 
    }
	...
	/* 	var isPreview=true;
		...
		//printTemplate("queryMedInByPrintTemlate.do?isCheck=false",para);
	}
	//条码打印模板设置
	function barCodePrintSet(){
		...
		officeFormTemplate({template_code:"081326",use_id:useId});
	}
	//模板条码打印
	function barCodePrintMenu(){
		...
		var errDetailIdStr="";//非个体码药品药品信息
		if(barData.length<1){
			$.ligerDialog.error('请选择待生成条码的药品!');
			return;
		}
		for(var i=0;i<barData.length;i++){
			if(barData[i].is_per_bar!=1){//如果is_per_bar为1说明为个体码
		...
		if(errDetailIdStr.length!=0){//含有非个体码药品
			$.ligerDialog.error('含非个体码药品('+errDetailIdStr+'),无法生成条形码,清重新选择!');
			return;
		}
		var para={
				template_code:'081326',
				class_name:"com.chd.hrp.med.serviceImpl.storage.in.MedStorageInServiceImpl",
				method_name:"queryMedInBarcodeByPrintTemlateNewPrint",
				isSetPrint:false,//是否套打，默认非套打
	...
	function printSet(){
		...
		officeFormTemplate({template_code:"08008",use_id:useId});
		/* parent.parent.$.ligerDialog.open({url : 'hrp/med/storage/in/storageInPrintSetPage.do?template_code=08010&use_id='+useId,
	...
	function mergePrint(){
		...
		var para={
				template_code:'08010',
				class_name:"com.chd.hrp.med.service.storage.in.MedStorageInService",
				method_name:"queryMedInByNewCombinePrint",
				bean_name:"medStorageInService",
				isSetPrint:false,//是否套打，默认非套打
				isPreview:true,//是否预览，默认直接打印
				in_id:$("#in_id").val(),
				store_id:liger.get("store_code").getValue().split(",")[0], 
	...
	<form name="form1" method="post"  id="form1" >
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
			...
			<tr>
				...
				<td align="left" class="l-table-edit-td">
					<input class="Wdate" name="bill_date" id="bill_date" type="text" value="${medInMain.bill_date}"
	            ...
	            <td align="left" class="l-table-edit-td" >
	            	<input name="brief" type="text" id="brief" ltype="text" value="${medInMain.brief}" validate="{required:false,maxlength:100}" />
	            </td>
			</tr>
	    </table>
    </form>
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%"  style="margin-top: 5px;">
			...
			<tr>	
				<td align="center" class="l-table-edit-td" colspan="4">
					...
					&nbsp;&nbsp; 
					<button id ="barcodePrint" accessKey="J"><b>条码打印(<u>J</u>)</b></button>
					&nbsp;&nbsp;  
