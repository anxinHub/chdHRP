										...
										},
										{
											...
											render : function(rowdata, rowindex,value) {
												rowdata.price = value == null ? ""
														: formatNumber(value,'${sessionScope.med_para_map["08006"] }',0);
												return value == null ? ""
														: formatNumber(value,'${sessionScope.med_para_map["08006"] }',1);
											}
										},
										{
											...
											render : function(rowdata, rowindex,value) {
												rowdata.amount_money = value == null ? ""
														: formatNumber(value,'${sessionScope.med_para_map["08005"] }',0);
												return value == null ? ""
														: formatNumber(value,'${sessionScope.med_para_map["08005"] }',1);
											},
											totalSummary : {
												align : 'right',
												render : function(suminf, column,cell) {
...
// 		liger.get("sup_code").setText("${sup_text}");
		...
		autoCompleteByData("#pur_type", medOrderMain_orderType.Rows, "id",
	...
	function f_onAfterEdit(e) {
		if (e.value != "") {
			if (e.column.name == "inv_id") {
				//判断是否为自动有效日期
				if ('${sessionScope.med_para_map["08009"] }' != 0) {
					grid.updateCell('inva_date', getDateAddDay(new Date(), '${sessionScope.med_para_map["08009"] }'), e.rowindex);
				}
