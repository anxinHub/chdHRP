...
<html style="overflow:hidden;">
  <head>
    ...
    <script type="text/javascript">
     ...
     function loadHead(){
    	 var medInMainList = ${medInMainList};
     	 inMainGrid = $("#maingrid").ligerGrid({
            columns: [ 
                      ...
                      data: medInMainList, allowAdjustColWidth : false ,
       	 ...
       	 grid =$(detailGrid).css('margin',10).ligerGrid({
             columns:
                    ...
                    width:'98%',height:'auto',isScoll:true ,allowAdjustColWidth : false , frozen : false , isAddRow:false,
			 		url : '../../bill/medbillmain/queryMedInDetail.do?isCheck=false&in_id='+row.in_id+'&bill_id='+row.bill_id+'&init='+row.init+'&flag=0',
   ...
   //勾选入库单后  自动填写 要生成的发票的  发票金额
    ...
    function setBillCode(){
    	...
    	var parm={
    	 ...
    	 ajaxJsonObjectByUrl("../../bill/medbillmain/setBillCode.do?isCheck=false",parm,function (responseData){
    ...
    function setInit(){
     ...
     function add_open(){
    	 if( liger.get("sup_id").getValue().split(",")[0] == '' | liger.get("dept_id").getValue().split(",")[0] ==''){
    		 ...
    		 $.ligerDialog.open({url: '../../bill/medbillmain/inMainPage.do?isCheck=false',height: 500,width: 1000, top: 0 ,
  ...
  /*    //删除
          	...
          	if(JSON.stringify(targetMap.map) != '{}'){
            	...
            	if(str != ''){
            		...
            		$.ligerDialog.confirm('<span style="color : red">您有入库单选择了明细记录,该入库单的发票明细将只保存您选择的明细.</span>确定要保存吗?', function (yes){
                    	if(yes){
                    		 ajaxJsonObjectByUrl("../../bill/medbillmain/addMedBillMain.do?isCheck=false",formPara,function(responseData){
                   	            if(responseData.state=="true"){
                   	                ajaxJsonObjectByUrl("../../bill/medbillmain/addMedBillDetail.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
                                   		if(responseData.state=="true"){
    ...
    function loadDict(){
		...
		//付款条件
		autocomplete("#pay_code", "../../queryMedPayTerm.do?isCheck=false", "id", "text", true, true,'',true,'',200);
