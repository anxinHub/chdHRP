...
<script type="text/javascript">
     ...
     $(function (){
		...
		$("#dept_id").bind("change",function(){
			if(liger.get("dept_id").getValue()){
		    	...
		    	autocomplete("#dept_emp", "../../queryMedEmpDict.do?isCheck=false", "id", "text", true, true, para, true, false, '180');
			}
		});
     });  
 ...
 function loadForm(){
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
					...
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : '../../../queryMedStockInvList.do?isCheck=false&store_id=' 
	         					+ liger.get("store_id").getValue().split(",")[0]+'&out_id=${medOutMain.out_id}',
	         			pageSize : 10,
			...
			usePager : false,width : '100%',height : '100%',enabledEdit : false,//state == 1 ? true : false,
					fixedCellHeight:true,data:medOutDetail,heightDiff:-20,
			...
			toolbar: { items: [
				/* { text: '保存', id:'save', click: saveMedOutMain,icon:'save', disabled: state == 1 ? false : true },
				{ line:true },
	...
	function medOutFifoImport(){//选择药品
		...
		$.ligerDialog.open({url: "medOutMainFifoPage.do?isCheck=false&paras="+paras, height: 500,width: 900, title:'选择药品',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '选择', onclick: function (item, dialog) { dialog.frame.checkMedOutFifo(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
