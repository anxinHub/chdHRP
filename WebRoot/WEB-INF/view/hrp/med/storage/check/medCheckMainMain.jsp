...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedCheckMain.do',
                     ...
                     toolbar: { items: [
										...
										{ line:true },
										{ text: '模板设置', id:'printSet', click: printSet1, icon:'print' },
										{ line:true } ,
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/med/storage/check/medCheckMainAddPage.do?isCheck=false',
    ...
    function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMedCheckMain.do", {ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/med/storage/check/medCheckMainUpdatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});   
	}
	...
	function openInMain(obj){
		...
		parent.$.ligerDialog.open({
			...
			//这个url只有查看权限
			//url: 'hrp/med/storage/check/medStorageInPage.do?isCheck=false&' + paras.toString(),
	...
	function openOutMain(obj){
		...
		parent.$.ligerDialog.open({
			...
			//这个url只有查看权限
			//url: 'hrp/med/storage/check/medStorageOutPage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});   
	}
	...
	function loadDict() {
		...
		autocompleteAsync("#store_id", "../../queryMedStoreDictDate.do?isCheck=false", "id","text", true, true, {read_or_write:1}, true, false, '180');
	...
	function imp() {
		...
		var index = layer.open({
			...
			area : [ '893px', '500px' ],
			content : 'medCheckMainImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
...
// 			} 
	    	...
	    	printTemplate("hrp/med/storage/check/queryMedCheckByPrintTemlate.do?isCheck=false",para);
