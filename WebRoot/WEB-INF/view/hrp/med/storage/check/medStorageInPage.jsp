...
<html style="overflow:hidden;">
  <head>
    ...
    <script type="text/javascript">
     ...
     function validateGrid() {  
 		...
 		//alert(JSON.stringify(data));
  		...
  		$.each(data,function(i, v){
 			...
 			if (v.inv_id) {
				...
				if(v.is_quality == 1){
					if(!v.inva_date){
						msg += "第"+(i+1)+"行按保质期管理的药品必须输入灭菌日期。<br>";
					}else{
						...
						ajaxJsonObjectByUrl("queryMedStorageInBatchInva.do?isCheck=false", para, function (responseData){
				...
				if(v.is_disinfect == 1){
					if(!v.disinfect_date){
						...
						ajaxJsonObjectByUrl("queryMedStorageInBatchDisinfect.do?isCheck=false", para, function (responseData){
							if(responseData.state=="false"){
				...
				var key=v.inv_id +"|"+v.batch_no+"|"+v.sn+"|"+v.location_id;
	 			...
	 			if(!targetMap.get(key)){
					...
					msg += targetMap.get(key)+"与"+value+"药品编码、生产批号、条形码、货位不能重复" + "<br>";
	 			}
				rows = rows + 1;
				store_inv += v.inv_id + ",";
		...
		if(sup_inv.length > 0){
			...
			is_flag = ajaxJsonObjectByUrl("../../existsMedInvOnlySup.do?isCheck=false", para, function (responseData){
				if(responseData.state=="false"){
				...
				editor : {
					...
					grid : {
						columns : [ {
							...
							display : '药品类别', name : 'med_type_name', minWidth : 100, align : 'left'
						}, {
	...
	function printDate(){
		...
		//查询打印数据，主从表
		ajaxJsonObjectByUrl("queryMedInByPrintTemlate.do",para,function(responseData){
	...
	//打印设置
	function printSet(){
		parent.parent.$.ligerDialog.open({url : 'hrp/acc/accvouch/superPrint/printSetPage.do?mod_code=08&template_code=08010',
