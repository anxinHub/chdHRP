...
<script type="text/javascript">
       ...
       var affiGrid = document.createElement('div'); 
        $(detailPanel).append(affiGrid);
      	$(affiGrid).css('margin',10).ligerGrid({
            columns:
                     [
                     { display: '药品编码', name: 'inv_code',width:150},
                     { display: '药品名称', name: 'inv_name',width:180},
                     { display: '计量单位', name: 'unit_name',width:108 },
                     ...
                     { display: '金额', name: 'amount_money',width:100,align : 'right',
                    	 render : function(rowdata, rowindex, value) {
								return formatNumber(rowdata.amount_money, '${p08005}', 1);
							}
                     },
                     ...
                     showTitle: false,columnWidth: 80,url : 'queryMedAffiDetail.do?isCheck=false&out_id='+row.out_id+'&store_id='+row.store_id,
  ...
  function comfirm(){
    ...
    function loadDict(){
		...
		//采购部门
    	autocomplete("#dept_id", "../../queryMedDeptDict.do?isCheck=false", "id", "text", true, true, "", true);
