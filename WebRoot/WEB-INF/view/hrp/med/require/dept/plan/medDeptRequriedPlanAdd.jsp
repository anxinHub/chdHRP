  ...
  <head>
	...
	function save(){
		...
		if(validateGrid()){
			...
			ajaxJsonObjectByUrl("addMedDeptRequriedPlan.do?isCheck=true", formPara, function(responseData) {
	...
	function query() {
		...
		grid.options.parms.push(
			{name : 'dept_id',value : liger.get("dept_code").getValue() == '' ? '':liger.get("dept_code").getValue().split(',')[0]}); 
    	grid.options.parms.push(
    		{name : 'dept_no',value : liger.get("dept_code").getValue() == '' ? '':liger.get("dept_code").getValue().split(',')[1]});
	...
	//字典加载
	function loadDict() {
		autocomplete("#dept_code", "../../../queryMedDept.do?isCheck=false", "id", "text", true, true, {is_last : '1'}, true);//主管部门
		if('${store_initValue}' != ''){
			autocomplete("#store_code","../../../queryMedStoreByRead.do?isCheck=false","id","text",true,true,'',false,'${store_initValue}');//仓库
		}else{
