...
<script type="text/javascript">
	...
	function loadHead() {
		...
		grid = $("#maingrid").ligerGrid({
		       columns: [ 
		                  { display: '药品编码', name: 'inv_code', align: 'left',width :'10%',
						 ...
						 { display: '药品名称(E)', name: 'inv_name', align: 'left' ,width :'15%'},
		                ...
		                url:'queryMedDeptRequriedCollect.do?isCheck=true',
			...
			var para = {
	         ...
	         detailGrid = $(invDetail).css('margin',1).ligerGrid({
	             columns:
							...
							parms :para, 
				 			url : 'queryMedCollectDetail.do?isCheck=false',
				 			enabledEdit : true,
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({ 
			url : 'medDeptCollectDetailPage.do?isCheck=false&'+parm,data:{}, 
	...
	function saveSub(){
	    ...
	    if(JSON.stringify(allData).length == 2){
    	    ...
    	    $.ligerDialog.open({url: 'medDeptRequriedCollectSaveAndSub.do?isCheck=false',data:{}, //2017-1-12修改此处代码 解决点击确定按钮页面报错的问题
    	    height: 250,width:450,title:'汇总提交',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
      		buttons: [ 
      				   { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMedDeptCollectSaveAndSub();} }, 
	...
	function save(){
		...
		ajaxJsonObjectByUrl("saveMedDeptRequriedCollect.do", parm, 
	...
	function dirSum(){
		...
		parent.$.ligerDialog.open({ 
			url: 'hrp/med/require/collect/medDeptRequriedDirCollectMainPage.do?isCheck=false'+parm, 
	...
	function loadDict() {
		...
		autocomplete("#year","../../queryMedYear.do?isCheck=false","id","text",true,true);
        liger.get("year").setValue(aa[0]);
        liger.get("year").setText(aa[0]);
        $("#year").ligerComboBox({cancelable: false});
        
        autocomplete("#month","../../queryMedMonth.do?isCheck=false","id","text",true,true);
        liger.get("month").setValue(aa[1]);
        liger.get("month").setText(aa[1]);
        $("#month").ligerComboBox({cancelable: false});
		
        autocomplete("#store_code", "../../queryMedStore.do?isCheck=false", "id", "text", true, true,{is_purchase : '1'},true,false,'160');//仓库
