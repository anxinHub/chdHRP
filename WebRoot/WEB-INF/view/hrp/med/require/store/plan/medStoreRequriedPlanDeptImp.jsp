	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
		       columns: [ 	
                            ...
                            {display : '药品名称',name : 'inv_name',align : 'left',minWidth:'180'}, 
		                 ...
		                 url:'queryMedStoreByDeptDetail.do?isCheck=false',
		     			...
		     			onBeforeCheckRow : function(checked, rowdata, rowindex, rowobj){
		    				if(checked){
		    					if(rowdata.store_id ==undefined){
		    						$.ligerDialog.warn("药品【"+rowdata.inv_name+"】没有维护默认库房");
		    						return false;
		    					}else{
		...
		if (data.length == 0){
			...
			ajaxJsonObjectByUrl("queryDeptCollectData.do?isCheck=false", formPara, function(responseData) {
				...
				parent.$.ligerDialog.open({ 
					...
					width: $(window).width(),
					url : 'hrp/med/require/store/plan/medStoreReqPlanByDeptAddPage.do?isCheck=false&'+paras, 
					data : detailData,
