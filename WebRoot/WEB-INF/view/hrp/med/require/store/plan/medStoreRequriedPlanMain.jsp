...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
		       columns: [ 
						 ...
						 { display: '其他需求药品', name: 'other_inv', align: 'left',minWidth:120 },
						 { display: '退回原因', name: 'return_reason', align: 'left' ,minWidth:120}
		                ],
		                
		                 dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedStoreRequriedPlan.do?isCheck=true',
	...
	function printSetDetail(){
		...
		officeFormTemplate({template_code:"08002",use_id:useId});
	}
	
	//打印
	function printDetail(){
		//按用户打印
		var  useId='${sessionScope.user_id }';
		...
		if (data.length == 0){
			...
			var para={
				class_name:"com.chd.hrp.med.serviceImpl.requrie.store.MedStoreRequirePlanServiceImpl",
				method_name:"printMedStoreRequireMainData",
	...
	function dept_imp(){
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/med/require/store/plan/medStoreRequriedPlanDeptImpPage.do?isCheck=false', 
	...
	function openAdd(){
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/med/require/store/plan/medStoreRequriedPlanAddPage.do?isCheck=false', 
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		}); 		
	}
	...
	function update_open(obj) {
		...
		parent.$.ligerDialog.open({ 
			...
			height: $(window).height(),
			width: $(window).width(),
			url : 'hrp/med/require/store/plan/medStoreRequriedPlanUpdatePage.do?isCheck=false&' + parm, 
			modal:true,showToggle:false,showMax:true ,showMin: false,isResize:true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
			});
	...
	function del(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要删除吗?', 
            		function (yes){
                    	if(yes){
                        	ajaxJsonObjectByUrl("deleteMedStoreRequriedPlan.do?isCheck=true",{ParamVo : ParamVo.toString()},
	...
	function abort(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要中止吗?', 
            		function (yes){
                    	if(yes){
                        	ajaxJsonObjectByUrl("abortMedStoreRequriedPlan.do?isCheck=true",{ParamVo : ParamVo.toString()},
                        			function (responseData){
	                            		if(responseData.state=="true"){
	                            			query();
	                            		}
	...
	function submit(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要提交吗?', 
            	function (yes){
                    if(yes){
                       	ajaxJsonObjectByUrl("submitMedStoreRequriedPlan.do?isCheck=true",{ParamVo : ParamVo.toString()},
                        	function (responseData){
	                           	if(responseData.state=="true"){
	                            	query();
	                           	}
	...
	function submitCancle(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要取消提交吗?', 
            	function (yes){
                   	if(yes){
                       	ajaxJsonObjectByUrl("unSubmitMedStoreRequriedPlan.do?isCheck=true",{ParamVo : ParamVo.toString()},
                        	function (responseData){
	                           	if(responseData.state=="true"){
	                            	query();
	                           	}
	...
	function copy(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要复制吗?', 
            	function (yes){
                   	if(yes){
                       	ajaxJsonObjectByUrl("copyMedStoreRequriedPlan.do?isCheck=true",{ParamVo : ParamVo.toString()},
                        	function (responseData){
	                           	if(responseData.state=="true"){
	                            	query();
	                           	}
	...
	function audit(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要审核吗?', 
            	function (yes){
                    if(yes){
                        ajaxJsonObjectByUrl("submitMedStoreRequriedConfirm.do?isCheck=true",{ParamVo : ParamVo.toString()},
                        	function (responseData){
	                            if(responseData.state=="true"){
	                            	query();
	                            }
	...
	function unaudit(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要取消审核吗?', 
            	function (yes){
                   	if(yes){
                        ajaxJsonObjectByUrl("unSubmitMedStoreRequriedConfirm.do?isCheck=true",{ParamVo : ParamVo.toString()},
                        	function (responseData){
	                           	if(responseData.state=="true"){
	                           		query();
	                          		}
