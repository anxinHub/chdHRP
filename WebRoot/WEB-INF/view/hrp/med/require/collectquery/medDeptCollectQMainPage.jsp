...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
		       columns: [ 
		                 ...
		                 url:'queryMedDeptRequriedCollectQ.do?isCheck=true',
	...
	function openReqDeatail(obj){
		...
		$.ligerDialog.open({ 
			url : 'medDeptCollectStoreDetail.do?isCheck=false&' + parm, 
	...
	function openCollectDeatail(obj){
		...
		$.ligerDialog.open({ 
			url : 'medDeptCollectDeptDetail.do?isCheck=false&' + parm,
			height: 550,width: 1000, 
	...
	function openPurDeatail(obj){
		...
		$.ligerDialog.open({ 
			url : 'medDeptCollectPurDetail.do?isCheck=false&' + parm,
			height: 550,width: 1000, 
			title:'查看明细',modal:true,showToggle:false,showMax:true ,showMin: false,isResize:true
		});
	...
	function loadDict() {
		...
		autocomplete("#yearB","../../queryMedYear.do?isCheck=false","id","text",true,true);
        liger.get("yearB").setValue(aa[0]);
        liger.get("yearB").setText(aa[0]);
        
        autocomplete("#monthB","../../queryMedMonth.do?isCheck=false","id","text",true,true);
        liger.get("monthB").setValue(aa[1]);
        liger.get("monthB").setText(aa[1]);
		
        autocomplete("#yearE","../../queryMedYear.do?isCheck=false","id","text",true,true);
        liger.get("yearE").setValue(aa[0]);
        liger.get("yearE").setText(aa[0]);
        
        autocomplete("#monthE","../../queryMedMonth.do?isCheck=false","id","text",true,true);
        liger.get("monthE").setValue(aa[1]);
        liger.get("monthE").setText(aa[1]);
        
        autocomplete("#store_code", "../../queryMedStore.do?isCheck=false", "id", "text", true, true,{is_purchase : '1'}, true,false,'160',"",200);//仓库
