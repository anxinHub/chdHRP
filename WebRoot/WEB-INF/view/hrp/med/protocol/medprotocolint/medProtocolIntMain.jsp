...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedProtocolInt.do?isCheck=true',
    ...
    function add_open(){
    	$.ligerDialog.open({ url : 'medProtocolIntAddPage.do?isCheck=false',data:{}, 
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMedProtocolInt.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"protocol_id="+vo[3] +"&"+ 
			"state="+vo[4] 
		$.ligerDialog.open({ url : 'medProtocolIntUpdatePage.do?isCheck=false&'+parm,data:{}, 
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryMedProtocolInt.do",printPara,function (responseData){
			 var trHtml='';
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryMedProtocolInt.do",exportPara,function (responseData){
			 var trHtml='';
			$.each(responseData.Rows,function(idx,item){ 
				 	 trHtml+="<tr>";
					 trHtml+="<td>"+item.protocol_code+"</td>"; 
