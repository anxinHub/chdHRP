...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedProtocolType.do',
    ...
    function add_open(){
    	$.ligerDialog.open({ url : 'medProtocolTypeAddPage.do?isCheck=false',data:{}, 
			height: 300,width: 320, title:'添加',modal:true,showToggle:false,showMax:false,
			showMin: false,isResize:true,
			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMedProtocolType(); },
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMedProtocolType.do",{ParamVo : ParamVo.toString()},function (responseData){
   ...
   function imp(){
    	$.ligerDialog.open({ url : 'medProtocolTypeImportPage.do?isCheck=false',data:{}, 
   ...
   function downTemplate(){
    ...
    function  openUpdate(obj){
		...
		var parm = 
			...
			"copy_code="+vo[2]   +"&"+ 
			"type_id="+vo[3] ;
		$.ligerDialog.open({ url : 'medProtocolTypeUpdatePage.do?isCheck=false&'+parm,data:{}, 
