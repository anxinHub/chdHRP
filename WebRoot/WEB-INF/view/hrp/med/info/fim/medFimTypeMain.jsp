...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '药品财务编码', name: 'fim_type_code', align: 'left',
                     ...
                     dataAction: 'server',dataType: 'server',usePager:false,url:'queryMedFimType.do?isCheck=false',
    ...
    function addPage(){
    	$.ligerDialog.open({url: 'addMedFimTypePage.do?isCheck=false', height: 300,width: 500, 
				title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
				buttons: [ 
				           { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMedFimType(); },cls:'l-dialog-btn-highlight' }, 
    ...
    function deletePage(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定删除?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("deleteMedFimType.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function openUpdate(data){
    	...
    	$.ligerDialog.open({url: 'updateMedFimTypePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 500, 
			title:'修改',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
			buttons: [ 
			           { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMedFimType(); },cls:'l-dialog-btn-highlight' }, 
			           { text: '取消', onclick: function (item, dialog) { dialog.close(); } } 
			         ] 
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        ...
        <tr>
        	<td align="right" class="l-table-edit-td"  style="padding-left:20px;">药品财务分类编码：</td>
            <td align="left" class="l-table-edit-td">
            	<input name="fim_type_code" type="text" id="fim_type_code" ltype="text" validate="{required:true,maxlength:20}" />
            </td>
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;" on>药品财务分类名称：</td>
            <td align="left" class="l-table-edit-td">
