...
<script type="text/javascript">
	...
	function add_open() {
		$.ligerDialog.open({
			url : 'medStoreSetAddPage.do?isCheck=false',
			height : 200,
			width : 300,
			title : '添加',
			modal : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMedStoreSet.do?isCheck=true", {
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url : 'medStoreSetUpdatePage.do?isCheck=false&' + parm,
			height : 200,
			width : 300,
			title : '修改',
			modal : true,
	...
	function openDetail(obj) {
		...
		$.ligerDialog.open({
			url : 'medStoreDetailPage.do?isCheck=false&' + parm,
			height : 600,
			width : 860,
			title : '设置仓库对应关系',
			modal : true,
	...
	function openAccount(obj) {
		...
		$.ligerDialog.open({
			url : 'medStoreSetAccountPage.do?isCheck=false&' + parm,
			height : 600,
			width : 860,
			title : '设置仓库是否结账',
			modal : true,
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryMedStoreSet.do", printPara, function(
				responseData) {
			var trHtml = '';
	...
	function exportExcel() {
		...
		ajaxJsonObjectByUrl("queryMedStoreSet.do", exportPara,
				function(responseData) {
					var trHtml = '';
					$.each(responseData.Rows, function(idx, item) {
						trHtml += "<tr>";
