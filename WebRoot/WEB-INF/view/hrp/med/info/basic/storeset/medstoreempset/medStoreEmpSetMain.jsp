...
<script type="text/javascript">
	...
	function add_open() {
		$.ligerDialog.open({
			url : 'medStoreEmpSetAddPage.do?isCheck=false',
			height : 300,
			width : 500,
			title : '添加',
			modal : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMedStoreEmp.do?isCheck=true", {
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url : 'medStoreEmpSetUpdatePage.do?isCheck=false&' + parm,
			height : 300,
			width : 500,
			title : '修改',
			modal : true,
	...
	function loadDict() {
    	...
    	autocomplete("#emp_id","../../../../queryMedStockEmp.do?isCheck=false","id","text",true,true);   
	}
	//键盘事件
