...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedFacAttr.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
            	case "add":
            		parent.$.ligerDialog.open({url: 'hrp/med/info/basic/medfacattr/medFacAttrAddPage.do?isCheck=false&type_code='+type_code, 
	          				height: 600,width:$(window).width()-180, title:'添加',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMedFacAttr.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
                    ...
                    return;
                case "import":
                	$.ligerDialog.open({url: 'medFacAttrImportPage.do?isCheck=false', height: 500,width: 1100, title:'导入',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,top:10 });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm ="&group_id="+ 
		   ...
		   vo[3]   +"&fac_code="+
		   vo[4];
    	parent.$.ligerDialog.open({ url : 'hrp/med/info/basic/medfacattr/medFacAttrUpdatePage.do?isCheck=false&' + parm,data:{}, 
    						height: 600,width: $(window).width()-180,title:'修改',modal:true,showToggle:false,showMax:false , 
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryMedFacAttr.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryMedFacAttr.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.fac_id+"</td>";
					 trHtml+="<td>"+item.fac_code+"</td>";
