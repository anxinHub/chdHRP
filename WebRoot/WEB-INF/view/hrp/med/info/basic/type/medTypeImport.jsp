...
$(function(){
	...
	function plupload(){  
		$("#uploader").pluploadQueue({           
	       	...
	       	url : "readMedTypeFiles.do?isCheck=false",    
	       	chunk_size : '2mb',           
	       	filters : {                
	           mime_types: [  
...
function loadHead() {
	grid = $("#maingrid").ligerGrid(
					{
				columns : [
							 { display: '药品类别编码', name: 'med_type_code', align: 'left',editor:{type:'text'}},
							 { display: '药品类别名称', name: 'med_type_name', align: 'left',editor:{type:'text'}},
							 { display: '是否末级', name: 'is_last', align: 'left',editor:{type:'text'}},
							 { display: '是否停用', name: 'is_stop', align: 'left',editor:{type:'text'}},
							 { display: '备注', name: 'note', align: 'left',editor:{type:'text'}},
							 { display: '自动有效期', name: 'is_auto_exp', align: 'left',editor:{type:'text'}},
        ...
        switch (item.id)
        {
            case "save":
            	var data = gridManager.getData();
           	 	ajaxJsonObjectByUrl("addBatchMedType.do?isCheck=false",{ParamVo : JSON.stringify(data)},function (responseData){
