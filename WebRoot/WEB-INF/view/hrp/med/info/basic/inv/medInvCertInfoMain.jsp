  ...
  <head>
    ...
    function loadHead(){
     	grid = $("#maingrid").ligerGrid({
            columns: [ { 
				...
				display: '证件类型ID', name: 'type_id', align: 'left',width:60,hide:true
			}, { 
				...
				editor : {
					...
					valueField : 'id',
					textField : 'text',
					url : '../../../qryMedInvCertType.do?isCheck=false',
					keySupport : true,
					autocomplete : true,
				...
				formed: 'yyyy-MM-dd',editor: {type: 'date'}
			}, { 
				display: '截止日期', name: 'end_date', align: 'left',width:105,type: 'date', 
				formed: 'yyyy-MM-dd',editor: {type: 'date'}
			}, { 
				display: '生产厂商', name: 'fac_id', align: 'left', width:260,textField : 'fac_name',
				editor : {
					type : 'select',
					valueField : 'fac_id',
					textField : 'fac_name',
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedInvCertInfo.do?isCheck=false&inv_id='+'${inv_id}',
			...
			onBeforeEdit : f_onBeforeEdit, onSelectRow : addSupData ,
			toolbar: { items: [
       	 ...
       	 gridSup = $("#maingridSup").ligerGrid({
                columns: [
				   ...
				   { display: '供应商名称', name: 'sup_name', align: 'left'}
        			...
        			//url:'../cert/invcert/queryMedCerSup.do?isCheck=false',
        			//data: supData,
	...
	function save(){
		...
		if(!validateGrid(data)){
   	 		...
   	 		// 除去 药品修改页面 与 证件信息页面的 重复供应商信息 用Map
