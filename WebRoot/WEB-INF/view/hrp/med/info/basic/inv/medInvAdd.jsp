...
<html style="overflow:auto;">
  <head>
        ...
        if($("form").valid()){
			...
			/*  ajaxJsonObjectByUrl("queryByUniqueness.do?isCheck=false",formPara,function(responseData){
  	            	if(responseData.info=="1"){
  	            		...
  	            		$.ligerDialog.confirm('药品名称已经存在是否继续添加?', function (yes){
  	    	           		if(yes){
	...
	function loadDict(){
		...
		var med_type_id = '';
		var med_type_text = '';
		if(parentFrameUse() && parentFrameUse().liger){
			var selectObj = parentFrameUse().liger.get('tree').getSelected();
			if(selectObj){
				//var selectObj = parentFrameUse().liger.get('tree').getSelected();
				//var med_type_ids = parentFrameUse().med_type_id.split(",");
				//med_type_id = med_type_ids[0] +","+ med_type_ids[1]; 
				med_type_text = selectObj.data.text;
				med_type_id = selectObj.data.id+','+selectObj.data.no;
				
			}
		}
		
		liger.get("med_type_code").setValue(med_type_id);
		liger.get("med_type_code").setText(med_type_text);
		var paras = {
			field_code : "price_type"
		}
		autocomplete("#price_type", "../../../queryMedSysList.do?isCheck=false", "id", "text", true, true, paras, true);
		paras = {
			field_code : "amortize_type"
		}
		autocomplete("#amortize_type", "../../../queryMedSysList.do?isCheck=false", "id", "text", true, true, paras, true);
		...
		autocomplete("#use_state", "../../../queryMedYearOrNo.do?isCheck=false", "id", "text", true, true, "", false, "2");
		$("#use_state").ligerComboBox({disabled:true,cancelable: false});*/
		autoCompleteByData("#abc_type", medInv_ABCType.Rows, "id", "text", true, true, "", true);
		autoCompleteByData("#source_plan", medInv_sourcePlan.Rows, "id", "text", true, true, "", true);
        ...
        <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" style="padding: 10px"  >
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="med_type_code" type="text" id="med_type_code" ltype="text"   validate="{required:true}" />
	            </td>
	            <td align="right" class="l-table-edit-td" >
