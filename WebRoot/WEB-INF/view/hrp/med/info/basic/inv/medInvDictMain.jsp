...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '药品编码', name: 'inv_code', align: 'left', width: '12%' 
					 		},
                     { display: '药品名称', name: 'inv_name', align: 'left', width: '20%'
					 		},
                     { display: '规格型号', name: 'inv_model', align: 'left', width: '12%'
					 		},
                     { display: '计量单位', name: 'unit_code', align: 'left', width: '8%'
					 		},
    ...
    function update_open(obj){
		...
		$.ligerDialog.open({
			...
			height: 500,
			width: 1000,
			url: 'medInvDictUpdatePage.do?isCheck=false&' + paras.toString(),
 ...
 /*  //打印数据
	 function printDate(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopPrinterTable("resultPrint","开始打印","08106 药品变更表",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMedInvDict.do",printPara,function (responseData){
	 ...
	 */
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","08106 药品变更表.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMedInvDict.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.inv_no+"</td>"; 
					 trHtml+="<td>"+item.inv_id+"</td>"; 
					...
					/*  trHtml+="<td>"+item.is_com+"</td>";  */
					 ...
					 trHtml+="<td>"+item.inv_structure+"</td>"; 
					 trHtml+="<td>"+item.inv_usage+"</td>";
					 trHtml+="<td>"+item.med_jx_name+"</td>";
					 trHtml+="<td>"+item.med_sx_name+"</td>";
					 trHtml+="<td>"+item.mzplxs+"</td>";
					 trHtml+="<td>"+item.user_code+"</td>"; 
					 trHtml+="<td>"+item.create_date+"</td>"; 
					 trHtml+="<td>"+item.note+"</td>"; 
					 trHtml+="<td>"+item.is_stop+"</td>"; 
			...
			<tr>
              ...
              <!--   <th width="200">是否专购</th>	 -->
               <!--  <th width="200">是否收费</th>	 -->
                ...
                <th width="200">药品剂型名称</th>	
                <th width="200">药品属性名称</th>	
                <th width="200">病区拆零系数</th>	
                <th width="200">门诊拆零系数</th>	
                <th width="200">变更人</th>	
