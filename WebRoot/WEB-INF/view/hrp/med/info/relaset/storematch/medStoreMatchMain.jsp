...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:false,url:'queryMedStoreMatch.do?isCheck=false',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'medStoreMatchAddPage.do?isCheck=false', height: 550,width: 1000, title:'添加',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMedStoreMatch(); },cls:'l-dialog-btn-highlight' }, { text: '关闭', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                       ...
                       $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMedStoreMatch.do?paramVo="+Param,{},function (responseData){
    ...
    function openUpdate(obj){
		...
		$.ligerDialog.open({ url : 'medStoreMatchUpdatePage.do?isCheck=false&para='+obj,data:{}, height: 550,width: 1000, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '保存', onclick: function (item, dialog) { dialog.frame.saveStoreMatchDetail(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
  ...
  //打印数据
	 ...
	 function loadDict(){
         //字典下拉框
 		autocomplete("#store_id","../../queryMedStore.do?isCheck=false","id","text",true,true);//仓库信息
         
 		autocomplete("#store_match_name","../../queryMedStoreMatch.do?isCheck=false","id","text",true,true);//配套表信息
