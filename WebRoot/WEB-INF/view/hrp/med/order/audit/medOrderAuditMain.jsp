...
<script type="text/javascript">
	...
	function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
			columns: [{
				...
				{display: '采购方式', name: 'pur_type', align: 'left', width: 120,
					render:function(rowdata){
						if(rowdata.pur_type == 1){
							return "自购订单";
						}else if(rowdata.pur_type == 2){
							return "统购订单";
						}else{
							return "";
						}
				...
				{display: '订单类型', name: 'order_type', align: 'left', width: 120,
					render :function(rowdata,rowindex,value){
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedOrderAudit.do?isCheck=true',
			...
			toolbar: { items: [
				...
				{ line:true },
				{ text: '发送（<u>S</u>）', id:'send', click: send, icon:'send' },
				{ line:true }/* , 	
	...
	function update_open(obj){		
		...
		$.ligerDialog.open({
			...
			height: 500,
			width: 1000,
			url: 'medOrderAuditDetailPage.do?isCheck=false&' + paras.toString(),
	...
	function audit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditMedOrderAudit.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function unAudit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要销审?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("unAuditMedOrderAudit.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function send(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要发送吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("sendOutMedOrderAudit.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function loadDict() {
		...
		//订单类型
		autoCompleteByData("#order_type", medOrderMain_orderType.Rows, "id", "text", true, true);
		//供应商
		...
		//采购方式
		autoCompleteByData("#pur_type", medOrderMain_purType.Rows, "id", "text", true, true);
		//收货单位
