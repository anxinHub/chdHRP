  ...
  <head>
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server', dataType: 'server', usePager:true, url:'queryMedPurMainForOrder.do?isCheck=false', 
	...
	function loadDetailHead(){
    	detailGrid = $("#detailgrid").ligerGrid({
			...
			url : 'queryMedPurDetailForOrder.do?isCheck=false',
	...
	function update_open(obj){		
		...
		$.ligerDialog.open({
			...
			height: 450,
			width: 950,
			url: 'medOrderInitGenPurDetailPage.do?isCheck=false&' + paras.toString(),
	...
	function closeRow(){
		...
		if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定关闭行?', function(yes) {
            	if(yes){
					ajaxJsonObjectByUrl("updateMedPurDetailByOrderClose.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定生成订单?', function (yes){
				if(yes){
					...
					ajaxJsonObjectByUrl("addMedOrderByPurImp.do?isCheck=false", para, function(responseData){
			            if(responseData.state=="true"){
			            	query();
