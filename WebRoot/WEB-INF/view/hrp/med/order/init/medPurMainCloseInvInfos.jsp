...
<head>
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
					...
					url : 'queryMedPurCloseInvInfos.do?isCheck=false',
	...
	function cancleCloseInv(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要取消关闭的药品？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("updateMedPurCancleCloseInvs.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData){
	...
	function loadDict() {
		...
		autocompleteAsync("#sup_code", "../../queryHosSupDict.do?isCheck=false", "id", "text", true, true, "", false, false, 180);
