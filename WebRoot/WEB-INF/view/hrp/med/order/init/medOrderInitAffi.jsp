...
<html style="overflow:hidden;">
  <head>
    ...
    function loadDict(){
		...
		autocomplete("#dept_code", "../../queryMedDept.do?isCheck=false", "id", "text", true, true,{is_last : '1'}, false);//科室		
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedOrderInitAffi.do?isCheck=false',
	...
	function update_open(obj){		
		...
		$.ligerDialog.open({
			...
			height: 500,
			width: 900,
			url: 'medOrderInitAffiDetailPage.do?isCheck=false&' + paras.toString(),
  	...
  	function gen(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要生成订单吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("genByAffiMedOrderInit.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
