...
<script>
	var grid, charge_itemSelect , consum_codeSelect, sic_quanlitySelect , quantity_typeSelect ; 
    ...
    function query() {
        params = [
			...
			{ name: 'unit_name', value: $("#unit_name").val() },
	...
	function initGrid() {
            var columns = [
            	 {display: '服务项目', name: 'charge_item_name',width: '10%', editable:setEdit ,
            		 editor: {
		...
		function save () {
			 var data = grid.getChanges();
		        ...
		        if (data.addList.length > 0 || data.updateList.length > 0) {
		            ...
		            ajaxPostData({
		                url: "saveAssEqcServDetCons.do?isCheck=false",
		...
		function validateGrid(data) {
	        ...
	        $.each(data, function(i, v) {
	            ...
	            msg += rowm;
	            var key = v.charge_item_id + v.consum_code
		...
		function remove() {
		     ...
		     if (data.length == 0) {
		         ...
		         $.etDialog.confirm('确定删除?', function () {
		             ajaxPostData({
		                 url: 'deleteAssEqcServDetCons.do',
		                 data: {
...
<body>
    <table class="table-layout">
        <tr>
        	<td class="label" style="width: 100px;">细项:</td>
            <td class="ipt">
                <input id="charge_item_id" type="text" style="width: 180px" />
            </td>
