...
<script type="text/javascript">
           ...
           columns: [ 
                     ...
                     { display: '绩效得分', name: 'kpi_score', align: 'right',
		                 		render : function(rowdata, rowindex,
										...
										rowdata.goal_code   + "|" + 
										rowdata.emp_id   + "|" + 
										rowdata.emp_no+"')>"+rowdata.kpi_score+"</a>";
								}
					 		},
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmEmpKpiScoreEmp.do',
	...
	function toolbar(){
		...
		obj.push({ text: '查询（<u>Q</u>）', id:'search', click: query,icon:'search' });
       	...
       	obj.push({ text: '计算（<u>D</u>）', id:'collect', click: collectEmpKpiScore, icon:'account' });
       	obj.push({ line:true });
       	
       	obj.push({ text: '审核（<u>A</u>）', id:'audit', click:auditEmpKpiScore,icon:'audit' });
       	obj.push({ line:true });
       	
       	obj.push({ text: '反审核（<u>B</u>）', id:'reAudit', click:reAuditEmpKpiScore,icon:'back' });
       	obj.push({ line:true });
	...
	function loadHotkeys(){
		...
		hotkeys('Q',query);
		hotkeys('P',print);
		hotkeys('D',collectEmpKpiScore);
        hotkeys('A',auditEmpKpiScore);
        hotkeys('B',reAuditEmpKpiScore);
    	...
    	if (acc_year.length == 0){
       	...
       	$.ligerDialog.confirm('确定审核?',function(yes){
       		if(yes){
       			...
       			ajaxJsonObjectByUrl("auditPrmEmpKpiScore.do",param,function (responseData){
		...
		if (acc_year.length == 0){
       	...
       	$.ligerDialog.confirm('确定取消审核?',function(yes){
       		if(yes){
       			...
       			ajaxJsonObjectByUrl("reAuditPrmEmpKpiScore.do",param,function (responseData){
       				if(responseData.state=="true"){
       					query();
       				}
       			});
	...
	function collectEmpKpiScore(){
		...
		$.ligerDialog.confirm('确定计算吗?',function(yes){
			if(yes){
				...
				ajaxJsonObjectByUrl("collectEmpKpiScore.do",param,function (responseData){
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit">
        ...
        <tr>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;" colspan="1">目标名称：</td>
