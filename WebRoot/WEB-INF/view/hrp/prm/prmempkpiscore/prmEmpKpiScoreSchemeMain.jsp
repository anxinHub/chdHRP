...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmKpiScoreEmpByScheme.do?isCheck=false&group_id=${group_id}&hos_id=${hos_id}&copy_code=${copy_code}'
         				+ '&acc_year=${acc_year}&acc_month=${acc_month}&goal_code=${goal_code}&emp_id=${emp_id}&emp_no=${emp_no}',
    ...
    function openFormulaView(obj){
    	...
    	$.ligerDialog.open({
			url : 'prmEmpKpiScoreFormulaMain.do?isCheck=false&'+param,
			height : 500,
			width : 800,
			title : '公式查看',
    ...
    function scoringEmpScheme(group_id, hos_id ,copy_code,
		...
		if (grade_meth == '01') {
			...
			$.ligerDialog.open({
				url : 'prmEmpKpiScoreSectionMain.do?isCheck=false&' + parm,
				height : 500,
				width : 800,
				title : '区间法评分标准设定',
				modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
		...
		if (grade_meth == '02') {
			...
			$.ligerDialog.open({
				url : 'prmEmpKpiScoreAdMain.do?isCheck=false' + parm,
				height : 500,
				width : 800,
				title : '加分法评分标准设定',
				modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
    ...
    function loadDict(){
	        ...
	        autocompleteAsync("#emp_id","../queryHosEmpDict.do?isCheck=false","id","text",true,true,"",false,"${emp_id}.${emp_no}");
     ...
     function onSelect(note){
	     ...
	     grid.options.parms.push({name:'emp_id',value:liger.get("emp_id").getValue().split(".")[1] == null?'':liger.get("emp_id").getValue().split(".")[1]}); 
         if(pid != -1){
      ...
      function loadTree(){
		...
		ajaxJsonObjectByUrl("queryPrmKpiScoreEmpBySchemeTree.do?isCheck=false&group_id=${group_id}&hos_id=${hos_id}&copy_code=${copy_code}&acc_year=${acc_year}&acc_month=${acc_month}&goal_code=${goal_code}&emp_id=${emp_id}&emp_no=${emp_no}", param,
