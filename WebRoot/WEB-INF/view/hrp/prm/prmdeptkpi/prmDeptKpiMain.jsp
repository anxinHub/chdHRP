...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmDeptKpi.do',width: '100%', height: '100%', 
    ...
    function openUpdate(data){
		...
		$.ligerDialog.open({
			url: 'prmDeptKpiUpdatePage.do?isCheck=false'+param.toString(),
			height: 400,width: 600,
			title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
			data: {hos_id:hos_id},
			buttons: [ 
				{ text: '确定', onclick: function (item, dialog) { dialog.frame.savePrmDeptKpi(); },cls:'l-dialog-btn-highlight' }, 
    ...
    function toobar(){
		$("#toptoolmod").ligerToolBar({ items: [
			...
			{ line:true },
			{ text: '添加（<u>A</u>）', id:'add', click: addDeptKpi, icon:'add' },
			{ line:true },
			{ text: '选择（<u>C</u>）', id:'add', click: checkClick, icon:'outbox' },
			{ line:true },
			{ text: '删除（<u>D</u>）', id:'delete', click: deleteDeptKpi,icon:'delete' },
			{ line:true }
		]
		});
    }
	...
	function loadHotkeys(){
		hotkeys('Q',query);
		hotkeys('A',addDeptKpi);
		hotkeys('C',checkClick);
		hotkeys('D',deleteDeptKpi);
	}
    ...
    function addRows(target_data,__id){
    	...
    	$.ligerDialog.confirm('确定保存?',function(yes){
    		if(yes){
    	    	...
    	    	ajaxJsonObjectByUrl("addBatchPrmDeptKpi.do",formPara,function(responseData){
	...
	//添加页跳转
    function addDeptKpi(){
    	$.ligerDialog.open({url: 'prmDeptKpiAddPage.do?isCheck=false', 
			...
			buttons: [ 
				{ text: '确定', onclick: function (item, dialog) { dialog.frame.addPrmDeptKpi(); },cls:'l-dialog-btn-highlight' }, 
			    { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    }
    
	//删除
	function deleteDeptKpi(){
		...
		var data = gridManager.getCheckedRows();
	  	if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
	          	if(yes){
	              	ajaxJsonObjectByUrl("deletePrmDeptKpi.do",{ParamVo : ParamVo.toString()},function (responseData){
	              		if(responseData.state=="true"){
	              			query();
	              			loadTree()
	              		}
