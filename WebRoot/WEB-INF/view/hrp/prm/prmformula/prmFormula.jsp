...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					onDblClickRow : function(rowdata, rowindex, value) {
						...
						parent.$("#formula_code").val(rowdata.formula_code);

						parent.$("#formula_name").val(rowdata.formula_name);

						parent.$("#formula_method_chs").val(
	...
	function addFormula() {
		$.ligerDialog.open({
			url : 'prmFormulaAddPage.do?isCheck=false',
			height : 500,
			width : 750,
			title : '添加',
			buttons : [ {
	...
	function deleteFormula() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deletePrmFormula.do", {
						ParamVo : ParamVo
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url : 'prmFormulaUpdatePage.do?isCheck=false&' + parm,
			data : {},
			height : 500,
			width : 750,
			title : '修改',
	...
	function loadDict() {
		//字典下拉框
		autocomplete("#formula_name", "../queryPrmFormula.do?isCheck=false",
				"id", "text", true, true);
	}
</script>
