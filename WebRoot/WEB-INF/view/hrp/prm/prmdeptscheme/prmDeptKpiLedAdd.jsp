...
<script type="text/javascript">
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                ...
                url:'queryPrmDeptKpiLed.do?group_id=${group_id}&hos_id=${hos_id}&copy_code=${copy_code}&acc_year=${acc_year}&acc_month=${acc_month}&goal_code=${goal_code}&kpi_code=${kpi_code}&dept_id=${dept_id}&dept_no=${dept_no}',
  ...
  //键盘事件
	...
	function saveSchemeSection(){
		...
		if(validateGrid()){
			ajaxJsonObjectByUrl("saveDeptKpiLed.do", formPara, function(responseData) {
	...
	function DeleteGridRow(){
	     ...
	     if (data.length == 0){
             ...
             $.ligerDialog.confirm('确定删除?', function (yes){
             	if(yes){
                 	ajaxJsonObjectByUrl("deletePrmDeptKpiLed.do",{ParamVo : ParamVo.toString()},function (responseData){
                 		if(responseData.state=="true"){
                 			query();
                 		}
                 	});
