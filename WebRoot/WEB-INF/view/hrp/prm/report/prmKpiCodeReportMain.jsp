...
<script type="text/javascript">
    ...
    $(function (){
    	toobar();
		check_column1_manager = $("#check_column1").ligerCheckBox();
		check_column1_manager.setValue(true);
		check_column2_manager = $("#check_column2").ligerCheckBox();
		check_column2_manager.setValue(false);
        loadDict();//加载下拉框
    ...
    function getMonthNumber(dateStart,dateEnd){
  	  	...
  	  	var monthStart = dateStart.substr(4,2);
  	  	var monthEnd = dateEnd.substr(4,2);
	...
	function getMonthData(dateStart, dateEnd){
		...
		while(dateStart < dateEnd){
			dateStart++;
			if(dateStart.toString().slice(-2) - 12 >0){
				dateStart = dateStart.toString().slice(0,4)*1+1+"01";dateStart = dateStart * 1;
			}
	...
	function getMonthDataSort(dateStart, dateEnd){
		...
		if (dateStart == dateEnd){
			months.push(dateStart);return months;
		}
        ...
        while (count_time.toString().slice(-2) * 1 <= 12) { // 遍历月份找出区间日期  从一月份开始
            if (count_time * 1 <= dateEnd && count_time >= dateStart) { 
            	months.push(count_time * 1);
            }
	...
	function getYearData(dateStart, dateEnd){
        ...
        while(yearStart < yearEnd){
        	yearStart++;yearStart = yearStart * 1;
        	years.push(yearStart);
        }
        return years;
    };
    ...
    function query(){//根据表字段进行添加查询条件
    	...
    	if(!start_year_month){
    		$.ligerDialog.warn('请选择开始区间');
    	}
    	if(!end_year_month){
    		$.ligerDialog.warn('请选择结束区间');
    	}
    	var monthNumber = getMonthNumber(start_year_month,end_year_month);
		if(monthNumber >24){
			$.ligerDialog.warn('月份跨度最大为24个月!');
			return false;
		}
		if(monthNumber < 0){
			$.ligerDialog.warn('开始区间要小于结束区间');
			return false;
		}
		...
		if(!dept_no){dept_no= ".";}
    	//-----------------------------------------------------------------------------------------------
    	gridRight.options.parms=[];
    	gridRight.options.newPage=1;
    	...
    	gridRight.options.parms.push({name:'goal_code',value:liger.get("goal_code").getValue() == null ? "" : liger.get("goal_code").getValue()}); 
    	gridRight.options.parms.push({name:'dim_code',value:liger.get("dim_code").getValue() == null ? "" : liger.get("dim_code").getValue()}); 
    	gridRight.options.parms.push({name:'kpi_code',value:liger.get("kpi_code").getValue() == null ? "" : liger.get("kpi_code").getValue()}); 
    	gridRight.options.parms.push({name:'dept_kind_code',value:liger.get("dept_kind_code").getValue() == null ? "" : liger.get("dept_kind_code").getValue()}); 
    ...
    function f_setColumns01(start_year_month,end_year_month){ 
    	var yearStart =  start_year_month.substr(0,4);
    	var yearEnd =  end_year_month.substr(0,4); 
		var monthStart = start_year_month.substr(4,2);
		var monthEnd = end_year_month.substr(4,2);
		var columns = [{ display: '指标', name: 'kpi_name', align: 'left',width:120 }];
    ...
    function f_setColumns02(start_year_month,end_year_month){ 
		...
		var monthStart = start_year_month.substr(4,2);
		var monthEnd = end_year_month.substr(4,2);
		var columns = [{ display: '指标名称', name: 'kpi_name', align: 'left',width:120 }];
		var arr_months = getMonthData(start_year_month, end_year_month);
    ...
    function loadRightGrid(){
    	gridRight = $("#rightgrid").ligerGrid({
           columns: [
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmKpiCodeReportGrid.do?isCheck=false',
    		...
    		selectEvent:function(value){
    			loadComboBox({id:"#dim_code",url:"../queryPrmKpiDim.do?isCheck=false",value:"id",text:"text",autocomplete:true,hightLight:true,selectBoxWidth:'auto',defaultSelect:true,async:false,
    				selectEvent:function(value){
    					...
    					loadComboBox({id:"#kpi_code",url:"../queryPrmDeptKpi.do?isCheck=false&is_last=1",value:"id",text:"text",autocomplete:true,parms:fromData,hightLight:true,selectBoxWidth:'auto',defaultSelect:false,async:false});
        			}
    			});
			}
    	});
		
		loadComboBox({id:"#dept_kind_code",url:"../queryPrmDeptKind.do?isCheck=false",value:"id",text:"text",autocomplete:true,hightLight:true,selectBoxWidth:'172',width:'172',defaultSelect:true,async:false,
			selectEvent:function(value){
				var fromData={
					dept_kind_code:value
            	}	
				loadComboBox({id:"#dept_no",url:"../queryPrmDeptDict.do?isCheck=false",value:"id",text:"text",autocomplete:true,parms:fromData,hightLight:true,selectBoxWidth:'auto',defaultSelect:true,async:false});
			}
		});
	...
	<div id="toptoolbar">
	   <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="dim_code" type="text" id="dim_code" ltype="text" validate="{required:false,maxlength:20}" /></td>
            <td align="left"></td>
			<td align="right" class="l-table-edit-td" style="padding-left: 20px;">KPI指标：</td>
			<td align="left" class="l-table-edit-td"><input name="kpi_code" type="text" id="kpi_code" ltype="text" validate="{required:false,maxlength:20}" /></td>
			<td align="left"></td>
