...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '审核状态', name: 'audit_date', align: 'left'
					 		}
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmEmpKpiValueSchemeCalculate.do',
    ...
    function toolbar(){
		...
		obj.push({ text: '查询（<u>Q</u>）', id:'search', click: query,icon:'search' });
       	...
       	/* obj.push({ text: '计算（<u>D</u>）', id:'collect', click: collectEmpKpiValue, icon:'account' });
       	obj.push({ line:true }); */
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@" + this.hos_id + "@"
								...
								+ this.emp_no + "@" + this.emp_id +this.kpi_value + "@")
					});

					ajaxJsonObjectByUrl("saveBatchEmpKpiValueCalculate.do", {
	...
	function createEmpKpiValue(){
		...
		$.ligerDialog.confirm('确定生成吗?',function(yes){
	    	...
	    	ajaxJsonObjectByUrl("createPrmEmpTargetDataCalculate.do",ParamVo,function (responseData){
	     		if(responseData.state=="true"){
	     			query();
	     		}
	    	});
	...
	function auditEmpKpiValue(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定审核?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("auditPrmEmpKpiValueCalculate.do",{ParamVo : ParamVo.toString()},function (responseData){
                		if(responseData.state=="true"){
                			query();
                		}
                	});
	...
	function reAuditEmpKpiValue(){
         ...
         if (data.length == 0){
             ...
             $.ligerDialog.confirm('确定取消审核?', function (yes){
             	if(yes){
                 	ajaxJsonObjectByUrl("reAuditPrmEmpKpiValueCalculate.do",{ParamVo : ParamVo.toString()},function (responseData){
                 		if(responseData.state=="true"){
                 			query();
                 		}
                 	});
    ...
    function loadDict(){
            //字典下拉框
        autocomplete("#goal_code","../quertPrmGoalDict.do?isCheck=false","id","text",true,true,"",true,"","410");
  	  ...
  	  autocompleteAsync("#kpi_code", "../queryPrmEmpKpi.do?isCheck=false", "id",
    ...
    function print() {
		...
		var printPara={
				title: "职工KPI指标数据计算",//标题
				columns: JSON.stringify(grid.getPrintColumns()),//表头
				class_name: "com.chd.hrp.prm.service.PrmEmpKpiValueCalculateService",
				method_name: "queryPrmEmpKpiValueSchemeCalculatePrint",
				bean_name: "prmEmpKpiValueCalculateService"/* ,
