...
<script type="text/javascript">
		...
		function plupload(){  
			...
			$("#uploader").pluploadQueue({    
					...
					url : "readPrmTargetFiles.do?isCheck=false",    
					chunk_size : '2mb',           
					filters : {                
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns: [ 
				{ display: '指标编码', name: 'target_code', align: 'left',editor:{type:'text'}},
				{ display: '科室性质编码', name: 'nature_code', align: 'left',editor:{type:'text'}},
				{ display: '指标名称', name: 'target_name', align: 'left',editor:{type:'text'}},
				{ display: '指标性质', name: 'target_nature', align: 'left',editor:{type:'text'}},
				{ display: '指标描述', name: 'target_note', align: 'left',editor:{type:'text'}},
				{ display: '错误说明', name: 'error_type', align: 'left',width: '35%',
					render:function(rowdata, rowindex, value){
						return "<span style='color:red'>"+rowdata.error_type+"</span>";
					}
				}],  
				width : '99%',height: '370',enabledEdit: true,usePager:false,rownumbers:true,checkbox:true,selectRowButtonOnly:true,
				toolbar: { items: [
	...
	function itemclick(item){ 
		if(item.id){
			...
			ajaxJsonObjectByUrl("addBatchPrmTarget.do?isCheck=false",{ParamVo : JSON.stringify(data)},function (responseData){
