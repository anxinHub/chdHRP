...
<html style="overflow:hidden;">
  <head>
	...
	function  save(){
	   ...
	   if(is_audit == 1){
		   $.ligerDialog.error('目标已审核不允许修改指标 ');
    	   return ; 
	   }
       ...
       dialogData = dialog.get('data');//获取data参数
        ...
        ajaxJsonObjectByUrl("updatePrmEmpKpi.do",formPara,function(responseData){
    ...
    function loadDict(){
        ...
        autocomplete("#goal_code","../quertPrmGoalDict.do?isCheck=false","id","text",true,true,'',false,'${goal_code}');//目标名称
    	autocomplete("#dim_code","../queryPrmKpiDim.do?isCheck=false","id","text",true,true,'',false,'${dim_code}');//维度名称
	    autocomplete("#super_kpi_code","../queryPrmEmpKpiSuperKpiCode.do?isCheck=false","id","text",true,true,'',false,'${super_kpi_code}');//上级编码
	    autocomplete("#dept_code","../queryPrmDeptDict.do?isCheck=false","id","text",true,true,'',false,'${dept_code}');//科室名称
    	$('#nature_code').val("${nature_code}");//指标性质
    	$('#is_last').val("${is_last}");
