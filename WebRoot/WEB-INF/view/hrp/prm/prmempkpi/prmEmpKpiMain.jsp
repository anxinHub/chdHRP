...
<script type="text/javascript">
	...
	function  query(){
    	...
    	grid.options.parms.push({
    		name:'dept_id',
    		value:liger.get("dept_code").getValue() == ""  ? "" : liger.get("dept_code").getValue().split(".")[0]
    	});
    	
    	grid.options.parms.push({
        	name:'dept_no',
        	value:liger.get("dept_code").getValue() == "" ? "" : liger.get("dept_code").getValue().split(".")[1]
        }); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmEmpKpi.do',width: '100%', height: '100%', 
    ...
    function openUpdate(data){
		...
		$.ligerDialog.open({
			url: 'prmEmpKpiUpdatePage.do?isCheck=false'+param.toString(),
			height: 400,width: 600,
			title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
			data: {hos_id:hos_id},
			buttons: [ 
				{ text: '确定', onclick: function (item, dialog) { dialog.frame.savePrmEmpKpi(); },cls:'l-dialog-btn-highlight' }, 
    ...
    function toobar(){
		$("#toptoolmod").ligerToolBar({ items: [
			...
			{ line:true },
			{ text: '添加（<u>A</u>）', id:'add', click: addEmpKpi, icon:'add' },
			{ line:true },
			{ text: '选择（<u>C</u>）', id:'add', click: checkClick, icon:'outbox' },
			{ line:true },
			{ text: '删除（<u>D</u>）', id:'delete', click: deleteEmpKpi,icon:'delete' },
			{ line:true }
		]
		});
    }
	...
	function loadHotkeys(){
		hotkeys('Q',query);
		hotkeys('A',addEmpKpi);
		hotkeys('C',checkClick);
		hotkeys('D',deleteEmpKpi);
	}
	...
	function checkClick(){ 
		...
		$.ligerDialog.open({
			url: '../prmKpiLibPage.do?isCheck=false',height:$(window).height(),width: $(window).width(),
			title:'选择',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
			data: {
                __id: note.data.id,
    ...
    function addRows(target_data,__id){
    	...
    	$.ligerDialog.confirm('确定保存?',function(yes){
    		if(yes){
    	    	...
    	    	ajaxJsonObjectByUrl("addBatchPrmEmpKpi.do",formPara,function(responseData){
	...
	//添加页跳转
    function addEmpKpi(){
    	$.ligerDialog.open({url: 'prmEmpKpiAddPage.do?isCheck=false', 
			height: 400,width: 600,title:'添加',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,data: {
				hos_id:liger.get("hos_id").getValue()
			},
			buttons: [ 
				{ text: '确定', onclick: function (item, dialog) { dialog.frame.addPrmEmpKpi(); },cls:'l-dialog-btn-highlight' }, 
			    { text: '取消', onclick: function (item, dialog) { dialog.close(); } } 
			] 
		});
    }
	...
	function deleteEmpKpi(){
		...
		var data = gridManager.getCheckedRows();
	  	if (data.length == 0){
          	...
          	if(flag == false){
          		$.ligerDialog.error('已审核的目标不允许删除 ');
          		return ; 
          	}
          	
			$.ligerDialog.confirm('确定删除?', function (yes){
	          	if(yes){
	              	ajaxJsonObjectByUrl("deletePrmEmpKpi.do",{ParamVo : ParamVo},function (responseData){
	              		if(responseData.state=="true"){
	              			query();
	              		}
	              	});
	...
	function loadTree(){
		...
		ajaxJsonObjectByUrl("queryPrmEmpKpiTree.do?isCheck=false",param,function(responseData) {
