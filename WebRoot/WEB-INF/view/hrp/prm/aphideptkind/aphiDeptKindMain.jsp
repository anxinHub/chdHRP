...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAphiDeptKind.do',
                     width: '100%', height: '100%', checkbox: true,rownumbers:true,
                     selectRowButtonOnly:true,//heightDiff: -10,
                     toolbar: { items: [
                     	{ text: '查询（<u>Q</u>）', id:'search', click: query,icon:'search' },
                     	{ line:true },
				    					{ text: '添加（<u>A</u>）', id:'add', click: addDeptKind, icon:'add' },
				    	                { line:true },
				    	                { text: '删除（<u>D</u>）', id:'delete', click: deleteDeptKind,icon:'delete' },
											{ line:true }, 
				    	                { text: '导出Excel（<u>E</u>）', id:'export', click: exportDeptKind,icon:'pager' },
						                { line:true },
						                { text: '打印（<u>P</u>）', id:'print', click: printDeptKind,icon:'print' },
						                { line:true },
						                { text: '下载导入模板（<u>L</u>）', id:'downTemplate', click:downTemplateDeptKind,icon:'down' },
						                { line:true },
						                { text: '导入（<u>I</u>）', id:'import', click: importDeptKind,icon:'up' }
				    				]},
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'aphiDeptKindUpdatePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAphiDeptKind(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    ...
    function addDeptKind(){
    	...
    	$.ligerDialog.open({url: 'aphiDeptKindAddPage.do?isCheck=false', height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAphiDeptKind(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    ...
    function deleteDeptKind(){
          ...
          if (data.length == 0){
              ...
              $.ligerDialog.confirm('确定删除?', function (yes){
              	if(yes){
                  	ajaxJsonObjectByUrl("deleteAphiDeptKind.do",{ParamVo : ParamVo},function (responseData){
    ...
    function importDeptKind(){
    	...
    	$.ligerDialog.open({url: 'aphiDeptKindImportPage.do?isCheck=false', height: 450,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryAphiDeptKind.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryAphiDeptKind.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.dept_kind_code+"</td>"; 
					 trHtml+="<td>"+item.dept_kind_name+"</td>"; 
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div id="resultPrint" style="display:none">
	   	<table width="100%">
			   	<thead>
				<tr>
                <th width="200">科室分类编码</th>	
                <th width="200">科室分类名称</th>	
                <th width="200">科室分类说明</th>	
