...
<script type="text/javascript">
		...
		function plupload(){  
			$("#uploader").pluploadQueue({    
				...
				url : "readPrmKpiDimFiles.do?isCheck=false",    
				chunk_size : '2mb',           
		...
		function loadHead() {
			grid = $("#maingrid").ligerGrid({
				columns: [ 
					...
					{ display: '维度名称', name: 'dim_name', align: 'left',editor:{type:'text'}},
		...
		function itemclick(item){ 
			if(item.id){
				switch (item.id){
					...
					ajaxJsonObjectByUrl("addBatchPrmKpiDim.do?isCheck=false",{ParamVo : JSON.stringify(data)},function (responseData){
