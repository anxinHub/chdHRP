...
<script type="text/javascript">
	...
	function query() {
		...
		//根据表字段进行添加查询条件
		grid.options.parms.push({name : 'para_code',value : liger.get("para_name").getValue()});
		grid.options.parms.push({name : 'para_sql',value : $("#para_sql").val()});
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
				{display : '参数代码',name : 'para_code',align : 'left',render : 
			...
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryPrmFunParaMethod.do',
	...
	function toolbar(){
		...
		obj.push({ text: '查询（<u>Q</u>）', id:'search', click: query,icon:'search' });
       	...
       	obj.push({ text: '下载导入模板（<u>T</u>）', id:'downTemplate', click: templateFunParaMethod,icon:'down' });
       	obj.push({ line:true });
       	
       	obj.push({ text: '导入（<u>I</u>）', id:'import', click: importFunParaMethod,icon:'up' });
       	obj.push({ line:true });
	...
	function loadHotkeys() {
		...
		hotkeys('T', templateFunParaMethod);

		hotkeys('I', importFunParaMethod);
	...
	function addFunParaMethod() {
		...
		$.ligerDialog.open({
			url : 'prmFunParaMethodAddPage.do?isCheck=false',
			height : 400,width : 670,title : '添加',modal : true,showToggle : false,
			showMax : false,showMin : true,isResize : true,
			buttons : [ {
				text : '确定',
				onclick : function(item, dialog) {
					dialog.frame.savePrmFunParaMethod();
				},
	...
	function deleteFunParaMethod() {
		...
		$.ligerDialog.confirm('确定删除?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("deletePrmFunParaMethod.do", {ParamVo : ParamVo}, function(responseData) {
	...
	function importFunParaMethod() {
		$.ligerDialog.open({
			url : 'prmFunParaMethodImportPage.do?isCheck=false',
    ...
    function print() {
		...
		var printPara={
				...
				class_name: "com.chd.hrp.prm.service.PrmFunParaMethodService",
				method_name: "queryPrmFunParaMethodPrint",
				bean_name: "prmFunParaMethodService"/* ,
	...
	function openUpdate(obj) { 
		$.ligerDialog.open({
			url : 'prmFunParaMethodUpdatePage.do?isCheck=false&para_code=' + obj,data : {},
			height : 400,width : 670,modal : true,showToggle : false,showMax : false,showMin : false,isResize : true,
			buttons : [ {
				text : '确定',
				onclick : function(item, dialog) {
					dialog.frame.savePrmFunParaMethod();
	...
	function loadDict() {
		...
		autocomplete("#para_name", "../queryPrmFunParaMethod.do?isCheck=false","id", "text", true, true);
