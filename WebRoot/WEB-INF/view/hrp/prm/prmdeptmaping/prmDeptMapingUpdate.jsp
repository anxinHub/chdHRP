...
<script type="text/javascript">
	...
	var gridTatalManager = null;
     ...
     function  savePrmDeptMaping(){
        ...
        ajaxJsonObjectByUrl("addPrmDeptMaping.do",{},function(responseData){
     ...
     function loadHead() {
    	...
    	gridcharge = $('#maingrid1').ligerGrid({
    	  ...
    	  dataAction : 'server',dataType : 'server',usePager : true,
   	 	  url : 'queryPrmDept.do?isCheck=false&is_stop=0',
   	      width : '100%',height : '50%',
   	      enabledEdit: true,
   	      title : '绩效科室',
   	      checkbox : true,
   	 ...
   	 gridChargeManager = $('#maingrid1').ligerGetGridManager();
 		...
 		gridtitle = $("#maingrid2").ligerGrid({
 			...
 			dataType : 'server',
 			usePager : true,
 			url : 'queryDeptDictByPrmDept.do?isCheck=false&is_stop=0',
 			width : '100%',
 			height : '50%',
 			checkbox : true,
 			title : '平台科室',
     ...
     function deleteDeptMaping(){
	     ...
	     if (data.length == 0){
             ...
             $.ligerDialog.confirm('确定删除?', function (yes){
             	if(yes){
                 	ajaxJsonObjectByUrl("deletePrmDeptMapping.do",{ParamVo : ParamVo.toString()},function (responseData){
                 		if(responseData.state=="true"){
                 			query();
                 		}
                 	});
     ...
     function addDeptMaping(){
    	 ...
    	 ajaxJsonObjectByUrl("addPrmDeptMaping.do",{maping_data : JSON.stringify(totalData)},function (responseData){
     		if(responseData.state=="true"){
     			query();
     		}
     	});
