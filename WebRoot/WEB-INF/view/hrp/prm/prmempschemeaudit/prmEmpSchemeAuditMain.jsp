...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
						...
						{ display: '评分标准', name: 'kpi', align: 'center',width:80,
                     	...
                     	{ display: '取值方法', name: 'method_code', textField: 'method_name',align: 'left',width:120,
	                     		render : function(rowdata, rowindex,
	    								value) {
	                     			var str = JSON.stringify(rowdata).replace(new RegExp("'","gm"),"");
		   							return "<a href\"=javascript:void(0)\"  onclick='MethodEmpScheme("+str+")'>"+rowdata.method_name+"</a>";
	                     		}		
                     	},
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmEmpSchemeAudit.do',
    ...
    function toobar(){
    	...
    	obj.push({ text : '审核（<u>A</u>）',id : 'audit',click : auditPrmEmpScheme,icon : 'audit'});
    	obj.push({ line:true });
    	
    	obj.push({ text : '反审核（<u>B</u>）',id : 'reAudit',click : reAuditPrmEmpScheme,icon : 'back'});
    	obj.push({ line:true });
	...
	function ledEmpScheme(group_id, hos_id ,copy_code,
		...
		var parm = "group_id=" + group_id + "&" + "hos_id="
			...
			$.ligerDialog.open({
				url : 'prmEmpSchemeKpiLedAudit.do?isCheck=false&' + parm,
				height : 500,
				width : 800,
				title : '指示灯设定',
    ...
    function scoringEmpScheme(group_id, hos_id ,copy_code,
		...
		if (grade_meth == '01') {
			...
			$.ligerDialog.open({
				url : 'prmEmpSchemeSectionAudit.do?isCheck=false&' + parm,
				height : 500,
				width : 800,
				title : '区间法评分标准设定',
				modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
		...
		if (grade_meth == '02'  || grade_meth == '03') {
			...
			$.ligerDialog.open({
				url : 'prmEmpSchemesAdAudit.do?isCheck=false' + parm,
				height : 500,
				width : 800,
				title : '加扣分法评分标准设定',
				modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
    ...
    function MethodEmpScheme(rowdata) {
		...
		if (rowdata.method_code == '02' || rowdata.method_code == '03') {
			...
			+ rowdata.goal_code + "&" + "emp_no="
			+ rowdata.emp_no +  "&" + "emp_id="
			+ rowdata.emp_id + "&" + "kpi_code="
			...
			parent.$.ligerDialog.open({
				url : 'hrp/prm/prmempschemeaudit/prmFormulaMethodEmpSchemeAudit.do?isCheck=false&'+ parm,
				height: '500',
				width: '800',
				title : '计算方式设定',
				modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
    ...
    function auditPrmEmpScheme(){
		...
		$.ligerDialog.confirm('确定审核?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("auditPrmEmpScheme.do", {
	...
	function reAuditPrmEmpScheme(){
		...
		$.ligerDialog.confirm('确定取消审核?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("reAuditPrmEmpScheme.do", {
					ParamVo : ParamVo.toString()
				}, function(responseData) {
					if (responseData.state == "true") {
						query();
	 ...
	 //键盘事件
		function loadHotkeys(){
			...
			hotkeys('Q',query);
			hotkeys('A',auditPrmEmpScheme);
			hotkeys('B',reAuditPrmEmpScheme);
		}
     ...
     function onSelect(note){
      ...
      function loadTree(){
		...
		ajaxJsonObjectByUrl("queryEmpSchemeTreeAudit.do?isCheck=false", param,
	...
	<div id="toptoolbar">
	   <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="acc_year_month" type="text" id="acc_year_month"  ltype="text" validate="{required:true,maxlength:20}"   class="Wdate" onFocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:'yyyyMM'})"/></td>
            <td align="left"></td>
        </tr> 
    </table>
	</div>
