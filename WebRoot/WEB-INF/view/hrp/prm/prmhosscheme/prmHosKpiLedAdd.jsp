...
<script type="text/javascript">
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                { display: '等级', name: 'sec_code', align: 'center',textField: 'sec_name',
					editor : {
						...
						grid : {
							...
							url : 'queryPrmHosLedByEditerGrid.do?isCheck=false',
							pageSize : 30
						},
						keySupport: true,
                ...
                url:'queryPrmHosKpiLed.do?group_id=${group_id}&hos_id=${hos_id}&copy_code=${copy_code}&acc_year=${acc_year}&acc_month=${acc_month}&goal_code=${goal_code}&kpi_code=${kpi_code}&check_hos_id=${check_hos_id}',
  ...
  //键盘事件
	...
	function saveSchemeSection(){
		...
		if(validateGrid()){
			ajaxJsonObjectByUrl("saveHosKpiLed.do", formPara, function(responseData) {
	...
	function DeleteGridRow(){
	     ...
	     if (data.length == 0){
             ...
             $.ligerDialog.confirm('确定删除?', function (yes){
             	if(yes){
                 	ajaxJsonObjectByUrl("deletePrmHosKpiLed.do",{ParamVo : ParamVo.toString()},function (responseData){
                 		if(responseData.state=="true"){
                 			query();
                 		}
                 	});
