...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmHosScheme.do',heightDiff: -20,
    ...
    function toobar(){
    	...
    	obj.push({ text: '生成（<u>C</u>）', id:'create', click: createHosScheme, icon:'add' });
    	obj.push({ line:true });
    	
    	obj.push({ text: '删除（<u>D</u>）', id:'delete', click: deleteHosScheme,icon:'delete' });
    	obj.push({ line:true });
    	
    	obj.push({ text: '保存（<u>S</u>）', id:'save', click: saveHosScheme,icon:'add' });
    	obj.push({ line:true });
    	
    	obj.push({ text: '评分方法设定（<u>B</u>）', id:'scoring', click: scoringHosScheme,icon:'config' });
    	obj.push({ line:true });
    	
    	obj.push({ text: '取值方法设定（<u>F</u>）', id:'Method', click: MethodHosScheme,icon:'settings' });
    	obj.push({ line:true });
    	if('${sessionScope.prm_para_map["0702"]}' == 1){
    		obj.push({ text: '指示灯设定（<u>G</u>）', id:'led', click: ledHosScheme,icon:'settings' });
    		obj.push({ line:true });
    	}
		obj.push({ text: '引入（<u>O</u>）', id:'introduce', click: introduceHosScheme,icon:'back' });
		obj.push({ line:true });
		//
	...
	function saveHosScheme() {
		...
		ajaxJsonObjectByUrl("savePrmHosScheme.do", formPara, function(responseData) {	tree.selectNode(id);});
	}
	...
	function ledHosScheme(group_id, hos_id ,copy_code,
		...
		if(hos_id == null){
			...
			$.ligerDialog.open({
				url : 'prmHosSchemeKpiLed.do?isCheck=false&' + parm,
				height : 500,
				width : 800,
				title : '指示灯设定',
	...
	function createHosScheme(){
		$.ligerDialog.open({url: 'prmHosSchemeCreate.do?isCheck=false', height: 230,width: 400, title:'生成',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
			data: {
    ...
    function deleteHosScheme(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定删除?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("deletePrmHosScheme.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function scoringHosScheme(group_id, hos_id ,copy_code,
		...
		if (grade_meth == '01') {
			...
			$.ligerDialog.open({
				url : 'prmHosSchemeSection.do?isCheck=false&' + parm,
				height : 500,
				width : 800,
				title : '区间法评分标准设定',
				modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
		...
		if (grade_meth == '02') {
			...
			$.ligerDialog.open({
				url : 'prmHosSchemesAd.do?isCheck=false' + parm,
				height : 500,
				width : 800,
				title : '加分法评分标准设定',
				modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
    ...
    function MethodHosScheme() {
		...
		if (data[0].method_code == '02' || data[0].method_code == '03') {
			...
			+ data[0].goal_code + "&" + "check_hos_id="
			+ data[0].check_hos_id + "&" + "kpi_code="
			...
			parent.$.ligerDialog.open({
				url : 'hrp/prm/prmhosscheme/prmFormulaMethodHosScheme.do?isCheck=false&'+ parm,
				height: '500',
				width: '800',
				title : '计算方式设定',
				modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
    ...
    function introduceHosScheme(){
    	...
    	var parm = "&acc_month="
    		+ $("#acc_year_month").val().substring(4,6) + "&"+ "check_hos_id="
    		+ liger.get("check_hos_id").getValue() + "&" + "hos_id="
            + liger.get("hos_id").getValue()+"&goal_code="+liger.get("goal_code").getValue()+"&"+"acc_year="
            + $("#acc_year_month").val().substring(0,4);
    		$.ligerDialog.open({
    			url : 'introduceHosScheme.do?isCheck=false'+ parm,
    			height : 300,
    			width : 770,
    			title : '引入',
    			modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryPrmHosScheme.do",printPara,function (responseData){
	 ...
	 //键盘事件
		function loadHotkeys(){
			...
			hotkeys('Q',query);
			hotkeys('C',createHosScheme);
			hotkeys('D',deleteHosScheme);
			hotkeys('S',saveHosScheme);
			hotkeys('B',scoringHosScheme);
			hotkeys('F',MethodHosScheme);
			hotkeys('O',introduceHosScheme);
			hotkeys('P',printDate);
		}
     ...
     function onSelect(note){
      ...
      function loadTree(){
		...
		ajaxJsonObjectByUrl("queryHosSchemeTree.do?isCheck=false", param,
