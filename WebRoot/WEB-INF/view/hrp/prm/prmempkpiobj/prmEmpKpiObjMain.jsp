...
<script type="text/javascript">
    ...
    var colunm_id = "";//单条模式记录值职工id
    var colunm_no = "";//单条模式记录值职工变更no
    ...
    function  query(){
		...
		grid.options.parms.push({name:'dept_id',value:liger.get("dept_code").getValue()}); 
	...
	function loadHead(){
    	grid = $("#maingrid").ligerGrid({columns: [],
        	dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmEmpKpiObj.do',
    ...
    function toobar(){
		$("#toptoolmod").ligerToolBar({ items: [
			...
			{ line:true },
			{ text: '保存（<u>S</u>）', id:'add', click: saveEmpKpiObj, icon:'add',hide:isHide},
    ...
    function loadDict(){
    	...
    	autocomplete("#dept_code","../queryPrmDept.do?isCheck=false","id","text",true,true,"",true);//科室信息
    	...
    	autodate("#acc_year","yyyy");//绩效年度
    }
	
  	//save函数用于公用js中save方法的调用
    function save(obj){
    	saveEmpKpiObj(obj);
    }
	
	//保存
    function saveEmpKpiObj(obj){
		...
		var emp_kpi_data;//存储改变的数据
		
		//编辑模式： 1为单条模式   2为批量模式
		if(edit_id == 1){//单条模式
			emp_kpi_data = obj;
		}else{//批量模式
			emp_kpi_data = gridManager.getUpdated();
			
			var columns_head = gridManager.columns;//获取所有表头
			var emp_head = "";
			$.each(columns_head,function(i,v){
				//获取职工表头字段、职工id、职工变更号
				if(v.id != undefined && v.name != undefined && v.name != ''){
					emp_head = emp_head + v.name + ";" + v.id + ",";
				}
			});
			
		}
		
		if(emp_kpi_data.length == 0){
		...
		var formPara = {
			...
			colunm_emp_id : colunm_id,//用于单条模式查询
			colunm_emp_no : colunm_no,//用于单条模式查询
			emp_colunm : v_colunm,//用于单条模式保存时获取勾选的值
			goal_code : liger.get("goal_code").getValue(),
			acc_year :acc_year,
			hos_id:hos_id,
			emp_head:emp_head,//所有列,当前职工id,no
			emp_kpi_data : JSON.stringify(emp_kpi_data)
		};
		
		ajaxJsonObjectByUrl("addPrmEmpKpiObj.do", formPara, function(responseData) {
	...
	function grid_setColumns() {
		...
		var dept_id = liger.get("dept_code").getValue();
		...
		ajaxJsonObjectByUrl("queryPrmEmpColumns.do?isCheck=false", params,function(responseData) {
			if (responseData != null) {
