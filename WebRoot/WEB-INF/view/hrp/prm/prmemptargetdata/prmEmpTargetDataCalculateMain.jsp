...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	  ...
    	  grid.options.parms.push({name:'emp_no',value:liger.get("emp_no").getValue()}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPrmEmpTargetPrmTargetDataCalculate.do',
	...
	function toolbar(){
       ...
       /* 	obj.push({ text: '计算（<u>D</u>）', id:'collect', click: collectEmpTarget, icon:'account' });
       	obj.push({ line:true });
       	 */
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@" + this.hos_id + "@"
								...
								+ + this.emp_no + "@" + this.emp_id + "@" + this.target_value + "@")
					});

					ajaxJsonObjectByUrl("saveBatchPrmEmpTargetDataCalculate.do", {
	...
	function createEmpTarget(){
		...
		$.ligerDialog.confirm('确定生成吗?',function(yes){
			if(yes){
		    	...
		    	ajaxJsonObjectByUrl("createPrmEmpTargetDataCalculate.do",{ParamVo : ParamVo.toString()},function (responseData){
		     		if(responseData.state=="true"){
		     			query();
		     		}
		     	});
	...
	function auditEmpTarget(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定取消审核?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("auditPrmEmpTargetDataCalculate.do",{ParamVo : ParamVo.toString()},function (responseData){
                		if(responseData.state=="true"){
                			query();
                		}
                	});
	...
	function reAuditEmpTarget(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定取消审核?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("reAuditPrmEmpTargetDataCalculate.do",{ParamVo : ParamVo.toString()},function (responseData){
                		if(responseData.state=="true"){
                			query();
                		}
                	});
    ...
    function print() {
		...
		var printPara={
				...
				class_name: "com.chd.hrp.prm.service.PrmEmpTargetDataCalculateService",
				method_name: "queryPrmEmpTargetDataCalculatePrint",
				bean_name: "prmEmpTargetDataCalculateService"/* ,
