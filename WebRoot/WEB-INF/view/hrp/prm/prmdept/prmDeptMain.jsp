...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
				...
				{display : '科室分类',name : 'a',align : 'left',render : 
					function(rowdata, rowindex, value) {
						return rowdata.dept_kind_name;
						}
				}, 
				{display : '科室性质',name : 'b',align : 'left',render : 
					function(rowdata, rowindex, value) {
			...
			dataAction : 'server',dataType : 'server',usePager : true,url : 'queryPrmDept_DeptKind_DeptNature.do',
	...
	function addDept() {
		$.ligerDialog.open({
			url : 'prmDeptAddPage.do?isCheck=false',height : 450,width :500,title : '添加',
			modal : true,showToggle : false,showMax : false,showMin : true,isResize : true,
			buttons : [ {
				text : '确定',
				onclick : function(item, dialog) {
					dialog.frame.savePrmDept();
				},
	...
	function deleteDept() {
		...
		$.ligerDialog.confirm('确定删除?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("deletePrmDept.do", {
					ParamVo : ParamVo
	...
	function importDept() {
		parent.$.ligerDialog.open({ 
			url:'hrp/prm/prmdept/prmDeptImportPage.do?isCheck=false',
			data:{columns : grid.columns, grid : grid}, height: 300,
			width: 450,title:'科室导入',modal:true,showToggle:false,
	...
	function openUpdate(obj) {
		...
		var parm = "&dept_id=" + vo[0]
				   ...
				   "&copy_code=" + vo[3]
		$.ligerDialog.open({
			url : 'prmDeptUpdatePage.do?isCheck=false&' + parm,data : {},
	...
	function loadDict() {
		$("#dept_kind_codes").ligerComboBox({
				url : '../../prm/queryPrmDeptKind.do?isCheck=false',
				valueField : 'id',
				textField : 'text',
				selectBoxWidth : 160,
				autocomplete : true,
				...
				onSelected : function(selectValue) {
					...
					autocomplete("#dept_id","../../prm/queryDeptDict.do?isCheck=false", "id","text", true, true, para);
				}
			});
		
		autocomplete("#dept_id", "../../prm/queryPrmDept.do?isCheck=false", "id","text", true, true);
