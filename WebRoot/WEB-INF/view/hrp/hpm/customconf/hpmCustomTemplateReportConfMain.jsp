...
<script type="text/javascript">
    ...
    $(function (){
    	...
    	autocomplete("#template_kind_code","../queryAphiTemplateKind.do?isCheck=false","id","text",true,true,'',true);
	 ...
	 */
  	...
  	function saveTemplateReportConf(){
		...
		if(template_code == ''){
  		...
  		ajaxJsonObjectByUrl("addAphiCustomTemplateReportConf.do",para,function (responseData){
	...
	function deleteHpmTemplateReportConf(){
		...
		$.ligerDialog.confirm('确定删除模板?',function(yes){
			if(yes){
				ajaxJsonObjectByUrl("deleteAphiCustomTemplateReportConf.do",para,function (responseData){
            		if(responseData.state=="true"){
	...
	function onSelect(note){
		...
		ajaxJsonObjectByUrl("queryAphiCustomTemplateReportConf.do", formPara, function(responseData) {//查询模板
   	...
   	//设置树形菜单 
	function loadTree(){
		ajaxJsonObjectByUrl("queryAphiCustomTemplateReportConfTree.do?isCheck=false",{},function(responseData) {
	...
	//新增语句块
  	...
  	function addSqlBlock(){
  		...
  		divStr.append('<td align="right" class="l-table-edit-td"  style="padding-left:20px;"><input type="radio" name="template_type' + divCount + '" value="04" />继承</td>');
  		divStr.append('<td align="left"></td>');
					    ...
					    <div id="detailtool" style="background-color: #d6e7fc">
					    	<table cellpadding="0" cellspacing="0" class="l-table-edit" >
					    		<tr>
						            ...
						            <td align="right" class="l-table-edit-td"  style="padding-left:20px;"><input type="radio" name="template_type1" value="04"/>继承</td>
						            <td align="left"></td>
