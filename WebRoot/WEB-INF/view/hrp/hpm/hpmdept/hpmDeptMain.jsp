...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryHpmDept_DeptKind_DeptNature.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function addDept() {
		...
		$.ligerDialog.open({
			url : 'hpmDeptAddPage.do?isCheck=false',
			height : 450,
			width :500,
			title : '添加',
			modal : true,
	...
	function deleteDept() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHpmDept.do", {
						ParamVo : ParamVo
	...
	function exportDept() {
		...
		ajaxJsonObjectByUrl("queryHpmDept_DeptKind_DeptNature.do", exportPara,
				function(responseData) {
	...
	function templateDept() {
		...
		location.href = "downTemplateHpmDept.do?isCheck=false";
	}

	function importDept() {
		parent.$.ligerDialog.open({ 
			url:'hrp/hpm/hpmdept/hpmDeptImportPage.do?isCheck=false',
	...
	function printDept() {
		...
		ajaxJsonObjectByUrl(
				"queryHpmDept_DeptKind_DeptNature.do",printPara,function(responseData) {
					$.each(responseData.Rows, function(idx, item) {
						var trHtml = "<tr>"; 
						trHtml += "<td>" + item.dept_id + "</td>";
						trHtml += "<td>" + item.dept_name + "</td>";
