...
<script type="text/javascript">
	...
	var gridTatalManager = null;
     ...
     function loadHead() {
    	...
    	gridcharge = $('#maingrid1').ligerGrid({
    	  ...
    	  dataAction : 'server',dataType : 'server',usePager : true,
   	 	  url : 'queryDeptIncome.do?isCheck=false',
   	      width : '100%',height : '48%',
   	      enabledEdit: true,
   	      title : '科室',
   	      checkbox : true,
   	 ...
   	 gridChargeManager = $('#maingrid1').ligerGetGridManager();
 		...
 		gridTotal = $("#maingrid3").ligerGrid({
            ...
            { display: '收入项目名称', name: 'income_item_name', align: 'left'
 			...
 			dataType : 'server',
 			usePager : true,
 			url : 'queryHpmDeptIncomeMaping.do',
 			width : '98%',
 			height : '44%',
 			enabledEdit : true,
 			alternatingRow : true,
     ...
     function deleteDeptMaping(){
	     ...
	     if (data.length == 0){
             ...
             $.ligerDialog.confirm('确定删除?', function (yes){
             	if(yes){
                 	ajaxJsonObjectByUrl("deleteHpmDeptIncome.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
     ...
     function addDeptMaping(){
    	 ...
    	 ajaxJsonObjectByUrl("addHpmDeptIncomeMaping.do",{maping_data : JSON.stringify(totalDataArr)},function (responseData){
     		if(responseData.state=="true"){
     			query();
     		}
     	});
	   	...
	   	for(var i = 0; i < data.length;i++){
	   		for(var k = 0; k < data2.length;k++){
	   			for(var j = 0;j < totalData.length;j++){
					 if(data[i].dept_id == totalData[j].dept_id && data2[k].income_item_code == totalData[j].income_item_code ){
	   			...
	   			data3.push({
					...
					income_item_code:data2[k].income_item_code,
					income_item_name:data2[k].income_item_name
				 });
	   		}
		 }
		...
		<div style="margin-left: 30px;     padding: 0; float: left;">
			<table cellpadding="0" cellspacing="0" class="l-table-edit" >
	            <tr>
	                ...
	                <td align="left" class="l-table-edit-td"><input name="income_item_code" type="text" id="income_item_code" ltype="text" validate="{required:true,maxlength:20}" /></td>
	                <td align="left"></td>
