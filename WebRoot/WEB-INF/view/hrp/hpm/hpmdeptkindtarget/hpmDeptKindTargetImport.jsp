...
var grid;
var fileName;
var acc_data = [{ is_acc: true, text: '是' }, { is_acc: false, text: '否'}];
$(function(){
	 loadHead(null);
    function plupload(){  
     $("#uploader").pluploadQueue({           
            runtimes : 'flash,html5,gears,browserplus,silverlight,html4',           
            url : "readDeptKindTarget.do?isCheck=false",    
            chunk_size : '1mb',           
            filters : {               
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [ {
				display : '指标编码',
				name : 'target_code',
				align : 'left', editor: { type: 'text' }
			},  {
				display : '分配动因',
				name : 'is_acc',
				align : 'left',
				type : 'formatYesNo',
				editor: { type: 'select', data:acc_data, valueField: 'is_acc' },
	        ...
	        switch (item.id)
	        {
	            case "save":
	            	var data = gridManager.getData();
	            	 ajaxJsonObjectByUrl("addBatchDeptKindTargetDict.do?isCheck=false",{ParamVo : JSON.stringify(data)},function (responseData){
