...
<script type="text/javascript">
			...
			columns: [ 
				{ display: '指标编码', name: 'target_code', align: 'left',render: 
					function (rowdata, rowindex, value){
						return "<a href='#' onclick=\"openUpdate('"+rowdata.target_code+"');\" >"+rowdata.target_code+"</a>";
					}
				},
                
				{ display: '指标名称', name: 'target_name', align: 'left'},
			 	{ display: '指标描述', name: 'target_note', align: 'left'},
			 	{ display: '分配动因', name: 'is_acc', align: 'left',type:'formatYesNo'}
            ],
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryHpmDeptKindTarget.do',
			width: '100%',height: '100%',   checkbox: true,rownumbers:true,
			selectRowButtonOnly:true,//heightDiff: -10,
			onDblClickRow : function (rowdata, rowindex, value){
  					openUpdate(rowdata.target_code);//实际代码中temp替换主键
  			} 
		});
	...
	function toolbar(){
       	...
       	obj.push({ text: '添加（<u>A</u>）', id:'add', click: addDeptKindTarget, icon:'add' });
       	obj.push({ line:true });
       	
       	obj.push({ text: '删除（<u>D</u>）', id:'delete', click: deleteDeptKindTarget,icon:'delete' });
       	obj.push({ line:true });
	...
	function loadHotkeys(){
		...
		hotkeys('A',addDeptKindTarget);
		
		hotkeys('D',deleteDeptKindTarget);
  	...
  	//添加
  	function addDeptKindTarget(){
  		$.ligerDialog.open({
  			url: 'hpmDeptKindTargetAddPage.do?isCheck=false', 
  			title:'添加',height: 250,width: 400,modal:true,showToggle:false,
  			showMax:false,showMin: true,isResize:true,
  			buttons: [ 
  				{ text: '确定', onclick: 
  					function (item, dialog) { 
  						dialog.frame.saveDeptKindTarget(); 
  	...
  	function deleteDeptKindTarget(){
        ...
        $(data).each(function (){
        	checkIds.push(this.target_code);//实际代码中temp替换主键
        });
        $.ligerDialog.confirm('确定删除?', function (yes){
        	if(yes){
            	ajaxJsonObjectByUrl("deleteHpmDeptKindTarget.do",{checkIds:checkIds.toString()},function (responseData){
  	...
  	function downTemplate(){
  		location.href = "downTemplateDeptKindTarget.do?isCheck=false";
  	}
  	
  	//导入
  	function importData(){
       	$.ligerDialog.open({
       		url: 'deptKindTargetImportPage.do?isCheck=false', height: 430,width: 760, isResize:true, title:'导入'
       	});
    ...
    function openUpdate(obj){
    	//实际代码中&temp替换主键
    	$.ligerDialog.open({ 
    		url: 'hpmDeptKindTargetUpdatePage.do?isCheck=false&target_code='+obj,data:{}, 
