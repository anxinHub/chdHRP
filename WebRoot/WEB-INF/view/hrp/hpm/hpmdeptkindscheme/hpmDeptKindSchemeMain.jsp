...
<script type="text/javascript">
           ...
           columns: [ 
                     { display: '科室分类编码', name: 'dept_kind_code', align: 'left',
                    	 render: function (rowdata, rowindex, value){
							return "<a href='#' onclick=\"openUpdate('"+rowdata.dept_kind_code+"','"+rowdata.item_code+"');\" >"+rowdata.dept_kind_code+"</a>";
			           }
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryHpmDeptKindScheme.do',
    				...
    				{
    					openUpdate(rowdata.dept_kind_code+","+dept_kind_code.item_code);//实际代码中temp替换主键
    				} 
                   });
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'hpmDeptKindSchemeAddPage.do?isCheck=false&app_mod_code='+app_mod_code, height: 500,width: 900, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveDeptKindScheme(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        var checkIds =[];
                        $(data).each(function (){
                        	checkIds.push(this.dept_kind_code+";"+this.item_code);//实际代码中temp替换主键
                        });
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteHpmDeptKindScheme.do",{checkIds:checkIds.toString()},function (responseData){
                    ...
                    return;
                case "downTemplate":location.href = "downTemplateDeptKindScheme.do?isCheck=false";return;

                case "import":
                   	$.ligerDialog.open({url: 'deptKindSchemeImportPage.do?isCheck=false', height: 430,width: 760, isResize:true, title:'导入'});
                   	return;
                case "Excel":
                case "Word":
                case "PDF":
    ...
    function openUpdate(obj1,obj2){
    	//实际代码中&temp替换主键
    	$.ligerDialog.open({ 
    		url: 'hpmDeptKindSchemeUpdatePage.do?isCheck=false&dept_kind_code='+obj1+'&item_code='+obj2,data:{}, 
    		height:$(window).height(),width:$(window).width(), title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
    		buttons: [ 
    			{ text: '确定', onclick: 
    				function (item, dialog) { 
    					dialog.frame.saveDeptKindScheme(); 
    				},
    				cls:'l-dialog-btn-highlight' 
    			}, 
    			{ text: '取消', onclick: 
