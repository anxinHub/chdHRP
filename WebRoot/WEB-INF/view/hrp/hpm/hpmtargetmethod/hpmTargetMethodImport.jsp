...
<script type="text/javascript">
		...
		function plupload(){  
			...
			$("#uploader").pluploadQueue({    
					...
					url : "readHpmTargetMethodFiles.do?isCheck=false",    
		...
		grid = $("#maingrid").ligerGrid(
				{
					columns : [
							...
							{
								display : '01:手工录入 02:计算公式 03:取值函数',
								name : 'method_code',
								align : 'left',
								editor : {
									type : 'text'
	...
	function itemclick(item) {
		if (item.id) {
			...
			case "save":
				...
				ajaxJsonObjectByUrl("addBatchHpmTargetMethod.do?isCheck=false",
						{
							...
							if (responseData.state == "true") {
								...
								parent.query();
								$.ligerDialog.confirm('导入成功！是否关闭当前页面',
										function(yes) {
											if (yes == true) {
												dialog.close();
											} else {
