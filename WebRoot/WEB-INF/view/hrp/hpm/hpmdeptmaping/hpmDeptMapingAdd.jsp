...
<script type="text/javascript">
	...
	var gridTatalManager = null;
     ...
     function  saveHpmDeptMaping(){
        ...
        ajaxJsonObjectByUrl("addHpmDeptMaping.do",{},function(responseData){
     ...
     function loadHead() {
   	 ...
   	 gridChargeManager = $('#maingrid1').ligerGetGridManager();
 		...
 		gridtitle = $("#maingrid2").ligerGrid({
 			...
 			dataType : 'server',
 			usePager : true,
 			url : 'queryDeptDictByHpmDept.do?isCheck=false',
 			width : '100%',
 			height : '50%',
 			checkbox : true,
 			title : '平台科室',
     ...
     function deleteDeptMaping(){
	     ...
	     if (data.length == 0){
             ...
             $.ligerDialog.confirm('确定删除?', function (yes){
             	if(yes){
                 	ajaxJsonObjectByUrl("deleteHpmDeptMapping.do",{ParamVo : ParamVo.toString()},function (responseData){
                 		if(responseData.state=="true"){
                 			query();
                 		}
                 	});
     ...
     function addDeptMaping(){
    	 ...
    	 ajaxJsonObjectByUrl("addHpmDeptMaping.do",{maping_data : JSON.stringify(totalDataArr)},function (responseData){
     		if(responseData.state=="true"){
     			query();
     		}
     	});
