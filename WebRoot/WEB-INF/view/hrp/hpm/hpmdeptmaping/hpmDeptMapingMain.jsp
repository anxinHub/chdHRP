...
<script type="text/javascript">
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
               ...
               { display: '拆分比例', name: 'spilt_perc', align: 'left'
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryHpmDeptMaping.do',
    ...
    function addDeptMaping(){
    	...
    	parent.$.ligerDialog.open({url: 'hrp/hpm/hpmdeptmaping/hpmDeptMapingAddPage.do?isCheck=false', height: $(window).height(),
    ...
    function deleteDeptMaping (){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定删除?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("deleteHpmDeptMapping.do",{ParamVo : ParamVo},function (responseData){
    ...
    function exportDeptMaping(){
		...
		ajaxJsonObjectByUrl("queryHpmDeptMaping.do",exportPara,function (responseData){
    ...
    function importDeptMaping (){
    	...
    	$.ligerDialog.open({url: 'hpmDeptMapingImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
    ...
    function printDeptMaping (){
		...
		ajaxJsonObjectByUrl("queryHpmDeptMaping.do",printPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.dept_id+"</td>"; 
					 trHtml+="<td>"+item.sys_dept_id+"</td>"; 
    ...
    function openUpdate(obj){
		...
		var parm = "group_id="+
			...
			vo[3]   +"&sys_dept_id="+ 
			vo[4];
		parent.$.ligerDialog.open({url: 'hrp/hpm/hpmdeptmaping/hpmDeptMapingUpdatePage.do?isCheck=false&'+parm, height: $(window).height(),
    ...
    function loadDict(){
    	...
    	autocomplete("#dept_code1","../queryDeptDict.do?isCheck=false","id","text",true,true);
