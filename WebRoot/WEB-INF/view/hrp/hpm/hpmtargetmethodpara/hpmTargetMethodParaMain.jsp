...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryHpmTargetMethodParaNature.do',
  ...
  //键盘事件
	function loadHotkeys(){
  	...
  	function addTargetMethodPara (){
  		...
  		$.ligerDialog.open({url: 'hpmTargetMethodParaAddPage.do?isCheck=false', height: 600,width: 1200, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveHpmTargetMethodPara(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
  	...
  	function deleteTargetMethodPara (){
         ...
         if (data.length == 0){
             ...
             $.ligerDialog.confirm('确定删除?', function (yes){
             	if(yes){
                 	ajaxJsonObjectByUrl("deleteHpmTargetMethodPara.do",{ParamVo : ParamVo},function (responseData){
  	...
  	function importTargetMethodPara (){
  		...
  		$.ligerDialog.open({url: 'hpmTargetMethodParaImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
  ...
  //打印数据
	 function printTargetMethodPara (){
		...
		ajaxJsonObjectByUrl("queryHpmTargetMethodParaNature.do",printPara,function (responseData){
	 ...
	 function exportTargetMethodPara (){
		...
		ajaxJsonObjectByUrl("queryHpmTargetMethodParaNature.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.method_code+"</td>"; 
					 trHtml+="<td>"+item.method_name+"</td>"; 
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'hpmTargetMethodParaUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveHpmTargetMethodPara(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });

    }
    function loadDict(){
            //字典下拉框
         autocomplete("#target_name","../quertHpmTargetDict.do?isCheck=false","id","text",true,true);
    	 autocomplete("#method_code","../queryHpmTargetMethodPara.do?isCheck=false","id","text",true,true);
    	 autocomplete("#nature_code","../queryHpmTargetNature.do?isCheck=false","id","text",true,true);   
