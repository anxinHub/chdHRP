...
<script type="text/javascript">
	...
	function toolbar(){
		...
		obj.push({ text: '查询（<u>Q</u>）', id:'search', click: query,icon:'search' });
       	...
       	obj.push({ text: '下载导入模板（<u>L</u>）', id:'downTemplate', click: templateDept,icon:'down' });
       	obj.push({ line:true });
	...
	function addDept() {
		...
		$.ligerDialog.open({
			url : 'hpmDeptHipAddPage.do?isCheck=false',
			height : 450,
			width :500,
			title : '添加',
			modal : true,
	...
	function deleteDept() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAphiDeptHip.do", {
						ParamVo : ParamVo
	...
	function templateDept() {
		...
		location.href = "downTemplateHpmDeptHip.do?isCheck=false";
	}

	function importDept() {
		parent.$.ligerDialog.open({ 
			url:'hrp/hpm/hpmdepthip/hpmDeptHipImportPage.do?isCheck=false',
	...
	function openUpdate(obj) {
		...
		var parm = "&dept_id=" + vo[0]
				   ...
				   "&copy_code=" + vo[3]
		$.ligerDialog.open({
			url : 'hpmDeptHipUpdatePage.do?isCheck=false&' + parm,
			data : {},
			height : 500,
			width : 500,
			title : '修改',
	...
	function loadDict() {
		...
		autocomplete("#dept_id", "../../hpm/queryAphiDeptHip.do?isCheck=false", "id","text", true, true);
