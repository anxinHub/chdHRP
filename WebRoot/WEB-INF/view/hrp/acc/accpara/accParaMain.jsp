...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '参数名称', name: 'para_name', align: 'left',width:300
					 },
                     { display: '参数值', name: 'para_value', align: 'center',width:159,
							render : function(rowdata, rowindex,
									value) {
								var str = "";
								if(rowdata.para_type == '0'){
									str = "<input name='para_value"+rowindex+"' type='text' id='para_value"+rowindex+"'"
										+" value='"+praseStrEmpty(rowdata.para_value)+"' style='margin-top:5px;width: 147px;'/>";
								}
                     ...
                     { display: '参数说明', name: 'note', align: 'left',width:540},
					 {display:'行号',name:'num',hide:true,width:0,
                    	 render:function(rowdata, rowindex,value) {
                    		 rowdata.num = rowindex
                    	 }
					 }/* ,
                     { display: '操作', name: 'edit', align: 'center',width:100,
							render : function(rowdata, rowindex,
									value) {
                     ...
                     dataAction: 'server',dataType: 'server',usePager:false,url:'queryAccPara.do',
	...
	/* function save(group_id,hos_id,copy_code,para_code,rowindex){
		...
		var formPara = {
		 ...
		 ajaxJsonObjectByUrl("updateAccPara.do?isCheck=false",formPara,function(responseData){
	...
	function save(){
		...
		$(data).each(function(){
			/* var reg = new RegExp("^[0-9][0-9]*$");*/
			var para_value = praseStrEmpty($("#para_value"+this.num+"").val());
			/*if(this.para_type == 0 && !reg.test(para_value)){
				str += this.para_code + ','; 
			}else{ */
				formPara.push(
					this.para_code +"@"+ para_value 
				)
			//}
		})
		
		if(formPara.length>0){
			/* if(str != ''){
				$.ligerDialog.error("参数编码:"+str+"参数值输入数据类型错误，请输入整数类型数据")
			}else{ */
				ajaxJsonObjectByUrl("updateBatchAccPara.do",{formPara:formPara.toString()},function(responseData){
		            if(responseData.state=="true"){
		                query();
		            }
		        }); 
