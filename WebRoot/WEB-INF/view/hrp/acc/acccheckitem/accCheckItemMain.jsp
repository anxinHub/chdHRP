...
<script type="text/javascript">
	...
	function loadHead(obj){
		if('maingrid' == obj){
			grid = $("#maingrid").ligerGrid({
    	    	columns: [
    	            ...
    	            { display: '核算项编码', name: 'check_item_code', align: 'left',width:160,render : 
				...
				parms:[{name:'is_stop',value:$("#is_stop").val()}],
				width: '100%', height: '100%', checkbox: true,rownumbers:true,selectRowButtonOnly:true,heightDiff: 0,
				onDblClickRow : 
					function (rowdata, rowindex, value){
 						openUpdate(
							rowdata.check_item_id   + "|" + 
    	...
    	if('typegrid' == obj ){
			grid = $("#typegrid").ligerGrid({
				columns: [
					{ display: '分类编码', name: 'type_code', align: 'left',width:240,render : 
						function(rowdata, rowindex,value) {
							return "<a href=javascript:openTypeUpdate('"+
									rowdata.check_type_id   + "|" + 
									rowdata.type_code   + "|" + 
									rowdata.group_id   + "|" + 
									rowdata.hos_id   + "|" + 
					...
					{ display: '是否停用', name: 'is_stop', align: 'left',width:200,render : 
						function(rowdata, rowindex,value) {
							if(rowdata.is_stop == 0){
								return "启用";
							}else{
	...
	function loadHotkeys() {
	 ...
	 function add_checkitem(){
		parent.$.ligerDialog.open({
			url: 'accCheckItemAddPage.do?isCheck=false&check_type_id=${check_type_id}&check_type_name=${check_type_name}', 
	...
	function add_checkItemType(){
		parent.$.ligerDialog.open({
			url: 'accCheckItemTypeAddPage.do?isCheck=false&check_type_id=${check_type_id}&check_type_name=${check_type_name}', 
			height: 280,width: 420, title:'核算分类添加',modal:true,showToggle:false,showMax:false,showMin: false,
			parentframename:window.name,isResize:true,
			buttons: [ 
				{ text: '确定', onclick: 
					function (item, dialog) { 
						dialog.frame.saveAccCheckItemType(); 
					},cls:'l-dialog-btn-highlight' 
				}, 
				{ text: '取消', onclick: 
					function (item, dialog) { 
    ...
    function openUpdate(obj){
    	...
    	parent.$.ligerDialog.open({ 
    		url : 'accCheckItemUpdatePage.do?isCheck=false&' + parm,data:{}, height: 400,width: 500, 
    ...
    function openTypeUpdate(obj){
		...
		parent.$.ligerDialog.open({ 
			url : 'accCheckItemTypeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 280,width: 420, 
    ...
    function extend(obj){
    	parent.$.ligerDialog.open({ 
    		url : 'accCheckItemExtendPage.do?isCheck=false&check_type_id=${check_type_id}&check_type_name=${check_type_name}' ,
			data:{}, height: 350,width: 400, title:'继承',modal:true,showToggle:false,showMax:false,
    ...
    function accCheckItemImportPage(){
    	$.ligerDialog.open({
    		url: 'accCheckItemImportPage.do?isCheck=false&check_type_id=${check_type_id}&check_type_name=${check_type_name}', 
    		height: 500,width:900, title:'核算项导入',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true 
    	});
    }
    ...
    function printDate(){
		...
		var printPara={
			...
			class_name: "com.chd.hrp.acc.service.commonbuilder.AccCheckItemService",
			method_name: "queryAccCheckItemPrint",
			bean_name: "accCheckItemService",
    ...
    function printDateType(){
		...
		var printPara={
			...
			title:'核算分类',
			columns: JSON.stringify(grid.getPrintColumns()),//表头
			class_name: "com.chd.hrp.acc.service.commonbuilder.AccCheckItemTypeService",
			method_name: "queryAccCheckItemTypePrint",
			bean_name: "accCheckItemTypeService",
			heads: JSON.stringify(heads),//表头需要打印的查询条件,可以为空
			check_type_id : ${check_type_id}
		};
