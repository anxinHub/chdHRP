     ...
     function singlePath(currNode) {
       ...
       function itemclick1(item){ 
           if(item.id)
           {
               ...
               {
               	case "add":
             		$.ligerDialog.open({url: '../accchecktype/accCheckTypeAddPage.do', height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAccCheckType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
             		return;
               	case "delete":
            		...
            		//var ParamVo =[];
                    	if(node[0].is_sys == 1){
            				...
            				$.ligerDialog.confirm('确定删除?', function (yes){
                             	if(yes){   
                                    $.post("../accchecktype/deleteAccCheckType.do",{check_type_id: node[0].id,group_id:node[0].group_id,hos_id:node[0].hos_id,copy_code:node[0].copy_code,check_type_name:node[0].name},function (responseData){
               	...
               	case "update":
            		...
            		$.each(node,function(index,value){
            			...
            			$.ligerDialog.open({url: '../accchecktype/accCheckTypeUpdatePage.do?check_type_id='+value.id+'&group_id='+value.group_id+'&hos_id='+value.hos_id+'&copy_code='+value.copy_code, 
