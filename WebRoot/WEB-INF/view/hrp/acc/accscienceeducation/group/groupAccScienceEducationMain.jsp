...
<script type="text/javascript">
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '合计',  align: 'center',
						 columns:[
                                    ...
                                    { display: '本期收入', name: 'total_sr', align: 'right',formatter:'###,##0.00',
                							render : function(rowdata, rowindex,
                									value) {
       								 return "<a href=javascript:openUpdate('" + rowdata.proj_id + "|"+ rowdata.proj_no + "|"
 		      	                       + "ScienceEducation_total_sr"+"')>" +  formatNumber(rowdata.bal_sr+rowdata.match_sr, 2, 1) + "</a>"; 
                                    ...
                                    { display: '本期支出', name: 'total_ot', align: 'right',formatter:'###,##0.00',
                    							render : function(rowdata, rowindex,
                    									value) {
                    								return "<a href=javascript:openUpdate('" + rowdata.proj_id + "|"+ rowdata.proj_no + "|"
              		      	                       + "ScienceEducation_total_ot"+"')>"+formatNumber(rowdata.bal_ot+rowdata.match_ot, 2, 1) + "</a>"; 
					 ...
					 { display: '外拨资金',  align: 'center',
						 columns:[
                                  { display: '期初余额', name: 'bal_os', align: 'right',formatter:'###,##0.00'},
                                  { display: '本期收入', name: 'bal_sr', align: 'right',formatter:'###,##0.00',
                            		  render : function(rowdata, rowindex,
          									value) {         									
	                       return "<a href=javascript:openUpdate('" + rowdata.proj_id + "|"+ rowdata.proj_no + "|"
	                       + "ScienceEducation_bal_sr"+"')>" + formatNumber(rowdata.bal_sr, 2, 1) + "</a>"; 
                           			}},
                                  { display: '本期支出', name: 'bal_ot', align: 'right',formatter:'###,##0.00',
                                		  render : function(rowdata, rowindex,value) {         									
		      	                       return "<a href=javascript:openUpdate('" + rowdata.proj_id + "|"+ rowdata.proj_no + "|"
		      	                       + "ScienceEducation_bal_ot"+"')>" + formatNumber(rowdata.bal_ot, 2, 1) + "</a>"; 
                                 			}},
                                  { display: '期末余额', name: 'bal_od', align: 'right',formatter:'###,##0.00'}
                                ]
					 },
					 { display: '配套资金',  align: 'center',
						 columns:[
                                  { display: '期初余额', name: 'match_os', align: 'right',formatter:'###,##0.00'},
                                  { display: '本期收入', name: 'match_sr', align: 'right',formatter:'###,##0.00',
                            		  render : function(rowdata, rowindex,value) {         									
   		      	                       return "<a href=javascript:openUpdate('" + rowdata.proj_id + "|"+ rowdata.proj_no + "|"
   		      	                       + "ScienceEducation_match_sr"+"')>" + formatNumber(rowdata.match_sr, 2, 1) + "</a>"; 
                                    			}},
                                  { display: '本期支出', name: 'match_ot', align: 'right',formatter:'###,##0.00',
                       				render : function(rowdata, rowindex,value) {         									
       		      	                       return "<a href=javascript:openUpdate('" + rowdata.proj_id + "|"+ rowdata.proj_no + "|"
       		      	                       + "ScienceEducation_match_ot"+"')>" + formatNumber(rowdata.match_ot, 2, 1) + "</a>"; 
                                        			}},
                                  { display: '期末余额', name: 'match_od', align: 'right',formatter:'###,##0.00'}
                                ]
					 }
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryGroupAccScienceEducation.do',
                     width: '100%', height: '93%', checkbox: false,rownumbers:true,delayLoad:true,
    ...
    function printDate(){
    	...
    	var heads={
     		...
     		var printPara={
     			...
     			class_name: "com.chd.hrp.acc.service.books.memorandumbook.GroupAccScienceEducationService",
	  			method_name: "queryGroupAccScienceEducationPrint",
	  			bean_name: "groupAccScienceEducationService",
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
			url: 'hrp/acc/accscienceeducation/group/groupAccscienceeducationDataMiningPage.do?isCheck=false&'+parm, data: {parm:parm},
