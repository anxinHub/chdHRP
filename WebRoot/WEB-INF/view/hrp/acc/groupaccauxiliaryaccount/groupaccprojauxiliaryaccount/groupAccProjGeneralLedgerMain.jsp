...
<jsp:include page="${path}/inc_jquery_1.9.0.jsp"/>
    ...
    function  query(){
        ...
        if(liger.get("proj_code").getValue() == '' || liger.get("proj_code").getValue() == undefined){
        	$.ligerDialog.error('项目为必填项');
        	return ;
        } 
        //根据表字段进行添加查询条件
    	...
    	if($("#source_id").val() == ""){
    		grid.options.parms.push({name:'source_id',value:'0'}); 
    	}else{
		...
		if(proj_code.split(",").length>1){
			 if($("#flag").val() == "true"){
			 	grid.options.parms.push({name:'obj_code',value:proj_code}); 
        	...
        	if(proj_code==""){
    	 		...
    	 		grid.options.parms.push({name:'obj_code',value:proj_code.split(".")[2]}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:false,url:'queryGroupAccProjSubjGeneralLedger.do',
    	...
    	autocomplete("#acc_year_month2","../../queryYearMonth.do?isCheck=false","id","text",true,true,'',false,false,'90');
    	 ...
    	 autocompleteObj({
   			...
   			var	para = {
   		 		...
   		 		autocomplete("#proj_code","../../../sys/queryProjDictDictByRela.do?isCheck=false","id","text",true,true,para,true,false,'180');
...
function projSelector(){
    	...
    	$.ligerDialog.open({ url : '../../groupbookselector/groupAccBookProjSelectorPage.do?isCheck=false&flag='+$("#flag").val()+'&rela_code='+liger.get("rela_code").getValue(),
...
//普通打印数据
	function print(){
		...
		var heads={
		 		...
		 		var printPara={
					...
					method_name: "collectGroupAccProjSubjGeneralLedgerPrint",
	...
	//打印维护
	 function myPrintSet(){
		 officeFormTemplate({template_code:"01018"});
	 }
    </script>
</head>
...
<input type="hidden" id="subj_query_code" name="subj_query_code"/>
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit"  border="0">
        ...
        <tr>
            ...
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;" ><font size="2" color="red">*</font>项目：</td>
            <td align="left" class="l-table-edit-td" style="padding-left:20px;"><input id="proj_code" name="proj_code" /></td>
<!--             <td> -->
<!--             	<input class="l-button l-button-test" style="float: right;" type="button" value="选择" onclick="projSelector();"/> -->
<!--             </td> -->
