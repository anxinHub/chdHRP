...
<script src="<%=path%>/lib/stringbuffer.js"></script>
    ...
    function  query(){
    	...
    	if($("#is_happend").attr("checked") == true){
    		is_happend = 1;
    	}else{  
    		is_happend = 0;
    	}
		...
		function loadHead() {
			grid = $("#maingrid").ligerGrid({
						columns : [{display : '期间',align : 'left',
									columns : [ {display : '年', isSort : false, name : 'acc_year', align : 'left', width : '4%'}, 
								...
								{ display : '凭证号', isSort : false, name : 'vouch_no', align : 'left', width : '8%',
									render : function(rowdata,rowindex, value) {
										if (rowdata.vouch_no != null) {
											return "<a href=javascript:openSuperVouch('" + rowdata.vouch_no.split('-')[2] + "')><div>"
													+ rowdata.vouch_no.split('-')[0] + "-" + rowdata.vouch_no.split('-')[1] + "</div></a>";
										}
									}
								...
								dataType : 'server',
								usePager : false,
								url : 'queryGroupAccSubjEmpDetailLedger.do',
								width : '100%',
								height : '100%',
								checkbox : false,
								rownumbers : true,
								...
								toolbar : {items : [ {text : '查询',id : 'search',icon : 'search',click : query}, 
								                     {line : true}, 
		...
		function loadDict() {
			...
			$("#emp_code").ligerTextBox({ width : 250 });
		...
		function subjSelector() {
			...
			$.ligerDialog.open({
				url : '../../groupbookselector/groupAccBookSubjSelectorPage.do?isCheck=false&flag=' + $("#subj_flag").val() + '&sign=2'+'&rela_code='+liger.get("rela_code").getValue(),
				data : {
		...
		function print() {
			...
			var heads={
			 		...
			 		var printPara={
						...
						method_name: "collectGroupAccSubjEmpDetailLedgerPrint",
		...
		function myPrintSet(){
			 officeFormTemplate({template_code:"01016"});
		 }
	</script>
</head>
...
<input type="hidden" id="subj_query_code" name="subj_query_code"/>
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit"  border="0">
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"  style="padding-left:20px;" colspan="3" >
