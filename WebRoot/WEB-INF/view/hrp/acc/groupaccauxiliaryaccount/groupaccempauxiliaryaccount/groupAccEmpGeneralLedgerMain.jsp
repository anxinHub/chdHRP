...
<jsp:include page="${path}/inc_jquery_1.9.0.jsp"/>
    ...
    function  query(){
        ...
        if(liger.get("emp_code").getValue() == '' || liger.get("emp_code").getValue() == undefined){
		...
		var emp_code = liger.get("emp_code").getValue();
		if(emp_code.split(",").length>1){
			 if($("#flag").val() == "true"){
				 grid.options.parms.push({name:'obj_code',value:emp_code}); 
        	...
        	if(emp_code==""){
        		 ...
        		 grid.options.parms.push({name:'obj_select_flag',value:"1"});
    	 		 ...
    	 		 grid.options.parms.push({name:'obj_code',value:emp_code.split(".")[2]}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:false,url:'queryGroupAccEmpSubjGeneralLedger.do',
...
function empSelector(){
    	...
    	$.ligerDialog.open({ url : '../../groupbookselector/groupAccBookEmpSelectorPage.do?isCheck=false&flag='+$("#flag").val() +'&rela_code='+liger.get("rela_code").getValue(),
    			data:{listBoxData : obj_box_data}, height: 470,width: 720, title:'账簿职工选择器',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
				buttons: [ 
 				           { text: '确定', onclick: 
 				        	   function (item, dialog) { 
								   ...
								   liger.get("emp_code").setValue(obj_param);
     				              
   				        	      liger.get("emp_code").setText(param);
	 				        	   dialog.close(); 
...
function subjSelector(){
	...
	$.ligerDialog.open({ url : '../../groupbookselector/groupAccBookSubjSelectorPage.do?isCheck=false&flag='+$("#subj_flag").val()+'&sign=2'+'&rela_code='+liger.get("rela_code").getValue(),
	...
	function print(){
		...
		var heads={
		 		...
		 		var printPara={
		 			...
		 			class_name: "com.chd.hrp.acc.service.books.groupauxiliaryaccount.GroupAccEmpAuxiliaryAccountService",
					method_name: "collectGroupAccEmpSubjGeneralLedgerPrint",
					bean_name: "groupAccEmpAuxiliaryAccountService",
	...
	function printDate(){
		...
		selPara["template_code"]="01013";
	   	selPara["class_name"]="com.chd.hrp.acc.service.books.groupauxiliaryaccount.GroupAccEmpAuxiliaryAccountService";
	   	selPara["method_name"]="collectGroupAccEmpSubjGeneralLedgerPrintDate";
	   	selPara["bean_name"]="groupAccEmpAuxiliaryAccountService";
	   	...
	   	selPara["obj_code"]=liger.get("emp_code").getValue();
		...
		selPara["source_id"]="0";
	   	...
	   	officeTablePrint(selPara);
			...
			ajaxJsonObjectByUrl("queryEmpSubjGeneralLedger.do?isCheck=false", selPara, function(responseData) {
	...
	//打印维护
	 function myPrintSet(){
		 officeFormTemplate({template_code:"01013"});
	 }
    </script>
</head>
...
<input type="hidden" id="subj_query_code" name="subj_query_code"/>
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit"  border="0">
        ...
        <tr>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;" ><font size="2" color="red">*</font>职工：</td>
            <td align="left" class="l-table-edit-td" style="padding-left:20px;"><input id="emp_code" name="emp_code" /></td>
<!--             <td> -->
<!--             	<input class="l-button l-button-test" style="float: right;" type="button" value="选择" onclick="empSelector();"/> -->
<!--             </td> -->
        ...
        <tr>
            ...
            <td align="right" class="l-table-edit-td" colspan="2" ><input name="is_state" type="checkbox" id="is_state"  checked="checked"/></td>
