...
<jsp:include page="${path}/inc_jquery_1.9.0.jsp"/>
    ...
    function  query(){
		...
		if(dept_code.split(",").length>1){
        	 ...
        	 if(dept_code==""){
     	 		...
     	 		grid.options.parms.push({name:'obj_code',value :liger.get("dept_code").getText().split(" ")[0]}); 
    	...
    	if($("#dept_code").val() == ""){
       	 ...
       	 grid.options.parms.push({name:'obj_select_flag',value:"1"});
   			...
   			grid.options.parms.push({name:'obj_code',value : $("#dept_code").val()}); 
   	 		grid.options.parms.push({name:'obj_select_flag',value:"2"});
      	 }
    	//加载查询条件
    ...
    function loadHead(){
        grid = $("#maingrid").ligerGrid({
            columns: [ 
                     { display: '部门编码', isSort:false, name:'obj_code', align: 'left'},
 					 { display: '部门名称', isSort:false, name: 'obj_name', align: 'left'
 				...
 				dataAction: 'server',dataType: 'server',usePager:false,url:'queryGroupAccDeptEndOs.do',
	...
	function print(){//打印
		...
		var heads={
		 		...
		 		var printPara={
					...
					method_name: "collectGroupAccDeptBalancePrint",
	...
	function printDate(){//打印
    	...
    	selPara["source_id"]="0";
   		...
   		ajaxJsonObjectByUrl("queryDeptEndOs.do?isCheck=false", selPara, function(responseData) {
    ...
    function loadDict(){
    	...
    	$("#dept_code").ligerTextBox({ width : 220 });
	...
	function deptSelector(){
    	$.ligerDialog.open({ url : '../../groupbookselector/groupAccBookDeptSelectorPage.do?isCheck=false&flag='+$("#flag").val()+'&rela_code='+liger.get("rela_code").getValue(),
    			data:{listBoxData : obj_box_data }, height: 470,width: 480, title:'账簿部门选择器',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
				buttons: [ 
 				           { text: '确定', onclick: 
 				        	   function (item, dialog) { 
								  ...
								  liger.get("dept_code").setValue(obj_param);
     				              
   				        	      liger.get("dept_code").setText(param);
	 				        	   /* $("#dept_code").val(param); */
	 				        	   dialog.close(); 
	...
	//科目选择器
	 ...
	 function myPrintSet(){
		 officeFormTemplate({template_code:"01012"});
	 }
</script>
</head>
...
<input type="hidden" id="subj_query_code" name="subj_query_code"/>
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit"  border="0">
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"  colspan="3"><input id="dept_code" name="dept_code" /></td>
