...
<jsp:include page="${path}/inc_jquery_1.9.0.jsp"/>
    ...
    function  query(){
        ...
        grid.options.parms.push({name:'user_id',value:liger.get("out_user_id").getValue()}); 
    	//加载查询条件
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '状态', name: 'is_check', align: 'left',
                    	 render:function(rowdata){
                    		 ...
                    		 if(rowdata.is_check=="1"){
                    			 return "购置中";
                    		 }else if(rowdata.is_check=="2"){
                    			 return "领用生效";
                    		 }else if(rowdata.is_check=="3"){
                    			 return "作废";
                    		 }else{
                    			 return "已核销";
                    		 }
                     ...
                     { display: '领用人', name: 'out_user_name', align: 'left'},
                     { display: '领用日期', name: 'out_date1', align: 'left'},
                   /*   { display: '起始号码', name: 'begin_num', align: 'left'},
                     { display: '终止号码', name: 'end_num', align: 'left'}, */
                     { display: '起始号码', name: 'begin_detail_num', align: 'left'},
                     { display: '终止号码', name: 'end_detail_num', align: 'left'},
                     { display: '数量', name: 'amount', align: 'right'},
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAccPaperStockDetail.do',
    ...
    function print(){
		...
		var printPara={
			...
			method_name: "queryAccPaperStockDetailPrint",
	...
	officeGridPrint(printPara); 
         	...
         	$.each(grid.options.parms,function(i,obj){
        		...
        		ajaxJsonObjectByUrl("queryAccPaperStockDetail.do?isCheck=false", selPara, function(responseData) {
    ...
    function loadDict(){
         ...
         autocomplete("#out_user_id","../../../sys/queryUserDict.do?isCheck=false","id","text",true,true);
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
         <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="out_user_id" type="text" id="out_user_id" ltype="text" validate="{required:true}" /></td>
            <td align="left"></td>
