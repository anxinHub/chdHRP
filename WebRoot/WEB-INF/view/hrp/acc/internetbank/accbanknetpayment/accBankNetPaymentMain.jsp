...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
	        	     { display: '报销单号', name: 'apply_code', align: 'left',width:130,
			 		 ...
			 		 { display: '申请日期', name: 'apply_date', width:90,align: 'left'},	
			 		 
			 		{ display: '申请事由', name: 'remark', align: 'left', width:80},
			 		 
                     { display: '科室', name: 'dept_name', width:180,align: 'left'},
                     
                     { display: '项目', name: 'proj_name', width:180,align: 'left'},
                     
                     { display: '报销人', name: 'emp_name', align: 'left', width:80},
                     
                     { display: '报销金额', name: 'pay_amount', align: 'right',width:80,
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAccPaymentApply.do',
                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad :true,
                     selectRowButtonOnly:true,
                     //checkBoxDisplay:isCheckDisplay,
                     pageSize:500,//heightDiff: -10,
                     toolbar: { items: [
    ...
    function isCheckDisplay(rowdata){
        //admin用户没有复选框
       	if(rowdata.result_state.indexOf('支付成功') == -1){
       		return true;
       	}else{
       	 return false;
       	}
	...
	function myPrint(){
    	...
    	var printPara={
        		...
        		method_name: "queryAccPaymentApplyPrint",
	 ...
	 //打印回调方法
    function lodopPrint(){
    	var head="<table class='head' width='100%'><tr><td>日期："+$("#apply_date_b").val()+"至"+$("#apply_date_e").val()+"</td>";
        ...
        if (data.length == 0){
        	...
        	$.ligerDialog.error('请选择行');
        	 ...
        	 $(data).each(function (){
				...
				ParamVo.push(this.apply_code+"@"+this.pay_amount);
				
        	 })
        	 
        	 var paramVo = ParamVo.toString();	
        	 $.ligerDialog.open({url: 'collectAccBankNetPaymentPage.do?isCheck=false&paramVo='+ParamVo,height: 450,width: 870,title:'补录网上银行信息',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
 				/* buttons: [ 
	...
	function openUpdate(obj){
    	...
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/acc/payable/reimbursemt/pay/budgPaymentApplyUpdatePage.do?isCheck=false&'+parm,
