...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '付款单号', name: 'pay_no', align: 'left',width:120,
					 			render:function(rowdata,index,value){
					 				if(rowdata.pay_no != null){
				 						...
				 						rowdata.pay_no+"|"+
				 						rowdata.state+"|"+
				 						rowdata.vouch_id+"')>"+rowdata.pay_no+"</a>";
					 				}else{
					 					return "合计";
					 				}
					 			}
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssPayMain.do?isCheck=false',
	...
	function myPrint(){
		...
		var printPara={
       		...
       		class_name: "com.chd.hrp.acc.service.InternetBank.AccBankNetAssService",
    		method_name: "queryMatPayMainPrint",
    		bean_name: "accBankNetAssService"
        ...
        if (data.length == 0){
        	$.ligerDialog.error('请选择行');
        	 ...
        	 $(data).each(function (){
				...
				if(this.state != 2){str_state += this.pay_no +",";}
				
				ParamVo.push(this.pay_no +"@"+ this.sup_id +"@"+ this.pay_money+"@"+this.pay_bill_no+"@"+this.vouch_id+"@"+this.vouch_state+"@"+ this.erpsqn);
        	 })
        	...
        	if(str_sup != ''){$.ligerDialog.error('必须选择相同供应商');return false;}
        	 ...
        	 $.ligerDialog.open({url: 'collectAccBankNetAssPayPage.do?isCheck=false&payFlag=0&paramVo='+ParamVo,height: 500,width: 870,title:'补录网上银行信息',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
 				/* buttons: [ 
 				           { text: '确定支付', onclick: function (item, dialog) {dialog.frame.saveAccBankNetAss(); },cls:'l-dialog-btn-highlight' }, 
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"copy_code="+vo[2]   +"&"+ 
			"pay_no="+vo[3]; 
			$.ligerDialog.open({ url : 'assPayMainUpdatePage.do?isCheck=false&'+parm,data:{}, top: 0,
    ...
    function loadDict(){//字典下拉框
		...
		//供应商下拉框
		autocomplete("#sup_id", "../../../ass/queryHosSupDictNo.do?isCheck=false", "id", "text", true, true,'',false,'',275);
