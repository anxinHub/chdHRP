...
<script type="text/javascript">
    ...
    function  updateAccBankNetWage(){//根据表字段进行添加查询条件
		...
		if (data.length == 0){
        	 ...
        	 $.ligerDialog.confirm('确定更新指令状态?', function (yes){if(yes){
     			...
     			ajaxJsonObjectByUrl("updateAccCBCPayWage.do?isCheck=false&ParamVo="+ParamVo.toString(),{}, function(responseData) {
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
			columns : [
					{display : '付款日期',name : 'opt_date',width: 140,align : 'left',
						render: function (rowdata)
	                    {
							return "<a href=javascript:openUpdate('"+rowdata.request_sn+"|"+rowdata.group_id+"|"+rowdata.hos_id+"|"+rowdata.copy_code+"|"+rowdata.acc_year+"|"+rowdata.acc_month+"')>"+rowdata.opt_date+"</a>";
	                    }
					},
					
					{display : '请求序列码',name : 'request_sn',width: 140,align : 'left'},
					
					{display : '总笔数',name : 'count',width: 140,align : 'left'},
					
					{display : '总金额',name : 'amount',width: 80,align : 'right',
						render:function(rowdata){ return rowdata.amount>0?formatNumber(rowdata.amount,2,1):formatNumber('0',2,1)}	
					},
					
					{display : '交易返回码',name : 'return_code',width: 160,align : 'left'},

					{display : '成功总笔数',name : 'success_num',width: 160,align : 'left',
						render: function (rowdata)
		                    {
								if(rowdata.success_num==null){
									if(rowdata.success_num==0){
										return rowdata.success_num;
									}else{
										return'';
									}
								}else{
									return "<a href=javascript:queryCountPay('"+rowdata.request_sn+"|"+rowdata.acc_year+"|"+rowdata.acc_month+"')>"+rowdata.success_num+"</a>";
								}
							}
					},
					{display : '失败总笔数',name : 'error_num',width: 160,align : 'left',
						render: function (rowdata)
	                    {
							if(rowdata.success_num==null){
								if(rowdata.success_num==0){
								...
								return "<a href=javascript:queryCountPay('"+rowdata.request_sn+"|"+rowdata.acc_year+"|"+rowdata.acc_month+"')>"+rowdata.error_num+"</a>";
							}
						}
					},
					{display : '交易返回说明',name : 'return_msg',width: 160,align : 'left'},
					],
					dataAction : 'server',dataType : 'server',usePager : true,url : 'queryAccCBCWageRd.do',pageSize:50,delayLoad:true,
