...
<script type="text/javascript">
    ...
    function  query(){//根据表字段进行添加查询条件
		...
		grid.options.parms=[];grid.options.newPage=1;
      	...
      	grid.options.parms.push({name:'result',value:liger.get("result").getValue()}); 
      	
		grid.options.parms.push({name:'fseqno',value:$("#fseqno").val()});
      	
      	grid.options.parms.push({name:'group_id',value:$("#group_id").val()});
      	
      	grid.options.parms.push({name:'copy_code',value:$("#copy_code").val()});
		...
		if (data.length == 0){
			...
			return false;
        	 ...
        	 $(data).each(function (){
 				...
 				ParamVo.push(this.iseqno);
 				
         	 })

        }
		
		var para = "&payFlag=2"+"&group_id="+data[0].group_id+"&hos_id="+data[0].hos_id+"&copy_code="+data[0].copy_code+"&fseqno="+'${fseqno}'+"&acc_year="+year_Month.substring(0,4)+"&acc_month="+year_Month.substring(4,6)+"&paramVo="+ParamVo.toString();
	...
	function myPrint(){
    	...
    	var printPara={
        		...
        		method_name: "queryAccBankNetWageRdPrint",
    ...
    function loadDict(){
        ...
        autocomplete("#wage_code","../../queryAccWage.do?isCheck=false","id","text",true,true,'',false,false,'180');
...
<body style="padding: 0px; overflow: hidden;">
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit">
			   	 	<tr>
			            <td align="right" class="l-table-edit-td"  style="padding-left:5px;">工资套：</td>
			            <td align="left" class="l-table-edit-td" ><input name="wage_code" type="text" id="wage_code" ltype="text" validate="{required:true,maxlength:18}" /></td>
			            <td align="left"></td>
			            ...
			            <td align="right" class="l-table-edit-td"  style="padding-left:5px;">成功状态：</td>
			            <td align="left" class="l-table-edit-td" ><input name="result" type="text" id="result" ltype="text" validate="{required:true,maxlength:18}" /></td>
			            <td align="left"><!--input class="l-button" type="button" id="closeBtn" value="关闭" onclick="btn_close();" /--></td>
			        </tr>
	   			  </table>
	   		<div id="maingrid"></div>
