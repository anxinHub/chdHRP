   	 ...
   	 $tool=$("#centertoolbar").ligerToolBar({ items: [   	      
	       ...
	       */
	       { line:true },
   	       ...
   	       { line:true },
   	       { text: '查询设置', id:'cashier', icon:'cashier', click: setQuery},
	       { line:true },
	    	...
	    	// dsRow=dsRow.replace(",","");
	    	 ...
	    	 for(var i=0;i<a.length-1;i++){ 
	    		 	...
	    		 	for (var col in rowData) {
	    		 		...
	    		 		if(typeof(colData)!='undefined'&&colData!=null&&(Object.prototype.toString.call(colData).toLowerCase() == "[object object]")==false){	    				    	colData=colData.substring(colData.indexOf("[")+1,colData.indexOf("."));
	    	 ...
	    	 dsName+="0";
	     	$.ligerDialog.open({url : 'reportQueryPage.do?isCheck=false&mod_code='+$("#mod_code").val()+"&ds_code="+dsName+"&report_code="+report_code,
	     		data:{}, height: $(window).height()-10,width: $(window).width()-10, title:'查询设置',modal:true,showToggle:false,initShowMax:false,showMax:true,showMin: false,isResize:false
	     		});
   		}
	    function getDs(){   	
	    	var dsNames=new Set();
	    	...
	    	// dsRow=dsRow.replace(",","");
	    	 ...
	    	 for(var i=0;i<a.length-1;i++){ 
	    		 	...
	    		 	for (var col in rowData) {
	    		 		...
	    		 		var colData=rowData[col].value;
	    				    if(typeof(colData)!='undefined'&&colData!=null&&(Object.prototype.toString.call(colData).toLowerCase() == "[object object]")==false){
	     	...
	     	$.ligerDialog.open({url : 'dsParaSetPage.do?isCheck=false&mod_code='+$("#mod_code").val()+"&dsNames="+dsName,
	     		data:{dsName:dsNames}, height: $(window).height()-10,width: $(window).width()-10, title:'参数设置',modal:true,showToggle:false,initShowMax:false,showMax:true,showMin: false,isResize:false
	     		});
	    	// return dsNames;
	    }
