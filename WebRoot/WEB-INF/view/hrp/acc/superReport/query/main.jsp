...
<script src="<%=path%>/lib/hrp/acc/report.js" type="text/javascript"></script>
...
var setting;
    ...
    function budgClick(){
    	...
    	var para={};
    	ajaxJsonObjectByUrl("../../../budg/queryBudgYear.do?isCheck=false",para,function(responseData){
			...
			$("#acc_year").val(responseData[0].text);
			$("#maxDate").val(responseData[0].text);
			$("#minDate").val(responseData[responseData.length-1].text);
			$("#period_td").css("display","none");
			$("#acc_report_tab").css("display","block");
    	},false);
    	
    	getReportQuery(node.id);
    }
	...
	function myClick(){
    	...
    	if(node==null || node.pId==null||node.pId=='~!@#1'||node.pId=='~!@#2'){
    		return false;
    	}
	...
	//加载报表自身查询条件
	function getReportQuery(rep_code){
		var para={
    			rep_code: rep_code
    	};
    	ajaxJsonObjectByUrl("../manage/getReportQuery.do?isCheck=false",para,function(responseData){
    		setQuery(responseData.Rows);
		});	
	}
    ...
    function myQuery(){
    	...
    	if(node.report_type==2 || node.report_type==3 || node.report_type==4 || node.report_type==5){
            ...
            $.each(t, function() {
            	auto_param.push([this.name]+"="+$('input[name="'+this.name+'"]').val());
            });    		
    	}
    	
    	var para={
    			rtype:node.report_type,
    			operation:"instance",
      		   	report_code:node.id,
      			mod_code:$("#mod_code").val(),
      			acc_year:accYear,
      			acc_month:accMonth,
      			auto_param:auto_param.toString()
      			};
    	queryReport(para);
    }
			...
			<div id="mainHead" style="height:0px;">
		        <div class="info">
					 ...
					 <form></form>
					  ...
					  <input style="bottom:5px;right:400px;position:absolute;" type="button" accessKey="Q"  id="rquery" value=" 查询（Q）" onclick="myQuery();"/>
					  <input style="bottom:5px;right:200px;position:absolute;" type="button" id="rimp" accessKey="I"  value=" 导入（I）" onclick="myImport();"/>
					  <!-- input style="bottom:5px;right:200px;position:absolute" type="button" accessKey="E"  value=" 导出Excel（E）" onclick="exportExcel();"/-->
					  <input style="bottom:5px;right:300px;position:absolute" type="button" accessKey="P"  value=" 打印（P）" onclick="myPrint();"/>
					  <input style="bottom:5px;right:100px;position:absolute" type="button" id="rcreate" accessKey="N"  value=" 生成（N）" onclick="myCreate();"/>
					  <input style="bottom:5px;right:1px;position:absolute" type="button" id="rsave" accessKey="S"  value=" 保存（S）" onclick="mySave();"/>
					  &nbsp;<br/>&nbsp;<br/>
			    </div>
		    </div>
