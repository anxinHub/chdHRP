...
<script type="text/javascript">
	...
	$(function() {
		...
		ajaxJsonObjectByUrl("../queryVouchState.do?isCheck=false&state="+3,{}, function(data) {
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			{ line:true },
			{ text: '全部审核', id:'auditAll', click: auditAll,icon:'audit' },
			{ line:true },
			{ text: '全部取消', id:'unAuditAll', click: unAuditAll,icon:'unaudit' },
			{ line:true },
			{ text: '批注', id:'label', click: itemclick, icon:'blabel' },
            { line:true },
            { text: '取消批注', id:'unlable', click: itemclick,icon:'bcancle' },
			{ line:true },
	...
	function itemclick(item) {
		if (item.id) {
			...
			case "modify":
				return;
			case "search"://查询
				$.ligerDialog.open({url: 'accVouchAuditSearchPage.do?isCheck=false', height: 420,width: 650, title:'高级查询',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.searchAccVouch(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
				return;
			case "audit"://审核
				...
				if (data.length == 0) {
					...
					if(ParamVo.length==0){
						...
						$.ligerDialog.error("请选择有效凭证进行审核操作！");
					...
					if(result>0){
						...
						$.ligerDialog.error('凭证：['+result_str+']不能审核自己的凭证！');
					...
					$.ligerDialog.confirm('确定审核?', function(yes) {
						if (yes) {
							ajaxJsonObjectByUrl("auditAccVouch.do", {
								ParamVo : ParamVo.toString(),state:parent_node_id
			...
			case "unaudit"://取消审核
				...
				if (data.length == 0) {
					...
					$(data).each(function() {
						...
						if(this.audit_user != '${sessionScope.user_id }'){
					...
					if(ParamVo.length==0){
						...
						$.ligerDialog.error("请选择有效凭证进行取消审核操作！");
					...
					$.ligerDialog.confirm('确定取消审核?', function(yes) {
						if (yes) {
							ajaxJsonObjectByUrl("unauditAccVouch.do", {
								ParamVo : ParamVo.toString(),state:parent_node_id
							}, function(responseData) {
								if (responseData.state == "true") {
									query();
			...
			case "label":
				...
				if (data.length == 0) {
					...
					$(data).each(function() {
						...
						ParamVo=ParamVo+this.vouch_id+"@";
						
					});
					$.ligerDialog.open({url: 'accVouchAuditLabelPage.do?paramVo='+ParamVo, height: 323,width: 650, title:'标注',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.accVouchLabel(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
				}
				return;
			case "unlable":
				...
				if (data.length == 0) {
					...
					$.ligerDialog.confirm('确定取消标注?', function(yes) {
						if (yes) {
							ajaxJsonObjectByUrl("accVouchUnlabel.do", {
	...
	function loadDict() {
		...
		autoCompleteByData("#state", [{"id": "1", "text": "未审核"}, {"id": "2", "text": "已审核"}], "id", "text", false, true, "", false,1);
	...
	function printDate(){
    	...
    	var heads={
     		...
     		var printPara={
   			...
   			class_name: "com.chd.hrp.acc.service.AccVouchAuditService",
  			method_name: "queryAccVouchAuditPrint",
  			bean_name: "accVouchAuditService",
	...
	function auditAll(){
		...
		$.ligerDialog.open({
			...
			height : 300,
			width : 400,
			url : 'auditAllPage.do?isCheck=false',
			modal : true,
			showToggle : false,
			showMax : false,
			showMin : false,
			...
			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.save(1); },cls:'l-dialog-btn-highlight' }, 
	...
	function unAuditAll(){
		$.ligerDialog.open({
			...
			isResize : false,
			data:{state_name: state_name},
			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.save(0); },cls:'l-dialog-btn-highlight' }, 
			           { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ],
			parentframename : window.name
		});
	}
