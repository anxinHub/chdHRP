...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '年初余额', name: 'bal_os', align: 'right',formatter:'###,##0.00'
					 }
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryGroupAccMatchInit.do',
    ...
    function print(){
    	...
    	var heads={
     		...
     		var printPara={
     			...
     			class_name: "com.chd.hrp.acc.service.books.memorandumbook.GroupAccMatchInitService",
	  			method_name: "queryGroupAccMatchInitPrint",
	  			bean_name: "groupAccMatchInitService",
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'groupAccMatchInitAddPage.do?isCheck=false', height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAccPayType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deletBatchGroupAccMatchInit.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'groupAccMatchInitUpdatePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAccPayType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
