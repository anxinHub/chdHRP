...
<script type="text/javascript">
	...
	function query() {
		...
		if(busi_type_code.split("@")[0] == "140301"){
			if($("#year_month").val() == ""){
	...
	function f_setColumns() {
		...
		$.post(
						"queryIssAutoVouchHead.do?isCheck=false",
						{
							"mod_code" : mod_code,
	...
	function loadDict() {
		...
		autocompleteObj({
			id : "#mod_code",
			urlStr : "../../../sys/querySysBusiMod.do?isCheck=false&mod_code_in='14'",
			valueField : 'id',
			textField : 'text',
			selectBoxWidth : 160,
			autocomplete : true,
			...
			selectEvent : function(value) { //  选择事件 等同于 onSelected
				autocompleteObj({
					...
					selectEvent : function(busiValue) {
						...
						}else if(busiTypeCode=="140101" || busiTypeCode=="140201"){
							$("td[name='billTd']").show();
							$("#biandong1").show();
							$("#biandong2").show();
							$("#biandong3").show();
		...
		autoCompleteByData("#bill_state", dataType.Rows, "id", "text", false, true,null,false,null,160,
				function(value){
					...
					if(bstype=="140101" || bstype=="140102"){
	...
	function createVouch(flag) {
		...
		if(initType==1){
		    ...
		    if (data.length>0){
		        $(data).each(function (){
		        	if(liger.get("acc_busi_type").getValue().split("@")[0] == "140301"){
		...
		if(flag==1){
			...
			$.ligerDialog.confirm('确定生成凭证?', function (yes){
		    	if(yes){
		    		...
		    		ajaxJsonObjectBylayer("queryIssVouchJsonByBusi.do",para,function(responseData){	
	...
	function printDate() {
		...
		var printPara = {
			...
			class_name : "com.chd.hrp.acc.service.autovouch.AccIssAutoVouchService",
			method_name : "queryIssAutoVouchPrint",
			bean_name : "accIssAutoVouchService",
