...
var edit = true;
	...
	function loadHead(){
    	deptgrid = $("#dept_list").ligerGrid({
			...
			dataAction: 'server',usePager:true,url:'queryAmortizeDeptList.do?isCheck=false&apply_code=${apply_code}',
			width: '100%', height: '100%', checkbox: true,rownumbers:true,
			selectRowButtonOnly:true,enabledEdit: edit,checkbox:true,
			toolbar: { 
				items: [
	...
	function loadHead2(){
		amortizedgrid = $("#amortized_histroy").ligerGrid({
			...
			dataAction: 'server',usePager:true,url:'queryAmortizeHistoryList.do?isCheck=false&apply_code=${apply_code}',
	...
	function loadHead3(){
		sourcegrid = $("#source").ligerGrid({
			columns: [
				...
				{display: '金额', name: 'money', align: 'right',editor: { type: 'float' },
					totalSummary:{
                        type: 'sum'
                    }
				}
			],
			dataAction: 'server',usePager:true,url:'queryAmortizeSourceList.do?isCheck=false&apply_code=${apply_code}',
			width: '100%', height: '100%', rownumbers:true,enabledEdit: edit,checkbox:true,
			toolbar: { 
				items: [
	...
	function loadDict() {
		...
		liger.get("apply_code").setValue("${apply_code}");
		$("#original_apply_code").ligerTextBox({width : 180});
		liger.get("original_apply_code").setValue("${original_apply_code}");
		$("#apply_name").ligerTextBox({width : 180});
		liger.get("apply_name").setValue("${apply_name}");
		$("#pact_code").ligerTextBox({width : 180});
		liger.get("pact_code").setValue("${pact_code}");
		$("#amortize_year").ligerTextBox({width : 180});
		liger.get("amortize_year").setValue("${amortize_year}");
		$("#amortized").ligerTextBox({width : 180});
		liger.get("amortized").setValue("${amortized}");
		$("#origin_value").ligerTextBox({width : 180});
		liger.get("origin_value").setValue("${origin_value}");
		$("#amortized_value").ligerTextBox({width : 180});
		liger.get("amortized_value").setValue("${amortized_value}");
		$("#net_value").ligerTextBox({width : 180});
		liger.get("net_value").setValue("${net_value}");
		...
		$.ajax({
		 ...
		 sup_id = autocomplete("#sup_id","../../../sys/querySupDictDict.do?isCheck=false", "id", "text",true, true, null, null, '${sup_id}', "180");
		type_code = autocomplete("#type_code", "../amortizetype/amortizeTypeSelect.do?isCheck=false", "id", "text", true, true, null, null, '${type_code}', "180");

		
		if(${state} != 0){
			$("#amortize_year").ligerTextBox({disabled:true});
			$("#amortized").ligerTextBox({disabled:true});
			$("#origin_value").ligerTextBox({disabled:true});
			$("#amortized_value").ligerTextBox({disabled:true});
			$("#net_value").ligerTextBox({disabled:true});
			$("#original_apply_code").ligerTextBox({disabled:true});
			$("#apply_name").ligerTextBox({disabled:true});
			$("#pact_code").ligerTextBox({disabled:true});
			$("#note").ligerTextBox({disabled:true});
			$("#sup_id").ligerComboBox({disabled:true,cancelable : false});
