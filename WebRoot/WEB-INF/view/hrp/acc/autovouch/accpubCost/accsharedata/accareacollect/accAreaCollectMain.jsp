...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [  
				...
				{display : '科室面积(㎡)',name:'dept_area',align:'right'},
				{display : '总面积(㎡)',name:'total_num',align:'right'},
			...
			url : 'queryAccAreaCollect.do',width : '100%',height : '100%',
		...
		$.ligerDialog.confirm('是否审核'+year_month.getValue()+'的全部数据',
			function (yes) {
				if(yes){
					...
					ajaxJsonObjectByUrl("updateAccAreaState.do", parm ,function (responseData){
	...
	function lead(){
		parent.$.ligerDialog.open({ 
       		url : 'hrp/acc/autovouch/accpubCost/accarea/accAreaCollectImportPage.do?'
       				+'&year_month=' + (year_month.getValue()).replace('.','').substring(0,6),
			data:{
				columns : grid.columns, 
				grid : grid
			}, height: 300,width: 450,title:'科室面积采集导入',modal:true,showToggle:false,showMax:true,
	...
	//继承上月
	function inherit(){
		$.ligerDialog.confirm('是否将'+year_month.getValue()+'的数据设置为上月数据',
			function (yes) {
				if(yes){
					...
					ajaxJsonObjectByUrl("updateAccAreaDataFromLastMonth.do", parm ,function (responseData){
		         		if(responseData.state=="true"){
		         			query();
		         		}
		         	});
	...
	function openUpdate(obj){
		...
		$.ligerDialog.open({ url : 'updateAccAreaCollectPage.do?' + parm, height: 300,width: 400, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
	...
	//添加
	function add(){
		$.ligerDialog.open({url: 'addAccAreaCollectPage.do', data:{"year_month":year_month.getValue()},height: 300,width: 400, title:'添加',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
	...
	function del(){
		 ...
		 if (data.length == 0){
             ...
             $.ligerDialog.confirm('确定删除?', function (yes){
             	if(yes){
                 	ajaxJsonObjectByUrl("deleteAreaCollect.do",{ParamVo : ParamVo.toString()},function (responseData){
                 		if(responseData.state=="true"){
                 			query();
                 		}
                 	});
	...
	function print(){
    	...
    	var heads={
   		...
   		var printPara={
   			...
   			class_name: "com.chd.hrp.acc.service.autovouch.accpubCost.AccAreaCollectService",
			method_name: "queryAccAreaPrint",
			bean_name: "accAreaCollectService",
