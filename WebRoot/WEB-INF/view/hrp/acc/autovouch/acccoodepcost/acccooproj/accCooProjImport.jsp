...
var grid=dialog.get("data").grid;
    ...
    function initSpreadData(){
    	...
    	sheet.suspendPaint();
    	//设置列
    	sheet.setColumnWidth(0, "250"); sheet.setText(0, 0, "项目编码");
    	sheet.setColumnWidth(1, "250"); sheet.setText(0, 1, "项目名称");
    	sheet.setColumnWidth(2, "250"); sheet.setText(0, 2, "科室编码");
    	sheet.setColumnWidth(3, "250"); sheet.setText(0, 3, "科室名称");
    	sheet.setColumnWidth(4, "250"); sheet.setText(0, 4, "分摊比例");
    	sheet.setColumnWidth(5, "250"); sheet.setText(0, 5, "备注");
    	
    	sheet.getRange(0, 0, 1, 6).hAlign(spreadNS.HorizontalAlign.center);
    ...
    function mySave(){
    	...
    	if(sheet.getText(0,0) != "项目编码"){
    		$.ligerDialog.error("导入模板不正确，请使用系统模板导入！");
			return false;
    	}
    	if(sheet.getText(0,1) != "项目名称"){
    		$.ligerDialog.error("导入模板不正确，请使用系统模板导入！");
			return false;
    	}
    	if(sheet.getText(0,2) != "科室编码"){
    		$.ligerDialog.error("导入模板不正确，请使用系统模板导入！");
			return false;
    	}
    	if(sheet.getText(0,3) != "科室名称"){
    		$.ligerDialog.error("导入模板不正确，请使用系统模板导入！");
			return false;
    	}
    	if(sheet.getText(0,4) != "分摊比例"){
    		$.ligerDialog.error("导入模板不正确，请使用系统模板导入！");
			return false;
    	}
    	if(sheet.getText(0,5) != "备注"){
    		$.ligerDialog.error("导入模板不正确，请使用系统模板导入！");
			return false;
    	}
    	...
    	ajaxJsonObjectBylayer("importAccCooProj.do?isCheck=false", para, function(responseData){	
