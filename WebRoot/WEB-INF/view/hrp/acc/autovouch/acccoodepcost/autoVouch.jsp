...
<script type="text/javascript">
    ...
    function  query(){
    	...
    	if($("#busi_date_b").val()=="" || $("#busi_date_e").val()==""){
 			$.ligerDialog.error("请选择业务日期！");
 			return;
 		}
				...
				for(var i = 0 ; i < name_dis.length; i++){
					...
					if(value_name == "AMOUNT_MONEY"){
						...
						columns.push({display:display_name,name:value_name,width: 160,align : 'left',frozen : true,
							render:function(rowdata){
								if(rowdata.BUSINESS_NO!=null && rowdata.BUSINESS_NO!="合计"){
    ...
    function loadDict(){
		...
		autocompleteObj({
	    	 id: '#proj_id',
	    	 urlStr: '../acccoodeptcost/acccoopcost/queryAccCoopProjDict.do?isCheck=false',
	    	 valueField: 'id',
	    	 textField: 'text',
	    	 autocomplete: true,
	    	 //initWidth: '400',
		...
		autocomplete("#template_code","../../autovouch/his/charge/queryAutoBusiTemplate.do?isCheck=false&busi_type_code=0104","id","text",true,true,"",true);
    ...
    function createVouch(flag){
		...
		var para={
				...
				mod_code: '01',
				busi_type_code: '0104',				
				busi_log_table: 'ACC_BUSI_LOG_0104'
		};
		    	...
		    	if(yes){
		    		...
		    		ajaxJsonObjectBylayer("queryPubVouchJsonByBusi.do",para,function(responseData){	
		    			...
		    			var para={
		    				...
		    				data:{auto_id:responseData.auto_id,busi_log_table:'ACC_BUSI_LOG_0104'}
	...
	function openUpdate(business_no){
		...
		var para={
			url:'hrp/acc/autovouch/acccoodeptcost/acccoopcost/accCoopCostUpdatePage.do?isCheck=false&ser_num='+business_no,
			title:"合作科室费用分摊",
			width:100,
			height:100,
			isDrag:true,
			data:{
				call: 'vouch',ser_num: business_no
			}
		};
		parent.openDialog(para);
	}
	...
	function printDate(){
  	...
  	var heads={
 		...
 		var printPara={
 			...
 			class_name: "com.chd.hrp.acc.service.autovouch.acccoodeptcost.AccCoopAutoVouchService",
			method_name: "queryCoopAutoVouchPrint",
			bean_name: "accCoopAutoVouchService",
			busi_type_code : '0104',
			mod_code : '01',
