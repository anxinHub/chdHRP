...
<script type="text/javascript">
    ...
    $(function ()
    {
    	...
    	$("#year_month1").ligerTextBox({width:100});
    	$("#year_month2").ligerTextBox({width:100});
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                                    ...
                                    { display: '本期收入', name: 'total_sr', align: 'right',formatter:'###,##0.00',
               							render : function(rowdata, rowindex,value) {
               								 return "<a href=javascript:openUpdate('" + 
               										 rowdata.proj_id + "|"+ 
               										 rowdata.proj_no + "|" + 
               										 "financialeducation_total_sr"+"')>" + formatNumber(rowdata.bal_sr+rowdata.match_sr, 2, 1) + "</a>"; 
               							}
                                    },
                                    { display: '本期支出', name: 'total_ot', align: 'right',formatter:'###,##0.00',
               							render : function(rowdata, rowindex,value) {
		                  				 	return "<a href=javascript:openUpdate('" + 
		                  				 			rowdata.proj_id + "|"+
		                  				 			rowdata.proj_no + "|" + 
		                  				 			"financialeducation_total_ot"+"')>" + formatNumber(rowdata.bal_ot+rowdata.match_ot, 2, 1) + "</a>";
                                  ...
                                  { display: '本期收入', name: 'bal_sr', align: 'right',
                               		  render : function(rowdata, rowindex,value) {         									
				                           return "<a href=javascript:openUpdate('" + 
				                        		   rowdata.proj_id + "|"+ 
				                        		   rowdata.proj_no + "|" + 
				                        		   "financialeducation_bal_sr"+"')>" + formatNumber(rowdata.bal_sr, 2, 1) + "</a>"; 
                                   	  },
                                	  formatter:'###,##0.00'
                           		  },
                                  { display: '本期支出', name: 'bal_ot', align: 'right',
                           			  render : function(rowdata, rowindex,value) {
				                             return "<a href=javascript:openUpdate('" + 
				                            		 rowdata.proj_id + "|"+ 
				                            		 rowdata.proj_no + "|" + 
				                            		 "financialeducation_bal_ot"+"')>" + formatNumber(rowdata.bal_ot, 2, 1) + "</a>";
                                   	  }, 
                           			  formatter:'###,##0.00'
                       			  },
                                  ...
                                  { display: '本期收入', name: 'match_sr', align: 'right',
                           			  render : function(rowdata, rowindex, value) {
		                           		 return "<a href=javascript:openUpdate('"  + 
		                           				 rowdata.proj_id + "|"+ 
		                           				 rowdata.proj_no + "|" + 
		                           				 "financialeducation_match_sr"+"')>" + formatNumber(rowdata.match_sr, 2, 1) + "</a>";
                                  ...
                                  { display: '本期支出', name: 'match_ot', align: 'right',
                           			  render : function(rowdata, rowindex, value) {
			                           	 return "<a href=javascript:openUpdate('" + 
			                           			 rowdata.proj_id + "|"+ 
			                           			 rowdata.proj_no + "|" + 
			                           			 "financialeducation_match_ot"+"')>" + formatNumber(rowdata.match_ot, 2, 1) + "</a>"; 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryGroupAccFinancialEducation.do',
    ...
    function printDate(){
    	...
    	$.each(grid.options.parms,function(i,obj){
    		...
    		ajaxJsonObjectByUrl("queryGroupAccFinancialEducation.do?isCheck=false", selPara, function(responseData) {
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
			url: 'hrp/acc/accfinancialeducation/group/groupAccFinancialEducationDataMiningPage.do?isCheck=false&'+parm, data: {parm:parm},
