...
<jsp:include page="${path}/inc_jquery_1.9.0.jsp"/>
    		...
    		var columns = "{ display: '项目编码', name: 'check_code', align: 'left'" +"},"
    			...
    			grid = $("#maingrid").ligerGrid({
    		           columns: eval("["+columns+"]"),
    		                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryPayAccount.do?isCheck=true',
    ...
    function f_setColumns(){ 
	      ...
	      $.each(dataAll,function(index,value){
				$.each(value,function(i,v){
					if(v.note !=undefined){
						var a=i+1;
	  					columns = columns + ",{display: '"+v.note+"', name: 'col_"+a+"', align: 'right'},"
	  					+"{display: '百分比', name: 'col_bal_"+a+"', align: 'right'"+"}"
	  					i=i+1;
					}
  	...
  	function search(){
  		if(rangeIsExits == 0){
			...
			if($("#showItem").attr("checked") == false){
				if($("form").valid()){
					loadHead();
	    			query();
				}
    ...
    function loadDict(){
    	 ...
    	 $("#analysisDate").ligerTextBox({width:180});
    	 $("#end_date").ligerTextBox({width:180});
    }   
...
<body style="padding: 0px; overflow: hidden;">
	...
	<form name="form1" method="post"  id="form1" >
	    <table cellpadding="0" cellspacing="0" class="l-table-edit"  border="0">
	        <tr>
	            <td align="right" class="l-table-edit-td"  style="padding-left:10px;">应付科目<span style="color:red">*</span>：</td>
