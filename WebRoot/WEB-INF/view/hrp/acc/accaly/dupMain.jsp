	...
	<script>
		...
		function loadGrid() {
			grid = $("#maingrid").ligerGrid({
				columns : [  
					{display : '杜邦指标编码',name : 'dup_code',align:'left',
						  render : function(rowdata, rowindex,value) {
							  return "<a href=javascript:openUpdate('"+rowdata.dup_code  +"')>"+rowdata.dup_code+"</a>";
		  				  }	
				    	},
					{display : '杜邦指标名称',name : 'dup_name',align:'left'},
				...
				url : 'queryAccAlyDup.do',width : '100%',height : '100%',
		...
		function openUpdate(dup_code) {
			$.ligerDialog.open({ url : 'dupUpdatePage.do?dup_code='+dup_code, height: 480,width: 660, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
		...
		function remove() {
			 ...
			 if (data.length == 0){
	             ...
	             $.ligerDialog.confirm('确定删除?', function (yes){
	             	if(yes){
	                 	ajaxJsonObjectByUrl("deleteAccAlyDup.do",{ParamVo : ParamVo.toString()},function (responseData){
		...
		//添加
		function add_open(){
			$.ligerDialog.open({url: 'dupAddPage.do', height: 480,width: 660, title:'杜邦分析指标添加',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
		...
		var printDate = function () {
        	...
        	var printPara={
                ...
                class_name: "com.chd.hrp.acc.serviceImpl.accAly.AccAlyDupServiceImpl",
                method_name: "queryDupPrintDate",
                bean_name: "accAlyDupService",
...
<body style="padding: 0px; overflow: hidden;">
	<div id="pageloading" class="l-loading" style="display: none"></div>
		<table cellpadding="0" cellspacing="0" class="l-table-edit">
			<tr>
				<td align="right" class="l-table-edit-td" style="padding-left: 20px;">杜邦指标：</td>
