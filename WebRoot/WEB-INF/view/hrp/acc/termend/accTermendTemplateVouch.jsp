		...
		grid = $("#subjgrid").ligerGrid({
			columns : [ {
				...
				columns : [ {
					...
					width : 100,
					render : function(rowdata){
						 return formatNumber(rowdata.ys_debit,2,1);
					}
				}, {
					display : '贷方金额',
					name : 'ys_credit',
					...
					width : 100,
					render : function(rowdata){
						 return formatNumber(rowdata.ys_credit,2,1);
					}
				} ]
			}, {
				display : '制单人',
			...
			dataType : 'server',
			usePager : false,
			url : 'queryAccTermendTemplateVouch.do?isCheck=false',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : false,
	...
	function itemclick(item) {
		if (item.id) {
			...
			case "export":
				...
				ajaxJsonObjectByUrl("queryAccTermendTemplateVouch.do?isCheck=false", selPara, function (resData) {
