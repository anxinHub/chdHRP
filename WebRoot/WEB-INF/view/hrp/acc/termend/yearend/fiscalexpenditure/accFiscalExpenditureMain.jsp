...
<script type="text/javascript">
	...
	$(function() {
		//布局
		$("#layout1").ligerLayout({ leftWidth: 265, centerBottomHeight: $(window).height()-180});
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			usePager : false, url : 'queryAccFiscalExpenditure.do', checkbox : true, isSingleCheck: true,  
			...
			case "delete":
				...
				if (data.length == 0) {
					...
					$.ligerDialog.confirm('确定删除?', function(yes) {
						if (yes) {
							ajaxJsonObjectByUrl("deleteAccFiscalExpenditure.do", {
	...
	function saveTemplate(){
		if($("form").valid()){
			...
			if(rows == 0){
				$.ligerDialog.warn('请添加财政收入科目！');
				return;
			}
			var formPara ={
				template_id : $("#template_id").val(),
				template_name : $("#template_name").val(),
				template_type_code : "Z008",
				template_type_name : "财政基本补助支出结转",
			...
			ajaxJsonObjectByUrl("saveAccFiscalExpenditure.do",formPara,function (responseData){
        		if(responseData.state=="true"){
        			query();
        		}
        	});
	...
	function setIncomSubj(){
		parent.$.ligerDialog.open({ 
			...
			url: 'hrp/acc/termend/accTermendTemplateSubjPage.do?isCheck=false&subj_type_code=04&code_like=410101', 
		...
		$.ligerDialog.confirm('确定生成凭证?', function (yes){
	        if(yes){
	        	...
	        	for (var i = 0; i < rows.length; i++){
	        		...
	        		ajaxJsonObjectBylayer("addAccFiscalExpenditureVouch.do",para,function(responseData){	
	        			...
	        			var paraVouch={
	        				...
	        				isShowMin:true,
	        				isModal:true,
	        				data:{auto_id:responseData.vouch_id, busi_log_table:'ACC_BUSI_LOG_ZZ', busi_type_code:'Z008',busi_no:para.template_id}
	        			};
	...
	function showVhouchList(){
		parent.$.ligerDialog.open({ 
			...
			width: $(window).width() - 20, 
			height: $(window).height() - 50, 
			url: 'hrp/acc/termend/accTermendTemplateVouchPage.do?isCheck=false&template_type_code=Z008&acc_year='+liger.get("acc_year_month").getValue().split(".")[0]+'&acc_month='+liger.get("acc_year_month").getValue().split(".")[1],
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div id="layout1" style="width:100%;margin:0; padding:0;">
		...
		<div position="center" title="模板信息">
			...
			<form name="form1" method="post"  id="form1" >
				<table cellpadding="0" cellspacing="0" class="l-table-edit" >
					...
					<tr>
						<td align="right" class="l-table-edit-td">
							<span style="color:red">*</span>借：　　结余科目：
						</td>
						<td align="left" colspan="3" class="l-table-edit-td" >
					...
					<tr>
						<td align="right" class="l-table-edit-td" >
							<span style="color:red">*</span>贷：财政补助结转：
						</td>
						<td align="left" colspan="3" class="l-table-edit-td">
		...
		<div position="centerbottom" id="navtab1" style="border:1px solid #A3C0E8; ">
			<div tabid="subjDiv" title="财政收入科目" lselected="true" >
