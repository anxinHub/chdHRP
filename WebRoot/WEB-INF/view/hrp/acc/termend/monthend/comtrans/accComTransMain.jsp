...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			usePager : false, url : 'queryAccComTrans.do', checkbox : true, isSingleCheck: true,  
			...
			case "delete":
				...
				if (data.length == 0) {
					...
					$.ligerDialog.confirm('确定删除?', function(yes) {
						if (yes) {
							ajaxJsonObjectByUrl("deleteAccComTrans.do", {
	...
	function setSubj(){
		...
		parent.$.ligerDialog.open({ 
			...
			url: 'hrp/acc/termend/accTermendTemplateSubjPage.do?isCheck=false', 
	...
	function saveTemplate(){
		if($("form").valid()){
			...
			var formPara ={
				...
				template_type_code : "Z007",
				template_type_name : "通用结转",
			...
			ajaxJsonObjectByUrl("saveAccComTrans.do",formPara,function (responseData){
        		if(responseData.state=="true"){
	...
	function createVouch(){
		...
		$.ligerDialog.confirm('确定生成凭证?', function (yes){
	        if(yes){
	        		...
	        		ajaxJsonObjectBylayer("addAccComTransVouch.do",para,function(responseData){	
	        			...
	        			var paraVouch={
	        				...
	        				isShowMin:true,
	        				isModal:true,
	        				data:{auto_id:responseData.vouch_id, busi_log_table:'ACC_BUSI_LOG_ZZ', busi_type_code:'Z007',busi_no:para.template_id}
	        			};
	...
	function showVhouchList(){
		parent.$.ligerDialog.open({ 
			...
			url: 'hrp/acc/termend/accTermendTemplateVouchPage.do?isCheck=false&template_type_code=Z007&acc_year='+liger.get("acc_year_month").getValue().split(".")[0]+'&acc_month='+liger.get("acc_year_month").getValue().split(".")[1],
	...
	<div id="layout1" style="width:100%;margin:0; padding:0;">
		...
		<div position="center" title="模板信息">
			...
			<form name="form1" method="post"  id="form1" >
				...
				<fieldset id="budg" style="border: 1px solid #7d7474; width: 450px; margin-left: 5px; margin-top: 10px; padding: 5px 0;">
