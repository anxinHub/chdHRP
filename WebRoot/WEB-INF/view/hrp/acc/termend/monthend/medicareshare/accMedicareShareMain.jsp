...
<script type="text/javascript">
	...
	$(function() {
		$("#layout1").ligerLayout({ leftWidth: 265, centerBottomHeight: $(window).height()-140});
		$("#navtab1").ligerTab({contextmenu: false,onAfterSelectTabItem: function(id){
			if(id == "incomDiv"){
				if(!liger.get("incomsubjgrid")){
					loadIncomSubjGrid();
				}
				if($("#template_id").val()){
					queryIncomSubj();
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			usePager : false, url : 'queryAccMedicareShare.do', checkbox : true, isSingleCheck: true,  
	...
	function itemclick(item) {
		if (item.id) {
			...
			case "add":
				...
				medSubjGrid.deleteAllRows();
				if(!liger.get("incomsubjgrid")){
					incomSubjGrid.deleteAllRows();
				}
				return;
			case "delete":
				...
				if (data.length == 0) {
					...
					$.ligerDialog.confirm('确定删除?', function(yes) {
						if (yes) {
							ajaxJsonObjectByUrl("deleteAccMedicareShare.do", {
	...
	//查询医保科目信息
	function queryMedSubj() {
		medSubjGrid.options.parms = [];
		medSubjGrid.options.newPage = 1;
		//根据表字段进行添加查询条件
		medSubjGrid.options.parms.push({
			name : 'template_id',
		...
		//加载查询条件
		medSubjGrid.loadData(medSubjGrid.where);
	}
	
	//医保科目Grid
	function loadMedSubjGrid() {
		//medSubjGrid = $("#medsubjgrid").css({"margin-left" : 40}).ligerGrid({
		medSubjGrid = $("#medsubjgrid").ligerGrid({
			columns : [ {
				display : '明细类别', name : 'detail_type', align : 'left', width: '80', hide: true,
				render : function(rowdata, rowindex,value) {
					rowdata.detail_type = "debit";
					return rowdata.detail_type;
				}
			}, {
			...
			toolbar : {
				items : [ {
					text : '科目设置', id : 'add', click : setMedSubj, icon : 'add'
				}, {
					line : true
				}, {
					text : '科目删除', id : 'delete', click : removeMedSubj, icon : 'delete'
				}]
			},
		});
	}
	...
	//科目删除
	function removeMedSubj(){
		medSubjGrid.deleteSelectedRow();
	}
	
	//查询收入科目信息
	function queryIncomSubj() {
		incomSubjGrid.options.parms = [];
		incomSubjGrid.options.newPage = 1;
		//根据表字段进行添加查询条件
		incomSubjGrid.options.parms.push({
			name : 'template_id',
			value : $("#template_id").val()
		});
		incomSubjGrid.options.parms.push({
		...
		//加载查询条件
		incomSubjGrid.loadData(incomSubjGrid.where);
	}
	
	//收入科目Grid
	function loadIncomSubjGrid() {
		//incomSubjGrid = $("#incomsubjgrid").css({"margin-left" : 40}).ligerGrid({
		incomSubjGrid = $("#incomsubjgrid").ligerGrid({
			columns : [ {
				display : '明细类别', name : 'detail_type', align : 'left', width: '80', hide: true,
				render : function(rowdata, rowindex,value) {
					rowdata.detail_type = "credit";
					return rowdata.detail_type;
				}
			}, {
				display : '科目编码', name : 'subj_code', align : 'left', width: "120"
			...
			toolbar : {
				items : [ {
					...
					text : '科目删除', id : 'delete', click : removeIncomSubj, icon : 'delete'
				}]
			},
		});
	}
	...
	//科目删除
	function removeIncomSubj(){
		incomSubjGrid.deleteSelectedRow();
	}
	...
	function loadDict() {
		...
		autocomplete("#debit_subj_code1", "../../../querySubjAll.do?isCheck=false", "id", "text", true, true, paras, false, "", "400", "", subjWidth); */	
	...
	function saveTemplate(){
		if($("form").valid()){
			...
			var medData = medSubjGrid.getData();
			var rows = 0;
			$.each(medData,function(i, rowdata){
				if(rowdata.subj_id){
					rows += 1;
				}
			})
			if(rows == 0){
				$.ligerDialog.warn('请添加医保科目！');
				return false;
			}
			var incomData = incomSubjGrid.getData();
			rows = 0;
			$.each(incomData,function(i, rowdata){
				if(rowdata.subj_id){
					rows += 1;
				}
			})
			...
			var formPara ={
				...
				template_type_code : "Z005",
				template_type_name : "医保结算差额分摊",
				vouch_type_code : liger.get("vouch_type_code").getValue(),
				summary : $("#summary").val(),
				/* debit_subj_code1 : liger.get("debit_subj_code1").getValue(), */
				detailData : JSON.stringify(medData),
				secondData : JSON.stringify(incomData)
			};
			ajaxJsonObjectByUrl("saveAccMedicareShare.do",formPara,function (responseData){
        		if(responseData.state=="true"){
        			query();
        		}
        	});
	...
	function setMedSubj(){
		parent.$.ligerDialog.open({ 
			...
			buttons: [
				{ text: '确定', onclick: writeMedSubj, cls:'l-dialog-btn-highlight' },
				{ text: '取消', onclick: closeDialog }
			]
		}); 
	}
	//写入选择的科目
	function writeMedSubj(item, dialog){ 
	...
	function setIncomSubj(){
		parent.$.ligerDialog.open({ 
			...
			modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
			buttons: [
				{ text: '确定', onclick: writeIncomSubj, cls:'l-dialog-btn-highlight' },
				{ text: '取消', onclick: closeDialog }
			]
		}); 
	}
	//写入选择的科目
	function writeIncomSubj(item, dialog){ 
		var rows = dialog.frame.getSelectRows();
        if (!rows){
            alert('请选择行!');
            return;
		...
		$.ligerDialog.confirm('确定生成凭证?', function (yes){
	        if(yes){
	        	...
	        	for (var i = 0; i < rows.length; i++){
	        		...
	        		ajaxJsonObjectBylayer("addAccMedicareShareVouch.do",para,function(responseData){	
	        			...
	        			var paraVouch={
	        				...
	        				isShowMin:true,
	        				isModal:true,
	        				data:{auto_id:responseData.vouch_id, busi_log_table:'ACC_BUSI_LOG_ZZ', busi_type_code:'Z005',busi_no:para.template_id}
	        			};
	...
	function showVhouchList(){
		parent.$.ligerDialog.open({ 
			...
			width: $(window).width() - 20, 
			height: $(window).height() - 50, 
			url: 'hrp/acc/termend/accTermendTemplateVouchPage.do?isCheck=false&template_type_code=Z005&acc_year='+liger.get("acc_year_month").getValue().split(".")[0]+'&acc_month='+liger.get("acc_year_month").getValue().split(".")[1],
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div id="layout1" style="width:100%;margin:0; padding:0;">
		...
		<div position="center" title="模板信息">
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				<tr>
					...
					<td align="right" class="l-table-edit-td" style="padding-left: 20px;">
						<input type="button" id="but_add" accessKey="T"  value="费用分摊(T)"/>
						&nbsp;&nbsp;
			...
			<form name="form1" method="post"  id="form1" >
				<table cellpadding="0" cellspacing="0" class="l-table-edit">
					...
					<!-- <tr>
						<td align="right" class="l-table-edit-td" > 
							<span style="color:red">*</span>借：医保结算差额科目：
						</td>
						<td align="left" colspan="3" class="l-table-edit-td"  >
					...
					<tr>
						<td align="right" class="l-table-edit-td" >
							<span style="color:red">*</span>贷：医疗收入：
						</td>
						<td align="left" colspan="3" class="l-table-edit-td" style="width: 80px;">
		...
		<div position="centerbottom"  id="navtab1" style="border:1px solid #A3C0E8; ">
			<div tabid="medDiv" title="借：医保结算差额科目" lselected="true" >
				<div id="medsubjgrid" style="margin-top: 5px"></div>
			</div>
			<div tabid="incomDiv" title="贷：医疗收入" >
				<div id="incomsubjgrid" style="margin-top: 5px"></div>
			</div>
		</div>
	</div>
</body>
