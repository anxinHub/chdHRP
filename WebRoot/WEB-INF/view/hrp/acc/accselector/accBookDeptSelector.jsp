...
$(function ()
        ...
        function moveToRight()
        {
        	...
        	$.each(nodes,function(index,value){ 
        		if((index+1) == nodes.length){
        			param = param +"{'id':'"+value.id+"."+value.dept_id+"','text':'"+value.name+"','isParent':'"+value.isParent+"'}";
        		}else{
        			param = param +"{'id':'"+value.id+"."+value.dept_id+"','text':'"+value.name+"','isParent':'"+value.isParent+"'},";
        		}
        	});
        ...
        function onBetween(){
        	...
        	if($("#between").attr("checked") == true){
        		...
        		if(nodes_value.length > 2){ 
        			box2.removeItems(data);
        			$("#between").attr("checked",false);
        			$.ligerDialog.error('只能添加两个部门进行范围查询');
        			return;
        		} 
        	}
    ...
    function loadTree(){
    	...
    	ajaxJsonObjectByUrl("queryDeptBySelector.do?isCheck=false",{'select_code':select_code,'queryTree':queryTree,'is_stop':'0'},function (responseData){
  ...
  <style type="text/css">
   		...
   		<tr>
   			<td align="center">
   				<select id="select_code" name="select_code">
   					...
   					<option value="dept_code">编码</option>
   					<option value="dept_name">名称</option>
   				</select>
   			</td>
   ...
   <div style="margin-left:160px; " ><input type="checkbox" id="between" name="between" onclick="onBetween();"/>按范围查询</div>
   </div>
   </div>
   </div>
    </body>
