...
$(function ()
        ...
        function onBetween(){
        	...
        	if($("#between").attr("checked") == true){
        		if(data == null || data == ""){
        			...
        			$("#between").attr("checked",false);
        			$.ligerDialog.error('请先又右添加两个供应商');
        			return;
        		}
        		if(data.length > 2){
        			box2.removeItems(data);
        			$("#between").attr("checked",false);
        			$.ligerDialog.error('只能添加两个供应商进行范围查询');
        			return;
        		}
        	}
    ...
    function loadTree(){
    	...
    	ajaxJsonObjectByUrl("querySupBySelector.do?isCheck=false",{'select_code':select_code,'queryTree':queryTree,'is_stop':'0'},function (responseData){
   		...
   		<tr>
   			<td align="center">
   				<select id="select_code" name="select_code">
   					...
   					<option value="sup_code">编码</option>
   					<option value="sup_name">名称</option>
   				</select>
   			</td>
