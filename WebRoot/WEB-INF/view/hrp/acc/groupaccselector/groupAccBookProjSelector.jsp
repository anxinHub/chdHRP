...
$(function ()
        ...
        function onBetween(){
        	...
        	if($("#between").attr("checked") == true){
        		if(data == null || data == ""){
        			...
        			$("#between").attr("checked",false);
        			$.ligerDialog.error('请先又右添加两个项目');
        			return;
        		}
        		if(data.length > 2){
        			box2.removeItems(data);
        			$("#between").attr("checked",false);
        			$.ligerDialog.error('只能添加两个项目进行范围查询');
        			return;
        		}
        	}
     ...
     function getListBox(){
    	 ...
    	 if($("#between").attr("checked") == true){
    		...
    		parent.$("#source_id").val(liger.get("source_code").getValue().split(".")[1]);
     	 }else{
     		if(data.length > 50){  
     			
      			$.ligerDialog.error('选中的项目不能超过50个');
      		//	box2.removeItems(data);
      			return;
      		}
    ...
    function loadTree(){
    	...
    	ajaxJsonObjectByUrl("queryGroupProjBySelector.do?isCheck=false",{'select_code':select_code,'queryTree':queryTree,'is_stop':'0','rela_code':'${rela_code}'},function (responseData){
    ...
    function loadSelect(){
    	$("#source_code").ligerComboBox({
          	...
          	onSuccess:function(data){
          			liger.get("source_code").setValue("0.0");
          			liger.get("source_code").setText("全部");
          	}
		 });
    }
  ...
  <style type="text/css">
   		...
   		<tr>
   			<td align="center">
   				<select id="select_code" name="select_code">
   					...
   					<option value="proj_code">编码</option>
   					<option value="proj_name">名称</option>
   				</select>
   			</td>
   ...
   <table cellpadding="0" cellspacing="0" class="l-table-edit"  border="0">
   	<tr>
   		<td>资金来源：</td>
   		<td><input id="source_code" name="source_code" /></td>
   	</tr>
   </table>
    </body>
</html>
