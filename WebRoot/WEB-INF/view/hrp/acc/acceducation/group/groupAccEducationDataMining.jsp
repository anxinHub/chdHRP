...
<script type="text/javascript">
    ...
    function loadHead(){
        if(identification == "Education_bal_sr" || identification == "Education_bal_ot"){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '摘要', name: 'summary', align: 'left',width:'35%'
					 },
                     { display: '科目', name: 'subj_name', align: 'left',width:'35%'
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:false,url:'queryGroupAccEducationDataMining.do?isCheck=false&'+data,
                     width: '100%', height: '100%', checkbox: false,rownumbers:true,delayLoad:false,
                     selectRowButtonOnly:true,//heightDiff: -10,
                   });

        }else if(identification == "Education_match_sr"){

        	grid = $("#maingrid").ligerGrid({
                columns: [ 
                          { display: '单据编号', name: 'business_no', align: 'left',width:'20%'},
                          { display: '批复文号', name: 'reply_no', align: 'left',width:'10%'},
                          { display: '批复日期', name: 'reply_date', align: 'left',width:'10%'},
     					  { display: '项目编码', name: 'proj_code', align: 'right',width:'10%'},
     					  { display: '项目名称', name: 'proj_name', align: 'right',width:'10%'},
     					  { display: '负责人', name: 'emp_name', align: 'right',width:'10%'},
     					  { display: '批复金额', name: 'reply_money', align: 'right',width:'20%',
         					  render : function(rowdata, rowindex,
									value) {
         ...
         }else if(identification == "Education_match_ot"){
        	 ...
        	 grid = $("#maingrid").ligerGrid({
                 columns: [ 
                            ...
                            { display: '支出金额', name: 'debit', align: 'left',width:'20%',
           					  render : function(rowdata, rowindex,
  									value) {
                        		  return formatNumber(rowdata.debit, 2, 1)
                        	  }}
          ...
          }else if(identification == "Education_bal_sr_cz_dy" || identification == "Education_bal_ot_cz_dy" || identification == "Education_bal_sr_kj_dy" || identification == "Education_bal_ot_kj_dy"){
          	grid = $("#maingrid").ligerGrid({
                columns: [ 
                          { display: '凭证号', name: 'vouch_no', align: 'left',width:'10%',
                              render:function(rowdata){
                          ...
                          dataAction: 'server',dataType: 'server',usePager:false,url:'queryGroupAccEducationDataMiningByDy.do?isCheck=false&'+data,
                          width: '100%', height: '100%', checkbox: false,rownumbers:true,delayLoad:false,
                          selectRowButtonOnly:true,//heightDiff: -10,
                        });
