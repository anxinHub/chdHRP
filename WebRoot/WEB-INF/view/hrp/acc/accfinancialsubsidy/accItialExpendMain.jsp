...
<script type="text/javascript">	
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
					 ...
					 { display: '科目', name: 'subj_name', align: 'left',width: '300'
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAccItialExpend.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'accItialExpendAddPage.do', height: 376,width: 650, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAccCashItem(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deletBatchAccItialExpend.do",{ParamVo : ParamVo.toString()},function (responseData){
