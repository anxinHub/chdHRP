...
<script type="text/javascript">	
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '支出金额', name: 'debit', align: 'right',formatter:'###,##0.00',
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryGroupAccItialExpend.do',
                     ...
                     toolbar: { items: [
    	                ...
    	                { line:true }/* ,
    	                { text: '导入凭证', id:'import', click: itemclick,icon:'import' },
    	                { line:true } */
    				]}
                   });
    ...
    function print(){
    	...
    	var heads={
	   		...
	   		var printPara={
	   			...
	   			class_name: "com.chd.hrp.acc.service.books.memorandumbook.GroupAccItialIncomeService",
				method_name: "queryGroupAccItialIncomePrint",
				bean_name: "groupAccItialIncomeService",
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'groupAccItialExpendAddPage.do', height: 376,width: 650, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAccCashItem(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deletBatchGroupAccItialExpend.do",{ParamVo : ParamVo.toString()},function (responseData){
                    ...
                    return;
                case "import":
              		$.ligerDialog.open({url: 'groupAccVouchImportMainPage.do?isCheck=false', height: 464,width: 590, title:'导入凭证',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.save(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "Excel":
                case "Word":
                case "PDF":
