...
<script type="text/javascript">
     ...
     $(function (){
        ...
        $("#emp_kind").ligerTextBox({width:180});
        ...
        BindKeyBoard([
              	    {keyCode:27,fn:function(){closeDialog()}}//关闭ESC
              	]);
     ...
     function loadHead(){
     	grid = $("#maingrid").ligerGrid({
            columns: [ 
 					 ...
 					 },{ display: '职工分类', name: 'kind_name',width:'20%', align: 'left'
	 				 },{ display: '计算公式', name: 'cal_name',width:'23%', align: 'left'
	 				 }],
                      dataAction: 'server',dataType: 'server',usePager:false,url:'queryAccWageVouchSubj.do?isCheck=false&scheme_code=null',
                      height: '98%',width: '63%', checkbox: true,rowindex_idbers:true,
     ...
     function openAddWageSubj(){
    	 ...
    	 $.ligerDialog.open({ 
 			...
 			width: 1150, 
 			height: 500, 
 			url: 'accWageVouchSetPage.do?isCheck=false&wage_code='+liger.get("wage_code").getValue(), 
         ...
         if(item.id)
         {
             switch (item.id)
             {
                 case "add":
                	 ...
                	 $.ligerDialog.open({url: 'accWageVouchSetPage.do?isCheck=false', height:400,width: 570, title:'添加',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '保存', onclick: function (item, dialog) { dialog.frame.saveAccWageVouch(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
     ...
     function  save(){
        ...
        ajaxJsonObjectByUrl("addAccWageVouch.do?isCheck=fasle",formPara,function(responseData){
 ...
 function loadForm(){
	...
	function checkCustomize () {
		...
		for (var i = 0; i < checkTr.length; i++) {
			...
			if (statusObj.$input.attr("checked")) {
	...
	function openUpdate(obj){
	        ...
	        liger.get("subj_code").setText(row.subj_name); 
	...
	function saveData(){
	       ...
	       /*  if(rowindex_id != "undefined" && rowindex_id != ""){
	        ...
	        $("input[name='is_balance']").attr("checked","false");
	...
	$.ligerDialog.open({
			url : 'accWageVouchCalPage.do?isCheck=false&wage_code=' + wage_code
					+ '&cal_name=' + cal_name.replace(/\+/g, "%2B"),
			data : {},
			height : 490,
			width : 1095,
			title : '设置公式',
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					...
					if(cal_name.trim() != null && cal_name.trim() != ""){
...
<body>
	...
	<form name="form1" method="post" id="form1">
		<table cellpadding="0" cellspacing="0" class="l-table-edit"
			...
			<tr>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;">凭证类型：</td>
				<td align="left" class="l-table-edit-td"><input
					name="vouch_type_code" type="text" id="vouch_type_code"
					ltype="text" validate="{required:true,maxlength:20}" /></td>
				<td align="left"></td>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;">工资套：</td>
				<td align="left" class="l-table-edit-td"><input
					name="wage_code" type="text" id="wage_code" ltype="text"
					validate="{required:true,maxlength:20}" /></td>
				<td align="left"></td>
				<td align="left"><input id="save" type="button" value="保存（S）"
					onclick="saveAccWageVouch();" /></td>
				<td align="left"><input id="close" type="button"
					value="关闭（ESC）" onclick="closeDialog();" /></td>
			</tr>
		</table>
	</form>
	<hr>
	<div style="width: 1365px;" >
		<div id="maingrid" style="width: 85%; float: left; display: inline"></div>
		<div style="width: 15%; float: left; display: inline; height: 200px">
			<table cellpadding="0" cellspacing="0" class="l-table-edit">
				<tr>
					<td align="right" class="l-table-edit-td"
				...
				<tr>
					<td align="left" class="l-table-edit-td" colspan="2"
						style="padding-left: 90px;"><input name="is_balance"
						type="checkbox" id="is_balance" ltype="text" />取借贷平衡 <input
						name="is_minus" type="checkbox" id="is_minus" ltype="text" />是否负数
						<input name="is_assign" type="checkbox" id="is_assign" ltype="text" />是否指定科目
					</td>
				</tr>
				<tr>
					<td align="left" colspan="2" class="l-table-edit-td"
						style="padding-left: 100px;"><input class="liger-button"
						type="button" value="重置" onClick="clearContent();"> <input
						class="liger-button" type="button" value="设置"
						onClick="showWindow();"> <input class="liger-button"
						type="button" value="确定" onClick="saveData();"></td>
				</tr>
			</table>
		</div>
	</div>
