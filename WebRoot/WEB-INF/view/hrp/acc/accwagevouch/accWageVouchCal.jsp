...
<script src="<%=path%>/lib/My97DatePicker/WdatePicker.js"	type="text/javascript"></script>
    ...
    <script type="text/javascript">
     ...
     function  save(){
        ...
        var formPara={
           ...
           cal_eng:$("#formula_method_eng").val(),
    ...
    function loadDict(){
    	...
    	var formPara={
     	...
     	ajaxJsonObjectByUrl("../accwageitemcal/queryAccWageItemTree.do?isCheck=false", formPara,
	...
	function onSelect(note){
		...
		if(note.data.pId!=null){
			
			$("#formula_method_chs").insertContent("{"+note.data.name+"}");
	 	    
		 	$("#formula_method_eng").insertContent("{"+note.data.name+"}");
    ...
    function saveAccWageTaxCal(){
    	...
    	return cal_name+"|"+$("#formula_method_eng").val();
    	
    }
    
    function buttonWageData(){
    	
    	$.ligerDialog.open({
			url : 'accWageVouchSetPage.do?isCheck=false',
			data : {},
			height : 350,
			width : 550,
			title : ' ',
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					...
					var cal_name = dialog.frame.showWindow();
					
					$("#formula_method_chs").insertContent("{取工资数据('"+eval(cal_name).group_name+"','"+eval(cal_name).hos_name+"','"+eval(cal_name).copy_name+"','"+eval(cal_name).acc_year
							+"','"+eval(cal_name).acc_month+"','"+eval(cal_name).wage_name+"','"+eval(cal_name).item_name+"','"+eval(cal_name).emp_code+"')}");
					
					$("#formula_method_eng").insertContent("{RF_ACC_WAGEITEMSRLY('"+eval(cal_name).group_id+"','"+eval(cal_name).hos_id+"','"+eval(cal_name).copy_code+"','"+eval(cal_name).wage_code+"','"+eval(cal_name).acc_year
							+"','"+eval(cal_name).acc_month+"','"+eval(cal_name).item_code+"')}");
   ...
   <div id="layout1" style="width:100%; margin:40px;  height:400px;margin:0; padding:0;">
            ...
            <div position="left" title="工资项目"  style="height:360px;overflow:auto;">
            	<ul id="tree1"></ul>
            </div>
            <div position="center" title="计算公式（中文）">
            	<table cellpadding="0" cellspacing="0" class="l-table-edit" ><tr>
					<td align="left" class="l-table-edit-td">
	                	<textarea class="liger-textarea" id="formula_method_chs"  name="formula_method_chs" style="height:245px;width:845px;font-size:16px;resize: none;" validate="{required:true}" ></textarea>
	              	    <input name="formula_method_eng" style="display: none;" id="formula_method_eng" ltype="text" validate="{required:true,maxlength:2000}" />
	                </td>
                <tr/></table>
            </div>  
				...
				<table align="center" >
		           ...
		           <tr>
		          	...
		          	<tr>
		          		...
		          		<td align="left" style="padding:0px 0px 10px 30px;" class="l-table-edit-td">
		          			<input class="liger-button" id="wage_data" type="button" value="取X年Y月工资数据" onClick="buttonWageData();">
		          		</td>
		          		<td align="left" style="padding:0px 0px 10px 30px;" class="l-table-edit-td">
		          			<input class="liger-button" id="old_day" type="button" value="上月自然天数" onClick="buttonOldDay();">
		          		</td>
		          		<td align="left" style="padding:0px 0px 10px 30px;" class="l-table-edit-td">
