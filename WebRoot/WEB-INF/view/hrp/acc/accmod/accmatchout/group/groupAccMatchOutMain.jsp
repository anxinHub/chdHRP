...
<script type="text/javascript">
    ...
    function  query(){
        ...
        grid.options.parms.push({name:'subj_id',value:liger.get("subj_id").getValue().split(".")[0]}); 
    	//加载查询条件
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryGroupAccMatchOut.do',
    ...
    function print(){
    	...
    	var heads={
       		...
       		var printPara={
       			...
       			class_name: "com.chd.hrp.acc.service.books.memorandumbook.GroupAccMatchOutService",
  	  			method_name: "queryGroupAccMatchOutPrint",
  	  			bean_name: "groupAccMatchOutService",
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'groupAccMatchOutAddPage.do?isCheck=false', height: 550,width: 600, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAccPayType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deletBatchGroupAccMatchOut.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function openUpdate(obj){
		...
		var parm = "&out_id=" + vo[0] + 
		   ...
		   "&copy_code=" + vo[3]+
		   "&acc_year=" + vo[4]
    	$.ligerDialog.open({ url : 'groupAccMatchOutUpdatePage.do?isCheck=false&' + parm,data:{}, height: 400,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAccPayType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    ...
    function loadDict(){
    	...
    	autocomplete("#subj_id","../../../querySubj.do?isCheck=false","id","text",true,true,'',false,'','300');  
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="subj_id" type="text" id="subj_id" ltype="text" validate="{required:true,maxlength:20}" /></td>
            <td align="left"></td>
        </tr> 
