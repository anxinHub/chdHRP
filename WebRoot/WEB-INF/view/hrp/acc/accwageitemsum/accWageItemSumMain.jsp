...
<script type="text/javascript">
    ...
    $(function ()
    {
    	...
    	//$("#acc_date").ligerTextBox({width:90});
    	
    	$("#dept_code").ligerTextBox({width:205});
    	
    	$("#dept_attr").ligerTextBox({width:205});
    	
    	$("#emp_kind").ligerTextBox({width:205});
    	
    	//$("#acc_date").ligerTextBox({cancelable: false});
    	...
    	$("#wage_code").bind("change",function(){
    		...
    		var fromData={
            	...
            	autocomplete("#scheme_name","../queryAccWageScheme.do?isCheck=false&scheme_type_code=03","id","text",true,true,fromData);
        })
        
        grid.toggleCol('OUT_CODE', false);

    	grid.toggleCol('OUT_NAME', false);
	
    	grid.toggleCol('KIND_CODE', false);
	
	    grid.toggleCol('KIND_NAME', false);
        ...
        $(":radio").click(function(){
     	   ...
     	   if(val=="1"){
     		  ...
     		  liger.get("dept_attr").setValue('');
     		  
     	      liger.get("dept_attr").setText('');
     	      
     	      liger.get("emp_kind").setValue('');
    		  
    	      liger.get("emp_kind").setText('');
    	      
    	      grid.toggleCol('DEPT_CODE', true);
    	      
    	      grid.toggleCol('DEPT_NAME', true);
    	      
    	      grid.toggleCol('DEPT_KIND_CODE', false);

    	      grid.toggleCol('DEPT_KIND_NAME', false);
    	      
    	      grid.toggleCol('EMP_ID', true);
			  ...
			  liger.get("dept_code").setValue('');
     		  
     	      liger.get("dept_code").setText('');
    	      ...
    	      grid.toggleCol('OUT_CODE', true);

    	      grid.toggleCol('OUT_NAME', true);

    	      grid.toggleCol('KIND_CODE', false);

    	      grid.toggleCol('KIND_NAME', false);
    	      
    	      grid.toggleCol('DEPT_CODE', false);
    	      ...
    	      grid.toggleCol('KIND_CODE', true);

    	      grid.toggleCol('KIND_NAME', true);
		  ...
		  liger.get("dept_code").setValue('');
   	      ...
   	      grid.toggleCol('DEPT_KIND_CODE', true);

	      grid.toggleCol('DEPT_KIND_NAME', true);
    ...
    function show_title(){
    	...
    	var val=$('input[name="acc_dept"]:checked ').val();
    	 ...
    	 if(val=="1"){
   	      ...
   	      liger.get("dept_kind_code").setValue('');
  		  
  	      liger.get("dept_kind_code").setText('');
	...
	function loadTree(obj){
    	...
    	ajaxJsonObjectByUrl("../accwagepay/queryAccWagePayTree.do?isCheck=false&scheme_type_code=03",obj,function (responseData){
...
//     	parent.openDialog({ url : 'hrp/acc/accwagepay/accWageSchemeMainPage.do?isCheck=false&node='+node_name+'&scheme_code='+scheme_code+'&scheme_name='+scheme_name+'&acc_year='+acc_year+'&scheme_type_code='+'03',data:{}, height: 0,width: 0, title:'工资方案维护',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,buttons: [ /* { text: '保存', onclick: function (item, dialog) { dialog.frame.save(); },cls:'l-dialog-btn-highlight' }, */ { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    	
    	parent.$.ligerDialog.open({
            url : 'hrp/acc/accwagepay/accWageSchemeMainPage.do?isCheck=false&node='+node_name+'&scheme_code='+scheme_code+'&scheme_name='+scheme_name+'&acc_year='+acc_year+'&scheme_type_code='+'03',
            data : {},
    ...
    function  query(){
		...
		grid.options.newPage=1;
      	...
      	//var acc_date= liger.get("acc_date").getValue().split(".")[0];
      	
		var acc_month= acc_times.getValue().split(".")[1];
      	
      	//var year_month= liger.get("acc_date").getValue().split(".")[1];
    	...
    	grid.options.parms.push({name:'is_check',value:$('input[name="acc_dept"]:checked ').val()}); 
    	
    	grid.options.parms.push({name:'kind_code',value:liger.get("emp_kind").getValue()}); 

    	
    	var dept_code = liger.get("dept_code").getValue().split(".")[0];
    	console.info("dept_code    "+dept_code);
    	if(dept_code=="true"){
    		dept_code = "";
    	}
    	grid.options.parms.push({name:'dept_code',value:dept_code}); 
    	
    	grid.options.parms.push({name:'dept_attr',value:liger.get("dept_attr").getValue()}); 
    	
    	grid.options.parms.push({name:'dept_kind_code',value:liger.get("dept_kind_code").getValue()}); 
    	
    	grid.set("url","queryAccWageItemSum.do");
    	//加载查询条件
    ...
    function loadHead(){
    	...
    	//var wage_scheme= liger.get("scheme_name").getValue();
    		
    		columns = columns +"{ display: '期间', name: 'ACC_YEAR', align: 'left',frozen: true},"
    		
    		+"{ display: '科室属性编码', name: 'OUT_CODE',id: 'EMP_CODE', align: 'left',frozen: true},"
    		
        	+"{ display: '科室属性名称', name: 'OUT_NAME',id: 'EMP_NAME',align: 'left',frozen: true},"
    		
        	+"{ display: '职工分类编码', name: 'KIND_CODE',id: 'KIND_CODE',align: 'left',frozen: true},"
        	
        	+"{ display: '职工分类名称', name: 'KIND_NAME',id: 'KIND_NAME',align: 'left',frozen: true},"
        	
        	+"{ display: '部门编码', name: 'DEPT_CODE',id: 'DEPT_CODE', align: 'left',frozen: true},"
        	
        	+"{ display: '部门名称', name: 'DEPT_NAME',id: 'DEPT_NAME', align: 'left',frozen: true},"
        	
			+"{ display: '部门分类编码', name: 'DEPT_KIND_CODE',id: 'DEPT_KIND_CODE', align: 'left',frozen: true},"
        	
        	+"{ display: '部门分类名称', name: 'DEPT_KIND_NAME',id: 'DEPT_KIND_NAME', align: 'left',frozen: true},"
        	
        	+"{ display: '人数', name: 'EMP_ID',id: 'EMP_ID',align: 'left',frozen: true}"
                ...
                success: function(data){
                	...
                	if(data.Rows.length>0){
                		...
                		$.each(data.Rows,function(i,v){
                			...
                			columns = columns + ",{ display: '"+v.ITEM_NAME+"',formatter: '###,##0.00', name: '"+v.COLUMN_ITEM+"', align: 'right',"+
    ...
    function loadDict(){
        ...
        autocomplete("#dept_kind_code","../../sys/queryDeptKindDict.do?isCheck=false&is_stop=0","id","text",true,true);
        
		autocomplete("#dept_code","../queryDeptDictNo.do?isCheck=false&is_stop=0","id","text",true,true,'',false,true,'','','205');
        
        autocomplete("#emp_code","../../sys/queryEmpDict.do?isCheck=false&is_stop=0","id","text",true,true);
		
        autocomplete("#dept_attr","../queryDeptOut.do?isCheck=false","id","text",true,true,'',false,true,'','','205');
        
        autocomplete("#emp_kind","../../sys/queryEmpKindDict.do?isCheck=false","id","text",true,true,'',false,true,'','','205');
    ...
    function myPrint(){
    	...
    	/* var heads={
        		...
        		"rows": [   
    	          {"cell":0,"value":"工资套名称: "+tree_wage_name,"colspan":2,"br":true} 
        	]}; */
    	
    	var printPara={  
          		title: acc_times.getValue().split(".")[0]+"年"+acc_times.getValue().split(".")[1]+"月"+tree_wage_name.split(" ")[1]+"工资统计",//标题
          		columns: JSON.stringify(grid.getPrintColumns()),//表头
          		class_name: "com.chd.hrp.acc.service.wagedata.AccWageItemSumService",
       			method_name: "queryAccWageItemSumPrint",
       			bean_name: "accWageItemSumService",
        	...
        	//执行方法的查询条件
       		...
       		printPara['is_check']=$('input[name="acc_dept"]:checked ').val();
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div id="layout1">
      ...
      <div position="center"  title="  ">
		    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
	       		<tr>
		            <td align="right" class="l-table-edit-td"  style="padding-left:20px;"><b><font color="red">*</font></b>期&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;间：</td>
		            ...
		            <!-- <td align="left" style="display: none"><input name="acc_date" type="hidden" id="acc_date"  ltype="text" validate="{required:true,maxlength:20}" /></td> --> 
		       		<td align="right" class="l-table-edit-td"  style="padding-left:20px;">汇总方式：</td>
		            <td align="left" class="l-table-edit-td"><input name="acc_dept" type="radio" id="acc_dept" value="1" checked="checked"/>按部门汇总</td>
		            <td align="left" class="l-table-edit-td"><input name="acc_dept" type="radio" id="acc_type" value="2"/>按科室属性汇总</td>
		            <td align="left" class="l-table-edit-td"><input name="acc_dept" type="radio" id="acc_attr" value="3"/>按人员类别汇总</td>
		            <td align="left" class="l-table-edit-td"><input name="acc_dept" type="radio" id="acc_dept_kind" value="4"/>按部门分类汇总</td>
		            <td align="center"></td>
		       </tr>  
		       <tr id="where_1">
		            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">职工部门：</td>
		            <td align="left" class="l-table-edit-td" colspan="3"><input name="dept_code" type="text" id="dept_code" ltype="text" validate="{required:true,maxlength:20}" /></td>
		            <td align="center"></td>
		       </tr>
		       <tr id="where_2" style="display: none">
		       		<td align="right" class="l-table-edit-td"  style="padding-left:20px;">科室属性：</td>
		            <td align="left" class="l-table-edit-td" colspan="3"><input name="dept_attr" type="hidden" id="dept_attr" ltype="text" validate="{required:true,maxlength:20}" /></td>
		            <td align="center"></td>
		       </tr>
		       <tr id="where_3" style="display: none">
		       		<td align="right" class="l-table-edit-td"  style="padding-left:20px;">人员类别：</td>
		            <td align="left" class="l-table-edit-td" colspan="3"><input name="emp_kind" type="hidden" id="emp_kind" ltype="text" validate="{required:true,maxlength:20}" /></td>
		            <td align="center"></td>
		       </tr>  
		       <tr id="where_4" style="display: none">
		       		<td align="right" class="l-table-edit-td"  style="padding-left:20px;">部门分类：</td>
		            <td align="left" class="l-table-edit-td" colspan="3"><input name="dept_kind_code" type="hidden" id="dept_kind_code" ltype="text" validate="{required:true,maxlength:20}" /></td>
		            <td align="center"></td>
		       </tr>
		    </table>
			<div id="maingrid"></div>
