...
<jsp:include page="${path}/inc_jquery_1.9.0.jsp"/>
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '总账期初余额', name: 'bal_os', align: 'right',formatter:'###,##0.00',
						 render:function(rowdata){
   	   						 return formatNumber(rowdata.bal_os ==null ? 0 : rowdata.bal_os, 2, 1);
   					     }
					 },
                     { display: '往来初始未核销金额', name: 'debit', align: 'right',formatter:'###,##0.00',
                    	 render:function(rowdata){
                    		 return formatNumber(rowdata.debit ==null ? 0 : rowdata.debit,2,1);
                    	 }	 
					 }
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAccCurrentAccountCheck.do?isCheck=true',
                     width: '100%', height: '100%', checkbox: false,rownumbers:true,alternatingRow:false,
                     selectRowButtonOnly:true,//heightDiff: -10,
                     rowAttrRender:function(rowdata,rowid){
                    	if((rowdata.bal_os-rowdata.debit)>0 || (rowdata.bal_os-rowdata.debit)<0){
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ 
    			url : 'accCurrentAccountCheckDetailPage.do?isCheck=false&' + parm,data:{}, 
    					height: 550,width: 900,title:'往来初始明细账校验',
