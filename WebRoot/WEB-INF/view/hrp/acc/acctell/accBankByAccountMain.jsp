...
<script type="text/javascript">	
    ...
    // 科目条件改变
    function subjConChangeTip(){
    	if(subjCode == null || subjCode == liger.get("subj_code").getValue()){
    		return true;
    	}else{
			$.ligerDialog.warn('科目改变，请先查询.');
			return false;
    	}
    }
    ...
    //查询
    function  query(){
    	subjCode = liger.get("subj_code").getValue();
		...
		parms.push({name:'subj_id',value: liger.get("subj_code").getValue().split(".")[0]});
        
    	parms.push({name:'nature_code',value: liger.get("nature_code").getValue()=="全部"?"":liger.get("nature_code").getValue()});
    	
    	parms.push({name:'begin_date',value:$("#begin_date").val()}); 
    	
    	parms.push({name:'end_date',value:$("#end_date").val()}); 
    	
    	if($('#is_con').val() != '' && $('#is_con').val() != null){
    		
    		parms.push({name:'is_con',value:$('#is_con').val()});
    	
    	}

		$( "#maingrid" ).pqGrid( "option" , "dataModel.getUrl", function(){
			return { url: "queryAccBankByAccount.do", data:parms }
		});
   ...
   // ============ligergrid 用的数据加载===============
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
    		  columns: [
    		            ...
    		            { display: '出纳日期', name: 's_occur_date', align: 'left',width:100,type :'date',
        				      format : 'yyyy-MM-dd',editor : {
      							type : 'date',
      						}
				 		},
                        ...
                        {display: '附件张数', name: 'att_num', align: 'left',width:80,editor:{
        					type : 'text',
        			     	}
                        },
   					 { display: '票据号', name: 'check_no', align: 'left',width:120,editor : {
     					type : 'text',}
     				 },{
     					display : '结算方式',
     					name : 'pay_code', 
   					 ...
   					 { display: '收入金额', name: 'debit', align: 'right',width:120,editor : {
							type : 'float',
   					 ...
   					 { display: '支出金额', name: 'credit', align: 'right',width:120,editor : {
							type : 'float',
						},render:function(rowdata){
   					 ...
   					 { display: '余额', name: 'bal', align: 'right',width:80,
   						 render:function(rowdata){
                     ...
                     dataAction: 'server',dataType: 'server',usePager:false,heightDiff:30,url:'queryAccBankByAccount.do',isAddRow:false,/* enabledEdit : true, */
                     width: '100%', height: '100%', rownumbers:true,checkbox:true,checkBoxDisplay:isCheckDisplay,onBeforeEdit: f_onBeforeEdit,
                     selectRowButtonOnly:true,delayLoad:true,onAfterEdit : f_onAfterEdit,enabledEdit : true, onLoaded :f_onLoaded/* , 
    		...
    		if (onoff) {   // 设置开关 当所对应的列是最后可编辑的列（支出金额或收入金额）时，执行
        		...
        		var validate_detail_buf = new StringBuffer();
        			...
        			var formPara = {
        				...
        				debit : (e.record.debit == null ? "":e.record.debit),

        				credit : (e.record.credit == null ? "":e.record.credit),
        				...
        				tell_type_code: liger.get("nature_code").getValue().split(" ")[0]
        			...
        			if (e.record.tell_id == null) {
        				if(acc_para == 1){
        					$.ligerDialog.confirm('是否往对方科目添加出纳账？',"出纳账",function(yes) {
        						if (yes) {
        		    				...
        		    				ajaxJsonObjectByUrl("addAccBankByAccount.do", formPara, function(
        		    						responseData) {
        		    					if (responseData.state == "true") {
        		    						grid.updateCell('tell_id',responseData.tell_id, e.record);
        				...
        				ajaxJsonObjectByUrl("updateAccBankByAccount.do", formPara, function(
        						responseData) {
         ...
         function add_open(){
          ...
          function delete_btn(){
                ...
                $(data).each(function (){
                	...
                	/*  if(this.tell_id == undefined || this.tell_id == ''){
						 $.ligerDialog.warn('只能选择出纳账每日发生的数据');
						 return flag = false;
					 } */
                ...
                if(ParamVo.length == 0 ){
				 ...
				 $.ligerDialog.confirm('确定要删除吗？', '删除', function(flag){
						if(flag){
							 ...
							 ajaxJsonObjectByUrl("deleteAccBankByAccount.do",{ParamVo : ParamVo.toString()},function(responseData){
          ...
          function downTemplate(){
        	  location.href = "downBankByAccountTemplate.do?isCheck=false";
           }
 
          function imp(){
         	 $.ligerDialog.open({url: 'accBankByAccountImportPage.do?isCheck=false', height: 500,width: 1135, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
           }
           ...
           function con(){
			 ...
			 if(this.is_con == '1'){
                ...
                if(flag == false){
           	   ...
           	   $.ligerDialog.open({url: 'updateAccBankByAccountStatePage.do?isCheck=false',
           			   data:{data:data},
           			   height: 300,
           			   width: 400,
           			   title:'',
           			   ...
           			   buttons: [ 
           			              { text: '确定',
           			            	onclick: function (item, dialog) { dialog.frame.accBankByAccountCon();},
             ...
             function cancelCon(){
                    ...
                    if(flag == false){
					 ...
					 $.ligerDialog.confirm('确定要进行取消确认操作吗？', '取消确认', function(flag){
							if(flag){
								 ajaxJsonObjectByUrl("updateAccBankByAccountCancelState.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
    ...
    function print_btn(){
	 ...
	 function add_btn(){
			...
			if(this.debit!=null&&this.credit!= null){
				...
				if(this.summary!= null){
					...
					ParamVo.push(
							...
							(this.att_num==""?0:this.att_num)+"@"+
							year + '-' + (month < 10 ? "0" + month : month) + '-' + (day < 10 ? "0" + day : day)
							+"@"+(this.other_subj_code == null ? "": this.other_subj_code)
							+"@"+ this.summary+"@"+ liger.get("nature_code").getValue().split(" ")[0]+"@"+subj+"@"+(this.tell_id==null?"":this.tell_id)
						)
				}
			}
		...
		if(ParamVo.length == 0){
			$.ligerDialog.warn('未更新任何数据');
			return ;
		}
		
		 ajaxJsonObjectByUrl("addBatchAccBankAccount.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
    ...
    function itemclickmenu(item, i)
    {
    	 if(item.id == "insert"){
    		 ...
    		 $.ligerDialog.open({url: 'addAccBankByAccountPage.do?isCheck=false&tell_number='+actionCustomerID.split(",")[1]+"&occur_date=" + actionCustomerID.split(",")[2],height: 400,width: 470,title:'添加',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) {dialog.frame.saveAccTell(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    	}
    ...
    function loadDict(){
        ...
        autocomplete("#subj_code","../querySubj.do?isCheck=false&is_last=1&subj_nature_code=03&acc_year="+begin_date.substring(0,4),"id","text",true,true,'',true);

    	autocompleteObj({
	    		id:"#nature_code",
	    		urlStr:"../queryAccTellType.do?isCheck=false&nature_code=03",
	    		valueField:"id",
	    		textField:"text",
	    		autocomplete:true,
	    		highLight:true,
...
/*         	if(num != 0){
		...
		ajaxJsonObjectByUrl("queryAccTellVouch.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
        	...
        	if(responseData.state=="true"){
            	...
            	$.ligerDialog.open({url: 'chooseAccVouchTypePage.do?isCheck=false&tell_id='+tell_id+'&subj_code='+liger.get("subj_code").getValue(),
        			data:{data:data},
        			height: 300,
        			width: 400,
        			title:'',
	...
	function cYearFunc(){
	 ...
	 function cFunc(obj){
		 ...
		 autocomplete("#subj_code","../querySubj.do?isCheck=false&is_last=1&subj_nature_code=03&acc_year="+c.newdate[p],"id","text",true,true,'',true);

	}
	
	 function addSummary (){ 
		 	
			parent.$.ligerDialog.open({ url : 'hrp/acc/acctell/accTellBankSummaryPage.do?isCheck=false' ,data:{}, height: 470,width:500, title:'摘要维护',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
				  });
	 ...
	 function printSet(){
		 var useId=0;//统一打印
		officeFormTemplate({template_code:"01045",use_id:useId});
	 }
	 ...
	 function templatePrint(){
		...
		var para={
				template_code:'01045',
				class_name:"com.chd.hrp.acc.serviceImpl.tell.AccTellServiceImpl",
				method_name:"queryAccBankByAccountForTemplatePrint",
				...
				nature_code:liger.get("nature_code").getValue()=="全部"?"":liger.get("nature_code").getValue().split(" ")[0],
				type_code:liger.get("nature_code").getValue()=="全部"?"":liger.get("nature_code").getValue().split(" ")[1],
				p_num:1
		};
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div style="border: 1px">
	    ...
	    <input type="button" value="模板打印" onclick="templatePrint();" />
	    <input type="button" value="模板设置" onclick="printSet();" />
	</div>
	<div id="maingrid"></div>
