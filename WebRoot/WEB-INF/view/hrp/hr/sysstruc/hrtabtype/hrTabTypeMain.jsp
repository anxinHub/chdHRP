...
<script>
	...
	function add() {
		$.etDialog.open({
			url : 'hrTabTypeAddPage.do?isCheck=false',
			height : 300,
			width : 700,
			title : '数据表分类添加页',
			btn : [ '确定', '取消' ],
	...
	function update(data, index, value) {
		...
		$.etDialog.open({
			url : 'hrTabTypeUpdatePage.do?isCheck=false&' + parm,
			height : 320,
			width : 650,
			title : '数据表分类修改页',
			btn : [ "确定", "取消" ],
	...
	function importExcel(){
		...
		importSpreadView("/hrp/hr/sysstruc/hrtabtype/importExcel.do?isCheck=false", para);
	}
