		...
		var columns = [ 
		...
		},
		 {
			...
			editor : {
				type : 'select',
				keyField : 'med_id',
				url : '../queryMedTypeSelect.do?isCheck=false&perm_type='+"${perm_type}",
			}
		},];
		var paramObj = {
			height : '100%',
			...
			toolbar : {
				items : [ {
					...
					icon : 'save',disabled:state == 0 ? false : true
				}, {
					type : 'button',
					label : '添加',
					listeners : [ {
					...
					icon : 'add',disabled: state == 0 ? false : true
				}, {
					type : 'button',
					label : '删除',
					listeners : [ {
					...
					icon : 'delete',disabled: state == 0 ? false : true
				} ]
			}
		};
