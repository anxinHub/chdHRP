...
function save() {
		...
		var ParamVo  = [];
		var resignation_state;
		resignation_state=data[0].resignation_state;
		if(resignation_state=='auditResignationHand'){
			if(liger.get("state").getValue()=='1'){
				 $(data).each(function (i){	
						
						ParamVo.push({
							resignation_id : data[i].resignation_id,
							emp_id : data[i].emp_id,
				 			group_id : data[i].group_id,
				 		    hos_id: data[i].hos_id,
				 		   handover_state : liger.get("state").getValue(),
				 		  handover_note : $("#note").val(),
			...
			ajaxJsonObjectByUrl("updateResignationHandoverStateBatch.do",{ParamVo : JSON.stringify(ParamVo)},function (responseData){
