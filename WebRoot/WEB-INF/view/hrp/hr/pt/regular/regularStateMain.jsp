...
function save() {
		...
		if(regular_state=='auditTeacher'){
			if(liger.get("state").getValue()=='1'){
				 $(data).each(function (i){	
						
						ParamVo.push({
							regular_id : data[i].regular_id,
				 			group_id : data[i].group_id,
				 		    hos_id: data[i].hos_id,
				 		   teacher_state : liger.get("state").getValue(),
				 		  teacher_dept_note : $("#note").val(),
				 			tab_code: 'HR_REGULAR'
						});
		      });
			}else{
			...
			ajaxJsonObjectByUrl("updateRegularTeacherBatch.do",{ParamVo : JSON.stringify(ParamVo)},function (responseData){
		...
		}else if(regular_state=='auditDept'){
			if(liger.get("state").getValue()=='1'){
				 $(data).each(function (i){	
			...
			ajaxJsonObjectByUrl("updateRegularDeptBatch.do",{ParamVo : JSON.stringify(ParamVo)},function (responseData){
				if (responseData.state == "true") {
					parent.$.ligerDialog.success("审核成功");
					parent.query();
					frameElement.dialog.close();
			...
			ajaxJsonObjectByUrl("updateRegularChargeBatch.do",{ParamVo : JSON.stringify(ParamVo)},function (responseData){
				if (responseData.state == "true") {
					parent.$.ligerDialog.success("审核成功");
					parent.query();
					frameElement.dialog.close();
			...
			ajaxJsonObjectByUrl("updateRegularHRBatch.do",{ParamVo : JSON.stringify(ParamVo)},function (responseData){
				if (responseData.state == "true") {
					parent.$.ligerDialog.success("审核成功");
					parent.query();
					frameElement.dialog.close();
				 ...
				 $(data).each(function (i){	
						...
						ParamVo.push({
							regular_id : data[i].regular_id,
				 			group_id : data[i].group_id,
				 		    hos_id: data[i].hos_id,
				 		   president_state : liger.get("state").getValue(),
				 		  president_state_note : $("#note").val(),
				 			tab_code: 'HR_REGULAR'
						});
		      });
			}else{
			...
			ajaxJsonObjectByUrl("updateRegularPresidentBatch.do",{ParamVo : JSON.stringify(ParamVo)},function (responseData){
				if (responseData.state == "true") {
					parent.$.ligerDialog.success("审核成功");
					parent.query();
					frameElement.dialog.close();
