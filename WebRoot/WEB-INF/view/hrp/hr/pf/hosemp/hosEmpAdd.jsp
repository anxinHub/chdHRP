<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <head>
        ...
        <script>
            ...
            var tab_code = parentWindow.mainAndChild;
            var tab= parentWindow.mainAndChild;
            ...
            function reloadChildGrid(value) {// 远程加载子集表格 child_grid
              ...
              //  jQuery.getScript("/CHD-HRP/lib/hrp/hr/data/"+"${group_id}"+"/"+"${hos_id}"+"/grid/"+tab_code+".data", function(data, status, jqxhr) {
           ...
           /*    }) */
                        	...
                        	if(node.id!="HOS_EMP"){
                                ...
                                tab=node.id;
                         		var from=getFromField({group_id:'${group_id}',hos_id:'${hos_id}',formTable:[node.id]})
                                	from.fieldItems[0].OPTIONS.defaultOption={id: emp_id, text: emp_name}
                              	 $('.form_content').html(''); //初始化前清空
            ...
            function initForm(value) {
                //jQuery.getScript("/CHD-HRP/lib/hrp/hr/data/"+"${group_id}"+"/"+"${hos_id}"+"/form/HOS_EMP.data", function(data, status, jqxhr) {
               var from;
                	if(value){
                		from=getFromField({group_id:'${group_id}',hos_id:'${hos_id}',formTable:[value]})
                    	
                	}else{
                	 from=getFromField({group_id:'${group_id}',hos_id:'${hos_id}',formTable:[tab_code]})
                	}
                	$('.form_content').html(''); //初始化前清空
                	if(tab_code!="HOS_EMP"){
                		 $('.file_content').hide(true);
            ...
            function initEvent() {
                ...
                $("#btn_save").click(function () {
                	var url;
                	var tab= child_tree.getSelectedNodes()[0].id ;
                	if(tab=="HOS_EMP"){
                		url="addHosEmp.do?isCheck=false";
                	}else{
                	 //拼接动态子集查询
              		var  arr=tab.toLowerCase().split("_");
              		 var url='add';
              	  for(var i=1;i< arr.length;i++){//遍历字符串
              		  url+=arr[i][0].toUpperCase() + arr[i].substring(1, arr[i].length);
              	      }
                    ...
                    if(formData.get('dept_id')!=null){
                    	var after_dept_id=formData.get('dept_id');
                    	formData.delete("dept_id");
                    	 var dept_id=after_dept_id.split('@')[0];
                         var dept_no=after_dept_id.split('@')[1];
                         formData.append('dept_id', dept_id);
                         formData.append('dept_no', dept_no);
                    }
                    
                    formData.append('tab_code', tab);
                    ...
                    ajaxPostFormData({
                        ...
                        success: function (data) {
                            if (data.state == 'true') {
                                emp_id = data.emp_id==undefined?emp_id:data.emp_id;
                                emp_name=formData.get('emp_name')==undefined?emp_name:formData.get('emp_name');
                                //initForm(tab);
                                reloadChildGrid(emp_id);
                                var parentFrameName = parent.$.etDialog.parentFrameName;
                                var parentWindow = parent.window[parentFrameName];
