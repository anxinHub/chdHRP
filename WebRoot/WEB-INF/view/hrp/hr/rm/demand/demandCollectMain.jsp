...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
//                      { display: '岗位编制人数', name: 'complie_count', align: 'left',width:100
// 					 },
// 		             { display: '期望到岗日期', name: 'attend_time', align: 'left',width:100
// 					 },
...
// 					 },
                     ...
                     url:'queryCollectDemand.do?tab_code=HR_RECRUIT_DEMAND',
                     ...
                     toolbar: { items: [
    	                ...
    	                { line:true },
    	                { text: '系统分析', id:'analysis', click:itemclick,icon:'settings' }
    				]},
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'recruitDemandCollectAddPage.do?isCheck=false', height: 480,width: 790, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true});
              		return;
                case "analysis":
                	$.ligerDialog.open({url: 'demandAnalysisMainPage.do?isCheck=false', height: 400,width: 880, title:'岗位人员分析',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("updateDemandState04Batch.do?isCheck=false&tab_code=HR_RECRUIT_DEMAND",{ParamVo : JSON.stringify(paramVo)},function (responseData){
  ...
  //导入
    function importMainGrid() {
    	$.ligerDialog.open({
            url: "recruitDemandImprotPage.do?isCheck=false&tab_code=" + 'HR_RECRUIT_DEMAND'+"&ui="+"liger",
