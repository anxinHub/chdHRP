...
<head>
	...
	var start_date,state,tab_code,col_code;
    var formValidate;
        var initValidate = function () {
    		...
    		state = $("#state").etSelect({
		        ...
		        defaultValue: "${vo.STATE}",
		    });

    		start_date = $("#start_date").etDatepicker({
            	range : false,
            	defaultDate: "${vo.START_DATE}"
            })
                  ...
                  ajaxPostData({
                     url: 'updateStandard.do',
                      ...
                      success: function (responseData) {
				                ...
				                parentWindow.query();
				                parentWindow.maintain(responseData.id);
				                $("#close").click();                                       
                      },
            ...
            var columns = [
                ...
                { display: '关联字段', name: 'col_codes', width: 150,
                    editor: {
                        type: 'select',
                        keyField:'col_code',
                        //url:'queryStandardColCodeOption.do?isCheck=false&code='+parentCode,
                    }
                },
            ];
            var paramObj = {
                ...
                columns: columns,
                editable:${vo.state == 0 ? true:false},
                cellSelect:cellSelect,
                toolbar: {
                    items: [
                       { type: 'button', label: '添加', listeners: [{ click: add }], icon: 'add',disabled:${vo.state == 0 ? false:true}},
                       { type: 'button', label: '删除', listeners: [{ click: remove }], icon: 'delete',disabled:${vo.state == 0 ? false:true}},
                    ]
                }
            };
        ...
        var cellSelect = function(event, rowData){
			grid.getColumns()[2].editor.url ='queryStandardColCodeOptionEditable.do?isCheck=false&code='+rowData.rowData.tab_code;
        };
...
/* 		    var sss = {"tab_code":tab_code.tab_code,"tab_codes":tab_code.tab_codes};
		...
		function query(){
			arr = [];
			grid.loadData(arr,'queryStandardCtd.do?isCheck=false&id='+'${vo.STAN_ID}');
		}
...
<body  style="overflow:hidden">
	<div  id="btn"  class="button-group" style="overflow-y: auto">
		...
		<input type="hidden" name="stan_id" value="${vo.STAN_ID}"> 
		 <table class="table-layout" style="width: 100%;">
	        <tr>
	            <td class="label"><font size="2" color="red">*</font>薪资标准编码：</td>
	            <td class="ipt">
	                <input id="stan_code" name="stan_code" value="${vo.STAN_CODE}" type="text" style="width:150px;" />
	            </td>
	            <td class="label"><font size="2" color="red">*</font>薪资标准名称：</td>
	            <td class="ipt">
	                <input id="stan_name" name="stan_name" value="${vo.STAN_NAME}" type="text" style="width:150px;" />
	            </td>
	        </tr>
	        <tr>
	            ...
	            <td class="ipt">
	                <input  id="start_date" name="start_date" value="${vo.START_DATE}" type="text" style="width:150px;" />
	            </td>
	        </tr>
    	</table>
    	</form>
