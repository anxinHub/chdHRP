...
<script type="text/javascript">
		...
		function query() {
	    	...
	    	//加载查询条件
	    	grid.loadData(parms,'queryBudgMatPurAdj.do?isCheck=false');
		}
		function loadHead() {
			...
			ajaxPostData({
 			    ...
 			    success: function (responseData) {
 			    	ajaxPostData({
 		 			    ...
 		 			    data: '',
 		 			    success: function (responseData) {
 		 			    	$.etDialog.open({ url : 'budgMatPurAdjAddPage.do?isCheck=false&',height: 300,width: 600, 
		...
		function remove() {
			...
			if(data.length > 1){
				...
				ajaxPostData({
     			    url: 'queryBudgMatPurAdjState.do?isCheck=false',
     			    data: {
     			    	budg_year : budg_year,
     			    ...
     			    success: function (responseData) {
     			    	$.etDialog.confirm('确定要销审选中数据吗?', function () {
							ajaxPostData({
		         			    url: 'deleteBudgMatPurAdj.do?isCheck=false',
	    ...
	    function openUpdate(obj) {
	        var vo = obj.split("|");
	        var parm = "budg_year=" + vo[0] + "&adj_code=" + vo[1];

	        parent.$.etDialog.open({
	            url: 'hrp/budg/budgpur/mat/matpuradj/budgMatPurAdjUpdatePage.do?isCheck=false&'+parm,
	            title: '材料采购预算调整申请修改',
	            isMax: true,
			...
			if (data.length == 0) {
				...
				ajaxPostData({
     			    ...
     			    success: function (responseData) {
     			    	$.etDialog.confirm('确定要销审选中数据吗?', function () {
							ajaxPostData({
		         			    url: 'auditBudgMatPurAdj.do?isCheck=false',
		         			    data: {ParamVo : ParamVo.toString()},
		         			    success: function (responseData) {
		         			    	query();
		         			    }
			...
			if (data.length == 0) {
				...
				ajaxPostData({
     			    ...
     			    success: function (responseData) {
     			    	$.etDialog.confirm('确定要销审选中数据吗?', function () {
							ajaxPostData({
		         			    url: 'unAuditBudgMatPurAdj.do?isCheck=false',
		         			    data: {ParamVo : ParamVo.toString()},
		         			    success: function (responseData) {
		         			    	query();
		         			    }
		...
		function down(adj_file) {
			location.href = "downMatPurAdjFile.do?isCheck=false&adj_file="+adj_file;
	 	}
		//调整前预算查看
	    function lookBeforAdjustInfo(obj) {
	        ...
	        parent.$.etDialog.open({
	            url: 'hrp/budg/budgpur/mat/matpuradj/adjLinkPage.do?isCheck=false&' + parm,
	            title: '调整前预算查看',
	            isMax: true,
	            frameName: window.name, //用于parent弹出层调用本页面的方法或变量
	        });
