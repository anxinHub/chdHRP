...
<script type="text/javascript">
	...
	function query() {
		...
		//加载查询条件
		grid.loadData(parms, 'queryBudgMatPurCheck.do?isCheck=false');
	}
	function loadHead() {
		grid = $("#maingrid").etGrid({
			columns: [
				...
				{
					display: '预算下达日期', name: 'issue_data', align: 'left',
				}
			],
			dataModel: {
				method: 'POST',
	...
	function add_open() {
        ...
        ajaxPostData({
            url: "../../../common/budgeditstate/queryMatPurFlag.do?isCheck=false&year=" + budg_year,
            success: function(responseData) {
                if (responseData.flag == "1") {
                    parent.$.etDialog.open({
                        url: 'hrp/budg/budgpur/mat/matpurcheck/budgMatPurCheckAddPage.do?isCheck=false',
                        title: '材料采购预算审批申请',
                        isMax: true,
	...
	function remove() {
		...
		if (data.length == 0) {
            ...
            //校验 选中数据状态
            ajaxPostData({
                url: "queryBudgMatPurCheckState.do?isCheck=false",
                data: {
                    budg_year: budg_year,
	...
	function imp() {
		var index = layer.open({
			...
			area: ['893px', '500px'],
			content: 'budgMatPurCheckImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
	//下载临时模板
    ...
    function openUpdate(obj) {
        ...
        parent.$.etDialog.open({
            url: 'hrp/budg/budgpur/mat/matpurcheck/budgMatPurCheckUpdatePage.do?isCheck=false&' + parm,
            title: '材料采购预算审批申请修改',
            isMax: true,
