...
<script type="text/javascript">
    ...
    function init(){
    	...
    	acc_subj_code2 = $("#acc_subj_code2").etSelect({
    		//url:"../../../queryAccSubj.do?isCheck=false&subj_type=04",
    		defaultValue: "none",
    		onChange: query
    	});
    	...
    	function loadHeadGrid(value){
        	/* 	subj_name_select.reload({
        		 ...
        		 grid.getColumns()[2].editor.url = 'queryBudgIncomeTypeSet.do?isCheck=false&budg_year='+value,
        	grid.getColumns()[4].editor.url = 'queryAccSubj.do?isCheck=false&acc_year='+value+'&subj_type='+ '04'
        	}
    }
    ...
    function  query(){
    	...
    	//加载查询条件
    	grid.loadData(search,"queryBudgAccSubjIncomeShip.do?isCheck=false");
     }
    
    function loadHead(){
    	grid = $("#maingrid").etGrid({
           columns: [ 
        	/*    {display: '预算年度', name: 'budg_year', align: 'center',width:"17%",editable:setEdit,
					 editor: {
		 				 keyField:'budg_year',
	               	     type: 'select',  //编辑框为下拉框时
	               	     url: '../../../queryBudgYear.do?isCheck=false',      //  动态数据接口
	               	     change:function(rowdata,celldata){
	                   		 grid.updateRow(celldata.rowIndx,{subj_code:"",subj_name:"",acc_subj_code2:"",acc_subj_name:""})
	                   	 }
               	 	 },
				}, */
				...
				{display: '预算科目名称', name: 'subj_name', align: 'left',width:"20%",
					editor: {
		 				 keyField:'subj_code',
	               	     ...
	               	     url: 'queryBudgIncomeTypeSet.do?isCheck=false&budg_year='+year_input.getValue(),
	               	     change:function(rowdata,celldata){
				 ...
				 { display: '会计科目名称', name: 'acc_subj_name', align: 'left', width: "20%",
					editor:{
		 				type:'select' ,
		 				keyField:'acc_subj_code2',
		 				url:'queryAccSubj.do?isCheck=false&acc_year='+year_input.getValue()+'&subj_type='+ '04',
		     ...
		     dataModel:{
		   	 ...
		   	 toolbar: {
	             items: [
					...
					/*  { type: "button", label:'请先维护预算科目和会计科目字典'}, */
	           	 ]
           	 }
        });
    }
    ...
    function save() {
		...
		if(data.addList.length > 0 || data.updateList.length > 0){
			...
			ajaxPostData({
				url: "saveBudgAccSubjIncomeShip.do?isCheck=false",
        ...
        if (data.length == 0){
            ...
            if(ParamVo.length > 0){
            	$.etDialog.confirm('确定删除?', function () {
					ajaxPostData({
						url: "deleteBudgAccSubjIncomeShip.do?isCheck=false",
						data: { ParamVo: ParamVo.toString() },
						success: function () {
							query();
						}
    ...
    function imp() {
		var index = layer.open({
			...
			area: ['893px', '500px'],
			content: 'budgAccSubjIncomeShipImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
  ...
  //打印数据
	function printDate(){
    	...
    	$.each(grid.options.parms,function(i,obj){
   		...
   		var printPara={
   			headCount:2,
   			title:"收入预算科目与会计科目对应关系",
   			type:3,
   			columns:grid.getColumns(1)
   		};
   		ajaxJsonObjectByUrl("queryBudgAccSubjIncomeShip.do?isCheck=false", selPara, function(responseData) {
	...
	<div class="main">
		<table class="table-layout">
			<tr>
				...
				<td class="label" style="width:100px">收入预算科目：</td>
				<td class="ipt">
