...
<script type="text/javascript">
     ...
     $(function (){
        ...
        $("body").keydown(function() {
            if (event.keyCode == "9") {//keyCode=13是Tab键
            	grid.addRowEdited({
                	type_code : '',
                	subj_code: ''
        		 });
			     ...
			     columns : [ 
							{display : '科目类别名称', name : 'type_code', textField : 'type_name', minWidth : 80, align : 'center',
									editor : {
										...
										textField : 'type_name',
										url : 'queryBudgIncomeSubjType.do?isCheck=false',
										keySupport : true,
										autocomplete : true,
									}
								},
							{display : '收入预算科目名称', name : 'subj_code', textField : 'subj_name', minWidth : 80, align : 'center',
									editor : {
										type : 'select',
										valueField : 'subj_code',
										textField : 'subj_name',
										url : 'queryBudgIncomeSubj.do?isCheck=false&budg_year='+budg_year,
										keySupport : true,
										autocomplete : true,
									}
								}
     ...
     function  save(){
    	 ...
    	 if (data.length == 0){
	        ...
	        ajaxJsonObjectByUrl("addBudgIncomeTypeSet.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
    ...
    function validateGrid(data) {  
 		...
 		$.each(data,function(i, v){
			...
			if(targetMap.get(key)== null || targetMap.get(key) == 'undefined' || targetMap.get(key) == ""){
				...
				msg += targetMap.get(key)+"与"+value+"科目类别名称 、收入预算科目名称 数据重复,请删除重复数据后再操作" + "\n\r";
			}
 		});
 		if(msg != ""){
