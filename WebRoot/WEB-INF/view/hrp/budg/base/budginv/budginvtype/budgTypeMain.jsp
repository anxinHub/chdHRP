...
<script type="text/javascript">
    ...
    function loadTree(){
		ajaxJsonObjectByUrl("queryBudgTypeByTree.do?isCheck=false", {is_stop:'0'},
    ...
    function  query(tid){
    	...
    	ajaxJsonObjectByUrl("queryBudgTypeById.do?isCheck=false", paras, function (responseData){
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [{
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryBudgTypeDict.do?isCheck=false',
		...
		if (mat_type_id == ""){
        	...
        	$.ligerDialog.confirm('确定删除?', function (yes){
        		if(yes){
        			...
        			ajaxJsonObjectByUrl("deleteBudgType.do?isCheck=false", paras, function (responseData){
    ...
    function save(){
		if($("form").valid()){
			...
			ajaxJsonObjectByUrl("addBudgType.do?isCheck=false",formPara,function (responseData){
        		if(responseData.state=="true"){
        			loadTree();
    ...
    function openChange(obj){
		...
		$.ligerDialog.open({
			...
			height: 300,
			width: 800,
			url: 'budgTypeDictPage.do?isCheck=false&' + paras.toString(),
    ...
    function imp(){
    	var index = layer.open({
					...
					area : [ '750px', '370px' ],
					content : 'budgTypeImportPage.do?isCheck=false'
				});
				/* layer.full(index); */
    	}
	...
	//打印数据
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryBudgType.do",exportPara,function (responseData){
