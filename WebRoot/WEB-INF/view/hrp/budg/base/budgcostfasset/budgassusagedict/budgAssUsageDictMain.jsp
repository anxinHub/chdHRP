...
<script type="text/javascript">
    ...
    $(function ()
    {
        ...
        $("#equi_usage_code").ligerTextBox({width:160});
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	  grid.options.parms.push({name:'equi_usage_code',value:$("#equi_usage_code").val()}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '用途编码', name: 'equi_usage_code', align: 'left',
                    	 render : function(rowdata, rowindex,
  								value) {
 								
 								return "<a href=javascript:openUpdate('" + rowdata.group_id   + "|" + rowdata.hos_id   + "|" + rowdata.copy_code   + "|"  + rowdata.equi_usage_code +"')>"+rowdata.equi_usage_code+"</a>";

                     	 }
					 		},
					 		
                     { display: '用途名称', name: 'equi_usage_name', align: 'left'
					 		},
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryBudgAssUsageDict.do',
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteBudgAssUsageDict.do",{ParamVo : ParamVo.toString()},function (responseData){
                    ...
                    return;
                case "import":
                	$.ligerDialog.open({url: 'budgAssUsageDictImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
...
// 				 var trHtml="<tr>";
// 					 trHtml+="<td>"+item.equi_usage_code+"</td>"; 
// 					 trHtml+="<td>"+item.equi_usage_name+"</td>"; 
// 					 if (item.is_stop==0){
//                     	 trHtml+="<td>否</td>"; 
...
// 	 }
	    function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryBudgAssUsageDict.do?isCheck=false", selPara, function (responseData) {
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryBudgAssUsageDict.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.equi_usage_code+"</td>"; 
					 trHtml+="<td>"+item.equi_usage_name+"</td>"; 
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div id="resultPrint" style="display:none">
	   	<table width="100%">
			   	<thead>
				<tr>
                <th width="200">用途编码</th>	
                <th width="200">用途名称</th>	
