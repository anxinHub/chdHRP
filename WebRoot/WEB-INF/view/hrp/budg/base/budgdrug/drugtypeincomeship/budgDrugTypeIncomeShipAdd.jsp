  ...
  <head>
    ...
    <script type="text/javascript">
     ...
     function loadHead(){
	     grid = $("#maingrid").ligerGrid({ 
			     columns : [ 
							{display : '收入预算科目', name : 'subj_code', textField : 'text', minWidth : 80, align : 'center',
								editor : {
									type : 'select',
									valueField : 'id',
									textField : 'text',
							...
							{display : '药品分类', name : 'drug_type_code', textField : 'drug_type_name', minWidth : 80, align : 'center',
									editor : {
										type : 'select',
										valueField : 'drug_type_code',
										textField : 'drug_type_name',
										url : '../drugtypecostship/queryDrugType.do?isCheck=false',
										keySupport : true,
										autocomplete : true,
									}
								}
     ...
     function  save(){
    	 ...
    	 if (data.length == 0){
	        ...
	        ajaxJsonObjectByUrl("addBudgDrugTypeIncomeShip.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
    ...
    function validateGrid(data) {  
 		...
 		$.each(data,function(i, v){
 			rowm = "";
			if (v.drug_type_code == "" || v.drug_type_code == null || v.drug_type_code == 'undefined') {
				rowm+="[药品分类]、";
			}  
			...
			if(targetMap.get(key)== null || targetMap.get(key) == 'undefined' || targetMap.get(key) == ""){
				...
				msg += targetMap.get(key)+"与"+value+"收入预算科目相同 ,多个药品分类不允许对应同一收入预算科目!" + "\n\r";
			}
 		});
 		if(msg != ""){
