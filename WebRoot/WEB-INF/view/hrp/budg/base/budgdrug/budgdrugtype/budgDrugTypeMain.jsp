...
<script type="text/javascript">
    ...
    function loadTree(){
		ajaxJsonObjectByUrl("queryBudgDrugTypeByTree.do?isCheck=false", {},
    ...
    function  query(tid){
    	...
    	$("#med_type_code").prop("disabled", true);
    	$("#med_type_name").prop("disabled", true);
    	$("#med_type_code").ligerTextBox({width:180,disabled:true,cancelable: false});
    	$("#med_type_name").ligerTextBox({width:180,disabled:true,cancelable: false});
    	//主表查询
    	paras = {
    		"med_type_id" : tid
    	};
    	ajaxJsonObjectByUrl("queryBudgDrugTypeById.do?isCheck=false", paras, function (responseData){
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [{
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryBudgDrugTypeDict.do?isCheck=false',
    ...
    function add_mt(){
		...
		$("#med_type_code").ligerTextBox({width:180,disabled:false});
    	$("#med_type_name").ligerTextBox({width:180,disabled:false});
		...
		if (med_type_id == ""){
        	...
        	$.ligerDialog.confirm('确定删除?', function (yes){
        		if(yes){
        			...
        			ajaxJsonObjectByUrl("deleteBudgDrugType.do?isCheck=false", paras, function (responseData){
    ...
    function save(){
		if($("form").valid()){
			...
			ajaxJsonObjectByUrl("saveBudgDrugType.do?isCheck=false",formPara,function (responseData){
        		if(responseData.state=="true"){
        			loadTree();
    ...
    function openChange(obj){
		...
		$.ligerDialog.open({
			...
			height: 400,
			width: 800,
			url: 'budgDrugTypeDictPage.do?isCheck=false&' + paras.toString(),
    ...
    function loadDict(){
    	...
    	autoCompleteByData("#is_auto_exp",yes_or_no.Rows, "id", "text", true, true, "", true);

    	$("#med_type_code").ligerTextBox({width:180,disabled:true,cancelable: false});
    	$("#med_type_name").ligerTextBox({width:180,disabled:true,cancelable: false});
    	$("#spell_code").ligerTextBox({width:180,disabled:true,cancelable: false});
    	$("#wbx_code").ligerTextBox({width:180,disabled:true,cancelable: false});
	}
    //键盘事件
    ...
    function imp(){
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'budgDrugTypeImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}
