...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '预算年度', name: 'budg_year', align: 'left',
                    	 	render : function(rowdata, rowindex,value) {
								return "<a href=javascript:openUpdate('"+rowdata.group_id+"|"+ rowdata.hos_id+"|"
									+rowdata.copy_code+"|"+rowdata.budg_year+ "|" + rowdata.wage_item_code +"')>"+rowdata.budg_year+"</a>";
							}
					 	},
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryBudgWageItemCostShip.do?isCheck=false',
    ...
    function add_open(){
		...
		$.ligerDialog.open({ url : 'budgWageItemCostShipAddPage.do?isCheck=false',data:{}, height: 300,width: 450, 
				title:'工资项目与预算支出科目对应关系',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
				buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgWageItemCostShip();},
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteBudgWageItemCostShip.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'budgWageItemCostShipImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    function openUpdate(obj){
		...
		var vo = obj.split("|");
		var parm = "group_id="+vo[0] +"&hos_id="+vo[1] +"&copy_code="+vo[2] +"&budg_year="+vo[3] +"&wage_item_code="+vo[4] 
		 
		 
		 $.ligerDialog.open({ url : 'budgWageItemCostShipUpdatePage.do?isCheck=false&'+parm,data:{}, height: 300,width: 400, 
			title:'工资项目与预算支出科目对应关系',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgWageItemCostShip(); },
			cls:'l-dialog-btn-highlight' },{ text: '取消', onclick: function (item, dialog) { dialog.close();}}]
    	}); 
    ...
    /*var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'budgWageItemCostShipUpdatePage.do?isCheck=false&' + parm
				});
				layer.full(index);	
    */
    }
	...
	function printDate(){
    	...
    	$.each(grid.options.parms,function(i,obj){
   		...
   		var printPara={
   			headCount:2,
   			title:"工资项目与预算支出科目对应关系",
   			type:3,
   			columns:grid.getColumns(1)
   		};
   		ajaxJsonObjectByUrl("queryBudgWageItemCostShip.do?isCheck=false", selPara, function(responseData) {
    ...
    function loadDict(){
        ...
        //工资项目下拉框
        autocomplete("#wage_item_code","../../../queryBudgWageItem.do?isCheck=false","id","text",true,true);
