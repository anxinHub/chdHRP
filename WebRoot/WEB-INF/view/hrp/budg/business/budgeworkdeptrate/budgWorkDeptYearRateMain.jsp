...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '部门ID', name: 'dept_id', align: 'left'
					 		},
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryBudgWorkDeptYearRate.do',
    ...
    function add_open(){
		...
		$.ligerDialog.open({ url : 'budgWorkDeptYearRateAddPage.do?isCheck=false&',data:{}, height: 300,width: 450, title:'科室年度业务概率预算',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.savebudgWorkDeptYearRate(); },cls:'l-dialog-btn-highlight' },
    	...
    	/*var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'budgWorkDeptYearRateAddPage.do?isCheck=false'
				});
				layer.full(index);
				*/
    	}
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteBudgWorkDeptYearRate.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'budgWorkDeptYearRateImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    function openUpdate(obj){
		...
		var parm = 
		 ...
		 $.ligerDialog.open({ url : 'budgWorkDeptYearRateUpdatePage.do?isCheck=false&',data:{}, height: 300,width: 450, title:'科室年度业务概率预算',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.savebudgWorkDeptYearRate(); },cls:'l-dialog-btn-highlight' },
			           { text: '取消', onclick: function (item, dialog) { dialog.close(); } } 
			          ]
    	}); 
    ...
    /*var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'budgWorkDeptYearRateUpdatePage.do?isCheck=false&' + parm
				});
				layer.full(index);	
    */
    }
