...
<html style="overflow:hidden;">
    ...
    var budg_year;    
     ...
     function loadHead(){
	     grid = $("#maingrid").ligerGrid({ 
			     columns : [ 
							...
							{display : '费用标准', name : 'charge_stan_code', align : 'left', valueField:"id",textField : 'text',
		                             editor : {
											type : 'select',
											valueField : 'id',
											textField : 'text',
     ...
     function  save(){
    	 ...
    	 if (data.length == 0){
	         ...
	         $(data).each(function (){					
	        	 ParamVo.push(					
	        		budg_year   +"@"+ 
					this.charge_stan_code  +"@"+this.text +"@"+
					(this.budg_value?this.budg_value:"") +"@"+(this.remark?this.remark:"-1"))
	         });
	        ajaxJsonObjectByUrl(" addBudgHosChargeStan.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
     ...
     function validateGrid(data) {  
   		...
   		$.each(data,function(i, v){
   			rowm = "";
  			if (v.charge_stan_code == "" || v.charge_stan_code == null || v.charge_stan_code == 'undefined') {
  				rowm+="[指标编码]、";
  			} 
  			if(rowm != ""){
 ...
 function loadForm(){
    ...
    function saveBudgHosChargeStan(){
        if($("form").valid()){
            save();
        }
   }
