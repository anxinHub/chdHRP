...
<script type="text/javascript">
	...
	var year_input, disease_code_select, insurance_code_select;
	...
	function query() {
		...
		//加载查询条件
		grid.loadData(parms,"queryBudgWorkHosDbz.do?isCheck=false");
	}

	function loadHead() {
		grid = $("#maingrid").etGrid({
			columns: [
				{display: '年度', name: 'year', align: 'left',width:"15%",editable:setEdit,
					editor: {
						type: 'select',
						keyField: 'year',
				...
				{display: '单病种编码', name: 'disease_code', align: 'left',width:"15%",editable:false},
				{display: '单病种名称', name: 'disease_name', align: 'left',width:"25%",editable:setEdit,
					editor: {
						type: 'select',
						keyField: 'disease_code',
						url: '../workdeptdbz/queryBudgSingleDC.do?isCheck=false',
						change : function(rowdata,celldata){
							var value = celldata.selected.value; 
							grid.updateRow(rowdata._rowIndx, {disease_code:value.split(" ")[0],
								insurance_name:"",insurance_code:""})
						}
					}
				},
				{display: '医保类型', name: 'insurance_name', align: 'left',width:"25%",editable:setEdit,
					editor: {
						...
						url:"../workdeptdbz/queryBudgYBTY.do?isCheck=false&disease_code="+disease_code,
					/* 	change : function(rowdata,celldata){
						...
						create:function(rowdata,celldata,setting){
							setting.url="../workdeptdbz/queryBudgYBTY.do?isCheck=false&disease_code="+rowdata.disease_code
						}
					}
				},
				{display: '业务预算(E)', name: 'budg_workload', align: 'right',dataType: 'float',width:"17%",
					render:function(ui){
	...
	function impNew() {
		...
		importSpreadView("hrp/budg/business/compilationbasic/workhosdbz/budgWorkHosDbzImportNewPage.do?isCheck=false", para);
	}

	function add_open() {
		$.etDialog.open({
			url: 'budgWorkHosDbzAddPage.do?isCheck=false',
			height: 300, width: 450, title: '医院单病种业务预算', 
			btn: ['确定', '取消'],
		...
		if (data.length == 0) {
			...
			$.etDialog.confirm('删除操作会刷新页面,页面未保存数据可能丢失！确定删除?', function (yes) {
				if (yes) {
					if (ParamVo.length > 0) {
						ajaxPostData({
		                    url: "deleteBudgWorkHosDbz.do?isCheck=false",
	...
	function imp() {
		var index = layer.open({
			...
			area: ['893px', '500px'],
			content: 'budgWorkHosDbzImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
	...
	function openUpdate(obj) {
		...
		$.etDialog.open({
			url: 'budgWorkHosDbzUpdatePage.do?isCheck=false',
			height: 300, width: 450, title: '医院单病种业务预算', 
			btn: ['确定', '取消'],
            btn1: function(index, el) {
                var iframeWindow = window[el.find('iframe').get(0).name];
	...
	function save() {
		...
		if( data.addList.length > 0 || data.updateList.length > 0 ){
    		if(data.addList.length > 0){
 			...
 			ajaxPostData({
                url: "saveBudgWorkHosDbz.do?isCheck=false",
                data: { ParamVo: ParamVo.toString() },
                success: function(responseData) {
                	query();
                }
	...
	function validateGrid(data) {  
    	var msg="";
 		...
 		$.each(data,function(i, v){
			...
			msg += rowm;
			var key=v.year + v.disease_code + v.insurance_code 
	...
	function lodopPrint() {
		...
		grid.options.lodop.title = "全院单病种业务预算";
	}
	...
	function collect() {
		...
		if (year) {
			ajaxPostData({
                url: "collectBudgWorkHosDbz.do?isCheck=false&year=" + year,
                success: function(responseData) {
                	query();
                }
            });
