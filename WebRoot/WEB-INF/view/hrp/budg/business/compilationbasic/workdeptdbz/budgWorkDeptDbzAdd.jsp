  ...
  <head>
    ...
    function loadHead(){
     	grid = $("#maingrid").ligerGrid({
     		columns: [ 
                { display: '预算科室', name: 'dept_id', align: 'left',valueField : 'dept_code',textField : 'dept_name',
	                   editor : {
						type : 'select',
                ...
                { display: '医保类型', name: 'insurance_code', align: 'left',valueField : 'idYBTY',
							textField : 'textYBTY',
				 			 editor : {
									...
									valueField : 'idYBTY',
									textField : 'textYBTY',
									url : 'queryBudgYBTP.do?isCheck=false&disease_code='+disease_code,
									keySupport : true,
									autocomplete : true,
								} 
				 		},
                { display: '业务预算(E)', name: 'budg_workload', align: 'right',editor :{type : 'float'},
                ...
                toolbar: { 
                	items: [
       						{ text: '保存', id:'add', click: saveBudgWorkDeptDbz , icon:'add' },
       	                	{ line:true },
     ...
     function  save(){
    	var data = gridManager.getData();
   	    budg_year = liger.get("budg_year").getValue();
	        ...
	        ajaxJsonObjectByUrl("addBudgWorkDeptDbz.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
