     ...
     function loadHead(){
     	grid = $("#maingrid").ligerGrid({
            columns: [ 
                      ...
                      { display: '科室医保额度(E)', name: 'dept_control_limit', align: 'right',editor : {type : 'float'} ,
                      ...
                      dataAction: 'server',dataType: 'server',checkbox: true,usePager:true,
	                    ...
	                    toolbar: { 
	                    	items: [
             						{ text: '保存', id:'add', click: saveBudgDeptYbLimit , icon:'add' },
             	                	{ line:true },
    	 ...
    	 if (data.length == 0){
	         ...
	         $(data).each(function (){					
	        	 ParamVo.push(					
					...
					this.insurance_code  +"@"+
					this.textYB +"@"+
					(this.dept_control_limit?this.dept_control_limit:0) +"@"+
					(this.remark?this.remark:"-1")				
					)
	         });
	         
	        ajaxJsonObjectByUrl("addBudgDeptYbLimit.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
