...
<script type="text/javascript">
	...
	function init(){
		...
		index_code_select=$("#index_code_select").etSelect({
			url:"../../../queryBudgDeptindex_code_name.do?isCheck=false&index_nature=01",
			defaultValue:"none",
			onChange:query
		})
	}
	...
	function query() {
		var parms = [
			...
			{ name: 'index_nature', value: "01" }
		];
		//加载查询条件
		grid.loadData(parms,"queryBudgHosBasicIndexData.do?isCheck=false");
	}
	function loadHead() {
			...
			columns: [
				...
				{display: '指标名称', name: 'index_name', align: 'left', width: "20%",editable:setEdit,
					editor: {
						...
						valueField: 'idDC',
						textField: 'textDC',
						url: 'queryIndex.do?isCheck=false',
						change:function(rowdata,celldata){
                    		 
                    		 grid.updateRow(celldata.rowIndx,{index_code:rowdata.idDC.split(",")[0]})
                    		 
                    	}
					}
				},
				{display: '指标值(E)', name: 'budg_value', align: 'right', width: "15%", dataType:"float",
					render:function(ui){
	...
	function add_open() {
		$.ligerDialog.open({
			url: 'budgHosBasicIndexDataAddPage.do?isCheck=false',
			height: 300, width: 450, title: '医院基本指标数据维护', 
			btn: ['确定', '取消'],
	        btn1: function(index, el) {
                var iframeWindow = window[el.find('iframe').get(0).name];
                iframeWindow.saveBudgHosBasicIndexData()
	        }
		});
	}
		...
		if (data.length == 0) {
			...
			$.etDialog.confirm('删除操作会刷新页面,页面未保存数据可能丢失！确定删除?', function (yes) {
				if (yes) {
					if (ParamVo.length > 0) {
						ajaxPostData({
		                    url: "deleteBudgHosBasicIndexData.do?isCheck=false",
	...
	function impNew() {
		parent.$.etDialog.open({
			url: 'hrp/budg/business/compilationbasic/hosindex/budgHosBasicIndexImportPage.do?isCheck=false',
			height: 300, width: 450, title: '医院基本指标数据维护导入', 
	...
	function imp() {
		...
		var index = layer.open({
			...
			area: ['893px', '500px'],
			content: 'budgHosBasicIndexDataImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
	...
	function openUpdate(obj) {
		...
		var parm =
		 ...
		 $.etDialog.open({
            url: 'budgHosBasicIndexDataUpdatePage.do?isCheck=false',
            height: 600,
            width: 1100,
            title: '医院基本指标数据维护',
            btn: ['确定', '取消'],
	...
	function save() {
		...
		if( data.addList.length > 0 || data.updateList.length > 0 ){
    		if(data.addList.length > 0){
 			...
 			ajaxPostData({
                url: "saveBudgHosBasicIndexData.do?isCheck=false",
                data: { ParamVo: ParamVo.toString() },
                success: function(responseData) {
                	query();
                }
	...
	function collect() {
		...
		ajaxPostData({
            url: "collectBudgHosBasicIndexData.do?isCheck=false&year=" + year,
            success: function(responseData) {
            	query();
            }
        });
	...
	function lodopPrint() {
		...
		grid.options.lodop.title = "医院基本指标数据维护";
	}
