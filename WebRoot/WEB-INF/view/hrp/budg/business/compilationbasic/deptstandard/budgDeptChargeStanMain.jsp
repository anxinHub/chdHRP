...
<script type="text/javascript">
	...
	var year_input,charge_stan_select,dept_name_select;
	...
	function init(){
		...
		charge_stan_select = $("#charge_stan_select").etSelect({
			url:"../../../queryBudgChargeStan.do?isCheck=false&charge_stan_nature=02",
			defaultValue:"none",
			onChange:query
		});
	...
	function query() {
		...
		//加载查询条件
		grid.loadData(parms,"queryBudgDeptChargeStan.do?isCheck=false");
	}
	...
	function loadHead() {
		grid = $("#maingrid").etGrid({
			columns: [
				...
				{display: '指标编码', name: 'charge_stan_code', align: 'left', width:"10%",editable:false},
				{display: '指标名称', name: 'charge_stan_name', align: 'left', width: "15%",editable:setEdit,
					editor: {
						type: 'select',
	...
	function add_open() {
		$.etDialog.open({
			...
			height: 300, width: 450, title: '科室费用标准维护', 
			btn: ['确定', '取消'],
            btn1: function(index, el) {
                var iframeWindow = window[el.find('iframe').get(0).name];
                iframeWindow.savebudgDeptChargeStan()
            }
		});
	}
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function (yes) {
				if (yes) {
					if (ParamVo.length > 0) {
						ajaxPostData({
		                    url: "deleteBudgDeptChargeStan.do?isCheck=false",
	...
	function impNew() {
		parent.$.etDialog.open({
			url: 'hrp/budg/business/compilationbasic/deptstandard/budgDeptChargeStanImportNewPage.do?isCheck=false',
			height: 300, width: 450, title: '科室费用标准维护',
	...
	function imp() {
		var index = layer.open({
			...
			area: ['893px', '500px'],
			content: 'budgDeptChargeStanImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
	...
	function openUpdate(obj) {
		...
		$.etDialog.open({
			url: 'budgDeptChargeStanUpdatePage.do?isCheck=falseparm=' + parm,
			height: 300, width: 450, title: '科室费用标准维护', 
			btn: ['确定', '取消'],
            btn1: function(index, el) {
                var iframeWindow = window[el.find('iframe').get(0).name];
	...
	function save() {
		...
		if( data.addList.length > 0 || data.updateList.length > 0 ){
    		if(data.addList.length > 0){
 			...
 			ajaxPostData({
                url: "saveBudgDeptChargeStan.do?isCheck=false",
                data: { ParamVo: ParamVo.toString() },
                success: function(responseData) {
                	query();
                }
	...
	function validateGrid(data) {  
    	var msg="";
 		...
 		$.each(data,function(i, v){
			...
			msg += rowm;
			var key=v.year + v.charge_stan_code + v.dept_id
	...
	function collect() {
		...
		ajaxPostData({
            url: "collectBudgDeptChargeStan.do?isCheck=false&year=" + year,
            success: function(responseData) {
            	query();
            }
        });
