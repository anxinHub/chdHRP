...
<script type="text/javascript">
    ...
    function loadHead() {
        var columns = [
            { display: '科室编码', name: 'dept_code', width: "20%" },
            { display: '科室名称', name: 'dept_name', width: "20%" },
            { display: '部门分类', name: 'type_name', width: "20%" },
            { display: '部门类型', name: 'kind_name', width: "20%" },
            { display: '部门性质', name: 'natur_name', width: "18%" }
        ];
    ...
    function loadDict () {
        ajaxPostData({
            ...
            success: function (data) {
                budg_year = $("#budg_year").etDatepicker({
                    ...
                    maxDate: data[0].text,
                    defaultDate: '${budgExpFixAmount.budg_year}',
                    onChanged: query
                })
            }
        })
        payment_item_id = $("#payment_item_id").etSelect({
            ...
            defaultValue: '${budgExpFixAmount.payment_item_id}.${budgExpFixAmount.payment_item_no}',
            onChanege: query
        })
    }
    ...
    <div class="mian">
        <table class="table-layout">
            <tr>
                ...
                <td class="ipt">
                    <input id="fix_amount" type="text" value="${budgExpFixAmount.fix_amount}" />
                </td>
            </tr>
        </table>
    </div>
