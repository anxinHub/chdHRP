...
<head>
        ...
        var paramObj = {
            ...
            toolbar: {
                items: [
                    ...
                    { type: 'button', label: '生成（<u>G</u>）', listeners: [{ click: generate }], icon: 'settle' },
    ...
    function add_open() {
        $.etDialog.open({
            url: 'budgExpenseAddPage.do?isCheck=false&',
            height: 450,
            width: 450,
            title: '科室其他费用预算编制添加',
            btn: ['确定', '取消'],
    ...
    function generate() {
        ...
        $.etDialog.confirm('生成功能产生的新数据将会将覆盖原有数据,确定生成?', function(yes) {
            ajaxPostData({
            	url: "generateBudgExpense.do?isCheck=false",
            	data: formPara,
            	success: function(responseData) {
                    queryNew();
                }
    ...
    function imp(){
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'budgExpenseImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    function queryNew() {
        ...
        grid.loadData(search, "queryBudgExpense.do");
    }
    //键盘事件
