...
<html style="overflow:hidden;">
  <head>
    ...
    <script type="text/javascript">
     ...
     $(function (){
		...
		$.post("queryBudgBuiltIn.do?isCheck=false&budg_level=02&work_budg_edit_mode=02",null,function(responseData){
        	
        	var para = eval("("+responseData+")") ;
        	
        	is_single_count = para.is_single_count ;
        	
        	liger.get("is_single_count").setValue(is_single_count);
        	
        	if(para.resolve_level){
        		resolve_level = para.resolve_level ;
        	}else{
        		resolve_level = '';
        	}
        	
        	if(para.sum_level){
        		sum_level = para.sum_level ;
        	}else{
        		sum_level = '';
        	}
        	
        	if(para.resolve_or_sum){
        		resolve_or_sum = para.resolve_or_sum ;
        		liger.get("resolve_or_sum").setValue(resolve_or_sum);
        	}else{
        		resolve_or_sum = '';
        	}
     ...
     // 是否独立核算，若是则编辑编制方法、取值方法、计算公式、取值函数，否则只能编辑 “分解&汇总”部分 
       ...
       $("#is_single_count").change(function(){
            ...
            if(liger.get("is_single_count").getValue() == 0 ){
    			...
    			liger.get("resolve_or_sum").setValue(resolve_or_sum);
           		liger.get("resolve_level").setValue(resolve_level);
        	 ...
        	 if(liger.get("is_single_count").getValue() == 0){
        		 ...
        		 if(liger.get("resolve_or_sum").getValue() =='01'){
            		 ...
            		 $("#resolve_way").removeClass("notValid");
               		 ...
               		 liger.get("resolve_way").setValue(resolve_way);
              		 liger.get("resolve_level").setValue(resolve_level);
               		 liger.get("sum_level").setValue("");
     ...
     function  save(){
        ...
        ajaxJsonObjectByUrl("addBudgHosMonthPlanDown.do?isCheck=fasle",formPara,function(responseData){
            ...
            if(responseData.state=="true"){
				/*  $("input[name='budg_year']").val('');
				 ...
				 $("input[name='resolve_level']").val('');
				 $("input[name='is_single_count']").val(is_single_count); */
     ...
     function selectFormula(){
   	...
   	function selectFun(){
 		...
 		if(!budg_year){
   		...
   		parent.$.ligerDialog.open({
   			url : 'hrp/budg/common/budgfun/budgFunBusinessPage.do?isCheck=false&budg_year='+budg_year+'&budg_level='+'02'+'&index_code='+index_code+'&index_type_code='+index_type_code,height : 500,width : 750,title : '取值函数选择',
   			modal : true,showToggle : false,showMax : true,showMin :false ,	isResize : true,
   			parentframename:window.name,  //用于parent弹出层调用本页面的方法或变量
  ...
  //选择自定义分解系数页面 
   	function selectResolve(){
   		...
   		parent.$.ligerDialog.open({
  			url : 'hrp/budg/common/budgresolvedatadict/budgResolveDataPage.do?isCheck=false&budg_level=02',
    ...
    function loadDict(){
        ...
        //预算指标下拉框
        autocomplete("#index_code","../../../../queryBudgIndexDict.do?isCheck=false","id","text",true,true,'',false,'',180);
        ...
        autocomplete("#resolve_way","../../../../queryBudgResolveWay.do?isCheck=false","id","text",true,true,'',false,'',180);
	 	...
	 	//分解层次 下拉框
	   	autocomplete("#resolve_level","../../../../queryBudgLevel.do?isCheck=false","id","text",true,true,'',false,'',180);
	 	
		  //自定义分解系数 下拉框
        autocomplete("#resolve_data","../../../../queryBudgResolveDataDict.do?isCheck=false&budg_level=02","id","text",true,true,'',false,'',180);

	  //参考年度
	   //autocomplete("#reference_years","../../../../queryBudgYear.do?isCheck=false","id","text",true,true,'',false,'',180);
	 	
	  	//汇总层次 下拉框
	   	autocomplete("#sum_level","../../../../queryBudgLevel.do?isCheck=false","id","text",true,true,'',false,'',180);
        
        autoCompleteByData("#is_single_count", yes_or_no.Rows, "id", "text", true, true,'',true,'',false,'',180);
        
        $("#budg_year").ligerTextBox({ width:180});
        
    	$("#index_code").ligerTextBox({ width:180});
   ...
   <form name="form" method="post"  id="form" >
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="formula_id" type="text" id="formula_id" disabled="disabled" ltype="text" validate="{required:true,maxlength:20}" /></td> 
            <td align="left"></td>
        ...
        <tr> 
            ...
            <td align="left" class="l-table-edit-td"><input name="fun_id" type="text" id="fun_id"  disabled="disabled" ltype="text" validate="{required:true,maxlength:20}" /></td>
            <td align="left"></td>
            <td align="center" class="l-table-edit-td" colspan="3">
        ...
        <tr>
         ...
         <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="resolve_level" type="text" id="resolve_level" disabled="disabled" ltype="text" validate="{required:true,maxlength:20}" /></td> 
            <td align="left"></td>
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="sum_level" type="text" id="sum_level"  disabled="disabled" ltype="text" validate="{required:true,maxlength:20}" /></td> 
            <td align="left"></td> 
        </tr>  
