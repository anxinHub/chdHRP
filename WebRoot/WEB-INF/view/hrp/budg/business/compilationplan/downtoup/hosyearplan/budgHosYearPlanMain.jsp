			...
			<script type="text/javascript">
				...
				function loadHead() {
					grid = $("#maingrid").ligerGrid({
						...
						dataAction: 'server', dataType: 'server', usePager: true, url: 'queryBudgHosYearPlanDown.do?isCheck=false&budg_level=01',
				...
				function add_open() {
					...
					if (budg_year) {
						...
						parent.$.ligerDialog.open({
							url: 'hrp/budg/business/compilationplan/downtoup/hosyearplan/budgHosYearPlanAddPage.do?isCheck=false&budg_year=' + budg_year, data: {}, height: 400, width: 700,
							title: '医院年度业务预算编制方案添加', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
				...
				function remove() {
					...
					if (data.length == 0) {
						...
						$.ligerDialog.confirm('确定删除?', function (yes) {
							if (yes) {
								ajaxJsonObjectByUrl("deleteBudgHosYearPlanDown.do?isCheck=false", { ParamVo: ParamVo.toString() }, function (responseData) {
				...
				function openUpdate(obj) {
					...
					parent.$.ligerDialog.open({
						url: 'hrp/budg/business/compilationplan/downtoup/hosyearplan/budgHosYearPlanUpdatePage.do?isCheck=false&' + parm, data: {}, height: 400, width: 700,
						title: '医院年度业务预算编制方案修改', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
						parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
						buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgHosYearPlan(); }, cls: 'l-dialog-btn-highlight' },
						{ text: '取消', onclick: function (item, dialog) { dialog.close(); } }
						]
