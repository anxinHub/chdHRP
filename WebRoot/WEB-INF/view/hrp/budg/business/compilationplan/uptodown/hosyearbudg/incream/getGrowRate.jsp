   ...
   //查询
     ...
     function loadHead(){
     	grid = $("#maingrid").etGrid({
            columns: [ 
                     { display: '指标编码', name: 'index_code', align: 'center',width:'30%',
 					 		},
 				 	 { display: '指标名称', name: 'index_name', align: 'center',width:'30%',
 					 		},
 			...
 			$.etDialog.error('请选择行');
 	          ...
 	          $(data).each(function (){
             	if(this.rowData.grow_rate){
             		...
             		err += this.rowData.index_code + this.rowData.index_name +"、"
             	}
 	          })
 	          
 	         if(err != ''){
 	        	 $.etDialog.error('以下指标指标数据:【'+err.substring(0,err.length-1)+'】增长比例不存在!');
 	         }else{
 	        	 ajaxPostData({
   ...
   <div class="main">
		<table class="table-layout">
			<tr>
				...
				<td class="ipt">
					<input type="text" name="index_code" id="end_year" disabled="disabled"  value="${end_year}" validate="{required:true,maxlength:20}" />
