...
<head>
	...
	<script type="text/javascript">
		...
		function query() {
			...
			//加载查询条件
			grid.loadData(parms, 'queryBudgWorkHosMonthUp.do?isCheck=false');
		}
			...
			grid = $("#maingrid").etGrid({
				...
				toolbar: {
					items: [
						...
						/* { type: "button", label: '分解计算', icon: 'arrow-4', listeners: [{ click: resolveData}] }, */
		...
		function add_open() {
			...
			$.ligerDialog.open({
				url: 'budgWorkHosMonthAddPage.do?isCheck=false', data: {}, height: 300, width: 600,
				title: '医院月份业务预算添加', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
				buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgWorkHosMonth(); }, cls: 'l-dialog-btn-highlight' },
			...
			if (data.length == 0) {
				...
				$.etDialog.confirm('确定删除?', function (yes) {
					if (yes) {
						if (ParamVo.length > 0) {
							ajaxPostData({
	                            url: "deleteBudgWorkHosMonthUp.do?isCheck=false",
		...
		function openUpdate(obj) {
			...
			parent.$.ligerDialog.open({
				url: 'hrp/budg/business/compilationplan/uptodown/hosmonthbudg/budgWorkHosMonthUpdatePage.do?isCheck=false&' + parm, data: {}, height: 500, width: 700,
		...
		function save() {
			...
			if (data.updateList.length == 0) {
				...
				ajaxPostData({
                    url: "updateBudgWorkHosMonthUp.do?isCheck=false",
                    data: { ParamVo: ParamVo.toString() },
                    success: function(responseData) {
                    	query();
                    }
		...
		function collect() {
			...
			ajaxPostData({
                url: "collectBudgWorkHosMonthUp.do?isCheck=false&year=" + year,
                data: {},
                success: function(responseData) {
                	query();
                }
		...
		function resolveData() {
			...
			ajaxPostData({
                url: "resolveData.do?isCheck=false&year=" + year+"&index_code="+index_code, 
                data: {},
                success: function(responseData) {
                	query();
                }
