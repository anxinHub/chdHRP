 ...
 */
	...
	function query() {
		...
		//加载查询条件
		grid.loadData(parms,'queryBudgMatPurExe.do?isCheck=false');
	}
	...
	function loadHead() {
		grid = $("#maingrid").etGrid({
			columns: [
				...
				{ display: '物资分类', name: 'mat_type_name', align: 'left',width:"25%",editable:setEdit ,
			 			editor : {
							type : 'select',
				...
				{ display: '采购金额',name: 'pur_amount',align: 'right',dataType:"float",width:"25%",
						render:function(ui){
	...
	function  save(){
    	...
    	if(data.addList.length > 0 || data.updateList.length > 0){
    		...
    		if(data.updateList.length > 0){
	  	    ...
	  	    ajaxPostData({url: 'saveBudgMatPurExe.do?isCheck=false',
		...
		if (data.length == 0) {
			...
			$.etDialog.confirm('确定删除?', function (yes) {
				if (yes) {
					if(ParamVo.length > 0){
						ajaxPostData({
	         			    url: 'deleteBudgMatPurExe.do?isCheck=false',
	         			    data: {ParamVo : ParamVo.toString()},
	         			    success: function (responseData) {
	         			    	query();
	         			    }
