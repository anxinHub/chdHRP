...
<head>
	...
	<script type="text/javascript">
		...
		function loadHead() {
			grid = $("#maingrid").etGrid({
				columns: [
					...
					{ display: '物资分类', name: 'mat_type_name', align: 'left',width:"20%",editable:setEdit ,
				 			editor : {
								type : 'select',
								keyField : 'mat_type_id',
								url:"queryBudgMatTypeSubj.do?isCheck=false&budg_year="+budg_year,
								keySupport : true,
								autocomplete : true,
								change : queryCostBudg ,
								create:function(rowdata,celldata,setting){
		                	    	if(rowdata.year){
		                	    		setting.url = 'queryBudgMatTypeSubj.do?isCheck=false&budg_year='+rowdata.year;
		                	    	}else{
	    ...
	    // 预算年度 、 月份 、物资分类 变化后 查询其支出预算 并更采购预算   支出预算 = 采购预算
	    function queryCostBudg(rowdata,celldata){
    		if(rowdata.year && rowdata.monthId && rowdata.mat_type_id ){
	  	   		$.post("queryCostBudg.do?isCheck=false&year="+rowdata.year+"&month="
  	   				+rowdata.monthId+"&mat_type_id="+rowdata.mat_type_id,null,function(responseData){
		...
		function collect() {
			if(year_input.getValue()){
				...
				$.etDialog.confirm('生成将会覆盖已有数据,确定生成?', function (yes) {
					if (yes) {
						ajaxPostData({
	         			    url: 'generateBudgMatPur.do?isCheck=false',
	         			    data: formPara,
		...
		function  save(){
	    	...
	    	if(data.addList.length > 0 || data.updateList.length > 0){
	    		...
	    		if(data.updateList.length > 0){
		  	    ...
		  	    ajaxPostData({url: 'saveBudgMatPur.do?isCheck=false',
		...
		function validateGrid(data) {  
	    	var msg="";
	 		...
	 		$.each(data,function(i, v){
				...
				if (v.mat_type_id == "" || v.mat_type_id == null || v.mat_type_id == 'undefined') {
					rowm+="[物资类别]、";
				}  
				...
				msg += rowm;
				var key= v.year + v.month + v.mat_type_id
			...
			if (data.length == 0) {
				...
				$.etDialog.confirm('确定删除?', function (yes) {
					if (yes) {
						if(ParamVo.length > 0){
							ajaxPostData({
		         			    url: 'deleteBudgMatPur.do?isCheck=false',
		         			    data: {ParamVo : ParamVo.toString()},
		         			    success: function (responseData) {
		         			    	query();
		         			    }
