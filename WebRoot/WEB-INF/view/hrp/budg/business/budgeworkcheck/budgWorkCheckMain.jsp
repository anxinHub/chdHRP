...
<script type="text/javascript">
	...
	function query(){
		...
		//加载查询条件
		grid.loadData(parms,"queryBudgWorkCheck.do?isCheck=false");
	}
    	...
    	grid = $("#maingrid").etGrid({
           columns: [ 
               { display: '审批单号', name: 'check_code', align: 'left',width:120,
	              	  render:function(ui) {
						return '<a href=javascript:openUpdate("' + ui.rowData.budg_year + "|" + escape(ui.rowData.check_code) + '")>'+ui.rowData.check_code+'</a>';
					  }
			   	  },
    ...
    function lodopPrint(){
    	var head="";
 		...
 		grid.options.lodop.title="业务预算审批";
    }
    ...
    function add_open(){  
    	...
    	ajaxPostData({
            url: "../../common/budgeditstate/queryWorkFlag.do?isCheck=false&year="+budg_year,
            success: function(responseData) {
            	if(responseData.flag=="1"){
            		 parent.$.etDialog.open({
                         url: 'hrp/budg/business/budgeworkcheck/budgWorkCheckAddPage.do?isCheck=false',
                         title: '业务预算审批申请添加',
                         isMax: true,
    ...
    function remove(){
        ...
        if (data.length == 0){
            ...
            //校验 选中数据状态
            ajaxPostData({
                url: "queryBudgWorkCheckState.do?isCheck=false",
                data: {
                    budg_year: budg_year,
    ...
    function imp(){
    	var index = layer.open({
			...
			area : [ '893px', '500px' ],
			content : 'budgWorkCheckImportPage.do?isCheck=false'
		});
		layer.full(index);
    }	
    ...
    function openUpdate(obj) {
        ...
        parent.$.etDialog.open({
            url: 'hrp/budg/business/budgeworkcheck/budgWorkCheckUpdatePage.do?isCheck=false&' + parm,
            title: '业务预算审核申请修改',
            isMax: true,
            frameName: window.name
        });
