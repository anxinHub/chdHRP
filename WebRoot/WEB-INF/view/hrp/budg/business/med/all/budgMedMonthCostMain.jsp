...
<script type="text/javascript">
    ...
    var year_input, subj_level_select,state_select;
  //ajax获取数据
        ...
        var paramObj = {
            ...
            toolbar: {
                items: [
					...
					{ type: 'button', label: '科室汇总', listeners: [{ click: collect }]},
    ...
    function collect(){
		...
		//校验 预算年度数据状态
        ajaxPostData({
            url: "queryYearDeptDataExistNoCheck.do?isCheck=false&year="+year,
            data: {},
            success: function(responseData) {
                if (responseData.work_state == "false") {
                	$.etDialog.warn("该年度科室支出预算存在未确认数据，请先确认后再汇总！");
                    return;
                } else {
                	ajaxPostData({
                        url: "queryYearHosDataExist.do?isCheck=false&year="+year,
                        data: {},
                        success: function(responseData) {
                            if (responseData.work_state == "false") {
                            	
                            	$.etDialog.confirm("科室汇总会清空该年度医院支出预算，确认是否进行汇总！", function(yes) {
                        			if (yes) {
                        				var str ='确定确认汇总'+$("#year_input").val().toString()+'年度的支出预算?';
                                		$.etDialog.confirm(str, function(yes) {
                                			if (yes) {
                                				ajaxPostData({
                                					url: "collectMedMonthCost.do?isCheck=false&year="+year,
                                					data: {},
                                					success: function (res){
                                						if (res.state == "true") {
                                							query();
