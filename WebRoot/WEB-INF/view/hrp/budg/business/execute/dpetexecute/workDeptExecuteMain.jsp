...
<script type="text/javascript">
    ...
    function  query(){
		...
		//加载查询条件
		grid.loadData(parms, 'queryWorkDeptExecute.do');
    }
    ...
    function loadHead(){
    	grid = $("#maingrid").etGrid({
           columns: [ 
			   ...
			   { display: '月份', name: 'month',width:80,editable: false
					},
			   { display: '科室编码', name: 'dept_code',width:120,editable: false
					},
			   { display: '科室名称', name: 'dept_name',width:150,editable: false
					},
               { display: '指标编码', name: 'index_code',width:120,editable: false
					},
               { display: '指标名称', name: 'index_name',width:150,editable: false
					},
			   ...
			   { display: '说明(E)', name: 'remark',minWidth: 420,dataType: 'string',
					}
            ],
            dataMedol: {
	         	method: 'POST',
    ...
    function add_open(){
    	$.etDialog.open({
            url: 'workDeptExecuteAddPage.do?isCheck=false',
            height: 350,
            width: 700,
            title: '科室月份业务预算执行添加',
            btn: ['确定', '取消'],
    ...
    function save(){
		...
		ajaxPostData({
		    url: 'updateWorkDeptExecute.do?isCheck=false',
    ...
    function imp(){
    	var index = layer.open({
			...
			area : [ '893px', '500px' ],
			content : 'workDeptExecuteImportPage.do?isCheck=false'
		});
		layer.full(index);
    }	
	...
	function printData(){
    	...
    	var printPara={
            ...
            class_name: "com.chd.hrp.budg.service.business.compilationbasic.dpetexecute.BudgWorkDeptExecuteService",
            method_name: "getPrintData",
            bean_name: "budgWorkDeptExecuteService",
