...
<jsp:include page="${path}/static_resource.jsp">
    <jsp:param value="select,datepicker,grid,dialog,pageOffice" name="plugins" />
</jsp:include>
<script type="text/javascript">
    ...
    function  query(){
		...
		//加载查询条件
		grid.loadData(parms, 'queryWorkHosExecute.do');
    }
    ...
    function loadHead(){
    	grid = $("#maingrid").etGrid({
           columns: [ 
			   ...
			   { display: '月份', name: 'month', align: 'center',width:80,editable: false
					},
			   { display: '指标编码', name: 'index_code',width:120,editable: false
					},
			   { display: '指标名称', name: 'index_name',width:140,editable: false
					},
               { display: '指标值(E)', name: 'execute_value', align: 'right',width:180,dataType: "float",
			 			render:function(ui){
				...
				{ display: '说明(E)', name: 'remark',minWidth:450,dataType: "string"
				 	}
           ],
           dataModel: {
				method: 'POST',
    ...
    function add_open() {
        $.etDialog.open({
            url: 'workHosExecuteAddPage.do?isCheck=false',
            height: 300,
            width: 700,
            title: '医院月份业务预算执行添加',
            btn: ['确定', '取消'],
    ...
    function imp(){
    	var index = layer.open({
			...
			area : [ '893px', '500px' ],
			content : 'workHosExecuteImportPage.do?isCheck=false'
		});
		layer.full(index);
   	}	
  ...
  //修改保存
    function save(){
		...
		var ParamVo =[]; 
		 ...
		 ajaxPostData({
		    url: 'updateWorkHosExecute.do?isCheck=false',
		    data: {ParamVo : ParamVo.toString()},
		    success: function (responseData) {
		    	query();
		    }
	...
	var printData = function () {
    	...
    	var printPara={
            ...
            class_name: "com.chd.hrp.budg.service.business.compilationbasic.hosexecute.BudgWorkHosExecuteService",
            method_name: "getPrintData",
            bean_name: "budgWorkHosExecuteService",
