...
<script type="text/javascript">
    ...
    function query() {
        ...
        if (check_type_preval == '01') {
            if (bc_state_preval == '04') {
                grid.loadData(parms, 'queryBudgElseCostCopy.do?isCheck=false');
            } else {
            ...
            if (bc_state_preval == '04') {
                grid.loadData(parms, 'queryBudgElseCostAdjustCopy.do?isCheck=false');
            } else {
                grid.loadData(parms, 'queryBudgElseCostAdjust.do?isCheck=false');
            }
        }
    }
    ...
    function loadHead() {
        grid = $("#maingrid").etGrid({
            ...
            checkbox: true,
            freezeCols: 4,
            hoverShowTitle: showAdjustPreData, //调整前数据 浮现
        });
    }
    // 调整前数据浮现
    function showAdjustPreData(ui) {

        if (check_type_preval == '02') {
            var data = ui.rowData;
            return {
    ...
    function save() {
        ...
        ajaxPostData({
        	url: "updateBudgElseCostCheck.do?isCheck=false",
        	data: formPara,
