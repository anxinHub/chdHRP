...
<script type="text/javascript">
    ...
    function query() {
        ...
        //加载查询条件
        grid.loadData(parms, 'queryBudgElseCostExecute.do?isCheck=false');
    }
    ...
    function save() {
        ...
        if (data.addList.length > 0 || data.updateList.length > 0) {
            ...
            ajaxPostData({
                url: "saveBudgElseCostExecute.do?isCheck=false",
        ...
        if (data.length == 0) {
            ...
            if (ParamVo.length > 0) {
                $.etDialog.confirm('确定删除?', function() {
                    ajaxPostData({
                        url: "deleteBudgElseCostExecute.do?isCheck=false",
                        data: { ParamVo: ParamVo.toString() },
                        success: function(responseData) {
                            query();
    ...
    function imp() {
        var index = layer.open({
            ...
            area: ['893px', '500px'],
            content: 'budgElseCostExecuteImportPage.do?isCheck=false'
        });
        layer.full(index);
    }
    ...
    function openUpdate(obj) {
        ...
        $.etDialog.open({
            url: 'budgElseCostExecuteUpdatePage.do?isCheck=false&' + parm,
            height: 300,
            width: 750,
            title: '其他收入预算执行修改',
            btn: ['确定', '取消'],
            btn1: function (index, el) {
                ...
                iframeWindow.saveBudgElseCostExecute()
            }
        });
    }
