...
<script type="text/javascript">
    ...
    function query() {
        ...
        grid.loadData(search, 'queryBudgCostAdj.do?isCheck=false');
    }
    ...
    function lodopPrint() {
        ...
        grid.options.lodop.title = "医疗支出预算调整";
    }
        ...
        ajaxPostData({
            ...
            success: function(responseData) {
            	ajaxPostData({
                    ...
                    success: function(responseData) {
                        $.etDialog.open({
                            url: 'budgCostAdjAddPage.do?isCheck=false&',
                            title: '医疗支出预算调整申请',
                            isMax: true
                        });
                    }
    ...
    function remove() {
        ...
        if (data.length > 1) {
            ...
            $.etDialog.confirm('确定删除?', function(yes) {
                ajaxPostData({
                    url: "deleteBudgCostAdj.do?isCheck=false",
    ...
    function openUpdate(obj) {
        var openParam = obj.split('|');
        var parm =
            "budg_year=" + openParam[0] + "&" +
            "adj_code=" + openParam[1];
        
        $.etDialog.open({
            url: 'budgCostAdjUpdatePage.do?isCheck=false&' + parm,
            title: '医疗支出预算调整申请修改',
            isMax: true
        });
    }
    ...
    function lookBeforAdjustInfo(obj) {
        var lookParam = obj.split('|');
        var parm = "year=" + lookParam[0] + "&check_code=" + lookParam[1] + "&bc_state=" + lookParam[2];

        parent.$.etDialog.open({
            url: 'hrp/budg/budgcost/adjust/adjLinkPage.do?isCheck=false&' + parm,
            title: '调整前预算查看',
            isMax: true,
    ...
    function down(adj_file) {
        location.href = "downCostAdjFile.do?isCheck=false&adj_file=" + adj_file;
    }
</script>
</head>
