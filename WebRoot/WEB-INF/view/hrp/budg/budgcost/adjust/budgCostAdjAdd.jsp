...
<script type="text/javascript">
    ...
    function save() {
        ...
        $.ajaxFileUpload({
            // 用于文件上传的服务器端请求地址  
            url: 'addBudgCostAdj.do?isCheck=false&budg_year=' + budg_year + '&adj_code=' + adj_code + '&adj_file=' + adj_file + "&adj_remark=" + adj_remark + "&last_check_code=" + last_check_code,
            // 一般设置为false  
            secureuri: false, 
            // 文件上传空间的id属性  <input type="file" id="file" name="file" />  
            fileElementId: 'adj_file',
            type: 'post',
            // 返回值类型 一般设置为json
            dataType: 'json',
            // 服务器成功响应处理函数  
