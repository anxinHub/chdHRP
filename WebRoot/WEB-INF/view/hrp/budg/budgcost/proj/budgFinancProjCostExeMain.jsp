...
<head>
	...
	<script type="text/javascript">
		...
		var renderFunc = {
			...
			cost_amount: function (value) { //执行数据
				return formatNumber(value, 2, 1);
			},
			rate: function (value) { //执行进度
				return formatNumber(value, 2, 1) + '%';
			}
		};
		$(function () {
		...
		function init() {
			...
			source_select = $("#source_select").etSelect({
				url: "../../queryBudgSource.do?isCheck=false&source_attr=2",
				defaultValue: "none",
				onChange: query
			});
		...
		function loadHead() {
			grid = $("#maingrid").etGrid({
				columns: [
					...
					{
						display: '支出预算', name: 'budg_amount', align: 'right', width: 120,
						render:function(ui) {
					...
					{
						display: '执行数据', name: 'cost_amount', align: 'right', width: 120,
						render:function(ui) {
                            if (ui.rowData.cost_amount) {
                                return formatNumber(ui.rowData.cost_amount, 2, 1);
                            }
                        }
					},
					{
						display: '执行进度', name: 'rate', align: 'right', minWidth: 80,
						render:function(ui) {
                            if (ui.rowData.rate) {
			    ...
			    dataModel: {
                     method: 'POST',
                    location: 'remote',
                    url: 'queryBudgFinancProjCostExe.do',
		...
		function lodopPrint() {
			...
			grid.options.lodop.title = "财政项目支出预算执行";
		}
		//键盘事件
