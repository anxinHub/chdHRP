...
<head>
	...
	<script type="text/javascript">
		...
		var renderFunc = {
			...
			y_budg_amount: function (value) { //期末余额  合计
				return formatNumber(value, 2, 1);
			},
			...
			y_budg_amountO: function (value) { //期末余额  外拨资金
				return formatNumber(value, 2, 1);
			},
			...
			y_budg_amountM: function (value) { //期末余额  配套资金
				return formatNumber(value, 2, 1);
			}
		};
		...
		var year_input, his_name_select, proj_name_select, proj_type_select, proj_level_select, proj_state_select, payment_item_select;
		...
		function query() {
			var parms = [
				...
				{ name: 'hos_id', value: his_name_select.getValue() },
			...
			//加载查询条件
			grid.loadData(parms,'queryBudgProjDetailYearGroupQuery.do');
		}
		...
		function loadHead() {
			grid = $("#maingrid").etGrid({
				columns: [
					...
					{
						display: '医院名称', name: 'hos_simple', align: 'left', width: 150
					},
					{
					...
					},
					{
						...
						[
							...
							{
								display: '期末余额', name: 'y_budg_amount', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
					...
					},
					{
						display: '外拨资金', columns: [
							...
							{
								display: '期末余额', name: 'y_budg_amountO', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
					...
					},
					{
						...
						[
							...
							{
								display: '期末余额', name: 'y_budg_amountM', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
		...
		function lodopPrint() {
			...
			grid.options.lodop.title = "项目预算明细查询(一)(集团)";
		}
