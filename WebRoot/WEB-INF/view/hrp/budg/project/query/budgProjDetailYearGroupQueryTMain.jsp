...
<head>
	...
	<script type="text/javascript">
		...
		//打印 单元格格式化 用
		var renderFunc = {
			y_budg_amount: function (value) { //预算余额  合计
				return formatNumber(value, 2, 1);
			},
			y_cost_amount: function (value) { //支出金额  合计
				return formatNumber(value, 2, 1);
			},
			remain_amoun: function (value) { //预算余额  合计
				return formatNumber(value, 2, 1);
			},
			t_rate: function (value) { //执行进度  合计
				return formatNumber(value, 2, 1) + "%";
			},
			y_budg_amountO: function (value) { //预算余额  外拨资金
				return formatNumber(value, 2, 1);
			},
			y_cost_amountO: function (value) { //支出金额  外拨资金
				return formatNumber(value, 2, 1);
			},
			remain_amounO: function (value) { //预算余额  外拨资金
				return formatNumber(value, 2, 1);
			},
			t_rateO: function (value) { //执行进度  外拨资金
				return formatNumber(value, 2, 1) + "%";
			},
			y_budg_amountM: function (value) { //预算余额  配套资金
				return formatNumber(value, 2, 1);
			},
			y_cost_amountM: function (value) { //支出金额  配套资金
				return formatNumber(value, 2, 1);
			},
			remain_amounM: function (value) { //预算余额  配套资金
				return formatNumber(value, 2, 1);
			},
		...
		var his_name_select, proj_name_select, proj_type_select, proj_level_select, proj_state_select, con_emp_select, payment_item_select;

		function init() {
			his_name_select = $("#his_name_select").etSelect({
		...
		function query() {
			...
			//加载查询条件
			grid.loadData(parms,'queryBudgProjDetailYearGroupQueryT.do?budg_year=' + budg_year);
		}
		...
		function loadHead() {
			grid = $("#maingrid").etGrid({
				columns: [
					...
					{
						display: '支出项目名称', name: 'payment_item_name', align: 'left', width: 150
					},
					{
						display: '合计', columns: [
							{
								display: '预算金额', name: 'y_budg_amount', align: 'right', width: 120,
								render:function(ui) {
							...
							{
								display: '支出金额', name: 'y_cost_amount', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
							...
							{
								display: '预算余额', name: 'remain_amoun', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
					...
					},
					{
						display: '外拨资金', columns: [
							{
								display: '预算金额', name: 'y_budg_amountO', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
							...
							{
								display: '支出金额', name: 'y_cost_amountO', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
							...
							{
								display: '预算余额', name: 'remain_amounO', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
					...
					},
					{
						...
						[
							{
								display: '预算金额', name: 'y_budg_amountM', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
							...
							{
								display: '支出金额', name: 'y_cost_amountM', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
							...
							{
								display: '预算余额', name: 'remain_amounM', align: 'right', width: 120,
								render:function(ui) {
									var value = ui.cellData;
									return formatNumber(value, 2, 1);
								}
		...
		function lodopPrint() {
			...
			grid.options.lodop.title = "项目预算明细查询(二)(集团)";
		}
	...
	<div class="main">
		<table class="table-layout">
			<tr>
				...
				<td class="ipt">
					<select name="" id="his_name_select" style="width:150px;"></select>
				</td>
				<td class="label">项目名称：</td>
				<td class="ipt">
					<select name="" id="proj_name_select" style="width:150px;"></select>
				</td>
			...
			<tr>
				...
				<td class="ipt">
					<select name="" id="con_emp_select" style="width:150px;"></select>
				</td>
				<td class="label">支出项目：</td>
				<td class="ipt">
					<select name="" id="payment_item_select" style="width:150px;"></select>
				</td>
				<td class=""></td>
				<td class=""></td>
			</tr>
