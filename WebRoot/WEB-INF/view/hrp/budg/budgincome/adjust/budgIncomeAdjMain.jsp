...
<head>
	...
	<script type="text/javascript">
		...
		function query(){
			...
			//加载查询条件
			grid.loadData(search,'queryBudgIncomeAdj.do?isCheck=false');
		}
			...
			ajaxPostData({
	            ...
	            success: function(responseData) {
	            	ajaxPostData({
	    	            ...
	    	            success: function(responseData) {
	    	                $.etDialog.open({
	    	                    url: 'budgIncomeAdjAddPage.do?isCheck=false&',
	    	                    title: '医疗收入预算调整申请',
	    	                    isMax: true
	    	                });
	    	            }
	 ...
	 function remove() {
	        ...
	        if (data.length > 1) {
	            ...
	            $.etDialog.confirm('确定删除?', function(yes) {
	                ajaxPostData({
	                    url: "deleteBudgIncomeAdj.do?isCheck=false",
		...
		function openUpdate(obj) {
			...
			var parm =
				...
				"adj_code=" + vo[1];
			$.etDialog.open({
	            url: 'budgIncomeAdjUpdatePage.do?isCheck=false&' + parm,
	            title: '医疗收入预算调整申请修改',
	            isMax: true
	        });
		}
		...
		function audit() {
			...
			if (data.length == 0) {
				...
				$.etDialog.confirm('确定要审核?', function() {
	                ajaxPostData({
	                    url: "auditBudgIncomeAdj.do?isCheck=false",
	                    data: { ParamVo: ParamVo.toString() },
	                    success: function() {
	                        query();
	                    }
		...
		function unAudit() {
			...
			if (data.length == 0) {
				...
				$.etDialog.confirm('确定要销审?', function () {
					ajaxPostData({
						url: "unAuditBudgIncomeAdj.do?isCheck=false",
						data: { ParamVo: ParamVo.toString()},
						success: function() {
							query();
						}
		...
		function lookBeforAdjustInfo(obj) {
			...
			parent.$.etDialog.open({
				url: 'hrp/budg/budgincome/adjust/adjLinkPage.do?isCheck=false&' + parm, 
				title: '调整前预算查看', 
				isMax: true,
		...
		function down(adj_file) {
			location.href = "downMedIncomeAdjFile.do?isCheck=false&adj_file="+adj_file;
	 	}
	</script>
</head>
