...
function  save(){
         ...
         $.ajaxFileUpload({  
             url : 'updateBudgIncomeAdj.do?budg_year='+budg_year+'&adj_code='+adj_code+'&adj_file='+adj_file
   ...
   //关闭当前页面
	...
	function audit(){
			...
			$.ligerDialog.confirm('确定要审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditBudgIncomeAdj.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function unaudit(){
		...
		$.ligerDialog.confirm('确定要销审?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("unAuditBudgIncomeAdj.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
					if(responseData.state=="true"){
						parent.query();
						location.reload() ;
					}
