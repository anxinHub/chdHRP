...
<script type="text/javascript">
	...
	function query() {
		...
		//加载查询条件
		grid.loadData(parms, 'queryBudgElseIncomeCheck.do?isCheck=false');
	}
	function loadHead() {
		grid = $("#maingrid").etGrid({
			columns: [
				...
				{display: '备注', name: 'remark', align: 'left', width: 210,},
				{
	...
	function add_open() {
    	...
    	ajaxPostData({
            url: "../../../common/budgeditstate/queryElseIncomeFlag.do?isCheck=false&year="+budg_year,
            success: function(responseData) {
                if (responseData.flag == "1") {
                    parent.$.etDialog.open({
                        url: 'hrp/budg/budgincome/elseincome/check/budgElseIncomeCheckAddPage.do?isCheck=false',
                        title: '其他收入预算审批申请添加',
                        isMax: true,
	...
	function remove() {
		...
		if (data.length == 0) {
          ...
          //校验 选中数据状态
            ajaxPostData({
                url: "queryBudgElseIncomeCheckState.do?isCheck=false",
                data: {
                    budg_year: budg_year,
                ...
                success: function(responseData) {
                    if (responseData.state == "false") {
                        ...
                        $.etDialog.confirm('确定删除?', function() {
                            ajaxPostData({
                                url: "deleteBudgElseIncomeCheck.do?isCheck=false",
                                data: formPara,
                                success: function(responseData) {
                                    query();
                                }
	...
	function imp() {
		var index = layer.open({
			...
			area: ['893px', '500px'],
			content: 'budgElseIncomeCheckImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
	//下载临时模板
	...
	function openUpdate(obj) {
		...
		parent.$.etDialog.open({
            url: 'hrp/budg/budgincome/elseincome/check/budgElseIncomeCheckUpdatePage.do?isCheck=false&' + parm,
            title: '其他收入预算审核申请修改',
            isMax: true,
