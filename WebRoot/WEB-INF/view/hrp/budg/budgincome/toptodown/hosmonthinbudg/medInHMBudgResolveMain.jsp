...
<script type="text/javascript">
 ...
 function loadInit(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '月份', name: 'month', align: 'left',width:40},
				 	 ...
				 	 { display: '医院年度收入预算(元)', name: 'hyValue', align: 'right',width:120,
				 		 render:function(rowdata,rowindex,value){
				 				return formatNumber(value, 2, 1);
				 		 }
				 	 },
				 	 { display: '上年收入(元)', name: 'last_year_income', align: 'right',width:120,
				 		 render:function(rowdata,rowindex,value){
				 				return formatNumber(value, 2, 1);
				 		 }
				 	 },
				 	{ display: '增减比例(E)', name: 'grow_rate', align: 'right',width:100,editor : {type : 'float'/*, onChanged : setCountValueAfterRate */},
				 		 render:function(rowdata,rowindex,value){
				 			if(value){
			 					return formatNumber(value, 2, 1)+"%";
			 				};
				 	...
				 	{ display: '分解比例(E)', name: 'resolve_rate', align: 'right',width:100,editor : {type : 'float'/*, onChanged : setCountValueAfterRate */},
				 		 render:function(rowdata,rowindex,value){
				 	...
				 	{ display: '计算值(元)', name: 'count_value', align: 'right',width:120,
				 		 render:function(rowdata,rowindex,value){
				 				return formatNumber(value, 2, 1);
				 		 }
				 	 },
				 	 { display: '预算值(元)(E)', name: 'budg_value', align: 'right',width:120,editor : {type : 'float'/*, onChanged : setCountValueAfterRate */},
				 		 render:function(rowdata,rowindex,value){
				 				return formatNumber(value, 2, 1);
				 		 }
				 	 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMedInHMBudgResolveUp.do?isCheck=false&is_last=1&resolve_or_sum=01',
    ...
    function count(){
		...
		ajaxJsonObjectByUrl("countMedInHMBudgUp.do?isCheck=false&year="+year+"&subj_code="+subj_code,{},function (responseData){
    ...
    function countAll(){
		...
		ajaxJsonObjectByUrl("countMedInHMBudgUp.do?isCheck=false&year="+year,{},function (responseData){
    		if(responseData.state=="true"){
    			parent.query();
    			query();
    		}
		...
		if( data.length == 0 ){
    		...
    		$.ligerDialog.error('没有需要保存的数据');
   			...
   			var ParamVo =[]; 
   			 ...
   			 ajaxJsonObjectByUrl("updateMedInHMBudgUpResolve.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
            		if(responseData.state=="true"){
            			query();
            		}
            	});
    ...
    function loadDict(){
       ...
       // autocomplete("#year","../../../queryBudgYear.do?isCheck=false","id","text",true,true,"",true,'${year}',60);
        ...
        autocomplete("#subj_code","../../../queryBudgSubj.do?isCheck=false&subj_type="+'04'+"&is_last=1&budg_year="+'${year}',"id","text",true,true,'',false,'${subj_code}',200);
    	//科目级次下拉框
