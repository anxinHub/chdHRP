...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server', dataType: 'server', usePager: true, url: 'queryBudgHosDeptYearIncomePlan.do?isCheck=false',
	...
	function add_open() {
		...
		$.ligerDialog.open({
			url: 'budgHosDeptYearIncomePlanAddPage.do?isCheck=false&', data: {}, height: 300, width: 450,
				title: '医院年度至科室年度医疗收入预算分解方案', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
			buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgHosDeptYearIncomePlanAdd(); }, cls: 'l-dialog-btn-highlight' },
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function (yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteBudgHosDeptYearIncomePlan.do", { ParamVo: ParamVo.toString() }, function (responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryBudgHosDeptYearIncomePlan.do?isCheck=false", selPara, function (responseData) {
	...
	function imp() {
		...
		var index = layer.open({
			...
			area: ['893px', '500px'],
			content: 'budgHosDeptYearIncomePlanImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
	function setWay() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.open({
				url: 'budgHosDeptYearIncomePlanSetPage.do?isCheck=false', data: {}, height: 250, width: 450, 
					title: '医院年度至科室年度医疗收入预算编制方案批量设置', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
				buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgHosDeptYearIncomePlan(); }, cls: 'l-dialog-btn-highlight' },
				{ text: '取消', onclick: function (item, dialog) { dialog.close(); } }
				]
			});
		}
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url: 'budgHosDeptYearIncomePlanUpdatePage.do?isCheck=false&' + parm, data: {}, height: 300, width: 450, title: '医院年度医疗收入预算编制方案修改', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
			buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgHosDeptYearIncomePlan(); }, cls: 'l-dialog-btn-highlight' },
			{ text: '取消', onclick: function (item, dialog) { dialog.close(); } }
			]
		});
