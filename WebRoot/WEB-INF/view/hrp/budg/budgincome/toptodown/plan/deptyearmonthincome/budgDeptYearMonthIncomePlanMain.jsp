...
<head>
	...
	<script type="text/javascript">
		...
		function loadHead() {
			grid = $("#maingrid").ligerGrid({
				columns: [
					...
					{
						display: '科目全称2', name: 'subj_name_all', align: 'left'
					},
					{
						display: '科目级次', name: 'subj_level', align: 'center'
					},
					{
						display: '分解方法', name: 'resolve_way_name', align: 'left',
					},
				],
				dataAction: 'server', dataType: 'server', usePager: true, url: 'queryBudgDeptYearMonthIncomePlan.do?isCheck=false',
		...
		function add_open() {
			...
			$.ligerDialog.open({
				url: 'budgDeptYearMonthIncomePlanAddPage.do?isCheck=false&', data: {}, height: 300, width: 450, title: '医院年度医疗收入预算编制方案添加', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
				buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgDeptYearMonthIncomePlanAdd(); }, cls: 'l-dialog-btn-highlight' },
		...
		function printDate() {
			...
			var printPara = {
				headCount: 2,
				title: "医院年度至科室年度医疗收入预算分解方案",
				type: 3,
				columns: grid.getColumns(1)
			};
			ajaxJsonObjectByUrl("queryBudgDeptYearMonthIncomePlan.do?isCheck=false", selPara, function (responseData) {
		...
		function remove() {
			...
			if (data.length == 0) {
				...
				$.ligerDialog.confirm('确定删除?', function (yes) {
					if (yes) {
						ajaxJsonObjectByUrl("deleteBudgDeptYearMonthIncomePlan.do?isCheck=false", { ParamVo: ParamVo.toString() }, function (responseData) {
		...
		function setWay() {
			...
			if (data.length == 0) {
				...
				$.ligerDialog.open({
					url: 'budgDeptYearMonthIncomePlanSetPage.do?isCheck=false', data: {}, height: 300, width: 450, 
						title: '医院年度医疗收入预算编制方案批量设置', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
					buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgDeptYearMonthIncomePlan(); }, cls: 'l-dialog-btn-highlight' },
					{ text: '取消', onclick: function (item, dialog) { dialog.close(); } }
					]
				});
			}
		...
		function openUpdate(obj) {
			...
			$.ligerDialog.open({
				url: 'budgDeptYearMonthIncomePlanUpdatePage.do?isCheck=false&' + parm, data: {}, height: 300, width: 450, title: '医院年度医疗收入预算编制方案修改', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
				buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgDeptYearMonthIncomePlan(); }, cls: 'l-dialog-btn-highlight' },
				{ text: '取消', onclick: function (item, dialog) { dialog.close(); } }
				]
			});
		...
		function init(){
			...
			setTimeout(function () {
				subj_name_select = $("#subj_name_select").etSelect({
					url:"../../../../queryBudgSubj.do?isCheck=false&is_last=1&subj_type=04&type_code=01&budg_year="+year_input.getValue(),
					defaultValue: "none",
					onChange: query
				});
			}, 10);
