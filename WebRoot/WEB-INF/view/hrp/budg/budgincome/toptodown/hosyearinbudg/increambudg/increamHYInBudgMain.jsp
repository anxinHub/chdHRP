...
<head>
	...
	<script type="text/javascript">
		...
		function query(){
			...
			//加载查询条件
			grid.loadData(params,"queryIncreamHYInBudgUp.do?isCheck=false&edit_method=02");
		}
		...
		function loadHead() {
			...
			grid = $("#maingrid").etGrid({
				columns: [
					...
					{display: '上年收入(元)', name: 'last_year_income', align: 'right',editable:false,width:'10%',
					...
					{display: '增长比例(E)', name: 'grow_rate', align: 'center', dataType:'float',width:'7%',editable:setRateEdit,
						editor: {change: setCountValueAfterRate },
						render:function(ui){
							return formatNumber(ui.rowData.grow_rate, 2, 1) + "%";
						}
					},
					{display: '增长额(E)', name: 'grow_value', align: 'right',dataType:'float',width:'10%', editable:setValueEdit,
						editor: { change: setCountValueAfterValue },
						render:function(ui){
							return formatNumber(ui.rowData.grow_value, 2, 1) ;
						}
					},
					{display: '计算值', name: 'count_value', align: 'right',width:'10%',editable:false,
						render:function(ui){
							if (ui.rowData.count_value) {
								return formatNumber(ui.rowData.last_year_income+ui.rowData.grow_value, 2, 1);
							}
						}
					},
					{display: '预算值(E)', name: 'budg_value', align: 'right', width:'10%',dataType:'float',
						render:function(ui){
							return formatNumber(ui.rowData.budg_value, 2, 1);
						}
					},
					{display: '说明(E)', name: 'remark', align: 'left', dataType: 'string',minWidth:'19%'},
				],
				dataModel:{
	           	 method:'POST',
	           	 location:'remote',
	 ...
	 // 根据 get_way 04:历史数据*增长比例   05:历史数据+增长额  返回 true 或 false  控制 增长比例 单元格可否编辑 用 
		...
		function setCountValueAfterRate(rowdata,celldata) {
			...
			if (rowdata.grow_rate) {
				...
				var count_value = Number(rowdata.last_year_income) * (1 + Number(rowdata.grow_rate) / 100);
		...
		function setCountValueAfterValue(rowdata,celldata) {
			if (rowdata.grow_value) {
				...
				var count_value = Number(rowdata.last_year_income + Number(rowdata.grow_value));
		...
		function add_open() {
			...
			$.ligerDialog.open({
				url: 'increamHYInBudgAddPage.do?isCheck=false', data: {}, height: 500, width: 800,
				title: '医院年度医疗收入预算增量预算添加', modal: true, showToggle: false, showMax: true, showMin: false, isResize: true
		...
		function save() {
			...
			if(data.addList.length > 0 || data.updateList.length > 0){
	    		...
	    		ajaxPostData({
	                url: "saveIncreamHYInBudgUp.do?isCheck=false",
			...
			if (data.length == 0) {
		        ...
		        if(ParamVo.length > 0){
		        	$.etDialog.confirm('确定删除?', function () {
						ajaxPostData({
							url: "deleteIncreamHYInBudgUp.do?isCheck=false",
							data: { ParamVo: ParamVo.toString() },
							success: function (res) {
								if (res.state == "true") {
									query();
		...
		function imp() {
			$.etDialog.open({
				url: 'increamHYInBudgImportPage.do?isCheck=false',
				title: '导入',
				shadeClose: false,
				shade: false,
		...
		function openUpdate(obj) {
			...
			var parm =
				...
				"subj_code=" + vo[4]
			$.ligerDialog.open({
				url: 'increamHYInBudgUpdatePage.do?isCheck=false&' + parm, data: {}, height: 300, width: 450, title: '医院年度医疗收入独立核算科目预算', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
		...
		function printDate() {
			...
			var printPara = {
				headCount: 2,
				title: "医院年度医疗收入预算增量预算信息",
				type: 3,
				columns: grid.getColumns(1)
			};
			ajaxJsonObjectByUrl("queryIncreamHYInBudgUp.do?isCheck=false&edit_method=02", selPara, function (responseData) {
