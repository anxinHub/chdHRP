   ...
   //查询
     ...
     function loadHead(){
     	grid = $("#maingrid").etGrid({
            columns: [ 
                     { display: '科目编码', name: 'subj_code', align: 'center',width:'30%',
 					 		},
 				 	 { display: '科目名称', name: 'subj_name', align: 'center',width:'30%',
 					 		},
 			...
 			$.etDialog.error('请选择行');
 	          ...
 	          $(data).each(function (){	
             	if(this.rowData.grow_rate){
             		...
             		err += this.rowData.subj_code + this.rowData.subj_name +"、"
             	}
 	          })
 	          
 	         if(err != ''){
 	        	 $.etDialog.error('以下指标科目数据:【'+err.substring(0,err.length-1)+'】增长比例不存在!');
 	         }else{
 	        	 ajaxPostData({
