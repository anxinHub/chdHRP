     	...
     	grid = $("#maingrid").ligerGrid({
            columns: [ 
                      { display: '科目编码', name: 'subj_code', align: 'left' ,
			 		  ...
			 		  /* { display: '上年业务量', name: 'last_year_income', align: 'left',
    		...
    		$.post("queryLastYearIncome.do?isCheck=false&budg_year="+budg_year+"&subj_code="+rowdata.record.subj_code,null,function(responseData){
               ...
               //	alert(get_way) ;
               	 ...
               	 if(para.last_year_income){
               		...
               		$.ligerDialog.confirm('<span style="color: red">该指标上年业务量数据不存在,</span>确定删除该数据,维护该指标上年执行数据?', 
               			function(yes){
    	           			if(yes){
     ...
     function  save(){
    	 if($("form").valid()){
        	 ...
        	 if (data.length == 0){
    	        ...
    	        ajaxJsonObjectByUrl("addCertenHYInBudgUp.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
