...
<script type="text/javascript">
	...
	function query() {
		...
		//加载查询条件
		grid.loadData(params,"queryCertenHYInBudgUp.do?isCheck=false&edit_method=03");
	}
			...
			columns: [
				...
				{display: '科目名称', name: 'subj_name', align: 'left',width:"20%",editable:setEdit,
						editor: {
	               	     ...
	               	     url: 'queryBudgSubjEditMethod.do?isCheck=false&subj_type=04&budg_year='+budg_year,      //  动态数据接口
	               	     ...
	               	     create:function(rowdata,celldata,setting){
	               	    	 if(rowdata.year){
	               	    		 setting.url = 'queryBudgSubjEditMethod.do?isCheck=false&subj_type=04&budg_year='+rowdata.year;
	               	    	 }else{
				...
				{display: '计算值', name: 'count_value', align: 'right',width:'16%',editable:false,
					render:function(ui){
						if (ui.rowData.fun_id){//取值函数链接
							return "<a href=javascript:openFunProcess('" + ui.rowData.group_id + "|" +
								ui.rowData.hos_id + "|" + ui.rowData.copy_code + "|" + ui.rowData.subj_code + "|" + ui.rowData.year + "|" +
				...
				{display: '预算值(E)', name: 'budg_value', align: 'right', dataType:'float',width:'16%',
					render:function(ui) {
	...
	function lodopPrint() {
		...
		grid.options.lodop.title = "医院年度业务预算—确定预算";
	}
	...
	function add_open() {
		$.ligerDialog.open({
			url: 'certenHYInBudgAddPage.do?isCheck=false', data: {}, height: 500, width: 800,
	...
	function save() {
		...
		if(data.addList.length > 0 || data.updateList.length > 0){
    		...
    		ajaxPostData({
                url: "saveCertenHYInBudgUp.do?isCheck=false",
        ...
        if (data.length == 0){
            ...
            if(ParamVo.length > 0){
            	$.etDialog.confirm('确定删除?', function () {
					ajaxPostData({
						url: "deleteCertenHYInBudgUp.do?isCheck=false",
						data: { ParamVo: ParamVo.toString() },
	...
	function imp() {
		$.etDialog.open({
			url: 'certenHYInBudgImportPage.do?isCheck=false',
			title: '导入',
			shadeClose: false,
			shade: false,
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url: 'certenHYInBudgUpdatePage.do?isCheck=false&' + parm, data: {}, height: 300, width: 450, title: '医院年度医疗收入独立核算科目预算', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
