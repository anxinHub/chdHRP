...
<head>
	...
	var gridManager = null;
		...
		function loadHead() {
			...
			grid = $("#maingrid").etGrid({
				...
				dataModel: {
					method: 'POST',
					location: 'remote',
					url: 'queryMedInDMBudgUp.do?isCheck=false',
					recIndx: 'year'
				},
				pageModel : {
					type: 'remote',
		   	...
		   	if(rowdata.year && rowdata.subj_code && rowdata.dept_id){
		   		...
		   		$.post("queryDeptyearValue.do?isCheck=false&year="+rowdata.year+"&subj_code="+rowdata.subj_code+"&dept_id="+rowdata.dept_id,null,function(responseData){
		   			if(responseData){
		   				...
		   				if(para){
		              		grid.updateRow(celldata.rowIndx,{'yearValue':para.yearValue});
		              	}
		   			}else{
		...
		function openRatePage() {
			...
			$.ligerDialog.open({
				url: 'medInDMBudgResolveMainPage.do?isCheck=false&year='+year+'&subj_code='+subj_code, data: {}, height: 500, width: 800,
			...
			if (data.length == 0) {
				...
				$.etDialog.confirm('确定删除?', function (yes) {
					if (yes) {
						if (ParamVo.length > 0) {
							ajaxPostData({
								url: "deleteMedInDMBudgUp.do?isCheck=false",
		...
		function imp() {
			$.etDialog.open({
				url: 'medInDMBudgImportPage.do?isCheck=false',
				title: '导入',
				shadeClose: false,
				shade: false,
	    ...
	    function save (){
			...
			if( data.updateList.length > 0){
                ...
                ajaxPostData({
					url: "updateMedInDMBudgUp.do?isCheck=false",
					data: { ParamVo: ParamVo.toString() },
					success: function (res) {
						if (res.state == "true") {
							query();
	...
	/* 	function printDate() {
			...
			ajaxJsonObjectByUrl("queryMedInDMBudgUp.do?isCheck=false", selPara, function (responseData) {
		...
		function printDate(){
        	...
        	var printPara={
                ...
                method_name: "queryMedInDMBudgUpPrintDate",
