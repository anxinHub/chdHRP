    ...
    <script type="text/javascript">
            ...
            columns: [
                      ...
                      { display: '科目编码', name: 'subj_code', align: 'left' ,
 					 			...
 					 			editor : {
 									...
 									valueField : 'id',
 									textField : 'text',
 									url : '../../../../qureySubjIndexFromPlan.do?isCheck=false&edit_method=02&budg_year='+budg_year,
 									keySupport : true,
 									autocomplete : true,
 					  ...
 					  { display: '增长额(E)', name: 'grow_value', align: 'left',editor : {type : 'float',onChanged : setCountValueAfterValue},
 					 			render:function(rowdata,rowindex,value){
					 				if(value){
					 					return formatNumber(value,2,1);
					 				}
 					  ...
 					  { display: '计算值(E)', name: 'count_value', align: 'left',
 					 			render:function(rowdata,rowindex,value){
					 				if(value){
					 					return formatNumber(value,2,1);
					 				}
    		...
    		$.post("queryLastYearIncome.do?isCheck=false&budg_year="+budg_year+"&subj_code="+rowdata.record.subj_code,null,function(responseData){
               ...
               //	alert(get_way) ;
               	 ...
               	 if(para.last_year_income){
               		gridManager.updateCell('last_year_income',para.last_year_income,rowdata.record);
               	 }else{
               		$.ligerDialog.confirm('<span style="color: red">该指标上年收入数据不存在,</span>确定删除该数据,维护该指标上年执行数据?', 
               			function(yes){
    	           			if(yes){
     ...
     function setCountValueAfterRate(rowdata){
    	 ...
    	 if(rowdata.record.grow_rate){
    		 ...
    		 var count_value = Number(rowdata.record.last_year_income)*(1+Number(rowdata.record.grow_rate)/100) ;
   ...
   //填写、修改 增长额后  更新 计算值单元格
     function setCountValueAfterValue(rowdata){
    	  if(rowdata.record.grow_value){
     		 ...
     		 var count_value = Number(rowdata.record.last_year_income + Number(rowdata.record.grow_value)) ;
     ...
     function  save(){
    	 if($("form").valid()){
        	 ...
        	 if (data.length == 0){
    	        ...
    	        ajaxJsonObjectByUrl("addIncreamDYInBudgUp.do?isCheck=false",{ParamVo : ParamVo.toString()},function(responseData){
