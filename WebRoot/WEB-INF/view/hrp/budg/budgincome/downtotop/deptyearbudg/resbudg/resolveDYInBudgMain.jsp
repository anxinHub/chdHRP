...
<head>
	...
	<script type="text/javascript">
		...
		function query(){
			...
			//加载查询条件
			grid.loadData(search,"queryResolveDYInBudgUp.do?isCheck=false&is_single_count=0");
		}
			...
			grid = $("#maingrid").etGrid({
				columns: [
					...
					{display: '科目名称', name: 'subj_name', align: 'left',width:'15%',editable:setEdit ,
						editor: {
	                	     ...
	                	     url: '../../../../queryBudgSubj.do?isCheck=false&subj_type=04&budg_year='+budg_year,      //  动态数据接口
	                	     change: queryLastYearIncomeload ,
	                	     //与年度联动查询
	                	     create:function(rowdata,celldata,setting){
	                	    	 if(rowdata.year){
	                	    		 setting.url = '../../../../queryBudgSubj.do?isCheck=false&subj_type=04&budg_year='+rowdata.year;
	                	    	 }else{
				...
				toolbar: {
	               items: [
						...
						{ type: "button", label: '分解比例维护',icon:'minus',listeners: [{ click: resolveRate_open}] },
	  ...
	  //选择 科目后 查询其上年收入 并更新上年收入单元格
	     function queryLastYearIncomeload(rowdata,celldata){
	    	...
	    	if(rowdata.subj_code){
	    		$.post("queryLastYearIncome.do?isCheck=false&budg_year="+budg_year+"&subj_code="+rowdata.subj_code+"&dept_id="+rowdata.dept_id,null,function(responseData){
		...
		function add_open() {
			$.ligerDialog.open({
				url: 'resolveDYInBudgAddPage.do?isCheck=false', data: {}, height: 500, width: 800,
				title: '科室年度医疗收入预算分解预算添加', modal: true, showToggle: false, showMax: true, showMin: false, isResize: true
			});
		}

		function resolveRate_open() {
			$.ligerDialog.open({
				url: 'resolveRateDYInBudgMainPage.do?isCheck=false', data: {}, height: 500, width: 800,
				title: '科室年度医疗收入预算分解预算分解比例维护', modal: true, showToggle: false, showMax: true, showMin: false, isResize: true
			});
		...
		function save() {
			...
			if(data.addList.length > 0 || data.updateList.length > 0){
	    		...
	    		ajaxPostData({
	    			url: "saveResolveDYInBudgUp.do?isCheck=false",
	        ...
	        if (data.length == 0){
	            ...
	            if(ParamVo.length > 0){
	            	$.etDialog.confirm('确定删除?',function (){
						ajaxPostData({
							url: "deleteResolveDYInBudgUp.do?isCheck=false",
							data: { ParamVo: ParamVo.toString() },
							success: function (res){
								if (res.state == "true") {
									query();
		...
		function imp() {
			$.etDialog.open({
				url: 'resolveDYInBudgImportPage.do?isCheck=false',
				title: '导入',
				shadeClose: false,
				shade: false,
