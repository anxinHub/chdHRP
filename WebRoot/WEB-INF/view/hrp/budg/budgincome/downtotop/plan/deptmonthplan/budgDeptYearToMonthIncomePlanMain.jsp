...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns: [
				...
				{display: '科目级次', name: 'subj_level', align: 'center',width:80},
				{display: '分解方法', name: 'resolve_way_name', align: 'left',width:300},
			],
			dataAction: 'server', dataType: 'server', usePager: true, url: 'queryBudgDeptYearToMonthIncomePlan.do?isCheck=false',
	...
	function add_open() {
		...
		$.ligerDialog.open({
			url: 'budgDeptYearToMonthIncomePlanAddPage.do?isCheck=false&', data: {}, height: 300, width: 450, title: '医院年度医疗收入预算编制方案添加', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
			buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgDeptYearToMonthIncomePlanAdd(); }, cls: 'l-dialog-btn-highlight' },
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryBudgDeptYearToMonthIncomePlan.do?isCheck=false", selPara, function (responseData) {
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function (yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteBudgDeptYearToMonthIncomePlan.do?isCheck=false", { ParamVo: ParamVo.toString() }, function (responseData) {
	...
	function setWay() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.open({
				url: 'budgDeptYearToMonthIncomePlanSetPage.do?isCheck=false', data: {}, height: 250, width: 450, title: '医院年度医疗收入预算编制方案修改', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
				buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgDeptYearToMonthIncomePlan(); }, cls: 'l-dialog-btn-highlight' },
				{ text: '取消', onclick: function (item, dialog) { dialog.close(); } }
				]
			});
		}
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url: 'budgDeptYearToMonthIncomePlanUpdatePage.do?isCheck=false&' + parm, data: {}, height: 300, width: 450, title: '医院年度医疗收入预算编制方案修改', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
			buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgDeptYearToMonthIncomePlan(); }, cls: 'l-dialog-btn-highlight' },
			{ text: '取消', onclick: function (item, dialog) { dialog.close(); } }
			]
		});
