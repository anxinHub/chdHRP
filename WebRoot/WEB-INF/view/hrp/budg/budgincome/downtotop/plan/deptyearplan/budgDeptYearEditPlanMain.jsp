...
<head>
	...
	<script type="text/javascript">
		...
		function query(){
			...
			//加载查询条件
			grid.loadData(search,'queryBudgDeptYearEditPlan.do?isCheck=false');
		}
		...
		var printDate = function () {
        	...
        	var printPara={
                ...
                class_name: "com.chd.hrp.budg.serviceImpl.budgincome.downtotop.medinbudgeditplan.budgMedIncomeEditPlanService",
                method_name: "getPrintData",
                bean_name: "budgMedIncomeEditPlanService",
		...
		function printDate1() {
			...
			grid.options.parms.push({ name: 'subj_code', value: subj_name_select.getValue().split(",")[0] });
			...
			var printPara = {
				headCount: 2,
				title: "科室年度医疗收入预算编制方案",
				type: 3,
				columns: grid.getColumns(1)
			};
			ajaxJsonObjectByUrl("queryBudgDeptYearEditPlan.do?isCheck=false", selPara, function (responseData) {
		...
		function generate(){
			...
			$.etDialog.confirm('生成功能产生的新数据将会将覆盖原有数据,确定生成?',function (){
				ajaxPostData({
					url: "generateBudgDeptYearIncomePlan.do?isCheck=false",
					data: formPara,
					success: function (res){
						if (res.state == "true") {
							query();
		...
		function add_open() {
			...
			parent.$.ligerDialog.open({
				url: 'hrp/budg/budgincome/downtotop/plan/deptyearplan/budgDeptYearEditPlanAddPage.do?isCheck=false', data: {},
				height: 400, width: 700, title: '科室年度医疗收入预算编制方案添加', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
				parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
				buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgDeptYearIncomePlanAdd(); }, cls: 'l-dialog-btn-highlight' },
		...
		/* function add_open() {
			...
			parent.$.etDialog.open({
				url: 'hrp/budg/budgincome/downtotop/plan/deptyearplan/budgDeptYearEditPlanAddPage.do?isCheck=false&',
				width: 700,
				height: 450,
				btn: ['确定', '取消'],
				title: '科室年度医疗收入预算编制方案添加',
		...
		function remove() {
			...
			if (data.length == 0) {
				...
				$.etDialog.confirm('确定删除?',function (){
					ajaxPostData({
						url: "deleteBudgDeptYearEditPlan.do?isCheck=false",
		...
		function imp() {
			var index = layer.open({
				...
				area: ['893px', '500px'],
				content: 'budgDeptYearEditPlanImportPage.do?isCheck=false'
			});
			layer.full(index);
		}
		...
		function openUpdate(obj) {
			...
			parent.$.ligerDialog.open({
				url: 'hrp/budg/budgincome/downtotop/plan/deptyearplan/budgDeptYearEditPlanUpdatePage.do?isCheck=false&' + parm, data: {},
				height: 400, width: 700, title: '科室年度医疗收入预算编制方案修改', modal: true, showToggle: false, showMax: false, showMin: false, isResize: true,
				parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
				buttons: [{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveBudgDeptYearIncomePlan(); }, cls: 'l-dialog-btn-highlight' },
				{ text: '取消', onclick: function (item, dialog) { dialog.close(); } }
				]
			});
		}
		...
		/* function openUpdate(obj) {
			...
			parent.$.etDialog.open({
				url: 'hrp/budg/budgincome/downtotop/plan/deptyearplan/budgDeptYearEditPlanUpdatePage.do?isCheck=false&' + parm,
				width: 600,
				height: 350,
				btn: ['确定', '取消'],
				title: '科室年度医疗收入预算编制方案修改',
				frameName: window.name,
				btn1: function (index, el) {
					var iframeWindow = parent.window[el.find('iframe').get(0).name];
					iframeWindow.saveBudgDeptYearIncomePlan();
				}
