...
<script type="text/javascript">
    ...
    function  query(){
    	...
    	grid.loadData(parms,'queryBudgElseIncomeExePre.do?isCheck=false');
    }
    ...
    function save(){
    	...
    	if(data.addList.length > 0 || data.updateList.length > 0){
    		...
    		ajaxPostData({
                url: "saveBudgElseIncomeExePre.do?isCheck=false",
        ...
        if (data.length == 0){
            ...
            if(ParamVo.length > 0){
            	$.etDialog.confirm('确定删除?', function() {
                    ajaxPostData({
                        url: "deleteBudgElseIncomeExePre.do?isCheck=false",
                        data: { ParamVo: ParamVo.toString() },
                        success: function(responseData) {
                            query();
                        }
    ...
    function imp(){
    	var index = layer.open({
			...
			area : [ '893px', '500px' ],
			content : 'budgElseIncomeExePreImportPage.do?isCheck=false'
		});
		layer.full(index);
    }	
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"month="+vo[4]   +"&"+ 
			"subj_code="+vo[5] 
		 $.ligerDialog.open({ url : 'budgElseIncomeExePreUpdatePage.do?isCheck=false&'+parm,data:{}, height: 300,width: 750, title:'其他收入预算执行修改', 
