...
<script type="text/javascript">	
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostParaManSet.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'costParaManSetAddPage.do?isCheck=false', height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostParaManSet(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "fast":
              		$.ligerDialog.open({url: 'costParaManSetFastPage.do?isCheck=false', height: 500,width: 800, title:'快速填充',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveFastCostParaManSet(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteCostParaManSet.do",{ParamVo : ParamVo.toString()},function (responseData){
                ...
                case "inherit":
                	...
                	if(year_month==''){
                		...
                		ajaxJsonObjectByUrl("inheritCostParaManSet.do",{year_month : year_month},function (responseData){
                    		if(responseData.state=="true"){
                    			query();
                    		}
                    	});
                    ...
                    return;
				case "import":
                	$.ligerDialog.open({url: 'costParaManSetImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"server_dept_id="+vo[6]   +"&"+  
			"cost_item_id="+vo[7] ;
    	$.ligerDialog.open({ url : 'costParaManSetUpdatePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostParaManSet(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
  ...
  //打印数据
	 function printDate(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopPrinterTable("resultPrint","开始打印","管理分摊设置",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryCostParaManSet.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","管理分摊设置.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryCostParaManSet.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
				 	trHtml+="<td>"+item.acc_year+item.acc_month+"</td>"; 
					 trHtml+="<td>"+item.dept_code+"</td>"; 
