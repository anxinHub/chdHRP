...
<script type="text/javascript">	
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                    ...
                    { display: '分摊参数', name: 'para_code', align: 'left'
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostParaAssSetData.do',
            ...
            switch (item.id)
            {
                case "add":
                	...
                	$.ligerDialog.confirm('确定采集数据?', function (yes){
                    	if(yes){
                        	ajaxJsonObjectByUrl("addCostParaAssSetData.do?isCheck=false",{'acc_year' : acc_year,'acc_month' : acc_month},function (responseData){
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteCostParaAssSetData.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
                    ...
                    return;
				case "import":
                	$.ligerDialog.open({url: 'costParaAssSetDataImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"cost_item_no="+vo[6]   +"&"+ 
			"para_code="+vo[7] 
    	$.ligerDialog.open({ url : 'costParaAssSetDataUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostParaAssSetData(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryCostParaAssSetData.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryCostParaAssSetData.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
                     trHtml+="<td>"+item.acc_year+item.acc_month+"</td>"; 
                     trHtml+="<td>"+item.dept_id+"</td>"; 
