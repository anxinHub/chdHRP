...
<script type="text/javascript">	
           ...
           columns: [ 
                   ...
                   { display: '成本项目编码', name: 'cost_item_code', align: 'left'
                    ...
                    { display: '分摊参数', name: 'para_code', align: 'left'
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostParaManSetData.do',
        ...
        if(item.id)
        {
            switch (item.id)
            {
                case "add":
                	...
                	var acc_month = $("#b_year_month").val().substring(4,6);
                	...
                	$.ligerDialog.confirm('确定采集数据?', function (yes){
                    	if(yes){
                        	ajaxJsonObjectByUrl("addCostParaManSetData.do?isCheck=false",{'acc_year' : acc_year,'acc_month' : acc_month},function (responseData){
				...
				case "import":
                	$.ligerDialog.open({url: 'costParaManSetDataImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"cost_item_no="+vo[6]   +"&"+ 
			"para_code="+vo[7] ;
    	$.ligerDialog.open({ url : 'costParaManSetDataUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostParaManSetData(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
...
*/
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryCostParaManSetData.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryCostParaManSetData.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
                     trHtml+="<td>"+item.acc_year+item.acc_month+"</td>";                                
                     trHtml+="<td>"+item.dept_code+"</td>"; 
