    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
					 ...
					 { display: '资产编码名称', name: 'asset_name', align: 'left'
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostIassetDetail.do',
...
// 		                { line:true },
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'costIassetDetailAddPage.do?isCheck=false', height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostIassetDetail(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteCostIassetDetail.do",{ParamVo : ParamVo.toString()},function (responseData){
                ...
                case "syn":
                	...
                	return; 
				case "import":
                	$.ligerDialog.open({url: 'costIassetDetailImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"source_id="+vo[7] ;
    	$.ligerDialog.open({ url : 'costIassetDetailUpdatePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostIassetDetail(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    ...
    function loadDict(){
	 ...
	 function exportExcel(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","无形资产摊销明细采集.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryCostIassetDetail.do",printPara,function (responseData){
	...
	function print(){
	    	...
	    	var heads={
	 	       ...
	 	       var printPara={
	 	      		...
	 	      		class_name: "com.chd.hrp.cost.service.CostIassetDetailService",
	 	   			method_name: "queryCostIassetDetailPrint",
	 	   			bean_name: "costIassetDetailService"
