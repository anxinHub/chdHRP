...
<script type="text/javascript">
	...
	function myChargShow() {
		...
		ajaxJsonObjectByUrl("queryCostBreakevenPointMedical.do?isCheck=false",
				para, function(responseData) {
					$.each(responseData.Rows, function(i, obj) {
						dept_name = obj.dept_name
						fixed_cost = obj.fixed_cost
						change_cost = obj.change_cost
						income_money = obj.t_1
						workload = obj.t_4  //工作量
						t_workload = obj.t_5  //保本工作量
						unit_income_money = obj.unit_income_money
						unit_change_cost = obj.unit_change_cost
						xAxis = obj.t_5
						yAxis = obj.t_6
					});
		...
		Costdata.push([ 1, fixed_cost + unit_change_cost ], [
				t_workload + workload,
				fixed_cost + unit_change_cost * (t_workload + workload) ]);
		incomedata.push([ 1, unit_income_money ], [ t_workload + workload,
				unit_income_money * (t_workload + workload) ]);

		var obj = {
			xAxis : xAxis, //X轴 
			yAxis : yAxis, //Y轴
			title : dept_name + '${year_month_begin}' + '--'
					+ '${year_month_end}' + '本量利分析图', //标题
