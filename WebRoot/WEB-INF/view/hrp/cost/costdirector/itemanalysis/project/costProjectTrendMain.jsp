     ...
     //打印 
       function print(){
	    	...
	    	var heads={
		   	...
		   	function loadHead(){
				grid =  $("#maingrid").ligerGrid({
		           dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostProjectTrend.do?isCheck=false',
        	...
        	$.each(result, function(v_index, v_data){ 
        		if(v_index==0){
        			columns.push({
         			   ...
         			   columns:[
 	        	        			{display:'金额',name:'t'+v_data.acc_year+v_data.acc_month, align: 'right',minWidth:'120',
	    ...
	    function queryChargShow(){
	    	   ...
	    	   var cost_item_code_str=[];
                if (data.length == 0){
               	 $.ligerDialog.error('请选择成本项目');
                    return false;
                }
                $(data).each(function (){					
                	 cost_item_code_str.push(
					  this.cost_item_code
					)
                });
                var legendData=[]
                ...
                var para={      	   
            			...
            			cost_item_code_str:'('+cost_item_code_str+')',
       			};
           	   ajaxJsonObjectByUrl("queryCostProjectTrendEcharts.do?isCheck=false",para,function(responseData){
       				$.each(data,function(i,obj){
       				   deptdata=[];
       					$.each(responseData.Rows,function(v_index,v_data){
           					  if(v_data.cost_item_code == obj.cost_item_code){
               					  if(v_data.year_month!=null){
               					   deptdata.push([v_data.year_month,v_data.all_amount])
                   			    }
               				 }
           				});	
           				
           				var  formPara = {
           						   name: obj.cost_item_name,
     		    			       type: 'line',
     		    			       stack: '总量',
     		    			       data: deptdata,
       	       				}

           				legendData.push(obj.cost_item_name)
