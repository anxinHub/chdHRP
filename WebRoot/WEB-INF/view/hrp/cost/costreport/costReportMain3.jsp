...
<head>
	...
	<script type="text/javascript">
		...
		function loadHead() {
	    		...
	    		if(responseData.Rows.length > 0){
	    			$.each(responseData.Rows, function(v_index, v_data){ 
	    				cost_type_columns="{"
	  					  ...
	  					  +"minWidth: '100'},"
	  					  +"{display : '%',"
	  				      +"name : 't_"+v_data.cost_type_code+"_bl',"
	  				      +"align : 'right',formatter:'###,##0.00',"
	  				      +"render : function(rowdata, rowindex, value) {return formatNumber(value, 2, 1)+'%'},"
	  					  +"minWidth: '100'},"
	  				      +"]"
			...
			columns.push(eval("({ display: '科室全成本合计', name: 'dept_sum', align: 'right',formatter:'###,##0.00',minWidth: '100',render:function(rowdata, rowindex, value){return formatNumber(value,2,1)}})"));
			columns.push(eval("({ display: '科室收入', name: 'sum_money', align: 'right',formatter:'###,##0.00',minWidth: '100',render:function(rowdata, rowindex, value){return formatNumber(value,2,1)}})"));
			columns.push(eval("({ display: '收入-成本', name: 'sum_ce', align: 'right',formatter:'###,##0.00',minWidth: '100',render:function(rowdata, rowindex, value){return formatNumber(value,2,1)}})"));
			columns.push(eval("({ display: '诊次成本', name: 'clinic_cost', align: 'right',formatter:'###,##0.00',minWidth: '100',render:function(rowdata, rowindex, value){return formatNumber(value,2,1)}})"));
			columns.push(eval("({ display: '床日成本', name: 'bed_cost', align: 'right',formatter:'###,##0.00',minWidth: '100',render:function(rowdata, rowindex, value){return formatNumber(value,2,1)}})"));
    	...
    	var heads={
 	       ...
 	       var printPara={
 	      		...
 	      		class_name: "com.chd.hrp.cost.service.CostReportService",
 	   			method_name: "queryCostReportPrint3",
