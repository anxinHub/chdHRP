    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostBonusCostRela.do',
...
// 		                { line:true },
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'costBonusCostRelaAddPage.do?isCheck=false', height: 500,width: 800, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostBonusCostRela(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteCostBonusCostRela.do",{ParamVo : ParamVo.toString()},function (responseData){
                 ...
                 */
                case "extend":
                    $.ligerDialog.open({url: 'costBonusCostRelaExtendPage.do?isCheck=false', height: 270,width: 500, title:'继承数据',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostWageCostRela(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
                  	return;
				case "import":
                	$.ligerDialog.open({url: 'costBonusCostRelaImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"copy_code="+vo[2]   +"&"+ 
			"id="+vo[3];
    	$.ligerDialog.open({ url : 'costBonusCostRelaUpdatePage.do?isCheck=false&' + parm,data:{}, height: 280,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostBonusCostRela(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
    ...
    function loadDict(){
	 ...
	 function exportExcel(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","奖金项与成本项目的对应关系.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryCostBonusCostRela.do",exportPara,function (responseData){
...
// 				   var data={
// 						   headers:[
// 									{ x: 0, y: 0, rowSpan: 1, colSpan: 1, displayName: "统计年月", name: "year_month",size:90},
// 									{ x: 1, y: 0, rowSpan: 1, colSpan: 1, displayName: "职工分类编码", name: "emp_kind_code",size:90},
// 									{ x: 2, y: 0, rowSpan: 1, colSpan: 1, displayName: "职工分类名称", name: "emp_kind_name",size:90},
// 									{ x: 3, y: 0, rowSpan: 1, colSpan: 1, displayName: "奖金项编码", name: "bonus_item_code" ,size:80 },
// 									{ x: 4, y: 0, rowSpan: 1, colSpan: 1, displayName: "奖金项名称", name: "bonus_item_name",size:90},
// 									{ x: 5, y: 0, rowSpan: 1, colSpan: 1, displayName: "成本项目编码", name: "cost_item_code" ,size:80 },
// 									{ x: 6, y: 0, rowSpan: 1, colSpan: 1, displayName: "成本项目名称", name: "cost_item_name" ,size:90},
// 									],
// 						    rows: res.Rows
						    
// 				   }
// 				   viewPrint(data, "奖金项与成本项目的对应关系");
// 			   },
// 			   error: function (res) {
// 						console.error(res);
// 					}
...
// 	 }
          function print(){
        	  ...
        	  if(grid.getData().length==0){
	 	       ...
	 	       var printPara={
	 	      		title: "奖金项与成本项目的对应关系",//标题
	 	      		columns: JSON.stringify(grid.getPrintColumns()),//表头
	 	      		class_name: "com.chd.hrp.cost.service.CostBonusCostRelaService",
	 	   			method_name: "queryCostBonusCostRelaPrint",
	 	   			bean_name: "costBonusCostRelaService"
