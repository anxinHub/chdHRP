...
<script type="text/javascript">
    ...
    function loadHead(dept_id){
   		grid = $("#maingrid").ligerGrid({
	           columns: [ 
					 ...
					 dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostHrpDeptRef.do?isCheck=false',
                     height: '100%', checkbox: false,rownumbers:true,
                     selectRowButtonOnly:true,
                     toolbar:{ items: [
									   { text: '查询', id:'search', click: query,icon:'search' },
									   { line:true },
                                       {text: '添加', icon:'add', click:addDeptRef },
									   { line:true },
                                       //{text: '删除',icon:'delete', click:deleteDeptRef  },
                                       {text: '停用',icon:'delete', click:endDeptRef  },
									   { line:true },
                                       {text: '导入',icon:'up', click:importDeptRef  },
									   { line:true },
	...
	<script type="text/javascript">
       ...
       function addDeptRef(){
    	   $.ligerDialog.open({url: 'addDeptRefPage.do?isCheck=false', height: 400,width: 400, title:'新增自定义科室对应关系',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true ,
    		   buttons: [  
       ...
       function updateDeptRef(rowStr){
    	   ...
    	   $.ligerDialog.open({url : 'updateDeptRefPage.do?isCheck=false&ref_id=' + rowArr[2]+'&hos_id='+rowArr[1]+'&group_id='+rowArr[0], 
       ...
       function importDeptRef(){
    	   $.ligerDialog.open({url: 'importDeptRefPage.do?isCheck=false', height: 430,width: 1000, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
       }
       function deleteDeptRef(){
         ...
         $.ligerDialog.confirm('确定删除?', function (yes){
          	if(yes){
              	ajaxJsonObjectByUrl("deleteCostHrpDeptRef.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function endDeptRef(){
         ...
         $.ligerDialog.confirm('确定停用吗?', function (yes){
          	if(yes){ 
              	ajaxJsonObjectByUrl("endCostHrpDeptRef.do",{ParamVo : ParamVo.toString()},function (responseData){
              		if(responseData.state=="true"){
              			query();
              		}
              	});
     ...
     //打印
   	function print(){
       	...
       	var colspan_num = grid.getColumns(1).length-1;
      		...
      		ajaxJsonObjectByUrl("queryCostHrpDeptRef.do?isCheck=false", selPara, function (responseData) {
