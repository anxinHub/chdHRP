...
$(function(){
	...
	function plupload(){  
		$("#uploader").pluploadQueue({           
	       	...
	       	url : "readCostInAcctVouchFiles.do?isCheck=false",    
	       	chunk_size : '2mb',           
	       	filters : {                
	           mime_types: [  
...
function loadHead() {
	grid = $("#maingrid").ligerGrid(
					{
				columns : [
							 { display: '收入项目ID', name: 'income_item_id', align: 'left',editor:{type:'text'}},
							 { display: '收入项目名称', name: 'income_item_name', align: 'left',editor:{type:'text'}},
							 { display: '统计年月', name: 'year_month', align: 'left',editor:{type:'text'}},
							 { display: '科室NO', name: 'dept_no', align: 'left',editor:{type:'text'}},
        ...
        switch (item.id)
        {
            case "save":
            	var data = gridManager.getData();
           	 	ajaxJsonObjectByUrl("addBatchCostInAcctVouch.do?isCheck=false",{ParamVo : JSON.stringify(data)},function (responseData){
