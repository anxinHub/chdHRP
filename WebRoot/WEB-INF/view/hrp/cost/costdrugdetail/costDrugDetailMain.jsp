    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '统计年月', name: 'year_month',align: 'left',render : function(rowdata, rowindex,value) {
                    		...
                    		return "<a href=javascript:openUpdate('"+rowdata.group_id   + "|"+rowdata.hos_id   + "|"+rowdata.copy_code   + "|"+rowdata.acc_year   + "|" +rowdata.acc_month   + "|"  + rowdata.dept_id   + "|" + rowdata.dept_no  + "|" + rowdata.cost_item_id  +"|" + rowdata.cost_item_no  +"|" + rowdata.source_id  +"')>"+rowdata.acc_year+rowdata.acc_month+"</a>";
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostDrugDetail.do',
...
// 		                { line:true },
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'costDrugDetailAddPage.do?isCheck=false', height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostDrugDetail(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteCostDrugDetail.do",{ParamVo : ParamVo.toString()},function (responseData){
                    ...
                    return;
				case "import":
                	$.ligerDialog.open({url: 'costDrugDetailImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"cost_item_no="+vo[8] +"&"+
			"source_id="+vo[9];
    	$.ligerDialog.open({ url : 'costDrugDetailUpdatePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostDrugDetail(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
  ...
  /* //打印数据
	 function printDate(){
		...
		//	lodopPrinterTable("resultPrint","开始打印","科室药品费用表",true);
		//	return;
		//}
		...
		ajaxJsonObjectByUrl("queryCostDrugDetail.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","科室药品费用表.xls",true);
			return;
		}
...
// 											{ x: 5, y: 0, rowSpan: 1, colSpan: 1, displayName: "金额", name: "amount" ,size:100}
// 											],
// 								    rows: res.Rows
								   
// 						   }
// 						   viewPrint(data, "药品数据采集");
// 					   },
// 					   error: function (res) {
// 								console.error(res);
// 							}
           ...
           function print(){
	    	...
	    	var heads={
	 	       ...
	 	       var printPara={
	 	      		...
	 	      		class_name: "com.chd.hrp.cost.service.CostDrugDetailService",
	 	   			method_name: "queryCostDrugDetailPrint",
	 	   			bean_name: "costDrugDetailService"
