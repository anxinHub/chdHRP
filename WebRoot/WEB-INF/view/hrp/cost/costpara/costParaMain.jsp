...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '参数值', name: 'para_value', align: 'center',
							render : function(rowdata, rowindex,
								...
								if(rowdata.para_type == '1'){
									...
									str = str + "</option>";
									$("#para_value"+rowindex).ligerComboBox({width:160});
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostPara.do',
	...
	/* function save(group_id,hos_id,copy_code,para_code,rowindex){
		...
		var formPara = {
		 ...
		 ajaxJsonObjectByUrl("updateCostPara.do",formPara,function(responseData){
	...
	function save(){
		...
		if(formPara.length>0){
			/* if(str != ''){
				...
				ajaxJsonObjectByUrl("updateBatchCostPara.do?isCheck=false",{formPara:formPara.toString()},function(responseData){
		            if(responseData.state=="true"){
		                query();
		            }
		        }); 
