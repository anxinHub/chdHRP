    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostOhterDetail.do',
...
// 		                { line:true },
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'costOhterDetailAddPage.do?isCheck=false', height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostOhterDetail(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteCostOhterDetail.do",{ParamVo : ParamVo.toString()},function (responseData){
                    ...
                    return;
				case "import":
                	$.ligerDialog.open({url: 'costOhterDetailImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"cost_item_no="+vo[5] +"&"+
			"source_id="+vo[6];
    	$.ligerDialog.open({ url : 'costOhterDetailUpdatePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveCostOhterDetail(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
  ...
  /* //打印数据
	 function printDate(){
	  ...
	  //有数据直接打印
		...
		//	lodopPrinterTable("resultPrint","开始打印","科室其他费用采集",true);
		//	return;
		//}
		...
		ajaxJsonObjectByUrl("queryCostOhterDetail.do",printPara,function (responseData){
	 ...
	 */
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","科室其他费用采集.xls",true);
			return;
		}
	      ...
	      function print(){
	    	...
	    	var heads={
	 	       ...
	 	       var printPara={
	 	      		...
	 	      		class_name: "com.chd.hrp.cost.service.CostOhterDetailService",
	 	   			method_name: "queryCostOhterDetailPrint",
	 	   			bean_name: "costOhterDetailService",
