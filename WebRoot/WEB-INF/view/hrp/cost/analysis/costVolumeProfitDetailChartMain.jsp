      ...
      function myChargShow(){
    	//保本工作量  (x = 固定成本 x 工作量 / 收入 - 变动成本)
    	  ...
    	  var Gdata = [];  //固定成本data
           ...
           for (var i= 1;i <= t_workload + workload;i++)
            {
		   	   Gdata.push([i,fixed_cost]);
		   	   Costdata.push([i,fixed_cost + (unit_change_cost * i)]);
		   	   incomedata.push([i,unit_income_money * i]);
            } 

           var obj = {
            		  xAxis:xAxis, //X轴 
            		  yAxis:yAxis, //Y轴
            		  title:dept_name + '${begin}' + '-'+'${end}' + '本量利分析图',  //标题
