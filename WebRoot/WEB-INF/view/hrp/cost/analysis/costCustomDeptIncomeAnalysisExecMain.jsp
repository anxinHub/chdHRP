...
<title>临床科室收入分析构成表(自定义执行科室)</title>
...
<script type="text/javascript">
      ...
      function query(){
     	 ...
     	 grid.options.parms.push({name:'exec_dept_code',value:$("#exec_dept").val().split(" ")[0]}); 
	       	//加载查询条件
      ...
      function loadDict(){
  		 ...
  		 autocomplete("#exec_dept","../costhrpdeptref/queryCostCustomDept.do?isCheck=false","id","text",true,true);
		...
		function initGrid(){
				grid =  $("#maingrid").ligerGrid({
			           dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostCustomDeptIncomeAnalysisExec.do',
		...
		function loadColumns(){
			  ...
			  var columns = [];
			  columns = [{ display: '执行科室编码', name: 'exec_dept_code', align: 'left',frozen:true,minWidth:120,
				             render:function(rowdata, rowindex,value) {
					            return rowdata.appl_dept_code==9999?"":rowdata.exec_dept_code
							 }
				          },
				         {display: '执行科室名称', name: 'exec_dept_name', align: 'left',frozen:true,minWidth:120}
					         ]
              var para = {}
		...
		function print(){
	        ...
	        if(grid.getData().length==0){
	    	 ...
	    	 var heads={
	 	       ...
	 	       var printPara={
	 	      		title: "临床科室收入分析构成表(自定义执行科室)",//标题
	 	      		columns: JSON.stringify(grid.getPrintColumns()),//表头
	 	      		class_name: "com.chd.hrp.cost.service.analysis.AnalysisService",
	 	   			method_name: "queryCostCustomDeptIncomeAnalysisExecPrint",
...
<body style="padding: 0px; overflow: hidden;">
	 ...
	 <table cellpadding="0" cellspacing="0" class="l-table-edit" >
	 	<tr>
           ...
           <td align="right" class="l-table-edit-td"  style="padding-left:20px;">执行科室(自定义)：</td>
