		...
		function initGrid(){
				grid =  $("#maingrid").ligerGrid({
					  columns: [
				         ...
				         { display: '全院',columns: [
				          				         { display: '收入', name: 't_1', align: 'left',
					          				         render : function(rowdata, rowindex,value) {
			     									   return formatNumber(rowdata.t_1,2,1);
			     									}
			          				             },
				          				         { display: '成本', name: 't_2', align: 'left',
					          				         render : function(rowdata, rowindex,value) {
				     									   return formatNumber(rowdata.t_2,2,1);
				     									}
			     								   },
				          				         { display: '收益', name: 't_3', align: 'left',
						          				         render : function(rowdata, rowindex,value) {
					     									   return formatNumber(rowdata.t_3,2);
					     									}
			     								   },
				          				         { display: '成本收益率', name: 't_4', align: 'left',
					          				         render : function(rowdata, rowindex,value) {
				     									   return formatNumber(rowdata.t_4,2)+ '%';
				     									}
			     									},
		     									 { display: '收入结余率', name: 't_5', align: 'left',
					          				         render : function(rowdata, rowindex,value) {
				     									   return formatNumber(rowdata.t_5,2)+ '%';
				     									}
			     									},
				        				       ]},
				         { display: '药占比', name: 't_6', align: 'left',
					          				         render : function(rowdata, rowindex,value) {
				     									   return formatNumber(rowdata.t_6,2) + '%';
				     									}},
				         { display: '单位职工',columns: [
					          				         { display: '收入', name: 't_7', align: 'left',
						          				         render : function(rowdata, rowindex,value) {
					     									   return formatNumber(rowdata.t_7,2);
					     									}},
					          				         { display: '成本', name: 't_8', align: 'left',
								          				         render : function(rowdata, rowindex,value) {
							     									   return formatNumber(rowdata.t_8,2);
							     									}},
					          				         { display: '收益', name: 't_9', align: 'left',
							          				         render : function(rowdata, rowindex,value) {
						     									   return formatNumber(rowdata.t_9,2);
						     									}},
					        				       ]},
				       ],
			           dataAction: 'server',dataType: 'server',usePager:true,url:'queryCostGeneralDetailHos.do?isCheck=false&year_month_begin='+'${year_month_begin}'+'&year_month_end='+'${year_month_end}'+'&natur_code='+'${item_code}',
