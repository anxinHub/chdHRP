...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                    {display:'门诊科室',name:'dept_name',align:'left'},
					{display:'门诊人次',name:'t_1',align:'right'},
					{ display: '单位收入', name: 't_2', align: 'center',formatter:'###,##0.00',
            			 render : function(rowdata, rowindex,
									value) {
								 return formatNumber(rowdata.t_2,2,1);
								}
					 },
					 { display: '单位变动成本', name: 't_3', align: 'center',formatter:'###,##0.00',
             			 render : function(rowdata, rowindex,
									value) {
								 return formatNumber(rowdata.t_3,2,1);
								}
					 },
					 { display: '医疗收益', name: 't_4', align: 'center',formatter:'###,##0.00',
             			 render : function(rowdata, rowindex,
									value) {
								 return formatNumber(rowdata.t_4,2,1);
								}
					 },
					 { display: '医疗收入', name: 't_5', align: 'center',formatter:'###,##0.00',
             			 render : function(rowdata, rowindex,
									value) {
								 return formatNumber(rowdata.t_5,2,1);
								}
					 },
					 { display: '固定成本', name: 't_6', align: 'center',formatter:'###,##0.00',
             			 render : function(rowdata, rowindex,
									value) {
								 return formatNumber(rowdata.t_6,2,1);
								}
					 },
					 { display: '变动成本', name: 't_7', align: 'center',formatter:'###,##0.00',
             			 render : function(rowdata, rowindex,
									value) {
								 return formatNumber(rowdata.t_7,2,1);
								}
					 },
					 { display: '保本诊次', name: 't_8', align: 'center',formatter:'###,##0.00',
             			 render : function(rowdata, rowindex,
									value) {
								 return formatNumber(rowdata.t_8,2,1);
								}
					 },
					 { display: '保本收入', name: 't_9', align: 'center',formatter:'###,##0.00',
             			 render : function(rowdata, rowindex,
									value) {
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAnalysisC0901.do',
    ...
    function printData(){
    	 $("#resultPrint > table > tbody").empty();
  		...
  		if($("#resultPrint > table > tbody").html()!=""){
  			lodopPrinterTable("resultPrint","开始打印","门诊盈亏平衡分析",true);
  			return;
  		}
  		//重新查询数据，避免分页导致打印数据不全
  		...
  		ajaxJsonObjectByUrl("queryAnalysisC0901.do",printPara,function (responseData){
