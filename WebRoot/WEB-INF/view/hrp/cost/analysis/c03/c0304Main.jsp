    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '门诊成本', name: '', align: 'center',width:'20%',
                    	 columns:[
                    	          {display:'本期',name:'t_1',align:'right',formatter:'###,##0.00',
                      				...
                      				{display:'比例',name:'t_2',align:'right',formatter:'###,##0.00',
	                    	 			render : function(rowdata, rowindex,
												value) {
									 	return formatNumber(rowdata.t_2,6,1);
									}
                      				},
						 			{display:'累计',name:'t_3',align:'right',formatter:'###,##0.00',
                        			 render : function(rowdata, rowindex,
										value) {
									 return formatNumber(rowdata.t_3,2,1);
									}
                      			},
					 			{display:'比例',name:'t_4',align:'right',formatter:'###,##0.00',
                       			 render : function(rowdata, rowindex,
										value) {
									 return formatNumber(rowdata.t_4,6,1);
									}
                     			}
                    	         ]
					 },
					 { display: '住院成本', name: '', align: 'center',width:'20%',
						 columns:[
                    	          {display:'本期',name:'t_5',align:'right',formatter:'###,##0.00',
                      				...
                      				{display:'比例',name:'t_6',align:'right',formatter:'###,##0.00',
	                    	 			render : function(rowdata, rowindex,
												value) {
									 	return formatNumber(rowdata.t_6,6,1);
									}
                      				},
						 			{display:'累计',name:'t_7',align:'right',formatter:'###,##0.00',
                        			 render : function(rowdata, rowindex,
										value) {
									 return formatNumber(rowdata.t_7,2,1);
									}
                      			},
					 			{display:'比例',name:'t_8',align:'right',formatter:'###,##0.00',
                       			 render : function(rowdata, rowindex,
										value) {
									 return formatNumber(rowdata.t_8,6,1);
									}
                     			}
                    	         ]
					 },
					 { display: '合计', name: '', align: 'center',width:'20%',
						 columns:[
                    	          {display:'本期',name:'t_9',align:'right',formatter:'###,##0.00',
                      				...
                      				{display:'比例',name:'t_10',align:'right',formatter:'###,##0.00',
	                    	 			render : function(rowdata, rowindex,
												value) {
									 	return formatNumber(rowdata.t_10,6,1);
									}
                      				},
						 			{display:'累计',name:'t_11',align:'right',formatter:'###,##0.00',
                        			 render : function(rowdata, rowindex,
										value) {
									 return formatNumber(rowdata.t_11,2,1);
									}
                      			},
					 			{display:'比例',name:'t_12',align:'right',formatter:'###,##0.00',
                       			 render : function(rowdata, rowindex,
										value) {
									 return formatNumber(rowdata.t_12,6,1);
									}
                     			}
                    	         ]
					 }
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAnalysisC0304.do',
...
//  								{ x: 2, y: 0, rowSpan: 1, colSpan: 4, displayName: "门诊成本" },
//  								{ x: 2, y: 1, rowSpan: 1, colSpan: 1, displayName: "本期", name: "t_1", formatter: "#,##0.00",size:60 },
//  								{ x: 3, y: 1, rowSpan: 1, colSpan: 1, displayName: "比例" , name: "t_2", formatter: "#,##0.00",size:60 },
//  								{ x: 4, y: 1, rowSpan: 1, colSpan: 1, displayName: "累计", name: "t_3", formatter: "#,##0.00",size:60 },
//  								{ x: 5, y: 1, rowSpan: 1, colSpan: 1, displayName: "比例", name: "t_4", formatter: "#,##0.00",size:60 },
//  								{ x: 6, y: 0, rowSpan: 1, colSpan: 4, displayName: "住院成本" },
//  								{ x: 6, y: 1, rowSpan: 1, colSpan: 1, displayName: "本期", name: "t_5", formatter: "#,##0.00",size:60 },
//  								{ x: 7, y: 1, rowSpan: 1, colSpan: 1, displayName: "比例" , name: "t_6", formatter: "#,##0.00",size:60 },
//  								{ x: 8, y: 1, rowSpan: 1, colSpan: 1, displayName: "累计", name: "t_7", formatter: "#,##0.00",size:60 },
//  								{ x: 9, y: 1, rowSpan: 1, colSpan: 1, displayName: "比例", name: "t_8", formatter: "#,##0.00",size:60 },
//  								{ x: 10, y: 0, rowSpan: 1, colSpan: 4, displayName: "合计" },
//  								{ x: 10, y: 1, rowSpan: 1, colSpan: 1, displayName: "本期", name: "t_9", formatter: "#,##0.00",size:60 },
//  								{ x: 11, y: 1, rowSpan: 1, colSpan: 1, displayName: "比例" , name: "t_10", formatter: "#,##0.00",size:60 },
//  								{ x: 12, y: 1, rowSpan: 1, colSpan: 1, displayName: "累计", name: "t_11", formatter: "#,##0.00",size:60 },
//  								{ x: 13, y: 1, rowSpan: 1, colSpan: 1, displayName: "比例", name: "t_12", formatter: "#,##0.00",size:60 },
//  					    ],
//  					    rows: res.Rows
//  			   }
 			  
//  			   viewPrint(data, "医疗成本分类构成明细表");
//  		   },
//  		   error: function (res) {
// 					console.error(res);
// 				}
