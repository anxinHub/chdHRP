...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '项目', name: 'item_name', align: 'left',width:'20%',render : space_item},
                     ...
                     { display: '医疗收入', name: '', align: 'center',width:'20%',
                    	 columns:[
                    	          {display:'本期',name:'',align:'right',columns:[
						 			...
						 			{display:'累计',name:'t_2',align:'right',columns:[
				                    	          {display:'2',name:'',align:'right',formatter:'###,##0.00',
				                         			 render : function(rowdata, rowindex,
				     										value) {
					 ...
					 { display: '成本', name: '', align: 'center',width:'20%',
                    	 columns:[
                    	          {display:'本期',name:'t_3',align:'right',columns:[
				                    	          {display:'3',name:'',align:'right',formatter:'###,##0.00',
					                    	 			render : function(rowdata, rowindex,
																value) {
						 			...
						 			{display:'累计',name:'t_4',align:'right',columns:[
				                    	          {display:'4',name:'',align:'right',formatter:'###,##0.00',
				                         			 render : function(rowdata, rowindex,
				     										value) {
					 ...
					 { display: '收益', name: '', align: 'center',width:'20%',
                    	 columns:[
                    	          {display:'本期',name:'t_5',align:'right',columns:[
				                    	          {display:'5',name:'',align:'right',formatter:'###,##0.00',
					                    	 			render : function(rowdata, rowindex,
																value) {
						 			...
						 			{display:'累计',name:'t_6',align:'right',columns:[
				                    	          {display:'6',name:'',align:'right',formatter:'###,##0.00',
				                         			 render : function(rowdata, rowindex,
				     										value) {
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAnalysisC0112.do',
    ...
    function printData(){
    	...
    	$("#resultPrint > table > tbody").empty();
 		...
 		if($("#resultPrint > table > tbody").html()!=""){
 			lodopPrinterTable("resultPrint","开始打印","医疗收益总表(不含财政收入)",true);
 			return;
 		}
 		//重新查询数据，避免分页导致打印数据不全
 		...
 		ajaxJsonObjectByUrl("queryAnalysisC0112.do",printPara,function (responseData){
