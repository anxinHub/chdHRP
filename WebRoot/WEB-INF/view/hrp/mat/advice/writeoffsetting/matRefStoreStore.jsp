...
<script type="text/javascript">
    	...
    	grid = $("#maingrid").ligerGrid({
			columns: [{
					...
					editor: {
						...
						valueField: 'id',
						textField: 'text',
						url: 'queryMatRefStoreList.do?isCheck=false',//&is_sec=1',  //is_sec是否过滤已存在的二级库
						keySupport: true,
						autocomplete: true,
					} 
				}, {
					...
					editor: {
						...
						valueField: 'id',
						textField: 'text',
						url: 'queryMatRefStoreList.do?isCheck=false',
						keySupport: true,
						autocomplete: true, 
					} 
				}
		 	],
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatRefStoreStore.do',
	...
	function f_onChangeRow(e){
		...
		ajaxJsonObjectByUrl("saveMatRefStoreStore.do", e.record, function(responseData) {
    ...
    function remove(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					if(is_delete){
						ajaxJsonObjectByUrl("deleteMatRefStoreStore.do",{ParamVo : ParamVo.toString()},function (responseData){
							if(responseData.state=="true"){
    ...
    function loadDict(){
		//字典下拉框
		autocompleteAsync("#sec_store_code", "../../queryMatStoreByRead.do?isCheck=false", "id", "text", true, true); 
