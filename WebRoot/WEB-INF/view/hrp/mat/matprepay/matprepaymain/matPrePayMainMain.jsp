...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatPrePayMain.do?isCheck=false',
    ...
    function add_open(){
    	...
    	$.ligerDialog.open({ url : 'matPrePayMainAddPage.do?isCheck=false',data:{}, 
        ...
        if (data.length == 0){
           ...
           if(ParamVo.length >0){
				$.ligerDialog.confirm('确定删除?', function (yes){
                   	if(yes){
                       	ajaxJsonObjectByUrl("deleteMatPrePayMain.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'matPrePayMainImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    function openUpdate(obj){
		...
		var parm = 
			...
			"pre_pay_no="+vo[4] +"&"+ 
			"state="+vo[5] 
			$.ligerDialog.open({ url : 'matPrePayMainUpdatePage.do?isCheck=false&'+parm,data:{}, 
     ...
     /* var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matPrePayMainImportPage.do?isCheck=false&' + parm
				});
				layer.full(index);	
     */
        ...
        if(str != ''){
        	$.ligerDialog.error('<span style="color:red">审核失败,单据号:'+str+'不是未审核状态！</span>(只有未审核状态的预付款单允许审核 )');
        	 ...
        	 $.ligerDialog.confirm('确定审核所选预付款单吗?', function (yes){
	             	if(yes){
	                 	ajaxJsonObjectByUrl("updateMatPrePayMainState.do",{ParamVo : ParamVo.toString()},function (responseData){
	                 		if(responseData.state=="true"){
	                 			loadHead(null);
	                 		}
	                 	});
	...
	//打印设置
	    ...
	    function print(){
			...
			if (data.length == 0){
				...
				$(data).each(function (){		
				 ...
				 var para={
							paraId :pre_pay_id.substring(0,pre_pay_id.length-1) ,
							template_code:'04025',
		    	...
		    	//printTemplate("hrp/mat/matprepay/matprepaymain/queryPrePayByPrintTemlate.do?isCheck=false",para);
	 ...
	 function exportExcel(){
		...
		var exportPara={
			...
			ajaxJsonObjectByUrl("queryMatPrePayMain.do?isCheck=false",exportPara,function (responseData){
				 var trHtml='';
