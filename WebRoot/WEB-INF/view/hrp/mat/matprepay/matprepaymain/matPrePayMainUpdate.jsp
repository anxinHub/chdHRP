     ...
     function loadHead(){
     	 inMainGrid = $("#maingrid").ligerGrid({
            columns: [ 
  					  ...
  					  { display: '已消耗金额', name: 'out_money', align: 'right',width:100,
  					 			render: function(rowdata,rowindex,value){
  	                    	  		if(rowdata.out_money == 0 ){
  										return formatNumber(rowdata.out_money ,'${p04005 }', 1);
  									}else{
  										return formatNumber(rowdata.out_money ,'${p04005 }', 1);
  									}
  	                    	  	}
 				 	  ...
 				 	  { display: '本次付款金额', name: 'pre_pay_money', align: 'right',width:100,
  					 		 render: function(rowdata,rowindex,value){
									if(rowdata.pre_pay_money == 0 ){
										return formatNumber(rowdata.pre_pay_money ,'${p04005 }', 1);
									}else{
										return formatNumber(rowdata.pre_pay_money ,'${p04005 }', 1);
									}
								}
                      ...
                      dataAction: 'server',dataType: 'server',usePager:false,url:'queryMatPrePayIn.do?isCheck=false&pre_pay_id=${pre_pay_id}&init=${is_init}',  
                      ...
                      toolbar:{items: [
              				...
              				{ line:true },
//                	            { text: '保存（<u>S</u>）', id:'save', click: saveMatPrePayMain,icon:'save',disabled:state ==1?false:true },
//               				{ line:true },
     ...
     function showDetail(row, detailPanel,callback)
     {
         ...
         $(detailPanel).append(detailGrid);
       	 grid =$(detailGrid).css('margin',10).ligerGrid({
             columns:
                     [
                      { display: '材料编码', name: 'inv_code',width:110},
                      ...
                      { display: '已消耗金额', name: 'out_money', align: 'right',width:100,
                    	  	render: function(rowdata,rowindex,value){
                    	  		if(rowdata.out_money == 0 ){
									return formatNumber(0,'${p04005 }', 1);
								}else{
									return formatNumber(rowdata.out_money ,'${p04005 }', 1);
								}
                    	  	}
                 ...
                 dataAction : 'server',dataType : 'server',usePager : false,selectRowButtonOnly:true,isScroll:true,
	 			 //url : 'queryMatInDetail.do?isCheck=false&in_id='+row.in_id+'&pre_pay_id='+row.pre_pay_id+'&init='+row.init,
  ...
  // 显示完数据 后  更新相应的 金额
     function updateMoney(){
 	   	...
 	   	$.each(dataMain,function(index,content){
	 	   	...
	 	   	var mainPre_pay_money = 0 ; //存储 该条入库单 所有材料的  本次付款金额 总额
   ...
   //生成流水号
    ...
    function setMatPrePayMainNo(){
    	var parm={
		 		 year:year,
		 		 month:month
		         };
    	 ajaxJsonObjectByUrl("setMatPrePayMainNo.do?isCheck=false",parm,function (responseData){
        ...
        if(str != ''){
    		...
    		if(error != ''){
        		...
        		$.ligerDialog.confirm('确定要保存发票及发票明细信息吗?<span style= "color:red">(注意核对各个金额)</span>', function (yes){
                	if(yes){
                		 ajaxJsonObjectByUrl("updateMatPrePayMain.do?isCheck=false",formPara,function(responseData){
               	            if(responseData.state=="true"){
               	                parent.query();
               	                ajaxJsonObjectByUrl("updateMatPrePayDetail.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
                               		if(responseData.state=="true"){
                               		}
                               	}); 
               	            }
	...
	function audit(){
		if (state != 1){
			...
			$.ligerDialog.confirm('确定审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("updateMatPrePayMainState.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
...
// 	        }
	      ...
	      //格式化按钮
	        if(state > 1){
	    		$("#save").ligerButton({click: saveMatPrePayMain, width:90, disabled:true});
	        }else{
	    		$("#save").ligerButton({click: saveMatPrePayMain, width:90});
	        }
  ...
  //打印
	function printDate(){
		...
		if('${p04017 }'==1){
		 ...
		 var para={
					...
					template_code:'04025',
					class_name:"com.chd.hrp.mat.serviceImpl.matprepay.MatPrePayMainServiceImpl",
	...
	function printSet(){
		...
		officeFormTemplate({template_code:"04025",user_id:useId});
		/**
		parent.parent.$.ligerDialog.open({url : 'hrp/mat/matprepay/matprepaymain/prePayPrintSetPage.do?isCheck=false&template_code=04025&use_id='+useId,
