...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:false,url:'queryMatPara.do',
    ...
    function add_open(){
    	...
    	if(ParamVo.toString().length >0 ){
    		ajaxJsonObjectByUrl("saveBatchMatPara.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
	...
	/* function save(group_id,hos_id,copy_code,para_code,rowindex){
		...
		var formPara = {
		 ...
		 ajaxJsonObjectByUrl("updateMatPara.do",formPara,function(responseData){
	            if(responseData.state=="true"){
	                query();
	            }
	      });
	...
	function save(){
		...
		if(formPara.length>0){
			/* if(str != ''){
				...
				ajaxJsonObjectByUrl("updateBatchMatPara.do?isCheck=false",{formPara:formPara.toString()},function(responseData){
		            if(responseData.state=="true"){
		                query();
		            }
		        }); 
