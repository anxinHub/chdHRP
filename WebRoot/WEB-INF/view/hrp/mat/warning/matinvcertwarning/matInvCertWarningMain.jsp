    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '证件类型', name: 'type_name', align: 'left',width:160},
                     ...
                     { display: '状态', name: 'warn_type_name', align: 'center',width:60,
		                     render:function(rowdata,index,value){
					 				if(rowdata.warn_type_name == '已过期'){
					 					return '<span style="color:red">已过期</span>';
					 				}else if(rowdata.warn_type_name == '到期'){
					 					return '<span style="color:blue">到期</span>';
					 				}else if(rowdata.warn_type_name == '临近'){
					 					return '<span style="color:#00009C">临近</span>';
					 				}else if(rowdata.warn_type_name == '未到期'){
					 					return '未到期'
					 				}else if(rowdata.warn_type_name == '缺失'){
					 					return '缺失'
					 				}
					 		}
				 	 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatInvCertWarning.do?isCheck=false',
...
/* 		autocomplete("#store_id", "../../queryMatStore.do?isCheck=false", "id", "text", true, true,'',false,'',200); */
		autocomplete("#store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true,{read_or_write:1},false,'',200);
    	//物资类别下拉框
/* 		autocomplete("#type_id", "../../queryMatType.do?isCheck=false", "id", "text", true, true,'',false,'',200); */
		autocomplete("#type_id", "../../queryMatTypeDictDate.do?isCheck=false", "id", "text", true, true,{read_or_write:1},false,'',200);
    	
		//生产厂商 下拉框
		autocomplete("#fac_id", "../../queryHosFacDict.do?isCheck=false", "id", "text", true, true,'',false,'',200);
    	
		//证件类型 下拉框
		autocomplete("#cert_type_id", "../../qryMatInvCertType.do?isCheck=false", "id", "text", true, true,'',false,'',200,true);
		
		//是否保质期管理
		autocomplete("#is_quality", "../../queryMatYearOrNo.do?isCheck=false", "id", "text", true, true,'',false,'',200);
		//查询内容下拉框
	...
	//打印
		function print(){
	    	...
	    	var colspan_num = grid.getColumns(1).length-1;
	   		...
	   		ajaxJsonObjectByUrl("queryMatInvCertWarning.do?isCheck=false", selPara, function (responseData) {
 ...
 /*  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryMatInvCertWarning.do?isCheck=false",printPara,function (responseData){
			 var trHtml='';
				$.each(responseData.Rows,function(idx,item){ 
					...
					trHtml+="<td>"+Math.abs(item.days)+"</td>";
					trHtml+="</tr>";
					$("#resultPrint > table > tbody").empty();
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryMatInvCertWarning.do?isCheck=false",exportPara,function (responseData){
			 var trHtml='';
			$.each(responseData.Rows,function(idx,item){ 
				 trHtml+="<tr>";
				 trHtml+="<td>"+item.inv_code+"</td>"; 
