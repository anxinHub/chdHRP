...
/*  		var path = window.location.pathname+"/maingrid";
   ...
   //隐藏或显示  采购单位、请购单位、付款单位
    ...
    function loadDict(){
		...
		autocomplete("#stocker", "../../queryMatStockEmp.do?isCheck=false", "id", "text", true, true, "", false, '${matOrderMain.stocker}');//采购人
    ...
    function loadHead() {
    	grid = $("#maingrid").ligerGrid({
			columns : [{ display : '材料编码',name : 'inv_code',width : 150,align : 'left',
				},{display : '材料名称(E)',name : 'inv_name',width : 150,align : 'left'
			...
			dataType : 'server',
			usePager : false,
			url : 'queryMatOrderAuditDetail.do?isCheck=false',
			width : '100%',
			height : '95%',
			checkbox : false,
			enabledEdit : false,
    ...
    function audit(){
		...
		$.ligerDialog.confirm('确定要审核?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("auditMatOrderAudit.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function unAudit(){
		...
		$.ligerDialog.confirm('确定要销审?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("unAuditMatOrderAudit.do",{ParamVo : ParamVo.toString()},function (responseData){
					if(responseData.state=="true"){
						parent.query();
						$("#unAudit").ligerButton({click: unAudit, width:90, disabled:true});
	...
	function printDate(){
	   ...
	   printTemplate("hrp/mat/order/audit/queryMatOrderAuditByPrintTemlate.do?isCheck=false",para);
	   //printTemplate("hrp/mat/initial/affiin/queryMatInByPrintTemlateQm.do?isCheck=false",para);
	   */
	   
		var para={
				template_code:'04006',
				class_name:"com.chd.hrp.mat.serviceImpl.order.audit.MatOrderAuditServiceImpl",
				method_name:"queryMatOrderAuditByPrintTemlateNewPrint",
	...
	function printSet(){
		...
		parent.parent.$.ligerDialog.open({url : 'hrp/mat/order/audit/matOrderAuditPrintSetPage.do?template_code=04006&use_id='+useId,
			data:{}, height: $(parent).height(),width: $(parent).width(), title:'打印模板设置',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
		});
		*/
		officeFormTemplate({template_code:"04006",use_id:useId});
	}
