	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
					...
					url : 'queryMatPurCloseInvInfos.do?isCheck=false',
	...
	function cancleCloseInv(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要取消关闭的材料？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("updateMatPurCancleCloseInvs.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData){
