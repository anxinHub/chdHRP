...
<html style="overflow:hidden;">
  <head>
    ...
    function loadDict(){
    	//字典下拉框
    	autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true,{codes:'9,28,30,33,34'}, false, '${matAffiOut.bus_type_code}');//业务类型
    	autocomplete("#store_code", "../../queryMatStore.do?isCheck=false", "id", "text", true, true, "", false);//仓库
    	if('$matAffiOut.store_id'){
    		liger.get("store_code").setValue('${matAffiOut.store_id},${matAffiOut.store_no}');
    		liger.get("store_code").setText('${matAffiOut.store_code} ${matAffiOut.store_name}');
    	}
		autocomplete("#dept_code", "../../queryMatDept.do?isCheck=false", "id", "text", true, true,"", false);//领料科室
		if('$matAffiOut.dept_id'){
    		liger.get("dept_code").setValue('${matAffiOut.dept_id},${matAffiOut.dept_no}');
    		liger.get("dept_code").setText('${matAffiOut.dept_code} ${matAffiOut.dept_name}');
    	}
    	autocomplete("#dept_emp", "../../queryMatEmp.do?isCheck=false", "id", "text", true, true,"", false);//领料人
    	if('$matAffiOut.dept_emp'){
    		liger.get("dept_emp").setValue('${matAffiOut.dept_emp}');
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			columns: [{display: '材料编码', name: 'inv_code', align: 'left', width: 150,
							totalSummary: {
			                    type: 'sum',
					  ...
					  {display: '零售单价', name: 'sell_price', align: 'right', width: 100,
						  render : function(rowdata, rowindex, value) {
							  return formatNumber(rowdata.sell_price ==null ? 0 : rowdata.sell_price,'${p04006 }',1);
							}  
					  }, 
					  {display: '零售金额', name: 'sell_money', align: 'right', width: 100,
						  render:function(rowdata){
			            		return formatNumber(rowdata.sell_money ==null ? 0 : rowdata.sell_money,'${p04005 }',1);
			             },
			             totalSummary: {
		                        type: 'sum',
		                        render: function (suminf, column, cell) {
			...
			url : 'queryMatOrderInitAffiOutDetail.do?isCheck=false',
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" border="0">
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="out_no" type="text" id="out_no" value="${matAffiOut.out_no}" ltype="text" disabled="disabled"/>
	            </td>
