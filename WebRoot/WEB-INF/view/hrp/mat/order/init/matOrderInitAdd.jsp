   ...
   //选择定向与非定向是否显示定向科室
     ...
     function  save(){
    	 if(validateGrid()){
            ...
            ajaxJsonObjectByUrl("addMatOrderInit.do?isCheck=true",formPara,function(responseData){
	...
	//	autocompleteAsync("#store_code","../../queryMatStoreDictDate.do?isCheck=false","id","text",true,true,"",true);//仓库信息
		
		autocompleteAsync("#store_code", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true, {is_com : 0}, true);
		...
		var dept_id = liger.get("dept_code").getValue() == null ? "" : liger.get("dept_code").getValue().split(",")[0];
	/* 	
		autocomplete("#stocker", "../../queryMatStockEmp.do?isCheck=false", "id", "text", true, true,
				"{dept_id : "+dept_id+"}", false);//采购人 */
		...
		autocomplete("#dir_dept_id","../../queryMatDeptDictDate.do?isCheck=false","id","text",true,true,{is_write:'1'},false,false,'160');//定向科室	
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [{
				display : '交易编码',name : 'bid_code',width : 150,align : 'left', hide: true
			},{
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" border="0">
	        ...
	        <tr>
	           ...
	           <td align="right" class="l-table-edit-td" >计划到货日期：</td>
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="arrive_date" id="arrive_date" required="false"  type="text" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
	            </td>
			</tr>
			<tr>
				<td align="right" class="l-table-edit-td" >到货地址：</td>
	            <td align="left" class="l-table-edit-td" colspan="3">
	            	<input name="arr_address" type="text" id="arr_address" ltype="text"  validate="{required:true,maxlength:50}" value="吴宁西路60号东阳人民医院行政楼1楼卫生材料仓库"/>
	            </td>
