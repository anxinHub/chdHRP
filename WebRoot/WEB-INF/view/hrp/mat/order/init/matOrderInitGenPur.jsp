...
<script type="text/javascript">
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatOrderGenPur.do?isCheck=false',
    ...
    function loadDetailHead(){
    	detailGrid = $("#detailgrid").ligerGrid({
			columns : [{
				...
				display : '材料名称', name : 'inv_name', width : 250, align : 'left'
			}, {
  	...
  	function gen(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要生成订单吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("genByPurMatOrderInit.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
