...
<%-- 	<script src='<%=path%>/lib/SpreadJS/scripts/pluggable/gcspread.sheets.print.9.40.20161.0.min.js' type='text/javascript'></script> --%>
    <script type="text/javascript">
     ...
     var grid;
     var gridManager;
     var state = '${matOrderMain.state}';
     var flag = '${matOrderMain.come_from}';
     var isHideCheck = '${p04033 }' == 1 ? false : true;
     $(function (){
		loadDict()//加载下拉框  
		loadHead();
		queryDetail();
		if('${matOrderMain.pur_type}'=='1'){
			$('input:radio:first').attr('checked', 'checked');
			$('#hos_name').hide();
		}
		if('${matOrderMain.pur_type}'=='2'){
     ...
     function  save(){
    	 if(validateGrid()){
    		 ...
    		 var formPara={
    	        ...
    	        ajaxJsonObjectByUrl("updateMatOrderInit.do?isCheck=true",formPara,function(responseData){
    		         if(responseData.state=="true"){
    		        	 parentFrameUse().query();
    		              queryDetail();
    		         }
    ...
    function loadDict(){
    	//字典下拉框
    	autocomplete("#sup_code", "../../queryHosSupDictDisable.do?isCheck=false", "id", "text", true, true);//供应商
    	...
    	autocomplete("#take_hos_id", "../../../sys/queryHosInfoDictPerm.do?isCheck=false", "id", "text", true, true, "", false, '${matOrderMain.take_hos_id}');//收货单位
		autocomplete("#pay_hos_id", "../../../sys/queryHosInfoDictPerm.do?isCheck=false", "id", "text", true, true, "", false, '${matOrderMain.pay_hos_id}');//付款单位
		
		autocompleteAsync("#store_code","../../queryMatStoreDictDate.do?isCheck=false","id","text",true,true);//仓库信息
		if('${matOrderMain.store_id}'){
			liger.get("store_code").setValue('${matOrderMain.store_id},${matOrderMain.store_no}');
			liger.get("store_code").setText('${matOrderMain.store_code} ${matOrderMain.store_name}');
		}
		autoCompleteByData("#order_type", matOrderMain_orderType.Rows, "id", "text", true, true, "", false, '${matOrderMain.order_type}');//订单类型
    	autocomplete("#stock_type_code", "../../queryMatStockType.do?isCheck=false", "id", "text", true, true, "", false, '${matOrderMain.stock_type_code}');//采购类型
    	autocomplete("#pay_code", "../../queryMatPayTerm.do?isCheck=false", "id", "text", true, true, "", false, '${matOrderMain.pay_code}');//付款方式
		
    	autocomplete("#dept_code", "../../queryPurDept.do?isCheck=false", "id", "text", true, true);//采购部门
		liger.get("dept_code").setValue('${matOrderMain.dept_id},${matOrderMain.dept_no}');
		liger.get("dept_code").setText('${matOrderMain.dept_code} ${matOrderMain.dept_name}');
		
		autocomplete("#stocker", "../../queryMatStockEmp.do?isCheck=false", "id", "text", true, true);//采购人
		liger.get("stocker").setValue('${matOrderMain.stocker}');
		liger.get("stocker").setText('${matOrderMain.stocker_name}');
		
		autoCompleteByData("#is_dir", yes_or_no.Rows, "id", "text", true, true,'',false,'${matOrderMain.is_dir}','160');//是否定向
		autocomplete("#dir_dept_id","../../queryMatDeptDictDate.do?isCheck=false","id","text",true,true,{is_write:'1'});//定向科室	
		liger.get("dir_dept_id").setValue('${matOrderMain.dir_dept_id},${matOrderMain.dir_dept_no}');
		liger.get("dir_dept_id").setText('${matOrderMain.dir_dept_code} ${matOrderMain.dir_dept_name}');
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [{ display : '交易编码',name : 'bid_code',width : 150,align : 'left', hide: true
					...
					editor : {
						...
						selectBoxHeight : 240,
						grid : {
							columns : [{display : '交易编码',name : 'bid_code',minWidth : 100,align : 'left', hide: true}, 
				...
				}, {display : '需求仓库', name : 'store_name', width : 150, align : 'left'
			...
			dataType : 'server',
			usePager : false,
			url : 'queryMatOrderInitDetail.do?isCheck=false',
			width : '100%',
			height : '90%',
			checkbox : true,
			enabledEdit : true,
			...
			toolbar : { 
				items : [ {
					...
					text: '审核（<u>A</u>）', id: 'audit', click: audit, icon: 'audit', hide: isHideCheck, disabled: state == 1 ? false : true 
				}, { 
					line:true, hide: isHideCheck, 
				}, { 
					text: '销审（<u>U</u>）', id: 'unAudit', click: unAudit, icon: 'unaudit', hide: isHideCheck, disabled: state == 2 ? false : true 
				}, { 
					line:true, hide: isHideCheck, 
				}, { 
					text: '更新订单状态', id: 'ystate', click: ystate, icon: 'edit', disabled: state == 2 ? false : true 
				} ] 
			}
		});
	...
	function audit(){
		...
		var ParamVo =[];
		ParamVo.push(
				'${matOrderMain.group_id}'   +"@"+ 
				'${matOrderMain.hos_id}'   +"@"+ 
				'${matOrderMain.copy_code}'   +"@"+ 
				'${matOrderMain.order_id}' 
		);
...
// 				use_id:useId
// 			};
			//printTemplate("queryMatOrderInitByPrintTemlate.do",para);
		//alert(11111111111);
		var para={
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" border="0">
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="order_id" type="hidden" id="order_id" value="${matOrderMain.order_id}" ltype="text" />
	            	<input name="order_code" type="text" id="order_code" readOnly value="${matOrderMain.order_code}" ltype="text" />
	            </td>
	            
	            <td align="right" class="l-table-edit-td" ><font color="red">*</font>编制日期：</td>
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="order_date" id="order_date" required="true" value="${matOrderMain.order_date}" type="text" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
	            </td>
	        ...
	        <tr>
	            ...
	            <td align="right" class="l-table-edit-td"><font color="red">*</font>需求库房：</td>
	            <td align="left" class="l-table-edit-td">
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	             ...
	             <td align="right" class="l-table-edit-td" >计划到货日期：</td>
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="arrive_date" id="arrive_date" required="false" value="${matOrderMain.arrive_date}" type="text" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
	            </td>
			...
			<tr>
	            ...
	            <td align="left" class="l-table-edit-td" colspan="3">
	            	<input name="arr_address" type="text" id="arr_address" ltype="text"  value="${matOrderMain.arr_address}" validate="{required:true,maxlength:50}" />
	            </td>
	            <td align="right" class="l-table-edit-td" >备注：</td>
	            <td align="left" class="l-table-edit-td" >
	            	<input name="note" type="text" id="note" ltype="text" value="${matOrderMain.note}" validate="{required:true,maxlength:50}" />
	            </td>
			</tr>
	    </table>
    </form>
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%"  style="margin-top: 5px;">
			<%-- <tr>	
				<td align="center" class="l-table-edit-td" >
					制单人：${matOrderMain.maker_name}
				</td>
				<td align="center" class="l-table-edit-td" >
					审核人：${matOrderMain.checker_name}
				</td>
			</tr> --%>
			<tr>	
