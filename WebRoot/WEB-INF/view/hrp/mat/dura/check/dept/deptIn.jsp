...
<html style="overflow: hidden;"  xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    function loadHead(){
		grid = $("#maingrid").ligerGrid({
			columns: [ { 
				...
				display: '入库数量', name: 'amount', align: 'right', width: 80, 
				totalSummary: {
					align: 'right', 
					render: function (suminf, column, cell) {
			...
			toolbar: { items: [ 
			                    ...
			                    { line: true }, */
			                    { text: '入库确认（<u>I</u>）', id: 'confirm', click: confirm, icon: 'account', disabled: state == 1 ? false : true },
								{ line:true }
				]}
		});
    ...
    function confirm(){
		if(state != 1){
			$.ligerDialog.error("入库确认失败！单据不是新建状态");
			return false; 
		}
    	var ParamVo = [];
