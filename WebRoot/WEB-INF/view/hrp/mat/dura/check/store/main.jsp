...
<script type="text/javascript">
	...
	function loadHead(){
		grid = $("#maingrid").ligerGrid({
			columns: [ { 
				display: '单据号', name: 'check_no', width : 130,  align: 'left',
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatDuraCheckStore.do',
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/dura/check/store/addPage.do?isCheck=false',
    ...
    function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMatDuraCheckStore.do", {ParamVo : ParamVo.toString()}, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/dura/check/store/updatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});   
	}
    ...
    function printSet(){
    	...
    	parent.$.ligerDialog.open({url : 'hrp/mat/dura/check/store/printSetPage.do?template_code=041313&use_id='+useId,
    ...
    function print(){
		...
		if (data.length == 0){
			...
			$(data).each(function (){		
 			...
 			/*if(check_nos != ""){
 				$.ligerDialog.error("打印失败！<br>以下单据不是确认状态：<br>"+in_nos);
				return;
			} */
			var para={
