...
<script type="text/javascript">
    ...
    function  query(){
		...
		grid.options.parms.push({name: 'in_dept_id', value: liger.get("in_dept_code").getValue().split(",")[0]}); 
	...
	function loadHead(){
		grid = $("#maingrid").ligerGrid({
			columns: [ { 
				...
				display: '移出科室', name: 'dept_name', width : 120, align: 'left'
			}, { 
				display: '移入科室', name: 'in_dept_name', width : 120, align: 'left'
			}, { 
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatDuraTranDeptDept.do',
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/dura/tran/deptDept/addPage.do?isCheck=false',
    ...
    function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMatDuraTranDeptDept.do", {ParamVo : ParamVo.toString()}, function(responseData) {
    ...
    function offset(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定冲账?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("offsetMatDuraTranDeptDept.do", {ParamVo : ParamVo.toString()}, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/dura/tran/deptDept/updatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});   
	}
	...
	function loadDict() {
		...
		autocomplete("#in_dept_code", "../../../queryHosDeptDictByPerm.do?isCheck=false", "id", "text", true, true, {is_last : 1});
    ...
    function printSet(){
    	...
    	parent.$.ligerDialog.open({url : 'hrp/mat/dura/tran/deptDept/printSetPage.do?template_code=041311&use_id='+useId,
    ...
    function print(){
		...
		if (data.length == 0){
			...
			$(data).each(function (){		
 			...
 			if(dura_nos != ""){
 				$.ligerDialog.error("打印失败！<br>以下单据不是移入确认状态：<br>"+dura_no);
				return;
			} 
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
		<tr>
			...
			<td align="left" class="l-table-edit-td">
				<input name="in_dept_code" type="text" id="in_dept_code" ltype="text" />
			</td>
		</tr> 
		<tr>
			<td align="right" class="l-table-edit-td" >
