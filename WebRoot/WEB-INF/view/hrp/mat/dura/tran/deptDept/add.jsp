  ...
  <head>
	...
	function  save(){
		...
		if(liger.get("dept_code").getValue() == liger.get("in_dept_code").getValue()){
			$.ligerDialog.warn('移出科室与移入科室不能一致！');
			return false;
		}
  			...
  			$.each(dura_detail_data, function(i, v){ 
	  	      	if(v.inv_id){
	  				...
	  				if(!targetMap.get(key)){
	  					...
	  					/* msg.append(targetMap.get(key)+"与"+value+"材料编码、单价不能重复;</br>"); */
        ...
        ajaxJsonObjectByUrl("addMatDuraTranDeptDept.do", formPara, function(responseData){
	...
	function loadDict(){
		...
		autocompleteAsync("#in_dept_code", "../../../queryHosDeptDict.do?isCheck=false", "id", "text", true, true, {is_last : 1}, true);
    ...
    function choice_inv(){
		...
		$.ligerDialog.open({url: "choiceInvPage.do?isCheck=false&dept_id="+dept_id+"&dept_text="+dept_text, 
