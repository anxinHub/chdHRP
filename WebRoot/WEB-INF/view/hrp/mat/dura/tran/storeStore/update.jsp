...
<html style="overflow: hidden;"  xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	var state = "${matDuraStoreStore.state}";
	...
	function  save(){
		...
		if(!liger.get("store_code").getValue()){
			$.ligerDialog.warn('移出库房不能为空！');
			return false;
		}
		if(!liger.get("in_store_code").getValue()){
			$.ligerDialog.warn('移入库房不能为空！');
			return false;
		}
		if(liger.get("store_code").getValue() == liger.get("in_store_code").getValue()){
			$.ligerDialog.warn('院内移拨移出、移入库房不能一致！');
			return false;
		}
        ...
        var formPara={
        		...
        		dura_no: $("#dura_no").val(), 
 				...
 				in_store_id: liger.get("in_store_code").getValue().split(",")[0], 
 				in_store_no: liger.get("in_store_code").getValue().split(",")[1], 
 				brief: $("#brief").val(), 
 				detailData: JSON.stringify(dura_detail_data)
         };
        ajaxJsonObjectByUrl("updateMatDuraTranStoreStore.do", formPara, function(responseData){
	...
	function loadDict(){
    	...
    	if('${matDuraStoreStore.store_id}'){
			liger.get("store_code").setValue('${matDuraStoreStore.store_id},${matDuraStoreStore.store_no}');
			liger.get("store_code").setText('${matDuraStoreStore.store_code} ${matDuraStoreStore.store_name}');
    	}
    	
		autocompleteAsync("#in_store_code", "../../../queryMatStoreAll.do?isCheck=false", "id", "text", true, true);
		if("${matDuraStoreStore.in_store_id}"){
			liger.get("in_store_code").setValue('${matDuraStoreStore.in_store_id},${matDuraStoreStore.in_store_no}');
			liger.get("in_store_code").setText('${matDuraStoreStore.in_store_code} ${matDuraStoreStore.in_store_name}');
		}
 	...
 	function queryDetail(){
 		...
 		grid.options.parms.push({
 			name: 'dura_id', 
 			value: '${matDuraStoreStore.dura_id}'
 		});
 		grid.options.parms.push({
 			name: 'store_id', 
 			value: '${matDuraStoreStore.store_code}'.split(",")[0]
 		});
    ...
    function loadHead(){
		grid = $("#maingrid").ligerGrid({
				...
				editor: {
					...
					grid: {
						columns: [ {
							...
							display: '条码', name: 'bar_code', width: 180, align: 'left'
						} ], 
						...
						url: '../../../queryMatDuraStoreInvList.do?isCheck=false&store_id=' 
								+ liger.get("store_code").getValue().split(",")[0], 
						pageSize: 100, //每页条数
			...
			dataAction: 'server', dataType: 'server', url: 'queryMatDuraTranStoreStoreDetailByCode.do?isCheck=false', delayLoad: true, //初始化明细数据
    ...
    function audit(){
		...
		$.ligerDialog.confirm('确定移出确认?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("auditMatDuraTranStoreStore.do", {ParamVo: ParamVo.toString()}, function(responseData) {
					if (responseData.state == "true") {
						state = 2;
						change_button();
				    	loadHead();
    ...
    function unAudit(){
		...
		$.ligerDialog.confirm('确定取消移出确认?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("unAuditMatDuraTranStoreStore.do", {ParamVo: ParamVo.toString()}, function(responseData) {
					if (responseData.state == "true") {
						state = 1;
						change_button();
				    	loadHead();
    ...
    function confirm(){
		...
		$.ligerDialog.confirm('确定移入确认?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("confirmMatDuraTranStoreStore.do", {ParamVo: ParamVo.toString()}, function(responseData) {
					if (responseData.state == "true") {
						state = 3;
						change_button();
				    	loadHead();
	...
	function printDate(){
		...
		var para={
				dura_id: $("#dura_id").val(),  
				template_code:'041309',
				class_name:"com.chd.hrp.mat.serviceImpl.dura.tran.MatDuraTranStoreStoreServiceImpl",
	...
	function printSet(){
		...
		officeFormTemplate({template_code:"041309",use_id:useId});
		/* parent.parent.$.ligerDialog.open({url: 'hrp/mat/dura/tran/storeStore/printSetPage.do?template_code=041309&use_id='+useId, 
	...
	<input name="hos_id"  type="hidden" id="hos_id" value="${matDuraStoreStore.hos_id}" />
	<input name="group_id"  type="hidden" id="group_id" value="${matDuraStoreStore.group_id}" />
	<input name="copy_code"  type="hidden" id="copy_code" value="${matDuraStoreStore.copy_code}" />
	<input name="dura_id"  type="hidden" id="dura_id" value="${matDuraStoreStore.dura_id}" />
	<div id="layout1">
		<div position="top">
				...
				<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
					<tr>
						...
						<td align="left" class="l-table-edit-td">
							<input name="dura_no" type="text" id="dura_no" disabled="disabled" ltype="text" value="${matDuraStoreStore.dura_no}"/>
						</td>
						...
						<td align="left" class="l-table-edit-td">
							<input name="make_date" type="text" id="make_date" ltype="text"  validate="{required: true}"  value="${matDuraStoreStore.make_date}" class="Wdate" onFocus="WdatePicker({isShowClear: true, readOnly: false, dateFmt: 'yyyy-MM-dd'})"/>
						</td>

						<td align="right" class="l-table-edit-td" >
							<span style="color: red">*</span>移出库房：
						</td>
						<td align="left" class="l-table-edit-td">
					...
					<tr>
						<td align="right" class="l-table-edit-td" >
							<span style="color: red">*</span>移入库房：
						</td>
						<td align="left" class="l-table-edit-td">
							<input name="in_store_code" type="text" id="in_store_code" ltype="text"  validate="{required: true}" />
						</td>
						...
						<td align="left" class="l-table-edit-td" colspan="3">
							<textarea class="l-textarea" name="brief" id="brief" rows="3" style="width: 380px;">${matDuraStoreStore.brief}</textarea>
						</td>
					</tr>
				</table>
			</form>
