...
<script type="text/javascript">
	...
	function loadHead(){
		grid = $("#maingrid").ligerGrid({
			columns: [ { 
				...
				display: '移入库房', name: 'in_store_name', width : 120, align: 'left'
			}, { 
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatDuraTranStoreStore.do',
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/dura/tran/storeStore/addPage.do?isCheck=false',
    ...
    function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMatDuraTranStoreStore.do", {ParamVo : ParamVo.toString()}, function(responseData) {
    ...
    function offset(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定冲账?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("offsetMatDuraTranStoreStore.do", {ParamVo : ParamVo.toString()}, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function loadDict() {
		...
		autocomplete("#in_store_code", "../../../queryMatStore.do?isCheck=false", "id", "text", true, true);
    ...
    function printSet(){
    	...
    	parent.$.ligerDialog.open({url : 'hrp/mat/dura/tran/storeStore/printSetPage.do?template_code=041309&use_id='+useId,
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
		<tr>
			...
			<td align="left" class="l-table-edit-td">
				<input name="in_store_code" type="text" id="in_store_code" ltype="text" />
			</td>
		</tr> 
		<tr>
			<td align="right" class="l-table-edit-td" >
