...
<html style="overflow: hidden;"  xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	var state = "${matDuraStoreDept.state}";
	...
	function  save(){
        ...
        ajaxJsonObjectByUrl("updateMatDuraTranStoreDept.do", formPara, function(responseData){
	...
	function loadDict(){
    	...
    	if('${matDuraStoreDept.store_id}'){
			liger.get("store_code").setValue('${matDuraStoreDept.store_id},${matDuraStoreDept.store_no}');
			liger.get("store_code").setText('${matDuraStoreDept.store_code} ${matDuraStoreDept.store_name}');
    	}
    	autocomplete("#bus_type_code", "../../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {codes : '37,38'}, false, '${matDuraStoreDept.bus_type_code}');
    	
		autocompleteAsync("#dept_code", "../../../queryHosDeptDictByPerm.do?isCheck=false", "id", "text", true, true, {is_last : 1});
		if("${matDuraStoreDept.dept_id}"){
			liger.get("dept_code").setValue('${matDuraStoreDept.dept_id},${matDuraStoreDept.dept_no}');
			liger.get("dept_code").setText('${matDuraStoreDept.dept_code} ${matDuraStoreDept.dept_name}');
		}
 	...
 	function queryDetail(){
 		...
 		grid.options.parms.push({
 			name: 'dura_id', 
 			value: '${matDuraStoreDept.dura_id}'
 		});
 		grid.options.parms.push({
 			name: 'store_id', 
 			value: '${matDuraStoreDept.store_code}'.split(",")[0]
 		});
    ...
    function loadHead(){
		grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server', dataType: 'server', url: 'queryMatDuraTranStoreDeptDetailByCode.do?isCheck=false', delayLoad: true, //初始化明细数据
	...
	function printSet(){
		...
		officeFormTemplate({template_code:"041307",use_id:useId});
		/* parent.parent.$.ligerDialog.open({url: 'hrp/mat/dura/tran/storeDept/printSetPage.do?template_code=041307&use_id='+useId, 
	...
	<input name="hos_id"  type="hidden" id="hos_id" value="${matDuraStoreDept.hos_id}" />
	<input name="group_id"  type="hidden" id="group_id" value="${matDuraStoreDept.group_id}" />
	<input name="copy_code"  type="hidden" id="copy_code" value="${matDuraStoreDept.copy_code}" />
	<input name="dura_id"  type="hidden" id="dura_id" value="${matDuraStoreDept.dura_id}" />
	<div id="layout1">
		<div position="top">
				...
				<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
					<tr>
						...
						<td align="left" class="l-table-edit-td">
							<input name="dura_no" type="text" id="dura_no" disabled="disabled" ltype="text" value="${matDuraStoreDept.dura_no}"/>
						</td>
						...
						<td align="left" class="l-table-edit-td">
							<input name="make_date" type="text" id="make_date" ltype="text"  validate="{required: true}"  value="${matDuraStoreDept.make_date}" class="Wdate" onFocus="WdatePicker({isShowClear: true, readOnly: false, dateFmt: 'yyyy-MM-dd'})"/>
						</td>
					...
					<tr>
						...
						<td align="right" class="l-table-edit-td" >
							<span style="color: red">*</span>领用科室：
						</td>
						<td align="left" class="l-table-edit-td">
					...
					<tr>
						...
						<td align="left" class="l-table-edit-td" colspan="3">
			            	<textarea class="l-textarea" name="brief" id="brief" rows="3" style="width: 380px;">${matDuraStoreDept.brief}</textarea>
						</td>
					</tr>
				</table>
			</form>
