...
<script type="text/javascript">
	...
	function loadHead(){
		grid = $("#maingrid").ligerGrid({
			columns: [ { 
				...
				display: '处置费用', name: 'sum_pay_money', width : 90, align: 'right',
				render : function(rowdata, rowindex, value) {
					return value == null ? "" : formatNumber(value, paraMoney, 1);
				}
			}, { 
				display: '处置收入', name: 'sum_inc_money', width : 90, align: 'right',
				render : function(rowdata, rowindex, value) {
					return value == null ? "" : formatNumber(value, paraMoney, 1);
				}
			}, { 
				...
				display: '审批人', name: 'approve_name', width : 80, align: 'left'
			}, { 
				display: '审批日期', name: 'approve_date', width : 90, align: 'left'
			}, { 
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatDuraScrapDept.do?isCheck=false',
			...
			toolbar: { items: [ { 
				...
				text: '审批（<u>P</u>）', id: 'approve', click: approve, icon: 'audit' 
			}, { 
				line:true 
			}, { 
				text: '销审批（<u>V</u>）', id: 'unApprove', click: unApprove, icon: 'unaudit'
			}, { 
				line:true 
			}, { 
				text: '确认回收（<u>I</u>）', id: 'confirm', click: confirm, icon: 'account' 
			}, { 
				line:true 
			/* }, { 
    ...
    function fieldTypeRender(r, i, value){
        for (var i = 0, l = matDuraScrap_state.Rows.length; i < l; i++){
            var o = matDuraScrap_state.Rows[i];
           // alert(JSON.stringify(matDuraScrap_state.Rows[i]))
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/dura/scrap/dept/addPage.do?isCheck=false',
    ...
    function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMatDuraScrapDept.do", {ParamVo : ParamVo.toString()}, function(responseData) {
    ...
    function approve(){
		...
		if (data.length == 0) {
			...
			if(scrap_nos != ""){
				$.ligerDialog.warn("审批失败！"+scrap_nos+"单据不是审核状态");
				return;
			}
			$.ligerDialog.confirm('确定审批?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("approveMatDuraScrapDept.do", {ParamVo : ParamVo.toString()}, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
    ...
    //销审批
 	function unApprove(){
		...
		if (data.length == 0) {
			...
			if(scrap_nos != ""){
				$.ligerDialog.warn("销审批失败！"+scrap_nos+"单据不是审批状态");
				return;
			}
			$.ligerDialog.confirm('确定销审批?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("unApproveMatDuraScrapDept.do", {ParamVo : ParamVo.toString()}, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
    ...
    function confirm(){
		...
		if (data.length == 0) {
			...
			if(scrap_nos != ""){
				$.ligerDialog.warn("确认失败！"+scrap_nos+"单据不是审批状态！");
				return;
			}
    ...
    function offset(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定冲账?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("offsetMatDuraScrapDept.do", {ParamVo : ParamVo.toString()}, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function loadDict() {
		//字典下拉框
		autocomplete("#dept_code", "../../../queryHosDeptDictByPerm.do?isCheck=false", "id", "text", true, true);
		...
		$("#funk_mark").ligerTextBox({width : 160});
    ...
    function printSet(){
    	...
    	parent.$.ligerDialog.open({url : 'hrp/mat/dura/scrap/dept/printSetPage.do?template_code=041325&use_id='+useId,
    ...
    function print(){
		...
		if (data.length == 0){
	    	...
	    	printTemplate("hrp/mat/dura/scrap/dept/queryDataByPrintTemlate.do?isCheck=false",para); */
		}
    }
</script>
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td" >
				<input name="funk_mark" type="text" id="funk_mark" ltype="text" />
			</td>
		</tr>
		<tr>
			<td align="right" class="l-table-edit-td">
