...
<script type="text/javascript">
			...
			columns: [ {
				...
				{ 
					...
					display: '单价', name: 'price', align: 'left', width: '80',
		 			...
		 			display: '金额', name: 'pay_money', align: 'right', width: '100',
		 			render : function(rowdata, rowindex, value) {
						return value ==null ? "" : formatNumber(value, paraMoney, 1);
					}
		 		}],
			dataAction: 'server', dataType: 'server', usePager: true, url: 'queryMatDuraQueryScrapDetailStore.do',
    ...
    function lodopPrint(){
    	...
    	head=head+"<tr><td>统计年月："+liger.get("year").getText()+liger.get("month").getText()+"</td></tr>";
 		...
 		grid.options.lodop.title=liger.get("year").getText()+liger.get("month").getText()+"耐用品报废明细表(仓库)";
    }
    	...
    	var colspan_num = grid.getColumns(1).length-1;
   		...
   		var printPara={
   			...
   			head:[
				...
				{"cell":0,"value":
					"统计日期: " + $("#begin_make_date").val().split("-")[0]+ "年"+ $("#begin_make_date").val().split("-")[1]+ "月" +
					" 至  "+ $("#end_make_date").val().split("-")[0] +"年"+ $("#end_make_date").val().split("-")[1] + "月","colspan":colspan_num,"br":true}
   			],
   			foot:[
   		...
   		ajaxJsonObjectByUrl("queryMatDuraQueryScrapDetailStore.do?isCheck=false", selPara, function (responseData) {
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <div align="center">
    	<h2>
	    	<span id="year_month_span"></span>耐用品报废明细表(仓库)
    	</h2>
    </div>
	<div id="maingrid"></div>
</body>
