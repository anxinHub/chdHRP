...
<script type="text/javascript">
    	...
    	grid = $("#maingrid").ligerGrid({
			columns: [{
					display: 'reg_id', name: 'reg_id', width: 20, align: 'left', isAllowHide: false, hide: true
		 			...
		 			display: '响应库房', name: 'store_name', align: 'left', width: '120', hide: true
		 		}, { 
					...
					display: '状态', name: 'state', width: 80, align: 'left',
					render: function (rowdata, rowindex, value) {
						if(value == '1'){
							return "未审核";
						}else if(value == '2'){
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatDuraInitStore.do',
	...
	function f_onChangeRow(e){
		if(liger.get("store_code").getValue()){
			...
			e.record.store_id = liger.get("store_code").getValue().split(",")[0];
			e.record.store_no = liger.get("store_code").getValue().split(",")[1];
			ajaxJsonObjectByUrl("saveMatDuraInitStore.do", e.record, function(responseData) {
    ...
    function remove(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					if(is_delete){
						ajaxJsonObjectByUrl("deleteMatDuraInitStore.do",{ParamVo : ParamVo.toString()},function (responseData){
							if(responseData.state=="true"){
	...
	function audit(){
		...
		if (data.length == 0||!store_id){
			...
			$.ligerDialog.confirm('确定审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditMatDuraInitStore.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function unAudit(){
		...
		if (data.length == 0){
			...
			if(inv_names_rows!= ""){
				$.ligerDialog.error("消审失败！<br>选择的记录中存在没有保存的数据");
				return;
			}
			if(inv_names != ""){
			...
			$.ligerDialog.confirm('确定消审?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("unAuditMatDuraInitStore.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function imp() {
		var para = {
			url : 'hrp/mat/dura/init/store/importPage.do?isCheck=false',
			title : '导入耐用品库房期初',
			width : 0,
			height : 0,
			isShowMin : false,
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" >
        <tr>
        	...
        	<td align="right" class="l-table-edit-td">
            	<span style="color: red">*</span>响应库房：
            </td>
            <td align="left" class="l-table-edit-td">
