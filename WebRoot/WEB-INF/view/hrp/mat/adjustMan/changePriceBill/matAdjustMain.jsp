...
<script type="text/javascript">
    	...
    	if(show_detail=="1"){
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    						 ...
    						 { display: '材料名称', name: 'inv_name', align: 'left',minWidth : 120},
						   ...
						   { display: '调价原因(E)', name: 'adjust_reason', align: 'left',minWidth : 80,type: 'string', editor: { type: 'string'}
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAdjust.do?isCheck=true&show_detail=1',
    		...
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAdjust.do?isCheck=true&show_detail=0',
    	                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad:true,
    	                     selectRowButtonOnly:true,//heightDiff: -10,
    	                     toolbar: { items: [
    	                     	{ text: '查询', id:'search', click: query,icon:'search' },
    ...
    function itemclick(item){ 
        if(item.id){
            switch (item.id){
                case "add":
              		$.ligerDialog.open({url: 'matAdjustAddPage.do?isCheck=false',height: 500,top:1,width: 1100, title:'添加',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,buttons: [ { text: '保存', onclick: function (item, dialog) { dialog.frame.saveMatAdjust(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                       ...
                       $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteBatchMatAdjust.do?paramVo="+Param,{},function (responseData){
                ...
                case "check":
                    ...
                    if (data.length == 0){
                       ...
                       $.ligerDialog.confirm('审核将修改材料零售价和计划价,确定修改吗?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("updateMatAdjustState.do?paramVo="+Param,{},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
                    ...
                    return;
                case "import":
                	$.ligerDialog.open({url: 'matAdjustImportPage.do?isCheck=false', height: 500,width: 1100, title:'导入',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,top:10 });
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		$.ligerDialog.open({ url : 'updateMatAdjustPage.do?isCheck=false&adjust_id='+adjust_id+'&state='+state,data:{}, height: 480,top:1,width: 1100, title:'修改',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,buttons: [ { text: '保存', onclick: function (item, dialog) { dialog.frame.saveMatAdjust(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
