...
<head>
    ...
    function loadHead() {
        inMainGrid = $("#maingrid").ligerGrid({
            ...
            dataType: 'server',
            usePager: true,
            url: 'queryMatPayDetailNew.do?isCheck=false&pay_id=${pay_id}',
            width: '99%',
            height: '100%',
            checkbox: true,
            rownumbers: true,
        ...
        if (str != '') {
            ...
            if (error != '') {
                ...
                $.ligerDialog.confirm('确定要保存付款单及付款单明细信息吗?', function(yes) {
                    if (yes) {
                        ajaxJsonObjectByUrl("updateMatPayForAccMain.do?isCheck=false", formPara, function(responseData) {
                            if (responseData.state == "true") {
                                parent.query();
                                ajaxJsonObjectByUrl("updateMatPayForAccDetail.do?isCheck=false", { ParamVo: ParamVo.toString() }, function(responseData) {
                                    if (responseData.state == "true") {}
                                });
                            }
                        });
    ...
    function loadDict() {
      ...
      liger.get("stocker").setValue('${stocker}');
        ...
        autocomplete("#pay_type_code", "../../queryMatPayType.do?isCheck=false", "id", "text", true, true, '', false, '', 200);
...
<body>
    ...
    <div class="l-layout" id="layout1" style="height: 100%;" ligeruiid="layout1">
        <div class="l-layout-content" position="top">
            ...
            <input type="hidden" id="vouch_id" name="vouch_id" value='${vouch_id}' />
