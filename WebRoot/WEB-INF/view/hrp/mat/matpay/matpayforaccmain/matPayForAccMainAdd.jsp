...
<script type="text/javascript">
    ...
    function loadHead() {
        inMainGrid = $("#maingrid").ligerGrid({
            ...
            dataType: 'server',
            usePager: true,
            url: 'queryMatBillMain_Pay.do?isCheck=false&pay_id=${pay_id}',
            width: '100%',
            height: '100%',
            checkbox: true,
            rownumbers: true,
            ...
            if (str != '') {
                ...
                if (error != '') {
                    ...
                    $.ligerDialog.confirm('确定要保存付款单及付款单明细信息吗?', function(yes) {
                        if (yes) {
                            ajaxJsonObjectByUrl("addMatPayForAccMain.do?isCheck=false", formPara, function(responseData) {
                                if (responseData.state == "true") {
                              /*   	parentFrameUse().query(); */
                                    ajaxJsonObjectByUrl("addMatPayForAccDetail.do?isCheck=false", { ParamVo: ParamVo.toString() }, function(responseData) {
                                        if (responseData.state == "true") {
    ...
    function loadDict() {
        ...
        //付款方式下拉框
        autocomplete("#pay_type_code", "../../queryMatPayType.do?isCheck=false", "id", "text", true, true, '', false, "003", 240);
        //付款部门下拉框
