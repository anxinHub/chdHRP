...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	  ...
    	  grid.options.parms.push({name:'sup_no',value:liger.get("sup_id").getValue().split(",")[1]});
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '付款日期', name: 'pay_date', align: 'left',width:100,formatter: "yyyy-MM-dd"
					 		},
					 ...
					 { display: '付款方式', name: 'pay_name', align: 'left',width:96
					 		},
					 { display: '制单人', name: 'maker_name', align: 'left',width:96
					 		},
					 { display: '制单日期', name: 'make_date', align: 'left',width:100,formatter: "yyyy-MM-dd"
					 		},
                     { display: '审核人', name: 'checker_name', align: 'left',width:96
					 		},
                     { display: '审核日期', name: 'chk_date', align: 'left',width:100,formatter: "yyyy-MM-dd"
					 		},
                     ...
                     toolbar: { items: [
		               ...
		               // { line:true },
						...
						{ line:true },
						{ text: '省平台发送（<u>S</u>）', id:'ppaysend', click: ppaysend, icon:'logout' },
	    				//{ line:true },
						//{ text: '模板设置', id:'printSet', click: printSet, icon:'print' },
						{ line:true } ,
						{ text: '批量打印（<u>P</u>）', id:'print', click: print, icon:'print' },
						{ line:true } ,
						{ text: '单据打印', id:'print', click: printBill, icon:'print' },
						//{ line:true } ,
						//{ text: '明细模板设置', id:'printDetailSet', click: printDetailSet, icon:'print' }
    				]},
  ...
  //是否显示复选框
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
    		url : 'hrp/mat/matpay/matpaymain/matPayMainAddPage.do?isCheck=false',
			height: 580,
			width: 1000,
			title:'添加付款单',
			modal:true,
		...
		var count=0;
                    if (data.length == 0){
                        ...
                        $(data).each(function (){	
                        	...
                        	if(this.maker != '${sessionScope.user_id }'){
						...
						if(ParamVo.length >0){
							$.ligerDialog.confirm('确定删除?', function (yes){
	                        	if(yes){
	                            	ajaxJsonObjectByUrl("deleteMatPayMain.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function openUpdate(obj){
     ...
     /* var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matBillMainImportPage.do?isCheck=false&' + parm
				});
				layer.full(index);	
     */
    }
        	...
        	$.ligerDialog.error('请选择行');
        	 ...
        	 $(data).each(function (){
        		...
        		if(count> 0){
					...
					$.ligerDialog.error("只能取消自己审核的单据！");
					
					return;
					
				}
        	 if(str != ''){
              	$.ligerDialog.error('付款单号：<span style="color:red">'+str+'不是已审核状态或已被财务科审核</span>（只有已审核并没被财务科审核状态的付款单允许消核） ');
             }else{
   ...
   /*  //确认
    ...
    function loadDict(){
			...
			autocomplete("#pay_type_code", "../../queryMatPayType.do?isCheck=false", "id", "text", true, true, '', false, '', 240);
    ...
    //键盘事件
	  ...
	  //打印设置
		  ...
		  //打印
		    function print(){
				...
				if (data.length == 0){
					...
					ajaxJsonObjectByUrl("queryMatPayState.do?isCheck=false",{paraId:pay_id.substring(0,pay_id.length-1),state:2},function(responseData){
						if (responseData.state == "true") {
	    ...
	    function printBill() {
	        ...
	        var para = {
	            pay_id: pay_id.substring(0,pay_id.length-1) ,
	  ...
	  /*  function printBill(){
		   ...
		   var heads={
	        		...
	        		"rows": [
	    	          {"cell":0,"value":"仓库:"},
	    	          {"cell":1,"value":""+liger.get("store_code").getText()==''?'空':liger.get("store_code").getText().split(" ")[1]},
	    	          {"cell":3,"value":"制单日期:"+$("#makeBeginDate").val()+"至"+$("#makeEndDate").val()}
			...
			var printPara={
	          		...
	          		class_name: "com.chd.hrp.mat.service.matpay.MatPayMainService",
	       			method_name: "queryMatPayMainByPrintBill",
	       			bean_name: "matPayMainService",
	 ...
	 function exportExcel(){
		...
		var exportPara={
			...
			ajaxJsonObjectByUrl("queryMatPayMain.do?isCheck=false",exportPara,function (responseData){
				 var trHtml='';
			...
			if (data.length == 0){
				...
				$.ligerDialog.confirm('确定要发送吗?', function (yes){
					if(yes){
						...
						ajaxJsonObjectByUrl("ppaySend.do?isCheck=false",param,function (responseData){
							if(responseData.state=="true"){
