...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatPayDetail.do',
    ...
    function add_open(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'matPayDetailAddPage.do?isCheck=false'
				});
				layer.full(index);
    	}
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatPayDetail.do",{ParamVo : ParamVo},function (responseData){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matPayDetailImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matPayDetailImportPage.do?isCheck=false&' + parm
				});
				layer.full(index);	
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryMatPayDetail.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryMatPayDetail.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.pay_id+"</td>"; 
					 trHtml+="<td>"+item.pay_detail_id+"</td>"; 
