...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatInitAffiIn.do?isCheck=true',
    ...
    function lodopPrint(){
    	var head="<table class='head' width='100%'><tr><td>单位：${sessionScope.hos_name}</td></tr>";
 		...
 		grid.options.lodop.title="代销材料期初入库";
    }
...
/* 		hotkeys('P', printDate); */
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/initial/affiin/addPage.do?isCheck=false',
    ...
    function update_open(obj){		
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/initial/affiin/updatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});  
    ...
    function remove(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("deleteMatInitAffiIn.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
			...
			if (data.length == 0){
				...
				$.ligerDialog.confirm('确定入库确认?', function (yes){
					if(yes){
						...
						ajaxJsonObjectByUrl("confirmMatInitAffiInBatch.do",{ParamVo : ParamVo.toString()},function (responseData){
							if(responseData.state=="true"){
								query();
							}
						});
 ...
 /*    //入库确认	
      function confirmData(confirmDate){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定入库确认?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("confirmMatInitAffiIn.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    */
    ...
    function loadDict(){
		//字典下拉框
		autocomplete("#store_code", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true,{is_com : '1',read_or_write:'1'},true,false);
		autocomplete("#sup_code", "../../queryHosSupDict.do?isCheck=false", "id", "text", true, true);
		var paras = {
			table_code : "mat_affi_in",
			field_code : "state",
			notValues : "2"
		}
    ...
    function printSet(){
    	...
    	/*
    	parent.$.ligerDialog.open({url : 'hrp/mat/initial/in/storageInPrintSetPageQc.do?template_code=04009&use_id='+useId,
    ...
    function print(){
	 ...
	 function exportExcel(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","MAT_IN_MAIN.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatAffiMain.do",exportPara,function (responseData){
