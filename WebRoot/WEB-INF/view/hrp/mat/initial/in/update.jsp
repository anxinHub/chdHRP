...
<%-- 	<script src='<%=path%>/lib/SpreadJS/scripts/pluggable/gcspread.sheets.print.9.40.20161.0.min.js' type='text/javascript'></script> --%>
	...
	function  save(){
     	if(validateGrid()){
	        ...
	        ajaxJsonObjectByUrl("updateMatInitIn.do",formPara,function(responseData){
    ...
    function loadDict(){
    	//字典下拉框
    	autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {sel_flag : 'init'}, false, '${matInMain.bus_type_code}');
		autocompleteAsync("#store_code", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true,{is_write:'1'});//, "", false, '${matInMain.store_id},${matInMain.store_no}');
		...
		autocomplete("#sup_code", "../../queryHosSupDictDisable.do?isCheck=false", "id", "text", true, true);//, "", false, '${matInMain.sup_id},${matInMain.sup_no}');
		if("${matInMain.sup_id}"){
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
					...
					grid : {
						columns : [ {
							...
							render : function(rowdata, rowindex, value) {
								return rowdata.plan_price ==null ? "" : formatNumber(rowdata.plan_price, '${p04006}', 1);
							}
						}, {
							display : '是否条码',
							name : 'is_bar',
						...
						url : '../../queryMatInvList.do?isCheck=false&is_com=0&store_id=' 
			...
			dataType : 'server',
			usePager : false,
			url : 'queryMatInitInDetail.do?isCheck=false',
			width : '100%',
			height : '93%',
			checkbox : true,
  ...
  //批量入库确认
    ...
    function confirmData(confirmDate){
		...
		$.ligerDialog.confirm('确定入库确认?', function (yes){
			if(yes){
				...
				ajaxJsonObjectByUrl("confirmMatInitIn.do", formPara, function (responseData){
					if(responseData.state=="true"){
						parentFrameUse().query();
						state = 3;
						change_button();
	...
	function printDate(){
		...
		printTemplate("queryMatInByPrintTemlateQc.do",para); */
	...
	function printSet(){
		...
		/* parent.parent.$.ligerDialog.open({url : 'hrp/mat/initial/in/storageInPrintSetPageQc.do?template_code=04008&use_id='+useId,
	...
	function imp(){
		...
		$.ligerDialog.open({
			...
			url: 'matInitInImprotPage.do?isCheck=false', 
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td" colspan="5">
	            	<input name="brief" type="text" id="brief" ltype="text" value="${matInMain.brief}" required="true" validate="{required:true,maxlength:50}" />
	            </td>
	        </tr>
	    </table>
    </form>
