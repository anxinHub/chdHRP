...
<html style="overflow:hidden;">
    ...
    <script type="text/javascript">
		...
		var priceMsg = "";
 		...
 		if(store_inv.length > 0){
			...
			ajaxJsonObjectByUrl("../../existsMatStoreIncludeInv.do?isCheck=false", para, function (responseData){
				if(responseData.state=="false"){
					msg += "以下材料不在该仓库中！<br>"+responseData.inv_ids+"<br>";
				}
			}, false);
	 			...
	 			if(liger.get("sup_code").getValue()){
		 			para = {
		 	 		...
		 	 		ajaxJsonObjectByUrl("../../existsMatSupIncludeInv.do?isCheck=false", para, function (responseData){
		 	 			if(responseData.state=="false"){
		 	 				msg += "以下材料不属于该供应商！<br>"+responseData.inv_ids+"<br>";
		 	 			}
		 	 		}, false);
	 			}
			}
     ...
     function  save(){
    	if(validateGrid()){
	        ...
	        ajaxJsonObjectByUrl("addMatInitIn.do",formPara,function(responseData){
	            if(responseData.state=="true"){
	            	parentFrameUse().query();
	            	$.ligerDialog.confirm('是否继续添加期初入库单?', function (yes){
	            		if(yes){
	            			this_refresh();
	            		}else{
 ...
 function loadForm(){
    ...
    function loadDict(){
    	//字典下拉框
    	autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {sel_flag : 'init'}, true);
			...
			columns : [{
				...
				editor : {
					...
					grid : {
						...
						url : '../../queryMatInvListInitial.do?isCheck=false&is_com=0&store_id=' 
