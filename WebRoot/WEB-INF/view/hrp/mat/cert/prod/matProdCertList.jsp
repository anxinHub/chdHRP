...
<script type="text/javascript">
	...
	function loadGrid(){
		grid = $("#grid").ligerGrid({
			...
			url:'queryMatProdCertList.do?isCheck=false', delayLoad: true, 
	...
	//修改
	function edit(cert_id, rowindex){
		parent.$.ligerDialog.open({ 
			title: '产品注册证维护',
			url : 'hrp/mat/cert/prod/matProdCertPage.do?isCheck=false',
			height: $(window).height(),
			width: $(window).width(),
			data: {cert_id: cert_id, rowindex: rowindex, is_update: 1}, 
			modal: true, showToggle: false, showMax: false,
			isResize: true, showMin: true, showMax: false, 
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		}); 
	...
	function openInv(index){
		...
		parent.$.ligerDialog.open({ 
			title: '关联材料',
			url : 'hrp/mat/cert/prod/matProdCertInvPage.do?isCheck=false',
	...
	function authent(){
		...
		$.each(data, function (){
			...
			if(this.authent_state == "2"){
				error_msg += this.cert_code+"<br/>";
			}
		})
		
		if(error_msg.length > 0){
			$.ligerDialog.error('以下产品注册证已认证：<br/>'+error_msg);
			return false;
		}
		
		$.ligerDialog.confirm('确定认证?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("updateMatProdCertToAuthent.do?isCheck=false",{ids: ids.substr(0, ids.length - 1)},function (responseData){
	...
	function unAuthent(){
		...
		if(error_msg.length > 0){
			$.ligerDialog.error('以下产品注册证未认证：<br/>'+error_msg);
			return false;
		}
		
		$.ligerDialog.confirm('确定取消认证?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("updateMatProdCertToUnAuthent.do?isCheck=false",{ids: ids.substr(0, ids.length - 1)},function (responseData){
					if(responseData.state=="true"){
				    	$.each(data, function(){
	...
	function check(){
		...
		if(error_msg.length > 0){
			$.ligerDialog.error('以下产品注册证已审核：<br/>'+error_msg);
			return false;
		}
		
		$.ligerDialog.confirm('确定审核?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("updateMatProdCertToCheck.do?isCheck=false",{ids: ids.substr(0, ids.length - 1)},function (responseData){
					if(responseData.state=="true"){
				    	$.each(data, function(){
	...
	function unCheck(){
		...
		if(error_msg.length > 0){
			$.ligerDialog.error('以下产品注册证未审核：<br/>'+error_msg);
			return false;
		}
		
		$.ligerDialog.confirm('确定消审?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("updateMatProdCertToUnCheck.do?isCheck=false",{ids: ids.substr(0, ids.length - 1)},function (responseData){
					if(responseData.state=="true"){
				    	$.each(data, function(){
	...
	function start(){
		...
		if(error_msg.length > 0){
			$.ligerDialog.error('以下产品注册证已启用：<br/>'+error_msg);
			return false;
		}
		
		$.ligerDialog.confirm('确定启用?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("updateMatProdCertToStart.do?isCheck=false",{ids: ids.substr(0, ids.length - 1)},function (responseData){
					if(responseData.state=="true"){
				    	$.each(data, function(){
	...
	function stop(){
		...
		if(error_msg.length > 0){
			$.ligerDialog.error('以下产品注册证已停用：<br/>'+error_msg);
			return false;
		}
		
		$.ligerDialog.confirm('确定停用?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("updateMatProdCertToStop.do?isCheck=false",{ids: ids.substr(0, ids.length - 1)},function (responseData){
					if(responseData.state=="true"){
				    	$.each(data, function(){
	...
	function imp(){
		var para = {
			url : 'hrp/mat/cert/type/matProdCertImportPage.do?isCheck=false',
			title : '产品注册证导入',
			width : 0,
			height : 0,
			isShowMin : false,
	...
	function remove(){
		...
		$.ligerDialog.confirm('确定删除?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("deleteMatProdCert.do?isCheck=false",{ids: ids.substr(0, ids.length - 1)},function (responseData){
					if(responseData.state=="true"){
