...
<script type="text/javascript">
	...
	function loadHead(){
		...
		grid = $("#maingrid").ligerGrid({
    		...
    		dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatFacCertInfo.do?isCheck=false',
	...
	function f_onAdd(){
		...
		var paras = "fac_id=" + fac_id+ "&fac_name=" + fac_name;
	...
	function f_onDel(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('是否确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMatFacCert.do", {
	...
	function f_authent(){
		...
		if (data.length == 0) {
			...
			ajaxJsonObjectByUrl("authentMatCertFac.do", {
				cert_ids : cert_ids.substring(0, cert_ids.length-1)
			}, function(responseData) {
				if (responseData.state == "true") {
					query();
	...
	function f_unAuthent(){
		...
		if (data.length == 0) {
			...
			ajaxJsonObjectByUrl("unAuthentMatCertFac.do", {
				cert_ids : cert_ids.substring(0, cert_ids.length-1)
			}, function(responseData) {
				if (responseData.state == "true") {
					query();
	...
	function f_check(){
		...
		if (data.length == 0) {
			...
			ajaxJsonObjectByUrl("checkMatCertFac.do", {
				cert_ids : cert_ids.substring(0, cert_ids.length-1)
			}, function(responseData) {
				if (responseData.state == "true") {
					query();
	...
	function f_unCheck(){
		...
		if (data.length == 0) {
			...
			ajaxJsonObjectByUrl("unCheckMatCertFac.do", {
				cert_ids : cert_ids.substring(0, cert_ids.length-1)
			}, function(responseData) {
				if (responseData.state == "true") {
					query();
