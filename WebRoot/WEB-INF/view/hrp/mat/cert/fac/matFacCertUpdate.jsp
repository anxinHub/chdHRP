...
<script type="text/javascript">
	...
	$(function() {
		...
		ajaxJsonObjectByUrl("queryCertType.do?isCheck=false", {
			cert_type_code : "${facCert.cert_type_code}"
	...
	function loadDict() {
		...
		liger.get("cert_type_code").setValue("${facCert.cert_type_code}");
		liger.get("cert_type_code").setText("${facCert.cert_type_code}" + " " + "${facCert.cert_type_name}");
		...
		if("${facCert.is_long == 1}"){
			$("#start_date").val("${facCert.start_date}");
			$("#is_long").prop("checked", true);
			$("#end_date").val("");
			$("#end_date").ligerTextBox({ width : 90, disabled: true });
		}else{
			$("#start_date").val("${facCert.start_date}");
			$("#end_date").val("${facCert.end_date}");
		}

		certFiles = $("#files").etUpload({multiple: true});
		if("${facCert.file_path}".length > 0){
			old_cert_files = "${facCert.file_path}".split(",");
			certFiles.setValues(old_cert_files);
		}
		
		$("#save").ligerButton({ click: save, width: 50, disabled: ${facCert.check_state == 1} ? false : true});
	...
	function save(){
		...
		formData.append("cert_id", "${facCert.cert_id}")
		...
		ajaxLigerPostFormData({
			url: "updateMatFacCert.do?isCheck=false", 
			para: formData, 
	...
	<form name="form" method="post" id="form" >
        <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" style="margin-top: 5px;">
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="cert_code" type="text" id="cert_code" type="text" validate="{required:true,maxlength:50}" value="${facCert.cert_code}"/>
	            </td>
	            <td align="left"></td>
	        </tr> 
	        <tr id="nameTr">
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="cert_name" type="text" id="cert_name" type="text" validate="{required:true,maxlength:200}" value="${facCert.cert_name}"/>
	            </td>
	            <td align="left"></td>
	        </tr> 
	        <tr>
	        ...
	        <tr id="busiTr">
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="cert_busi" type="text" id="cert_busi" type="text"  validate="{required:true,maxlength:500}" value="${facCert.cert_busi}"/>
	            </td>
	            <td align="left"></td>
	        </tr>
	      	<tr>
