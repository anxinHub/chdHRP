  ...
  <head>
	...
	function save(){
		if(validateGrid()){
			...
			var formPara = {
					//主表信息
					req_id : '${matDeptMain.req_id}',
					req_code : '${matDeptMain.req_code}',
					dept_id : dept_id ,
					dept_no : dept_no ,
					store_id : store_id,
					store_no : store_no,
		        ...
		        ajaxJsonObjectByUrl("updateMatStoreRequriedPlan.do?isCheck=true", formPara, function(responseData) {
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
		       columns: [ { display: '交易编码', name: 'bid_code', align: 'left', width:'120'},
						 ...
						 },{ display: '供应商', name : 'sup_name', textField : 'sup_name', width : 250, align : 'left',
							 render : function(rowdata, rowindex, value) {	
						 ...
						 url:'queryMatStoreRequriedDetail.do?isCheck=false&req_id='+'${matDeptMain.req_id}',
		                 checkbox : true,
	...
	function f_url(){
		return '../../../queryMatInvListDept.do?isCheck=false&store_id='+liger.get("store_code").getValue().split(",")[0];
	}
	...
	function pimport(){
		...
		+'&store_id=' +store_id
		+'&store_name='+'${matDeptMain.store_name}'
	...
	//字典加载
	function loadDict() {
		autocomplete("#store_code","../../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true, {is_write:'1'},false,'${matDeptMain.stock_id},${matDeptMain.stock_no}','180');//仓库
		//$("#store_code").ligerComboBox({disabled:true});
		
		autocomplete("#dept_code","../../../queryMatDeptDictDate.do?isCheck=false", "id", "text", true, true, {is_last : '1',is_read:'1'},false,false,'180');//部门
