...
<html style="overflow:hidden;">
  <head>  
						 ...
						 { display: '材料名称(E)', name: 'inv_name', align: 'left',width:'150',
							 editor : {
									...
									grid : {
										...
										gid: 'invGrid', 
										url : f_url,//'../../../queryMatInvListDept.do?isCheck=false&store_id='+liger.get("store_code").getValue().split(",")[0],
										pageSize : 100,
		                 ...
		                 toolbar: { items: 
		                	 	[	{text: '保存', id:'save', click: save ,icon:'save' },
		                            ...
		                            /* {text : '科室消耗导入',id : 'kimport',click : kimport ,icon : 'up'}, 
		      						{line : true},*/
		      						{text : '安全库存导入',id : 'simport',click : simport ,icon : 'up'},
		      						{line : true}, 
	...
	function pimport(){
		...
		//alert(parm);
		$.ligerDialog.open({
			url: 'matStoreRequriedPlanSupportImp.do?isCheck=false&'+parm, 
	...
	function simport(){
		...
		+'&dept_no='+liger.get("dept_code").getValue().split(',')[1];
    	$.ligerDialog.open({
    		url: '../../dept/plan/matDeptRequriedPlanSafeImpPage.do?isCheck=false&'+parm,
	...
	//字典加载
	function loadDict() {
		autocomplete("#dept_code", "../../../queryMatDeptDictDate.do?isCheck=false", "id", "text", true, true, {is_last : '1',is_read:'1'});//主管部门
		autocomplete("#store_code","../../../queryMatStoreDictDate.do?isCheck=false","id","text",true,true,{is_write:'1'},true);
    	autodate("#make_date");
    	autodate("#rdate");
