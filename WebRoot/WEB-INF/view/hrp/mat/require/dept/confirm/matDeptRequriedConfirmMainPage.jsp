...
<script type="text/javascript">
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns: [ 
		                 ...
		                 { display: '需求来源', name: 'come_from', align: 'left',width:80 },
						 ...
						 { display: '审核人', name: 'checker', align: 'left' ,width:120,hide:isHideCheck},
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({ 
			...
			width: $(window).width(),
			url : 'hrp/mat/require/dept/confirm/matDeptRequriedConfirmDetail.do?isCheck=false&'+parm, 
	...
	function audit(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要审核吗?', 
            	function (yes){
                    if(yes){
                        ajaxJsonObjectByUrl("submitMatDeptRequriedConfirm.do?isCheck=true",{ParamVo : ParamVo.toString()},
	...
	function unaudit(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定要取消审核吗?', 
            	function (yes){
                   	if(yes){
                        ajaxJsonObjectByUrl("unSubmitMatDeptRequriedConfirm.do?isCheck=true",{ParamVo : ParamVo.toString()},
                        	function (responseData){
	                           	if(responseData.state=="true"){
	                           		query();
	                          		}
	...
	function loadDict() {
		...
		autoCompleteByData("#state", matRequireMain_state.Rows, "id", "text", true, true);
