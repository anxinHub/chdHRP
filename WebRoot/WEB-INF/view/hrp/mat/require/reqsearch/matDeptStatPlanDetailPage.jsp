...
<script type="text/javascript">
	...
	function query() {
    	...
    	grid.options.parms.push({name : 'dept_no',value : liger.get("dept_cide").getValue().split(',')[1]});
    	
    	grid.options.parms.push({name : 'type_id',value : liger.get("type_code").getValue().split(',')[0]}); 
    	grid.options.parms.push({name : 'type_no',value : liger.get("type_code").getValue().split(',')[1]});
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
		       columns: [ 
						 { display: '需求计划号', name: '', align: 'left'},
						 { display: '需求科室', name: 'dept_name', align: 'left' },
						 { display: '材料代码', name: 'inv_code', align: 'left'},
						 { display: '材料名称', name: 'inv_name', align: 'left'},
						 
						 { display: '规格型号', name: 'inv_model', align: 'left' },
						 { display: '计量单位', name: 'inv_unit', align: 'left' },
						 { display: '计划数量', name: '', align: 'right' },
						 { display: '执行数量', name: '', align: 'right' },
						 
						 { display: '计划单价', name: '', align: 'right' },
						 { display: '计划金额', name: '', align: 'right' },
						 { display: '执行金额', name: '', align: 'right' },
						 { display: '状态', name: '', align: 'left' }
	...
	function loadDict() {
		...
		$("#state").ligerComboBox({
           	url: '../../queryMatPlanState.do?isCheck=false',
           	valueField: 'id',
            textField: 'text', 
            selectBoxWidth: 160,
           	autocomplete: true,
...
<body style="padding: 0px; overflow: hidden;">
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit" border="0">
		...
		<tr>
			<td align="right" class="l-table-edit-td" style="padding-left: 10px;">需求部门：</td>
			<td align="left" class="l-table-edit-td">
			...
			<td align="left"></td>
			<td align="left"></td>
			<td align="right" class="l-table-edit-td" style="padding-left: 10px;">物资分类：</td>
			<td align="left" class="l-table-edit-td">
				<input name="type_code" type="text" requried="false" id="type_code" />
			</td>
			<td align="left"></td>
