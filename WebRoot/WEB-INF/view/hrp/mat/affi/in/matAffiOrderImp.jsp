...
<head>
	...
	function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
					...
					url : 'queryMatAffiInOrderN.do?isCheck=false',pageSize:200,
	...
	function closeInvInfo(){
		...
		$.ligerDialog.open({
			url: "matAffiOrderImpCloseInvPage.do?isCheck=false&paras="+paras, 
	...
	function closeInv(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要关闭选中的材料？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("updateMatStorageAffiInCloseInv.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData){
	...
	function save() {
		if (validateGrid()) {
			...
			ajaxJsonObjectByUrl("addMatAffiInByOrder.do?isCheck=true", formPara, function (responseData) {
				if (responseData.state == "true") {
	...
	function loadDict() {
        ...
        autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true,{codes:'27,31'},false,"27");//业务类型
		autocompleteAsync("#store_code", "../../queryMatStoreStocker.do?isCheck=false", "id", "text", true, true, {is_com : 1}, true);
