...
<script type="text/javascript">
    	...
    	if(show_detail=="1"){
    		grid = $("#maingrid").ligerGrid({
				columns: [{
			 		...
			 		},{ display: '供应商', name: 'sup_name', align: 'left', width: '80'
				...
				dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAffiInCommon.do?isCheck=true&show_detail=1',
				...
				toolbar: { items: [
					...
					{ line:true },
					{ text: '入库确认（<u>R</u>）', id:'confirm', click: confirmData,icon:'account' },
					{ line:true },
	    	...
	    	grid = $("#maingrid").ligerGrid({
				...
				dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAffiInCommon.do?isCheck=true&show_detail=0',
    ...
    function isCheckDisplay(rowdata){
  	...
  	function generateMain(){
  		parent.$.ligerDialog.open({
			...
			width : $(window).width(),
			url : 'hrp/mat/affi/in/matAffiDeliveryOrderImpMainPage.do?isCheck=false',
			modal : true,
			showToggle : false,
			showMax : true,
			showMin : false,
    ...
    function orderImp(){
    	parent.$.ligerDialog.open({
			...
			height : $(window).height(),
			width : $(window).width(),
			url : 'hrp/mat/affi/in/matAffiOrderImpPage.do?isCheck=false&',
			modal : true,
			showToggle : false,
			showMax : true,
			showMin : false,
	...
	function loadHotkeys() {
		...
		hotkeys('C', copy);//复制
		hotkeys('V', offSet);//冲销
		hotkeys('R', confirmData);//入库确认
		//hotkeys('T', printSelect);//批量打印
		//hotkeys('P', printDate);//模板打印
	}
    function loadDict(){
		//字典下拉框
		//autocomplete("#store_code", "../../queryMatStore.do?isCheck=false", "id", "text", true, true,{is_com : '1'},false);//仓库
		autocomplete("#store_code", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true, {is_com : '1',read_or_write:'1'}, false);
		autocomplete("#sup_code", "../../queryHosSupDict.do?isCheck=false", "id", "text", true, true);//供应商
		autoCompleteByData("#state", matInMain_state.Rows, "id", "text", true, true);//状态
		autocompleteAsync("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true,{codes:'4,6,8,27,31'},false,"27");//业务类型
       ...
       /*  autocomplete("#inv_code", "../../queryMatInv.do?isCheck=false", "id", "text", true, true, "", false, false, "", "", "160"); */
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/affi/in/matAffiInCommonAddPage.do?isCheck=false',
    ...
    //删除	
    function remove(){
    	//根据参数04011判断是否倒序删除，如果是，则只能删除单据号最大的入库单。否则，医院人为控制单据连续性。
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("deleteMatAffiInCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
    	...
    	if (data.length == 0){
			...
			return ;
				...
				$.ligerDialog.confirm('确定要复制吗?', function (yes){
					if(yes){
						ajaxJsonObjectByUrl("copyMatAffiInCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
							if(responseData.state=="true"){
								query();
							}
						});
    	...
    	if (data.length == 0){
			...
			if(data.length > 1){
				...
				$.ligerDialog.confirm('确定要冲销吗?', function (yes){
					if(yes){
						ajaxJsonObjectByUrl("offsetMatAffiInCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
							if(responseData.state=="true"){
								query();
							}
						});
    ...
    function confirmData(){
		...
		$(data).each(function (){		
			...
			ParamVo.push(
				...
				is_store+ "@" +this.store_id+"@"+this.in_no
			) 
		});
		if(in_nos != ""){
  ...
  //打印设置
    ...
    function print(){
		...
		if (data.length == 0){
	    	...
	    	//printTemplate("hrp/mat/affi/in/queryMatAffiInByPrintTemlate.do?isCheck=false",para);
