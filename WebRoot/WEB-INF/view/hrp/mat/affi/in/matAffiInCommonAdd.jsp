...
<head>
	...
	<script type="text/javascript">
     ...
     function loadDict(){
     	 ...
     	 autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true,{codes:'4,6,8,27,31'},false,"27");//业务类型
     	 //autocompleteAsync("#store_code", "../../queryMatStore.do?isCheck=false", "id", "text", true, true, {is_com : '1'},true);//仓库
 		 autocompleteAsync("#store_code", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true, {is_com : '1',is_write:'1'}, true);	
 		 ...
 		 autocomplete("#sup_code", "../../queryHosSupDictDisable.do?isCheck=false", "id", "text", true, true, "",false,"","","",160);//供应商
 		 autocomplete("#stock_type_code", "../../queryMatStockType.do?isCheck=false", "id", "text", true, true,"",true);//采购类型
 		 //autocomplete("#app_dept", "../../queryMatAppDept.do?isCheck=false", "id", "text", true, true, {is_last : '1'});
 		 autocomplete("#app_dept", "../../queryMatDeptDictDate.do?isCheck=false", "id", "text", true, true, {is_last : '1',is_write:'1'});
 		 //autocomplete("#protocol_code", "../../queryMatProtocolMain.do?isCheck=false", "id", "text", true, true, "","","","","",500);
 		 autocomplete("#proj_code", "../../queryMatProj.do?isCheck=false", "id", "text", true, true,"","","","","",160);
		...
		$("#examiner").ligerTextBox({width:160});
         ...
         $("#proj_code").ligerTextBox({width:160});
     ...
     function  save(){
    	...
    	if(validateGrid()){	
	        ...
	        ajaxJsonObjectByUrl("addMatAffiInCommon.do?isCheck=true",formPara,function(responseData){
		...
		grid = $("#maingrid").ligerGrid({
			columns : [{
				...
				{display : '材料编码',name : 'inv_code',width : 120,align : 'left',
					...
					editor : {
						...
						grid : {
							columns : [ 
								...
								url : '${p04021}' == 0 ? '../../queryMatAffiInvListIn.do?isCheck=false&is_stop=1&is_com=1&store_id='+liger.get("store_code").getValue().split(",")[0]
					...
					render : function(rowdata, rowindex, value) {
						return rowdata.amount_money == null ? "" : formatNumber(rowdata.amount_money, '${p04006}', 1);
					},
					totalSummary: {
						align: 'right',
						render: function (suminf, column, cell) {
    ...
    function invBatch(){
    	...
    	$.ligerDialog.open({
			...
			height: 550,
			width: 1000,
			url: 'matAffiInvBatchImpPage.do?isCheck=false&' + para,
    ...
    function historyImp(){
    	...
    	$.ligerDialog.open({
			...
			height: 550,
			width: 1000, 
			url: 'matAffiHistoryImpPage.do?isCheck=false&' + para,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, top: 1
		});
    }
	...
	function back_imp(){
		$.ligerDialog.open({
			...
			url: 'matAffiInBackImpPage.do?isCheck=false', 
	...
	function f_onAfterEdit(e) {
		if(e.value != "" ){
			...
			if(e.column.name == "sn"){
				if('${p04078 }' != 0){
					var formPara = {sn: e.value};
				ajaxJsonObjectByUrl("../../existsMatSnInv.do?isCheck=false",formPara, function (responseData){
						if(responseData.state=="false"){
							$.ligerDialog.confirm("已存在");
						}
					}, false);
				}
			}
	...
	function orderImp(){
		...
		$.ligerDialog.open({
			...
			height: 550,
			width: 1000,
			url: 'matAffiInCommonOrderImpPage.do?isCheck=false&'+para,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, top: 1
		});  
	}
	...
	function matchImp(){
		...
		$.ligerDialog.open({
			...
			height: 550,
			width: 1000,
			url: 'matAffiInCommonMatchImpPage.do?isCheck=false&'+para,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, top: 1
	...
	function proImp(){
    	...
    	$.ligerDialog.open({
			...
			height: 550,
			width: 1000,
			url: 'matAffiInCommonProImpPage.do?isCheck=false&'+para,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, top: 1
		});
    }
	...
	<form name="form1" method="post"  id="form1" >
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" border="0">
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td" width="20%">
	            	<input class="Wdate" name="in_date" id="in_date" required="true" type="text" onchange="changeIndate()" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
	            </td>
