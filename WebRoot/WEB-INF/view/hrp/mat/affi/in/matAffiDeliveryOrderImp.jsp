...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#store_code").bind("change",function(){
			change_Store(); 
			//grid.columns[4].editor.grid.url = '../../queryMatInvList.do?isCheck=false&is_stop=1&is_com=1&store_id='+liger.get("store_code").getValue().split(",")[0]+'&sup_id='+liger.get("sup_code").getValue().split(",")[0];
			//grid.reRender();
...
// 			});
	...
	function query() {
		...
		grid.options.parms.push({
			name : 'delivery_nos',value : $("#delivery_nos").val()
		});
		//加载查询条件
		...
		grid = $("#maingrid").ligerGrid(
				{
					columns : [  {
						...
						display : '备注',name : 'note',minWidth : 120,align : 'left'
					}, */{
					...
					dataType : 'server',
					usePager : true,
					url : '../../../sup/supdeliverymain/querySupDeliveryMain.do?isCheck=false&bill_type='+liger.get("bill_type").getValue(),
					width : '100%',
					height : '95%',
					checkbox : false,
					rownumbers : true,
		...
		detailGrid =$(batchSn).css({'margin-top':10, 'margin-left':60}).ligerGrid({
			columns : [
						...
						{ 
							...
							display : '仓库编码', name : 'store_code', width : 120, align : 'left'
						},{
							display : '仓库', name : 'store_name', width : 120, align : 'left'
						},
						{
							display : '材料名称',name : 'inv_name',align : 'left',width : '220'
						},
						{
							display : '规格型号',name : 'inv_model',minWidth : 120,align : 'left'
						},
						{
							display : '计量单位',name : 'unit_name',minWidth : 50,align : 'left'
						},
						{
							display : '送货单数量',name : 'delivery_amount',minWidth : 80,type : 'float',align : 'left',
							totalSummary : {
								align : 'left',
						...
						{
							display : '已入库数量(E)',name : 'already_amount',minWidth : 100,type : 'float',align : 'left',
							totalSummary : {
								align : 'left',
								render : function(suminf, column,cell) {
									return '<div>'+ formatNumber(suminf.sum == null ? 0: suminf.sum,0, 0)+ '</div>';
						...
						{
							display : '入库数量(E)',name : 'amount',minWidth : 100,type : 'float',align : 'left',
							editor : {type : 'float',},
							totalSummary : {
								align : 'left',
								render : function(suminf, column,cell) {
						...
						{
							display : '金额(E)',name : 'amount_money',minWidth : 100,align : 'right',
							render : function(rowdata, rowindex,value) {
						...
						{
							display : '注册证号',name : 'cert_code',minWidth : 180,align : 'left',
						}, {
							...
							display : '灭菌批号',name : 'disinfect_no',align : 'left',width: '150',editor : {type : 'text',}
						},{
							display : '条形码(E)',name : 'sn',minWidth : 200,align : 'left',
							editor : {type : 'text',}
						},{
							display : '是否个体码', name : 'is_per_bar', minWidth : 100, align : 'left',
							render : function(rowdata, rowindex, value) {
								if(value == 0){
									return "否";
								}else if(value == 1){
							...
							display : '个体码', name : 'bar_code', minWidth : 140, align : 'left',
						}, {
				...
				dataType : 'server',
				usePager : true,
				url : '../../../sup/supdeliverydetail/queryDetailByStore.do?isCheck=false&is_com=1&delivery_no='+row.delivery_no,
				width : '100%',
				columnWidth:80, 
				checkbox : true,
				rownumbers : true,
...
// 				groupColumnDisplay : '材料名称    '
	...
	function validateGrid(data) {
		...
		if (!liger.get("examiner").getValue()) {
			$.ligerDialog.warn("请先维护仓库验收员！");
			return false;
		}
	...
	function changeStoreCode(rowdata){
		
		$("#delivery_nos").val(rowdata.delivery_no); 
		...
		liger.get("stocker").setText(rowdata.stock_code+" "+rowdata.stock_name);
		
		liger.get("examiner").setValue(rowdata.examiner);
		liger.get("examiner").setText(rowdata.examiner_code+" "+rowdata.examiner_name);
...
// 		liger.get("sup_code").setText("${sup_text}");
		...
		liger.get("bill_type").setValue(2);
		autocomplete("#app_dept", "../../queryHosDeptByPerm.do?isCheck=false","id", "text", true, true, {is_last : 1});

		autoCompleteByData("#is_dir", yes_or_no.Rows, "id", "text", true, true,'', true, false, '160');//是否定向

		autocompleteAsync("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true,{codes:'27,31'},false,"27");//业务类型
		autocompleteAsync("#store_code", "../../queryMatStore.do?isCheck=false", "id", "text", true, true, {is_com : 1}, false);
		 var store_id = liger.get("store_code").getValue().split(",")[0];
		autocomplete("#stocker", "../../queryMatStockEmp.do?isCheck=false", "id", "text", true, true, "",  {store_id :store_id},true);
		...
		$("#bill_type").ligerTextBox({ width: 160 ,disabled:true});
		
		if(!liger.get("store_code").getValue()){
    		var store_id = liger.get("store_code").getValue().split(",")[0]; 
        	autocomplete("#examiner", "../../queryMatExaminerEmp.do?isCheck=false", "id", "text", true, true,{store_id : store_id}, false);
    	}else{
    		autocomplete("#examiner", "../../queryMatExaminerEmp.do?isCheck=false", "id", "text", true, true,'',false);
    	}
		$("#examiner").ligerTextBox({width : 160});
		$("#delivery_nos").ligerTextBox({width : 160});
		//格式化按钮
		/* $("#query").ligerButton({
			click : query,
			width : 90
		...
		function f_detail_onBeforeSubmitEdit(e) {
		 ...
		 //根据仓库改变set验收员
	 	 function change_Store(){
			...
			liger.get("stocker").setText("");
	  		var store_id = liger.get("store_code").getValue().split(",")[0]; 
	 		autocomplete("#stocker", "../../queryMatStockEmpByStore.do?isCheck=false", "id", "text", true, true, {store_id :store_id}, false);//采购员
...
<form name="form1" method="post"  id="form1" >
	<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
		<tr>
			...
			<td align="right" class="l-table-edit-td" width="10%"><span style="color:red">*</span>送货单类型：</td>
			<td align="left" class="l-table-edit-td" width="20%">
				<input name="bill_type" type="text" id="bill_type" ltype="text" validate="{required:false,maxlength:100}" />
			</td>
		</tr>
		<tr>
			...
			<td align="right" class="l-table-edit-td" width="10%">定向科室：</td>
			<td align="left" class="l-table-edit-td" width="20%">
				<input name="app_dept" type="text" id="app_dept" ltype="text" />
			</td>
		</tr>
		<tr>
			<td align="right" class="l-table-edit-td" width="10%"><span style="color: red">单据条码</span>：</td>
			<td align="left" class="l-table-edit-td" width="20%">
				<input name="delivery_no"	type="text" id="delivery_no" ltype="text" validate="{required:true}" />
			</td>
...
<!-- 				</td> -->
			<td align="right" class="l-table-edit-td">送货单号：</td>
			<td align="left" class="l-table-edit-td"><input name="delivery_nos" type="text" id="delivery_nos" ltype="text" validate="{required:true}" /></td>
		</tr>
	</table>
	<hr />
	<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
		...
		<tr>
			<td align="right" class="l-table-edit-td" width="10%"><span id="stocker_span" style="color:red">*</span>采购员：</td>
			<td align="left" class="l-table-edit-td" width="20%">
			...
			<td align="right" class="l-table-edit-td">
				<span style="color:red">*</span>验收员：
			</td>
			<td align="left" class="l-table-edit-td">
				<input name="examiner" type="text" id="examiner" ltype="text" validate="{required:true}" />
			</td>
		</tr>	
	</table>
	</form>
