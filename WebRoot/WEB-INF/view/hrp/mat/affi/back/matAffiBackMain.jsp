...
<script type="text/javascript">
    ...
    function loadHead(){
    	if (show_detail == "1") {
    		grid = $("#maingrid").ligerGrid({
    			...
    			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAffiBack.do?isCheck=false&show_detail=1',
    			...
    			toolbar: { items: [
    				...
    				{ line:true }, 
    				{ text: '退库确认（<u>C</u>）', id:'confirm', click: confirmData, icon:'account' },
    		...
    		grid = $("#maingrid").ligerGrid({
    			...
    			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAffiBack.do?isCheck=false&show_detail=0',
    			width: '100%', height: '100%', checkbox: true,rownumbers:true,
    			delayLoad: true,//初始化不加载，默认false
    			selectRowButtonOnly:true,//heightDiff: -10,
    			checkBoxDisplay:isCheckDisplay,
  ...
  //字段类型渲染器
    function fieldTypeRender(r){
    	 ...
    	 for (var i = 0, l = matInMain_state.Rows.length; i < l; i++){
                var o = matInMain_state.Rows[i];
                if (o.id == r.state) {
                	return o.text;
                }
            }
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/affi/back/matAffiBackAddPage.do?isCheck=false',
    ...
    function update_open(obj){		
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/affi/back/matAffiBackUpdatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, initShowMax:true,isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		}); 
    ...
    function remove(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("deleteMatAffiBack.do",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function copy_no(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定复制?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("copyMatAffiBack.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function audit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditMatAffiBackBatch.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function unAudit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定消审?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("unAuditMatAffiBackBatch.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function confirmData(){
		...
		$.ligerDialog.confirm('确定退库确认?', function (yes){
			if(yes){
				...
				ajaxJsonObjectByUrl("confirmMatAffiBackBatch.do",{ParamVo : ParamVo.toString()},function (responseData){
					if(responseData.state=="true"){
						query();
					}
				});
  ...
  //打印设置
    ...
    function print(){
		...
		if (data.length == 0){
	    	...
	    	//printTemplate("hrp/mat/affi/back/queryMatAffiBackByPrintTemlate.do?isCheck=false",para);
    ...
    function loadDict(){
		//字典下拉框
		autoCompleteByData("#state", matInMain_state.Rows, "id", "text", true, true,null,null,null,"180");
		//alert(liger.get("bus_type_code").getValue());
		autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {codes : '29'}, true,null,"180");
		//autocomplete("#store_code", "../../queryMatStore.do?isCheck=false", "id", "text", true, true,{is_com : '1'},null,null,"180");
		autocomplete("#store_code", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true,{is_com : '1',read_or_write:1},null,null,"180");
        ...
        $("#inv_code").ligerTextBox({width : 219});
