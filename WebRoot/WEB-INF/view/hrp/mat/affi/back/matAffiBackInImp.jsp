 ...
 function loadForm(){
    ...
    function imp(){
    	...
    	$.each(detailData, function(d_index, d_content){ 
    		sqlBuf.append("(in_id = "+this.in_id+")"+" and "+" midet.inv_id = "+this.inv_id) ;
    	...
    	var para={
         ...
         ajaxJsonObjectByUrl("queryMatAffiBackDetailByImp.do?isCheck=false",para,function (responseData){
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [
                ...
                { display: '材料编码', name: 'inv_code', width: 120},
                { display: '材料名称', name: 'inv_name', width: 120},
                { display: '规格型号', name: 'inv_model', width: 80},
                { display: '单价', name: 'price', width: 80},
                { display: '数量', name: 'amount', width: 80},
                { display: '金额', name: 'amount_money', width: 80},
                { display: '批号', name: 'batch_no', width: 80},
                { display: '条形码', name: 'bar_code', width: 80},
                { display: '生产厂商', name: 'fac_name', width: 120},
                { display: '交易编码', name: 'bid_code', width: 80},
                { display: '证件信息', name: 'cert_code', width: 120},
			],
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAffiBackIn.do?isCheck=false',
			fixedCellHeight:true,isScroll : true,selectRowButtonOnly:true,heightDiff: 10,checkbox: true,
			delayLoad: true,//初始化不加载，默认false
			width: '100%', height: '98%', /* detail: { onShowDetail: f_showOutDetail},  */frozen:true,//这个属性有detail明细的时候必须为false,否则明细显示不出来
			toolbar: { items: [
         ...
         gridDetail = $(ele).css({'margin-top':10, 'margin-left':60}).ligerGrid({
             columns:
						...
						dataAction : 'server',dataType : 'server',usePager : false,
			 			parms :para,
			 			url : 'queryMatAffiBackInDetail.do?isCheck=false',fixedCellHeight:true,
			 			width: '95%',height : '100%',
			 			frozen:false
         });  
     }
