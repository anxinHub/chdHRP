...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    ...
    <script type="text/javascript">
  		...
  		$.each(data,function(i, v){
  			...
  			if(v.inv_id){
  				...
  				if (v.inv_id == "" || v.inv_id == null || v.inv_id == 'undefined') {
  	 			...
  	 			msg += rowm;
  	 			if(typeof(v.inva_date) != "undefined" && v.inva_date != null && v.inva_date != ""){
	      			inva_date = v.inva_date;
	      		}else{
	      			inva_date = "";
	      		}
	      		
	      		if(typeof(v.disinfect_date) != "undefined" && v.disinfect_date != null && v.disinfect_date != ""){
	      			disinfect_date = v.disinfect_date;
	      		}else{
	      			disinfect_date = "";
	      		}
	      		
	      		var key=v.inv_id +"|"+v.batch_no+"|"+v.bar_code+"|"+v.price+"|"+inva_date+"|"+disinfect_date;
	      		if(targetMap.get(key)== null || targetMap.get(key) == 'undefined' || targetMap.get(key) == ""){
					targetMap.put(key ,value);
				}else{
					msg += targetMap.get(key)+"与"+value+"材料编码、生产批号、条形码、单价、有效日期、灭菌日期不能重复"+ "\n\r";
				}
  	 			...
  	 			if(targetMap.get(key)== null || targetMap.get(key) == 'undefined' || targetMap.get(key) == ""){
  	 				...
  	 				msg += targetMap.get(key)+"与"+value+"材料编码、生成批号、条形码单价不能重复" + "\n\r";
  	 			} */
  	 			rows = rows + 1;
  			}
     ...
     function  save(){
    	...
    	if(validateGrid()){
	        ...
	        ajaxJsonObjectByUrl("addMatAffiOutCommon.do?isCheck=true",formPara,function(responseData){
    	...
    	$("#dept_id").ligerComboBox({
    		...
    		onSelected: function (selectValue){    			
    			if(selectValue !=null ){    				
        			...
        			autocomplete("#dept_emp", "../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true,dept_emp_paras,false,false,'160');
    			}
			}
    	 });
    	
    	//autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {codes:'5,7,9,28,30,32,33,34'},false,'28');
    	autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {codes:'5,7,9,28,32,33,34'},false,'28');
    	 
		autocomplete("#dept_emp", "../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true); 
    	autocomplete("#use_code", "../../queryMatOutUse.do?isCheck=false", "id", "text", true, true, "", false, false, 160);
    	autocomplete("#proj_code", "../../queryMatProj.do?isCheck=false", "id", "text", true, true,"","","","","",510);
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [{
					display : '材料编码',name : 'inv_code',minWidth : 120,align : 'left',
					totalSummary: {
						align: 'right',
	...
	function choice_inv(){
		...
		$.ligerDialog.open({url: "matAffiOutMainChoiceInvPage.do?isCheck=false&store_id="+store_id+"&store_text="+store_text,
	...
	/* function addInv(){
		...
		//alert(paras);
		$.ligerDialog.open({
			url: "matMatAffiOutInvFifoPage.do?isCheck=false&paras="+paras, height: 450,width: 950, 
	...
	function wholeOut(){
		...
		var para = "store_id=" + liger.get("store_id").getValue() +"&store_text=" + liger.get("store_id").getText() +
		"&dept_id=" + liger.get("dept_id").getValue()+"&dept_text=" + liger.get("dept_id").getText();
		...
		$.ligerDialog.open({
			...
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, top: 1,
			buttons: [ { text: '选择', onclick: function (item, dialog) { dialog.frame.checkMatOutFifo(); },cls:'l-dialog-btn-highlight' }, 
			           { text: '取消', onclick: function (item, dialog) { dialog.close(); } }
			]
		});
	...
	/* function matOutHistoryImport(){		
		...
		$.ligerDialog.open({
			url: "matOutMainHistoryPage.do?isCheck=false&paras="+paras, 
	...
	function paddingInv_beginBar(){
		if(event.keyCode==13){  
			...
			if(store_id && begin_bar){
				...
				ajaxJsonObjectByUrl("../../queryMatAffiOutInvListByBar.do?isCheck=false",para,function (responseData){
					if(responseData.state == "true"){
...
<body onload="is_addRow()">
	...
	<form name="form1" method="post"  id="form1" >
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
	        ...
	        <tr>
				<td align="right" class="l-table-edit-td"  style="padding-left:20px;">物资用途：</td>
