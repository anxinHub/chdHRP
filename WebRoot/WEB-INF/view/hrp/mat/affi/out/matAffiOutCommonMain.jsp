...
/* 	  		grid.options.parms.push({name:'inv_code',value:liger.get("inv_code").getText().split(" ")[0]}); */
    ...
    function loadHead(){
    	if(show_detail=="1"){
    		grid = $("#maingrid").ligerGrid({
    			...
    			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAffiOutCommon.do?isCheck=true&show_detail=1',
    			...
    			toolbar: { items: [
    				...
    				{ line:true },
    				{ text: '出库确认（<u>R</u>）', id:'confirm', click: confirmData,icon:'account' },
    		...
    		grid = $("#maingrid").ligerGrid({
    			...
    			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatAffiOutCommon.do?isCheck=true&show_detail=0',
    ...
    function loadDict(){
		...
		autocomplete("#dept_code", "../../queryMatDeptDictDate.do?isCheck=false", "id", "text", true, true,{is_last : '1',read_or_write:1}, false,null,"200");
	 	autocompleteAsync("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {codes:'5,7,9,28,30,32,33,34'},false,'28',"180");
		autocomplete("#sup_code", "../../queryHosSupDict.do?isCheck=false", "id", "text", true, true,null,null,null,"219");//供应商

		autoCompleteByData("#state", matOutMain_state.Rows, "id", "text", true, true,null,null,null,"180");

		autocomplete("#maker", "../../../sys/queryUserDict.do?isCheck=false",  "id", "text", true, true,null,null,null,"180");
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/affi/out/matAffiOutCommonAddPage.do?isCheck=false',
    ...
    function remove(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("deleteMatAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function audit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定审核吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditMatAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function unAudit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定销审?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("unAuditMatAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function copy(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要复制单据吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("copyMatAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
		...
		$.ligerDialog.confirm('确定出库确认?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("/CHD-HRP/hrp/mat/storage/in/verifyMatClosingDate.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
					if(responseData.state=="true"){
						ajaxJsonObjectByUrl("confirmMatAffiOutCommon.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
							if(responseData.state=="true"){
								query();
							}
						});
