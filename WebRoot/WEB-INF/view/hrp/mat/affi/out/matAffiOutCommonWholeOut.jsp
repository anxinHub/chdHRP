...
<script type="text/javascript">
	...
	function loadHead() {
		...
		grid = $("#maingrid").ligerGrid({
		       columns: [ { display: '入库单号', name: 'in_no',align: 'left', width: 170},
		                  { display: '摘要', name: 'brief',align: 'left', width: 260},
		                  { display: '供应商', name: 'sup_name',align: 'left', width: 310},
		                  { display: '制单人', name: 'maker_name',align: 'left', width: 110},
		                  { display: '入库日期', name: 'in_date',align: 'left', width: 110 },
		                  { display: '确认人', name: 'confirmer_name',align: 'left', width: 110},
		                  { display: '确认日期', name: 'confirm_date',align: 'left', width: 110}
		                ],
		                dataAction: 'server',dataType: 'server',usePager:true,
		                url:'queryMatAffiOutWholeMain.do?isCheck=false',
						isScroll : true, selectRowButtonOnly:true, isSingleCheck : false, heightDiff: 10, checkbox: true,
	                     width: '100%', height: '90%', detail: { onShowDetail: f_showOutDetail},frozen: false,
	                     toolbar: { items: [
	...
	function f_showOutDetail(row, detailPanel,callback){
        ...
        gridDetail = $(ele).css('margin',10).ligerGrid({
             columns:[	 { display: '材料编码', name: 'inv_code',align: 'left',width: 150},
                         { display: '材料名称', name: 'inv_name',align: 'left',width: 250},
                         { display: '规格型号', name: 'inv_model',align: 'left',width: 200},
                         { display: '批号', name: 'batch_no',align: 'left',width: 150 },
                         { display: '批次', name: 'batch_sn',align: 'left',width: 60 },
                         { display: '条形码', name: 'bar_code' ,align: 'left',width: 150},
                         { display: '剩余数量', name: 'imme_amount' ,align: 'right',width: 80},
                   ...
                   parms :para,
			 	   url : 'queryMatAffiOutWholeInv.do?isCheck=false',
	...
	function checkMatOutFifo(){
    	...
    	var para={
         ...
         ajaxJsonObjectByUrl("queryMatAffiOutDetailByIsDir.do?isCheck=false",para,function (responseData){
	...
	function loadDict(){
    	...
    	autocomplete("#store_id", "../../queryMatStore.do?isCheck=false", "id", "text", true, true,"",false,"",'180');    	
    	liger.get("store_id").setValue('${store_id}');
		liger.get("store_id").setText('${store_text}');
    	autocomplete("#dept_id", "../../queryMatDeptDict.do?isCheck=false", "id", "text", true, true,'',false,"",'180');    	
