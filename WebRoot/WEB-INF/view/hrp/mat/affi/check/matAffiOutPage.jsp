   ...
   //键盘事件
    ...
    function  save(){
        ...
        if(validateGrid()){
            ...
            ajaxJsonObjectByUrl("updateMatAffiOutCommon.do?isCheck=true",formPara,function(responseData){
	...
	function loadDict(){
    	...
    	autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true,{codes:'9,28,30,33,34'},false,'${bus_type_code}','180');
    	$("#dept_id").ligerComboBox({
    	 	parms : {is_last : 1},
          	url: '../../queryMatDeptDict.do?isCheck=false',
          	valueField: 'id',
           	textField: 'text', 
           	selectBoxWidth: 180,
          	autocomplete: true,
    		...
    		onSelected: function (selectValue){   			
    			if(selectValue !=null ){   				
    				...
    				autocomplete("#dept_emp", "../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true,dept_emp_paras,true,false,'180');
    			}
			}
    	 });
    	
    	autocomplete("#dept_emp", "../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true,"",false,false,'180');
    	liger.get("dept_emp").setValue('${dept_emp}');
		liger.get("dept_emp").setText('${dept_emp_name}');
    	autocomplete("#use_code", "../../queryMatOutUse.do?isCheck=false", "id", "text", true, true, "", false, "${use_code}", '180');
    	autocomplete("#proj_id", "../../queryMatProj.do?isCheck=false", "id", "text", true, true, "", false, "${proj_id}", '180');
	...
	function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
					...
					type : 'select',
	         		...
	         		grid : {
	         			columns : [ {
	         				...
	         				display : '物资类别', name : 'mat_type_name', minWidth : 80, align : 'left'
	         			}, {
			...
			data:matOutDetail,heightDiff:-20,
	...
	//审核
 	function audit(){
		...
		$.ligerDialog.confirm('确定审核?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("auditMatAffiOutCommon.do", {ParamVo : ParamVo.toString()}, function(responseData) {
					if (responseData.state == "true") {
						$("#state").val('2');
						parent.query();
						state = 2;
	...
	//销审
 	function unAudit(){
		...
		$.ligerDialog.confirm('确定消审?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("unAuditMatAffiOutCommon.do", {ParamVo : ParamVo.toString()}, function(responseData) {
					if (responseData.state == "true") {
						$("#state").val('1');
						parent.query();
						state = 1;
