...
<script type="text/javascript">
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                    ...
                    { display: '盘亏单号', name: 'out_no', align: 'left',width : 150,
                     ...
                     url:'queryMatAffiCheckMain.do?isCheck=true',
                     ...
                     toolbar: { items: [
										...
										{ line:true },
										{ text: '生成出入库单（<u>G</u>）', id:'createInOut', click: createInOut,icon:'create' },
										//{ line:true },
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/affi/check/matAffiCheckMainAddPage.do?isCheck=false',
    ...
    function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteMatAffiCheckMain.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData) {
    ...
    function createInOut(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定生成出入库单?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("createMatAffiInOut.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/affi/check/matAffiCheckMainUpdatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});  
		/* var index = layer.open({
			...
			area : [ '893px', '500px' ],
			content : 'matAffiCheckMainUpdatePage.do?isCheck=false&' + parm.toString()
		});
		layer.full(index); */
	...
	function openInMain(obj){
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/affi/check/matAffiInPage.do?isCheck=false&' + paras.toString(),
	...
	function openOutMain(obj){
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/affi/check/matAffiOutPage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});   
	}
	...
	function loadDict() {
		//字典下拉框
		autocompleteAsync("#store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id","text", true, true, {is_com : '1',read_or_write:1}, true, false, '180');
/* 		autocompleteAsync("#store_id", "../../queryMatStore.do?isCheck=false", "id","text", true, true, {is_com : '1'}, true, false, '180'); */
	...
	function imp() {
		...
		var index = layer.open({
			...
			maxmin : true, //开启最大化最小化按钮
			area : [ '893px', '500px' ],
			content : 'matAffiCheckMainImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
	...
	function printDate() {
		...
		if ($("#resultPrint > table > tbody").html() != "") {
			lodopPrinterTable("resultPrint", "开始打印", "MatCheckMain", true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatCheckMain.do", printPara, function(
				responseData) {
	...
	function exportExcel() {
		...
		ajaxJsonObjectByUrl("queryMatAffiCheckMain.do", exportPara,
