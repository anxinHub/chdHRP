...
<script type="text/javascript">
 ...
 function checkMatOutFifo(){
	//根据选择回冲数据
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定生成盘点单吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("queryMatAffiStoreInvDetail.do?isCheck=false", formPara, function(responseData) {
    ...
    function loadDict(){
    	...
    	autocomplete("#mat_type_id", "../../queryMatTypeDict.do?isCheck=false", "id", "text", true, true,'',false,false,'160');  	
    	autocomplete("#inv_id", "../../queryMatInvDict.do?isCheck=false", "id", "text", true, true,"",false,false,'230');
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [
				...
				{display : '有效日期', name : 'inva_date', minWidth : 80, align : 'left'}, 
			...
			url:'queryMatAffiCheckMainByMatInv.do?isCheck=false',fixedCellHeight:true,
