	...
	var state = "${state}";     
     ...
     function  save(){
        ...
        var formPara={        		
         ...
         ajaxJsonObjectByUrl("updateMatAffiCheckMain.do?isCheck=true",formPara,function(responseData){           
...
/*     	autocompleteAsync("#store_id", "../../queryMatStore.do?isCheck=false", "id", "text", true, true,{is_com : '1'},false, false,'180'); */
    	autocompleteAsync("#store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true,{is_com : '1',is_write:1},false, false,'180');
...
/*     	autocompleteAsync("#dept_id", "../../queryMatDeptDict.do?isCheck=false", "id", "text", true, true,{is_last : 1},false,false,'180'); */
    	...
    	autocomplete("#emp_id", "../../queryMatEmp.do?isCheck=false", "id", "text", true, true,{dept_id: "${dept_id}"},false,false,'180');
    	...
    	$("#dept_id").bind("change",function(){
	    	...
	    	autocomplete("#emp_id", "../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true,para,true,false,'180');
		});
	...
	function printDate(){
		...
		var para={
			check_id : $("#check_id").val(), 
			template_code:'04028',
			class_name:"com.chd.hrp.mat.serviceImpl.affi.check.MatAffiCheckMainServiceImpl",
			method_name:"queryMatAffiCheckByPrintTemlate",
	...
	function printSet(){
		...
		officeFormTemplate({template_code:"04028",use_id:useId});
  ...
  /* 	
	...
	function printSet(){
		...
		parent.parent.$.ligerDialog.open({
			url : 'hrp/mat/affi/check/matAffiCheckPrintSetPage.do?template_code=04028&use_id='+useId,
			data:{}, height: $(parent).height(),width: $(parent).width(), 
			title:'打印模板设置',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
		});
	}  */
...
/* 	function printDate(){
		...
		printTemplate("queryMatAffiCheckByPrintTemlate.do",para);
	}
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
					...
					url:'queryMatCheckDetail.do?isCheck=false',parms :para,
					onsuccess:function(){
						//is_addRow();
					},
    ...
    function checkMobileNum(){
    	if(gridManager.rows.length > 0){
    		...
    		ajaxJsonObjectByUrl("../../storage/checkMobile/queryCheckMmobileCheAmount.do?isCheck=false", mapVo, function(responseData) {
    			var check_detail_data = gridManager.rows;
    	    	$.each(check_detail_data, function(d_index, d_content){ 
    ...
    function createInOut(){	
		...
		$.ligerDialog.confirm('确定生成出入库单?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("createMatAffiInOut.do?isCheck=ture", {ParamVo : ParamVo.toString()}, function(responseData) {
					if (responseData.state == "true") {
						state = 3;
						parent.query();
				    	loadHead();
    ...
    function audit(){
		...
		$.ligerDialog.confirm('确定审核?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("auditMatAffiCheckMain.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData) {
					if (responseData.state == "true") {
						state = 2;
						parent.query();
				    	loadHead();
    ...
    function unAudit(){
		...
		$.ligerDialog.confirm('消审审核?', function(yes) {
			if (yes) {
				ajaxJsonObjectByUrl("unAuditMatAffiCheckMain.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData) {
					if (responseData.state == "true") {
						state = 1;
						parent.query();
				    	loadHead();
    ...
    //引入仓库材料	
    function matCheckByStoreImport(){		
