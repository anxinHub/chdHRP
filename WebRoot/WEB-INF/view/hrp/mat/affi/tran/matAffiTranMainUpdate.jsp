...
<script type="text/javascript">
     ...
     var state = '${matAffiTranMain.STATE}';
   /*   var is_apply = '${is_apply}'; */
     $(function (){
		...
		$("#out_store_id").bind("change",function(){
	    	/* loadHead(); 
	    	grid.reRender(); */
	    	grid.columns[4].editor.grid.url ='../../queryMatAffiOutInvList.do?isCheck=false&store_id=' 
     ...
     function queryDetail(){
  		...
  		grid.options.parms.push({
  			name : 'tran_id',
  			value : '${matAffiTranMain.TRAN_ID}'
  		});
  		grid.options.parms.push({
  			name : 'out_store_id',
  			value : '${matAffiTranMain.OUT_STORE_ID}'
  		});
  		grid.options.parms.push({
  			name : 'in_store_id',
  			value : '${matAffiTranMain.IN_STORE_ID}'
  		});
     ...
     function  save(){
        ...
        var formPara={				
        		...
        		tran_no : $("#tran_no").val(),       		
 				brief : $("#brief").val(),	
 				year : $("#tran_date").val().substr(0, 4),
 				month : $("#tran_date").val().substr(5, 2),
        ...
        ajaxJsonObjectByUrl("updateMatAffiTranMain.do?isCheck=true",formPara,function(responseData){           
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	autoCompleteByData("#bus_type", matTranMain_busType.Rows, "id", "text",true, true,'',false,'${matAffiTranMain.BUS_TYPE}','180');
    	autoCompleteByData("#tran_type", matTranMain_tranType.Rows, "id", "text",true, true,'',false,'${matAffiTranMain.TRAN_TYPE}','180');

		$("#tran_method").ligerComboBox({width:180,disabled:true,cancelable: false});
        liger.get("tran_method").setValue("${matAffiTranMain.TRAN_METHOD}");
        if("${matAffiTranMain.TRAN_METHOD}" == 1){
		...
		//字典下拉框 
		autocompleteAsync("#out_hos_id", "../../queryMatHosInfoDict.do?isCheck=false", "id","text", true, true, "", false, false, '180');
		liger.get("out_hos_id").setValue("${matAffiTranMain.OUT_HOS_ID},${matAffiTranMain.OUT_HOS_NO}");
		liger.get("out_hos_id").setText("${matAffiTranMain.OUT_HOS_CODE} ${matAffiTranMain.OUT_HOS_NAME}");
		
//		autocompleteAsync("#out_store_id", "../../queryMatStore.do?isCheck=false", "id","text", true, true, {is_com : 1}, false, false, '180');
		autocompleteAsync("#out_store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id","text", true, true, {is_com : 1,is_write:1}, false, false, '180');
		liger.get("out_store_id").setValue("${matAffiTranMain.OUT_STORE_ID},${matAffiTranMain.OUT_STORE_NO}");
		liger.get("out_store_id").setText("${matAffiTranMain.OUT_STORE_CODE} ${matAffiTranMain.OUT_STORE_NAME}");
		
		autocompleteAsync("#in_hos_id", "../../queryMatHosInfoDict.do?isCheck=false", "id","text", true, true, "", false, false, '180');
		liger.get("in_hos_id").setValue("${matAffiTranMain.IN_HOS_ID},${matAffiTranMain.IN_HOS_NO}");
		liger.get("in_hos_id").setText("${matAffiTranMain.IN_HOS_CODE} ${matAffiTranMain.IN_HOS_NAME}");
		
		//autocomplete("#in_store_id", "../../queryMatStore.do?isCheck=false", "id","text", true, true, {is_com : 1}, false, false, '180');
		autocomplete("#in_store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id","text", true, true, {is_com : 1,is_write:1}, false, false, '180');
		liger.get("in_store_id").setValue("${matAffiTranMain.IN_STORE_ID},${matAffiTranMain.IN_STORE_NO}");
		liger.get("in_store_id").setText("${matAffiTranMain.IN_STORE_CODE} ${matAffiTranMain.IN_STORE_NAME}");
    ...
    function loadHead(){
		grid = $("#maingrid").ligerGrid({
	           columns: [ 
							      	...
							      	grid :{
								    	columns : [ 
				        	         		...
				        	         		url : '../../queryMatAffiOutInvList.do?isCheck=false&store_id=' 
						         					+ liger.get("out_store_id").getValue().split(",")[0]+'&tran_id=${matAffiTranMain.TRAN_ID}',
				        	         		pageSize : 500,
					...
					heightDiff:-20,
					url : 'queryMatAffiTranDetailByCode.do?isCheck=false&tran_id=${matAffiTranMain.TRAN_ID}&store_id=${matAffiTranMain.OUT_STORE_ID}',
					...
					toolbar: { 
						items: [
						        { text: '保存', id:'add', click: saveMatOutMain,icon:'save' , disabled: state == 1 ? false : true},
						        { line:true },
						        { text: '删除', id:'delete', click: deleteRow,icon:'delete' , disabled: state == 1 ? false : true },
						        { line:true },
						        { text: '整单调拨', id:'imp', click: matTranBySingleImport,icon:'up' },
						        { line:true },
						        { text: '调出确认', id:'audit', click: confirm,icon:'audit', disabled: state == 1 ? false : true },
						        { line:true },
						        { text: '取消调出确认', id:'unAudit', click: unOutConfirm,icon:'unaudit' , disabled: state == 2 ? false : true },
						        { line:true },
						        { text: '调入确认', id:'confirm', click: confirmData,icon:'account' , disabled: state == 2 ? false : true },
						        { line:true },
    ...
    function outConfirm(){
		...
		if('${p04047 }'==0){
			...
			$.ligerDialog.open({
				content: "确认日期:<input id='confirmData' value=" + today + " style='text-align:center; border: 1px solid blue; height: 18px;' onFocus='WdatePicker({isShowClear:true,readOnly:true,dateFmt:\"yyyy-MM-dd\"})' />",
				width: 300,
				height: 150,
				buttons: [
					{ text: '确定', onclick: function (item, dialog) {
						var aa = $("#confirmData").val();
						if (confirmData) {
							dialog.close();
							confirm(aa);
						}
	...
	function confirmData(){
		...
		ParamVo.push(
			...
			confirmDate+"@"+
			is_store +"@"+
			liger.get("in_store_id").getValue().split(",")[0] +"@"+
			liger.get("tran_no").getValue()
		);	
		var ParamVo2 = [];	
		ParamVo2.push(
	 ...
	 function matTranBySingleImport(){//整单出库			
		...
		var paras = store_id;			
		$.ligerDialog.open({
			url: "matAffiTranMainBySinglePage.do?isCheck=false&paras="+paras, 
	...
	/* 	parent.parent.$.ligerDialog.open({url : 'hrp/mat/affi/tran/matAffiTranPrintSetPage.do?template_code=04021&use_id='+useId,
   ...
   <div id="pageloading" class="l-loading" style="display: none"></div>
	<input name="hos_id"  type="hidden" id="hos_id" value="${matAffiTranMain.HOS_ID}" />
	<input name="group_id"  type="hidden" id="group_id" value="${matAffiTranMain.GROUP_ID}" />
	<input name="copy_code"  type="hidden" id="copy_code" value="${matAffiTranMain.COPY_CODE}" />
	<input name="tran_id"  type="hidden" id="tran_id" value="${matAffiTranMain.TRAN_ID}" />
  	<input name="state"  type="hidden" id="state" value="${matAffiTranMain.STATE}" />
	<div id="layout1">
		<div position="top">
				...
				<form name="form1" method="post"  id="form1" >
			       ...
			       <tr>
			            ...
			            <td align="left" class="l-table-edit-td"><input name="tran_no" type="text" id="tran_no" ltype="text" value="${matAffiTranMain.TRAN_NO}"/></td>
			            <td align="left"></td>
			        ...
			        <tr>
			            ...
			            <td align="left" class="l-table-edit-td"><input name="tran_date" type="text" id="tran_date" ltype="text"  validate="{required:true}"  value="${matAffiTranMain.TRAN_DATE}" class="Wdate" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/></td>
			            <td align="left"></td>
			        ...
			        <tr>
			            ...
			            <td align="left" class="l-table-edit-td" colspan="7"><input name="brief" type="text" id="brief" ltype="text"  value="${matAffiTranMain.BRIEF}"/></td>
			            <td align="left"></td>
			        </tr>
			    </table>
			    </form>
