    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                  ...
                  dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatPurMainCheckMain.do?isCheck=false',
    ...
    function audit(){
        ...
        if (data.length == 0){
           ...
           $.ligerDialog.confirm('确定审核?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("checkMatPurMain.do?isCheck=false&paramVo="+Param,{},
...
//     	var vo = obj.split("|");
// 		$.ligerDialog.open({ 
// 			url : 'matPurMainCheckDetailPage.do?isCheck=false&pur_id='+vo[0]+'&state='+vo[1],data:{}, height: 480,width: 1100,top:1,
// 			title:'审核采购计划单',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,
// 			buttons: [ 
// 				//{ text: '打印', onclick: function (item, dialog) { dialog.frame.saveMatPurMain(); },cls:'l-dialog-btn-highlight' }, 
...
// 			] });
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/purchase/check/matPurMainCheckDetailPage.do?isCheck=false&' + paras.toString(),
  ...
  //打印数据
	...
	//加载字典下拉框
	 function loadDict(){
		 ...
		 autocomplete("#planState", "../../queryMatPlanState.do?isCheck=false", "id", "text", true, true);//状态 下拉框		 
