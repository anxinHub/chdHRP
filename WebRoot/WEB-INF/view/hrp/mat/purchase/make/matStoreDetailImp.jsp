  ...
  <head>
    ...
    <script type="text/javascript">
     ...
     $(function (){
        loadDict();//加载下拉框
        loadHead(null);//加载数据
    	/* $("#is_quote").bind('change', function (){
			if($('#is_quote').is(':checked')) {
				is_quote = 1;
			}else{
				is_quote = 0;
			}
     ...
     function  query(){
		...
		if(!liger.get("stock_id").getValue()){
 		...
 		store_text = liger.get("stock_id").getText();
   	    ...
   	    //grid.options.parms.push({name:'is_quote',value : is_quote});
    	 ...
    	 grid = $("#maingrid").ligerGrid({
             columns: [
                       ...
                       { display: '编制日期', name: 'make_date', align: 'left',minWidth : 120},
  					   ...
  					   { display: '仓库',    name: 'store_name', align: 'left',minWidth : 150},
					   ...
					   { display: '交易编码', name: 'bid_code', align: 'left',minWidth : 120},
					   { display: '供应商', name: 'sup_name', align: 'left',minWidth : 120},
					   { display: '生产厂商', name: 'fac_name', align: 'left',minWidth : 120}
               		],
                         dataAction: 'server',dataType: 'server',usePager:true,
                         url:'queryMatReqStoreMain.do?isCheck=false',
	...
	function addNew(){ 
		...
		if (data.length == 0){
			...
			for (var i = 0; i < data.length; i++) {
				if (data[i] && !data[i].req_detail_id) {
					grid.deleteRow(i);
					data = grid.getData();
					i--;
				}
			...
			ajaxJsonObjectByUrl("queryStoreCollectData.do?isCheck=false", formPara, function(responseData) {
				var detailData = responseData;
				var paras ="store_id="+store_id + "&store_no="+store_no + 
					"&store_text="+store_text+"&is_dir="+is_dir+"&is_dept=0";	
		...
		function openUpdate(obj){
			$.ligerDialog.open({
				url : 'matStoreRequireDetail.do?isCheck=false&req_id='+obj,data:{}, height: 420,width: 950,top:1,
