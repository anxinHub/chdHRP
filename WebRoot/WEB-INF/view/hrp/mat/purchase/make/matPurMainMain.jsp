...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	  ...
    	  grid.options.parms.push({name:'store_id',value:liger.get("store_id").getValue().split(",")[0]});//需求库房
    	  ...
    	  grid.options.parms.push({name:'useAudit',value : useAudit});
    	...
    	if(show_detail=="1"){
    		grid = $("#maingrid").ligerGrid({
 	           columns: [ 
						 ...
						 { display: '物资类别', name: 'mat_type_name', align: 'left' ,width:110},
 	                     ...
 	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatPurMain.do?isCheck=true&show_detail=1',
 	                     ...
 	                     toolbar: { items: [
	        				...
	        				{ line:true } ,
	        				{ text: '关闭材料列表', id:'closeInvInfo', click: closeInvInfo, icon:'search' },
	        				{ line:true } 
 	                     	]},  
    		...
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatPurMain.do?isCheck=true&show_detail=0',
    	                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad:true,isScroll:true,
    	                     selectRowButtonOnly:true,//heightDiff: -10,
    	                     toolbar: { items: [
    	                     	{ text: '查询（<u>Q</u>）', id:'search', click: query,icon:'search' },
    ...
    function deleteDetail(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要删除选中的材料？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("deleteMatPurDetailInv.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData){
    ...
    function closeInv(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要关闭选中的材料？', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("updateMatPurCloseInv.do?isCheck=true", {ParamVo : ParamVo.toString()}, function(responseData){
			            if(responseData.state=="true"){
							query();
			            }
					});
    ...
    function closeInvInfo(){
		$.ligerDialog.open({
			url: "matPurMainCloseInvInfoPage.do?isCheck=false", 
	...
	function changeSupCode(rowdata){		
		...
		liger.get("store_id").setValue(rowdata.store_id+","+rowdata.store_no);
		liger.get("store_id").setText(rowdata.store_code+" "+rowdata.store_name);
    ...
    function openSupUpdate(obj){
		...
		$.ligerDialog.open({
			...
			height: 450,
			width: 1000,
			url: 'matPurSupMainPage.do?isCheck=false&' + paras,
            					...
            					if (responseData.work_flag =="4" ){
    	        					$.ligerDialog.warn(  
    	        						responseData.work_msg,  
    	               					function (index, el) {  
    	        							ajaxJsonObjectByUrl("checkMatPurMain.do?isCheck=flase&paramVo="+Param,{},
    ...
    function unaudit(){
        ...
        if (data.length == 0){
           ...
           $.ligerDialog.confirm('确定取消审核?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("cancelCheckMatPurMain.do?isCheck=flase&paramVo="+Param,{},function (responseData){
                		if(responseData.state=="true"){
                			query();
                		}
                	});
    ...
    function pur_add(para){
    	parent.$.ligerDialog.open({
    		url: 'hrp/mat/purchase/make/matRequireProdAddPage.do?isCheck=false&para='+para+'&is_dir='+is_dir,
    ...
    function addNew(){
    	parent.$.ligerDialog.open({
  			url: 'hrp/mat/purchase/make/addMatPurMainPage.do?isCheck=false',
			height: $(window).height(), 
			width: $(window).width(), 
        ...
        if (data.length == 0){
        	...
        	var Param =[];
             ...
             $.ligerDialog.confirm('确定删除?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("deleteMatPurMain.do?isCheck=false&paramVo="+Param,{},function (responseData){
                		if(responseData.state=="true"){
                			query();
                		}
                	});
    ...
    function suspend(){
        ...
        if (data.length == 0){
           ...
           $.ligerDialog.confirm('确定中止计划?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("updateMatPurPlanState.do?isCheck=false&paramVo="+Param,{},function (responseData){
                		if(responseData.state=="true"){
                			query();
                		}
                	});
  ...
  //按基数生成采购计划
    function secuLimitGen(){
    	$.ligerDialog.open({url: 'matPurPlanGenBySecuLimitPage.do?isCheck=false', height: 300,width: 500, 
    ...
    function produceDept(){
    	parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/purchase/make/matDeptDetailImpPage.do?isCheck=false', 
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
    }
    ...
    function produceStore(){
    	parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/purchase/make/matStoreDetailImpPage.do?isCheck=false', 
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
    }
    ...
    function openUpdate(obj){ 	
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/purchase/make/updateMatPurMainPage.do?isCheck=false&' + paras.toString(),
    ...
    function  purImpN(){
		...
		"&sup_no="+(liger.get("sup_code").getValue().split(",")[1] == "" ? "" :liger.get("sup_code").getValue().split(",")[1]) + 
        ...
        if (data.length == 0){
        	...
        	var Param =[];
       	 	var flag;
       	 	var is_com
       	 	var isSame = true ;//是否 全部为代销材料 或非代销材料   是则生成相应代销备货单 或普通订单  否则 不能生成订单
       		for(var i=0 ; i<data.length ;i++){
       			is_com = data[i].is_com;
				break;
		  	}
            ...
            if(!isSame){
            	$.ligerDialog.error('请选择单据中的材料不是全部为代销材料或非代销材料！');
            ...
            ajaxJsonObjectByUrl("queryPurDetailCollectData.do?isCheck=false", formPara, function(responseData) {
				...
				parent.$.ligerDialog.open({ 
					...
					height: $(window).height(),
					width: $(window).width(),
					url : 'hrp/mat/purchase/make/addOrderPage.do?isCheck=false&'+paras +'&is_com='+is_com,
					data : detailData,
					modal:true,showToggle:false,showMax:true ,showMin: false,isResize:true,
					parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
				});
    ...
    //键盘事件
	 ...
	 function loadDict(){
         //字典下拉框
         if(useAudit=="0"){
        	 autoCompleteByData("#planState", matPurMain_state0.Rows, "id", "text", true, true,"",false,"2");
         }
         if(useAudit=="1"){
        	 autoCompleteByData("#planState", matPurMain_state1.Rows, "id", "text", true, true,"",false,"2");
         }
		 ...
		 autocomplete("#pur_hos_id","../../queryMatHosInfoDict.do?isCheck=false","id","text",true,true);//采购单位下拉框 
		 autocomplete("#req_hos_id","../../queryMatHosInfoDict.do?isCheck=false","id","text",true,true);//请购单位下拉框 
		 ...
		 loadComboBox({id:"#sup_code",url:"../../queryHosSupDict.do?isCheck=false",value:"id",text:"text",autocomplete:true,hightLight:true,selectBoxWidth:'247',maxWidth:'250',defaultSelect:false,async:false})
	...
	function printSetDetail(){
		...
		officeFormTemplate({template_code:"040031",use_id:useId});
	}
	...
	function printDetail(){
		...
		if (data.length == 0){
			...
			var para={
				class_name:"com.chd.hrp.mat.service.purchase.make.MatPurMainService",
				method_name:"queryMatPurMainDetail",
				bean_name:"matPurMainService",
				paraId: pur_detail_ids.substring(0,pur_detail_ids.length-1) ,
				template_code:'040031',
