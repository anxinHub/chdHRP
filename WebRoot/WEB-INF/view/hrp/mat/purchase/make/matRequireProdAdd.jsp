	...
	function f_onSelectRow_detail(data, rowindex, rowobj) {
	 ...
	 function save() {
	    ...
	    if(allData.length != 0){	    		
		    $(allData).each(function(d_index, d_content){	
		    	if(this.inv_id){
					...
					var mat_pare = '${p04013 }';
					if(mat_pare == 1){
				    	if(matPurMap.get(d_index)!= 0 && matPurMap.get(d_index)!= null && matPurMap.get(d_index)!= ""){ 
							var price = matPurMap.get(d_index);
							if(this.price > price){
								$.ligerDialog.error('第' + row_index +'行采购材料单价大于协议中供应商设定的单价');	
								return flag = false;
							}
				 		}  
			    	}
	    ...
	    var formPara = {
				...
				store_id : liger.get("store_id").getValue().split(",")[0],//需求仓库
		...
		ajaxJsonObjectByUrl("addProdMatPurMain.do?isCheck=false", formPara,function(responseData) {
				if (responseData.state == "true") {
						...
						parentFrameUse().parentFrameUse().query();
						this_close();
				}
			});
		}
		...
		function loadDict() {
			...
			autocompleteAsync("#store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true);
			if("${store_id}"){
		...
		function openUpdate(obj){		    	
			$.ligerDialog.open({ 
				url : 'matInvCurAmountDetailPage.do?isCheck=false&inv_id='+obj,data:{}, 
				height: 480,width: 980,top:1, title:'材料当前库存明细',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true
			});		    
		}
		...
		function openDetail(obj,rowindex){		
			 ...
			 $.ligerDialog.open({ 
				url : 'matRequireAmountDetailPage.do?isCheck=false&req_rela='+req_rela+'&rowindex='+rowindex,data:{}, 
