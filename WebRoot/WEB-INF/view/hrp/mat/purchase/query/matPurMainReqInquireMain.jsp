...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
					 ...
					 { display: '材料编码', name: 'brif', align: 'left'
					 		},
					 { display: '材料名称', name: 'plan_type', align: 'left'
						 	},
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatPurMain.do',
                ...
                case "check":
                    ...
                    if (data.length == 0){
                       ...
                       $.ligerDialog.confirm('确定审核?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("checkMatPurMain.do?paramVo="+Param,{},function (responseData){
                ...
                case "cancelCheck":
                    ...
                    if (data.length == 0){
                       ...
                       $.ligerDialog.confirm('确定取消审核?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("cancelCheckMatPurMain.do?paramVo="+Param,{},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		$.ligerDialog.open({ url : 'matPurMainCheckDetailPage.do?isCheck=false&pur_id='+obj,data:{}, height: 500,width: 1100,top:1,title:'修改',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,buttons: [ { text: '打印', onclick: function (item, dialog) { dialog.frame.saveMatPurMain(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
  ...
  //打印数据
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryMatStoreMatch.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.type_code+"</td>"; 
					 trHtml+="<td>"+item.type_name+"</td>"; 
