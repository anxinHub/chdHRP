...
<html style="overflow:auto;">
  <head>
    ...
    $(function (){
		...
		if("${matInv.is_per_bar}" == 1){
			$("#is_per_bar").prop("checked", true);
		}
		if("${matInv.is_inv_bar}" == 1){
			$("#is_inv_bar").prop("checked", true);
		}
		if("${matInv.is_quality}" == 1){
			$("#is_quality").prop("checked", true);
		}
		if("${matInv.is_disinfect}" == 1){
			$("#is_disinfect").prop("checked", true);
		}
		if("${matInv.is_cert}" == 1){
			$("#is_cert").prop("checked", true);
		}
		if("${matInv.is_sec_whg}" == 1){
			$("#is_sec_whg").prop("checked", true);
		}
		if("${matInv.is_shel_make}" == 1){
			$("#is_shel_make").prop("checked", true); 
		}
		
		$("#change_spell_code").bind("change",function(){
			if($("#change_spell_code").prop("checked")){
				$("#spell_code").prop("disabled",false);
			}else{
				$("#spell_code").prop("disabled",true);
			}
		})
	...
	function loadHead(){
		grid = $("#maingrid").ligerGrid({
			...
			url: "queryMatInvSup.do?isCheck=false",
			async:false,
			...
			toolbar: { items: [
				{ text: '删除', id:'delete', click: deleteRow,icon:'delete',disabled: (state == 1 ) ? true : false },{ line:true },
				{ text: '添加行', id:'add', click: addCenterRow, icon:'add',disabled: (state == 1 ) ? true : false },{ line:true },
				{ text: '保存', id:'save', click: saveSup, icon:'save' ,disabled: (state == 1 ) ? true : false},{ line:true }
			]}
		});

		gridManager = $("#maingrid").ligerGetGridManager();
		
		certgrid = $("#certgrid").ligerGrid({
			columns: [ {
	            	display: '证件号', name: 'cert_code', align: 'left', width:260
				}, { 
	            	display: '产品名称', name: 'prod_name', align: 'left', width:250
				}, { 
					display: '证件类型编码', name: 'cert_type_code', align: 'left',width:60,hide:true
				}, { 
					display: '证件类型', name: 'cert_type_name', align: 'left', width:160
				}, { 
					display: '证件日期', name: 'cert_date', align: 'left',width:140,
				}, { 
					display: '生产厂商', name: 'fac_name', align: 'left', width:250
				},{ 
					display: '状态', name: 'is_stop', align: 'left',width:60,
			...
			dataAction: 'server',dataType: 'server',usePager:false,
			url:'queryMatInvCertInfo.do?isCheck=false&inv_id='+'${matInv.inv_id}',
			async:false,
			width: '98%', height: '200', checkbox: false,rownumbers:true,
			enabledEdit : false, isAddRow:false, selectRowButtonOnly:true
		});
		certGridManager = $("#certgrid").ligerGetGridManager();  
	...
	function saveSup() {
		...
		var formPara = {
	 	...
	 	ajaxJsonObjectByUrl("addMatInvSup.do?isCheck=false", formPara,function(responseData) {
    ...
    function save(){
        ...
        if($("form").valid()){
	        ...
	        var formPara={
				...
				plan_price : $("#plan_price").val() == "" ? "0.00" : $("#plan_price").val(),
	    		...
	    		spell_code:$("#spell_code").val().toUpperCase(), 
	    		change_spell_code:$("#change_spell_code").prop("checked")?"1":"0"
	        }; 
	        ajaxJsonObjectByUrl("updateMatInv.do",formPara,function(responseData){
	            
	            if(responseData.state=="true"){
		          /* 即墨需求修改材料信息以后页面停留在当前页面的材料页数 */
    ...
    function loadDict(){
		...
		liger.get("mat_type_code").setValue("${matInv.mat_type_id},${matInv.mat_type_no}");
		liger.get("mat_type_code").setText("${matInv.mat_type_name}");
		liger.get("instru_type_id").setValue("${matInv.instru_type_id}");
		liger.get("instru_type_id").setText("${matInv.instru_type_text}");
    	var paras = {
			field_code : "price_type"
		}
		autocomplete("#price_type", "../../../queryMatSysList.do?isCheck=false", "id", "text", true, true, paras, false, "${matInv.price_type}");
		paras = {
			field_code : "amortize_type"
		}
		autocomplete("#amortize_type", "../../../queryMatSysList.do?isCheck=false", "id", "text", true, true, paras, false, "${matInv.amortize_type}");
		autocomplete("#unit_code", "../../../queryHosUnit.do?isCheck=false", "id", "text", true, true, "", false);
		liger.get("unit_code").setValue("${matInv.unit_code}");
		liger.get("unit_code").setText("${matInv.unit_name}");
    	
		
		autocomplete("#fac_code", "../../../queryHosFacDict.do?isCheck=false", "id", "text", true, true, "", false);

		
		liger.get("fac_code").setValue("${matInv.fac_id},${matInv.fac_no}");
		liger.get("fac_code").setText("${matInv.fac_name}");
        $("#use_state").ligerComboBox({width:160, disabled:true, cancelable: false});
        liger.get("use_state").setValue("${matInv.use_state}"); 
		liger.get("use_state").setText("${matInv.use_state}" == 1 ? "是" : "否");
		//autocomplete("#use_state", "../../../queryMatYearOrNo.do?isCheck=false", "id", "text", true, true, "", false, "${matInv.use_state}");
		autoCompleteByData("#abc_type", matInv_ABCType.Rows, "id", "text", true, true, "", false, "${matInv.abc_type}");
		autocomplete("#manage_type", "../../../queryMatManageType.do?isCheck=false", "id", "text", true, true, "", false, "${matInv.manage_type}");
		autoCompleteByData("#source_plan", matInv_sourcePlan.Rows, "id", "text", true, true, "", false, "${matInv.source_plan}");
	...
	//	$("#bar_code_new").ligerTextBox({width:160});
		...
		$("#mat_type_code").ligerComboBox({width:160, disabled:true, cancelable: false});
    ...
    //改变物资类别数值触发改变材料编码事件(根据最大编码号累加，而非总数累加)
	function changeMatTypeCode(){
    	var inv_code = $("#inv_code").val();
		var type_code = liger.get("mat_type_code").getValue().split(",")[0];
		$("#mat_type_code").bind("change",function() {
			var paramVo = {
				mat_type_id:liger.get("mat_type_code").getValue().split(",")[0]	
			};
			if(paramVo.mat_type_id == type_code){
				liger.get("inv_code").setValue(inv_code);
			}else{
				ajaxJsonObjectByUrl("queryChangeMatTypeCode.do?isCheck=false",paramVo,function(responseData){
  ...
  //改变启用日期事件
	...
	function change_open(){
		...
		$.ligerDialog.open({
			...
			height: 400,
			width: 800,
			url: 'matInvDictMainPage.do?isCheck=false&' + paras.toString(),
	...
	function openUpdate(){
		...
		var paras ="&inv_id="+'${matInv.inv_id}'
			+"&"+"fac_id="+'${matInv.fac_id}' 
			+"&"+ "fac_no="+'${matInv.fac_no}'
			+"&"+"fac_text="+liger.get("fac_code").getText()
			+"&"+"group_id="+'${matInv.group_id}' 
			+"&"+ "hos_id="+'${matInv.hos_id}'
			+"&"+ "copy_code="+'${matInv.copy_code}'; 
			parent.$.ligerDialog.open({
				url: 'hrp/mat/info/basic/inv/matInvCertInfoMainPage.do?isCheck=false&'+paras, 
			...
			height: 400,
			width: 800,
			url: 'matInvCertInfoMainPage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, top: 1 ,
			buttons: [ 
					{ text: '保存', onclick: function(item, dialog){dialog.frame.save();}, cls:'l-dialog-btn-highlight' },
					{ text: '关闭', onclick: function (item, dialog) { dialog.close(); } } 
	    ...
	    /* var fac_id = ${'#fac_code'}.liger.get
	    var fac_text = 
		parent.$.ligerDialog.open({
			url: 'hrp/mat/info/basic/inv/matInvCertInfoMainPage.do?isCheck=false&inv_id='+'${matInv.inv_id}'+'&fac_id', 
			height: 300,width:950, title:'材料证件对应关系',
         	modal:true, showToggle:false, showMax:true ,showMin:false, isResize:true,
         	parentframename: window.name,
       		buttons: [ 
	 ...
	 function stopTimes(){
		...
		function audit(){
			...
			var formPara={
	     		...
	     		hos_id : "${matInv.hos_id}",
	     		copy_code : "${matInv.copy_code}",
	     		inv_id : "${matInv.inv_id}",
	     		flag : 1 ,
	     		is_stop : 0
	 		};
			$.ligerDialog.confirm('确定审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditMatInv.do?isCheck=false", formPara, function (responseData){
						if(responseData.state=="true"){
							parentFrameUse().query();
							state = 1;
							change_button();
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" style="padding: 10px" border="0">
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td" width="20%">
	            	<input name="inv_code" type="text" id="inv_code" ltype="text" required="true" value="${matInv.inv_code}" validate="{required:true,maxlength:20}" />
	            </td>
	            <td align="right" class="l-table-edit-td"  width="10%">
	            	<span style="color:red">*</span>材料名称：
	            </td>
	            <td align="left" class="l-table-edit-td" width="20%">
	            	<input name="inv_name" type="text" id="inv_name" ltype="text" required="true" value="${matInv.inv_name}" validate="{required:true,maxlength:50}" onmouseover="showTip(event, this)" onmouseout="hideTip(event)"/>
	            </td>
	            <td align="right" class="l-table-edit-td"  width="10%">
	            	别名：
	            </td>
	            <td align="left" class="l-table-edit-td" width="20%">
	            	<input name="alias" type="text" id="alias" ltype="text"  value="${matInv.alias}"  validate="{required:false}" onmouseover="showTip(event, alias)" onmouseout="hideTip(event)"/>
	            </td>
	        </tr> 
	        <tr>
			...
			<tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="inv_model" type="text" id="inv_model" ltype="text" value="${matInv.inv_model}" validate="{required:false,maxlength:200}" onmouseover="showTip(event, inv_model)" onmouseout="hideTip(event)"/>
	            </td>
	            <td align="right" class="l-table-edit-td" >
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="plan_price" type="text" id="plan_price" ltype="text" value="${matInv.plan_price}"  validate="{required:false,maxlength:20}" onchange="getSellPrice()"/>
	            </td>
	            <td align="right" class="l-table-edit-td" >
	            	加价率：
	            </td>
	            <td align="left" class="l-table-edit-td">
	            	<input name="price_rate" type="text" id="price_rate" ltype="text" value="${matInv.price_rate}" validate="{required:false,maxlength:20}" onchange="getSellPrice()"/>
	            </td>
	            <td align="right" class="l-table-edit-td" >
	            	零售价：
	            </td>
	            <td align="left" class="l-table-edit-td">
	            	<input name="sell_price" type="text" id="sell_price" ltype="text" value="${matInv.sell_price}" validate="{required:false,maxlength:20}" />
	            </td>
	        </tr> 
	        <tr>
	        	 <td align="right" class="l-table-edit-td"  >
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="sdate" id="sdate" type="text" onchange="change_eDate()" value="${matInv.sdate}" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
	            </td>
	            <td align="right" class="l-table-edit-td"  >
	            	停用日期：
	            </td>
	            <td align="left" class="l-table-edit-td">
		            <input class="Wdate" name="edate" id="edate" type="text" value="${matInv.edate}" onchange="changeDate()" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
	            </td>
	            <td align="right" class="l-table-edit-td" >
	            	代理商：
	            </td>
	            <td align="left" class="l-table-edit-td">
	            	<input name="agent_name" type="text" id="agent_name" ltype="text" value="${matInv.agent_name}" validate="{required:false,maxlength:60}" />
	            </td>
	        </tr>
	        <tr>
	            <td align="right" class="l-table-edit-td" >
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="brand_name" type="text" id="brand_name" ltype="text" value="${matInv.brand_name}" validate="{required:false,maxlength:60}" />
	            </td>
	            <td align="right" class="l-table-edit-td" >
					材料用途：
				</td>
	            <td align="left" class="l-table-edit-td">
	            	<input name="inv_usage" type="text" id="inv_usage" ltype="text" value="${matInv.inv_usage}" validate="{required:false,maxlength:60}" />
	            </td>
	            <td align="right" class="l-table-edit-td" >
	            	包装规格：
	            </td>
	            <td align="left" class="l-table-edit-td">
	            	<input name="inv_structure" type="text" id="inv_structure" ltype="text" value="${matInv.inv_structure}" validate="{required:false,maxlength:60}" />
	            </td>
	        </tr> 
	        <tr>
	            <td align="right" class="l-table-edit-td" >
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="per_weight" type="text" id="per_weight" ltype="text" value="${matInv.per_weight}" validate="{required:false,maxlength:20}" />
	            </td>
	            <td align="right" class="l-table-edit-td"  >
	            	单位体积：
	            </td>
	            <td align="left" class="l-table-edit-td">
	            	<input name="per_volum" type="text" id="per_volum" ltype="text" value="${matInv.per_volum}" validate="{required:false,maxlength:20}" />
	            </td>
	            <td align="right" class="l-table-edit-td"  >
	            	ABC分类：
	            </td>
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="stora_tran_cond" type="text" id="stora_tran_cond" value="${matInv.stora_tran_cond}" ltype="text" validate="{required:false,maxlength:50}" />
	            </td>
	        </tr>
	        <tr>
	            <td align="right" class="l-table-edit-td"  >
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="bid_date" id="bid_date" type="text" vaonFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" value="${matInv.bid_date}"/>
	            </td> 
	            <td align="right" class="l-table-edit-td"  >
	            	项目中标编码：
	            </td>
	            <td align="left" class="l-table-edit-td">
	            	<input name="bid_code" type="text" id="bid_code" ltype="text" validate="{required:false,maxlength:50}" value="${matInv.bid_code}"/>
	            </td>
	        </tr>
	        <tr>
	            <td align="right" class="l-table-edit-td"  >
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="memory_encoding" type="text" id="memory_encoding" ltype="text" validate="{required:false,maxlength:50}" value="${matInv.memory_encoding}"/>
	            </td>
                <td align="right" class="l-table-edit-td"  >
	            	计划来源：
	            </td>
			...
			<tr>
			   ...
			   <td align="right" class="l-table-edit-td"  width="10%">
	            ...
	            <td align="left" class="l-table-edit-td" width="20%">
	            	<input name="bar_code_new" type="text" id="bar_code_new" ltype="text" value="${matInv.bar_code_new}" />
	            </td>
			   <td align="right" class="l-table-edit-td"  width="10%">
	            	拼音码：
	            </td>
	            <td align="left" class="l-table-edit-td" width="20%">
	            	<input name="spell_code" type="text" id="spell_code" ltype="text" value="${matInv.spell_code}" disabled/>
	            	<input name="change_spell_code" type="checkbox" id="change_spell_code" ltype="text" />更改拼音码
	            </td>
	            <td align="right" class="l-table-edit-td"  width="10%">
	            	<span style="color:red"></span>医保收费代码：
	            </td>
	            <td align="left" class="l-table-edit-td" width="20%">
	            	<input name="charge_code" type="text" id="charge_code" ltype="text" value="${matInv.charge_code}"/>
	            </td>
			</tr>
			<tr>
				<td align="right" class="l-table-edit-td"  width="10%">
	        ...
	        <tr>
	        	<td colspan="6" align="left" class="l-table-edit-td" >
	        		<input type="hidden" name="group_id" id="group_id" value="${matInv.group_id}">
	        		<input type="hidden" name="hos_id" id="hos_id" value="${matInv.hos_id}">
	        		<input type="hidden" name="copy_code" id="copy_code" value="${matInv.copy_code}">
	        		<input type="hidden" name="inv_id" id="inv_id" value="${matInv.inv_id}">
	        		<input type="hidden" name="inv_no" id="inv_no" value="${matInv.inv_no}">
	        		<input type="hidden" name="old_is_bar" id="old_is_bar" value="${matInv.is_bar}">
	        		<input type="hidden" name="old_is_com" id="old_is_com" value="${matInv.is_com}">
	        	</td>
	        </tr>
	    </table>
    </form>
