  ...
  <head>
    ...
    function loadHead(){
     	grid = $("#maingrid").ligerGrid({
            columns: [ { 
            	...
            	display: '证件产品名称', name: 'prod_name', align: 'left', width:250,editor : {type : 'string'}
			}, { 
				display: '证件类型编码', name: 'cert_type_code', align: 'left',width:60,hide:true
			}, { 
				display: '证件类型', name: 'cert_type_name', align: 'left', width:160, valueField : 'id',textField : 'text',
				editor : {
					type : 'select',
					valueField : 'id',
					textField : 'text',
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatInvCertInfo.do?isCheck=false&inv_id='+'${inv_id}',
	 ...
	 function validateGrid(){
	    	 //明细
	   		...
	   		$.each(data,function(i, v){
	   			...
	   			if (v.is_stop == null || v.is_stop == "" || v.is_stop == "undefined") {
					rowm+="[状态]、";
				}  
				if(rowm != ""){
	...
	function save(){
		...
		if(!validateGrid(data)){
   	 		...
   	 		// 除去 材料修改页面 与 证件信息页面的 重复供应商信息 用Map
