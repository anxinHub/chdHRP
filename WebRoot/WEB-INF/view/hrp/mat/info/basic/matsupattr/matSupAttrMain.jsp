...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	grid.options.parms.push({name:'sup_id',value:liger.get("sup_id").getValue()}); //获取下拉列框的值
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '供应商编码', name: 'sup_code', align: 'left',width: 100,
                    	render : function(rowdata, rowindex,value) {
 							return "<a href=javascript:openUpdate('" + rowdata.group_id   + "|" + rowdata.hos_id  
   								+ "|" + rowdata.copy_code   + "|"  + rowdata.sup_id  
   								+ "|" + rowdata.sup_code + "|"+ rowdata.sup_name
   								+ "')>"+rowdata.sup_code+"</a>";
                       	 }
					 },
			 		 { display: '供应商名称', name: 'sup_name', align: 'left', width: 250,
			 				},
			 		 { display: '分类', name: 'type_name', align: 'left', width: 80,
			 				},
			 		 { display: '所属系统', name: 'mod_name', align: 'left', width: 80,
			 				},
                     { display: '法人', name: 'legal', align: 'left',width: 80
					 		},
                     { display: '纳税人登记号', name: 'regis_no', align: 'left',width: 120,
					 		},
                     ...
                     { display: '传真', name: 'fax', align: 'left',width: 80
					 		},
                     { display: 'EMAIL', name: 'email', align: 'left',width: 100
					 		},
                     { display: '地区', name: 'region', align: 'left',width: 150,
					 		},
                     { display: '邮政编码', name: 'zip_code', align: 'left',width: 100,
					 		},
                     { display: '地址', name: 'address', align: 'left',width: 200
					 		},
					 { display: '支付方式', name: 'pay_name', align: 'left',width: 100
					 		},
                     { display: '备注', name: 'note', align: 'left',width: 200
					 		}
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatSupAttr.do?isCheck=false',
    				 ...
    				 onDblClickRow : function (rowdata, rowindex, value)  //点击事件为双击事件
    				{
						openUpdate(
								rowdata.group_id   + "|" + 
								rowdata.hos_id   + "|" + 
            	...
            	case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatSupAttr.do",{ParamVo : ParamVo.toString()},function (responseData){
                    ...
                    return;
                case "import":
                	$.ligerDialog.open({url: 'matSupAttrImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function add_open(){
  		parent.$.ligerDialog.open({url: 'hrp/mat/info/basic/matsupattr/matSupAttrAddPage.do?isCheck=false&type_code='+type_code, 
  				height: 600,width: 980, title:'添加',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,top:0,
  				parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
  				 buttons: [ 
  				           { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAssSupAttr(); },cls:'l-dialog-btn-highlight' }, 
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({ 
    		...
    		url : 'hrp/mat/info/basic/matsupattr/matSupAttrUpdatePage.do?isCheck=false&' + parm,
			title:'修改', height: 600, width: 980, 
			modal:true,showToggle:false,showMax:true,showMin: false,isResize:true, top:2,
			parentframename: window.name//,  //用于parent弹出层调用本页面的方法或变量
			/* buttons: [ 
				{ text: '确定', onclick: function (item, dialog) { dialog.frame.saveAssSupAttr(); },cls:'l-dialog-btn-highlight' }, 
				{ text: '取消', onclick: function (item, dialog) { dialog.close(); } } 
    ...
    function loadTree(){
    	$.post("queryHosSupTypeByMenu.do?isCheck=false",null,function (responseData){
    ...
    function loadDict(){
		 ...
		 //字典下拉框
    	autocomplete("#sup_id","../../../queryHosSup.do?isCheck=false","id","text",true,true,"",false,false,260);
  ...
  //打印数据
	 function printDate(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopPrinterTable("resultPrint","开始打印","供应商附属表",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatSupAttr.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","供应商附属表集.xls",true);
			return;
		}
		//重新查询数据，避免分页导致打印数据不全
		...
		ajaxJsonObjectByUrl("queryMatSupAttr.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.sup_id+"</td>"; 
					 trHtml+="<td>"+item.sup_name+"</td>"; 
			...
			manager.close();
			lodopExportExcel("resultPrint","导出Excel","供应商附属表.xls",true);
	    },true,manager);
		return;
	 }		  
    </script>
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div class="l-layout" id="layout1" style="height: 100%;" >
		 <div position="left" title="供应商分类">
	     		<div  style="overflow:auto;" >
	     			<ul class="ztree"  id="tree"></ul>
	     		</div>
	     </div>
	     <div position="center" title="供应商信息">
			<div id="toptoolbar" ></div>
		    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
		    	<tr>
		    		 <td align="right" class="l-table-edit-td"  style="padding-left:20px;">供应商信息：</td>
		            <td align="left" class="l-table-edit-td"><input name="sup_code" type="text" id="sup_code" ltype="text" value="${sup_id }" validate="{required:true,maxlength:20}" /></td>
		            <td align="left"></td>
		            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">供应商：</td>
		            <td align="left" class="l-table-edit-td"><input name="sup_id" type="text" id="sup_id" ltype="text" value="${sup_id }" validate="{required:true,maxlength:20}" /></td>
		            <td align="left"></td>
