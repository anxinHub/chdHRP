...
<script type="text/javascript">
    ...
    function  query(newpage){
    	...
    	grid.options.parms.push({
			name : 'source_plan',//计划来源
			value : liger.get("source_plan").getValue()
		});
    	grid.options.parms.push({
			name : 'inv_model',//规格型号
			value : $("#inv_model").val()
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     /* { display: '货位', name: 'location_name',align: 'left',width:150,
				 			...
				 			editor: { 
				 				type: 'select', 
				 				url:'../../../../queryMatInvLocationDict.do?isCheck=false&store_id=',
				 				valueField : 'id',textField: 'text',
				 				keySupport: true,
				 				autocomplete : true 
				 			}
                     ...
                     { display: '货位', name: 'location_code', align: 'left',width:90,hide:isHide,
                      	render : function(rowdata, rowindex,value){
                      		if(rowdata.location_code==null){
    							...
    							+ ',' + rowdata.inv_id +',' + rowdata.location_id +',' + rowindex + '")>'+"编辑"+'</a>';
                      		}else{
                      			//return rowdata.location_name;
                      			return '<a href=javascript:openUpdate("' 
    							+ rowdata.group_id + ',' + rowdata.hos_id 
    							+ ',' + rowdata.copy_code + ',' + rowdata.store_id
    							+ ',' + rowdata.inv_id +',' + rowdata.location_id +',' +rowindex +  '")>'+rowdata.location_name+'</a>';
                      		}
                      	}
                  	  },
                     ...
                     url:'queryMatStoreInvNew.do?isCheck=true',
                     ...
                     toolbar: { items: [
                     	...
                     	{ line:true },
                     	{ text: '添加材料', id:'add', click: addInv,icon:'add' },
                     	{ line:true },
						{ text: '选择材料（<u>S</u>）', id:'select', click: selectMatInv,icon:'add' },
						{ line:true },
    ...
    */
    ...
    function addInv(){
    	...
    	+"&"+ "store_name="+liger.get("store_id").getText(); 
 		...
 		$.ligerDialog.open({
 			url: 'matAddInvPage.do?isCheck=false&'+ paras.toString(),height: 500,width: 1000, 
 			title:'添加材料',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true
 		});
    }
    ...
    function delete_inv(){
		if (checkedCustomer.length==0){
 			  $.ligerDialog.warn("请选择要删除的材料");
 		}else{
 			//alert(JSON.stringify(checkedCustomer));
			$.ligerDialog.confirm('确定删除?',function(yes){
	 			 ajaxJsonObjectByUrl("addMatStoreInvNew.do?isCheck=true", {ParamVo : checkedCustomer.toString()},function(responseData){
    ...
    //选择材料
    function selectMatInv(){
    	var paras = "store_id="+liger.get("store_id").getValue().split(",")[0]
    	+"&"+ "store_name="+liger.get("store_id").getText(); 
    	
    	
 		$.ligerDialog.open({url: 'matInvPage.do?isCheck=false&'+ paras.toString(),height: 500,width: 1050, 
 				title:'选择 材料',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true
 					});
 	...
 	function addAll(){
        ...
        $(data).each(function (){					
				...
				this.inv_id +"@" + store_id
 		        ...
 		        ajaxJsonObjectByUrl("addMatStoreInvAll.do?isCheck=true",{ParamVo : ParamVo.toString()},function(responseData){
 		            if(responseData.state=="true"){
 		            	query();
 		            }
 		        });
    ...
    function remove(){
        ...
        $.ligerDialog.confirm('确定删除?', function (yes){
         	if(yes){
             	ajaxJsonObjectByUrl("deleteMatStoreInv.do?isCheck=true",{ParamVo : ParamVo.toString()},function (responseData){
             		if(responseData.state=="true"){
             			query();
             		}
             	});
    ...
    function openUpdate(obj ){
		...
		var parm = 
			...
			"store_id="+vo[3]   +"&"+ 
			"inv_id="+vo[4] +"&"+ 
 			"location_id="+ (vo[5] == "undefined"?"":vo[5]) ; 
		$.ligerDialog.open({url: 'matStoreInvUpdatePage.do?isCheck=false&'+ parm.toString(),height: 500,width: 700, 
				title:'维护货位',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,selectedRow: vo[6],
				buttons: [ 
						    { text: '保存', 
    ...
    function loadDict(){
        ...
        autocomplete("#mat_type_id","../../../../queryMatTypeDictCode.do?isCheck=false","id","text",true,true,'',false,'','200');
        ...
        autoCompleteByData("#state", yes_or_no.Rows, "id", "text", true, true);//是否在用
	...
	function audit(){
        ...
        if (data.length == 0){
           ...
           /*  if(msg != ''){
            ...
            //查询材料申领库房
            ajaxJsonObjectByUrl("queryMatInvApplyStore.do?isCheck=false&store_id="+store_id+"&inv_ids=" + inv_ids,{},
                	function (responseData){
                        if(responseData.state=="false"){
                        	$.ligerDialog.warn('已设置申领库房的材料不能重复设置');
                        	return ; 
                        }else{
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
    	<tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="sup_id" type="text" id="sup_id" ltype="text" validate="{maxlength:50}" /></td>
            <td align="left"></td>
	      ...
	      <td align="left" class="l-table-edit-td">
      	  ...
      	  <td align="right" class="l-table-edit-td"  >是否在用：</td>
	      <td align="left" class="l-table-edit-td">
	         <input name="state" type="text" id="state" ltype="text"  validate="{maxlength:20}" />
	      </td>
      	  <td align="left"></td>
      	</tr>
    </table>
