  ...
  <head>
    ...
    <script type="text/javascript">
     ...
     function  save(){
        ...
        ajaxJsonObjectByUrl("addMatStoreType.do",formPara,function(responseData){
     ...
     function loadHead(){
     	...
     	grid = $("#maingrid").ligerGrid({
            columns: [ 
                      { display: '物质类别编码', name: 'mat_type_code', align: 'left'
 					 		},
                      { display: '物质类别名称', name: 'mat_type_name', align: 'left'
 					 		}
 					 ],
                      dataAction: 'server',dataType: 'server',usePager:false,
                      url:'queryMatTypeByStore.do',delayLoad:true,enabledEdit : true,
   ...
   //关闭
	    ...
	    function  query(){
	        ...
	        //根据表字段进行添加查询条件
	         grid.options.parms.push({name:'store_id',value:liger.get("store_id").getValue().split(",")[0]}); 
	    	  ...
	    	  grid.options.parms.push({name:'mat_type_id',value:liger.get("mat_type_id").getText().split(" ")[0]}); 
	 ...
	 function f_onCheckAllRow(checked)
	    {
	        ...
	        {
	            if(checked)
	                addCheckedCustomer(this.records[rowid]['mat_type_id']+"@"+'${store_id}');
	            else
	                removeCheckedCustomer(this.records[rowid]['mat_type_id']+"@"+'${store_id}');
	        }
	    }
	    ...
	    */
	    ...
	    {
	        if (findCheckedCustomer(rowdata.mat_type_id+"@"+'${store_id}') == -1)
	            return false;
	        return true;
	    }
	    ...
	    {
	        if (checked){
	        	addCheckedCustomer(data.mat_type_id+"@"+'${store_id}');
	        }else{
	        	removeCheckedCustomer(data.mat_type_id+"@"+'${store_id}');
	        }
	    }
	    ...
	    function add_open(){
	    	...
	    	if (checkedCustomer.length==0){
			  alert("请选择要保存的材料！");
		        ...
		        ajaxJsonObjectByUrl("addMatTypeByStore.do?isCheck=false", {ParamVo : checkedCustomer.toString()},function(responseData){
		            if(responseData.state=="true"){
		            	checkedCustomer=[];
		            	query();
		            	parent.query();
    ...
    function loadDict(){
            //字典下拉框
    	 //字典下拉框
    	autocomplete("#store_id","../../../../queryMatStore.do?isCheck=false","id","text",true,true,'',false,'','160');
  		...
  		$("#store_id").ligerComboBox({disabled:true});
      		$("#mat_type_id").ligerComboBox({
                 	url: '../../../../queryMatTypeDict.do?isCheck=false',
                 	valueField: 'id',
                  textField: 'text', 
                  selectBoxWidth: 160,
                 	autocomplete: true,
   ...
   <div id="pageloading" class="l-loading" style="display: none"></div>
        ...
        <tr>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">物资类别ID：</td>
