...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	  ...
    	  grid.options.parms.push({name:'mat_type_code',value:liger.get("mat_type_id").getText().split(" ")[0]}); 
    ...
    function f_getWhere(){
        ...
        var clause = function (rowdata, rowindex){
                	...
                	if($("#mat_type_id").val()!=""){
                		return rowdata.mat_type_id.indexOf(liger.get("mat_type_id").getValue().split(",")[0]) > -1;	
                	}
//                 	if($("#type_level").val()!=""){
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryStoreType.do?isCheck=false',
                   ...
                   //  delayLoad :true, 
                     toolbar: { items: [
                     	...
                     	{ line:true },
                     	{ text: '选择类别（<u>Q</u>）', id:'select', click: selectMatType,icon:'add' },
                     	{ line:true },
  ...
  //选择类别
    function selectMatType(){
    	...
    	+"&"+ "store_name="+liger.get("store_id").getText(); 
 		...
 		$.ligerDialog.open({url: 'matStoreTypeAddPage.do?isCheck=false&'+ paras.toString(),height: 500,width: 1000, 
    ...
    function add_open(){
    	...
    	var data = grid.getCheckedRows();
 		...
 		if(data.length == 0){
 			...
 			ajaxJsonObjectByUrl("saveMatStoreType.do?isCheck=false", {ParamVo : ParamVo.toString()},function(responseData){
	        ...
	        $(data).each(function (){					
					...
					this.hos_id   +"@"+ 
					this.copy_code   +"@"+ 
					this.mat_type_id + "@"+ store_id
					) });
		        ajaxJsonObjectByUrl("addMatStoreType.do?isCheck=false", {ParamVo : ParamVo.toString()},function(responseData){
		            if(responseData.state=="true"){
		            	query();
		            }
		        });
    ...
    function loadDict(){
	    	...
	    	//autocomplete("#type_level","../../../../queryMatTypeLevel.do?isCheck=false","id","text",true,true,'',false,'','160');
