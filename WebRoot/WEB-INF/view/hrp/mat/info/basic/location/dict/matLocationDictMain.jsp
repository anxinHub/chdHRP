...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					toolbar : {
						items : [ {text : '查询（<u>Q</u>）',	id : 'search',click : query,icon : 'search'	}, 
						          {	line : true	}, 
	...
	function itemclick(item){ 
        if(item.id){
            switch (item.id){
                ...
                case "add":
                	$.ligerDialog.open({url: 'matLocationDictAddPage.do?isCheck=false', height: 400,width: 600, 
          				title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
          				buttons: [ 
          				           { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMatLocationDict(); },cls:'l-dialog-btn-highlight' }, 
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatLocationDict.do",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function imp() {
		...
		var index = layer.open({
			...
			area : [ '893px', '500px' ],
			content : 'matLocationDictImportPage.do?isCheck=false'
		});
		layer.full(index);
	}
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({ url : 'matLocationDictUpdatePage.do?isCheck=false&' + parm,data:{}, height: 400,width: 600, 
