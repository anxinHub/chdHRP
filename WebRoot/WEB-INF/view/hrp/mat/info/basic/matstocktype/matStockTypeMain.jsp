...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatStockType.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'matStockTypeAddPage.do?isCheck=false', height: 300,width: 500, 
              				title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
              				buttons: [ 
              				           { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMatStockType(); },cls:'l-dialog-btn-highlight' }, 
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatStockType.do",{ParamVo : ParamVo},function (responseData){
                    ...
                    return;
                case "import":
                	$.ligerDialog.open({url: 'matStockTypeImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'matStockTypeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 300,width: 400, 
  ...
  //打印数据
	 function printDate(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopPrinterTable("resultPrint","开始打印","04118 采购类型",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatStockType.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","04118 采购类型.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatStockType.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.stock_type_code+"</td>"; 
					 trHtml+="<td>"+item.stock_type_name+"</td>"; 
