...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
					 ...
					 },{ display: '来源', name: 'source_code', align: 'left'
					 },
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatPayType.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'matPayTypeAddPage.do?isCheck=false', 
              				height: 300,width: 500, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
              				buttons: [ 
              				           { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMatPayType(); },cls:'l-dialog-btn-highlight' }, 
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatPayType.do",{ParamVo : ParamVo.toString()},function (responseData){
                    ...
                    return;
                case "extend":
              		$.ligerDialog.open({url: 'matPayTypeExtendPage.do?isCheck=false', 
              				height: 240,width: 500, title:'继承数据',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
              				buttons: [ 
              				           { text: '确定', onclick: function (item, dialog) { dialog.frame.extendMatPayType(); },cls:'l-dialog-btn-highlight' }, 
              				           { text: '取消', onclick: function (item, dialog) { dialog.close(); } } 
              				         ] 
              		});
              		return;
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'matPayTypeUpdatePage.do?isCheck=false&' + parm,data:{}, 
