...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',url:'queryMatInvUnitMap.do',
    ...
    function selectMatInv(){
 		$.ligerDialog.open({url: 'matInvPage.do?isCheck=false',height: 500,width: 1000, 
    ...
    function add_open(){
    	...
    	grid.endEdit(); 
	    	...
	    	if(ParamVo.toString().length >0 ){
	    		ajaxJsonObjectByUrl("saveBatchMatInvUnitMap.do",{ParamVo : ParamVo.toString()},function (responseData){
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatInvUnitMap.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'matInvUnitMapImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matInvUnitMapImportPage.do?isCheck=false&' + parm
				});
				layer.full(index);
  ...
  //打印数据
	 function printDate(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopPrinterTable("resultPrint","开始打印","04116 材料包装单位关系表",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatInvUnitMap.do",printPara,function (responseData){
			var trHtml ='';
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","04116 材料包装单位关系表.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatInvUnitMap.do",exportPara,function (responseData){
			var trHtml ='';
			$.each(responseData.Rows,function(idx,item){ 
				 	 trHtml+="<tr>";
				 	trHtml+="<td>"+item.inv_code+"</td>";
