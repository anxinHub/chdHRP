...
<html style="overflow:hidden;">
  <head>
    ...
    function validateGrid(){
    	if(!$("#sup_cert_code").val()){
    		$.ligerDialog.warn("证件号不能为空！");
    		return false;
    	}
    	/* if(!$("#sup_cert_inv_name").val()){
    ...
    function save(){
    	...
    	if(validateGrid()){
            ...
            var formPara={
            		sup_cert_id:'${sup_cert_id}',
    		        sup_cert_code:$("#sup_cert_code").val(),
    		        sup_cert_inv_name:$("#sup_cert_inv_name").val(),
    		 ...
    		 ajaxJsonObjectByUrl("updateMatInvCertSup.do",formPara,function(responseData){
    ...
    function loadHead(){
   	 grid = $("#maingrid").ligerGrid({
            columns: [
                      ...
                      { display: '材料名称',
 					     ...
 					     editor :{
								...
								grid : {
									...
									url : 'queryMatCertInvList.do?isCheck=false&sup_cert_id='+'${sup_cert_id}',
									//delayLoad:true,
									usePager:true,
									pageSize : 30,
                      ...
                      url:'queryMatInvCertSupDetail.do?isCheck=false&sup_cert_id='+'${sup_cert_id}',
                      width: '100%', height: '65%', checkbox: true,rownumbers:true,
      	 ...
      	 gridSup = $("#maingridSup").ligerGrid({
               columns: [
				   ...
				   { display: '供应商名称', name: 'sup_name', align: 'left'}
	                ...
	                url:'queryMatCerSup.do?isCheck=false&sup_cert_id='+'${sup_cert_id}',
		 ...
		 var data = gridManager.getCheckedRows();
	        if (data.length == 0){
	            ...
	            $.ligerDialog.confirm('确定删除吗?', function (yes){
	            	if(yes && is_delete){
	                	ajaxJsonObjectByUrl("deleteMatInvCertSupRela.do",{ParamVo : ParamVo.toString()},function (responseData){
	                		if(responseData.state=="true"){
	                			parentFrameUse().query();
	                			loadDict();
	       	                 	grid.loadData();
    ...
    function loadDict(){
    	...
    	$("#sup_cert_code").ligerTextBox({width:400});
    	$("#sup_cert_inv_name").ligerTextBox({width:400});
        ...
        <table cellpadding="0" cellspacing="0" class="l-table-edit" border="0" width="100%">
		 	<tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="sup_cert_code" type="text" id="sup_cert_code" value="${sup_cert_code}" ltype="text" validate="{required:true,maxlength:100}" />
	            </td>
				
	            <td align="right" class="l-table-edit-td" ><b>证件产品名称:</b></td>
	            <td align="left" class="l-table-edit-td" >
	            	<input name="sup_cert_inv_name" type="text" id="sup_cert_inv_name"  value="${sup_cert_inv_name}"  ltype="text" validate="{required:true,maxlength:100}" />
				</td>
