...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	  grid.options.parms.push({name:'sup_cert_code',value:$("#sup_cert_code").val()}); 
    	  grid.options.parms.push({name:'sup_cert_inv_name',value:$("#sup_cert_inv_name").val()}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [  { 
                     ...
                     { display: '证件号', name: 'sup_cert_code', align: 'left', width:240,
                    	 render:function(rowdata,index,value){
                    		 return '<a href=javascript:openUpdate("'+rowdata.group_id+"|"+rowdata.hos_id+"|"+rowdata.copy_code+"|"+rowdata.sup_cert_id+"|"+index+'")>'+rowdata.sup_cert_code+'</a>';
                    	 }
					 },{ 
						display: '注册证产品名称', name: 'sup_cert_inv_name', align: 'left',width:200,
					}, { 
					...
					{ display: '查询日期', name: 'queryDate', align: 'left',width:80, hide: true}
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatInvCertSup.do',
                     ...
                     toolbar: { items: [
						...
						{ line:true },
						{ text: '供应商证件审核（<u>S</u>）', id:'audit', click: supAudit,icon:'audit' },
						{ line:true },
    ...
    function add_open(){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/info/basic/cert/invcertsup/matInvCertSupAddPage.do?isCheck=false',
    ...
    function remove(){
        ...
        if (data.length == 0){
            ...
            var in_nos = "";
            $(data).each(function (){	
            	if(this.cert_state != 0){
					in_nos = in_nos + this.sup_cert_code + ",";
				}else{
					ParamVo.push(
							this.group_id   +"@"+ 
							this.hos_id   +"@"+ 
            ...
            if(in_nos != ""){
				$.ligerDialog.error("删除失败！"+in_nos+"证件不是未用状态");
				return;
			}
            $.ligerDialog.confirm('确定删除?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("deleteMatInvCertSup.do",{ParamVo : ParamVo.toString()},function (responseData){
    	...
    	if (data.length == 0){
    		...
    		$(data).each(function (){
	    		paras = "&group_id="+this.group_id +"&hos_id="+this.hos_id
						+"&copy_code="+this.copy_code +"&sup_cert_id="+this.sup_cert_id;
	    		return ;
    		})
			parent.$.ligerDialog.open({
				title: '证件信息复制',height: 550,width: 1000,
				url: 'hrp/mat/info/basic/cert/invcertsup/matInvCertSupCopyPage.do?isCheck=false'+paras,
    ...
    function imp() {
		var para = {
			url : 'hrp/mat/info/basic/cert/invcertsup/matInvCertSupImportPage.do?isCheck=false',
			title : '材料证件导入',
			width : 0,
			height : 0,
			isShowMin : false,
    ...
    function openUpdate(obj){
		...
		var vo = obj.split("|");
		var parm = "group_id="+vo[0]+"&hos_id="+ vo[1] +"&copy_code="+vo[2] +"&sup_cert_id="+ vo[3];
		parent.$.ligerDialog.open({
			title: '修改',
			height: $(window).height(),
			width: $(window).width(),
			selectedRow: vo[4],
			url: 'hrp/mat/info/basic/cert/invcertsup/matInvCertSupUpdatePage.do?isCheck=false&' + parm.toString(),
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});  
...
function supAudit(){
		...
		if (data.length == 0){
			...
			$(data).each(function (){		
				if(this.cert_state != 0){
					in_nos = in_nos + this.sup_cert_code + "<br>";
				}
				ParamVo.push(
					this.group_id   +"@"+ this.hos_id   +"@"+ this.copy_code   +"@"+ 
					this.sup_cert_id  +"@"+ this.sup_cert_code +"@"+ this.sup_cert_inv_name  +"@"+ 
					this.type_id  +"@"+ this.fac_id +"@"+ this.cert_date  +"@"+ 
					this.issuing_authority  +"@"+ this.start_date +"@"+ this.end_date  +"@"+ 
					this.link_person  +"@"+ this.link_tel +"@"+ this.link_mobile  +"@"+ 
					this.cert_memo  +"@"+ this.file_path +"@"+ this.file_address  +"@"+ 
					this.cert_state
				) 
			});
			if(in_nos != ""){
				$.ligerDialog.error("审核失败！<br>以下单据不是未使用状态：<br>"+in_nos);
				return;
			}
			$.ligerDialog.confirm('确定审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("supAuditMatInvCertSupBatch.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
  ...
  //批量审核
	function audit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditMatInvCertSupBatch.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function unAudit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定消审?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("unAuditMatInvCertSupBatch.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function loadDict(){
        ...
        $("#sup_cert_code").ligerTextBox({width:160});
        $("#sup_cert_inv_name").ligerTextBox({width:160});
    }  
    //键盘事件
  ...
  //打印数据
	 function printDate(){
		...
		var printPara={
			usePager:false,
           sup_cert_code:$("#sup_cert_code").val(),
           type_id:liger.get("type_id").getValue()
         };
		ajaxJsonObjectByUrl("queryMatInvCertSup.do",printPara,function (responseData){
			var trHtml='';
			$.each(responseData.Rows,function(idx,item){ 
				 trHtml+="<tr>";
				 trHtml+="<td>"+item.sup_cert_code+"</td>"; 
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryMatInvCertSup.do",exportPara,function (responseData){
			var trHtml='';
			$.each(responseData.Rows,function(idx,item){ 
				 trHtml+="<tr>";
				 trHtml+="<td>"+item.sup_cert_code+"</td>"; 
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="sup_cert_code" type="text" id="sup_cert_code" ltype="text" validate="{required:true}" /></td>
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">证件名称：</td>
            <td align="left" class="l-table-edit-td"><input name="sup_cert_inv_name" type="text" id="sup_cert_inv_name" ltype="text" validate="{required:true}" /></td>
            <td align="left"></td>
