...
<script type="text/javascript">
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
					...
					{ display: '效期类型', name: 'validity_type', align: 'left', 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatInvCertType.do',
	...
	function add_open(){
		$.ligerDialog.open({url: 'matInvCertTypeAddPage.do?isCheck=false', height: 280,width: 400, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMatInvCertType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
	}
		 ...
		 var ParamVo =[];
		  if (data.length == 0){
	            ...
	            $.ligerDialog.confirm('确定删除?', function (yes){
	            	if(yes){
	                	ajaxJsonObjectByUrl("deleteMatInvCertType.do?ParamVo="+ParamVo,{ParamVo : ParamVo},function (responseData){
	...
	function imp(){
		$.ligerDialog.open({url: 'matInvCertTypeImportPage.do?isCheck=false', height: 500,width: 800, title:'导入',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true });
	}
    ...
    function openUpdate(obj){
    	...
    	$.ligerDialog.open({ url : 'matInvCertTypeUpdatePage.do?isCheck=false&' + parm,data:{}, height: 280,width: 400, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMatInvCertType(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
