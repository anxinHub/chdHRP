...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
            columns: [ 
                      ...
                      dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatVenCertDetailCert.do?sup_id=${sup_id}',
    ...
    function add_open(){
    	...
    	$.ligerDialog.open({url: 'matVenCertDetailAddPage.do?isCheck=false&sup_id=${sup_id}&sup_name=${sup_name}', height: 440,width: 800, title:'添加',
			modal:true,showToggle:false,showMax:true,showMin: false,isResize:true,
			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMatVenCertDetail(); },
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatVenCertDetail.do",{ParamVo : ParamVo.toString()},function (responseData){
   ...
   /*  function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'matVenCertDetailImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
   ...
   /* //打印数据
	 function printDate(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopPrinterTable("resultPrint","开始打印","04605 供应商证件信息",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatVenCertDetailCert.do?sup_id=${sup_id}",printPara,function (responseData){
			var trHtml='';
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","04605 供应商证件信息.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatVenCertDetailCert.do",exportPara,function (responseData){
			var trHtml ='';
			$.each(responseData.Rows,function(idx,item){ 
				 trHtml+="<tr>";
				 trHtml+="<td>"+item.type_name+"</td>"; 
