...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatVenCertType.do',
    ...
    function add_open(){
    	...
    	$.ligerDialog.open({url: 'matVenCertTypeAddPage.do?isCheck=false', height: 280,width: 400, title:'添加',
    			modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
    			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMatVenCertType(); },
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatVenCertType.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function imp(){
    	...
    	$.ligerDialog.open({url: 'matVenCertTypeImportPage.do?isCheck=false', height: 500,width: 800, 
    ...
    function openUpdate(obj){
		...
		$.ligerDialog.open({url: 'matVenCertTypeUpdatePage.do?isCheck=false&'+parm, height: 280,width: 400, title:'修改',
			modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,
			buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveMatVenCertType(); },
				cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
	}	
  ...
  //打印数据
	 function printDate(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopPrinterTable("resultPrint","开始打印","04604 供应商证件类型",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatVenCertType.do",printPara,function (responseData){
			var trHtml ='';
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","04604 供应商证件类型.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatVenCertType.do",exportPara,function (responseData){
			 var trHtml="";
			$.each(responseData.Rows,function(idx,item){ 
				 	 trHtml+="<tr>";
					 trHtml+="<td>"+item.type_code+"</td>"; 
