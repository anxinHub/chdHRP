...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatInvCertRela.do',
                     ...
                     toolbar: { items: [
    	                ...
    	                { line:true },
    	                { text: '选择材料（<u>N</u>）', id:'openInv', click: openInv, icon:'add' },
    	                { line:true }
    ...
    function openInv(){
    	...
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/info/basic/cert/invcertrela/matInvCertRelaAddInvPage.do?isCheck=false'+ parm.toString(),
    ...
    function getCheck(rowdata){
 	...
 	/* function addRela(){
 		...
 		if(data.length == 0){
	        ...
	        ajaxJsonObjectByUrl("addMatInvCertRela.do", {ParamVo : ParamVo.toString()},function(responseData){
 	...
 	function addAll(){
        ...
        ajaxJsonObjectByUrl("addMatInvCertRelaAll.do?isCheck=false",para,function(responseData){
            if(responseData.state=="true"){
            	query();
            }
        });
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'matInvCertRelaImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matInvCertRelaImportPage.do?isCheck=false&' + parm
				});
				layer.full(index);	
    ...
    function loadDict(){
            //字典下拉框
            /* 证件编码下拉框 */
            autocomplete("#cert_id", "../../../../queryMatInvCert.do?isCheck=false", "id", "text", true, true,'',true,'','160');
            /*物资类别下拉框 */
            autocomplete("#mat_type_id", "../../../../queryMatTypeDict.do?isCheck=false","id", "text", true, true, {is_last : 1},false,'','160');
			/* 生产厂商 下拉框 */
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryMatInvCertRela.do",printPara,function (responseData){
			var trHtml='';
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryMatInvCertRela.do",exportPara,function (responseData){
			var trHtml='';
			$.each(responseData.Rows,function(idx,item){ 
				 	 trHtml+="<tr>";
					 trHtml+="<td>"+item.cert_id+"</td>"; 
