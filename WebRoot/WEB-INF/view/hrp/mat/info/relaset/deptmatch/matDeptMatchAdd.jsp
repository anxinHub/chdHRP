...
// 		});
             ...
             columns: [ 
                       ...
                       { display: '材料名称(E)',width:230,
  					     ...
  					     editor :{
								...
								grid : {
									columns : [ {
										display : '交易编码', name : 'bid_code', align : 'left', minWidth : '110'
									}, {
										display : '物资类别', name : 'mat_type_name', align : 'left', minWidth : '110'
									},{
										display : '材料编码', name : 'inv_code', align : 'left', minWidth : '110'
									}, {	
										display : '材料名称', name : 'inv_name', align : 'left', minWidth : '240'
									}, { 
										display : '规格型号', name : 'inv_model', align : 'left', minWidth : '200'
									}, { 
										display: '包装规格', name: 'inv_structure', align: 'left', minWidth: '200'
									}, {
										display : '单价', name : 'price', align : 'left', minWidth : '200'
									}, { 
										display : '计量单位', name : 'unit_name', align : 'left', minWidth : '60'
									}, { 
										display : '供应商', name : 'sup_name', align : 'left', minWidth : '180'
									}, { 
										display : '生产厂商', name : 'fac_name', align : 'left', minWidth : '180'
									}, {
										display : '供应商', name : 'sup_name', align : 'left', minWidth : '180'
									} ],
									...
									url : '${p04032}' == 0 ? '../../queryMatInvListNotStore.do?isCheck=false'
										    : '../../queryMatInvList.do?isCheck=false&store_id=' + liger.get("store_id").getValue().split(",")[0],
									//delayLoad:true,
									usePager:true,
									pageSize : 30,
                       ...
                       { display: '包装规格', name: 'inv_structure',  align: 'left',minWidth:200},
		  			   ...
		  			   { display: '数量', name: 'amount', align: 'left',type: 'int', minWidth:120,editor: { type: 'int'}},
		  			   { display: '生产厂商', name: 'fac_name', align: 'left',minWidth:180},
					   { display: '供应商', name: 'sup_name', align: 'left',minWidth:180} ,
                       ...
                       toolbar: { items: [
				    	        ...
				    	        { text: '保存（<u>S</u>）', id:'save', click: saveMatDeptMatch, icon:'save' },{ line:true },
	...
	function f_onSelectRow_detail(data, rowindex, rowobj) {
		...
		if (column_name == "inv_name") {
			if (selectData != "" || selectData != null) {
				...
				grid.updateRow(rowindex_id,{
					    mat_type_name : data.mat_type_name,
						...
						price:data.price,
						inv_structure:data.inv_structure
				});
 	 ...
 	 function f_onSelectRow(data, rowindex, rowobj) {
		...
		function save() {
	    	...
	    	if(allData.length != 0){
		    	$.each(allData,function(i, v){
		    		if (v.inv_id) {
			    		//由于科室不知道要申请多少数量，申请的材料为0，但是我下次要申领
			...
			ajaxJsonObjectByUrl("addMatDeptMatch.do", formPara,function(responseData) {
		...
		function choice_inv(){
			var allData = gridManager.getData();
			$.ligerDialog.open({url: "matDeptMatchChoiceInvPage.do?isCheck=false",
		...
		function loadDict() {
		   ...
		   // autocomplete("#store_id","../../queryMatStoreByReaddisable.do?isCheck=false", "id","text", true,true,"",false,"","","",200);//仓库信息
			 autocomplete("#store_id","../../queryMatStoreAll.do?isCheck=false", "id","text", true,true,"",false,"","","",200);//仓库信息
			autocomplete("#dept_name","../../queryMatDept.do?isCheck=false","id","text",true,true,dept_para,true,"","","",200);//科室信息
