  ...
  <head>
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     usePager:false,//东阳需求去掉分页
                     url:'queryMatDeptMatch.do',
            ...
            switch (item.id)
            {
                case "add":
              		parent.$.ligerDialog.open({
              			url: 'hrp/mat/info/relaset/matDeptMatchAddPage.do?isCheck=false', height: 660,width: 1300, title:'添加',
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                       ...
                       $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatDeptMatch.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function openUpdate(obj){
		...
		var paras = "dept_match_id="+vo[0]+
			...
			"&dept_id="+vo[2];
		parent.$.ligerDialog.open({ 
			url : 'hrp/mat/info/relaset/matDeptMatchUpdatePage.do?isCheck=false&'+paras,data:{}, height: 550,width: 1000, 
  ...
  //打印数据
	 ...
	 function loadDict(){
        ...
        var dept_para ={is_last:1};
 		...
 		//autocomplete("#store_id","../../queryMatStoreByReaddisable.do?isCheck=false","id","text",true,true);//仓库信息
 		autocomplete("#store_id","../../queryMatStoreAll.do?isCheck=false","id","text",true,true);//仓库信息
         
 		autocomplete("#dept_match_name","../../queryMatDeptMatchRead.do?isCheck=false","id","text",true,true);//配套表信息

 		autocomplete("#dept_name","../../queryMatDept.do?isCheck=false","id","text",true,true,dept_para);//科室信息
