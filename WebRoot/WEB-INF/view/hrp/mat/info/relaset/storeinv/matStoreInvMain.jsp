...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '物资材料ID', name: 'inv_id', align: 'left'
					 		},
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatStoreInv.do',
    ...
    function add_open(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'matStoreInvAddPage.do?isCheck=false'
				});
				layer.full(index);
    	}
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatStoreInv.do",{ParamVo : ParamVo},function (responseData){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matStoreInvImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matStoreInvImportPage.do?isCheck=false&' + parm
				});
				layer.full(index);	
  ...
  //打印数据
	 function printDate(){
		...
		if($("#resultPrint > table > tbody").html()!=""){
			lodopPrinterTable("resultPrint","开始打印","04110 仓库材料信息",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatStoreInv.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","04110 仓库材料信息.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryMatStoreInv.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.store_id+"</td>"; 
					 trHtml+="<td>"+item.inv_id+"</td>"; 
