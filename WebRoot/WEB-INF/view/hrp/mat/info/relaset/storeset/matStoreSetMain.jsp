...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatStoreSet.do',
    ...
    function add_open(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'matStoreSetAddPage.do?isCheck=false'
				});
				layer.full(index);
    	}
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatStoreSet.do",{ParamVo : ParamVo},function (responseData){
    ...
    var index = layer.open({
					...
					maxmin : true, //开启最大化最小化按钮
					area : [ '893px', '500px' ],
					content : 'matStoreSetImportPage.do?isCheck=false&' + parm
				});
				layer.full(index);	
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("queryMatStoreSet.do",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryMatStoreSet.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.set_id+"</td>"; 
					 trHtml+="<td>"+item.set_name+"</td>"; 
