...
<script type="text/javascript">
     ...
     $(function (){ 
		...
		$("#store_code").bind("change",function(){
	    	...
	    	grid.columns[5].editor.grid.url= f_get_url;
	    		/* '../../queryMatStockInvListBackNew.do?isCheck=false&store_id=' + liger.get("store_code").getValue().split(",")[0]+ '&sup_id=' + liger.get("sup_code").getValue().split(",")[0]; */
	    	grid.reRender();
		})
    ...
    //已选中的材料,下拉框中不再显示,不可重复选择
 	...
 	function f_get_url(){
 		...
 		return liger.get("store_code").getValue() ? '../../queryMatStockInvListBackNew.do?isCheck=false&store_id=' + liger.get("store_code").getValue().split(",")[0]+'&sup_id=' + liger.get("sup_code").getValue().split(",")[0]+params: '../../queryMatStockInvListBackNew.do?isCheck=false&store_id=' + liger.get("store_code").getValue().split(",")[0]+params;
 	}
     ...
     function  save(){
		...
		ajaxJsonObjectByUrl("addMatStorageBack.do",formPara,function(responseData){
 ...
 function loadForm(){
    ...
    function loadDict(){
    	//字典下拉框
    	autocomplete("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {codes : '10,12,16,22,48'}, false,"12");
    ...
    //仓库对应多个采购员
    	var store_id = liger.get("store_code").getValue().split(",")[0]; 
		autocomplete("#stocker", "../../queryMatStockEmpByStore.do?isCheck=false", "id", "text", true, true,{store_id:store_id}, false);
	//	autocomplete("#stocker", "../../queryMatStockEmpByStore.do?isCheck=false", "id", "text", true, true, "", false); 
					...
					type : 'select',
	         		...
	         		grid : { 
	         			columns : [{
	         				...
	         				display : '供应商编码', name : 'sup_code', width : 140, align : 'left'
	         			}, {
	         				display : '供应商名称', name : 'sup_name', width : 140, align : 'left'
	         			}, {
	...
	function queryallMaterials(){
		...
		var sup_id=liger.get("sup_code").getValue().split(",")[0];
		ajaxJsonObjectByUrl("queryAllStorageMatbySupId.do?isCheck=false&store_id="+store_id+"&sup_id="+sup_id,null,function(responseData){
		...
		detailGrid =$(batchSn).css({'margin-top':10, 'margin-left':60}).ligerGrid({
    		columns: [{ 
    			...
    			display: '批次', name: 'batch_sn', align : 'left',width:90 
     			...
     			display : '库存', name : 'cur_amount', width : 60, align : 'right'
     		}, { 
     			display : '即时库存', name : 'imme_amount', width : 60, align : 'right', 
     		}, { 
