...
<script type="text/javascript">
 ...
 function loadForm(){
    ...
    function checkMatOutFifo(){
    	...
    	if(gridFifo ==null){//没有点击加号的情况下 情况1
    		...
    		if(typeof(row.amount) == "undefined" || row.amount == "" || row.amount == 0){
   				...
   				$.ligerDialog.warn("材料名称对应：["+row.inv_name+"]对应数量为空 请输入\n");
    		...
    		if( row.amount >= 0){
   				...
   				$.ligerDialog.warn("材料名称对应：["+row.inv_name+"]对应数量为负数 请修改\n");
            ...
            ajaxJsonObjectByUrl("queryMatStorageBackInvBatchList.do?isCheck=false",para,function(responseData){
    		...
    		if(detail_fifo_data != null && detail_fifo_data.length > 0){
    	    	...
    	    	$.each(detail_fifo_data, function(d_index, d_content){ 
	 	      		...
	 	      		if(typeof(d_content.amount) == "undefined" || d_content.amount == ""){
	 	      				...
	 	      				validate_detail_buf.append("材料名称对应：["+d_content.inv_name+"]对应数量为空 请输入\n");
	 	    		...
	 	    		if( row.amount >= 0){
	 	    			...
	 	    			validate_detail_buf.append("材料名称对应：["+d_content.inv_name+"]对应数量为为负数 请修改\n");
	 			  	
	 	    		}
	 	      		
	 	      		if(d_content.amount >d_content.left_amount){
	 	      			
	 	      			validate_detail_buf.append("材料名称对应：["+d_content.inv_name+"]对应出库数量大于库存数量 请重新输入\n");
    ...
    function loadDict(){
    	...
    	autocomplete("#store_id", "../../queryMatStore.do?isCheck=false", "id", "text", true, true,"",false,'${store_id}','180');
    	
    	autocomplete("#inv_id", "../../queryMatInvDict.do?isCheck=false", "id", "text", true, true,"",false,false,'280');
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [
                ...
                { display: '库存', name: 'cur_amount', align: 'right', width: 50},
                { display: '即时库存', name: 'imme_amount', align: 'right', width: 50},
                { display: '出库数量', name: 'amount', align: 'right', width: 80,editor : {type : 'int'}}
				],
                     checkbox: false,dataAction : 'server',dataType : 'server',usePager : true,url:'queryMatStorageBackInvList.do?isCheck=false',enabledEdit : true,fixedCellHeight:true,
					isScroll : true,selectRowButtonOnly:true,isSingleCheck:true,heightDiff: 10,
                     ...
                     toolbar: { items: [
				                     	...
				                     	{ line:true },
				                     	{ text: '确定', id:'add_inv', click: checkMatOutFifo,icon:'add' },
				                     	{ line:true },
	 ...
	 function f_showFifoBalance(row, detailPanel,callback){
         ...
         gridFifo = $(eleFifo).css('margin',1).ligerGrid({
             columns:
                         ...
                         { display: '库存', name: 'left_amount', align: 'right' ,width: 50},
                         { display: '即时库存', name: 'imme_amount', align: 'right' ,width: 50,
                          	 render: function (rowdata, rowindex, value){
                         ...
                         { display: '出库数量', name: 'amount', align: 'right' ,width: 50,editor : {type : 'int'}}
                         ], 
						dataAction : 'server',dataType : 'server',usePager : false,checkbox: true,
			 			parms :para,
			 			url : 'queryMatStorageBackInvBatchList.do?isCheck=false',enabledEdit : true,fixedCellHeight:true,
