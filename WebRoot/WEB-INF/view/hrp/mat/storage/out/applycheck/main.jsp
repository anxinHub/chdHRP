    	...
    	grid = $("#maingrid").ligerGrid({
			columns: [{
					...
					render : function(rowdata, rowindex, value) {
		 			...
		 			display: '材料编码', name: 'inv_code', align: 'left', width: '130',
		 		}, { 
		 			display: '材料名称', name: 'inv_name', align: 'left', width: '130',
		 		}, { 
		 			display: '规格型号', name: 'inv_model', align: 'left', width: '130',
		 		},{ 
		 			display: '数量', name: 'app_amount', align: 'right', width: '130',
		 		},{ 
		 			display: '单价', name: 'plan_price', align: 'left', width: '130',
		 		},{ 
		 			display: '金额', name: 'sum_money', align: 'left', width: '130',
		 		},{ 
		 			...
		 			display: '是否已生成需求计划', name: 'is_req', align: 'left', width: '140',
		 			render: function(rowdata, rowindex, value){
		 				if(value == 0){
		 					return "否";	
		 				}else if(value == 1){
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatCommonOutApplyCheckDetailN.do',
			...
			/* 	{ line:true },
				...
				{ line:true } ,
				{ text: '作废（<u>B</u>）', id:'nullify', click: nullify, icon:'delete' },
				{ line:true } 
        ...
        gridManager = $("#maingrid").ligerGetGridManager();
     	...
     	grid = $("#maingrid").ligerGrid({
			columns: [{
 			...
 			alternatingRow:false,
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatCommonOutApplyCheck.do',
			width: '100%', height: '100%', checkbox: true,rownumbers:true,
			delayLoad: true,//初始化不加载，默认false
			selectRowButtonOnly:true,//heightDiff: -10,
			enabledSort : true,
  ...
  //汇总生成采购计划
    ...
    function rela_open(obj){		
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/storage/out/applycheck/relaPage.do?isCheck=false&' + paras.toString(),
	...
	function nullify(){
		...
		if (data.length == 0){
			...
			$(data).each(function (index,value){ 
			 ...
			 if(rela_state != 1){ 
					$.ligerDialog.error("作废失败！<br>只有待处理的状态才能进行作废：<br>");
					return false;
				}   
			 
			$.ligerDialog.confirm('确定作废?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("nullifyMatCommonOutApply.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function createOut(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定生成出库单?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("addOutMatCommonOutApplyCheckBatch.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function createOutCollect(){
		...
		if (data.length == 0){
			...
			var store_dept = "";
			var is_create = true;
			 var nullify_state = true;
			$(data).each(function (index){		
				if(this.rela_state != 3){
					apply_nos = apply_nos + this.apply_no + ",";
				} 
				 if(this.rela_state == 4){
			...
			if(apply_nos == ""){
			  ...
			  if(!nullify_state){
				$.ligerDialog.warn("生成出库单失败！所选单据的状态是作废状态");
				return;
			}  
					...
					parent.$.ligerDialog.open({
						...
						url: 'hrp/mat/storage/out/applycheck/addOutMatCommonOutApplyCheckCollect.do?isCheck=false&para=' + ParamVo.toString(),
						modal: true, showToggle: false, showMax: true, showMin: false, isResize: true, 
						parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
					});
		...
		if (data.length == 0){
			...
			$.ligerDialog.prompt('退回原因', true, function (yes, value){
				if(yes){
					...
					ajaxJsonObjectByUrl("backMatCommonOutApplyCheck.do?",paras,function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function printSet(){
		...
		/* parent.$.ligerDialog.open({url : 'hrp/mat/storage/out/applycheck/storageOutPrintSetPage.do?template_code=04013&use_id='+useId,
...
// 			} 
	    	...
	    	//printTemplate("hrp/mat/storage/out/applycheck/queryMatOutByPrintTemlate.do?isCheck=false",para);
    ...
    function loadDict(){
		...
		autoCompleteByData("#do_state", matApplyMain_storeCheckState.Rows, "id", "text", true, true,null,null,'1');
		...
		autocomplete("#app_emp", "../../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true, para, false);
		autocomplete("#response_emp", "../../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true, para, false);
