...
<html style="overflow:hidden;"> 
  <head>
	...
	var grid; 
	var gridManager;
	var apply_id = "${matAffiOut.apply_id}";
	var is_add = "${matAffiOut.is_add}";
    ...
    function  save(){
        ...
        if(validateGrid()){
            ...
            //ajaxJsonObjectByUrl("../../../affi/out/addMatAffiOutCommon.do?isCheck=true",formPara,function(responseData){
            ajaxJsonObjectByUrl("addMatAffiOutByApp.do?isCheck=true",formPara,function(responseData){
	...
	function loadDict(){
    	...
    	if("${matAffiOut.out_no}"){
    		$("#out_no").val("${matAffiOut.out_no}");
    	}  
    	$("#out_date").ligerTextBox({width:180});     
    	if("${matAffiOut.out_date}"){
    		$("#out_date").val("${matAffiOut.out_date}");
    	}else{
    		autodate("#out_date");
    	}	
    	...
    	autocomplete("#bus_type_code", "../../../queryMatBusType.do?isCheck=false", "id", "text", true, true,bus_type_code_paras,false,'${matAffiOut.bus_type_code}','180');
            
    	autocompleteAsync("#store_id", "../../../queryMatStore.do?isCheck=false", "id", "text", true, true, "", false, false, '180');
    	if("${matAffiOut.store_id}"){
	    	liger.get("store_id").setValue("${matAffiOut.store_id},${matAffiOut.store_no}");
	    	liger.get("store_id").setText("${matAffiOut.store_code} ${matAffiOut.store_name}");
    	}
    	autocompleteAsync("#dept_id", "../../../queryMatDeptDict.do?isCheck=false", "id", "text", true, true,{is_last: 1},false,false,'180');
    	if("${matAffiOut.dept_id}"){
	    	liger.get("dept_id").setValue("${matAffiOut.dept_id},${matAffiOut.dept_no}");
	    	liger.get("dept_id").setText("${matAffiOut.dept_code} ${matAffiOut.dept_name}");
    	}
    	
    	/* 
    	autocompleteAsync("#dept_emp", "../../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true, {dept_id: '${matAffiOut.dept_id}'},false,false,'180'); 
    	if("${matAffiOut.dept_emp}"){
	    	liger.get("dept_emp").setValue("${matAffiOut.dept_emp},${matAffiOut.emp_no}");
	    	liger.get("dept_emp").setText("${matAffiOut.emp_code} ${matAffiOut.emp_name}");
    	} */
    	
    	autocompleteAsync("#dept_emp", "../../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true, "",false,false,'180'); 

    	autocomplete("#use_code", "../../../queryMatOutUse.do?isCheck=false", "id", "text", true, true, "", false, false, '180');
    	if("${matAffiOut.use_code}"){
    		liger.get("use_code").setValue("${matAffiOut.use_code}");
        	liger.get("use_code").setText("${matAffiOut.use_name}");
    	}
    	autocomplete("#proj_code", "../../../queryMatProj.do?isCheck=false", "id", "text", true, true, "", false, false, '180');
    	if("${matAffiOut.proj_id}"){
    		liger.get("proj_code").setValue("${matAffiOut.proj_id}");
        	liger.get("proj_code").setText("${matAffiOut.proj_code} ${matAffiOut.proj_name}");
    	} 
	} 
	
	function loadHead(){
    	var matAffiOutDetail  = ${matAffiOutDetail};
    	grid = $("#maingrid").ligerGrid({
					...
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : '../../../queryMatAffiOutInvList.do?isCheck=false&store_id=' 
         					+ liger.get("store_id").getValue().split(",")[0]+'&out_id=${matAffiOut.out_id}',
			...
			data:matAffiOutDetail,heightDiff:-20,
				...
				editor : {
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : '../../../queryMatAffiOutInvList.do?isCheck=false&store_id=' 
	         					+ liger.get("store_id").getValue().split(",")[0] + '&out_id=${matAffiOut.out_id}&inv_id=' + row.inv_id
		...
		<div position="top">
				<form name="form1" method="post"  id="form1" >
			        ...
			        <tr>
			            ...
			            <td align="left" class="l-table-edit-td" colspan="4"><input name="brief" type="text" id="brief" ltype="text"  value="${matAffiOut.brief}" validate="{maxlength:50}" /></td>
			            <td align="left"></td>
			        </tr> 			
			    </table>			    
			</form>		
