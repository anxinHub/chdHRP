...
<script type="text/javascript">
	...
	var apply_id = "${matOutMain.apply_id}";
	var is_add = "${matOutMain.is_add}";
	...
	function  save(){
		...
		ajaxJsonObjectByUrl("addMatOutMainByApp.do?isCheck=true",formPara,function(responseData){
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	if("${matOutMain.out_no}"){
    		$("#out_no").val("${matOutMain.out_no}");
    	}
    	$("#out_date").ligerTextBox({width:180}); 
    	if("${matOutMain.out_date}"){
    		$("#out_date").val("${matOutMain.out_date}");
    	}else{
    		autodate("#out_date");
    	}
    	...
    	autocomplete("#bus_type_code", "../../../queryMatBusType.do?isCheck=false", "id", "text", true, true,bus_type_code_paras,false,'${matOutMain.bus_type_code}','180');
    	...
    	autocompleteAsync("#dept_emp", "../../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true, {dept_id: '${matOutMain.dept_id}'},false,false,'180'); 
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
			columns: [ { 
					...
					type : 'select',
	         		...
	         		grid : {
	         			columns : [ {
	         				...
	         				display : '货位', name : 'location_name', width : 100, align : 'left',hide:true
	         			},{
				...
				display: '货位名称', name: 'location_name', align: 'left',width:180,hide:true
			}, { 
				display: '货位名称', name: 'location_new_name', align: 'left',width:180
			},{ 
			...
			usePager : false,width : '100%',height : '100%',enabledEdit : is_add == 1 ? true : false,fixedCellHeight:true,data:matOutDetail,heightDiff:-20,
			onBeforeEdit : f_onBeforeEdit, onBeforeSubmitEdit : f_onBeforeSubmitEdit, onAfterEdit : f_onAfterEdit,
			checkbox: true, rownumbers:true, frozen:false,isAddRow:false,//这个属性有detail明细的时候必须为false,否则明细显示不出来
			detail: { onShowDetail: showBatchSn, reload: true, single: true},//材料批次明细
			toolbar: { items: [
				{ text: '保存（<u>S</u>）', id:'save', click: saveMatOutMain,icon:'save', hide: is_add == 1 ? false : true},
		...
		<div position="top">
				<form name="form1" method="post"  id="form1" >
			        ...
			        <tr>
	                    ...
	                    <td align="left" class="l-table-edit-td">
	            			<input class="Wdate" name="app_date" id="app_date" type="text" value="${matOutMain.app_date}"    onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd HH:mm:ss '})"   />
	           		    </td>
			        </tr> 
