...
<script type="text/javascript">
	...
	var apply_id = "${matAffiTran.apply_id}";
	var is_add = "${matAffiTran.is_add}";
	...
	$(function (){
     ...
     function  save(){
        ...
        //ajaxJsonObjectByUrl("../../../affi/out/addMatAffiTranMain.do?isCheck=true",formPara,function(responseData){
        ajaxJsonObjectByUrl("addMatAffiTranMainByApp.do?isCheck=true",formPara,function(responseData){
            if(responseData.state=="true"){	
            	...
            	parent.parent.$.ligerDialog.open({
					...
					url: 'hrp/mat/affi/tran/matAffiTranMainUpdatePage.do?isCheck=false&'+paras.toString(),
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	if("${matAffiTran.tran_no}"){
			$("#tran_no").val("${matAffiTran.tran_no}");
		}
    	autoCompleteByData("#tran_type", matTranMain_tranType.Rows, "id", "text",true, true,'',false,"${matAffiTran.tran_type}",'180');
    	autoCompleteByData("#bus_type", matTranMain_busType.Rows, "id", "text",true, true,'',false,"${matAffiTran.bus_type}",'180');
        ...
        liger.get("tran_method").setValue("${matAffiTran.tran_method}");
        if("${matAffiTran.tran_method}" == 1){
		...
		if("${matAffiTran.out_hos_id}"){
			liger.get("out_hos_id").setValue('${matAffiTran.out_hos_id}');
			liger.get("out_hos_id").setText('${matAffiTran.out_hos_code} ${matAffiTran.out_hos_name}');
    	}
		autocompleteAsync("#out_store_id", "../../../queryMatStore.do?isCheck=false", "id","text", true, true, {hos_id : "${matAffiTran.out_hos_id}",is_com:'1'}, false, false, '180');
		if("${matAffiTran.out_store_id}"){
			liger.get("out_store_id").setValue('${matAffiTran.out_store_id},${matAffiTran.out_store_no}');
			liger.get("out_store_id").setText('${matAffiTran.out_store_code} ${matAffiTran.out_store_name}');
		}
		autocompleteAsync("#in_hos_id", "../../../queryMatHosInfo.do?isCheck=false", "id","text", true, true, "", false, false, '180');
		if("${matAffiTran.in_hos_id}"){
			liger.get("in_hos_id").setValue('${matAffiTran.in_hos_id}');
			liger.get("in_hos_id").setText('${matAffiTran.in_hos_code} ${matAffiTran.in_hos_name}');
		}
		autocompleteAsync("#in_store_id", "../../../queryMatStore.do?isCheck=false", "id","text", true, true, {hos_id : "${matAffiTran.in_hos_id}"}, false, false, '180');
		if("${matAffiTran.in_store_id}"){
			liger.get("in_store_id").setValue('${matAffiTran.in_store_id},${matAffiTran.in_store_no}');
			liger.get("in_store_id").setText('${matAffiTran.in_store_code} ${matAffiTran.in_store_name}');
		}
		$("#tran_date").ligerTextBox({width : 180});
		if("${matAffiTran.tran_date}"){
			$("#tran_date").val("${matAffiTran.tran_date}");
		}else{
			autodate("#tran_date");
		}
	} 
    ...
    function loadHead(){
		var matTranDetail = ${matAffiTranDetail};
		grid = $("#maingrid").ligerGrid({
	           columns: [ 
					...
					usePager : false,width : '100%',height : '100%',enabledEdit : is_add == 1 ? true : false,fixedCellHeight:true,
					heightDiff:25,
					...
					toolbar: { 
						items: [
						        { text: '保存（<u>S</u>）', id:'add', click: saveMatOutMain,icon:'save', hide: is_add == 1 ? false : true},
    ...
    function showBatchSn(row, detailPanel,callback){
		...
		detailGrid =$(batchSn).css({'margin-top':10, 'margin-left':60}).ligerGrid({
    		columns: [{ 
				...
				editor : {
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : '../../queryMatAffiInvDetailList.do?isCheck=false&store_id=' 
    			...
    			display: '数量(E)', name: 'amount', width: 90, align : 'right', editor : {type : 'float'},
				totalSummary: {
					align : 'right',
                    render: function (suminf, column, cell) {
                        return '<div>' + formatNumber(suminf.sum ==null ? 0 : suminf.sum, 2, 1)+ '</div>';
				...
				<form name="form1" method="post"  id="form1" >
			       ...
			       <tr>
			        ...
			        <tr>
			            ...
			            <td align="left" class="l-table-edit-td" colspan="7"><input name="brief" type="text" id="brief" ltype="text"  value="${matAffiTran.brief}" validate="{maxlength:50}" /></td>
			            <td align="left"></td>
			        </tr>
			    </table>
			    </form>
