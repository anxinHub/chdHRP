...
<%-- 	<script src='<%=path%>/lib/SpreadJS/scripts/pluggable/gcspread.sheets.print.9.40.20161.0.min.js' type='text/javascript'></script> --%>
    <script type="text/javascript">
     var grid;
     var gridManager;
     var do_state = '${matApplyMain.do_state}';
...
function loadForm(){
    ...
    function loadDict(){
		...
		liger.get("app_emp").setText("${matApplyMain.emp_name}");
		//响应人
		...
		liger.get("response_emp").setText("${matApplyMain.response_emp_name}");
		//格式化文本框
	...
	function loadButton(){
    	...
    	$("#pur").ligerButton({click: pur, width:150, disabled:false});
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [  { display : '交易编码', name : 'bid_code', width : 110, align : 'left'
				...
				display : '已处理数量', name : 'rela_amount', width : 90, align : 'right',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
				...
				display : '库存量', name : 'common_amount', width : 90, align : 'right',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
				...
				display : '即时库存量', name : 'imme_amount', width : 90, align : 'right',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
				...
				display : '代销物资库存量', name : 'affi_amount', width : 90, align : 'right',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
				...
				display : '代销物资即时库存量', name : 'affi_imme_amount', width : 110, align : 'right',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
			...
			url : 'queryMatCommonOutApplyCheckDetail.do?isCheck=false',
			...
			toolbar : {
				items : [ {
					text : '关闭材料（<u>G</u>）', id : 'closeInv', click : closeInv, icon : 'close'
				}]
			}
		});
	...
	function closeInv(){
		...
		if (data.length == 0){
			...
			ajaxJsonObjectByUrl("updateMatCommonOutApplyCheckCloseInv.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData){
	...
	function out(){
		...
		if (data.length == 0){
			...
			//验证库存
			ajaxJsonObjectByUrl("checkMatCommonOutApplyCheckForAdd.do?isCheck=false", paras, function(responseData){
	            if(responseData.state=="true"){
							$.ligerDialog.open({
								title: '生成出库单',
	...
	function affiout(){
		...
		//防止用户重复点击按钮 进行置灰操作
		$("#affiout").ligerButton({click: affiout, width:110, disabled:true});
		//控制时间为5秒 可按照要求自行控制时间
	    setTimeout(function(){
	    	$("#affiout").ligerButton({click: affiout, width:110, disabled:false});
	    },5000);
	...
	function tran(){
		...
		if (data.length == 0){
			...
			//验证仓库
	        ajaxJsonObjectByUrl("existsMatCommonOutApplyCheckStoreManage.do?isCheck=false", paras, function(responseData){
				if(responseData.state=="true"){
					//验证库存
					ajaxJsonObjectByUrl("checkMatCommonOutApplyCheckForAdd.do?isCheck=false", paras, function(responseData){
			            if(responseData.state=="true"){
	...
	function affitran(){
		...
		//防止用户重复点击按钮 进行置灰操作
		$("#affitran").ligerButton({click: affitran, width:110, disabled:true});
		//控制时间为5秒 可按照要求自行控制时间
	    setTimeout(function(){
	    	$("#affitran").ligerButton({click: affitran, width:110, disabled:false});
	    },5000);
	...
	function pur(){
		//防止用户重复点击按钮 进行置灰操作
		$("#pur").ligerButton({click: pur, width:110, disabled:true});
		//控制时间为5秒 可按照要求自行控制时间
	    setTimeout(function(){
	    	$("#pur").ligerButton({click: pur, width:110, disabled:false});
	    },5000);
		var data = gridManager.getCheckedRows();
		if (data.length == 0){
			$.ligerDialog.error('请选择行');
				...
				$(data).each(function (index, vo){	
					if(index == 0){
						paras = "&group_id=" + vo.group_id
							+ "&hos_id=" + vo.hos_id
							+ "&copy_code=" + vo.copy_code
							+ "&store_id=" + '${matApplyMain.store_id}'
				...
				$.ligerDialog.confirm('确定汇总生成采购计划？', function (yes){
					if(yes){
						ajaxJsonObjectByUrl("../applycollect/addMatPurByApplyCollect.do?isCheck=false", paras, function(responseData){
				            if(responseData.state=="true"){
								query();
				            }
						});
...
function is_exists(){
	...
	var formPara={
			 apply_id :'${matApplyMain.apply_id}' 
	 };
	//已经生成科室需求计划的明细 暂时处理方式  按钮置灰 后期需要改造 
	ajaxJsonObjectByUrl("queryMatOutRequirelExists.do?isCheck=false", formPara, function(responseData){
	...
	function print(){
		...
		printTemplate("queryMatOutByPrintTemlate.do?isCheck=false",para); */
		var para={
    			template_code:'04013',
    			class_name:"com.chd.hrp.mat.serviceImpl.storage.out.MatCommonOutApplyCheckServiceImpl",
    			method_name:"queryMatOutByPrintPage",
	...
	function printSet(){
		...
		/* parent.parent.$.ligerDialog.open({url : 'hrp/mat/storage/out/applycheck/storageOutPrintSetPage.do?template_code=04013&use_id='+useId,
			data:{}, height: $(parent).height(),width: $(parent).width(), title:'打印模板设置',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
		}); */
		officeFormTemplate({template_code:"04013",use_id:useId});
	}
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="apply_no" type="text" id="apply_no" value="${matApplyMain.apply_no}" ltype="text" disabled="disabled"/>
	            </td>
	            <td align="right" class="l-table-edit-td" > 
	            	申请日期：
	            </td>
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="app_date" id="app_date" type="text" value="${matApplyMain.app_date}" required="true" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" validate="{required:true}"  disabled="disabled"/>
	            </td>
	            <td align="right" class="l-table-edit-td"  >
	            	响应仓库：
	            </td>
			...
			<tr>
	            ...
	            <td align="left" class="l-table-edit-td" colspan="5">
	            	<input name="brief" type="text" id="brief" ltype="text" value="${matApplyMain.brief}" validate="{required:false,maxlength:50}" disabled="disabled"/>
	            </td>
			</tr>
	    </table>
    </form>
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%"  style="margin-top: 5px;">
			...
			<tr>	
				<td align="center" class="l-table-edit-td" >
					...
					&nbsp;&nbsp;
					<button id ="pur" accessKey="B"><b>生成请购单（<u>B</u>）</b></button>
					&nbsp;&nbsp;
