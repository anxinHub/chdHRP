    	...
    	if(show_detail=="1"){
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatOutBackMain.do?isCheck=true&show_detail=1',
    		...
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                    ...
    	                    { display: '制单日期', name: 'out_date', align: 'left',width: '90'},
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatOutBackMain.do?isCheck=true&show_detail=0',
    						 ...
    						 toolbar: { items: [
    							...
    							{ text: '消审（<u>U</u>）', id:'unaudit', click: unAudit,icon:'unaudit', hide: isHideCheck },
    							{ line:true, hide: isHideCheck },
    							{ text: '退库确认（<u>F</u>）', id:'confirm', click: confirmData,icon:'account' },
    							{ line:true },
    							/* { text: '申请导入（<u>S</u>）', id:'appImp', click: apply_imp,icon:'up' },
    							{ line:true } , */
    							{ text: '批量打印', id:'print', click: print, icon:'print' },
  ...
  //字段类型渲染器
	...
	function add_open(){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/storage/out/outback/matOutBackMainAddPage.do?isCheck=false', 
	...
	function copyMatOutMain(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定复制?', function(yes) {
            	if (yes) {
		            ajaxJsonObjectByUrl("copyMatOutBackMain.do",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function balanceMatOutMain(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定冲账？', function(yes) {
            	if (yes) {
		            ajaxJsonObjectByUrl("balanceMatOutBackMain.do",{ParamVo : ParamVo.toString()},function (responseData){
		        		if(responseData.state=="true"){
		        			query();
		        		}
		        	});
	     ...
	     $.ligerDialog.confirm('确认出库?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("../../in/verifyMatClosingDate.do?isCheck=false", {ParamVo : ParamVo.toString()}, function(responseData){
						...
						if (yes) ajaxJsonObjectByUrl("confirmOutMatOutBackMain.do",{ParamVo : ParamVo.toString()},function (responseData){
									        		if(responseData.state=="true"){
									        			query();
									        		}
									        	});
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteMatOutBackMain.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/mat/storage/out/outback/matOutBackMainUpdatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});	
    }
    ...
    function loadDict(){
    	...
    	autocomplete("#affirm", "../../../querySysUser.do?isCheck=false", "id", "text", true, true,"", false, false, '180'); 
    	...
    	autocomplete("#checker", "../../../../sys/queryUserDict.do?isCheck=false",  "id", "text", true, true, "", false);
