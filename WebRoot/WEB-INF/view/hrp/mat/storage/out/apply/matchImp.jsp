...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			columns: [{ 
		 			...
		 			display: '物资分类', name: 'mat_type_name', align: 'left'
		 		},  { 
     				...
     				display : '库存数量', name : 'imme_amount', width : 100, align : 'right'
     			} ,{ 
		 			display: '生产厂家', name: 'fac_name', align: 'left'
		 		} ],
		 	enabledEdit: true,
			dataAction: 'server',dataType: 'server',usePager:false,url:'queryMatCommonOutApplyMatch.do?isCheck=false',
    ...
    function imp(){
		...
		if (data.length == 0){
			...
			$.each(data, function(index, data){
				...
				detail_rows.append('"app_amount":').append(parseFloat(data.app_amount)*parseFloat(num)).append(',');
				detail_rows.append('"price":').append(data.price).append(',');
				detail_rows.append('"is_com":').append(data.is_com).append(',');
				detail_rows.append('"amount_money":').append(parseFloat(data.price)*parseFloat(data.app_amount)*parseFloat(num)).append('}');
			});
			detail_rows.append("]");
    ...
    function loadDict(){
		...
		autocompleteAsync("#dept_match", "../../../queryMatDeptMatchRead.do?isCheck=false", "id", "text", true, true, {dept_id:dept_id.split(",")[0],store_id:store_id.split(",")[0]}, true,"",230);
        //配套表联动查询，点击配套表直接查询当前配套表的材料信息
		$("#dept_match").bind("change", function () {
			query(); 
		});
