...
<script type="text/javascript">
	...
	var parentData = frameElement.dialog.options.data;
     ...
     function  save(){
        ...
        var url = parentData.matOutMain.url ? parentData.matOutMain.url : "addMatOutMain";
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	$("#brief").val(parentData.matOutMain.brief);
            
    	autocompleteAsync("#store_id", "../../../queryMatStore.do?isCheck=false", "id", "text", true, true, {is_com : 0});
    	if(parentData.matOutMain.store_id){
	    	liger.get("store_id").setValue(parentData.matOutMain.store_id+','+parentData.matOutMain.store_no);
	    	liger.get("store_id").setText(parentData.matOutMain.store_code+' '+parentData.matOutMain.store_name);
    	}
    	var bus_type_code_paras={sel_flag : "out"};
    	autocomplete("#bus_type_code", "../../../queryMatBusType.do?isCheck=false", "id", "text", true, true,bus_type_code_paras, false, parentData.matOutMain.bus_type_code);
		
    	autocompleteAsync("#dept_id", "../../../queryMatDeptDict.do?isCheck=false", "id", "text", true, true,{is_last: 1});
    	if(parentData.matOutMain.dept_id){
	    	liger.get("dept_id").setValue(parentData.matOutMain.dept_id+","+parentData.matOutMain.dept_no);
	    	liger.get("dept_id").setText(parentData.matOutMain.dept_code+" "+parentData.matOutMain.dept_name);
    	}
    	autocomplete("#dept_emp", "../../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true, isEmpByDept ? {dept_id: parentData.matOutMain.dept_id} : "");
    	if(parentData.matOutMain.dept_emp){
	    	liger.get("dept_emp").setValue(parentData.matOutMain.dept_emp+","+parentData.matOutMain.emp_no);
	    	liger.get("dept_emp").setText(parentData.matOutMain.emp_code+" "+parentData.matOutMain.emp_name);
    	}
    	autocomplete("#use_code", "../../../queryMatOutUse.do?isCheck=false", "id", "text", true, true);
    	if(parentData.matOutMain.use_code){
    		liger.get("use_code").setValue(parentData.matOutMain.use_code);
        	liger.get("use_code").setText(parentData.matOutMain.use_name);
    	}
    	autocomplete("#proj_code", "../../../queryMatProj.do?isCheck=false", "id", "text", true, true);
    	if(parentData.matOutMain.proj_id){
    		liger.get("proj_code").setValue(parentData.matOutMain.proj_id);
        	liger.get("proj_code").setText(parentData.matOutMain.proj_code+" "+parentData.matOutMain.proj_name);
    	} 
	...
	function loadHead(){
    	var matOutDetail  = JSON.parse(parentData.matOutDetail);
		grid = $("#maingrid").ligerGrid({
				...
				editor : {
					type : 'select',
	         		...
	         		grid : {
	         			...
	         			url : '../../../queryMatStockInvListOut.do?isCheck=false&is_com=0&store_id=' 
			...
			data: matOutDetail, checkbox: true, rownumbers:true, frozen:false,//这个属性有detail明细的时候必须为false,否则明细显示不出来
				...
				<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
			        ...
			        <tr>
			            ...
			            <td align="left" class="l-table-edit-td" colspan="4">
			            	<textarea class="l-textarea" name="brief" id="brief" rows="3" style="width: 380px;">parentData.matOutMain.brief</textarea>
			            </td> 
			            <td align="left"></td>
			        </tr> 
			    </table>
