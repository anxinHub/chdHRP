...
<script type="text/javascript">   
     ...
     $(function (){
		...
		$("#store_id").bind("change",function(){
	    	grid.columns[5].editor.grid.url =f_get_url;//已添加的材料下拉框不再显示
	    	grid.reRender();
		});
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	autocompleteAsync("#bus_type_code", "../../../queryMatBusType.do?isCheck=false", "id", "text", true, true,bus_type_code_paras, false, "${matOutMain.bus_type_code}");
		...
		$("#printType").ligerButton({click: printType, width:90,disabled:false});
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
	         		...
	         		grid : {
	         			columns : [{
							...
							display : '包装规格', name : 'inv_structure', width : 70, align : 'left'
	        	         	...
	        	         	display: '批号', name: 'batch_no', align: 'left', width: 90, align : 'left'
	       	         	}, { 
	        	         	display: '条形码', name: 'bar_code', align: 'left', width: 90, align : 'left'
	       	         	}, {
			...
			dataAction : 'server', 
			dataType : 'server', 
			url : 'queryMatOutDetailByCode.do?isCheck=false', 
	...
	/* 	$(grid.rows).each(function(i,obj){
		...
		return '../../../queryMatStockInvListOut.do?isCheck=false&is_com=0&store_id=' 
			+ liger.get("store_id").getValue().split(",")[0] + '&bus_type_code=' 
				+ liger.get("bus_type_code").getValue()+ '&out_id=${matOutMain.out_id}'+params;
	}
	...
	function renderSelect(){
		...
		for(var i = 0,len =data.length;i<len;i++){
			var item = data[i];
			list_inv[item.inv_id] = item.inv_name;
 		} 
		lodopPrint();
	}
	  //导出回调方法
    function lodopPrint(){ 
 		grid.options.lodop.fn={
 			inv_id:function(value){//数量
				return list_inv[value]||"";
			}
 		};  
    }
	...
	function printSet(){
		...
		/* parent.parent.$.ligerDialog.open({url : 'hrp/mat/storage/out/outlibrary/storageOutPrintSetPage.do?template_code=04014&use_id='+useId,
		...
		<div position="center" >
			...
			<tr>	
				<td align="center" class="l-table-edit-td" colspan="3">
					...
					&nbsp;&nbsp;
					<button id ="printType" accessKey="P"><b>类别打印（<u>L</u>）</b></button>
					&nbsp;&nbsp; 
