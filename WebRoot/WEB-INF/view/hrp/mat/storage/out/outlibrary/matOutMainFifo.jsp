...
<head>
    ...
    function checkMatOutFifo(){
    	...
    	if(gridFifo == null){//没有点击加号的情况下 情况1
            ...
            ajaxJsonObjectByUrl("queryMatOutMainByFifoBalances.do?isCheck=false",para,function(responseData){
    		...
    		if(detail_fifo_data != null && detail_fifo_data.length > 0){
        		...
        		if(typeof(row.amount) == "undefined" || row.amount == "" || row.amount == 0){
       				$.ligerDialog.warn("材料["+row.inv_name+"]对应数量为空 请输入\n");
       				return false;
    		  	}
        		if(row.imme_amount < row.amount){
      				$.ligerDialog.warn("材料["+row.inv_name+"]对应数量大于即时库存 请输入\n");
    				return false;
    	  		}
                ...
                ajaxJsonObjectByUrl("queryMatOutMainByFifoBalance.do?isCheck=false",para,function(responseData){
                	var detail_rows = new StringBuffer();
                	detail_rows.append("[");
                	$.each(responseData.Rows, function(d_index, d_content){ 
                		if(d_index != 0){detail_rows.append(",");}
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [
				...
				url:'queryMatOutMainByInvHold.do?isCheck=false',isScroll : true,selectRowButtonOnly:true,parms :para,
				detail: { onShowDetail: f_showFifoBalance, reload: true, single: true},//材料批次明细
                toolbar: { 
                	items: [
	 ...
	 function f_showFifoBalance(row, detailPanel,callback){
         ...
         gridFifo = $(eleFifo).css('margin',1).ligerGrid({
             columns:[
                         ...
                         { display: '库存', name: 'left_amount' ,width: 80,align: 'right'},
                         { display: '即时库存', name: 'imme_amount' ,width: 80,align: 'right'},
                         { display: '出库数量', name: 'amount' ,width: 80,editor : {type : 'int'},align: 'right'}
                         ], 
						dataAction : 'server',dataType : 'server',usePager : false,checkbox: true,
			 			parms : para,isAddRow : false,
			 			url : 'queryMatOutMainByFifoBalance.do?isCheck=false',enabledEdit : true,
