...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatApplyMainOut.do',
    ...
    function loadDetailHead(){
    	detailGrid = $("#detailgrid").ligerGrid({
			columns : [{
				...
				display : '非代销物资库存量', name : 'common_amount', width : 110, align : 'left',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
				...
				display : '非代销物资即时库存量', name : 'imme_amount', width : 110, align : 'left',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
				...
				display : '代销物资库存量', name : 'affi_amount', width : 110, align : 'left',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
				...
				display : '代销物资即时库存量', name : 'affi_imme_amount', width : 110, align : 'left',
				render : function(rowdata, rowindex, value) {
					return formatNumber(value == null ? 0 : value, 2, 1);
				},
				totalSummary: {
			...
			url : 'queryMatApplyDetailOut.do?isCheck=false',
		...
		if (JSON.stringify(applyObj) == "{}"){
			...
			$.ligerDialog.confirm('确定生成出库单?', function (yes){
				if(yes){
                	...
                	ajaxJsonObjectByUrl("queryMatOutByOther.do?isCheck=false", para, function (responseData){
                		if(responseData.state=="true"){
        					parentFrameUse().openOutAddByApplyImp(responseData);
        					this_close();
                		}
                	});
				}
