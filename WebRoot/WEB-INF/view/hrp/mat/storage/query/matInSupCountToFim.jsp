...
<script type="text/javascript">
    ...
    function  query(){
    	...
    	ajaxJsonObjectByUrl("../../../hip/queryALLMatFimTypeDict.do?isCheck=false", selPara, function (responseData) {
    		...
    		var show_store = $("#is_showStore").is(":checked") ? true:false;
	    		if(show_store){
	    			columns.push({ display:'仓库名称', id:'', name:'store_name', align:'left', minWidth:'140'});
	    		}
	    		var name='';
    			for(var i=0;i<responseData.length;i++){
    				...
    				columns.push({display:'单据张数', id:'', name:'bus_amount', align:'right', minWidth:'80'});
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			columns: [],
			dataAction: 'server',dataType: 'server',usePager:false,url:'queryMatInSupCount.do',
    ...
    function f_setColumns(){
    	...
    	if(show_store){
	    	columns = [
				...
				{display: '供应商编码', name: 'sup_code', align: 'left', minWidth: '140',}, 
				{display: '供应商名称', name: 'sup_name', align: 'left', minWidth: '150'}, 
				{display: '合计', name: 'amount_money', align: 'right', minWidth: '80',formatter:"###,##0.00",
					render : function(rowdata, rowindex, value) {
						return formatNumber(rowdata.amount_money ==null ? 0 : rowdata.amount_money, '${p04005 }', 1);
					},formatter:'###,##0.00'
				}, 
				{display: '进价品种', name: 'mat_type_count', align: 'left', minWidth: '150'}, 
				{display: '单据张数', name: 'in_id_count', align: 'left', minWidth: '80'}
	    	];
    	}else{
    		columns = [
				{display: '虚仓名称', name: 'set_name', align: 'left', minWidth: '140',render:
    ...
    function lodopPrint(){
    	var head="<table class='head' width='100%'><tr><td>单位：${sessionScope.hos_name}</td></tr>";
 		...
 		grid.options.lodop.title="供应商入库汇总表(财务)";
    }
	...
	function print(){
    	...
    	var heads={
        		...
        		"rows": [
    	          ...
    	          {"cell":3,"value":"仓库：", "align" : "right"},
    	          {"cell":4,"value":""+liger.get("store_code").getText()==''?'空':liger.get("store_code").getText().split(" ")[1]+""},
    	          {"cell":3,"value":"虚仓：", "align" : "right", "br" : true},
    	          {"cell":4,"value":""+liger.get("set_code").getText()==''?'空':liger.get("set_code").getText().split(" ")[1]+""}
        	]}; 
    	//表尾
		var foots = {
			rows: [
    	...
    	var printPara={
          		title: "供应商入库汇总表（财务）",//标题
          		columns: JSON.stringify(grid.getPrintColumns()),//表头
          		class_name: "com.chd.hrp.mat.service.storage.query.MatInDetailToFimService",
       			method_name: "queryMatInSupCountPrint",
       			bean_name: "matInDetailToFimService",
...
<body style="padding: 0px; overflow: hidden;" onload="f_setColumns()">
	...
	<div class="search-block clearfix">
	 ...
	 <table>
		...
		<div class="cell w1">
			<div style="display: none;">物资类别：</div>
