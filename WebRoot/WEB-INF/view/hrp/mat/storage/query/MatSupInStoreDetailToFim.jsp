...
<script type="text/javascript">
    ...
    function  query(){
    	...
    	ajaxJsonObjectByUrl("queryOccurMatFimTypeDictForHead.do?isCheck=false", selPara, function (responseData) {
    		 ...
    		 columns=[
    			...
    			for(var i=0;i<responseData.length;i++){
    				name=responseData[i].fim_type_name;
    				columns.push({
    					display:name,
    					id:'',
    					formatter:'###,##0.00',
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			columns: [],
			dataAction: 'server',dataType: 'server',usePager:false,url:'queryMatSupInStoreDetailToFim.do',
	...
	function print(){
    	...
    	if(liger.get("set_code").getValue()== "" && liger.get("store_code").getValue()== ""){ 
    		var heads={
            		...
            		"rows": [
        	          {"cell":0,"value":"统计年月："},
        	          {"cell":1,"value":liger.get("begin_year").getText()+"-"+liger.get("begin_month").getText()} ,
    	...
    	var printPara={
          		title: "入库业务汇总表(财务分类)",//标题
          		columns: JSON.stringify(grid.getPrintColumns()),//表头
          		class_name: "com.chd.hrp.mat.service.storage.query.MatSupInStoreDetailToFimService",
       			method_name: "queryMatSupInStoreDetailToFimPrint",
       			bean_name: "matSupInStoreDetailToFimService",
    ...
    function loadDict(){
        ...
        year = aa[0];  
        month = aa[1];
        autocompleteAsync("#begin_year", "../../queryMatYear.do?isCheck=false", "id", "text", true, true, "", false, year, "90");
        autocompleteAsync("#begin_month", "../../queryMatMonth.do?isCheck=false", "id", "text", true, true, "", false, month, "90");
		...
		autocompleteAsync("#bus_type_code", "../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {codes:"'10' ,'12' ,'2', '22' ,'4' ,'6', '8' ,'18','16'"}, true);
	 ...
	 <table>
		<tr>
            ...
            <td align="left" class="l-table-edit-td" width="20%"> 
            	<input name="set_code" type="text" id="set_code" ltype="text" validate="{required:true}" />
  			</td> 
		...
		<tr>
	         ...
	         <td align="left" class="l-table-edit-td" width="20%">
	            	<input name="bus_type_code" type="text" id="bus_type_code" ltype="text" validate="{required:false}" />
            </td>
          </tr>
	</table>
	</div>
