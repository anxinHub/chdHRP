    ...
    function loadHead(){
    	if(show_detail=="1"){
    		grid = $("#maingrid").ligerGrid({
    			columns: [
    			     {display: '单据号', name: 'special_no', align: 'left', width: 130,frozen :true,
    				...
    				{display: '金额', name: 'amount_money', align: 'right',width: 90,
			 			render : function(rowdata, rowindex, value) {
							...
							return formatNumber(rowdata.amount_money ==null ? 0 : rowdata.amount_money, '${paras.para_04005}', 1);
						}
		 			}, 
		 			{display: '单据状态', name: 'state', align: 'left',width: 70,hide:true
		 				
		 			},
					{display: '状态', name: 'state_name', align: 'left',width: 70,
    				...
    				{display: '生产批号', name: 'batch_no', align: 'left',width: 80,},
    			...
    			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatSpecial.do?isCheck=false&show_detail=1',
    		...
    		grid = $("#maingrid").ligerGrid({
    			columns: [
					...
					{display: '验收员', name: 'examiner_name', align: 'left',width: 80,
    		 		...
    		 		{
    		 			display: '是否生成发票', name: 'bill_state', align: 'left', width: '90',
    		 			render : function(rowdata, rowindex, value) {
    		 				if(value == 1){
    		 					return "是";
    		 				}else{
    			...
    			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatSpecial.do?isCheck=false&show_detail=0',
  ...
  //是否显示复选框
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
			...
			height: 550,
			width: 1150,
			url: 'hrp/mat/storage/special/matSpecialAddPage.do?isCheck=false',
    ...
    function update_open(obj){		
		...
		parent.$.ligerDialog.open({
			...
			height: 550,
			width: 1150,
			url: 'hrp/mat/storage/special/updatePage.do?isCheck=false&' + paras.toString(),
			modal: true, showToggle: false, showMax: true, showMin: true, isResize: true, top : 1,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});   
    }
   ...
   //批量复制
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@" + this.hos_id + "@"
								+ this.copy_code + "@" + this.special_id)
					});
			$.ligerDialog.confirm('确定复制?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("copyMatSpecial.do?isCheck=false", {
    ...
    function affiOut(){		
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/mat/storage/special/matAffiImpPage.do?isCheck=false',
			//url: 'hrp/mat/storage/special/matAffiImpOldPage.do?isCheck=false', 
		...
		if (data.length == 0){
			...
			if(ParamVo != null && ParamVo != ''){
				$.ligerDialog.confirm('确定删除?', function (yes){
					if(yes){
						ajaxJsonObjectByUrl("deleteMatSpecial.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
							if(responseData.state=="true"){
								query();
							}
						});
	...
	function offset(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定冲账?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("offsetMatSpecial.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function printSet(){
    	...
    	/* parent.$.ligerDialog.open({url :'hrp/mat/storage/special/matSpecialPrintSetPage.do?template_code=04017&use_id='+useId,
...
//			} 
	    	...
	    	printTemplate("hrp/mat/storage/special/queryMatSpecialByPrintTemlate.do?isCheck=false",para); */
    ...
    function loadDict(){
		...
		autocomplete("#mat_type_code", "../../queryMatTypeDictDate.do?isCheck=false", "id", "text", true, true, {read_or_write:'1'}, false, false, '220');
		//autocomplete("#store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true, {read_or_write:'1'}, false, false, '220');
		autocomplete("#store_id", "../../queryMatStoreDictPro.do?isCheck=false", "id", "text", true, true, {read_or_write:'1'}, false, false, '220');
		autocomplete("#sup_id", "../../queryHosSupDict.do?isCheck=false", "id", "text", true, true, "", false, false, '220');
		autocomplete("#dept_id", "../../queryMatDeptDictDate.do?isCheck=false", "id", "text", true, true, {is_last:1,read_or_write:'1'}, false, false, '220');
      ...
      //  autocomplete("#inv_code", "../../queryMatInv.do?isCheck=false", "id", "text", true, true, "", false, false, "", "", "220");
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%" >
       <tr>
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td" width="20%">
            	<input name="bus_type" type="text" id="bus_type" ltype="text" />
            </td>
        </tr>
