...
<script type="text/javascript">
 ...
 function loadForm(){
	...
	function loadDict(){
		...
		autocompleteAsync("#in_hos_id", "../../queryMatHosInfo.do?isCheck=false", "id","text", true, true, {hos_id:null}, false, false, '180');
		...
		$("#bcPrint").ligerButton({click: bcPrint, width:90});
		$("#bcPrintSet").ligerButton({click: bcPrintSet, width:90});
  ...
  //已选中的材料,下拉框中不再显示,不可重复选择
    ...
    function showBatchSn(row, detailPanel,callback){
		...
		detailGrid =$(batchSn).css({'margin-top':10, 'margin-left':60}).ligerGrid({
    		...
    		width: '65%',height: '70%',data : f_getInvDetailData(row),
    		toolbar: { items: [
	...
	function printSet(){
		...
		/* parent.parent.$.ligerDialog.open({url : 'hrp/mat/storage/tran/matTranPrintSetPage.do?template_code=04015&use_id='+useId,
			data:{}, height: $(parent).height(),width: $(parent).width(), title:'打印模板设置',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
		}); */
		officeFormTemplate({template_code:"04015",use_id:useId});
	}
	...
	function bcPrintSet(){
		...
		officeFormTemplate({template_code:"041328",use_id:useId});
	}
	//模板条码打印
	function bcPrint(){ 
		var useId=0;//统一打印
		...
		for(var i=0;i<barData.length;i++){
			if(barData[i].bar_code==='-'){
				if(errDetailIdStr.length!=0){
					errDetailIdStr+=',';
				}
				errDetailIdStr+=barData[i].inv_code+barData[i].inv_name;
			}else{
				if(tranDetailIdStr.length!=0){
					tranDetailIdStr+=',';
				}
				console.log(barData[i].inv_detail_data.split(",")[0].split(":")[1]);
				//barData[0].inv_detail_data[0]为字符串,如下格式:"[{"tran_detail_id":1339695,"inv_id":6384,"inv_code":"01080310105","inv_name":"心脏瓣膜成形环(SOVERING)",...}]"
				var tran_detail_id=barData[i].inv_detail_data.split(",")[0].split(":")[1];
				tranDetailIdStr+=tran_detail_id;
			}
		}
		if(errDetailIdStr.length!=0){//没有条码
			$.ligerDialog.error('含非个体码材料('+errDetailIdStr+'),无法生成条形码,清重新选择!');
			return;
		} 
		var para={
				template_code:'041328',
				class_name:"com.chd.hrp.mat.serviceImpl.storage.tran.MatTranMainServiceImpl",
				method_name:"queryMatTranDetailBarcodeByPrintTemlate",
				isSetPrint:false,//是否套打，默认非套打
				isPreview:true,//是否预览，默认直接打印
				in_id:$("#in_id").val(),
				"tranDetailIdStr":tranDetailIdStr,
				p_num:0,
				use_id:useId
		}; 
		officeFormPrint(para); 
				...
				<form name="form1" method="post"  id="form1" >
			       ...
			       <tr>
			        ...
			        <tr>
			            ...
			            <td align="left"></td>
			            <td align="right" class="l-table-edit-td hos_id"  style="padding-left:20px;"><span style="color:red;display:none;">*</span>调拨方式：</td>
		...
		<div position="center" >
			...
			<tr>	
				<td align="center" class="l-table-edit-td" colspan="3">
					...
					&nbsp;&nbsp;
					<button id ="bcPrint" accessKey="B"><b>条码模板打印（<u>B</u>）</b></button>
					&nbsp;&nbsp;
					<button id ="bcPrintSet" accessKey="S"><b>条码模板设置（<u>S</u>）</b></button>
					&nbsp;&nbsp;
