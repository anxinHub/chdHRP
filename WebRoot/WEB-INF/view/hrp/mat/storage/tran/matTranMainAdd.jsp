...
<script type="text/javascript">
   ...
   //已选中的材料,下拉框中不再显示,不可重复选择
	...
	/* $.ajax({
		 ...
		 ajaxJsonObjectByUrl("addMatTranMain.do", formPara, function(
				responseData) {
	...
	function loadDict() {
		...
		autoCompleteByData("#tran_type", matTranMain_tranType.Rows, "id",
				"text", true, true, '', true, false, '180');

		autoCompleteByData("#bus_type", matTranMain_busType.Rows, "id", "text",
				true, true, '', false, false, '180');

		//autoCompleteByData("#tran_method", matTranMain_tranMethod.Rows, "id", "text",true, true,'',true,false,'180');
		$("#tran_method").ligerComboBox({
			width : 180,
			disabled : true,
			cancelable : false
		});
		...
		//字典下拉框
		autocompleteAsync("#out_hos_id",
				"../../queryMatHosInfo.do?isCheck=false", "id", "text", true,
				true, "", false, "${sessionScope.hos_id}", '180');
		autocompleteAsync("#out_store_id",
		...
		$("#out_hos_id").bind(
				...
				function() {
					...
					if (liger.get("out_store_id").data[0]) {
						liger.get("out_store_id").selectValue(
								liger.get("out_store_id").data[0].id);
					} else {
						liger.get("out_store_id").setValue("");
						liger.get("out_store_id").setText("");
					}
					//autocomplete("#out_store_id", "../../queryMatStore.do?isCheck=false", "id","text", true, true, para, true, false, '180');	    	
				})
		autocompleteAsync("#in_hos_id",
				"../../queryMatHosInfo.do?isCheck=false", "id", "text", true,
				true, {hos_id:null}, false, "${sessionScope.hos_id}", '180');
		autocompleteAsync("#in_store_id",
				"../../queryMatStoreDictDate.do?isCheck=false", "id", "text",
				true, true, {
					is_com : '0',
		...
		$("#in_hos_id").bind(
				...
				function() {
					...
					if (liger.get("in_store_id").data[0]) {
						liger.get("in_store_id").selectValue(
								liger.get("in_store_id").data[0].id);
					} else {
						liger.get("in_store_id").setValue("");
						liger.get("in_store_id").setText("");
					}
					//autocompleteAsync("#in_store_id", "../../queryMatStoreAll.do?isCheck=false", "id","text", true, true, para, true, false, '180');	    	
				})
		...
		$("#tran_date").ligerTextBox({
			width : 180
		});
		autodate("#tran_date");
		//$("#brief").ligerTextBox({width:480}); 

		$("#bus_type").ligerTextBox({
			disabled : true
		});
		$("#out_hos_id").ligerTextBox({
			disabled : true
		});
		$("#in_hos_id").ligerTextBox({
			disabled : true
		});
	}
	...
	function changeTranType() {
		if (liger.get("tran_type").getValue() == 1) {
			...
			$("#bus_type").attr("readonly", "readonly").ligerComboBox({
				width : 180,
				disabled : true,
				cancelable : false
			});
			...
			$(".hos_id").attr("style","visibility:hidden");
			$(".store_id").attr("style","visibility:true");
			$("#out_hos_id").attr("readonly", "readonly").ligerComboBox({
				width : 180,
				disabled : true,
				cancelable : false
			});
			$("#in_hos_id").attr("readonly", "readonly").ligerComboBox({
				width : 180,
				disabled : true,
				cancelable : false
			});
			...
			$("#bus_type").removeAttr("readonly").prop("disabled", false)
					.ligerComboBox({
						width : 180,
						disabled : false,
						cancelable : true
			...
			$(".hos_id").attr("style","visibility:true");
			
			$("#out_hos_id").removeAttr("readonly").prop("disabled", false)
					.ligerComboBox({
						width : 180,
						disabled : false,
						cancelable : true
					});
			$("#in_hos_id").removeAttr("readonly").prop("disabled", false)
					.ligerComboBox({
						width : 180,
						disabled : false,
						cancelable : true
											...
											grid : {
												...
												onSuccess : function(data, g) { //加载完成时默认选中
													if (grid.editor.editParm) {
														...
														var index = item
																.indexOf(editor.inv_name) == -1 ? 0
																: item
																		.indexOf(editor.inv_name);
														//加载完执行
														setTimeout(
																function() {
																	g
																			.select(data.Rows[index]);
																}, 80);
													}
												}
											},
	...
	function choice_inv() {
		...
		$.ligerDialog.open({
			url : "matTranMainChoiceInvPage.do?isCheck=false&store_id="
					+ store_id + "&store_text=" + store_text,
			height : 500,
			width : 900,
			title : '选择材料',
			modal : true,
	...
	function choice_bar() {
		...
		$.ligerDialog.open({
			url : "matTranMainChoiceInvBarPage.do?isCheck=false&store_id="
					+ store_id + "&store_text=" + store_text,
			height : 500,
			width : 900,
			title : '选择材料',
						...
						.updateRow(
								rowindex_id,
								{
									...
									bar_code : data.bar_code == null ? ""
											: data.bar_code,
									cur_amount : data.cur_amount == null ? ""
											: data.cur_amount,
									imme_amount : data.imme_amount == null ? ""
											: data.imme_amount,
									...
									sell_price : data.sell_price == null ? ""
											: data.sell_price,
									inva_date : data.inva_date == null ? ""
											: data.inva_date,
									disinfect_date : data.disinfect_date == null ? ""
											: data.disinfect_date,
									location_id : data.location_id == null ? ""
											: data.location_id,
									location_new_name : data.location_new_name == null ? ""
											: data.location_new_name
								});
	...
	function f_onAfterEdit(e) {
		if ("inv_id" == column_name) {
			...
			grid.updateCell('amount_money', e.record.amount * e.record.price,
					e.record);
			grid.updateCell('sale_money',
					e.record.amount * e.record.sale_price, e.record);
			grid.updateCell('sum_amount', 0, e.record);
			grid.updateCell('inv_detail_data', "", e.record);
		}
							...
							columns : [
									...
									},
									{
										...
										editor : {
											...
											delayLoad : false,
											keySupport : true,
											autocomplete : true,// rownumbers : true,
											onSuccess : function() {
												this.parent("tr").next(
														".l-grid-row").find(
														"td:first").focus();
	...
	function f_getInvDetailData(rowdata) {
		...
		if (validateStr(rowdata.inv_id) && validateStr(rowdata.amount)
				&& rowdata.amount != 0) {
			//明细中有批次信息并且主数量和明细数量相等
			if (validateStr(rowdata.inv_detail_data)
					&& validateStr(rowdata.sum_amount)
					&& rowdata.amount == rowdata.sum_amount) {
				...
				ajaxJsonObjectByUrl("../../queryMatInvByFifo.do?isCheck=false",
						invPara, function(responseData) {
							data = responseData;
						}, false);
				//变更主数据中材料批次信息
				grid.updateCell('sum_amount', gridRowData.amount, gridRowData);
				grid.updateCell('inv_detail_data', JSON.stringify(data.Rows),
						gridRowData);
			}
		}
		return data;
	...
	function changeDetailAmount() {
		...
		if (dataDetail.length > 0) {
			...
			grid.updateCell('amount_money', sumAmount * gridRowData.price,
					gridRowData);
			grid.updateCell('sale_money', sumAmount * gridRowData.sale_price,
					gridRowData);
			grid.updateCell('sell_money', sumAmount * gridRowData.sell_price,
					gridRowData);
			grid.updateCell('sum_amount', sumAmount, gridRowData);
			grid.updateCell('inv_detail_data', JSON.stringify(dataDetail),
					gridRowData);
		} else {
			grid.updateCell('amount', sumAmount, gridRowData);
			grid.updateCell('amount_money', sumAmount * gridRowData.price,
	...
	function f_detail_onAfterEdit(e) {
		if ("amount" == detail_name) {
			detailGrid.updateCell('amount_money', e.record.amount
					* e.record.price, e.record);
			detailGrid.updateCell('sale_money', e.record.amount
					* e.record.sale_price, e.record);
			detailGrid.updateCell('sell_money', e.record.amount
					* e.record.sell_price, e.record);
			changeDetailAmount();
		}
	}
	// 编辑单元格提交编辑状态之前作判断限制
	...
	function matTranBySingleImport() {//整单出库 
		...
		var store_id = liger.get("out_store_id").getValue() ? liger.get(
				"out_store_id").getValue().split(',')[0] : 'null';
		var store_code = liger.get("out_store_id").getText() ? liger.get(
				"out_store_id").getText().split(' ')[0] : 'null';
		var store_name = liger.get("out_store_id").getText() ? liger.get(
				"out_store_id").getText().split(' ')[1] : 'null';
	...
	function paddingInv_beginBar() {
		if (event.keyCode == 13) {
			...
			if (store_id && begin_bar) {
				...
				ajaxJsonObjectByUrl(
						"../../queryMatOutInvListByBar.do?isCheck=false", para,
		...
		<div position="top">
				<form name="form1" method="post"  id="form1" >
			       ...
			       <tr>
			        ...
			        <tr >
			           	<td align="right" class="l-table-edit-td store_id"  style="padding-left:20px;"><span style="color:red">*</span>调出仓库：</td>
			            <td align="left" class="l-table-edit-td store_id"><input name="out_store_id" type="text" id="out_store_id" ltype="text" validate="{required:true}" /></td>
			            <td align="left"></td>
			            <td align="right" class="l-table-edit-td store_id"  style="padding-left:20px;"><span style="color:red">*</span>调入仓库：</td>
			            <td align="left" class="l-table-edit-td store_id"><input name="in_store_id" type="text" id="in_store_id" ltype="text"  validate="{required:true}" /></td>
			            <td align="left"></td>
			            <td align="right" class="l-table-edit-td hos_id"  style="padding-left:20px;"><span id="bus_type_span" style="color:red;display:none;">*</span>业务类型：</td>
			            <td align="left" class="l-table-edit-td hos_id" ><input name="bus_type" type="text" id="bus_type" ltype="text" disabled="disabled" validate="{required:true}" /></td>
			            <td align="left"></td>
			        </tr>  
			         <tr>
			            <td align="right" class="l-table-edit-td hos_id"  style="padding-left:20px;"><span style="color:red;display:none;">*</span>调出单位：</td>
			            <td align="left" class="l-table-edit-td hos_id" ><input name="out_hos_id" type="text" id="out_hos_id" ltype="text" disabled="disabled" validate="{required:true}" /></td>
			            <td align="left"></td>
			            <td align="right" class="l-table-edit-td hos_id"  style="padding-left:20px;"><span id="tran_method_span" style="color:red;display:none;">*</span>调拨方式：</td>
			            <td align="left" class="l-table-edit-td hos_id" ><input name="tran_method" type="text" id="tran_method" ltype="text" disabled="disabled"  validate="{required:true}" /></td>
			            <td align="left"></td>
			            <td align="right" class="l-table-edit-td hos_id"  style="padding-left:20px;"><span style="color:red;display:none;">*</span>调入单位：</td>
			            <td align="left" class="l-table-edit-td hos_id"><input name="in_hos_id" type="text" id="in_hos_id" ltype="text" disabled="disabled"  validate="{required:true}" /></td>
			            <td align="left"></td>
