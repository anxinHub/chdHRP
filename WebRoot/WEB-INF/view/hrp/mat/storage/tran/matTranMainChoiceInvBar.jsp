...
<script type="text/javascript">
    ...
    function  query(){
    		...
    		grid.options.parms.push({
    			name : 'is_bar', 
    			value : $('input[name="is_bar"]:checked ').val()
    		});  
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
				{ 
					...
					display: '单位', name: 'unit_name', align: 'left',width: '60'
				}, { 
			...
			dataAction: 'local', dataType: 'server', usePager:true, url:'queryMatTranInvBatchList.do?isCheck=false',
			...
			toolbar: { 
				items: [
					...
					{ line:true }, 
					{ text: '生成调拨单（<u>A</u>）', id:'add', click: addNew, icon:'add' },
					{ line:true }, 
	...
	function addNew(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定生成调拨单?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("queryTranInvListByChoiceInv.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
                		//清空父页面表格数据
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
        ...
        <tr>
			...
			<td align="left">
				<input name="is_bar" type="radio" id="is_bar1" value="1" checked="checked" ltype="text" />按条码
				&nbsp;&nbsp;
				<input name="is_bar" type="radio" id="is_bar2" value="0" ltype="text" />不按条码
			</td>
