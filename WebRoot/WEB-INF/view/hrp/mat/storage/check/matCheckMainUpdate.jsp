...
<script type="text/javascript">
     ...
     $(function (){
        ...
        $("#store_id").bind("change",function(){
	    	//loadHead(null);
	    	grid.columns[4].editor.grid.url = '../../queryMatStockInvList.do?isCheck=false&store_id=' 
     ...
     function  save(){
 		...
 		if(check_detail_data.length > 0){
   			$.each(check_detail_data, function(d_index, d_content){ 
   	      		if(d_content.inv_id){
	   	      		...
	   	      		var key=d_content.inv_id +"|"+d_content.bar_code+"|"+d_content.price;
   				
	   				if(targetMap.get(key)== null || targetMap.get(key) == 'undefined' || targetMap.get(key) == ""){   					
	   					targetMap.put(key ,value);   					
	   				}else{   					
	   					validate_detail_buf.append(targetMap.get(key)+"与"+value+"材料编码、条形码、单价不能重复" + "\n\r");   					
	   				}
	   				if(d_index!=0){
	   					invMsgs+='@';
	   				}
	   				invMsgs+="'"+d_content.inv_msg+"'";
	   				rows = rows + 1;
   	      		}	  
        ...
        ajaxJsonObjectByUrl("updateMatCheckMain.do",formPara,function(responseData){
 ...
 function loadForm(){
    ...
    function loadDict(){
    	...
    	//autocompleteAsync("#store_id", "../../queryMatStore.do?isCheck=false", "id", "text", true, true,"",false, false,'180');
    	autocompleteAsync("#store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true,{is_write:1},false, false,'180');
    	liger.get("store_id").setValue("${store_id},${store_no}");
		liger.get("store_id").setText("${store_code} ${store_name}");
    	//autocompleteAsync("#dept_id", "../../queryMatDeptDict.do?isCheck=false", "id", "text", true, true,{is_last : 1},false,false,'180');
		...
		autocompleteAsync("#emp_id", "../../queryMatEmpDict.do?isCheck=false", "id", "text", true, true,{dept_id: "${dept_id}"},false,false,'180');
    ...
    function loadHead(){
           ...
           columns: [ 
                     ...
                     { display: '材料编码', name: 'inv_code', align: 'left',width:100,
                    	 render : function(rowdata, rowindex, value) {
	     						...
	     						return '<a href=javascript:updateOpen("' 
	     							+ rowdata.check_id+'@'+rowdata.inv_msg
	     							+ '")>'+value+'</a>';
	     				 },
                    	 totalSummary: {
      	                    align : 'right',
							    ...
							    delayLoadGrid : false,
						                grid: {
						                	columns : [ {display : '交易编码', name : 'bid_code', width : 100, align : 'left'}, 
						                	            ...
						                	            {display : '账面数量', name : 'cur_amount', width : 90, align : 'right'}, 
						                	            {display : '账面金额', name : 'cur_money', width : 100, align : 'right', 
							                            	render : function(rowdata, rowindex, value) {
								                  				return value == null ? "" : formatNumber(value, '${p04005 }', 1);
							                  				}
						                	            }, 
						                	            {display : '有效日期', name : 'inv_date', width : 90, align : 'left'}, 
						                	            {display : '供应商', name : 'sup_name', width : 140, align : 'left'}, 
                     ...
                     { display: '材料信息', name: 'inv_msg',hide:true,  align: 'left',width:180},
                     { display: '材料no', name: 'inv_no',hide:true,  align: 'left',width:180},
                     ...
                     usePager : false,width : '100%',height : '93%',enabledEdit : state == 1 ? true : false,fixedCellHeight:true,//data:matCheckDetail,
					selectRowButtonOnly:true,checkbox: true,rownumbers:true,isScroll:true,
					url:'queryMatCheckDetailByCheckID.do?isCheck=false',parms :para,
					onBeforeEdit: f_onBeforeEdit, onAfterEdit: f_onAfterEdit,heightDiff:-20,
				 	toolbar: { items: [
			                     	{ text: '保存', id:'add', click: saveMatOutMain,icon:'add', disabled: state == 1 ? false : true },
			                     	{ line:true },
			                     	{ text: '删除', id:'delete', click: deleteRow,icon:'delete', disabled: state == 1 ? false : true },
			                     	{ line:true },
			                     	{ text: '引入仓库材料', id:'add', click: matCheckByStoreImport,icon:'up',disabled: state == 1 ? false : true },
 					                { line:true },
			                     	...
			                     	{ line:true },
			                     	{ text: '引入高值耗材盘点数量', id:'checkMobileNum', click: checkMobileNum,icon:'copy' , disabled: state == 1 ? false : true }
							]}
                 });
	    ...
	    function  query(){
        	...
        	grid.options.parms.push({name:'check_code',value : '${check_code}'}); 
	    ...
	    function updateOpen(msg){	
			...
			parent.$.ligerDialog.open({
				...
				url: "hrp/mat/storage/check/updateMatCheckDetailByInvBatchPage.do?isCheck=false&"+paras, 
	    ...
	    function reCalculation(){   	
	    	var check_detail_data = gridManager.rows;
	    	$.each(check_detail_data, function(d_index, d_content){ 
	    		gridManager.updateCell('pl_amount', (d_content.chk_amount -d_content.cur_amount), d_content); 
	    		gridManager.updateCell('pl_money', (d_content.chk_amount -d_content.cur_amount)*d_content.price, d_content);       		  
			}); 
	    }
	    ...
	    function checkMobileNum(){
	    	if(gridManager.rows.length > 0){
	    		...
	    		ajaxJsonObjectByUrl("../checkMobile/queryCheckMmobileCheAmount.do?isCheck=false", mapVo, function(responseData) {
	    			var check_detail_data = gridManager.rows;
	    	    	$.each(check_detail_data, function(d_index, d_content){ 
	    	    		if(responseData.length > 0){
		    	    		for(var i in responseData){
		    	    			if(responseData[i].BAR_CODE == d_content.bar_code) {
				    	    		gridManager.updateCell('chk_amount', responseData[i].CHK_AMOUNT, d_content);
				    	    		gridManager.updateCell('chk_money', d_content.cur_money, d_content); 
				    	    		gridManager.updateCell('pl_amount', responseData[i].CHK_AMOUNT - d_content.cur_amount + "", d_content);
				    	    		gridManager.updateCell('pl_money', eval(responseData[i].CHK_AMOUNT - d_content.cur_amount) * d_content.price, d_content); 
		    	    			}
		    	    		}
	    	    		}
	    			}); 
	    ...
	    function matCheckByStoreImport(){//引入仓库材料
			...
			$.ligerDialog.open({
				...
				url: "matCheckMainByStorePage.do?isCheck=false&paras="+paras, 
				modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
				buttons: [ 
		...
		function printSet(){
			...
			/* parent.parent.$.ligerDialog.open({
				url : 'hrp/mat/storage/check/matCheckPrintSetPage.do?template_code=04027&use_id='+useId,
		...
		<form name="form1" method="post"  id="form1" >
				<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
			        ...
			        <tr>
			            ...
			            <td align="left" class="l-table-edit-td" colspan="4"><input name="brief" type="text" id="brief" ltype="text"  value="${brif}"/></td>
			            <td align="left"></td>
			        </tr> 
			    </table>
			</form>
