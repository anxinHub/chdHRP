...
// 		});
	...
	function query() {
		...
		grid.options.parms.push({
			name : 'begin_order_date',value : $("#begin_order_date").val()
		});
		grid.options.parms.push({
			name : 'end_order_date',value : $("#end_order_date").val()
		});
		grid.options.parms.push({
			name : 'sup_id',
		...
		grid.options.parms.push({
			name : 'dept_id',value : liger.get("app_dept").getValue() == null ? "" : liger.get("app_dept").getValue().split(".")[0]
		});
		grid.options.parms.push({
			name : 'dept_no',value : liger.get("app_dept").getValue() == null ? "" : liger.get("app_dept").getValue().split(".")[1]
		});
		grid.options.parms.push({
			name : 'is_com',value : 0
		});
	...
	function deliveryNo() {
		...
		if (data.length == 0) {
			...
			ajaxJsonObjectByUrl("../../../sup/supdeliverydetail/queryDetail.do?isCheck=false&delivery_no="+data,{}, function(responseData) {
							...
							columns : [
									{
										...
										display : '送货单ID',name : 'delivery_id',align : 'left',hide : true
									},{
										display : '送货单号',name : 'delivery_no',align : 'left',width : '120',
...
// 										}
										...
										display : '需求仓库',name : 'store_name',align : 'left',width : '120',
									},{
										...
										display : '送货单数量',name : 'delivery_amount',width : 80,type : 'float',align : 'right',
										totalSummary : {
											align : 'right',
										...
										display : '灭菌批号',name : 'disinfect_no',width : 120,align : 'left'
									},{
									...
									dataAction : 'server',dataType : 'server',usePager : true,width : '98%',height : '90%',
									url : '../../../sup/supdeliverydetail/queryDetailByStore.do?isCheck=false',pageSize:200,
	...
	function save() {
		...
		if (validateGrid()) {
			...
			$.ligerDialog.confirm('确定生成入库单?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("../../../sup/supdeliverymain/addGenerateInDetail.do?isCheck=false",
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
		<tr>
			...
			<td align="right" class="l-table-edit-td" width="10%">
				<span id="sup_code_span" style="color:red">*</span>供货单位：
			</td>
			<td align="left" class="l-table-edit-td" width="20%">
			...
			<td align="right" class="l-table-edit-td">
				<span id="sup_code_span" style="color:red">*</span>送货单类型：
			</td>
		...
		<tr>
			<td align="right" class="l-table-edit-td"><span id="sup_code_span" style="color: red">单据条码</span>：</td>
			<td align="left" class="l-table-edit-td"><input name="delivery_no"	type="text" id="delivery_no" ltype="text" validate="{required:true}" /></td>
			<td align="right" class="l-table-edit-td"><span id="sup_code_span" style="color: red">条形码</span>：</td>
			<td align="left" class="l-table-edit-td"><input name="sn" type="text" id="sn" ltype="text" validate="{required:true}" /></td>
