...
<script type="text/javascript">
    ...
    function loadHead(){
		...
		//获取本期增加、减少查询字段
    	ajaxJsonObjectByUrl("queryMatAccountReportInvStockToFimColumns.do?isCheck=false", "", function (responseData){
    		if(responseData.Rows.length > 0){
    			...
    			add_columns.append("{display: '合计金额', name: 'add_sum_money', align: 'right',formatter:'###,##0.00',minWidth: 100, "+render+"} ]");
    			reduct_columns.append("{display: '合计金额', name: 'reduct_sum_money', align: 'right',formatter:'###,##0.00', minWidth: 100,formatter:'###,##0.00', "+render+"} ]");
			}
    	}, false);
    	...
    	grid = $("#maingrid").ligerGrid({
    		columns : columns, 
			dataAction: 'server', dataType: 'server', usePager: false, url: 'queryMatAccountReportInvStockToFim.do', 
...
/*     function lodopPrint(){
    	...
    	var head="<table class='head' width='100%'><tr><td>单位：${hos_name}</td></tr>";
 		...
 		grid.options.lodop.title=liger.get("store_code").getText().split(" ")[1]+"材料库存汇总表(财务)";
    } */
	...
	function print(){
    	...
    	var heads={
        		...
        		"rows": [
    	          {"cell":0,"value":"统计年月："},
    	          {"cell":1,"value":""+liger.get("begin_year").getText()+"年"+liger.get("begin_month").getText()+"至"+liger.get("end_year").getText()+"年"+liger.get("end_month").getText()},
    	          {"cell":4,"value":""+$("#store_type_td").find(":radio:checked").val()==1?'虚仓':'仓库'+"："},
    	          {"cell":5,"value":""+liger.get("store_code").getText()==''?'空':liger.get("store_code").getText().split(" ")[1]+""}
    	          ]}; 
    	//表尾
    	var maker="${user_name}";
		var foots = {
