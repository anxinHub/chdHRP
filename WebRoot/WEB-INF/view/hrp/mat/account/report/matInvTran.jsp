...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
			...
			dataAction: 'server',dataType: 'server',usePager:true,url:'queryMatCommonIn.do',
    ...
    function remove(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定删除?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("deleteMatCommonIn.do",{ParamVo : ParamVo.toString()},function (responseData){
  ...
  //批量复制
	function copy_no(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定复制?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("copyMatCommonIn.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function offset(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定冲账?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("offsetMatCommonIn.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function audit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定审核?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("auditMatCommonInBatch.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
	...
	function unAudit(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定消审?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("unAuditMatCommonInBatch.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function confirm(){
		...
		if (data.length == 0){
			...
			$.ligerDialog.confirm('确定入库确认?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("confirmMatCommonInBatch.do",{ParamVo : ParamVo.toString()},function (responseData){
						if(responseData.state=="true"){
							query();
						}
					});
    ...
    function loadDict(){
		...
		autocomplete("#bus_type_code", "../../../queryMatBusType.do?isCheck=false", "id", "text", true, true, {sel_flag : 'in'}, true);
