...
<script type="text/javascript">
	...
	function  query(){
		...
		grid.options.parms.push({
			name: 'bill_date_begin',
			value: $("#bill_date_begin").val()
		});
		grid.options.parms.push({
			name: 'bill_date_end',
			value: $("#bill_date_end").val()
		});
		grid.options.parms.push({
			name: 'store_id',
	...
	function loadHead(){
		grid = $("#maingrid").ligerGrid({
			columns: [ { 
				display: '发票流水号', name: 'bill_code', align: 'left', width: 140
			}, {  
				...
				display: '已优惠款金额', name: 'had_dis_money', align: 'right', width: 100,
				render:function(rowdata, rowindex, value){
					return formatNumber(value, '${p04005 }', 1);
				}
			}, { 
			...
			url: 'queryMatBillByPay.do?isCheck=false',
	...
	function checkAllRow(checked, element){
		...
		$.each(data, function(index, item){
			if(item.detail_data){
				detailData = jsonRowsToObject(item.detail_data, "bill_detail_id");
				$.each(detailData, function(){
					this.check_row = checked ? "1" : "0";
					this.pay_money = checked ? this.not_pay_money : 0;
				})
	...
	function checkRow(checked, rowdata, rowindex, rowobj){
		if(rowdata.detail_data){
			detailData = jsonRowsToObject(rowdata.detail_data, "bill_detail_id");
			$.each(detailData, function(){
				this.check_row = checked ? "1" : "0";
				this.pay_money = checked ? this.not_bill_money : 0;
			})
	...
	function showDetail(row, detailPanel, callback){
		...
		detailGrid = $(detailDiv).css({'margin-top':10, 'margin-left':60}).ligerGrid({
			columns: [ { 
				display: '入库单号', name: 'in_no', width: 120, align: 'left'
			}, { 
				...
				display: '付款金额', name: 'pay_money', align: 'right', width: 60, hide: true
			}, { 
	...
	function f_getDetailData(rowdata){
		...
		//有明细信息
		if(rowdata.detail_data){
			var rows = jsonRowsToObject(rowdata.detail_data, "bill_detail_id"); 
			...
			ajaxJsonObjectByUrl("queryMatBillDetailByPay.do?isCheck=false", para, function(responseData){
				data = responseData;
			}, false);
			//变更主数据中材料批次信息
	...
	function selectDetailRow(rowdata, rowid, rowobj){
		...
		detailGrid.updateCell('pay_money', rowdata.not_pay_money, rowdata);
	...
	function unSelectDetailRow(rowdata, rowid, rowobj){
		...
		detailGrid.updateCell('pay_money', 0, rowdata);
		var detailDate =  detailGrid.getData();
		var is_check = false;
	...
	function addToParent(){
		...
		ajaxJsonObjectByUrl("queryMatPayDetailByBill.do?isCheck=false", para, function(resData){
			if(resData.Rows){
	...
	function loadDict(){
		...
		autoCompleteByData("#bill_type", [{id: "1", text: "普通发票"}, {id: "2", text: "红冲发票"}], "id", "text", true, true, "", false, parentData.pay_bill_type == '1' ? '0' : '1');
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td">
				<table>
					<tr>
						<td>
							<input class="Wdate" type="text" id="bill_date_begin" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
						</td>
						<td align="left" class="l-table-edit-td">至</td>
						<td>
							<input class="Wdate" type="text" id="bill_date_end" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
						</td>
					</tr>
				</table>
			</td>
			...
			<td align="right" class="l-table-edit-td" >发票类别：</td>
			<td align="left" class="l-table-edit-td">
