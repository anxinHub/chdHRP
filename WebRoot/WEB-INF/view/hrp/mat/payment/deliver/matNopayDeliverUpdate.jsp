...
<html style="overflow:hidden;">
  <head>
    ...
    var deliver_id = '${matInMain.deliver_id}';
    var deliver_no = '${matInMain.deliver_no}';
    ...
    var store_id = '${matInMain.store_id}';
			...
			if (v.inv_id) {
				...
				if(v.is_quality == 1){
					if(!v.inva_date){
						...
						ajaxJsonObjectByUrl("../../storage/in/queryMatStorageInBatchInva.do?isCheck=false", para, function (responseData){
				...
				if(v.is_disinfect == 1){
					if(!v.disinfect_date){
						...
						ajaxJsonObjectByUrl("../../storage/in/queryMatStorageInBatchDisinfect.do?isCheck=false", para, function (responseData){
							if(responseData.state=="false"){
     ...
     function  save(){
    	if(validateGrid()){
	        var formPara={
	        		deliver_id : '${matInMain.deliver_id}',
	        		deliver_no : '${matInMain.deliver_no}',
		        ...
		        ajaxJsonObjectByUrl("updateMatNopayDeliver.do?isCheck=false",formPara,function(responseData){
...
function loadForm(){
    ...
    function loadHead() {
		grid = $("#maingrid").ligerGrid({
			columns : [
						...
						editor : {
							...
							grid : {
								columns : [ 
										...
										{display : '材料名称', name : 'inv_name', width : 180, align : 'left'
											}, 
										{display : '物资类别', name : 'mat_type_code', width : 120, align : 'left'
											},
					...
					{display : '零售单价(E)', name : 'sell_price', width : 80, align : 'right', type : 'number',editor : {type : 'number'},
							render : function(rowdata, rowindex, value) {
								return rowdata.sell_price == null ? "" : formatNumber(rowdata.sell_price, '${p04006}', 1);
							}
						}, 
					{display : '零售金额', name : 'sell_money', width : 80, type : 'number', align : 'right',
							render : function(rowdata, rowindex, value) {
					...
					dataAction : 'server', dataType : 'server', usePager : false, url : 'queryMatNopayDeliverDetail.do?isCheck=false&deliver_id=${matInMain.deliver_id}', width : '100%', height : '93%',
			...
			toolbar : {
				items : [ 
					...
					{line : true}, 
					{text : '消审（<u>X</u>）', id : 'unAudit', click : unAudit, icon : 'unaudit', disabled:state == 1 ? true: false},
					{line : true}, 
					{text : '生成发票(<u>W</u>)', id : 'create_bill', click : create_bill, icon : 'edit', disabled: state == 1 ? true : false},
    ...
    function create_bill(){
    	if(state != 3){
			$.ligerDialog.error("单据未审核，该功能不可用");
			return;
		}
    	...
    	$.ligerDialog.confirm('确定生成发票?', function (yes){
			if(yes){
				ajaxJsonObjectByUrl("addMatBillByBill.do?isCheck=false&"+para, "", function (responseData){});
			}
		}); 
    }
	...
	function before_no(){
		...
		ajaxJsonObjectByUrl("queryMatNopayDeliverBeforeNo.do?isCheck=false",paras.toString(),function(responseData){
			if(responseData.state=="true"){
	...
	function next_no(){
		var paras = 
			...
			"deliver_no="+deliver_no  +"&"+ 
			"bus_type_code="+bus_type_code;
		ajaxJsonObjectByUrl("queryMatNopayDeliverNextNo.do?isCheck=false",paras.toString(),function(responseData){
			if(responseData.state=="true"){
				parentFrameUse().update_open(responseData.update_para);
				this_close();
			}else{
...
/* 		autocomplete("#store_id", "../../queryMatStore.do?isCheck=false", "id", "text", true, true); */
		autocomplete("#store_id", "../../queryMatStoreDictDate.do?isCheck=false", "id", "text", true, true,{is_write:1});
		...
		liger.get("confirmer").setValue('${matInMain.confirmer}');
    	liger.get("confirmer").setText('${matInMain.confirmer_name}');
    	//采购员 
		...
		liger.get("protocol_code").setValue('${matInMain.protocol_id}');
        ...
        $("#brief").ligerTextBox({width:480, value : '${matInMain.brief}'});
		...
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input name="deliver_no" type="text" id="deliver_no" value="${matInMain.deliver_no}" ltype="text" disabled="disabled"/>
	            </td>
	            <td align="right" class="l-table-edit-td" ><b>原单据号<font color="red">*</font>:</b></td>
	            <td align="left" class="l-table-edit-td">
	            	<input name="origin_no" type="text" id="origin_no" value="${matInMain.origin_no}" ltype="text" />
	            </td>
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td">
	            	<input class="Wdate" name="in_date" id="in_date" value="${matInMain.in_date}" type="text"  onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" validate="{required:true}" />
	            </td>
	        ...
	        <tr>
	            ...
	            <td align="left" class="l-table-edit-td" >
	            	<input id="bill_no" type="text" value=" ${matInMain.bill_no}" />
	            </td>
	            <td align="right" class="l-table-edit-td" >发票日期：</td>
	            <td align="left" class="l-table-edit-td" >
	            	<input class="Wdate" id="bill_date" type="text" value=" ${matInMain.bill_date}" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/>
	            </td>
			</tr>
			<tr>
