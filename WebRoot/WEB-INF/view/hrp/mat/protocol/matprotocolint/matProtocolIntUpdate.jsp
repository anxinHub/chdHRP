  ...
  <head>
     ...
     $(function (){
		 ...
		 $("#inv").click(function(){
			 if(!$("#detailgrid").is(":hidden")){
				 $.ligerDialog.open({url: '../matprotocolmain/matInvDetailPage.do?isCheck=false',height: 500,width: 900, 
            	 ...
            	 $.ligerDialog.confirm('确定删除?', function (yes){
                 	if(yes){
                 		var delData =[];
	             	 	...
	             	 	if(delData.length >0){
	             	 		ajaxJsonObjectByUrl("../matprotocolmain/deleteDetail_File.do",{delData : delData.toString()},function (responseData){
   ...
   //保存
     ...
     function  save(){
        ...
        ajaxJsonObjectByUrl("updateMatProtocolInt.do",formPara,function(responseData){
     ...
     function loadHead(obj){
     	if("detail"==obj){
     		detailGrid = $("#detailgrid").ligerGrid({
     	           columns: [ 
                     ...
                     width: '100%', height: '240',url:'../matprotocolmain/queryMatProtocolDetail.do?isCheck=false&protocol_id=${protocol_id}',
                   });
		...
		if("file"==obj){
			fileGrid = $("#filegrid").ligerGrid({
				 ...
				 width: '100%', height: '240',url:'../matprotocolmain/queryMatProtocolFile.do?isCheck=false&protocol_id=${protocol_id}',
