    ...
    <script type="text/javascript">
     ...
     $(function (){
		 ...
		 $("#toptoolbar").ligerToolBar({ items: [
				{ text: '选择材料',icon:'down', click : addInv},
				{line:true},
     ...
     function typeId(){
		 ...
		 if(parm.type_id != null && parm.type_id !='' && parm.type_id !='${type_id}' ){
			 ajaxJsonObjectByUrl("queryMatProtocolTypePre.do?isCheck=false",parm,function (responseData){
	...
	function startDate(){
	 ...
	 function addInv(){
		 if(!$("#detailgrid").is(":hidden")){
			 $.ligerDialog.open({url: 'matInvDetailPage.do?isCheck=false',height: 500,width: 900, 
	 				title:'选择 材料',modal:true,showToggle:false,showMax:true,showMin: false,isResize:true
	 				});
		 }else{
            	 ...
            	 $.ligerDialog.confirm('确定删除?', function (yes){
                 	if(yes){
                 		var delData =[];
	             	 	...
	             	 	if(delData.length >0){
	             	 		ajaxJsonObjectByUrl("deleteDetail_File.do?isCheck=false",{delData : delData.toString()},function (responseData){
 	...
 	function add(){
 		if(!$("#detailgrid").is(":hidden")){
 			...
 			if (data.length == 0){
 	        	$.ligerDialog.error('请选择材料或修改后再保存!');
 	        	return;
 	        }else{
 	        	var ParamVo =[];
 	        	if(data.length >0){
	 			...
	 			ajaxJsonObjectByUrl("addMatProtocolDetail.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
	           		if(responseData.state=="true"){
	           			loadHead("detail");
	           		}
	           	});
			...
			var data = fileGrid.getData();
 			if (data.length == 0){
            	...
            	return;
 	        	...
 	        	ajaxJsonObjectByUrl("addMatProtocolFile.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
              		if(responseData.state=="true"){
              			loadHead("file");
              		}
              	});
     ...
     function  save(){
        ...
        ajaxJsonObjectByUrl("updateMatProtocolMain.do?isCheck=false",formPara,function(responseData){
     ...
     function loadHead(obj){
     	if("detail"==obj){
     		detailGrid = $("#detailgrid").ligerGrid({
     	           columns: [ 
                     ...
                     width: '100%', height: '240',url:'queryMatProtocolDetail.do?isCheck=false&protocol_id=${protocol_id}',
                   });
		...
		if("file"==obj){
			fileGrid = $("#filegrid").ligerGrid({
  	           columns: [ 
                    { display: '文档类别', name:'type_name', align: 'left',width:100,
                    	valueField : 'type_code',textField: 'typeName',
			 			editor: { type: 'select', url:'queryMatFileType.do?isCheck=false',valueField : 'type_code',textField: 'typeName',autocomplete : true }
				 		},
				 	{ display: '文档编码', name: 'file_code', align: 'left',editor: { type: 'text'},width:100
						},
			 		{ display: '文档名称', name: 'file_name', align: 'left',editor: { type: 'text'},width:100
			 			},
			 		{ display: '文档原件存放位置', name: 'at_local', align: 'left',editor: { type: 'text'},width:200
				 		},
				 	{ display: '文档路径', name: 'file_path', align: 'left',width:500
				 		},
				 	{ display: '管理科室', name: 'dept_name', align: 'left',width:120,
				 			valueField : 'dept_id',textField: 'deptName',
				 			editor: { type: 'select',valueField : 'dept_id',textField: 'deptName', url:'queryManaDept.do?isCheck=false',keySupport : true,autocomplete : true }
			 			},
				 	{ display: '管理员', name: 'emp_name', align: 'left',width:120,
			 				valueField : 'mana_emp',textField: 'mana_name',
				 			editor: { type: 'select', url:'queryManaEmp.do?isCheck=false',valueField : 'mana_emp',textField: 'mana_name',keySupport : true,autocomplete : true }
			 			},
				 	{ display: '上传', name: 'up', align: 'center',width:70,
			 				render:function(rowdata,index,value){
			 					return "<a href=javascript:upLodePage('"+rowdata.group_id+"|"+rowdata.hos_id+"|"+rowdata.copy_code+"|"+rowdata.file_id+"|"+rowdata.protocol_id+"|"+index+"')>上传</a>";
						 	}
				 		},
			 		{ display: '查看', name: 'look', align: 'center',width:70,
					 		render:function(rowdata,index,value){
					 			if(rowdata.look == 1){
					 				return "";
					 			}else{
				 ...
				 width: '100%', height: '240',url:'queryMatProtocolFile.do?isCheck=false&protocol_id=${protocol_id}',
     ...
     function openUpdateFlie(data){
		...
		var parm = 
			...
			"file_id="+vo[3]   +"&"+
			"protocol_id="+vo[4] 
		$.ligerDialog.open({ url : 'matProtocolFilePage.do?isCheck=false&'+parm,data:{}, 
			height: 205,width: 620, title:'修改',modal:true,showToggle:false,showMax:true,
	      	 ...
	      	 <form name="form1" method="post"  id="form1" >
			     <table cellpadding="0" cellspacing="0" class="l-table-edit" >
			        ...
			        <tr>
			            ...
			            <td align="left" class="l-table-edit-td" colspan="5">
			            	<textarea class="liger-textarea" name="contract_detail" id="contract_detail" rows="4" style="width: 510px;" >${contract_detail}</textarea>
			            </td>
			        ...
			        <tr>
	                	<td align="center" colspan="9"><input class="liger-button" id="save"  type="button" width="100" value="保存" /></td>
			        </tr>
			     </table>
		    </form>
        	</div>
