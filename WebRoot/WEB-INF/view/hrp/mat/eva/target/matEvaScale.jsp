...
<script type="text/javascript">
	...
	function saveScale(flag){
		...
		if(!$("#scale_point").val()) {
			...
			return false;
		}else if(parseFloat($("#scale_point").val()) > 1){
			$.ligerDialog.warn("标度比例不能大于1！");
			return false;
		}
		
		if($("#high_point").val() && parseFloat($("#high_point").val()) > parseFloat($("#scale_point").val())){
			$.ligerDialog.warn("上限值不能大于得分比例！");
			return false;
		}
		if($("#low_point").val() && parseFloat($("#low_point").val()) > parseFloat($("#high_point").val())){
			$.ligerDialog.warn("下限值不能大于上限值！");
			return false;
		}
		...
		ajaxJsonObjectByUrl("saveMatEvaScale.do?isCheck=false", para, function (responseData) {
