...
<script type="text/javascript">
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/asshouse/asschange/assChangeHouseAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssChangeHouse.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/asshouse/asschange/assChangeHouseUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
	...
	function printDetail(){
		...
		if (data.length == 0){
			...
			ajaxJsonObjectByUrl("queryAssChangeHouseStates.do?isCheck=false",{change_no: change_no.substring(0,change_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
