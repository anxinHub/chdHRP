...
<script type="text/javascript">
				...
				grid = $("#maingrid").ligerGrid(
						{
							...
							usePager : false,
							url : 'queryAssDisposalRecordHouse.do?isCheck=false&show_detail=1',
							width : '100%',
							height : '100%',
							checkbox : true,
							delayLoad :true,
							...
							toolbar : {
								...
								{line : true},  */
								{text : '记录确认（<u>S</u>）',id : 'audit',click : audit,icon : 'right'}, 
								{line : true},
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						usePager : false,
						url : 'queryAssDisposalRecordHouse.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						delayLoad :true,
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmDisposalRecordHouse.do", {
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/asshouse/assdisposal/disposalrecord/assDisposalRecordHouseAddPage.do?isCheck=false',
			title : '记录信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openView(obj){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/asshouse/assdisposal/disposalrecord/assViewSpecialPage.do?isCheck=false&'+parm,
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ "copy_code=" + vo[2] + "&" + "dis_r_no=" + vo[3];
		parent.$.ligerDialog.open({
			url : 'hrp/ass/asshouse/assdisposal/disposalrecord/assDisposalRecordHouseUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '记录信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssDisposalRecordHouse.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function printDetail(){
		...
		if (data.length == 0){
			...
			ajaxJsonObjectByUrl("queryAssDisposalRecordHouseStates.do?isCheck=false",{dis_r_no: dis_r_no.substring(0,dis_r_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
