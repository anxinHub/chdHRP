...
<script type="text/javascript">
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmChkRecordHouse.do", {
	...
	function openView(obj){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/asshouse/check/chkrecord/assViewSpecialPage.do?isCheck=false&'+parm,
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/asshouse/check/chkrecord/assChkRecordHouseAddPage.do?isCheck=false',
			title : '申报信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ "copy_code=" + vo[2] + "&" + "ass_chk_no=" + vo[3];
		parent.$.ligerDialog.open({
			url : 'hrp/ass/asshouse/check/chkrecord/assChkRecordHouseUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '申报信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssChkRecordHouse.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
