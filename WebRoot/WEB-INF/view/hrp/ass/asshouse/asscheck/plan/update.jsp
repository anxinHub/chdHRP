	...
	grid = $("#maingrid").ligerGrid(
			{
				...
				dataType : 'server',
				usePager : false,
				url : 'queryAssCheckDdetailHouse.do?isCheck=false',
				width : '100%',
				height : '100%',
				checkbox : true,
				rownumbers : true,
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssCheckDetailHouse.do?isCheck=false',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
...
function inventory() {
	...
	$(data).each(function(){
		...
		data1.push(this.ass_id+"@"+this.hos_id+"@"+this.copy_code+"@"+this.group_id+"@"+this.store_id+"@"+this.ass_card_no+"@"+this.ass_type_name+"@"+this.ass_name+"@"+this.store_name+"@"+this.acc_amount+"@"+this.check_amount+"@"+this.pl_amount+"@"+this.pl_reason+"@"+this.store_no+"@"+this.ass_no+"@"+this.check_no+"@"+this.check_plan_no);
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("saveHouseInventory.do?isCheck=false"  ,
	...
	if(is_dept=="01"){
		...
		if (data.length == 0 ) {
			...
			var formPara = {
					...
					ajaxJsonObjectByUrl("saveAssCheckSDetailHouse.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
...
function tree_open() {
	...
	if(is_dept=="01" || v_text=="仓库"){
		$.ligerDialog.open({
			url : 'assCheckSHouseAddPage.do?isCheck=false&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckSHouse();
				},
		...
		$.ligerDialog.open({
			url : 'assCheckDHouseAddPage.do?isCheck=falsefalse&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				text : '确定',
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckDHouse();
				},
				cls : 'l-dialog-btn-highlight'
			}, {
				text : '取消',
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckSHouse.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckDHouse.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
function generate(){
	var parm = "check_plan_no=" + '${check_plan_no}' + "&" + "check_no="
		+ $("#check_no").val() + "&" +"ass_type_id="+liger.get("ass_type_id").getValue();
	
$.ligerDialog.confirm('确定生成?', function(yes) {
if (yes) {
	ajaxJsonObjectByUrl("HouseGenerateDetail.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
			query();
			$("#check_no").val(responseData.check_no);
			//loadTree();
		}
	});
}
...
if (yes) {
	ajaxJsonObjectByUrl("auditHouse.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
...
if (yes) {
	ajaxJsonObjectByUrl("unAuditHouse.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
...
function copyAmount(){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("copyAmountAssCheckHouse.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function remove(){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定删除?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("delAmountAssCheckHouse.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function checkRgister(){
	...
	$.ligerDialog.open({
		...
		width: $(window).width(),
		url: 'assCheckSpHousePage.do?isCheck=false&'+parm,
...
function printSet(){
	...
	officeFormTemplate({template_code:"05060",use_id : useId})
}
	...
	/* if (data.length == 0){
		...
		$(data).each(function (){	
		 ...
		 var para={
				...
				template_code:'05060',
				class_name:"com.chd.hrp.ass.serviceImpl.check.house.AssCheckDetailHouseServiceImpl", 
		...
		<input type="hidden" name="check_plan_no" value="${check_plan_no}"/>
				...
				<tr>
					...
					<td align="left" class="l-table-edit-td"><input
						...
						validate="{required:true,maxlength:20}" value="${check_no}"/></td>
					<td align="left"></td>
					<td align="right" class="l-table-edit-td"
					...
					<%-- <input
						name="state_name" type="text" id="state_name" value="${state_name}" ltype="text" 
						validate="{required:true,maxlength:20}" /></td> --%>
