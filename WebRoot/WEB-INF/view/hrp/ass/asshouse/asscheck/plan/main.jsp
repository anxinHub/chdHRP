...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					onDblClickRow : function(rowdata, rowindex, value) {
						openUpdate(rowdata.group_id + "|" + rowdata.hos_id
								+ "|" + rowdata.copy_code + "|"+ rowdata.state + "|"
								+ rowdata.check_plan_no);
					}
				});
	...
	function add_open() {
		...
		$.ligerDialog.open({
			url : 'assCheckPlanHouseAddPage.do?isCheck=false&',
			data : {},
			height : 300,
			width : 450,
			title : '盘点任务单(专用设备)',
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckPlanHouse();
				},
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssCheckPlanHouse.do?isCheck=false",{ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("HouseAudit.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unAudit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("HouseUnAudit.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function finish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定盘点完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("HouseFinish.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unFinish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("HouseUnFinish.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function generate() {
		...
		if (data.length == 0 || data.length > 1) {
			...
			$.ligerDialog.confirm('生成的盘点单将会覆盖原有数据?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("HouseGenerate.do?isCheck=false	", formPara, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ "copy_code=" + vo[2] + "&" + "check_plan_no=" + vo[4]+ "&" + "state=" + vo[3]
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/asshouse/asscheck/plan/assCheckPlanHouseUpdatePage.do?isCheck=false&'+ parm,
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssCheckPlanHouse.do?isCheck=false",
