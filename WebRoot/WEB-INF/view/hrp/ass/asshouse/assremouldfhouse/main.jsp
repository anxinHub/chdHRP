...
<script type="text/javascript">
	...
	function confirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认竣工?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmAssRemouldFHouse.do?isCheck=false", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assremould/assremouldfhouse/assRemouldFHouseAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssRemouldFHouse.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assremould/assremouldfhouse/assRemouldFHouseUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
		...
		hotkeys('P', printDate);
         ...
         function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryAssRemouldFHouse.do?isCheck=false", selPara, function (responseData) {
