...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssInMainHouse.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssInMainHouse.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function initCard(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认入账?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmInMainHouse.do", {
	...
	function add_open() {
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/asscard/assInCardAddPage.do?isCheck=false&ass_nature=01&in_type=01',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssInMainHouse.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ " copy_code=" + vo[2] + "&" + "ass_card_no=" + vo[3]+ "&" + "ass_nature=" + '01'+"&" + "in_type=" + '01';
	...
	function printSetDetail(){
		...
		officeFormTemplate({template_code:"05054",use_id:useId});
	}
	...
	function printDetail(){
		...
		if (data.length == 0){
			...
			var para={
				class_name:"com.chd.hrp.ass.serviceImpl.card.AssCardHouseServiceImpl",
				method_name:"printAssCardHouseData",
			...
			ajaxJsonObjectByUrl("queryAssInHouseStates.do?isCheck=false",{ass_in_no:ass_in_no.substring(0,ass_in_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
