...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssRemouldAHouse.do?isCheck=false',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function Confirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认申报?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateAssRemouldAHouseConfirmState.do?isCheck=false", {
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assremould/assremouldahouse/assRemouldAHouseAddPage.do?isCheck=false',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssRemouldAHouse.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assremould/assremouldahouse/assRemouldAHouseUpdatePage.do?isCheck=false&'
							+ param,
					height : $(window).height(),
					width : $(window).width(),
					modal : true,
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssRemouldAHouse.do?isCheck=false", selPara,
				function(responseData) {
