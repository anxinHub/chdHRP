...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'assQueryMatFacAttr.do?isCheck=false',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
            	case "add":
            		parent.$.ligerDialog.open({url: 'hrp/ass/assfinadict/assAddMatFacAttr.do?isCheck=false&type_code='+type_code, 
                ...
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("assDeleteMatFacAttr.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
                    ...
                    return;
                case "import":
                	$.ligerDialog.open({url: 'assMatFacAttrImportPage.do?isCheck=false', height: 500,width: 1100, title:'导入',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,top:10 });
                case "export":
                	return;
                case "downTemplate":
    ...
    function openUpdate(obj){
		...
		var parm ="&group_id="+ 
		   ...
		   vo[3]   +"&fac_code="+
		   vo[4];
    	parent.$.ligerDialog.open({ url : 'hrp/ass/assfinadict/assMatFacAttrUpdatePage.do?isCheck=false&' + parm,data:{}, 
    ...
    function loadTree(){
    	$.post("assQueryHosFacTypeByMenu.do?isCheck=false",null,function (responseData){
  ...
  //打印数据
	 function printDate(){
		...
		ajaxJsonObjectByUrl("assQueryMatFacAttr.do?isCheck=false",printPara,function (responseData){
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("assQueryMatFacAttr.do?isCheck=false",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
					 trHtml+="<td>"+item.fac_id+"</td>";
					 trHtml+="<td>"+item.fac_code+"</td>";
