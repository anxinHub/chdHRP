...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssAcceptPhoto.do?isCheck=false',
					width : '100%',
					height : '90%',
					checkbox : true,
					rownumbers : true,
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					...
					height : 350,
					width : 600,
					url : 'hrp/ass/assacceptmain/acceptphoto/assAcceptPhotoAddPage.do?isCheck=false&accept_no=${accept_no}',
					modal : false,
					showToggle : false,
					showMax : false,
					showMin : true,
					...
					buttons : [ {
						...
						onclick : function(item, dialog) {
							dialog.frame.saveAssAcceptPhoto();
							/* viewImg(); */
						},
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@" + this.hos_id + "@"
								+ this.copy_code + "@" + this.accept_no + "@"
								+ this.photo_code + "@" + this.file_url)
					});
			parent.$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssAcceptPhoto.do?isCheck=false", {
					 ...
					 /* $.ajax({
			              type:'post',
			              url:'deleteAssPhoto.do?isCheck=false',
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ "copy_code=" + vo[2] + "&" + "accept_no=" + vo[3] + "&"
				+ "photo_code=" + vo[4] ;
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assacceptmain/acceptphoto/assAcceptPhotoUpdatePage.do?isCheck=false&'+parm,
			data : {},
			height : 350,
			width : 600,
			title : '验收照片修改',
