...
<script type="text/javascript">
	...
	$(function() {
		...
		if ('${state}' != 0) {//置黑不能点击
			...
			toobarmanageItem.setDisabled('allDdopt');
			toobarmanageItem.setDisabled('allNotDdopt');
	...
	function query(accept_id) {
		...
		grid.options.parms.push({
			name : 'accept_id',
			value : accept_id == "" ||accept_id ==  null || accept_id == "undefined"? "0" : accept_id
		});
	...
	function queryItem(accept_id,accept_detail_id) {
		...
		gridItem.options.parms.push({
			...
			value : accept_detail_id == "" || accept_detail_id ==  null || accept_detail_id == "undefined"? "0" : accept_detail_id
		});
				...
				.ligerGrid(
						{
							columns : [
									{
										...
										render : function(rowdata, rowindex,
											...
											if(rowdata.is_well == 0){
												return rowdata.ass_code+"&nbsp;&nbsp;<font color='red'><b>验</b></font>"
											}
									...
									},
									{
										...
										render : function(item) {
											item.accept_money = item.accept_amount * item.accept_price;
											return formatNumber(
							...
							dataType : 'server',
							usePager : false,
							url : 'queryAssAcceptDetail.do?accept_id=${accept_id}',
							width : '100%',
							height : '98%',
							checkbox : true,
							enabledEdit : true,
							...
							},
							{
								...
								editor : {
									...
									grid : {
										...
										url : '../queryAssAcceptItem.do?isCheck=false&ass_id='+ass_id,
										pageSize : 30
									},
									alwayShowInDown : true,
									keySupport : true,
							...
							},
							{
								...
								render : function(rowdata, index, value) {
									...
									if(rowdata.is_normal == 1){
										str1 = "<input type='radio' onChange='getRadioValue("+index+")' value='1' id='is_normal_yes"+index+"' name='is_normal"+index+"' checked='checked' />";
										str2 = "<input type='radio' onChange='getRadioValue("+index+")' value='2' id='is_normal_yno"+index+"' name='is_normal"+index+"'/>";
										str3 = "<input type='radio' onChange='getRadioValue("+index+")' value='9' id='is_normal_null"+index+"' name='is_normal"+index+"'/>";
									}else if(rowdata.is_normal == 2){
										str1 = "<input type='radio' onChange='getRadioValue("+index+")' value='1' id='is_normal_yes"+index+"' name='is_normal"+index+"'/>";
										str2 = "<input type='radio' onChange='getRadioValue("+index+")' value='2' id='is_normal_yno"+index+"' name='is_normal"+index+"' checked='checked' />";
										str3 = "<input type='radio' onChange='getRadioValue("+index+")' value='9' id='is_normal_null"+index+"' name='is_normal"+index+"'/>";
									}else if(rowdata.is_normal == 9){
										str1 = "<input type='radio' onChange='getRadioValue("+index+")' value='1' id='is_normal_yes"+index+"' name='is_normal"+index+"' />";
										str2 = "<input type='radio' onChange='getRadioValue("+index+")' value='2' id='is_normal_yno"+index+"' name='is_normal"+index+"' />";
										str3 = "<input type='radio' onChange='getRadioValue("+index+")' value='9' id='is_normal_null"+index+"' name='is_normal"+index+"' checked='checked'/>";
									}else{
										str1 = "<input type='radio' onChange='getRadioValue("+index+")' value='1' id='is_normal_yes"+index+"' name='is_normal"+index+"' />";
										str2 = "<input type='radio' onChange='getRadioValue("+index+")' value='2' id='is_normal_yno"+index+"' name='is_normal"+index+"'/>";
										str3 = "<input type='radio' onChange='getRadioValue("+index+")' value='9' id='is_normal_null"+index+"' name='is_normal"+index+"'/>";
  ...
  //打印
	function printDate() {
		 var useId=0;//统一打印
			...
			if (data.length == 0){
				...
				$(data).each(function (){		
					if(this.state != 2){
						accept_nos = accept_nos + this.accept_no + "<br>";
					}
	...
	function allDdopt(){
		...
		$.each(data,function(i,v){
			...
			row.is_normal = 1;
			$("input[name='is_normal"+i+"'][value=2]").attr('checked',false);
			$("input[name='is_normal"+i+"'][value=9]").attr('checked',false);
			$("input[name='is_normal"+i+"'][value=1]").attr('checked',true);
		});
		gridItem.reRender();
	}
											...
											ajaxJsonObjectByUrl(
													...
													function(responseData) {
														if (responseData.state == "true") {
														 	query($("#accept_id").val());
														 	queryItem($("#accept_id").val(),this.accept_detail_id); 
	...
	function copy(){
		...
		if (data.length == 0 || data.length > 1) {
			...
			var formPara = {'ass_id':ass_id.split("@")[0]};
	...
	function save() {
		...
		var formPara = {
			...
			source_code : liger.get("source_code").getText().split(" ")[0],
			ass_naturs:liger.get("ass_naturs").getValue(),
	...
	function loadDict() {
		...
		liger.get("contract_id").setValue(
				"${contract_id}@${contract_detail_id}");
		liger.get("contract_id").setText("${contract_name}");
		...
		autocomplete("#accept_emp",
				...
				"text", true, true);
		liger.get("accept_emp").setValue("${accept_emp}");
		liger.get("accept_emp").setText("${accept_emp_name}");
		$("#state").val("${state}");
		...
		liger.get("buy_code").setText("${buy_code} ${buy_name}");
		autocomplete("#device_code", "../queryDeviceDict.do?isCheck=false", "id",
				"text", true, true, null, false, null, "160");
		liger.get("device_code").setValue("${device_code}");
		liger.get("device_code").setText("${device_code} ${device_name}");
		autocomplete("#ass_naturs", "../queryAssNaturs.do?isCheck=false", "id",
				"text", true, true, null, false, null, "160");
		liger.get("ass_naturs").setValue("${ass_naturs}");
		liger.get("ass_naturs").setText("${ass_naturs} ${naturs_name}");
	}
	function photo(){
		if($("#accept_no").val() == ""){
...
<body onload="loadRows()">
	...
	<form name="form1" method="post" id="form1">
		<input name="accept_id" type="hidden" id="accept_id"
			value="${accept_id}" />
		<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
			<tr id="">
				...
				<td align="left" class="l-table-edit-td"><input
					name="accept_no" type="text" id="accept_no" ltype="text"
					value="${accept_no}" disabled="disabled"
					validate="{required:true,maxlength:20}" /></td>
				<td align="left"></td>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;" id="ass_year_font"><b>
			            <font color="red" >*</font></b>统计年度：</td>
			            <td align="right" class="l-table-edit-td"
					style="padding-left: 20px;" id="ass_year_f"><b>
			            <font color="red" >*</font></b>审核年度：</td>
				...
				<td align="right" class="l-table-edit-td"
					...
					<font color="red">*</font></b>统计月份：</td>
						<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;" id="ass_month_f"><b>
					<font color="red">*</font></b>审核月份：</td>
				<td align="left" class="l-table-edit-td"><input
					name="ass_month" type="text" id="ass_month" ltype="text"
					value="${ass_month}" class="Wdate"
					onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'MM'})" /></td>
				<td align="left"></td>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;"><b><font color="red">*</font></b>验收日期：</td>
				<td align="left" class="l-table-edit-td"><input
					name="accept_date" type="text" id="accept_date" ltype="text"
					value="${accept_date}" class="Wdate"
			...
			<tr>
				...
				<td align="left" class="l-table-edit-td"><input name="dept_id"
					type="text" id="dept_id" ltype="text" value="${dept_id}"
					validate="{required:true,maxlength:20}" /></td>
				<td align="left"></td>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;"><b><font color="red">*</font></b>验&nbsp;&nbsp;收&nbsp;&nbsp;人：</td>
				<td align="left" class="l-table-edit-td"><input
					name="accept_emp" type="text" id="accept_emp" ltype="text"
					value="${accept_emp}" validate="{required:true,maxlength:20}" /></td>
				<td align="left"></td>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;">合&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;同：</td>
				<td align="left" class="l-table-edit-td"><input
					name="contract_id" type="text" id="contract_id" ltype="text"
					value="${contract_id}" validate="{required:true,maxlength:20}" /></td>
				<td align="left"></td>
			</tr>
			<tr>
				<td align="right" class="l-table-edit-td"
				...
				<td align="left" class="l-table-edit-td"><input
					name="ass_naturs" type="text" ltype="text" value="${ass_naturs}" disabled="disabled" id="ass_naturs" /></td>
				<td align="left"></td>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;">设备来源：</td>
				<td align="left" class="l-table-edit-td"><input
					name="contract_id" type="text" ltype="text" value="${device_code}" id="device_code" /></td>
				<td align="left"></td>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;">申购类别：</td>
				<td align="left" class="l-table-edit-td"><input
					name="contract_id" type="text"  ltype="text" value="${buy_code}" id="buy_code" /></td>
				<td align="left"></td>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;">资金来源：</td>
				<td align="left" class="l-table-edit-td"><input
					name="contract_id" type="text"  ltype="text" value="${source_code}" id="source_code" /></td>
				<td align="left"></td>
				
			</tr>
			<tr>
				<td align="right" class="l-table-edit-td"
					style="padding-left: 20px;">摘&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;要：</td>
				<td align="left" class="l-table-edit-td" colspan="9"><textarea
						name="accept_desc" id="accept_desc" rows="3" cols="50"  style="border-color: #aecaf0;">${accept_desc}</textarea>
				</td>
				<td align="left"></td>
			</tr>
