...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#pay_date_beg").ligerTextBox({
			width : 90
		});
		$("#pay_date_end").ligerTextBox({
			width : 90
		});
		$("#ven_id").ligerTextBox({
			width : 160
	...
	function query() {
		...
		grid.options.parms.push({
			name : 'ass_nature',
			value : liger.get("ass_nature").getValue().split("@")[0]
		});
		grid.options.parms.push({
			name : 'beginDate',
			value : $("#pay_date_beg").val()
		});
		grid.options.parms.push({
			name : 'endDate',
			value : $("#pay_date_end").val()
		});
		grid.options.parms.push({
			name : 'ven_id',
	...
	function openViewPay(obj) {
		...
		parent.$.ligerDialog
				.open({
					...
					width : $(window).width(),
					url : 'hrp/ass/asspay/main/assViewPayPage.do?isCheck=false&'
							+ parm,
					modal : true,
					showToggle : false,
					showMax : true,
		...
		if (data.length == 0) {
			...
			$(data).each(
					function() {
						...
						busi_no=busi_no+this.pay_no+","

					});
			$.ligerDialog.confirm('确定付款?', function(yes) {

				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmAssPayForAccMain.do", {
						...
						if(responseData.state=="true"){
	                 			...
	                 			var para={
	        							...
	        							init_type:1,
	        							mod_code:'05',
	        							busi_type_code:'050401',
	        							busi_no:busi_no.substr(0,busi_no.length-1),
	        							busi_log_table:'ACC_BUSI_LOG_050401',
	        							huizong_sql:''
	        					};
	        					//console.log(para);
	        					var loadIndex = layer.load(1);
	        		    		ajaxJsonObjectBylayer("/CHD-HRP/hrp/acc/autovouch/assautovouch/queryVouchJsonByBusi.do",para,function(responseData){	
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			height : $(window).height(),
			width : $(window).width(),
			url : 'hrp/ass/asspay/main/assPayMainAddPage.do?isCheck=false&',
			modal : true,
			showToggle : false,
			showMax : true,
			showMin : false,
	...
	function lodopPrint() {
		...
		grid.options.lodop.title = "资产付款单";
	}
	...
	function printDate() {
		...
		if (data.length == 0) {
			...
			var para = {
				...
				template_code : '05087',
				class_name : "com.chd.hrp.ass.serviceImpl.pay.main.AssPayMainServiceImpl",
				method_name : "queryAssPayDY",
			...
			ajaxJsonObjectByUrl("queryAssPayState.do?isCheck=false", {
				paraId : pay_no.substring(0, pay_no.length - 1),
				state : 2
			}, function(responseData) {
				if (responseData.state == "true") {
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssPayMain.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height : $(window).height(),
			width : $(window).width(),
			url : 'hrp/ass/asspay/asspayforaccmain/assPayForAccMainUpdatePage.do?isCheck=false&'
					+ parm,
			modal : true,
			showToggle : false,
			showMax : true,
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit"
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td" width="5%"><input
				name="pay_date_beg" type="text" id="pay_date_beg" class="Wdate"
				onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" /></td>
			<td align="left" width="2%">至：</td>
			<td align="left" class="l-table-edit-td"><input
				name="pay_date_end" type="text" id="pay_date_end" class="Wdate"
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td">
            	...
            	</select> --> <input name="state" type="text" id="state" />
			</td>
			<td align="left"></td>
			...
			<td align="left" class="l-table-edit-td"><input
				name="ass_nature" type="text" id="ass_nature" /></td>
		</tr>
	</table>
