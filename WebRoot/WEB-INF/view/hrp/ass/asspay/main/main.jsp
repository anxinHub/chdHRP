...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '付款单号', name: 'pay_no', align: 'left', frozen: true,width : 120,
                     ...
                     { display: '付款日期', name: 'pay_date', align: 'left',width : 120
					 		},
                     { display: '付款金额', name: 'pay_money', align: 'right',width : 120,
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssPayMain.do',delayLoad :true,
                     ...
                     toolbar: { items: [
				    	...
				    	{ line:true },
				    	{ text: '付款确认（<u>G</u>）', id:'confirm', click: confirm,icon:'ok' }
   	...
   	function openViewPay(obj){
   		...
   		parent.$.ligerDialog.open({
   			...
   			width: $(window).width(),
   			url: 'hrp/ass/asspay/main/assViewPayPage.do?isCheck=false&'+parm,
    ...
    function confirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定付款?', function(yes) {
				...
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmAssPay.do", {
	...
	officeFormTemplate({template_code:"05087",use_id:useId});
	}
	...
	//打印
	    function printDate(){
		    	...
		    	ajaxJsonObjectByUrl("queryAssPayState.do?isCheck=false",{paraId:pay_no.substring(0,pay_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
						officeFormPrint(para);
					}
				});
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssPayMain.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/asspay/main/assPayMainUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
    }
