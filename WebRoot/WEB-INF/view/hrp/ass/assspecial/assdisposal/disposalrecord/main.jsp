...
<script type="text/javascript">
				...
				grid = $("#maingrid").ligerGrid(
						{
							...
							usePager : false,
							url : 'queryAssDisposalRecordSpecial.do?isCheck=false&show_detail=1',
							width : '100%',
							height : '100%',
							checkbox : true,
							rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						usePager : false,
						url : 'queryAssDisposalRecordSpecial.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function unconfirm() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateUnConfirmDisposalRecordSpecial.do?isCheck=false", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openView(obj){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assspecial/assdisposal/disposalrecord/assViewSpecialPage.do?isCheck=false&'+parm,
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assspecial/assdisposal/disposalrecord/assDisposalRecordSpecialAddPage.do?isCheck=false',
			title : '记录信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ "copy_code=" + vo[2] + "&" + "dis_r_no=" + vo[3];
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assspecial/assdisposal/disposalrecord/assDisposalRecordSpecialUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '记录信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssDisposalRecordSpecial.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
