...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						usePager : false,
						url : 'queryAssDisposalApplySpecial.do?isCheck=false&show_detail=1',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						usePager : false,
						url : 'queryAssDisposalApplySpecial.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function unconfirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('取消确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateUnConfirmDisposalApplySpecial.do?isCheck=false", {
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmDisposalApplySpecial.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
    ...
    function print(){
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			paraId :dis_a_no.substring(0,dis_a_no.length-1) ,
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assspecial/assdisposal/disposalapply/assDisposalApplySpecialAddPage.do?isCheck=false',
			title : '申报信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assspecial/assdisposal/disposalapply/assDisposalApplySpecialUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '申报信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssDisposalApplySpecial.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function loadDict() {
		...
		$("#create_date_beg,#create_date_end,#apply_date_beg,#apply_date_end,#ass_ins").ligerTextBox({width : 85});
