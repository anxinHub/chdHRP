...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssBackMainSpecial.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
				...
				grid = $("#maingrid").ligerGrid(
						{
							...
							dataType : 'server',
							usePager : true,
							url : 'queryAssBackMainSpecial.do?isCheck=false&show_detail=0',
							width : '100%',
							height : '100%',
							checkbox : true,
							rownumbers : true,
	...
	function openViewApply(obj) {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assspecial/assback/assViewBackPage.do?isCheck=false&'
			+ parm,
			modal: true,
			showToggle: false,
			showMax: true,
	...
	function backConfirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认退货?', function(yes) {
				if (yes) {
	...
	function importApplys() {
		parent.$.ligerDialog
			.open({
				...
				height: $(window).height(),
				width: $(window).width(),
				url: 'hrp/ass/assspecial/assback/assBackMainSpecialPage.do?isCheck=false&',
				modal: true,
				showToggle: false,
				showMax: true,
				showMin: false,
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assspecial/assback/assBackSpecialAddPage.do?isCheck=false',
					height : $(window).height(),
					width : $(window).width(),
					modal : true,
					showToggle : false,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssBackMainSpecial.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function printSet(){
	  ...
	  function printDate(){
			...
			if (data.length == 0){
				...
				ajaxJsonObjectByUrl("queryAssBackSpecialState.do?isCheck=false",{paraId:ass_back_no.substring(0,ass_back_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
