...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					usePager : false,
					url : 'queryAssChkRecordSpecial.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmChkRecordSpecial.do", {
	...
	function openView(obj){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assspecial/check/chkrecord/assViewSpecialPage.do?isCheck=false&'+parm,
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assspecial/check/chkrecord/assChkRecordSpecialAddPage.do?isCheck=false',
			title : '盘亏记录增加',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				...
				+ "is_import=" + vo[4];
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assspecial/check/chkrecord/assChkRecordSpecialUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '盘亏记录修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssChkRecordSpecial.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
