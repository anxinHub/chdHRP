	...
	grid = $("#maingrid").ligerGrid(
			{
				...
				dataType : 'server',
				usePager : false,
				url : 'queryAssCheckDdetailSpecial.do?isCheck=false',
				width : '100%',
				height : '100%',
				checkbox : true,
				rownumbers : true,
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : false,
					url : 'queryAssCheckSdetailSpecial.do?isCheck=false',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	if(is_dept=="01"){
		...
		if (data.length == 0 ) {
			...
			var formPara = {
					...
					ajaxJsonObjectByUrl("saveAssCheckSDetailSpecial.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
		...
		if (data.length == 0 ) {
			...
			ajaxJsonObjectByUrl("saveAssCheckDDetailSpecial.do?isCheck=false", formPara, function(
					responseData) {
				if (responseData.state == "true") {
					query();
				}
...
function tree_open() {
	...
	if(is_dept=="01" || v_text=="仓库"){
		$.ligerDialog.open({
			url : 'assCheckSspecialAddPage.do?isCheck=false&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckSspecial();
				},
		...
		$.ligerDialog.open({
			url : 'assCheckDspecialAddPage.do?isCheck=false&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				text : '确定',
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckDspecial();
				},
				cls : 'l-dialog-btn-highlight'
			}, {
				text : '取消',
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckSspecial.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckDspecial.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
function generate(){
	...
	if (yes) {
		ajaxJsonObjectByUrl("specialGenerateDetail.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
if(isnull($("#check_no").val())){
	...
	if (yes) {
		ajaxJsonObjectByUrl("auditSpecial.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
...
if(isnull($("#check_no").val())){
	...
	if (yes) {
		ajaxJsonObjectByUrl("unAuditSpecial.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
...
if(isnull($("#check_no").val())){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("copyAmountAssCheckSspecial.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
		...
		"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定复制账面数量?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("copyAmountAssCheckDspecial.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function remove(){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定删除?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("delAmountAssCheckSspecial.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
		...
		"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定删除?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("delAmountAssCheckDspecial.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function checkRgister (){
	...
	if(is_dept=="01"){
		$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'assCheckSpSpecialPage.do?isCheck=false&'+parm,
		...
		$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'assCheckDpSpecialPage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
...
function printSet(){
	...
	officeFormTemplate({template_code:"05020",use_id : useId})
}
	...
	/* if (data.length == 0){
		...
		$(data).each(function (){	
		 ...
		 var para={
				...
				template_code:'05020',
				class_name:"com.chd.hrp.ass.serviceImpl.check.special.AssCheckSdetailSpecialServiceImpl", 
