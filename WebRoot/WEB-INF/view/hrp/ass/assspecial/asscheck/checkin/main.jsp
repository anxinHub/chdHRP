...
<script type="text/javascript">
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmChkInSpecial.do", {
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assspecial/check/checkin/assChkInSpecialAddPage.do?isCheck=false',
			title : '申报信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assspecial/check/checkin/assChkInSpecialUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '申报信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssChkInSpecial.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0502001",use_id : useId});
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.check.special.AssChkInMainSpecialServiceImpl",
	    			method_name:"assChkInMainSpecialByPrintTemlate",
	    			template_code:'0502001',
	...
	function loadDict() {
		...
		autocomplete("#store_id", "../../../queryHosStoreDict.do?naturs_code=02&isCheck=false","id", "text",true,true,param,true,null,"160");
