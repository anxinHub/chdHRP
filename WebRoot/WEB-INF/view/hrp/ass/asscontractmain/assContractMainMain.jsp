...
//     	  grid.options.parms.push({name:'contract_detail',value:$("#contract_detail").val()}); 
//     	  grid.options.parms.push({name:'service_detail',value:$("#service_detail").val()}); 
...
//     	  grid.options.parms.push({name:'contract_money',value:$("#contract_money").val()}); 
...
//     	  grid.options.parms.push({name:'is_group',value:$("#is_group").val()}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssContractMain.do',
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		width: $(window).width(),
    		url: 'hrp/ass/asscontractmain/assContractMainAddPage.do?isCheck=false&',
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("../asscontractmain/deleteAssContractMain.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'assContractMainImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/asscontractmain/assContractMainUpdatePage.do?isCheck=false&'+parm,
    		modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
    		parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
    	});
    ...
    function openManage(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/asscontractmain/assContractMainManagePage.do?isCheck=false&'+parm,
    		modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
    		parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
    	});
    }
...
//     	autocomplete("#check_emp","../../../hrp/sys/queryUserDict.do?isCheck=false", "id","text", true, true);
    ...
    //键盘事件
	  ...
	  function toExamine(){
	   ...
	   function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryAssContractMain.do?isCheck=false", selPara, function (responseData) {
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryAssContractMain.do",exportPara,function (responseData){
...
<!-- 	<div class="l-layout-top" style="left: 0px; width: 100%; height: 300px;"> -->
    <table cellpadding="0" cellspacing="0" class="l-table-edit" border="0">
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="ass_month1" type="text" id="ass_month1"   class="Wdate" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'ass_month2\')}',isShowClear:true,readOnly:false,dateFmt:'MM'})"/></td>
