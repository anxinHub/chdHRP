...
//     	  grid.options.parms.push({name:'is_group',value:$("#is_group").val()}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssInitContractMain.do',
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		width: $(window).width(),
    		url: 'hrp/ass/assinitcontractmain/assInitContractMainAddPage.do?isCheck=false&',
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("../assinitcontractmain/deleteAssInitContractMain.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'assInitContractMainImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assinitcontractmain/assInitContractMainUpdatePage.do?isCheck=false&'+ parm,
    		modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
    		parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
    	});
    ...
    function openManage(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assinitcontractmain/assInitContractMainManagePage.do?isCheck=false&'+ parm,
    		modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
    		parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
    	});
    }
...
//     	autocomplete("#check_emp","../../../hrp/sys/queryUserDict.do?isCheck=false", "id","text", true, true);
    ...
    //键盘事件
	  ...
	  function toExamine(){
	   ...
	   function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryAssInitContractMain.do?isCheck=false", selPara, function (responseData) {
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryAssInitContractMain.do",exportPara,function (responseData){
