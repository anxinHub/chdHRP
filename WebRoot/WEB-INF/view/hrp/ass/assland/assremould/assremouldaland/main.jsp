...
<script type="text/javascript">
	...
	function Confirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认申报?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateAssRemouldAlandConfirmState.do?isCheck=false", {
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assremould/assremouldaland/assRemouldAlandAddPage.do?isCheck=false',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssRemouldAland.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assremould/assremouldaland/assRemouldAlandUpdatePage.do?isCheck=false&'
							+ param,
					height : $(window).height(),
					width : $(window).width(),
					modal : true,
	...
	function loadDict() {
		...
		autocomplete("#store_id", "../../queryHosStoreDict.do?naturs_code=06&isCheck=false","id", "text",true,true,param,true,null,"300");
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssRemouldAland.do", selPara,
				function(responseData) {
