...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					columns : [  {
						...
						render : function(rowdata, rowindex,
							...
							+ "|" + rowdata.copy_code + "|"
							+ rowdata.fcord_no  +"')>"+rowdata.fcord_no+"</a>";
						}, frozen: true
					}, {
						display : '摘要',
						name : 'note',
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@" + this.hos_id + "@"
								+ this.copy_code + "@" + this.fcord_no);
					});
			$.ligerDialog.confirm('确认竣工?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmAssRemouldFland.do?isCheck=false", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assremould/assremouldfland/assRemouldFlandAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssRemouldFland.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ " copy_code=" + vo[2] + "&" + "fcord_no=" + vo[3];

		parent.$.ligerDialog.open({
			title: '资产改造竣工修改',
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assremould/assremouldfland/assRemouldFlandUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
		...
		hotkeys('P', printDate);
         ...
         function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryAssRemouldFland.do?isCheck=false", selPara, function (responseData) {
...
<body style="padding: 0px; overflow: hidden;">
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit" border="0"
		...
		<tr>
			...
			<td align="left"><input name="create_date_begin" type="text"
				id="create_date_begin" class="Wdate"
		...
		<tr>
			...
			<td align="left"><input name="change_date_beg" type="text"
				id="change_date_beg" class="Wdate"
				onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" /></td>
			<td align="left">至：</td>
			<td align="left"><input name="change_date_end" type="text"
				id="change_date_end" class="Wdate"
	...
	<div id="topmenu"
		style="background: #FFFFFF; color: #FFFFFF; border: 1px solid #A4D3EE"></div>
	<div id="maingrid"></div>
</body>
</html>
