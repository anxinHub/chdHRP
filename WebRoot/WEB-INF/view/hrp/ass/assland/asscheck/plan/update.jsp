	...
	grid = $("#maingrid").ligerGrid(
			{
				...
				dataType : 'server',
				usePager : false,
				url : 'queryAssCheckDdetailLand.do?isCheck=false',
				width : '100%',
				height : '100%',
				checkbox : true,
				rownumbers : true,
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : false,
					url : 'queryAssCheckDetailLand.do?isCheck=false',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
...
function inventory() {
	...
	$(data).each(function(){
		...
		data1.push(this.ass_id+"@"+this.hos_id+"@"+this.copy_code+"@"+this.group_id+"@"+this.store_id+"@"+this.ass_card_no+"@"+this.ass_type_name+"@"+this.ass_name+"@"+this.store_name+"@"+this.acc_amount+"@"+this.check_amount+"@"+this.pl_amount+"@"+this.pl_reason+"@"+this.store_no+"@"+this.ass_no+"@"+this.check_no+"@"+this.check_plan_no);
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("saveLandInventory.do?isCheck=false"  ,
	...
	if(is_dept=="01"){
		...
		if (data.length == 0 ) {
			...
			var formPara = {
					...
					ajaxJsonObjectByUrl("saveAssCheckSDetailLand.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
...
function tree_open() {
	...
	if(is_dept=="01" || v_text=="仓库"){
		$.ligerDialog.open({
			url : 'assCheckSlandAddPage.do?isCheck=false&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckSland();
				},
		...
		$.ligerDialog.open({
			url : 'assCheckDlandAddPage.do?isCheck=falsefalse&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				text : '确定',
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckDland();
				},
				cls : 'l-dialog-btn-highlight'
			}, {
				text : '取消',
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckSland.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckDland.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
if (yes) {
	ajaxJsonObjectByUrl("landGenerateDetail.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
			query();
...
if (yes) {
	ajaxJsonObjectByUrl("auditLand.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
...
if (yes) {
	ajaxJsonObjectByUrl("unAuditLand.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
...
function copyAmount(){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("copyAmountAssCheckLand.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function remove(){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定删除?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("delAmountAssCheckLand.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function checkRgister(){
	...
	$.ligerDialog.open({
		...
		width: $(window).width(),
		url: 'assCheckSpLandPage.do?isCheck=false&'+parm,
...
function printSet(){
	...
	officeFormTemplate({template_code:"05070",use_id : useId})
}
	...
	/* if (data.length == 0){
		...
		$(data).each(function (){	
		 ...
		 var para={
				...
				template_code:'05070',
				class_name:"com.chd.hrp.ass.serviceImpl.check.land.AssCheckSdetailLandServiceImpl", 
			 ...
			 var useId=0;//统一打印
				if('${ass_05070}'==1){
				 ...
				 var para={
						...
						template_code:'05070',
						class_name:"com.chd.hrp.ass.serviceImpl.check.land.AssCheckDdetailLandServiceImpl", 
