...
<script type="text/javascript">
	...
	function confirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('变动确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmChangeLand.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assland/asschange/assChangeLandAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssChangeLand.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assland/asschange/assChangeLandUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
	...
	function printDetail(){
		...
		if (data.length == 0){
			...
			var para={
				...
				change_no: change_no.substring(0,change_no.length-1) ,
				template_code:'05071',
				isPrintCount:false,//更新打印次数
				isPreview:true,//预览窗口，传绝对路径
				use_id:useId,
			}; 
			ajaxJsonObjectByUrl("queryAssChangeLandStates.do?isCheck=false",{change_no: change_no.substring(0,change_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
