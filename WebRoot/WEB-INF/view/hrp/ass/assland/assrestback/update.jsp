...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#bus_type_code").change(function(){
			editor.grid.url='../../queryAssCardTable.do?isCheck=false&ass_nature=06&use_state=1,2,3,4,5&bus_type_code='
				+ liger
				.get(
						"bus_type_code")
				.getValue()+'&in_date='+$("#create_date").val()+'&is_dept=0';
		});
	...
	function cardSelect(){
		...
		editor.grid.url='../../queryAssCardTable.do?isCheck=false&ass_nature=05&use_state=1,2,3,4,5&bus_type_code='
			...
			.get(
					"bus_type_code")
			.getValue()+'&in_date='+$("#create_date").val()+'&ass_card_not_exists='+ass_card_nos.toString()+'&is_dept=0';
	...
	function save() {
		...
		if (validateGrid()) {
			ajaxJsonObjectByUrl("saveAssBackRestMainLand.do", formPara, function(
					responseData) {
	...
	function loadHead() {
		editor = {
				...
				grid : {
					...
					url :'../../queryAssCardTable.do?isCheck=false&ass_nature=06&use_state=1,2,3,4,5&bus_type_code='
						+ liger
						.get(
								"bus_type_code")
						.getValue()+'&in_date='+$("#create_date").val()+'&is_dept=0',
				...
				.ligerGrid(
						{
							...
							dataAction : 'server',
							dataType : 'server',
							url:'queryAssBackRestDetailLand.do',
							usePager : false,
							width : '100%',
							height : '100%',
							checkbox : true,
			...
			$.ligerDialog
					.confirm(
							...
							function(yes) {
								if (yes) {
									ajaxJsonObjectByUrl(
											"deleteAssBackRestDetailLand.do",
											{
	...
	function printSetDetail(){
		...
		officeFormTemplate({template_code:"05067",use_id:useId});
	}
	...
	function printDetail(){
		...
		//if($("#create_date_b").val())
			...
			var para={
				class_name:"com.chd.hrp.ass.serviceImpl.back.rest.AssBackRestLandServiceImpl",
				method_name:"printAssBackRestLandData",
			...
			ajaxJsonObjectByUrl("queryAssBackRestLandStates.do?isCheck=false",{ass_back_no:ass_back_no,state:2},function(responseData){
				if (responseData.state == "true") {
