...
<script type="text/javascript">
	...
	function add_open() {
		...
		$.ligerDialog.open({
			url : 'assCheckPlanAddPage.do?isCheck=false&',
			data : {},
			height : 300,
			width : 450,
			title : '盘点任务单',
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckPlanGeneral();
				},
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@" + this.hos_id + "@"
								+ this.copy_code + "@" + this.check_plan_no+"@"+this.check_plan_no_s+"@"+this.check_plan_no_g+"@"+this.check_plan_no_o);
					});

			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssCheckPlan.do", {ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("checkAudit.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unAudit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("checkUnAudit.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function finish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定盘点完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("checkFinish.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unFinish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("checkUnFinish.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function generate() {
		...
		if (data.length == 0 || data.length > 1) {
			...
			$(data).each(function() {
				...
				check_plan_no_s=this.check_plan_no_s;
				check_plan_no_g=this.check_plan_no_g;
				check_plan_no_o=this.check_plan_no_o;
			...
			$.ligerDialog.confirm('生成的盘点单将会覆盖原有数据?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("checkGenerate.do", formPara, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function queryEcharts(){
		...
		if (data.length == 0 || data.length > 1) {
			...
			parent.$.ligerDialog.open({
				...
				width: $(window).width(),
				url: 'hrp/ass/asscheck/plan/assCheckPlanEchartsMainPage.do?isCheck=false&check_plan_no='+ check_plan_no,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/asscheck/plan/assCheckPlanUpdatePage.do?isCheck=false&'+ parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
 ...
 */
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssCheckPlan.do?isCheck=false",
