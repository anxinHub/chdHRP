...
<script type="text/javascript">
	...
	function confirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('变动确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmDepreInassets.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assinassets/assdepre/assDepreInassetsAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssDepreInassets.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assinassets/assdepre/assDepreInassetsUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
		...
		hotkeys('P', printDate);
         ...
         function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryAssDepreInassets.do?isCheck=false", selPara, function (responseData) {
...
<body style="padding: 0px; overflow: hidden;">
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit" border="0" id="table1" width="100%">
		<tr>
			...
			<td align="left" width="5%"><input name="audit_date_beg"
