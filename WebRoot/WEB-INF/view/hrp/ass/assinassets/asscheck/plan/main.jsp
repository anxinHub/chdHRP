...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssCheckPlanInassets.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function add_open() {
		...
		$.ligerDialog.open({
			url : 'assCheckPlanInassetsAddPage.do?isCheck=false&',
			data : {},
			height : 300,
			width : 450,
			title : '盘点任务单(专用设备)',
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckPlanInassets();
				},
	...
	function remove() {
		...
		/* 	var check_plan_no_all = [];
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssCheckPlanInassets.do", {ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("inassetsAudit.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unAudit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("inassetsUnAudit.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function finish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定盘点完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("inassetsFinish.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unFinish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("inassetsUnFinish.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function generate() {
		...
		if (data.length == 0 || data.length > 1) {
			...
			$.ligerDialog.confirm('生成的盘点单将会覆盖原有数据?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("inassetsGenerate.do", formPara, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assinassets/asscheck/plan/assCheckPlanInassetsUpdatePage.do?isCheck=false&'+ parm,
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssCheckPlanInassets.do?isCheck=false",
