	...
	grid = $("#maingrid").ligerGrid(
			{
				...
				dataType : 'server',
				usePager : false,
				url : 'queryAssCheckDdetailInassets.do?isCheck=false',
				width : '100%',
				height : '100%',
				checkbox : true,
				rownumbers : true,
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : false,
					url : 'queryAssCheckSdetailInassets.do?isCheck=false',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	if(is_dept=="01"){
		...
		if (data.length == 0 ) {
			...
			var formPara = {
					...
					ajaxJsonObjectByUrl("saveAssCheckSDetailInassets.do?isCheck=false", formPara, function(
							responseData) {
		...
		if (data.length == 0 ) {
			...
			ajaxJsonObjectByUrl("saveAssCheckDDetailInassets.do?isCheck=false", formPara, function(
					responseData) {
				if (responseData.state == "true") {
					query();
				}
...
function inventory() {
	...
	$(data).each(function(){
		...
		data1.push(this.ass_id+"@"+this.hos_id+"@"+this.copy_code+"@"+this.group_id+"@"+this.store_id+"@"+this.ass_card_no+"@"+this.ass_type_name+"@"+this.ass_name+"@"+this.store_name+"@"+this.acc_amount+"@"+this.check_amount+"@"+this.pl_amount+"@"+this.pl_reason+"@"+this.store_no+"@"+this.ass_no+"@"+this.check_no+"@"+this.check_plan_no);
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("saveInassetsInventory.do?isCheck=false"  ,
			 ...
			 $(data).each(function(){
				...
				data2.push(this.ass_id+"@"+this.hos_id+"@"+this.copy_code+"@"+this.group_id+"@"+this.dept_id+"@"+this.ass_card_no+"@"+this.ass_type_name+"@"+this.ass_name+"@"+this.dept_name+"@"+this.acc_amount+"@"+this.check_amount+"@"+this.pl_amount+"@"+this.pl_reason+"@"+this.dept_no+"@"+this.ass_no+"@"+this.check_no+"@"+this.check_plan_no);
					 ...
					 $.ligerDialog.confirm('确定生成么?', function(yes) {
					 if (yes) {
					 	ajaxJsonObjectByUrl("saveDeptInassetsInventory.do?isCheck=false"  ,
...
function tree_open() {
	...
	if(is_dept=="01" || v_text=="仓库"){
		$.ligerDialog.open({
			url : 'assCheckSinassetsAddPage.do?isCheck=false&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckSinassets();
				},
		...
		$.ligerDialog.open({
			url : 'assCheckDinassetsAddPage.do?isCheck=falsefalse&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				text : '确定',
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckDinassets();
				},
				cls : 'l-dialog-btn-highlight'
			}, {
				text : '取消',
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckSinassets.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckDinassets.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
if (yes) {
	ajaxJsonObjectByUrl("inassetsGenerateDetail.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
			query();
...
if (yes) {
	ajaxJsonObjectByUrl("auditInassets.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
...
if (yes) {
	ajaxJsonObjectByUrl("unAuditInassets.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
...
if(isnull($("#check_no").val())){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("copyAmountAssCheckSinassets.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
		...
		"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定复制账面数量?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("copyAmountAssCheckDinassets.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function remove(){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定删除?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("delAmountAssCheckSinassets.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
		...
		"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定删除?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("delAmountAssCheckDinassets.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function checkRgister(){
	...
	if(is_dept=="01"){
		$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'assCheckSpInassetsPage.do?isCheck=false&'+parm,
		...
		$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'assCheckDpInassetsPage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
...
function printSet(){
	...
	officeFormTemplate({template_code:"05050",use_id : useId})
}
	...
	/* if (data.length == 0){
		...
		$(data).each(function (){	
		 ...
		 var para={
				...
				template_code:'05050',
				class_name:"com.chd.hrp.ass.serviceImpl.check.inassets.AssCheckSdetailInassetsServiceImpl", 
			 ...
			 var useId=0;//统一打印
				if('${ass_05050}'==1){
				 ...
				 var para={
						...
						template_code:'05050',
						class_name:"com.chd.hrp.ass.serviceImpl.check.special.AssCheckDdetailSpecialServiceImpl", 
