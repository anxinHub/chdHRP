...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					usePager : false,
					url : 'queryAssCheckApInassets.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			$(data).each(
					function() {
							ParamVo.push(this.group_id + "@" + this.hos_id + "@"
									+ this.copy_code + "@" + this.check_ap_no);
						
					});
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmCheckApInassets.do", {
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assinassets/check/checkapply/assCheckApInassetsAddPage.do?isCheck=false',
			title : '申报信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ "copy_code=" + vo[2] + "&" + "check_ap_no=" + vo[3];
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assinassets/check/checkapply/assCheckApInassetsUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '申报信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssCheckApInassets.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
