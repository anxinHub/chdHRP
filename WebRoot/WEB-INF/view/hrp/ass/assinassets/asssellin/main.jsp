...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					columns : [  {
						...
						render : function(rowdata, rowindex,
							...
							+ "|" + rowdata.copy_code + "|"
							+ rowdata.sell_in_no  +"|"+ rowdata.is_import
							+"')>"+rowdata.sell_in_no+"</a>";
						}, frozen: true
					}, {
						display : '摘要',
						name : 'note',
						...
						render : function(rowdata, rowindex,
							...
							if (rowdata.is_import  == 1) {
								return "<a href=javascript:openViewSellOut('"
								+ rowdata.group_id
								+ "|"
								+ rowdata.hos_id
								+ "|"
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssSellInInassets.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssSellInInassets.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function openViewSellOut(obj){
		var vo = obj.split("|");
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ " copy_code=" + vo[2] + "&" + "sell_in_no=" + vo[3] + "&"
				+ "out_group_id=" + vo[4] + "&" + "out_hos_id="+vo[5] + "&"
				+ "out_store_id=" + vo[6] + "&" + "out_store_no="+vo[7]+ "&"
				+ "bus_type_code="+vo[7];
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assinassets/asssellin/assViewSellOutInassetsPage.do?isCheck=false&'+parm,
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@" + this.hos_id + "@"
								+ this.copy_code + "@" + this.sell_in_no);
					});
			$.ligerDialog.confirm('确认入库?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmSellInMainInassets.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assinassets/asssellin/assSellInInassetsAddPage.do?isCheck=false&',
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssSellInInassets.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assinassets/asssellin/assSellInInassetsUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
	function loadDict() {
		...
		autocomplete("#in_store_id", "../../queryHosStoreDict.do?naturs_code=05&isCheck=false","id", "text",true,true,null,false,null,"300");
    	
		autocomplete("#bus_type_code", "../../queryAssBusType.do?isCheck=false","id", "text",true,true,{is_menu:5},false,null,"160");
		...
		$("#out_group_id").change(function(){
			...
			$("#out_hos_id").change(function(){
				autocomplete("#out_store_id", "../../queryHosStoreDict.do?naturs_code=05&isCheck=false","id", "text",true,true,{group_id:liger.get("out_group_id").getValue(),hos_id:liger.get("out_hos_id").getValue()},false,null,"160");
			});
		});
		$('#state').ligerComboBox({
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0504901",use_id : useId})
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){	
			 ...
			 var para={
					...
					template_code:'0504901',
					class_name:"com.chd.hrp.ass.serviceImpl.sell.in.AssSellInInassetsServiceImpl", 
					method_name:"querySellInInassetsByPrintTemlatePrint",
					isSetPrint:false,//是否套打，默认非套打
					isPreview:true,//是否预览，默认直接打印
	    			paraId :sell_in_no.substring(0,sell_in_no.length-1) ,
	    			isPrintCount:false,//更新打印次数
	    			use_id:useId,
	    			p_num:1
	    			//isSetPrint:flag
	    	}; 
			ajaxJsonObjectByUrl("queryAssSellInInassetsState.do?isCheck=false",{paraId:sell_in_no.substring(0,sell_in_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
