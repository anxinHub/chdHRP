...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					usePager : false,
					url : 'queryCheckApSpecialByImport.do?isCheck=false&state=2&dis_r_no=${dis_r_no}',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assinassets/assdisposal/disposalrecord/assCheckViewUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '查看申报信息',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function save() {
		...
		if (data.length == 0) {
			...
			$.post("addAssPlanDeptImport.do?isCheck=false", {
				...
				dispose_type:dispose_type.toString(),
			...
			/* 		if(	parentFrameUse().grid.rows.ass_card_no != 'undefined' && parentFrameUse().grid.rows.ass_card_no !=""){
					...
					parentFrameUse().liger.get("dispose_type").setValue(dispose_type);
					parentFrameUse().grid.addRows(responseData.Rows);
				    this_close();
