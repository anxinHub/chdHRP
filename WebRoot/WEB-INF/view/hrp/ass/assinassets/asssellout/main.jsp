...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssSellOutInassets.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssSellOutInassets.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function initCard(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认出库?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmSellOutInassets.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assinassets/asssellout/assSellOutInassetsAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssSellOutInassets.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assinassets/asssellout/assSellOutInassetsUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
  ...
  //打印 最新版
   function printDate(){
		...
		if (data.length == 0){
			...
			ajaxJsonObjectByUrl("queryAssSellOutInassetsState.do?isCheck=false",{paraId:sell_out_no.substring(0,sell_out_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
