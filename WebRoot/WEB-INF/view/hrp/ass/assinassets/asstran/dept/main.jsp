...
<script type="text/javascript">
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranDeptInassets.do?isCheck=false&show_detail=1',
    		...
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranDeptInassets.do?isCheck=false&show_detail=0',
    	                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad :true,
    	                     selectRowButtonOnly:true,//heightDiff: -10,
    	                     toolbar: { items: [
    	                     	{ text: '查询（<u>E</u>）', id:'search', click: query,icon:'search' },
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		width: $(window).width(),
    		url: 'hrp/ass/assinassets/asstran/dept/assTranDeptInassetsAddPage.do?isCheck=false&',
	...
	function updateConfirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定移库?', function(yes) {
				...
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmTranDeptInassets.do", {
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssTranDeptInassets.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assinassets/asstran/dept/assTranDeptInassetsUpdatePage.do?isCheck=false&'+ parm,
