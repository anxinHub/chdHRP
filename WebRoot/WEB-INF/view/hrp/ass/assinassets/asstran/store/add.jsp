...
<script type="text/javascript">
	...
	function cardSelect(){
		...
		var str = "select * from ASS_TRAN_STORE_DETAIL_INASSETS p "+
		" left join ASS_TRAN_STORE_INASSETS b on"+
	...
	function save() {
		...
		if (validateGrid()) {
			ajaxJsonObjectByUrl("saveAssTranStoreInassets.do", formPara, function(responseData) {
				...
				.ligerGrid(
						{
							...
							usePager : false,
							width : '100%',
							url : 'queryAssTranStoreDetailInassets.do',
							height : '100%',
							checkbox : true,
							enabledEdit : true,
							alternatingRow : true,
			...
			$.ligerDialog
					.confirm(
							...
							function(yes) {
								if (yes) {
									ajaxJsonObjectByUrl(
											"deleteAssTranStoreDetailInassets.do",
											{
												ParamVo : ParamVo.toString()
											},
											function(responseData) {
	...
	function loadDict() {
		...
		$("#in_store_id").ligerComboBox({
			url : '../../../queryHosStoreDict.do?naturs_code=05&isCheck=false',
			valueField : 'id',
			textField : 'text',
			selectBoxWidth : 160,
			autocomplete : true,
