...
<script type="text/javascript">
    ...
    function  query(){
        ...
        //根据表字段进行添加查询条件
    	  ...
    	  grid.options.parms.push({name:'in_store_no',value:liger.get("in_store_id").getValue().split("@")[1]}); 
    	  grid.options.parms.push({name:'out_store_id',value:liger.get("out_store_id").getValue().split("@")[0]}); 
    	  grid.options.parms.push({name:'out_store_no',value:liger.get("out_store_id").getValue().split("@")[1]}); 
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '移入仓库', name: 'in_store_name', align: 'left',width : 100,
					 		},
                     { display: '移出仓库', name: 'out_store_name', align: 'left',width : 100,
					 		},
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranStoreInassets.do?isCheck=false&show_detail=1',
    		...
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                     ...
    	                     { display: '移入仓库', name: 'in_store_name', align: 'left'
    						 		},
    	                     { display: '移出仓库', name: 'out_store_name', align: 'left'
    						 		},
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranStoreInassets.do?isCheck=false&show_detail=0',
    	                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad :true,
    	                     selectRowButtonOnly:true,//heightDiff: -10,
    	                     toolbar: { items: [
    	                     	{ text: '查询（<u>E</u>）', id:'search', click: query,icon:'search' },
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		width: $(window).width(),
    		url: 'hrp/ass/assinassets/asstran/store/assTranStoreInassetsAddPage.do?isCheck=false&',
	...
	function updateConfirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定移库?', function(yes) {
				...
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmTranStoreInassets.do", {
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssTranStoreInassets.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assinassets/asstran/store/assTranStoreInassetsUpdatePage.do?isCheck=false&'+ parm,
    ...
    function loadDict(){
    	...
    	autocomplete("#out_store_id", "../../../queryHosStoreDict.do?naturs_code=05&isCheck=false","id", "text",true,true,param,true);
    	
    	autocomplete("#in_store_id", "../../../queryHosStoreDict.do?naturs_code=05&isCheck=false","id", "text",true,true,param,true);
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0509101",use_id : useId});
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.tran.store.AssTranStoreInassetsServiceImpl",
	    			method_name:"assTranStoreInassetsByPrintTemlate",
	    			template_code:'0509101',
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
        <tr>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">移出仓库：</td>
            <td align="left" class="l-table-edit-td"><input name="out_store_id" type="text" id="out_store_id"  /></td>
            <td align="left"></td>
        </tr> 
        <tr>
            ...
            <td align="left"></td>
            <td align="right" class="l-table-edit-td"  style="padding-left:20px;">移入仓库：</td>
            <td align="left" class="l-table-edit-td"><input name="in_store_id" type="text" id="in_store_id"  /></td>
            <td align="left"></td>
        </tr> 
        <tr>
