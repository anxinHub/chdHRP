...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssOutInassets.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssOutInassets.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					...
					width : $(window).width(),
					url : 'hrp/ass/assinassets/asstran/out/assOutInassetsAddPage.do?isCheck=false&',
					modal : true,
					showToggle : false,
					showMax : true,
					showMin : true,
	...
	function updateConfirm() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定移库?', function(yes) {
				...
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmOutInassets.do", {
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssOutInassets.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog
				.open({
					...
					height : $(window).height(),
					width : $(window).width(),
					url : 'hrp/ass/assinassets/asstran/out/assOutInassetsUpdatePage.do?isCheck=false&'
							+ parm,
					modal : true,
					showToggle : false,
					showMax : true,
	...
	function loadDict() {
		...
		autocomplete("#store_id",
				"../../../queryHosStoreDict.do?naturs_code=05&isCheck=false", "id", "text",
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0509103",use_id : useId});
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.tran.out.AssOutInassetsServiceImpl",
	    			method_name:"assOutInassetsByPrintTemlate",
	    			template_code:'0509103',
    ...
    function openViewAssIn(obj){
  	  var vo = obj.split("|");
  		...
  		parent.$.ligerDialog.open({
  			...
  			height: $(window).height(),
  			width: $(window).width(),
  			url: 'hrp/ass/assinassets/asstran/out/assViewInMainPage.do?isCheck=false&'+parm,
