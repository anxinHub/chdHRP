...
//     	  grid.options.parms.push({name:'ven_no',value:$("#ven_no").val()}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '招标日期', name: 'bid_date', align: 'left',frozen: true,width:100
					 		},
                     { display: '供应商', name: 'ven_name', align: 'left',width:200
					 		},
                     { display: '联系人', name: 'link_man', align: 'left',width:100
					 		},
                     { display: '手机号码', name: 'tel_mobile', align: 'left',width:100
					 		},
                     { display: '办公电话', name: 'tel_office', align: 'left',width:100
					 		},
                     ...
                     { display: '备注', name: 'bid_note', align: 'left',width:100
					 		}
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssBidMain.do',
                     ...
                     toolbar: { items: [
                     	...
                     	{ line:true },
										...
										{ line:true }, 
										{ text: '引入购置计划单（<u>I</u>）', id:'importPlan', click:importPlans,icon:'refresh' },
						                { line:true },
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		width: $(window).width(),
    		url: 'hrp/ass/assbidmain/assBidMainAddPage.do?isCheck=false&',
    ...
    function importPlans(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assbidmain/assBidMainImportPlanPage.do?isCheck=false&',
    		modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
    		parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
    	});
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssBidMain.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assbidmain/assBidMainUpdatePage.do?isCheck=false&'+ parm,
    		modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
    		parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
    	});
	  ...
	  function loadHotkeys() {
	   ...
	   function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryAssBidMain.do?isCheck=false", selPara, function (responseData) {
...
<body style="padding: 0px; overflow: hidden;">
    ...
    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="bid_no" type="text" id="bid_no"  /></td>
            <td align="left"></td>
        ...
        <tr>
            ...
            <td align="left" class="l-table-edit-td"><input name="audit_date1" type="text" id="audit_date1"  class="Wdate" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'audit_date2\')}',isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/></td>
            <td align="left" class="l-table-edit-td">至：</td>
            <td align="left" class="l-table-edit-td"><input name="audit_date2" type="text" id="audit_date2"  class="Wdate" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'audit_date1\')}',isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})"/></td>
            <td align="left"></td>
