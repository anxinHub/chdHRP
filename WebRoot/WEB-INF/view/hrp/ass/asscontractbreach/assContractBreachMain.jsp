...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '违约单号', name: 'breach_no', align: 'left',
						render : function(rowdata, rowindex,
								value) {
							return "<a href=javascript:openUpdate('"+rowdata.group_id   + "|" + 
							rowdata.hos_id   + "|" + 
							rowdata.copy_code   + "|" + 
							rowdata.breach_no  +"')>"+rowdata.breach_no+"</a>";
						}
					 		},
                     ...
                     { display: '违约金额', name: 'breach_money', align: 'right',
                     ...
                     { display: '违约方', name: 'breach_flag', align: 'left',
								render : function(rowdata, rowindex,
										value) {
									if(rowdata.state == 1){
										return "乙方";
                     ...
                     { display: '违约原因', name: 'breach_reason', align: 'left'
					 		},
                     { display: '处理结果', name: 'result', align: 'left'
					 		}
                     ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssContractBreach.do',
    ...
    function add_open(){
		...
		.open({
			...
			width : $(window).width(),
			url : 'hrp/ass/asscontractbreach/assContractBreachAddPage.do?isCheck=false&',
			modal : true,
			showToggle : false,
			showMax : false,
			showMin : true,
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssContractBreach.do",{ParamVo : ParamVo.toString()},function (responseData){
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'assContractBreachImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    function openUpdate(obj){
		...
		.open({
			...
			height : $(window).height(),
			width : $(window).width(),
			url : 'hrp/ass/asscontractbreach/assContractBreachUpdatePage.do?isCheck=false&'+ parm,
			modal : true,
			showToggle : false,
			showMax : false,
			showMin : true,
...
// 			lodopPrinterTable("resultPrint","开始打印","050502 资产合同违约",true);
// 			return;
// 		}
...
//          };
// 		ajaxJsonObjectByUrl("queryAssContractBreach.do",printPara,function (responseData){
// 			$.each(responseData.Rows,function(idx,item){ 
// 				 var trHtml="<tr>";
// 					 trHtml+="<td>"+item.breach_no+"</td>"; 
...
// 							trHtml+="<td>新建</td>"; 
// 						 } 
// 						 if(item.breach_flag == 1){
// 							trHtml+="<td>乙方</td>";
// 						 }else if(item.breach_flag == 0){
// 							trHtml+="<td>甲方</td>";
// 						 } 
...
// 					 trHtml+="<td>"+item.breach_reason+"</td>"; 
// 					 trHtml+="<td>"+item.result+"</td>"; 
// 				 trHtml+="</tr>";
...
// 	 }
	  function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryAssContractBreach.do?isCheck=false", selPara, function (responseData) {
	 ...
	 function exportExcel(){
		//有数据直接导出
		if($("#resultPrint > table > tbody").html()!=""){
			lodopExportExcel("resultPrint","导出Excel","050502 资产合同违约.xls",true);
			return;
		}
		...
		ajaxJsonObjectByUrl("queryAssContractBreach.do",exportPara,function (responseData){
			$.each(responseData.Rows,function(idx,item){ 
				 var trHtml="<tr>";
				 trHtml+="<td>"+item.breach_no+"</td>"; 
				 trHtml+="<td>"+item.contract_name+"</td>"; 
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div id="resultPrint" style="display:none">
	   	<table width="100%">
			...
			<tr>
                <th width="200">违约单号</th>	
                <th width="200">合同</th>	
                <th width="200">供应商</th>	
                <th width="200">登记日期</th>	
                <th width="200">违约金额</th>	
                <th width="200">状态</th>	
                <th width="200">违约方</th>	
