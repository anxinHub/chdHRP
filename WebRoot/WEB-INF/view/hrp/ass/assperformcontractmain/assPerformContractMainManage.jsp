...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssPerformContractManage.do?isCheck=false&contract_id=${contract_id}',
	...
	$.ligerDialog.open({ url : 'assPerformContractManageAddPage.do?isCheck=false&'+parm,data:{}, height: 500,width: 500, title:'添加付款分期',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveAssContractManage(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
 ...
 function openUpdate(obj){
		...
		var vo = obj.split("|");
			var parm ="&group_id="+ 
			   ...
			   vo[5]   +"&id="+ 
			   vo[6]   ;
    	$.ligerDialog.open({ url : 'assPerformContractManageUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 500, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,buttons: [ { text: '关闭', onclick: function (item, dialog) { dialog.close(); } } ] });
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("../assperformcontractmain/deleteAssPerformContractManage.do",{ParamVo : ParamVo.toString()},function (responseData){
