...
//     	  grid.options.parms.push({name:'is_group',value:$("#is_group").val()}); 
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssPerformContractMain.do',
                     ...
                     toolbar: { items: [
                     	...
                     	{ line:true },
										{text : '履行（<u>S</u>）',id : 'toExamine',click : toExamine,icon : 'ok'},
										{line : true},
										{text : '取消履行（<u>X</u>）',id : 'notToExamine',click :notToExamine ,icon : 'right'}, 
										{line : true},
		...
		var parm = "group_id="+vo[0] +"&"+ 
			...
			"state="+vo[4];
				parent.$.ligerDialog.open({
					...
					width: $(window).width(),
					url: 'hrp/ass/assperformcontractmain/assPerformContractMainUpdatePage.do?isCheck=false&'+parm,
    ...
    function openManage(obj){
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assperformcontractmain/assPerformContractMainManagePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});	
    ...
    //键盘事件
	  ...
	  function toExamine(){
			...
			if (data.length == 0) {
				...
				$.ligerDialog.confirm('确定履行?', function(yes) {
		...
		function notToExamine(){
			...
			if (data.length == 0) {
				...
				$.ligerDialog.confirm('确定取消履行?', function(yes) {
	  ...
	  function printDate(){
	    	...
	    	var printPara={
	   		...
	   		ajaxJsonObjectByUrl("queryAssPerformContractMain.do?isCheck=false", selPara, function (responseData) {
	 ...
	 function exportExcel(){
		...
		ajaxJsonObjectByUrl("queryAssPerformContractMain.do",exportPara,function (responseData){
