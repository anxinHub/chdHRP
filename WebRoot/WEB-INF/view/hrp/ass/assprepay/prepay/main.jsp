...
<script type="text/javascript">
	...
	function query() {
		...
		grid.options.parms.push({
			name : 'audit_date_beg',
			value : liger.get("audit_date_beg").getValue()
		});
		grid.options.parms.push({
			name : 'audit_date_end',
			value : liger.get("audit_date_end").getValue().split(".")[0]
		});
		grid.options.parms.push({
			name : 'state',
		...
		grid.options.parms.push({
			name : 'operator_emp',
			value : liger.get("operator_emp").getValue()
		});
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					onDblClickRow : function(rowdata, rowindex, value) {
						openUpdate(rowdata.group_id + "|" + rowdata.hos_id
								+ "|" + rowdata.copy_code + "|" + rowdata.pay_no);
					},
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('预付款确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmPrePay.do?isCheck=false", {
	...
	function unaudit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('取消确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateCancelConfirmPrePay.do?isCheck=false", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assprepay/prepay/assPrePayAddPage.do?isCheck=false',
			title : '信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assprepay/prepay/assPrePayUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssPrePay.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function loadDict() {
		...
		$("#create_date_beg,#create_date_end,#audit_date_beg,#audit_date_end").ligerTextBox({width : 90});
	...
	//打印
	    function printDate(){
		    	...
		    	var para={
		    			...
		    			template_code:'05085',
		    			class_name:"com.chd.hrp.ass.serviceImpl.prepay.AssPrePayMainServiceImpl",
		    			method_name:"queryAssPrePayMainDY",
		    	...
		    	ajaxJsonObjectByUrl("queryAssPrepayState.do?isCheck=false",{paraId:pay_no.substring(0,pay_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
						officeFormPrint(para);
					}
				});
...
<body style="padding: 0px; overflow: hidden;">
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit" width="100%">
		<tr>
            ...
            <td align="left" class="l-table-edit-td" width="2%">至:</td>
			...
			<td align="left" class="l-table-edit-td">
				<input name="ven_id"type="text" id="ven_id" />
			</td>
			<td align="left"></td>
		...
		<tr>
            ...
            <td align="left" class="l-table-edit-td" >
            	<input name="audit_date_beg" class="Wdate" type="text" id="audit_date_beg" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" />
            </td>	
             <td align="left" class="l-table-edit-td">至:</td>
             <td align="left" class="l-table-edit-td">
            	<input name="audit_date_end" class="Wdate" type="text" id="audit_date_end" onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd'})" />
			</td>
			<td align="left"></td>
