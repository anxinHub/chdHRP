...
<script type="text/javascript">
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assprepay/backprepay/assBackPrePayAddPage.do?isCheck=false',
			title : '信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	//打印
	    function printDate(){
		    	...
		    	ajaxJsonObjectByUrl("queryAssBackPrepayState.do?isCheck=false",{paraId:pay_no.substring(0,pay_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
						officeFormPrint(para);
					}
				});
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assprepay/backprepay/assBackPrePayUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssBackPrePay.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
