...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
 <title>Insert title here</title>
	...
	function update(data, index, value) {
        var parm = 'loc_code='+value;
        $.etDialog.open({
            url: 'assLocationUpdatePage.do?isCheck=false&'+parm,
            height: 400,
            width: 400,
            title: '修改位置',
            btn: ["确定", "取消"],
    ...
    function loadTree() {
        tree = $("#mainTree").etTree({
            async: {
                enable: true,
                url: 'queryAssLocationTree.do?isCheck=false'
            } ,
            callback: {
                onClick: function (event, treeId, treeNode) {
                    var loc_code = treeNode.loc_code;
                    search();
                }
            }, 
         /*    addSuffix: function () {
    ...
    function search() {
        ...
        //加载查询条件
        grid.loadData(parms, 'queryAssLocation.do?isCheck=false');
    }
    
    function addAssLocationTree() {
    	 $.etDialog.open({
             url: 'assLocationAddPage.do?isCheck=false',
             height: 400,
             width: 400,
             title: '添加位置',
             btn: ['确定', '取消'],
    ...
    function deleteData() {
        ...
        if (data.length == 0) {
            ...
            $(data).each(function () {
                ...
                ParamVo.push({'loc_code':rowdata.loc_code});
            });
            $.etDialog.confirm('确定删除?', function () {
                ajaxPostData({
                    url: "deleteAssLocation.do?isCheck=false",
                    data: {
