...
<html>
	...
	<head>
		...
		<jsp:include page="${path}/resource.jsp">
			<jsp:param value="dialog,grid,select,datepicker,pageOffice,jquery_print" name="plugins" />
		</jsp:include>
			...
			*/
			...
			function addRep() {
				  ...
				  parent.$.etDialog.open({
		    		   url: 'hrp/ass/repair/repairrecord/addAssRepairRecordPage.do?isCheck=false',isMax:true, frameName : window.name,
						title: '维修记录添加页',/* modal:true, */ showMax:true,isResize:true,/*showMin: true, */
						   btn: ['保存', '关闭'],
				            btn1: function (index, el) {
				                var iframeWindow = parent.window[el.find('iframe')[0].name]
				                iframeWindow.savePrmGoal()
				            }

		    		/*    buttons: [ 
						           { text: '保存', onclick: function (item, dialog) { dialog.frame.savePrmGoal();  search(); },cls:'l-dialog-btn-highlight' }, 
				 ...
				 var data = grid.selectGet();
			        if (data.length == 0) {
			            ...
			            $.etDialog.confirm('确定删除?', function () {
			                ajaxPostData({
			                    url: "deleteAssRepairRecord.do?isCheck=false",
			                    data: {
			...
			function queryRepairOrder(data, index, value) {
				  parent.$.etDialog.open({
						url: 'hrp/ass/repair/repairrecord/updateAssRepairRecordPage.do?isCheck=false&rep_code=' + data.rep_code,
						isMax:true,
						title: '维修记录修改页',/* modal:true, */ showMax:true,isResize:true,/*showMin: true, */
		    			   parentframename: window.name,
		    			   btn: ['保存', '关闭'],
				            btn1: function (index, el) {
				                var iframeWindow = parent.window[el.find('iframe')[0].name]
				                iframeWindow.updatePrmGoal()
				            }
		    		  /*  buttons: [ 
						           { text: '保存', onclick: function (item, dialog) { dialog.frame.updatePrmGoal(); },cls:'l-dialog-btn-highlight' }, 
						           { text: '关闭', onclick: function (item, dialog) { dialog.close(); } } 
						         ] */});
			}
			// 弹出资产卡片页面
			function queryCard(data, index, value) {
				var parm = '';
				$.etDialog.open({
					url: 'assCardRecordMainPage.do?isCheck=false&ass_card_no=' + value,
					height: 500,
					width: 700,
					title: '资产卡片页',
					btn: ["关闭"],
			...
			function timeLineRender(data) {
				ajaxPostData({
					url: "queryTimeLineRecordRender.do?isCheck=false",
					data: {
			...
			function starRatyRender(data) {
				...
				details_message.text(data.assess);
			...
			function printData() {
				...
				if(data.length == 0) {
					...
					var para = {
						...
						template_code: '05220',
						class_name: "com.chd.hrp.ass.serviceImpl.repair.repairrecord.AssRepairRecordServiceImpl",
						method_name: "queryAssRepairRecordPrint",
