<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	%>
		...
		<head>
			...
			/* 	function initStatus() {
				...
				function search() {
					...
					grid.loadData(param,'queryAssMyRepairCenter.do?isCheck=false');
				}
				// 添加维修单据方法
				function addRep() {
					$.etDialog.open({
						url: 'assMyRepairAddPage.do?isCheck=false',
						isMax:true,
						title: '报修单据添加页',
						btn: ['确定', '取消'],
					 ...
					 var data = grid.selectGet();
				        if (data.length == 0) {
				            ...
				            $.etDialog.confirm('确定删除?', function () {
				                ajaxPostData({
				                    url: "deleteAssMyRepair.do?isCheck=false",
				                    data: {
				...
				function submit() {
					 var data = grid.selectGet();
				        if (data.length == 0) {
				            ...
				            $.etDialog.confirm('确定发送由人工派单?', function () {
				                ajaxPostData({
				...
				function assMyRepairUpdate(data, index, value) {
					$.etDialog.open({
						url: 'assMyRepairUpdatePage.do?isCheck=false&rep_code='+data.rep_code,
						isMax:true,
						title: '报修单据修改页',
						btn: ['确定', '取消'],
						btn1: function (index, el) {
				...
				function starRatyRender(data) {
					gridRowData = data;
					var details_message = $('<textarea id="details_message"></textarea>');
					var details_mark = $('<textarea id="details_mark" disabled="disabled"></textarea>');
					$('#details_assess').empty();
					$('#details_assess').prepend('评分等级：<div id="star" style="display:inline-block;vertical-align:middle"></div><input id="scoreCheck" type="checkbox" ><span title="匿名评价时将不对工程师显示评价内容">匿名评价</span>');
		...
		<body style="position: relative">
			...
			<div id="mainGrid"></div>
			<!-- 维修进度弹窗	 -->
			<div id="details" class="dialog" style="display:none;height: 330px">
