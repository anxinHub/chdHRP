			...
			*/
			...
			function submit() {
				...
				if(data.length == 0) {
					...
					$.etDialog.confirm('确定发送由系统派单?', function() {
						ajaxPostData({
			...
			function submitScore() {
				...
				ParamVo.push({
					name: 'is_any',
					value: $('#scoreCheck')[0].checked == 'true' ? 0 : 1
				})
				ParamVo.push({
					name: 'score',
					value: $('#star').raty('score')
				})
				ParamVo.push({
					name: 'score_note',
					value: $('#details_message').val()
				})
				ParamVo.push({
					name: 'rep_code',
					value: gridRowData.rep_code
				})
				console.log(ParamVo);
				ajaxPostData({
			...
			function printData() {
				...
				if(data.length == 0) {
					...
					var para = {
						...
						template_code: '05312',
						class_name: "com.chd.hrp.ass.serviceImpl.repair.repreportcentre.AssRepReportCentreServiceImpl",
