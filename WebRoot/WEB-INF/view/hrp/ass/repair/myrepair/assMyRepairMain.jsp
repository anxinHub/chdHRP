			...
			*/
			...
			function back() {
				...
				if(data.length == 0) {
					...
					$.etDialog.confirm('确定无法完成要退单吗?', function() {
						ajaxPostData({
							url: "repairBack.do?isCheck=false",
							data: {
								rep_code: ParamVo.toString()
							},
							success: function(res) {
			...
			function starRatyRender(data) {
				...
				var details_mark = $('<textarea id="details_mark"></textarea>');
			...
			function submitScore(){
				if($('#star')==''){
					  $.etDialog.error('备注不能为空！');
					  return;
				}
				var ParamVo = [];
	
				ParamVo.push({name: 'mark',value:$('#details_mark').val()})
			...
			function printData() {
					...
					if(data.length == 0) {
						...
						var para = {
							...
							template_code: '05314',
							class_name: "com.chd.hrp.ass.serviceImpl.repair.myrepair.AssMyRepairServiceImpl",
