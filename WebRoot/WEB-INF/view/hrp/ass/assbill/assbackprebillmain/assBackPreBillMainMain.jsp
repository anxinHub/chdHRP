...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '供应商名称', name: 'ven_id', align: 'left',textField : 'sup_name'
					 		},
                     { display: '合同编码', name: 'pact_code', align: 'left',textField : 'contract_no'
					 		},
                     { display: '发票金额', name: 'bill_money', align: 'right',render : function(rowdata, rowindex,
								value) {
						 return formatNumber(
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssBackPreBillMain.do',
    ...
    function check(){
    	...
    	$.ligerDialog.confirm('确定审核?', function (yes){
        	if(yes){
            	ajaxJsonObjectByUrl("checkAssBackPreBillMain.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function notToExamine(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定销审?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateNotToExamineAssBackPreBillMain.do?isCheck=false", {
  ...
  //打印回调方法
	...
	//打印
	    function printDate(){
		    	...
		    	ajaxJsonObjectByUrl("queryAssBackPreBillState.do?isCheck=false",{paraId:bill_no.substring(0,bill_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
						officeFormPrint(para);
					}
				});
    ...
    function add_open(){
					...
					parent.$.ligerDialog.open({
						...
						width: $(window).width(),
						url: 'hrp/ass/bill/assbackprebillmain/assBackPreBillMainAddPage.do?isCheck=false&',
						modal: true, 
						showToggle: false, 
						showMax: true, 
						showMin: false, 
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssBackPreBillMain.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function imp(){
    	...
    	var index = layer.open({
					...
					area : [ '893px', '500px' ],
					content : 'AssBackPreBillMainImportPage.do?isCheck=false'
				});
				layer.full(index);
    	}	
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({ 
			url : 'hrp/ass/bill/assbackprebillmain/assBackPreBillMainUpdatePage.do?isCheck=false&parm='+parm, 
					height: $(window).height(),
					width: $(window).width(), 
					title:'预退发票登记修改',
					modal:true,
