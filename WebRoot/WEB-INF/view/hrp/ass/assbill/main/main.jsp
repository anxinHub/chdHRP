...
<script type="text/javascript">
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '发票金额', name: 'bill_money', align: 'right',width : 120,
					            ...
					            {
					                    return formatNumber(item.bill_money,'${ass_05055}',1);
					            }
					 		},
                     ...
                     toolbar: { items: [
						...
						{ line: true},
						{ text:'生成发票单',id:'invoice',click: add_invoice,icon : 'add'},
						{ line : true},
    ...
    function confirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定登记?', function(yes) {
				...
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmAssBill.do", {
    ...
    function add_invoice (){
    	parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assbill/main/assBillMainInvoicePage.do?isCheck=false',
	...
	function notToExamine(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定销审?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateNotToExamineAssBill.do?isCheck=false", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	//打印
	    function printDate(){
		    	...
		    	ajaxJsonObjectByUrl("queryAssBillState.do?isCheck=false",{paraId:bill_no.substring(0,bill_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
						officeFormPrint(para);
					}
				});
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssBillMain.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assbill/main/assBillMainUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
    }
