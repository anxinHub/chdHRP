...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssAccessory.do',
    ...
    function save(){
		...
		if (validateGrid()) {
			ajaxJsonObjectByUrl("saveAssCardAccessory.do", formPara, function(
					responseData) {
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        parent.$.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssAccessory.do",{ParamVo : ParamVo.toString(),ass_nature:'${ass_nature}'},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function imp(){
    	...
    	var para={
    		...
    		parent.importSpreadView("hrp/ass/asscard/importAssAccessory.do?isCheck=false&ass_naturs=${ass_nature}&ass_card_no="+ass_card_no,para);
