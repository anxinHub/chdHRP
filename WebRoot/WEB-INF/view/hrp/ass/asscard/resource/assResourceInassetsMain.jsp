...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
					 ...
					 { display: '累计摊销',name: 'depre_money',align: 'right',
								editor : {
									type : 'float',
									precision : 2
								},
					...
					{ display: '残值', name: 'fore_money',align: 'right',
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssResource.do',
    ...
    function save(){
		...
		if (validateGrid()) {
			ajaxJsonObjectByUrl("saveAssCardResource.do", formPara, function(
					responseData) {
                    ...
                    if (data.length == 0){
                        ...
                        parent.$.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	...
                            	ajaxJsonObjectByUrl("deleteAssResource.do",{ParamVo : ParamVo.toString(),ass_nature:'${ass_nature}'},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
