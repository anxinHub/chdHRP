...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'assqueryStore.do',
    ...
    function itemclick(item){ 
        if(item.id)
        {
            ...
            {
                case "add":
              		$.ligerDialog.open({url: 'assstoreAddPage.do?isCheck=false', height: 280,width: 640, title:'添加',modal:true,showToggle:false,showMax:false,showMin: true,isResize:true,buttons: [ { text: '确定', onclick: function (item, dialog) { dialog.frame.saveStore(); },cls:'l-dialog-btn-highlight' }, { text: '取消', onclick: function (item, dialog) { dialog.close(); } } ] });
              		return;
                case "modify":
                    return;
                case "delete":
                    ...
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("assdeleteStore.do",{ParamVo : ParamVo},function (responseData){
            	...
            	case "import":
                   	$.ligerDialog.open({url: 'asssysStoreImportPage.do?isCheck=false', height: 430,width: 760, isResize:true, title:'导入'});
                   	return;
            }   
        }
    ...
    function openUpdate(obj){
		...
		var parm = "group_id="+vo[0]+"&hos_id="+vo[1]+"&store_id="+ vo[2]+"&store_code="+vo[3];
		
    	$.ligerDialog.open({ url : 'assstoreUpdatePage.do?isCheck=false&' + parm,data:{}, height: 500,width: 700, title:'修改',modal:true,showToggle:false,showMax:false,showMin: false,isResize:true,
    			buttons: [ 
    ...
    function loadDict(){
            //字典下拉框
    	autocomplete("#store_code","../queryHosStoreDict.do?isCheck=false","id","text",true,true);
         }   
    </script>
