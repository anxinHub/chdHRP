...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssRemouldAspecial.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assremould/assremouldaspecial/assRemouldAspecialAddPage.do?isCheck=false',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssRemouldAspecial.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assremould/assremouldaspecial/assRemouldAspecialUpdatePage.do?isCheck=false&'
							+ param,
					height : $(window).height(),
					width : $(window).width(),
					modal : true,
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssBackMainSpecial.do", selPara,
				function(responseData) {
