...
<script type="text/javascript">
	...
	function initTab() {
		...
		$.post("getAssCardInitTab.do?isCheck=false", {
	...
	function selectTab(tabid){
		...
		$.each(tabData.Rows,function(i,v){
			if(v.tab_id == tabid){
				navtab.setTabItemSrc(tabid,v.init_tab_url+"&ass_card_no=${ass_card_no}");
			}
		});
		tab_ids.push(tabid);
	}
	...
	function initTable() {
		$
				.post(
						"getAssCardInitTitle.do?isCheck=false",
						{
							"ass_nature" : "${ass_nature}"
						},
						function(data) {
	...
	function save() {
		...
		ajaxJsonObjectByUrl("updateAssCardInit.do?isCheck=false", formPara,
		...
		$.each(rowData.Rows,
					...
					if (v.is_init_view == 1) {
						...
						if (v.type_code == 0) {
							...
							if (v.is_change == 1) {
								...
								liger.get("" + str + "").setValue(
										obj[v.col_code.toLowerCase()] == null?"":obj[v.col_code.toLowerCase()]);
								liger.get("" + str + "")
										.setText(
												obj[v.col_code.toLowerCase()
														+ "_name"] == null?"":obj[v.col_code.toLowerCase()
															+ "_name"]);
							}
			...
			$.post("queryAssDictByID.do?isCheck=false",{ass_id:liger.get("ASS_ID").getValue().split("@")[0],ass_no:liger.get("ASS_ID").getValue().split("@")[1]},function(resdata){
				if('${ass_nature}' == '01'){
					...
					liger.get("GB_CODE").setValue(resdata.gb_code);
					liger.get("GB_CODE").setText(resdata.gb_code+' '+resdata.gb_name);
