...
<script type="text/javascript">
		...
		var data = grid.getData();
	         ...
	         if($(".btn1").length > 0)
	           ...
	           $.each(data,function(i,v){
	        	   var file_url = "viewAssPhotoInit.do?isCheck=false&file_url="+v.file_url;
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					...
					height : 350,
					width : 600,
					url : 'assPhotoInitAddPage.do?isCheck=false&ass_nature=${ass_nature}&ass_card_no=${ass_card_no}',
					modal : false,
					showToggle : false,
					showMax : false,
					showMin : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			parent.$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssPhotoInit.do", {
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'assPhotoInitUpdatePage.do?isCheck=false&'+parm,
			data : {},
			height : 350,
			width : 600,
			title : '资产照片修改',
