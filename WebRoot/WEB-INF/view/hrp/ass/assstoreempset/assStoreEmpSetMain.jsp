...
<script type="text/javascript">
	...
	function add_open() {
		$.ligerDialog.open({
			url : 'assStoreEmpSetAddPage.do?isCheck=false',
			height : 300,
			width : 500,
			title : '添加',
			modal : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssStoreEmp.do?isCheck=true", {
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url : 'assStoreEmpSetUpdatePage.do?isCheck=false&' + parm,
			height : 300,
			width : 500,
			title : '修改',
			modal : true,
	...
	function loadDict() {
		//字典下拉框
		autocomplete("#store_id","../queryHosStoreDict.do?isCheck=false","id","text",true,true);
