...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     { display: '参数编码', name: 'para_code',width: '5%', align: 'left'
					 },
                     { display: '参数名称', name: 'para_name',width: '20%', align: 'left'
					 },
                     { display: '参数值', name: 'para_value',width: '13%', align: 'center',
							render : function(rowdata, rowindex,
									value) {
								var str = "";
								if(rowdata.para_type == '0'){
									str = "<input   name='para_value"+rowindex+"' type='text' id='para_value"+rowindex+"'"
										+" value='"+rowdata.para_value+"' style='margin-top:5px;width:80px;'/>";
								}
                     ...
                     { display: '参数说明', name: 'note',width: '45%', align: 'left'
					 },
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssPara.do',
	...
	/* function save(group_id,hos_id,copy_code,para_code,rowindex){
		...
		if(flag){
		 ajaxJsonObjectByUrl("updateAssPara.do",formPara,function(responseData){
	...
	function save(){
		...
		$(data).each(function(){
			/* var reg = new RegExp("^[0-9][0-9]*$");*/
			var para_value = $("#para_value"+this.num+"").val();
		...
		if(formPara.length>0){
			/* if(str != ''){
				...
				ajaxJsonObjectByUrl("updateAssPara.do?isCheck=false",{formPara:formPara.toString()},function(responseData){
		            if(responseData.state=="true"){
		                query();
		            }
		        }); 
