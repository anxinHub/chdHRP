...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssBackRestMainGeneral.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssBackRestMainGeneral.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function backConfirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认退库?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmBackRestGeneral.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assgeneral/assrestback/assBackRestGeneralAddPage.do?isCheck=false',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssBackRestMainGeneral.do",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assgeneral/assrestback/assBackRestGeneralUpdatePage.do?isCheck=false&'
							+ param,
					height : $(window).height(),
					width : $(window).width(),
					modal : true,
	...
	function printSet(){
	  ...
	  function printDate(){
			...
			if (data.length == 0){
				...
				ajaxJsonObjectByUrl("queryAssBackRestGeneralState.do?isCheck=false",{paraId:ass_back_no.substring(0,ass_back_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
