...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssBackMainGeneral.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssBackMainGeneral.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function openViewApply(obj) {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assgeneral/assback/assViewBackPage.do?isCheck=false&'
			+ parm,
			modal: true,
			showToggle: false,
			showMax: true,
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					url : 'hrp/ass/assgeneral/assback/assBackGeneralAddPage.do?isCheck=false',
					height : $(window).height(),
					width : $(window).width(),
					modal : true,
					showToggle : false,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssBackMainGeneral.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function printSet(){
	  ...
	  function printDate(){
			...
			if (data.length == 0){
				...
				ajaxJsonObjectByUrl("queryAssBackGeneralState.do?isCheck=false",{paraId:ass_back_no.substring(0,ass_back_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
