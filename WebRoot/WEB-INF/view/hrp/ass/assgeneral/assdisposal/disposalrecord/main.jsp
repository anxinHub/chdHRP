...
<script type="text/javascript">
				...
				grid = $("#maingrid").ligerGrid(
						{
							...
							},
							{
								...
								render : function(rowdata, rowindex,
										value) {
									return formatNumber(
											rowdata.dispose_cost == null ? 0
													: rowdata.dispose_cost,
											'${ass_05005}',
											1);
								}
							},
							{
								...
								render : function(rowdata, rowindex,
										value) {
									return formatNumber(
											rowdata.dispose_income == null ? 0
													: rowdata.dispose_income,
											'${asS_05005}',
											1);
								}
							},
							...
							usePager : false,
							url : 'queryAssDisposalRecordGeneral.do?isCheck=false&show_detail=1',
							width : '100%',
							height : '100%',
							checkbox : true,
							rownumbers : true,
							...
							toolbar : {
								items : [ {
								 ...
								 //{ text: '取消确认', id:'unconfirm', click: unconfirm,icon:'candle' }, 
								 //{ line:true },
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						usePager : false,
						url : 'queryAssDisposalRecordGeneral.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmDisposalRecordGeneral.do", {
	...
	function unconfirm() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('取消确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateUnConfirmDisposalRecordGeneral.do?isCheck=false", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assgeneral/assdisposal/disposalrecord/assDisposalRecordGeneralAddPage.do?isCheck=false',
			title : '记录信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openView(obj){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assgeneral/assdisposal/disposalrecord/assViewSpecialPage.do?isCheck=false&'+parm,
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ "copy_code=" + vo[2] + "&" + "dis_r_no=" + vo[3];
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assgeneral/assdisposal/disposalrecord/assDisposalRecordGeneralUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '记录信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssDisposalRecordGeneral.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"05033",use_id : useId});
    }
    ...
    function print(){
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			paraId :dis_r_no.substring(0,dis_r_no.length-1) ,
	    			class_name:"com.chd.hrp.ass.serviceImpl.assdisposal.general.AssDisposalRecordGeneralServiceImpl",
	    			method_name:"assDisposalRecordGeneralByPrintTemlate",
	    			template_code:'05033',
           	...
           	ajaxJsonObjectByUrl("queryState.do?isCheck=false",{paraId :dis_r_no.substring(0,dis_r_no.length-1) },function (responseData){
           		if(responseData.state=="true"){
