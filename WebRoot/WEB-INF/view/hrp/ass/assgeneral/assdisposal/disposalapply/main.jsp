...
<script type="text/javascript">
				...
				grid = $("#maingrid").ligerGrid(
						{
							...
							usePager : false,
							url : 'queryAssDisposalApplyGeneral.do?isCheck=false&show_detail=1',
							width : '100%',
							height : '100%',
							checkbox : true,
							rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						usePager : false,
						url : 'queryAssDisposalApplyGeneral.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function unconfirm() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateUnConfirmDisposalApplyGeneral.do?isCheck=false", {
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmDisposalApplyGeneral.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assgeneral/assdisposal/disposalapply/assDisposalApplyGeneralAddPage.do?isCheck=false',
			title : '资产处置申请增加',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assgeneral/assdisposal/disposalapply/assDisposalApplyGeneralUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '资产处置申请修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssDisposalApplyGeneral.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"05311",use_id : useId});
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.assdisposal.general.AssDisposalApplyGeneralServiceImpl",
	    			method_name:"assDisposalApplyGeneralByPrintTemlate",
	    			template_code:'05311',
