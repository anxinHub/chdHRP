...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssCheckPlanGeneral.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function add_open() {
		...
		$.ligerDialog.open({
			url : 'assCheckPlanGeneralAddPage.do?isCheck=false&',
			data : {},
			height : 300,
			width : 450,
			title : '盘点任务单(一般设备)',
 ...
 */
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssCheckPlanGeneral.do", {ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("GeneralAudit.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unAudit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("GeneralUnAudit.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function finish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定盘点完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("GeneralFinish.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unFinish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("GeneralUnfinish.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function generate() {
		...
		if (data.length == 0 || data.length > 1) {
			...
			$.ligerDialog.confirm('生成的盘点单将会覆盖原有数据?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("GeneralGenerate.do", formPara, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assgeneral/asscheck/plan/assCheckPlanGeneralUpdatePage.do?isCheck=false&'+ parm,
 ...
 */
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssCheckPlanGeneral.do?isCheck=false",
