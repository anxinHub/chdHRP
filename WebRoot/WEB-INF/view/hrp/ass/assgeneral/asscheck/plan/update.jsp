	...
	grid = $("#maingrid").ligerGrid(
			{
				...
				dataType : 'server',
				usePager : false,
				url : 'queryAssCheckDdetailGeneral.do?isCheck=false',
				width : '100%',
				height : '100%',
				checkbox : true,
				rownumbers : true,
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : false,
					url : 'queryAssCheckSdetailGeneral.do?isCheck=false',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	if(is_dept=="01"){
		...
		if (data.length == 0 ) {
			...
			var formPara = {
					...
					ajaxJsonObjectByUrl("saveAssCheckSDetailGeneral.do?isCheck=false", formPara, function(
							responseData) {
		...
		if (data.length == 0 ) {
			...
			ajaxJsonObjectByUrl("saveAssCheckDDetailGeneral.do?isCheck=false", formPara, function(
					responseData) {
				if (responseData.state == "true") {
					query();
				}
...
function inventory() {
	...
	$(data).each(function(){
		...
		data1.push(this.ass_id+"@"+this.hos_id+"@"+this.copy_code+"@"+this.group_id+"@"+this.store_id+"@"+this.ass_card_no+"@"+this.ass_type_name+"@"+this.ass_name+"@"+this.store_name+"@"+this.acc_amount+"@"+this.check_amount+"@"+this.pl_amount+"@"+this.pl_reason+"@"+this.store_no+"@"+this.ass_no+"@"+this.check_no+"@"+this.check_plan_no);
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("saveGeneralInventory.do?isCheck=false"  ,
			 ...
			 $(data).each(function(){
				...
				data2.push(this.ass_id+"@"+this.hos_id+"@"+this.copy_code+"@"+this.group_id+"@"+this.dept_id+"@"+this.ass_card_no+"@"+this.ass_type_name+"@"+this.ass_name+"@"+this.dept_name+"@"+this.acc_amount+"@"+this.check_amount+"@"+this.pl_amount+"@"+this.pl_reason+"@"+this.dept_no+"@"+this.ass_no+"@"+this.check_no+"@"+this.check_plan_no);
					 ...
					 $.ligerDialog.confirm('确定生成么?', function(yes) {
					 if (yes) {
					 	ajaxJsonObjectByUrl("saveDeptGeneralInventory.do?isCheck=false"  ,
...
function tree_open() {
	...
	if(is_dept=="01" || v_text=="仓库"){
		$.ligerDialog.open({
			url : 'assCheckSGeneralAddPage.do?isCheck=false&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
		...
		$.ligerDialog.open({
			url : 'assCheckDGeneralAddPage.do?isCheck=falsefalse&' + parm,
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckSGeneral.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
$.ligerDialog.confirm('确定删除?', function(yes) {
	if (yes) {
		ajaxJsonObjectByUrl("delAssCheckDGeneral.do?isCheck=false&" + parm,
				{}, function(
				responseData) {
			if (responseData.state == "true") {
				query();
...
if (yes) {
	ajaxJsonObjectByUrl("GeneralGenerateDetail.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
			query();
...
if (yes) {
	ajaxJsonObjectByUrl("auditGeneral.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
...
if (yes) {
	ajaxJsonObjectByUrl("unAuditGeneral.do?isCheck=false&" + parm,
			{}, function(
			responseData) {
		if (responseData.state == "true") {
...
if(isnull($("#check_no").val())){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 if (yes) {
		 	ajaxJsonObjectByUrl("copyAmountAssCheckSGeneral.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
		...
		"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定复制账面数量?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("copyAmountAssCheckDGeneral.do?isCheck=false&" + parm,
...
function remove(){
	...
	"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定删除?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("delAmountAssCheckSGeneral.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
		...
		"&"+"group_id="+group_id+"&hos_id="+hos_id+"&copy_code="+copy_code;
		 ...
		 $.ligerDialog.confirm('确定删除?', function(yes) {
		 if (yes) {
		 	ajaxJsonObjectByUrl("delAmountAssCheckDGeneral.do?isCheck=false&" + parm,
		 			{}, function(
		 			responseData) {
		 		if (responseData.state == "true") {
		 			query();
...
function checkRgister(){
	...
	if(is_dept=="01"){
		$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'assCheckSpGeneralPage.do?isCheck=false&'+parm,
		...
		$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'assCheckDpGeneralPage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
...
function printSet(){
	...
	officeFormTemplate({template_code:"05030",use_id : useId})
}
	...
	/* if (data.length == 0){
		...
		$(data).each(function (){	
		 ...
		 var para={
				...
				template_code:'05030',
				class_name:"com.chd.hrp.ass.serviceImpl.check.general.AssCheckSdetailGeneralServiceImpl", 
			 ...
			 var useId=0;//统一打印
				if('${ass_05030}'==1){
				 ...
				 var para={
						...
						template_code:'05030',
						class_name:"com.chd.hrp.ass.serviceImpl.check.general.AssCheckDdetailGeneralServiceImpl", 
