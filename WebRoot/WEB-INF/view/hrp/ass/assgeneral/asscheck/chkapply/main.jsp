...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					usePager : false,
					url : 'queryAssChkApplyGeneral.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmChkApplyGeneral.do", {
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assgeneral/check/chkapply/assChkApplyGeneralAddPage.do?isCheck=false',
			title : '申报信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assgeneral/check/chkapply/assChkApplyGeneralUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '申报信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssChkApplyGeneral.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
