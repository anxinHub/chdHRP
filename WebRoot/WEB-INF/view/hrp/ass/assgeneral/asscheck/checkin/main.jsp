...
<script type="text/javascript">
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmChkInGeneral.do", {
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assgeneral/check/checkin/assChkInGeneralAddPage.do?isCheck=false',
			title : '盘盈入库增加',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assgeneral/check/checkin/assChkInGeneralUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '盘盈入库修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssChkInGeneral.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
	...
	function loadDict() {
		...
		autocomplete("#store_id", "../../../queryHosStoreDict.do?naturs_code=03&isCheck=false","id", "text",true,true,param,true,null,"160");
