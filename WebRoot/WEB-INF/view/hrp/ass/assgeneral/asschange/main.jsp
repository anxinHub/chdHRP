...
<script type="text/javascript">
	...
	function confirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('变动确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmChangeGeneral.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assgeneral/asschange/assChangeGeneralAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssChangeGeneral.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assgeneral/asschange/assChangeGeneralUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.change.AssChangeGeneralServiceImpl",
	    			method_name:"assChangeGeneralByPrintTemlate",
	    			template_code:'05031',
