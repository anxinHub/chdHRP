...
<script type="text/javascript">
			...
			.change(
					function() {
						var store_id = liger.get("out_store_id").getValue()
								.split("@")[0];
						var store_no = liger.get("out_store_id").getValue()
								.split("@")[1];
	...
	function save() {
		...
		if (validateGrid()) {
			ajaxJsonObjectByUrl("saveAssTranStoreGeneral.do", formPara,
	...
	function cardSelect(){
		...
		var str = "select * from ASS_TRAN_STORE_DETAIL_GENERAL p "+
		" left join ASS_TRAN_STORE_GENERAL b on"+
		...
		editor.grid.url = '../../../queryAssCardTable.do?isCheck=false&ass_nature=03&use_state=1,2,3,4,5&in_date='
				...
				+ '&is_dept=0&store_id='
				+ store_id
				+ '&store_no=' + store_no+'&ass_card_not_exists='+ass_card_nos.toString()+"&sql="+str;
				...
				.ligerGrid(
						{
							...
							usePager : false,
							width : '100%',
							url : 'queryAssTranStoreDetailGeneral.do',
							height : '97%',
							checkbox : true,
							enabledEdit : true,
							alternatingRow : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssTranStoreDetailGeneral.do", {
	...
	function loadDict() {
		...
		$("#in_store_id").ligerComboBox({
			url : '../../../queryHosStoreDict.do?naturs_code=03&isCheck=false',
			valueField : 'id',
			textField : 'text',
			selectBoxWidth : 160,
			autocomplete : true,
		...
		liger.get("out_store_id")
				.setText("${out_store_code} ${out_store_name}");
	}
	...
	function print() {
		...
		ajaxJsonObjectByUrl("queryState.do?isCheck=false", {
			tran_no : $("#tran_no").val()
		}, function(responseData) {
			if (responseData.state == "true") {
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit"
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td"><input name="tran_no"
				value="${tran_no }" type="text" id="tran_no" /></td>
			<td align="left"></td>
			<td align="right" class="l-table-edit-td" style="padding-left: 20px;"><b><font
					color="red">*</font></b>制单日期：</td>
			<td align="left" class="l-table-edit-td"><input
				name="create_date" type="text" id="create_date" class="Wdate"
				onFocus="WdatePicker({isShowClear:true,readOnly:false,dateFmt:'yyyy-MM-dd',onpicked:changeCreateDate})"
				value="${create_date }" /></td>
			<td align="left"></td>
			<td align="right" class="l-table-edit-td" style="padding-left: 20px;"><b><font
					color="red">*</font></b>移出仓库：</td>
			<td align="left" class="l-table-edit-td"><input
				name="out_store_id" type="text" id="out_store_id" /></td>
			<td align="left"></td>
			<td align="right" class="l-table-edit-td" style="padding-left: 20px;"><b><font
					color="red">*</font></b>移入仓库：</td>
			<td align="left" class="l-table-edit-td"><input
