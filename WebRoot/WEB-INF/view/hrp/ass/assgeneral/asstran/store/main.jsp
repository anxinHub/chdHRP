...
<script type="text/javascript">
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranStoreGeneral.do?isCheck=false&show_detail=1',
    		...
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranStoreGeneral.do?isCheck=false&show_detail=0',
    	                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad :true,
    	                     selectRowButtonOnly:true,//heightDiff: -10,
    	                     toolbar: { items: [
    	                     	{ text: '查询（<u>E</u>）', id:'search', click: query,icon:'search' },
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		width: $(window).width(),
    		url: 'hrp/ass/assgeneral/asstran/store/assTranStoreGeneralAddPage.do?isCheck=false&',
	...
	function updateConfirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定移库?', function(yes) {
				...
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmTranStoreGeneral.do", {
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssTranStoreGeneral.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assgeneral/asstran/store/assTranStoreGeneralUpdatePage.do?isCheck=false&'+ parm,
    ...
    function loadDict(){
    	...
    	autocomplete("#out_store_id", "../../../queryHosStoreDict.do?naturs_code=03&isCheck=false","id", "text",true,true,param,true);
    	
    	autocomplete("#in_store_id", "../../../queryHosStoreDict.do?naturs_code=03&isCheck=false","id", "text",true,true,param,true);
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0508901",use_id : useId});
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.tran.store.AssTranStoreGeneralServiceImpl",
	    			method_name:"assTranStoreGeneralByPrintTemlate",
	    			template_code:'0508901',
