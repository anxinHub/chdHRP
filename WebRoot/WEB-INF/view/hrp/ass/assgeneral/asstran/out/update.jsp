...
<script type="text/javascript">
				...
				.change(
						function() {
							var dept_id = liger.get("dept_id").getValue()
									.split("@")[0];
							var dept_no = liger.get("dept_id").getValue()
									.split("@")[1];

							if (dept_no == null || dept_id == null
									|| dept_id == "" || dept_no == "") {
								dept_no = "";
								dept_id = "";
							}
	...
	function save() {
		...
		if (validateGrid()) {
			ajaxJsonObjectByUrl("saveAssOutGeneral.do", formPara, function(
					responseData) {
				...
				.ligerGrid(
						{
							...
							usePager : false,
							width : '100%',
							url : 'queryAssOutDetailGeneral.do',
							height : '97%',
							checkbox : true,
							enabledEdit : true,
							alternatingRow : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssOutDetailGeneral.do", {
	...
	$("#dept_id")
			.change(
					function() {
						...
						var store_id = liger.get("store_id").getValue().split(
								"@")[0];
						var store_no = liger.get("store_id").getValue().split(
								"@")[1];
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit"
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td"><input name="out_no"
				value="${out_no }" type="text" id="out_no" /></td>
			<td align="left"></td>
			<td align="right" class="l-table-edit-td" style="padding-left: 20px;"><b><font
