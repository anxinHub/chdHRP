		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssOutGeneral.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssOutGeneral.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
...
function offset(){
    	...
    	if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要冲销吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("offsetOutGeneral.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					...
					width : $(window).width(),
					url : 'hrp/ass/assgeneral/asstran/out/assOutGeneralAddPage.do?isCheck=false&',
					modal : true,
					showToggle : false,
					showMax : true,
					showMin : true,
	...
	function updateConfirm() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定移库?', function(yes) {
				...
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmOutGeneral.do", {
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssOutGeneral.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog
				.open({
					...
					height : $(window).height(),
					width : $(window).width(),
					url : 'hrp/ass/assgeneral/asstran/out/assOutGeneralUpdatePage.do?isCheck=false&'
							+ parm,
					modal : true,
					showToggle : false,
					showMax : true,
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0508903",use_id : useId});
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.tran.out.AssOutGeneralServiceImpl",
	    			method_name:"assOutGeneralByPrintTemlate",
	    			template_code:'0508903',
    ...
    function openViewAssIn(obj){
  	  var vo = obj.split("|");
  		...
  		parent.$.ligerDialog.open({
  			...
  			height: $(window).height(),
  			width: $(window).width(),
  			url: 'hrp/ass/assgeneral/asstran/out/assViewInMainPage.do?isCheck=false&'+parm,
