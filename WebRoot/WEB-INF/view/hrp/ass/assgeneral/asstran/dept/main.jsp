...
<script type="text/javascript">
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     { display: '移出科室', name: 'out_dept_name', align: 'left',width : 100,
			 		 ...
			 		 {display : '卡片编码',name : 'ass_card_no',align : 'left',width : 100,
							}, 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranDeptGeneral.do?isCheck=false&show_detail=1',
    		...
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranDeptGeneral.do?isCheck=false&show_detail=0',
    	                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad :true,
    	                     selectRowButtonOnly:true,//heightDiff: -10,
    	                     toolbar: { items: [
    	                     	{ text: '查询（<u>E</u>）', id:'search', click: query,icon:'search' },
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		width: $(window).width(),
    		url: 'hrp/ass/assgeneral/asstran/dept/assTranDeptGeneralAddPage.do?isCheck=false&',
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssTranDeptGeneral.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assgeneral/asstran/dept/assTranDeptGeneralUpdatePage.do?isCheck=false&'+ parm,
	...
	//打印模板设置
	    function printSet(){
			...
			officeFormTemplate({template_code:"0508902",use_id : useId});
	    }
			...
			if (data.length == 0){
				...
				$(data).each(function (){
				 ...
				 var para={
		    			...
		    			class_name:"com.chd.hrp.ass.serviceImpl.tran.dept.AssTranDeptGeneralServiceImpl",
		    			method_name:"assTranDeptGeneralByPrintTemlate",
		    			template_code:'0508902',
