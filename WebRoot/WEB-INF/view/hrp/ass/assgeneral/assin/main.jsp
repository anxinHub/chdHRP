			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssInMainGeneral.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssInMainGeneral.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
...
function offset(){
    	...
    	if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要冲销吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("offsetInGeneral.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assgeneral/assin/assInMainGeneralAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssInMainGeneral.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
    ...
    function printDate(){
		...
		if (data.length == 0){
			...
			//ajaxJsonObjectByUrl("queryAssInGeneralState.do?isCheck=false",{paraId:ass_in_no.substring(0,ass_in_no.length-1),state:2},function(responseData){
				//if (responseData.state == "true") {
