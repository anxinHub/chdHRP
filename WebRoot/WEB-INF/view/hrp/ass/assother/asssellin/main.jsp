...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssSellInOther.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssSellInOther.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function openViewSellOut(obj){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assother/asssellin/assViewSellOutOtherPage.do?isCheck=false&'+parm,
	...
	function initCard(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认入库?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmSellInMainOther.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assother/asssellin/assSellInOtherAddPage.do?isCheck=false&',
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssSellInOther.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assother/asssellin/assSellInOtherUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
	function loadDict() {
		...
		autocomplete("#in_store_id", "../../queryHosStoreDict.do?naturs_code=04&isCheck=false","id", "text",true,true,null,false,null,"300");
		...
		$("#out_group_id").change(function(){
			...
			$("#out_hos_id").change(function(){
				autocomplete("#out_store_id", "../../queryHosStoreDict.do?naturs_code=04&isCheck=false","id", "text",true,true,{group_id:liger.get("out_group_id").getValue(),hos_id:liger.get("out_hos_id").getValue()},false,null,"160");
			});
		});
		$('#state').ligerComboBox({
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0503901",use_id : useId})
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){	
			 ...
			 var para={
					...
					template_code:'0503901',
					class_name:"com.chd.hrp.ass.serviceImpl.sell.in.AssSellInOtherServiceImpl", 
					method_name:"querySellInOtherByPrintTemlatePrint",
			...
			ajaxJsonObjectByUrl("queryAssSellInOtherState.do?isCheck=false",{paraId:sell_in_no.substring(0,sell_in_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
