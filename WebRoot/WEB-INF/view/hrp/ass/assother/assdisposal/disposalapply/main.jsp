...
<script type="text/javascript">
				...
				grid = $("#maingrid").ligerGrid(
						{
							...
							usePager : false,
							url : 'queryAssDisposalApplyOther.do?isCheck=false&show_detail=1',
							width : '100%',
							height : '100%',
							checkbox : true,
							rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						usePager : false,
						url : 'queryAssDisposalApplyOther.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function unconfirm() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('取消确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateUnConfirmDisposalApplyOther.do?isCheck=false", {
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmDisposalApplyOther.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assother/assdisposal/disposalapply/assDisposalApplyOtherAddPage.do?isCheck=false',
			title : '申报信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assother/assdisposal/disposalapply/assDisposalApplyOtherUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '申报信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssDisposalApplyOther.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
