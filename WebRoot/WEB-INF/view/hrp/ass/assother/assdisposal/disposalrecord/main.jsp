...
<script type="text/javascript">
				...
				grid = $("#maingrid").ligerGrid(
						{
							...
							usePager : false,
							url : 'queryAssDisposalRecordOther.do?isCheck=false&show_detail=1',
							width : '100%',
							height : '100%',
							checkbox : true,
							rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						usePager : false,
						url : 'queryAssDisposalRecordOther.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('处置确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateConfirmDisposalRecordOther.do", {
	...
	function unconfirm() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('取消确认?', function (yes){
            	if(yes){
					ajaxJsonObjectByUrl("updateUnConfirmDisposalRecordOther.do?isCheck=false", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function add_open() {
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assother/assdisposal/disposalrecord/assDisposalRecordOtherAddPage.do?isCheck=false',
			title : '记录信息录入',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function openView(obj){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assother/assdisposal/disposalrecord/assViewSpecialPage.do?isCheck=false&'+parm,
	...
	function openUpdate(obj) {
		...
		var parm = "group_id=" + vo[0] + "&" + "hos_id=" + vo[1] + "&"
				+ "copy_code=" + vo[2] + "&" + "dis_r_no=" + vo[3];
		parent.$.ligerDialog.open({
			url : 'hrp/ass/assother/assdisposal/disposalrecord/assDisposalRecordOtherUpdatePage.do?isCheck=false&' + parm.toString(),
			title : '记录信息修改',
			modal : true,
			showToggle : false,
			showMax : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssDisposalRecordOther.do?isCheck=false",
							{
								ParamVo : ParamVo.toString()
							}, function(responseData) {
								if (responseData.state == "true") {
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"05043",use_id : useId});
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.assdisposal.other.AssDisposalRecordOtherServiceImpl",
	    			method_name:"assDisposalRecordOtherByPrintTemlate",
	    			template_code:'05043',
