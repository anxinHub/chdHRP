...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssAllotInOther.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssAllotInOther.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function openViewAllotOut(obj){
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assother/assallotin/assViewAllotOutOtherPage.do?isCheck=false&'+parm,
	...
	function initCard(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认入库?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmAllotInMainOther.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assother/assallotin/assAllotInOtherAddPage.do?isCheck=false&',
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssAllotInOther.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assother/assallotin/assAllotInOtherUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0503801",use_id : useId})
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){	
			 ...
			 var para={
					...
					template_code:'0503801',
					class_name:"com.chd.hrp.ass.serviceImpl.allot.in.AssAllotInOtherServiceImpl", 
					method_name:"queryAssAllotInOtherByPrintTemlatePrint",
			...
			ajaxJsonObjectByUrl("queryAssAllotInOtherState.do?isCheck=false",{paraId:allot_in_no.substring(0,allot_in_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
