...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssAllotOutOther.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssAllotOutOther.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@" + this.hos_id + "@"
								+ this.copy_code + "@" + this.allot_out_no);
					});
			$.ligerDialog.confirm('确认出库?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmAllotOutOther.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assother/assallotout/assAllotOutOtherAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssAllotOutOther.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assother/assallotout/assAllotOutOtherUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0503802",use_id : useId})
    }
   ...
   function printDate(){
		...
		if (data.length == 0){
			...
			$(data).each(function (){	
				...
				allot_out_no  += "'"+this.allot_out_no+"',"
					
			});
			
			 var para={
					 
					template_code:'0503802',
					class_name:"com.chd.hrp.ass.serviceImpl.allot.out.AssAllotOutOtherServiceImpl", 
					method_name:"queryAssAllotOutOtherByPrintTemlatePrint",
					isSetPrint:false,//是否套打，默认非套打
					isPreview:true,//是否预览，默认直接打印
	    			paraId :allot_out_no.substring(0,allot_out_no.length-1) ,
	    			isPrintCount:false,//更新打印次数
	    			use_id:useId,
	    			p_num:1
	    			//isSetPrint:flag
	    	}; 
			ajaxJsonObjectByUrl("queryAssAllotOutOtherState.do?isCheck=false",{paraId:allot_out_no.substring(0,allot_out_no.length-1),state:2},function(responseData){
				if (responseData.state == "true") {
