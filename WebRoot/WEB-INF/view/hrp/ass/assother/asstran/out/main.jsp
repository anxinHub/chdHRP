		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssOutOther.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssOutOther.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
...
function offset(){
    	...
    	if (data.length == 0){
			...
			$.ligerDialog.confirm('确定要冲销吗?', function (yes){
				if(yes){
					ajaxJsonObjectByUrl("offsetOutOther.do?isCheck=false",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function add_open() {
		...
		parent.$.ligerDialog
				.open({
					...
					width : $(window).width(),
					url : 'hrp/ass/assother/asstran/out/assOutOtherAddPage.do?isCheck=false&',
					modal : true,
					showToggle : false,
					showMax : true,
					showMin : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssOutOther.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog
				.open({
					...
					height : $(window).height(),
					width : $(window).width(),
					url : 'hrp/ass/assother/asstran/out/assOutOtherUpdatePage.do?isCheck=false&'
							+ parm,
					modal : true,
					showToggle : false,
					showMax : true,
	...
	function loadDict() {
		...
		autocomplete("#store_id",
				"../../../queryHosStoreDict.do?naturs_code=04&isCheck=false", "id", "text",
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssOutOther.do?isCheck=false", selPara,
				function(responseData) {
    ...
    function printSet(){
		...
		officeFormTemplate({template_code:"0509003",use_id : useId});
    }
		...
		if (data.length == 0){
			...
			$(data).each(function (){
			 ...
			 var para={
	    			...
	    			class_name:"com.chd.hrp.ass.serviceImpl.tran.out.AssOutOtherServiceImpl",
	    			method_name:"assOutOtherByPrintTemlate",
	    			template_code:'0509003',
    ...
    function openViewAssIn(obj){
    	  var vo = obj.split("|");
    		...
    		parent.$.ligerDialog.open({
    			...
    			height: $(window).height(),
    			width: $(window).width(),
    			url: 'hrp/ass/assother/asstran/out/assViewInMainPage.do?isCheck=false&'+parm,
