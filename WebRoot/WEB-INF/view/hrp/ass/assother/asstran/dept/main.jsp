...
<script type="text/javascript">
    ...
    function loadHead(){
    	...
    	grid = $("#maingrid").ligerGrid({
           columns: [ 
                     ...
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranDeptOther.do?isCheck=false&show_detail=1',
    		...
    		grid = $("#maingrid").ligerGrid({
    	           columns: [ 
    	                     ...
    	                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryAssTranDeptOther.do?isCheck=false&show_detail=0',
    	                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad :true,
    	                     selectRowButtonOnly:true,//heightDiff: -10,
    	                     toolbar: { items: [
    	                     	{ text: '查询（<u>E</u>）', id:'search', click: query,icon:'search' },
    ...
    function add_open(){
    	...
    	parent.$.ligerDialog.open({
    		...
    		width: $(window).width(),
    		url: 'hrp/ass/assother/asstran/dept/assTranDeptOtherAddPage.do?isCheck=false&',
	...
	function updateConfirm(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定移库?', function(yes) {
				...
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmTranDeptOther.do", {
    	...
    	var data = gridManager.getCheckedRows();
                    if (data.length == 0){
                        ...
                        $.ligerDialog.confirm('确定删除?', function (yes){
                        	if(yes){
                            	ajaxJsonObjectByUrl("deleteAssTranDeptOther.do",{ParamVo : ParamVo.toString()},function (responseData){
                            		if(responseData.state=="true"){
                            			query();
                            		}
                            	});
    ...
    function openUpdate(obj){
		...
		parent.$.ligerDialog.open({
    		...
    		height: $(window).height(),
    		width: $(window).width(),
    		url: 'hrp/ass/assother/asstran/dept/assTranDeptOtherUpdatePage.do?isCheck=false&'+ parm,
	...
	//打印模板设置
	    function printSet(){
			...
			officeFormTemplate({template_code:"0509002",use_id : useId});
	    }
			...
			if (data.length == 0){
				...
				$(data).each(function (){
				 ...
				 var para={
		    			...
		    			class_name:"com.chd.hrp.ass.serviceImpl.tran.dept.AssTranDeptOtherServiceImpl",
		    			method_name:"assTranDeptOtherByPrintTemlate",
		    			template_code:'0509002',
