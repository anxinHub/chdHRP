...
<script type="text/javascript">
			...
			grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryAssInRestMainOther.do?isCheck=false&show_detail=1',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
			...
			grid = $("#maingrid").ligerGrid(
					{
						...
						dataType : 'server',
						usePager : true,
						url : 'queryAssInRestMainOther.do?isCheck=false&show_detail=0',
						width : '100%',
						height : '100%',
						checkbox : true,
						rownumbers : true,
	...
	function initCard(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确认入库?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("updateConfirmInRestMainOther.do", {
	...
	function add_open() {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assother/assrestin/assInRestMainOtherAddPage.do?isCheck=false&',
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssInRestMainOther.do", {
						ParamVo : ParamVo.toString()
					}, function(responseData) {
						if (responseData.state == "true") {
							query();
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			height: $(window).height(),
			width: $(window).width(),
			url: 'hrp/ass/assother/assrestin/assInRestMainOtherUpdatePage.do?isCheck=false&'+parm,
			modal: true, showToggle: false, showMax: true, showMin: false, isResize: true,
			parentframename: window.name,  //用于parent弹出层调用本页面的方法或变量
		});
	}
	...
	function printSet(){
	  ...
	  function printDate(){
			...
			if (data.length == 0){
				...
				ajaxJsonObjectByUrl("queryAssInRestOtherState.do?isCheck=false",{paraId:ass_in_no.substring(0,ass_in_no.length-1),state:2},function(responseData){
					if (responseData.state == "true") {
