...
<script type="text/javascript">
	...
	function add_open() {
		...
		$.ligerDialog.open({
			url : 'assCheckPlanOtherAddPage.do?isCheck=false&',
			data : {},
			height : 300,
			width : 450,
			title : '盘点任务单(其他固定资产)',
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveAssCheckPlanOther();
				},
	...
	function remove() {
		...
		/* 	var check_plan_no_all = [];
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteAssCheckPlanOther.do",  {ParamVo : ParamVo.toString()}, function(responseData) {
	...
	function audit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("OtherAudit.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unAudit() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消确认?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("OtherUnAudit.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function finish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定盘点完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("OtherFinish.do", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function unFinish() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定取消完成?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("OtherUnfinish.do?isCheck=false", formPara, function(
							responseData) {
						if (responseData.state == "true") {
							query();
						}
	...
	function generate() {
		...
		if (data.length == 0 || data.length > 1) {
			...
			$.ligerDialog.confirm('生成的盘点单将会覆盖原有数据?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("OtherGenerate.do", formPara, function(responseData) {
						if (responseData.state == "true") {
							query();
						}
					});
	...
	function openUpdate(obj) {
		...
		parent.$.ligerDialog.open({
			...
			width: $(window).width(),
			url: 'hrp/ass/assother/asscheck/plan/assCheckPlanOtherUpdatePage.do?isCheck=false&'+ parm,
	...
	function printDate() {
		...
		ajaxJsonObjectByUrl("queryAssCheckPlanOther.do?isCheck=false",
