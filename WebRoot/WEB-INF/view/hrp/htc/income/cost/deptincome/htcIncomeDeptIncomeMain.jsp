...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					usePager : true,
					delayLoad :true ,
					url : 'queryHtcIncomeDeptIncome.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function add_open(){
			$.ligerDialog.open({
				url : 'htcIncomeDeptIncomeAddPage.do?isCheck=false',
				height : 380,
				width : 500,
				title : '添加',
				modal : true,
	...
	function remove(){
			...
			if (data.length == 0) {
				...
				$.ligerDialog.confirm('确定删除?', function(yes) {
					if (yes) {
						ajaxJsonObjectByUrl("deleteHtcIncomeDeptIncome.do", {
	...
	function imp(){
		...
		var para={
			...
			importSpreadView("hrp/htc/income/cost/deptincome/impHtcIncomeDeptIncome.do?isCheck=false",para); 
		}
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url : 'htcIncomeDeptIncomeUpdatePage.do?isCheck=false&' + parm,
			data : {},
			height : 350,
			width : 500,
			title : '修改',
