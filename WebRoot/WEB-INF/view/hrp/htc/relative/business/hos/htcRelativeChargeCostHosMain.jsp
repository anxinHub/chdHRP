...
<script type="text/javascript">
    ...
    function loadHead(){
    	grid = $("#maingrid").ligerGrid({
    		columns: [{ display: '方案编码', name: 'plan_code', align: 'left'},
                      ...
                      { display: '直接成本', name: 'dir_cost', align: 'right',
							render: function(rowdata, rowindex, value) {
								return  formatNumber(rowdata.dir_cost, 2, 1);
							}
 					 },
                      { display: '公共分摊成本', name: 'pub_cost', align: 'right',
							render: function(rowdata, rowindex, value) {
								return  formatNumber(rowdata.pub_cost, 2, 1);
							}
 					 },
                      { display: '管理分摊成本', name: 'man_cost', align: 'right',
							render: function(rowdata, rowindex, value) {
								return  formatNumber(rowdata.man_cost, 2, 1);
							}
 					 },
                      { display: '医辅分摊成本', name: 'mea_cost', align: 'right',
							render: function(rowdata, rowindex, value) {
								return  formatNumber(rowdata.mea_cost, 2, 1);
							}
 					 }
                      ],
                     dataAction: 'server',dataType: 'server',usePager:true,url:'queryHtcRelativeChargeCostHos.do',
                     width: '100%', height: '100%', checkbox: true,rownumbers:true,delayLoad:true,
                     selectRowButtonOnly:true,//heightDiff: -10,
                     toolbar: { items: [
                     	{ text: '查询', id:'search', click: query,icon:'search' },
                     	{ line:true },
                     	{ text: '计算', id:'collect', click: collect,icon:'plus' },
    	                { line:true }
    				]}
                   });
    ...
    function  collect(){
        ...
        ajaxJsonObjectByUrl("addHtcRelativeChargeCostHos.do?isCheck=false",formPara,function(responseData){
