...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					usePager : true,
					delayLoad :true ,
					url : 'queryHtcRelativeDeptCost.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					rownumbers : true,
	...
	function add_open(){
			$.ligerDialog.open({
				url : 'htcRelativeDeptCostAddPage.do?isCheck=false',
				height : 350,
				width : 500,
				title : '添加',
				modal : true,
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcRelativeDeptCost.do", {
	...
	function openSelect(obj) {
		...
		//实际代码中&temp替换主键
		$.ligerDialog.open({
			url : 'htcRelativeDeptCostSelectPage.do?isCheck=false&' + parm,
			data : {},
			height : 470,
			width : 1000,
			title : '查询',
	 ...
	 function imp(){
			var para={
				...
				importSpreadView("hrp/htc/relative/cost/deptcost/impHtcRelativeDeptCost.do?isCheck=false",para); 
		}
	function loadDict() {
		//字典下拉框
