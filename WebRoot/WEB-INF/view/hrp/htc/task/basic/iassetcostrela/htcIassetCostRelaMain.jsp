...
<script type="text/javascript">
	...
	function add_open() {
		$.ligerDialog.open({
			url : 'htcIassetCostRelaAddPage.do?isCheck=false',
			height : 300,
			width : 400,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveIassetCostRela();
				},
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcIassetCostRela.do", {
	...
	function openUpdate(obj, obj1) {
		...
		$.ligerDialog.open({
			url : 'htcIassetCostRelaUpdatePage.do?isCheck=false&' + parm,
			data : {},
			height : 300,
			width : 400,
			title : '修改',
