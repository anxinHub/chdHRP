...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					columns : [
							{
								...
								render : function(rowdata, rowindex, value) {
									return "<a href='#' onclick=\"openUpdate('"
									        ...
									        + rowdata.hos_id + "|"
									        + rowdata.copy_code + "|"
											+ rowdata.work_type_code
											+ "');\" >"
											+ rowdata.work_type_code + "</a>";
								}
							}, {
								display : '作业分类名称',
								name : 'work_type_name',
					...
					onDblClickRow : function(rowdata, rowindex, value) {
						openUpdate(   rowdata.group_id + "|"
							        + rowdata.hos_id + "|"
							        + rowdata.copy_code + "|"
								    + rowdata.work_type_code);//实际代码中temp替换主键
					}
				});
	...
	function add_open() {
		$.ligerDialog.open({
			url : 'htcWorkTypeAddPage.do?isCheck=false',
			height : 350,
			width : 500,
			title : '添加',
			modal : true,
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcWorkType.do", {
	...
	function openUpdate(obj) {
		...
		var parm =  "group_id=" + vo[0] + 
		      ...
		      "&" + "work_type_code=" + vo[3];
		$.ligerDialog.open({
			url : 'htcWorkTypeUpdatePage.do?isCheck=false&'+ parm,
			data : {},
			height : 350,
			width : 500,
			title : '修改',
...
<body style="padding: 0px; overflow: hidden;">
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit">
		<tr>
			...
			<td align="left" class="l-table-edit-td"><input	name="work_type_code" type="text" id="work_type_code" ltype="text" /></td>
			<td align="left"></td>
			<td align="right" class="l-table-edit-td" style="padding-left: 20px;">是否末级：</td>
			<td align="left" class="l-table-edit-td"><input name="is_last" type="text" id="is_last" ltype="text" /></td>
			<td align="left"></td>
