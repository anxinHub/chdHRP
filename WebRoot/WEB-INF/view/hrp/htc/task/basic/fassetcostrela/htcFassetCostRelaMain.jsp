...
<script type="text/javascript">
	...
	function add_open() {
		$.ligerDialog.open({
			url : 'htcFassetCostRelaAddPage.do?isCheck=false',
			height : 250,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveFassetCostRela();
				},
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			var ParamVo = [];
			$(data).each(function() {
				ParamVo.push(this.asset_type_code);//实际代码中temp替换主键
			});
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcFassetCostRela.do", {
	...
	function openUpdate(obj) {
		...
		+ "&" + "acc_year=" + vo[3]+ "&" + "asset_type_code=" + vo[4];
		$.ligerDialog.open({
			url : 'htcFassetCostRelaUpdatePage.do?isCheck=false&' + parm,
			data : {},
			height : 300,
			width : 370,
			title : '修改',
