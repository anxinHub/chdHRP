...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryHtcMaterialTypeDict.do',
					width : '100%',
					height : '100%',
					checkbox : true,
					delayLoad:true,
	...
	function add_open(){
		$.ligerDialog.open({
			url: 'htcMaterialTypeDictAddPage.do?isCheck=false',
			height: 300,
			width: 400,
			title: '添加',
			modal: true,
		    ...
		    buttons: [{
		    	...
		    	onclick: function(item, dialog) {
		    		dialog.frame.saveMaterialTypeDict();
		    	},
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcMaterialTypeDict.do", {
	  ...
	  function synchro(){
			ajaxJsonObjectByUrl("synchroHtcMaterialTypeDict.do",null,function(responseData) {
				if (responseData.state == "true") {
					query()
				}
			});
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url : 'htcMaterialTypeDictUpdatePage.do?isCheck=false&'+parm,
			data : {},
			height :300,
			width : 400,
			title : '修改',
	...
	function loadDict(){
		...
		$("#supper_code").ligerTextBox({width:160});
