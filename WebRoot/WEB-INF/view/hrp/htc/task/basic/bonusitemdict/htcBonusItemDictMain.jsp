...
<script type="text/javascript">
	...
	function add_open() {
		$.ligerDialog.open({
			url : 'htcBonusItemDictAddPage.do?isCheck=false',
			height : 350,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveBonusItemDict();
				},
	...
	function remove() {
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcBonusItemDict.do", {
	...
	function openUpdate(obj) {
		...
		//实际代码中&temp替换主键
		$.ligerDialog.open({
					url : 'htcBonusItemDictUpdatePage.do?isCheck=false&' + parm,
					data : {},
					height : 350,
					width : 500,
					title : '修改',
