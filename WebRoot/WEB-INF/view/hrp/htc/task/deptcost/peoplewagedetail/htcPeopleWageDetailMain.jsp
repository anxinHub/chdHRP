...
<script type="text/javascript">
	...
	function query() {
		...
		grid.options.parms.push({ name: 'wage_clum', value: wage_clum});
	...
	function loadHead() {
		...
		var wage_item_clum_columns="";
    	ajaxJsonObjectByUrl("queryHtcPeopleWageItemClumHead.do?isCheck=false", "", function (responseData){
    		if(responseData.Rows.length > 0){
    			$.each(responseData.Rows, function(v_index, v_data){ 
    				wage_item_clum_columns="";
    				wage_item_clum_columns="{"
        				      +"display: '"+v_data.wage_item_name+"',"
    				          ...
    				          +",}"; 
    				wage_clum += ", "+v_data.clum_code+" as " + v_data.clum_code;
    				...
    				columns.push(eval("("+wage_item_clum_columns+")"));
    			}); 
			}
    	}, false);
		...
		grid = $("#maingrid").ligerGrid({
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcPeopleWageDetail.do',
			width : '100%',
			height : '100%',
			checkbox : true,
			rownumbers : true,
	...
	function add_open(){
		$.ligerDialog.open({
			url : 'htcPeopleWageDetailAddPage.do?isCheck=false',
			height : 350,
			width : 400,
			title : '添加',
			modal : true,
	...
	function remove(){
        ...
        if (data.length == 0){
            ...
            $.ligerDialog.confirm('确定删除?', function (yes){
            	if(yes){
                	ajaxJsonObjectByUrl("deleteHtcPeopleWageDetail.do",{ParamVo : ParamVo.toString()},function (responseData){
	...
	function f_onAfterEdit(e){
		...
		if(is_DataCanSave){
		   ...
		   $(len).each(function(index, value) {
             ...
             ajaxJsonObjectByUrl("updateHtcPeopleWageDetailItem.do?isCheck=false", formPara, function(responseData) {
                 /* if (responseData.state == "true") {
                 	query();
                 } */
             });
