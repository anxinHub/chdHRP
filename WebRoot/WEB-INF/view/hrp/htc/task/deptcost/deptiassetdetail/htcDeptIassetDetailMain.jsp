...
<script type="text/javascript">
		...
		grid = $("#maingrid").ligerGrid(
				{
					...
					dataType : 'server',
					usePager : true,
					url : 'queryHtcDeptIassetDetail.do',
					width : '100%',
					height : '100%',
					delayLoad : true,
					checkbox : true,
	...
	function add_open(){
		$.ligerDialog.open({
			url : 'htcDeptIassetDetailAddPage.do?isCheck=false',
			height : 400,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveDeptIassetDetail();
				},
			...
			$(data).each(
					function() {
						ParamVo.push(this.group_id + "@"
								...
								+ this.dept_no + "@"
								+ this.dept_id + "@"
								+ this.asset_code);//实际代码中temp替换主键);//实际代码中temp替换主键
					});
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcDeptIassetDetail.do",
							{
	...
	function openUpdate(obj) {
		...
		var parm =  "group_id=" + vo[0] + 
			  ...
			  "&" + "asset_code=" + vo[7]
		$.ligerDialog.open({
			url : 'htcDeptIassetDetailUpdatePage.do?isCheck=false&'+parm,
			data : {},
			height : 430,
			width : 500,
			title : '修改',
