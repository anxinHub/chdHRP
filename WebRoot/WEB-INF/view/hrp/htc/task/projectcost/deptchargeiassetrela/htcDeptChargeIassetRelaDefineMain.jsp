...
<script type="text/javascript">
	...
	function queryIasset(parm) {
		grid_iasset.options.parms = [];
		grid_iasset.options.newPage = 1;
		//根据表字段进行添加查询条件
		grid_iasset.options.parms.push({
			name : 'acc_year',
			value : parm.acc_year
		});
		grid_iasset.options.parms.push({
		...
		//加载查询条件
		grid_iasset.loadData(grid_iasset.where);
	}
	...
	function loadHeadCharge(){ 
		grid_charge = $("#maingrid_charge").ligerGrid({
			...
			usePager : true,
			checkbox : 'checkbox',
			url : 'queryHtcDeptChargeIassetRelaCharge.do?isCheck=false',
			width : '100%',
			height : '100%',
			delayLoad : true,
	...
	function loadHeadIasset(){
		grid_iasset = $("#maingrid_iasset").ligerGrid({
			columns : [ {
				display : '卡片号',
				name : 'asset_code',
				align : 'left'
			...
			dataType : 'server',
			usePager : true,
			url : 'queryHtcDeptChargeIassetRelaIasset.do?isCheck=false',
			width : '100%',
			height : '100%',
			checkbox : 'checkbox',
			delayLoad : true,
		...
		gridManager_fasset = $("#maingrid_iasset").ligerGetGridManager();
    }
	 ...
	 function f_onAfterEdit(rowdata, rowid, rowobj){
   		...
   		ajaxJsonObjectByUrl("saveHtcDeptChargeIassetRela.do", formPara, function(responseData) {
...
<body style="padding: 6px; overflow: hidden; width: 100%; height: 100%;">
    ...
    <div id="layout">
        ...
        <div position="right" title="无形资产信息" id="maingrid_iasset"></div>  
    </div> 
</body>
</html>
