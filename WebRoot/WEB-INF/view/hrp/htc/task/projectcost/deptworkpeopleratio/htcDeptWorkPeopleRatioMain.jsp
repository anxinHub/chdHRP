...
<script type="text/javascript">
	...
	function query() {
		...
		grid.options.parms.push({
			name:'work_code',
			value:liger.get("work_code").getValue()
		}); 
		//加载查询条件
				...
				.ligerGrid(
						{
							...
							dataType : 'server',
							usePager : true,
							url : 'queryHtcDeptWorkPeopleRatio.do',
							width : '100%',
							height : '100%',
							checkbox : true,
							rownumbers : true,
	...
	function add_open(){
		$.ligerDialog.open({
			url : 'htcDeptWorkPeopleRatioAddPage.do?isCheck=false',
			height : 300,
			width : 500,
			title : '添加',
			modal : true,
			...
			buttons : [ {
				...
				onclick : function(item, dialog) {
					dialog.frame.saveDeptWorkPeopleRatio();
				},
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl(
							"deleteHtcDeptWorkPeopleRatio.do", {
	...
	function openUpdate(obj) {
		...
		var parm =       "group_id="  + vo[0] 
		         ...
		         + "&" + "work_code=" + vo[8]
		$.ligerDialog.open({
			url : 'htcDeptWorkPeopleRatioUpdatePage.do?isCheck=false&' + parm,
			data : {},
			height : 300,
			width : 500,
			title : '修改',
	...
	<table cellpadding="0" cellspacing="0" class="l-table-edit">
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td"><input class="Wdate" name="acc_year" type="text" id="acc_year" ltype="text"
				onFocus="WdatePicker({isShowClear:false,readOnly:true,dateFmt:'yyyy'})" tyle="width: 160px;" /></td>
			<td align="left"></td>
		...
		<tr>
			...
			<td align="left" class="l-table-edit-td"><input name="work_code" type="text" id="work_code" ltype="text" /></td>
			<td align="left"></td>
		</tr>
	</table>
	<div id="maingrid"></div>
