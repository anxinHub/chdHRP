...
<script type="text/javascript">
	...
	function add_open(){
		$.ligerDialog.open({
			url : 'htcDeptWorkRelaAddPage.do?isCheck=false',
			height : 350,
			width : 450,
			title : '添加',
			modal : true,
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcDeptWorkRela.do", {
	...
	function openUpdate(obj) {
		...
		$.ligerDialog.open({
			url : 'htcDeptWorkRelaUpdatePage.do?isCheck=false' + parm,
			data : {},
			height : 350,
			width : 500,
			title : '修改',
