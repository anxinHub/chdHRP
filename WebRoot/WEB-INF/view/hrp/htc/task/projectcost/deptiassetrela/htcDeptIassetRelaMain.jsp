...
<script type="text/javascript">
	...
	function add_open(){
		$.ligerDialog.open({
			url : 'htcDeptIassetRelaAddPage.do?isCheck=false',
			height : 350,
			width : 500,
			title : '添加',
			modal : true,
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcDeptIassetRela.do", {
	...
	function openUpdate(obj) {
		...
		var parm =       "group_id="  + vo[0] 
		         ...
		         + "&" + "asset_code=" + vo[7]
		$.ligerDialog.open({
			url : 'htcDeptIassetRelaUpdatePage.do?isCheck=false&'+parm,
			height : 350,
			width : 500,
			title : '修改',
			modal : true,
