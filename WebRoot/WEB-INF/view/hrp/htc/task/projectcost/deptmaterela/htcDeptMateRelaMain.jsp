...
<script type="text/javascript">
	...
	function query() {
		...
		grid.options.parms.push({
			name:'mate_code',
			value:liger.get("mate_code").getValue()
		}); 
		//加载查询条件
	...
	function add_open(){
		$.ligerDialog.open({
			url : 'htcDeptMateRelaAddPage.do?isCheck=false',
			height : 350,
			width : 500,
			title : '添加',
			modal : true,
	...
	function remove(){
		...
		if (data.length == 0) {
			...
			$.ligerDialog.confirm('确定删除?', function(yes) {
				if (yes) {
					ajaxJsonObjectByUrl("deleteHtcDeptMateRela.do", {
	...
	function openUpdate(obj) {
		...
		var parm =       "group_id="  + vo[0] 
		         ...
		         + "&" + "mate_code=" + vo[7]
		$.ligerDialog.open({
			url : 'htcDeptMateRelaUpdatePage.do?isCheck=false&'+parm,
			data : {},
			height : 350,
			width : 500,
			title : '修改',
