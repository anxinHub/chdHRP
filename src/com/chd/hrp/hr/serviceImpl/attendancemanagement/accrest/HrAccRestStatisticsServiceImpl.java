...
@Service("hrAccRestStatisticsService")
public class HrAccRestStatisticsServiceImpl  implements HrAccRestStatisticsService{
	private static Logger logger = Logger.getLogger(HrAccRestStatisticsServiceImpl.class);
	@Resource(name = "hrAccRestStatisticsMapper")
	private final HrAccRestStatisticsMapper hrAccRestStatisticsMapper = null;

	/**
	 * 查询
	 */
	@Override
	public String queryAccRestStatistics(Map<String, Object> entityMap) throws DataAccessException {
		...
		//查询考勤周期设置
		Map<String,Object> cycMap = hrAccRestStatisticsMapper.queryHrCycDate(entityMap);
	...
	/*	//期初天数的取值方式
		...
		if (sysPage.getTotal() == -1) {
			List<Map<String, Object>> list = ChdJson.toListLower((List<Map<String, Object>>) hrAccRestStatisticsMapper.query(entityMap));
			return ChdJson.toJson(list);
		} else {
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<Map<String, Object>> list = ChdJson.toListLower((List<Map<String, Object>>) hrAccRestStatisticsMapper.query(entityMap, rowBounds));
	 ...
	 */
	@Override
	public String updateAccRestStatistics(Map<String, Object> entityMap) throws DataAccessException {
		try{
			...
			//查看当月存在的员工数据
			List<Map<String,Object>> existsList = ChdJson.toListLower(hrAccRestStatisticsMapper.queryAccYearMonthExists(entityMap));
			...
			if(saveList.size()>0){
				hrAccRestStatisticsMapper.addBatch(saveList);
			}
			if(updateList.size()>0){
				hrAccRestStatisticsMapper.updateBatch(updateList);
			}
	 ...
	 */
	@Override
	public List<Map<String, Object>> queryAccRestStatisticsPrint(Map<String, Object> entityMap) throws DataAccessException {
		...
		return (List<Map<String, Object>>) hrAccRestStatisticsMapper.queryByPrint(entityMap);
	}
