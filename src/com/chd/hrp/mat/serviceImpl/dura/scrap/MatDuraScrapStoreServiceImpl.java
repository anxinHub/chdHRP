...
import com.chd.hrp.mat.dao.dura.scrap.MatDuraScrapStoreMapper;
 ...
 * @Version: 1.0
 */ 
@Service("matDuraScrapStoreService")
public class MatDuraScrapStoreServiceImpl implements MatDuraScrapStoreService {

	private static Logger logger = Logger.getLogger(MatDuraScrapStoreServiceImpl.class);
	//引入DAO操作
	@Resource(name = "matDuraScrapStoreMapper")
	private final MatDuraScrapStoreMapper matDuraScrapStoreMapper = null;
	...
	*/
	...
	public String add(Map<String,Object> entityMap)throws DataAccessException{
		try {
			...
			//取出主表ID（自增序列）
			entityMap.put("scrap_id", matDuraScrapStoreMapper.queryMainSeq());
			...
			if(entityMap.get("detailData") != null && !"[]".equals(String.valueOf(entityMap.get("detailData")))){
				...
				while (it.hasNext()) {
					...
					if( !"".equals(jsonObj.get("inv_id")) && jsonObj.get("inv_id") != null){
						...
						detailMap.put("detail_id", matDuraScrapStoreMapper.queryDetailSeq());  //明细表ID
				...
				//新增入库主表数据
				matDuraScrapStoreMapper.addMain(entityMap);
				//新增入库明细数据
				matDuraScrapStoreMapper.addDetail(detailList);
			}else{
				return "{\"error\":\"明细数据为空，请选择材料\",\"state\":\"false\"}";
			}
		}
	...
	*/
	...
	public String update(Map<String,Object> entityMap)throws DataAccessException{
		...
		try {
			...
			int flag = matDuraScrapStoreMapper.existsState(entityMap);
			if(flag > 0){
			...
			if(entityMap.get("detailData") != null && !"[]".equals(String.valueOf(entityMap.get("detailData")))){
				...
				while (it.hasNext()) {
					...
					if( !"".equals(jsonObj.get("inv_id")) && jsonObj.get("inv_id") != null){
						...
						if(jsonObj.get("detail_id") == null || "".equals(jsonObj.get("detail_id"))){
							//获取明细ID
							detailMap.put("detail_id", String.valueOf(matDuraScrapStoreMapper.queryDetailSeq()));  //明细表ID
							detailList.add(detailMap);
						}else{
				...
				//修改入库主表数据
				matDuraScrapStoreMapper.updateMain(entityMap);
				
				//删除明细记录重新添加
				matDuraScrapStoreMapper.deleteDetail(entityMap);
				matDuraScrapStoreMapper.addDetail(detailList);
	...
	*/
	...
	public String deleteBatch(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {	
			//状态不为新建不能删除
			int flag = matDuraScrapStoreMapper.existsStateBatch(entityList);
			if(flag > 0){
				return "{\"error\":\"所选单据含有状态不是未审核的单据，请重新选择！\",\"state\":\"true\"}";
			}
				
			//批量删除明细表
			matDuraScrapStoreMapper.deleteDetailBatch(entityList);
			//批量删除主表
			matDuraScrapStoreMapper.deleteMainBatch(entityList);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public String auditOrUnAuditBatch(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {	
			...
			//批量审核
			matDuraScrapStoreMapper.auditOrUnAuditBatch(entityList);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public synchronized String confirmBatch(List<Map<String, Object>> entityList)throws DataAccessException{
		...
		try {	
			...
			//获取所选单据明细
			List<Map<String, Object>> detailList = matDuraScrapStoreMapper.queryDetailListForConfirm(entityList);
			...
			//修改单据状态
			matDuraScrapStoreMapper.confirmBatch(entityList);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public String query(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			...
			List<?> list = matDuraScrapStoreMapper.query(entityMap);
			
			return ChdJson.toJson(list);
		}else{
			
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<?> list = matDuraScrapStoreMapper.query(entityMap, rowBounds);
	...
	*/
	...
	public <T> T queryMainByCode(Map<String,Object> entityMap)throws DataAccessException{
		...
		return matDuraScrapStoreMapper.queryMainByCode(entityMap);
	}
	...
	*/
	...
	public String queryDetailByCode(Map<String,Object> entityMap)throws DataAccessException{
		...
		List<?> list = matDuraScrapStoreMapper.queryDetailByCode(entityMap);
	...
	*/
	...
	public Map<String,Object> queryDataByPrintTemlate(Map<String,Object> entityMap) throws DataAccessException{
		try {
			...
			MatDuraScrapStoreMapper matDuraScrapStoreMapper = (MatDuraScrapStoreMapper)context.getBean("matDuraScrapStoreMapper");
			if("1".equals(String.valueOf(entityMap.get("p_num")))){ 
				List<Map<String,Object>> map=matDuraScrapStoreMapper.queryMainForPrintTemlateBatch(entityMap);
				//查询明细表
				List<Map<String,Object>> list=matDuraScrapStoreMapper.queryDetailForPrintTemlate(entityMap);
				reMap.put("main", map);
				reMap.put("detail", list);
			}else{
				Map<String,Object> map=matDuraScrapStoreMapper.queryMainForPrintTemlate(entityMap);
				//查询明细表
				List<Map<String,Object>> list=matDuraScrapStoreMapper.queryDetailForPrintTemlate(entityMap);
