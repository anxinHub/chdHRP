 ...
 * @Version: 1.0
 */ 
@Service("matDuraTranStoreStoreService")
public class MatDuraTranStoreStoreServiceImpl implements MatDuraTranStoreStoreService {

	private static Logger logger = Logger.getLogger(MatDuraTranStoreStoreServiceImpl.class);
	//引入DAO操作
	@Resource(name = "matDuraTranStoreStoreMapper")
	private final MatDuraTranStoreStoreMapper matDuraTranStoreStoreMapper = null;
	...
	public String add(Map<String,Object> entityMap)throws DataAccessException{
		try {
			...
			if("自动生成".equals(entityMap.get("dura_no"))){
				entityMap.put("table_code", "MAT_DURA_STORE_STORE");
			...
			//取出主表ID（自增序列）
			entityMap.put("dura_id", matDuraTranStoreStoreMapper.queryMainSeq());
			...
			if(entityMap.get("detailData") != null && !"[]".equals(String.valueOf(entityMap.get("detailData")))){
				...
				while (it.hasNext()) {
					...
					if( !"".equals(jsonObj.get("inv_id")) && jsonObj.get("inv_id") != null){
						...
						detailMap.put("detail_id", matDuraTranStoreStoreMapper.queryDetailSeq());  //明细表ID
				...
				//新增入库主表数据
				matDuraTranStoreStoreMapper.addMain(entityMap);
				//新增入库明细数据
				matDuraTranStoreStoreMapper.addDetail(detailList);
			}else{
				return "{\"error\":\"明细数据为空，请选择材料\",\"state\":\"false\"}";
			}
		}
	...
	*/
	...
	public String update(Map<String,Object> entityMap)throws DataAccessException{
		...
		try {
			...
			int flag = matDuraTranStoreStoreMapper.existsState(entityMap);
			if(flag > 0){
			...
			if(entityMap.get("detailData") != null && !"[]".equals(String.valueOf(entityMap.get("detailData")))){
				...
				while (it.hasNext()) {
					...
					if( !"".equals(jsonObj.get("inv_id")) && jsonObj.get("inv_id") != null){
						...
						if(jsonObj.get("detail_id") == null || "".equals(jsonObj.get("detail_id"))){
							//获取明细ID
							detailMap.put("detail_id", String.valueOf(matDuraTranStoreStoreMapper.queryDetailSeq()));  //明细表ID
							detailList.add(detailMap);
						}else{
				...
				//修改入库主表数据
				matDuraTranStoreStoreMapper.updateMain(entityMap);
				
				//删除明细记录重新添加
				matDuraTranStoreStoreMapper.deleteDetail(entityMap);
				matDuraTranStoreStoreMapper.addDetail(detailList);
	...
	*/
	...
	public String deleteBatch(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {	
			//状态不为新建不能删除
			int flag = matDuraTranStoreStoreMapper.existsStateBatch(entityList);
			if(flag > 0){
				return "{\"error\":\"所选单据含有状态不是未审核的单据，请重新选择！\",\"state\":\"true\"}";
			}
				
			//批量删除明细表
			matDuraTranStoreStoreMapper.deleteDetailBatch(entityList);
			//批量删除主表
			matDuraTranStoreStoreMapper.deleteMainBatch(entityList);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public String auditOrUnAuditBatch(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {	
			...
			//批量审核
			matDuraTranStoreStoreMapper.auditOrUnAuditBatch(entityList);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public synchronized String confirmBatch(List<Map<String, Object>> entityList)throws DataAccessException{
		...
		try {	
			...
			//获取所选单据明细
			List<Map<String, Object>> detailList = matDuraTranStoreStoreMapper.queryDetailListForConfirm(entityList);
			...
			for(Map<String, Object> detailMap : detailList){
				...
				searchMap.put("store_id", detailMap.get("store_id"));  //先处理移出库房
				...
				//查询移入库房信息
				searchMap.put("store_id", detailMap.get("in_store_id"));  //处理移入库房
				
				barKey = detailMap.get("in_store_id").toString() + detailMap.get("inv_id");
				balanceKey = barKey + detailMap.get("price").toString() + detailMap.get("bar_code").toString();
				...
				if(map == null || map.size() == 0){
					...
					map.put("store_id", detailMap.get("in_store_id")); 
			...
			if(addBalanceList.size() > 0){
				matDuraStoreBalanceMapper.addBatch(addBalanceList);
			}
			if(updateBalanceList.size() > 0){
				matDuraStoreBalanceMapper.updateBatch(updateBalanceList);
			}
			
			//修改单据状态
			matDuraTranStoreStoreMapper.confirmBatch(entityList);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public String query(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			...
			List<?> list = matDuraTranStoreStoreMapper.query(entityMap);
			
			return ChdJson.toJson(list);
		}else{
			
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<?> list = matDuraTranStoreStoreMapper.query(entityMap, rowBounds);
	...
	*/
	...
	public <T> T queryMainByCode(Map<String,Object> entityMap)throws DataAccessException{
		...
		return matDuraTranStoreStoreMapper.queryMainByCode(entityMap);
	}
	...
	*/
	...
	public String queryDetailByCode(Map<String,Object> entityMap)throws DataAccessException{
		...
		List<?> list = matDuraTranStoreStoreMapper.queryDetailByCode(entityMap);
	...
	*/
	...
	public Map<String,Object> queryDataByPrintTemlate(Map<String,Object> entityMap) throws DataAccessException{
		try {
			...
			MatDuraTranStoreStoreMapper matDuraTranStoreStoreMapper = (MatDuraTranStoreStoreMapper)context.getBean("matDuraTranStoreStoreMapper");
			if("1".equals(String.valueOf(entityMap.get("p_num")))){ 
				List<Map<String,Object>> map=matDuraTranStoreStoreMapper.queryMainForPrintTemlateBatch(entityMap);
				//查询明细表
				List<Map<String,Object>> list=matDuraTranStoreStoreMapper.queryDetailForPrintTemlate(entityMap);
				reMap.put("main", map);
				reMap.put("detail", list);
			}else{
				Map<String,Object> map=matDuraTranStoreStoreMapper.queryMainForPrintTemlate(entityMap);
				//查询明细表
				List<Map<String,Object>> list=matDuraTranStoreStoreMapper.queryDetailForPrintTemlate(entityMap);
