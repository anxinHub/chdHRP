...
import com.chd.hrp.mat.dao.dura.init.MatDuraInitDeptMapper;
 ...
 * @Version: 1.0
 */
@Service("matDuraInitDeptService")
public class MatDuraInitDeptServiceImpl implements MatDuraInitDeptService {

	private static Logger logger = Logger.getLogger(MatDuraInitDeptServiceImpl.class);
	//引入DAO操作
	@Resource(name = "matDuraInitDeptMapper")
	private final MatDuraInitDeptMapper matDuraInitDeptMapper = null;
	...
	*/
	...
	public String query(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			List<?> list = matDuraInitDeptMapper.query(entityMap);
			
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<?> list = matDuraInitDeptMapper.query(entityMap, rowBounds);
	...
	public String save(Map<String,Object> entityMap)throws DataAccessException{
		try {
			...
			if(entityMap.get("dept_id") == null || "".equals(entityMap.get("dept_id"))){
				return "{\"error\":\"科室不能为空\",\"state\":\"false\"}";
			}
			//材料ID
			...
			if(flag == 0){
				return "{\"error\":\"科室已记账.\",\"state\":\"false\"}";
			}
				
			if(entityMap.get("reg_id") != null && !"".equals(entityMap.get("reg_id").toString())){
				//修改数据
				matDuraInitDeptMapper.update(entityMap);
			}else{
				//取自增序列
				entityMap.put("reg_id", matDuraInitDeptMapper.querySeq());
				//新增数据
				matDuraInitDeptMapper.add(entityMap);
			}
		}
		catch (Exception e) {
	...
	*/
	...
	public String deleteBatch(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {	
			...
			int state =  matDuraInitChargeMapper.existsDeptAccount(entityMap);
			if(state == 0){
				return "{\"error\":\"科室已记账.\",\"state\":\"false\"}";
			}
			
			//判断状态是否已审核（审核后不能删除修改）
			state = matDuraInitDeptMapper.existsState(entityList);
			if(state > 0){
				return "{\"error\":\"期初已审核不能删除\"}";
			}
			//批量表数据
			matDuraInitDeptMapper.deleteBatch(entityList);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public String auditOrUnAudit(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {	
			...
			//批量审核
			matDuraInitDeptMapper.auditOrUnAudit(entityList);
	...
	*/
	...
	public String importData(Map<String, Object> entityMap)throws DataAccessException{
		...
		try {	
			...
			//耐用品材料列表
			List<Map<String, Object>> invList = matDuraInitDeptMapper.queryInvListForImport(seachMap);
			...
			//库房列表含记账标识state
			List<Map<String, Object>> deptList = matDuraInitDeptMapper.queryDeptListForImport(seachMap);
			Map<String, Map<String, Object>> deptMap = new HashMap<String, Map<String, Object>>();
			for(Map<String, Object> dept : deptList){
				if(dept.get("DEPT_CODE") != null){
					deptMap.put(dept.get("DEPT_CODE").toString(), dept);
				}
			}
			...
			for(Map<String, List<String>> dataMap : dataList){
				...
				/**校验科室**************begin*****************/
				rowList = dataMap.get("科室编码");
				if(rowList == null || rowList.get(1) == null || "".equals(rowList.get(1))){
					return "{\"warn\":\"" + rowList.get(0) + "，科室编码为空！\",\"state\":\"false\",\"row_cell\":\"" + rowList.get(0) + "\"}";
				}
				dept_code = rowList.get(1);
				Map<String, Object> v_dept = deptMap.get(dept_code);
				if(v_dept == null){
					return "{\"warn\":\"" + rowList.get(0) + "，科室编码不存在！\",\"state\":\"false\",\"row_cell\":\"" + rowList.get(0) + "\"}";
				}
				//判断科室是否记账
				if(v_dept.get("state") != null && !"0".equals(v_dept.get("state"))){
					return "{\"warn\":\"" + v_dept.get("DEPT_NAME") + "已记账不能导入！\",\"state\":\"false\",\"row_cell\":\"" + rowList.get(0) + "\"}";
				}
				//验证通过存放库房信息
				map.put("dept_id", v_dept.get("DEPT_ID"));
				map.put("dept_no", v_dept.get("DEPT_NO"));
				/**校验科室**************end*******************/
				...
				map.put("reg_id", matDuraInitDeptMapper.querySeq());
				
				duraList.add(map);
			}
			
			//批量添加
			matDuraInitDeptMapper.addBatch(duraList);
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
