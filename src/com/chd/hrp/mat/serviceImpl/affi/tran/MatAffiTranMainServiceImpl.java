...
package com.chd.hrp.mat.serviceImpl.affi.tran; 
...
import com.chd.hrp.mat.dao.affi.tran.MatAffiTranDetailMapper;
import com.chd.hrp.mat.dao.affi.tran.MatAffiTranMainMapper;
...
import com.chd.hrp.mat.entity.MatAffiTranRela;
...
@Service("matAffiTranMainService")
public class MatAffiTranMainServiceImpl implements MatAffiTranMainService{

	private static Logger logger = Logger.getLogger(MatAffiTranMainServiceImpl.class);
	//引入DAO操作
	@Resource(name = "matAffiTranMainMapper")
	private final MatAffiTranMainMapper matAffiTranMainMapper = null;
	
	@Resource(name = "matAffiTranDetailMapper")
	private final MatAffiTranDetailMapper matAffiTranDetailMapper = null;
	...
	*/
	...
	public String add(Map<String,Object> entityMap)throws DataAccessException{
		...
		try {
			...
			Long tran_id = matAffiTranMainMapper.queryMatAffiTranMainSeq();// 查询序列
			...
			long out_id = matAffiOutMapper.queryMatAffiOutMainSeq();
					...
					if(validateJSON(String.valueOf(jsonObj.get("sum_amount"))) && validateJSON(String.valueOf(jsonObj.get("amount"))) && validateJSON(String.valueOf(jsonObj.get("inv_detail_data"))) && String.valueOf(jsonObj.get("sum_amount")).equals(String.valueOf(jsonObj.get("amount")))){
						...
						for(Map<String, Object> m : JsonListMapUtil.getListMap(jsonObj.get("inv_detail_data").toString())){
							...
							if(m.get("inv_id") != null && !"".equals(String.valueOf(m.get("inv_id")))){
								...
								detailMap.put("tran_detail_id", matAffiTranDetailMapper.queryMatAffiTranDetailSeq());
								...
								outDetailMap.put("detail_id", matAffiOutMapper.queryMatAffiOutDetailSeq());
			...
			if(tran_detail_batch.size() > 0){
				//新增调拨单主表
				matAffiTranMainMapper.add(entityMap);
				matAffiTranDetailMapper.addBatch(tran_detail_batch);
				//新增出库单主表
				matAffiOutMapper.addMatAffiOutMain(outMap);
				//新增出库单明细表
				matAffiOutMapper.addMatAffiOutDetail(outDetailList);
				//新增入库单主表
				if(is_in){
...
调拨类型：1 院内调拨  2 院外调拨<BR> 
	...
	*/
	...
	public String addBatch(List<Map<String,Object>> entityList)throws DataAccessException{		
		try {			
			matAffiTranMainMapper.addBatch(entityList);			
			return "{\"msg\":\"添加成功.\",\"state\":\"true\"}";
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
			throw new SysException("{\"error\":\"操作失败\"}");
	...
	*/
	...
	public String update(Map<String,Object> entityMap)throws DataAccessException{		
		try {
			...
			boolean is_in = false;
			//获取调拨单与出入库单对应关系
			MatAffiTranRela matTranRela = matAffiTranRelaMapper.queryByCode(entityMap);
			...
			if(tran_detail_batch.size() > 0){
				//修改调拨单主表
				matAffiTranMainMapper.update(entityMap);
				//修改调拨单明细表先清空再添加
				matAffiTranDetailMapper.delete(entityMap);
				matAffiTranDetailMapper.addBatch(tran_detail_batch);
			}
			
			if(outDetailList.size() > 0){
				//修改出库单主表
				matAffiOutMapper.updateMatAffiOutMain(outMap);
				//修改出库单明细表先清空再添加
				matAffiOutMapper.deleteMatAffiOutDetail(outMap);
				matAffiOutMapper.addMatAffiOutDetail(outDetailList);
			}
			
			if(inDetailList.size()>0){
				//修改入库单主表
				matAffiInMapper.updateMatAffiInMain(inMap);
				//修改入库单明细表先清空再添加
				matAffiInMapper.deleteMatAffiInDetail(inMap);
				matAffiInMapper.addMatAffiInDetail(inDetailList);
			}
...
调拨类型：1 院内调拨  2 院外调拨<BR> 
	...
	*/
	...
	public String updateBatch(List<Map<String,Object>> entityList)throws DataAccessException{		
		try {			
			matAffiTranMainMapper.updateBatch(entityList);			
			return "{\"msg\":\"更新成功.\",\"state\":\"true\"}";
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
			throw new SysException("{\"error\":\"操作失败\"}");
...
调拨类型：1 院内调拨  2 院外调拨<BR> 
	...
	*/
    ...
    public String delete(Map<String, Object> entityMap) throws DataAccessException {    	
		try {			
			int state = matAffiTranMainMapper.delete(entityMap);			
	...
	*/
	...
	public String deleteBatch(List<Map<String,Object>> entityList)throws DataAccessException{		
		try {
			...
			//获取调拨单与出入库单对应关系
			List<Map<String, Object>> relaList = toListMapLower(matAffiTranRelaMapper.queryListForDelete(entityList));
			if(relaList.size() > 0){
				...
				if(delete_out.size()>0){
					//删除已经生成的出库单
					matAffiOutMapper.deleteBatchMatAffiOutDetail(delete_out);//明细表
					matAffiOutMapper.deleteBatchMatAffiOutMain(delete_out);//主表
				}
				if(delete_in.size()>0){
					//删除已经生成的入库单
					matAffiInMapper.deleteBatchMatAffiInDetail(delete_in);//明细表
					matAffiInMapper.deleteBatchMatAffiInMain(delete_in);//主表
				}
				
				//删除对应关系表
				matAffiTranRelaMapper.deleteBatch(entityList);
			}
			if(entityList.size() > 0){
				//删除明细
				matAffiTranDetailMapper.deleteBatch(entityList);
				//删除主表
				matAffiTranMainMapper.deleteBatch(entityList);
			}
	...
	*/
	...
	public String addOrUpdate(Map<String,Object> entityMap)throws DataAccessException{
		...
		*/
		...
		List<MatAffiTranMain> list = (List<MatAffiTranMain>)matAffiTranMainMapper.queryExists(mapVo);		
		if (list.size()>0) {
			int state = matAffiTranMainMapper.update(entityMap);		
			return "{\"msg\":\"更新成功.\",\"state\":\"true\"}";
		}
		
		try {			
			int state = matAffiTranMainMapper.add(entityMap);		
			return "{\"msg\":\"添加成功.\",\"state\":\"true\"}";
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
			throw new SysException("{\"error\":\"操作失败\"}");
	...
	*/
	...
	public String query(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){			
			List<MatAffiTranMain> list = (List<MatAffiTranMain>)matAffiTranMainMapper.query(entityMap);			
			return ChdJson.toJson(list);			
		}else{			
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());			
			List<MatAffiTranMain> list = (List<MatAffiTranMain>)matAffiTranMainMapper.query(entityMap, rowBounds);			
...
调拨类型：1 院内调拨  2 院外调拨<BR> 
	...
	*/
	...
	public <T> T queryByCode(Map<String,Object> entityMap)throws DataAccessException{
		return matAffiTranMainMapper.queryByCode(entityMap);
	}
...
调拨类型：1 院内调拨  2 院外调拨<BR> 
	 * @param  entityMap<BR>
	 *  参数为要检索的字段
	 * @return MatAffiTranMain
	 * @throws DataAccessException
	*/
	@Override
	public <T> T queryByUniqueness(Map<String,Object> entityMap)throws DataAccessException{
		return matAffiTranMainMapper.queryByUniqueness(entityMap);
	}
...
调拨类型：1 院内调拨  2 院外调拨<BR> 
	 * @param  entityMap<BR>
	 *  参数为要检索的字段
	 * @return List<MatAffiTranMain>
	 * @throws DataAccessException
	*/
	@Override
	public List<?> queryExists(Map<String,Object> entityMap)throws DataAccessException{
		return matAffiTranMainMapper.queryExists(entityMap);
	}
	/**
	 * 代销调拨--主页面查询
	 */
	...
	public String queryMatAffiTranMain(Map<String, Object> entityMap) throws DataAccessException {
		...
		sysPage = (SysPage) entityMap.get("sysPage");		
		if (sysPage.getTotal()==-1){			
			List<Map<String, Object>> list = (List<Map<String, Object>>)matAffiTranMainMapper.queryMatAffiTranMain(entityMap);			
			return ChdJson.toJsonLower(list);
		}else{			
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());			
			List<Map<String, Object>> list = (List<Map<String, Object>>)matAffiTranMainMapper.queryMatAffiTranMain(entityMap, rowBounds);			
			PageInfo page = new PageInfo(list);			
	...
	public String queryMatInMainBySingle(Map<String, Object> entityMap) throws DataAccessException {
		...
		sysPage = (SysPage) entityMap.get("sysPage");		
		if (sysPage.getTotal()==-1){			
			List<Map<String, Object>> list = (List<Map<String, Object>>)matAffiTranMainMapper.queryMatInMainBySingle(entityMap);			
			return ChdJson.toJsonLower(list);			
		}else{			
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());			
			List<Map<String, Object>> list = (List<Map<String, Object>>)matAffiTranMainMapper.queryMatInMainBySingle(entityMap, rowBounds);			
			PageInfo page = new PageInfo(list);			
			return ChdJson.toJsonLower(list, page.getTotal());			
		}
	}
	...
	public String queryMatInDetailBySingle(Map<String, Object> entityMap) throws DataAccessException {		
		...
		sysPage = (SysPage) entityMap.get("sysPage");		
		if (sysPage.getTotal()==-1){			
			List<Map<String, Object>> list = (List<Map<String, Object>>)matAffiTranMainMapper.queryMatInDetailBySingle(entityMap);			
			return ChdJson.toJsonLower(list);			
		}else{			
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());			
			List<Map<String, Object>> list = (List<Map<String, Object>>)matAffiTranMainMapper.queryMatInDetailBySingle(entityMap, rowBounds);			
			PageInfo page = new PageInfo(list);			
			return ChdJson.toJsonLower(list, page.getTotal());			
		}
	}
	...
	public String queryMatAffiTranDetailBySingle(Map<String, Object> entityMap) throws DataAccessException {
		try {			
			return ChdJson.toJsonLower(matAffiTranDetailMapper.queryMatAffiTranDetailBySingle(entityMap));
	...
	public String getNextIn_no(Map<String, Object> entityMap) {
		...
		map.put("table_code", "MAT_AFFI_TRAN_MAIN");
	 ...
	 */
	@Override
	public String outConfirmMatAffiTranMain(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {
			for (Map<String, Object> tmp : entityList) {
				...
				MatAffiTranRela mtr = matAffiTranRelaMapper.queryByCode(tmp);
			...
			if(inList.size() > 0){
				matAffiInMapper.updateBatchMatAffiInMain(inList);
			}
			if(outList.size() > 0){
				matAffiOutMapper.updateBatchMatAffiOutMain(outList);
			}
			if(mainList.size() > 0){
				matAffiTranMainMapper.updateBatchState(mainList);
			}
			...
			logger.error(e.getMessage(), e);
			throw new SysException("{\"error\":\"操作失败\"}");
			//return "{\"error\":\"调出确认失败 数据库异常 请联系管理员! 方法 outConfirmMatAffiTranMain\"}";
		}
	}
	 ...
	 */
	@Override
	public String unOutConfirmMatAffiTranMain(List<Map<String, Object>> entityList) throws DataAccessException {
		List<Map<String, Object>> mainList = new ArrayList<Map<String, Object>>();// 存放mat_out_main字段
		List<Map<String, Object>> inList = new ArrayList<Map<String, Object>>();// 入库List
		List<Map<String, Object>> outList = new ArrayList<Map<String, Object>>();// 出库List
		...
		try {
			...
			if(inList.size() > 0){
				matAffiInCommonMapper.unAuditMatAffiInCommon(inList);
			}
			if(outList.size() > 0){
				matAffiOutCommonMapper.unAuditMatAffiOutCommon(outList);
			}
			if(mainList.size() > 0){
				matAffiTranMainMapper.updateBatchState(mainList);//更新调拨主表
			}
			...
			logger.error(e.getMessage(), e);
			throw new SysException("{\"error\":\"操作失败\"}");
			//return "{\"error\":\"取消调出确认失败 数据库异常 请联系管理员! 方法 unOutConfirmMatAffiTranMain\"}";
		}
	}
	 ...
	 */
	@Override
	public String inConfirmMatAffiTranMain(List<Map<String, Object>> entityList) throws DataAccessException {
		try {
			
			String date = entityList.get(0).get("confirm_date").toString();
			...
			*/
			for(Map<String, Object> tmp : entityList){
				...
				mapMainTran.put("confirm_date",date);
				mapMainTran.put("year", year);
				mapMainTran.put("month", month);
				listUpdateMainTran.add(mapMainTran);
				
				//查询对应的出入库单据ID
				MatAffiTranRela mtr = matAffiTranRelaMapper.queryByCode(mapMainTran);
			...
			//获取所选单据中的所有出库材料
			List<MatAffiTranDetail> outDetailList = matAffiTranDetailMapper.queryMatTranDetailForOutConfirm(entityList);
			//获取所选单据中的所有入库材料
			List<MatAffiTranDetail> inDetailList = matAffiTranDetailMapper.queryMatTranDetailForInConfirm(entityList);
			...
			/**出库数据-----------------------------------------begin---------------------------*/
			for(MatAffiTranDetail mtd : outDetailList){
			...
			/**入库数据-----------------------------------------begin---------------------------*/
			for(MatAffiTranDetail mtd : inDetailList){
				/**如果确认日期按编制日期走则放开年月的重新赋值并改detailList相应的xml*/
				//year = mtd.getYear();
				//month = mtd.getMonth();
				//材料编码和名称用于库存不足的提示
			...
			//------------------------------查询账表 取出相应数据 并且组织账表数据 mat_affi_fifo----------------------------------
			...
			//出库判断
			for (String key : mapOutAffiFifo.keySet()) {//循环当前材料的明细 key查询条件 取相应的值				
			...
			if(is_enough){
				//入库判断
				for (String key : mapInAffiFifo.keySet()) {//循环当前材料的明细 key查询条件 取相应的值				
					Map<String,Object> mapBatch = new HashMap<String,Object>();			
			...
			if(listAddBatchBalance.size() >0 || listUpdateBatchBalance.size() >0){
				...
				throw new SysException("{\"error\":\"操作失败 mat_affi_batch帐表无数据变动\"}");
			}

			//添加/更新mat_affi_balance帐表
			if(listAddInvBalance.size() >0 || listUpdateInvBalance.size() >0){
				...
				throw new SysException("{\"error\":\"操作失败 mat_affi_balance帐表无数据变动\"}");
			}

			//添加/更新mat_affi_inv_hold帐表
			if(listAddInvHold.size() >0 || listUpdateInvHold.size() >0){
				if(listAddInvHold.size() >0){
					matAffiInvHoldMapper.addBatch(listAddInvHold);
				}
				if(listUpdateInvHold.size() >0){
					matAffiInvHoldMapper.updateBatch(listUpdateInvHold);
				}
			}else{
				throw new SysException("{\"error\":\"操作失败 mat_affi_inv_hold帐表无数据变动\"}");
			}
			
			//修改调拨单状态为调入确认
			matAffiTranMainMapper.updateConfirmBatch(listUpdateMainTran);
			//修改出库单状态为出库确认
			matAffiOutMapper.updateBatchConfirm(listUpdateMainOut);
			//修改入库单状态为入库确认
			matAffiInMapper.updateBatchConfirm(listUpdateMainIn);
			...
			logger.error(e.getMessage(), e);
			throw new SysException("{\"error\":\"操作失败\"}");
			//return "{\"error\":\"调入确认失败 数据库异常 请联系管理员! 方法 inConfirmMatTranMain\"}";
		}
	}
	...
	public String balanceConfirmMatAffiTranMain(List<Map<String, Object>> entityMap) throws DataAccessException {
		...
		try {
			...
			for (Map<String, Object> tmp : entityMap) {
				...
				MatAffiTranMain mtm = (MatAffiTranMain) matAffiTranMainMapper.queryByCode(tmp);
				...
				//查询明细表
				List<MatAffiTranDetail> list_detail = (List<MatAffiTranDetail>)matAffiTranDetailMapper.query(tmp);				
				for(MatAffiTranDetail mtd:list_detail){					
			...
			//判断即时库存是否充足(防止多人同时操作同一材料，额外加的判断)
			String invEnough = matCommonMapper.existsMatAffiInvIsEnough(outDetailList);
			if(invEnough != null && !"".equals(invEnough)){
				return "{\"error\":\"以下材料库存物资不足!"+invEnough+"\"}";
			}
			//调拨明细表
			if(detailList.size()>0){
				matAffiTranMainMapper.addBatch(mainList);
				matAffiTranDetailMapper.addBatch(detailList);
				matAffiOutMapper.addMatAffiOutMainBatch(outMainList);
				matAffiOutMapper.addMatAffiOutDetail(outDetailList);
				matAffiInMapper.addMatAffiInMainBatch(inMainList);
				matAffiInMapper.addMatAffiInDetail(inDetailList);
				matAffiTranRelaMapper.addBatch(mainList);
			}else{
	...
	public String queryMatAffiTranMainByMatInv(Map<String, Object> entityMap) throws DataAccessException {		
		...
		sysPage = (SysPage) entityMap.get("sysPage");		
		if (sysPage.getTotal()==-1){			
			List<Map<String,Object>> list = (List<Map<String,Object>>)matAffiTranMainMapper.queryMatAffiTranMainByMatInv(entityMap);		
			return ChdJson.toJsonLower(list);			
		}else{			
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());			
			List<Map<String,Object>> list = (List<Map<String,Object>>)matAffiTranMainMapper.queryMatAffiTranMainByMatInv(entityMap, rowBounds);		
			PageInfo page = new PageInfo(list);			
			return ChdJson.toJsonLower(list, page.getTotal());			
		}
	}
	...
	// 返回出库明细用于保存的默认值
		...
		//调拨主表信息
		public Map<String, Object> defaultBeanToMap(MatAffiTranMain mtm) {
			Map<String, Object> map = new HashMap<String, Object>();
		...
		//调拨明细表信息
		public Map<String, Object> detailBeanToMap(MatAffiTranDetail mtd) {
		...
		public String queryMatAffiTranDetailByTranID( Map<String, Object> entityMap) throws DataAccessException {
			...
			sysPage = (SysPage) entityMap.get("sysPage");			
			if (sysPage.getTotal()==-1){				
				List<Map<String,Object>> list = (List<Map<String,Object>>)matAffiTranDetailMapper.queryMatAffiTranDetailByTranID(entityMap);				
				return ChdJson.toJsonLower(list);				
			}else{				
				RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());				
				List<Map<String,Object>> list = (List<Map<String,Object>>)matAffiTranDetailMapper.queryMatAffiTranDetailByTranID(entityMap, rowBounds);				
				PageInfo page = new PageInfo(list);
				return ChdJson.toJsonLower(list, page.getTotal());				
			}
		}
		...
		public String queryMatAffiTranDetailByCode( Map<String, Object> entityMap) throws DataAccessException {
			List<Map<String,Object>> list = (List<Map<String,Object>>)matAffiTranDetailMapper.queryMatAffiTranDetailByCode(entityMap);				
		 ...
		 */
		@Override
		public MatAffiTranMain queryMatAffiTranMainByCode(Map<String, Object> entityMap) throws DataAccessException {
			return matAffiTranMainMapper.queryMatAffiTranMainByCode(entityMap);
		}

		@Override
		public Map<String, Object> queryMainByCode(Map<String, Object> entityMap) throws DataAccessException {
			return matAffiTranMainMapper.queryMainByCode(entityMap);
		}
		...
		@Override
		public String queryMatAffiTranByPrintTemlate(Map<String, Object> entityMap)
				throws DataAccessException {
			try {
				if("1".equals(String.valueOf(entityMap.get("p_num")))){ 
					List<Map<String, Object>> map =matAffiTranMainMapper.queryMatAffiTranPrintTemlateByMainBatch(entityMap);
					List<Map<String,Object>> list=matAffiTranMainMapper.queryMatAffiTranPrintTemlateByDetail(entityMap);
					return superPrintService.getBatchListByPrintTemplateJson(entityMap,map,list);
				}else{
					//查询入库主表
					Map<String,Object> map=matAffiTranMainMapper.queryMatAffiTranPrintTemlateByMain(entityMap);
		...
		//PageOffice批量打印凭证-返回Map
				@Override
				public Map<String,Object> queryMatAffiTranPrintTemlateMain(Map<String, Object> map)throws DataAccessException {
					
					try{
						Map<String,Object> reMap=new HashMap<String,Object>();
						WebApplicationContext context = ContextLoader.getCurrentWebApplicationContext();
						MatAffiTranMainMapper matAffiTranMainMapper = (MatAffiTranMainMapper)context.getBean("matAffiTranMainMapper");
						if("1".equals(String.valueOf(map.get("p_num")))){ 
						//查询凭证主表
						//Map<String,Object> mainList=matAffiTranMainMapper.queryMatAffiTranPrintTemlateByMain(map);
						List<Map<String, Object>> mainList =matAffiTranMainMapper.queryMatAffiTranPrintTemlateByMainBatch(map);		
						//查询凭证明细表
						List<Map<String,Object>> detailList=matAffiTranMainMapper.queryMatAffiTranPrintTemlateByDetail(map);
				...
				public String queryMatAffiTranMainDetails(Map<String, Object> entityMap) throws DataAccessException {
					...
					sysPage = (SysPage) entityMap.get("sysPage");			
					if (sysPage.getTotal()==-1){				
						List<Map<String,Object>> list = (List<Map<String,Object>>)matAffiTranMainMapper.queryMatAffiTranMainDetails(entityMap);				
						return ChdJson.toJsonLower(list);				
					}else{				
						RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());				
						List<Map<String,Object>> list = (List<Map<String,Object>>)matAffiTranMainMapper.queryMatAffiTranMainDetails(entityMap, rowBounds);				
						PageInfo page = new PageInfo(list);
						return ChdJson.toJsonLower(list, page.getTotal());				
					}
				}
				...
				@Override
				public void updateMatAffiTranRela(Map<String, Object> entityMap) throws DataAccessException {
					List<Map<String,Object>> relaList= JsonListMapUtil.toListMapLower(matAffiTranMainMapper.queryMatAffiTranRela(entityMap));
					if(relaList.size()>0){
						matAffiTranMainMapper.deleteMatAffiTranRela(relaList);
					}
				 ...
				 */
				@Override
				public int queryMatAffiTranMainIsApply(Map<String, Object> entityMap) throws DataAccessException {
					Integer  is_apply = matAffiTranMainMapper.queryMatAffiTranMainIsApply(entityMap);
					return is_apply;
				}
