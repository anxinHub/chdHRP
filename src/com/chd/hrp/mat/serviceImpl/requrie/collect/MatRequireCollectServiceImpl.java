...
package com.chd.hrp.mat.serviceImpl.requrie.collect;
...
import com.chd.hrp.mat.dao.requrie.collect.MatRequireCollectMapper;
 ...
 */
@Transactional
@Service("matRequireCollectService")
public class MatRequireCollectServiceImpl implements MatRequireCollectService {

	private static Logger logger = Logger.getLogger(MatRequireCollectServiceImpl.class);
	// 引入DAO操作
	...
	@Resource(name = "matRequireCollectMapper")
	private final MatRequireCollectMapper matRequireCollectMapper = null;
	...
	public String queryCollect(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			list = (List<?>) matRequireCollectMapper.queryCollect(entityMap);
			return ChdJson.toJson(list);

		} else {
			RowBounds rowBounds = new RowBounds(sysPage.getPage(),sysPage.getPagesize());
			list = (List<?>) matRequireCollectMapper.queryCollect(entityMap, rowBounds);
	...
	public String queryCollectDetail(Map<String, Object> entityMap) throws DataAccessException {
		...
		//System.out.println(ChdJson.toJson(matRequireCollectMapper.queryCollectDetail(entityMap)));
		
		return ChdJson.toJson(matRequireCollectMapper.queryCollectDetail(entityMap));
		/*SysPage sysPage = new SysPage();
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal() == -1) {
			List<?> list = (List<?>) matRequireCollectMapper.queryCollectDetail(entityMap);
			return ChdJson.toJson(list);

		} else {
			RowBounds rowBounds = new RowBounds(sysPage.getPage(),sysPage.getPagesize());
			List<?> list = (List<?>) matRequireCollectMapper.queryCollectDetail(entityMap, rowBounds);
			PageInfo page = new PageInfo(list);
			return ChdJson.toJson(list, page.getTotal());
		}*/
	}
	...
	public String queryCollectDetailPlan(Map<String, Object> entityMap) throws DataAccessException {
		List<MatRequireMain> list = (List<MatRequireMain>) matRequireDetailMapper.query(entityMap);
		return ChdJson.toJson(list);
	}
	...
	public String addCollectNotDir(Map<String, Object> entityMap) throws DataAccessException {
		try{
			...
			//5、插入对应关系表
			if(relaList.size()>0){
				matRequireCollectMapper.addBatchReal(relaList);
				//更新单据状态为 state = 3
				matRequireCollectMapper.updateBatchCollect(relaList);
	...
	public String addCollect(Map<String, Object> entityMap) throws DataAccessException {
		...
		try{	
				...
				//组装明细数据
				List<Map<String, Object>> detailLists= matRequireCollectMapper.queryMatCollectDetail(entityMap);
				...
				//查询数据
				detailList = matRequireDetailMapper.queryDetail(entityMap);
				...
				if(detailList1.size()>0){
					matRequireCollectMapper.addBatchReal(detailList1);
					//更新单据状态为 state = 3
					matRequireCollectMapper.updateBatchCollect(detailList1);
					return "{\"msg\":\"添加成功.\",\"state\":\"true\",\"update_para\":\""+entityMap.get("req_code").toString()+","+"\"}";
				}
				return "{\"msg\":\"添加失败.\",\"state\":\"false\"}";
	...
	public String queryDirCollect(Map<String, Object> entityMap) throws DataAccessException {		
		return ChdJson.toJson(matRequireCollectMapper.queryDirCollect(entityMap));	
	}
	...
	public String queryCollectNotCheck(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			...
			list = (List<?>) matRequireCollectMapper.queryCollectNotCheck(entityMap);
			...
			list = (List<?>) matRequireCollectMapper.queryCollectNotCheck(entityMap, rowBounds);
	...
	public String queryDirCollectNotCheck(Map<String, Object> entityMap) throws DataAccessException {
		...
		return ChdJson.toJson(matRequireCollectMapper.queryDirCollectNotCheck(entityMap));
	}
}
