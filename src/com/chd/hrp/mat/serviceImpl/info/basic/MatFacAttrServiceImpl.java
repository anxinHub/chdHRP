...
import com.chd.hrp.mat.dao.info.basic.MatFacAttrMapper;
...
@Service("matFacAttrService")
public class MatFacAttrServiceImpl implements MatFacAttrService {

	private static Logger logger = Logger.getLogger(MatFacAttrServiceImpl.class);
	//引入DAO操作
	@Resource(name = "matFacAttrMapper")
	private final MatFacAttrMapper matFacAttrMapper = null;
	...
	*/
	...
	public String addMatFacAttr(Map<String,Object> entityMap)throws DataAccessException{
		...
		String p98007 = MyConfig.getSysPara(SessionManager.getGroupId(), "0", "0", "98007");
		
		String p04078 = MyConfig.getSysPara(SessionManager.getGroupId(), SessionManager.getHosId(), SessionManager.getCopyCode(), "04078");
		
		String p04079 = MyConfig.getSysPara(SessionManager.getGroupId(), SessionManager.getHosId(), SessionManager.getCopyCode(), "04079");
		
		if (null != mv) {
			if (mv.get("HOS_FAC").get("is_auto").toString().equals("1")||p04079.equals("1")) {
				if(p98007.equals("1")||p04078.equals("1")){
					supJson=facService.addFacType(entityMap);
				}else{
					// 台州单独自动生成编码规则
		...
		/*处理银行信息-------------------end-------------*/
		//获取对象050115 生产厂商附属表
		//AssFacAttr assFacAttr = queryMatFacAttrByCode(entityMap);
		int isExists = matFacAttrMapper.queryFacAttrExist(entityMap);
		try {
			if (isExists>0) {
				return "{\"error\":\"生产厂商附属表信息已存在,请重新添加.\",\"state\":\"true\"}";
			}else{
				//新增
				matFacAttrMapper.addMatFacAttr(entityMap);
	...
	*/
	...
	public String addBatchMatFacAttr(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			...
			matFacAttrMapper.addBatchMatFacAttr(entityList);
	...
	*/
	...
	public String updateMatFacAttr(Map<String,Object> entityMap)throws DataAccessException{
		...
		String mod_code = String.valueOf(entityMap.get("mod_code"));//记录前台传的 mod_code 的值（中间有变动）
		//String mod_code = String.valueOf(SessionManager.getModCode());
		String supJson = "";
		...
		AssFacAttr assFacAttr = queryMatFacAttrByCode(entityMap);
		int count = matFacAttrMapper.queryHosFacModExist(entityMap);
		try {
			if (assFacAttr == null) {
				matFacAttrMapper.addMatFacAttr(entityMap);
				/*if(count > 0){
					matFacAttrMapper.updateHosFacMod(entityMap);
				}else{
					matFacAttrMapper.addHosFacMod(entityMap);
				}*/
				...
				matFacAttrMapper.updateMatFacAttr(entityMap);
				/*if(count > 0){
					matFacAttrMapper.updateHosFacMod(entityMap);
				}else{
					matFacAttrMapper.addHosFacMod(entityMap);
	...
	*/
	...
	public String updateBatchMatFacAttr(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			...
			matFacAttrMapper.updateBatchMatFacAttr(entityList);
	...
	*/
    ...
    try {
			...
			int state = matFacAttrMapper.deleteMatFacAttr(entityMap);
	...
	*/
	...
	public String deleteBatchMatFacAttr(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			...
			//删除 生产厂商 与系统模块对应关系
			matFacAttrMapper.deleteBatchHosFacMod(entityList);
			
			hosFacBankMapper.deleteHosFacBankBatch(entityList);
			
			matFacAttrMapper.deleteBatchMatFacAttr(entityList);
			//删除 供应商变更表数据
	...
	*/
	...
	public String queryMatFacAttr(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssFacAttr> list = matFacAttrMapper.queryMatFacAttr(entityMap);
			...
			List<AssFacAttr> list = matFacAttrMapper.queryMatFacAttr(entityMap, rowBounds);
	...
	*/
	...
	public AssFacAttr queryMatFacAttrByCode(Map<String,Object> entityMap)throws DataAccessException{
		return matFacAttrMapper.queryMatFacAttrByCode(entityMap);
	}
	...
	*/
	...
	public String init(Map<String, Object> entityMap) throws DataAccessException {
		...
		try {
			...
			int state = matFacAttrMapper.init(entityMap);
	...
	*/
	...
	public Long queryFacIdByCode(Map<String, Object> mapVo)	throws DataAccessException {
		return matFacAttrMapper.queryFacIdByCode(mapVo);
	}
	/**
	 * 查询 生产厂商名称是否存在 
	 */
	...
	public int queryFacExists(Map<String, Object> mapVo) throws DataAccessException {
		return matFacAttrMapper.queryFacExists(mapVo);
	}
	/**
	 * 查询 生产厂商分类 编码是否存在 
	 */
	...
	public int queryFacTypeExist(Map<String, Object> mapVo)	throws DataAccessException {
		return matFacAttrMapper.queryFacTypeExist(mapVo);
	}
	/**
	 * 查询 所属系统模块编码是否存在
	 */
	...
	public int queryModExist(Map<String, Object> mapVo) throws DataAccessException {
		return matFacAttrMapper.queryModExist(mapVo);
	}
}
