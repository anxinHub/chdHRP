...
import com.chd.hrp.mat.dao.info.basic.MatInvCertSupMapper;
...
@Service("matInvCertSupService")
public class MatInvCertSupServiceImpl implements MatInvCertSupService {

	private static Logger logger = Logger.getLogger(MatInvCertSupServiceImpl.class);
	//引入DAO操作
	@Resource(name = "matInvCertSupMapper")
	private final MatInvCertSupMapper matInvCertSupMapper = null;
	...
	*/
	...
	public String addMatInvCertSup(Map<String,Object> entityMap)throws DataAccessException, ParseException{
		...
		List<MatInvCertSup> list = matInvCertSupMapper.queryMatInvCertSupByID(entityMap);
		if (list.size() >0) {
	...
	/*	Boolean flag = compare_date(String.valueOf(entityMap.get("start_date")),String.valueOf(entityMap.get("end_date")));
		...
		try {
			entityMap.put("sup_cert_id", matInvCertSupMapper.queryMatInvCertNextId());
			...
			if(entityMap.get("detailData") != null && !"".equals(entityMap.get("detailData"))){
				...
				while (it.hasNext()) {
					...
					if( jsonObj.get("inv_id") != null && !"".equals(jsonObj.get("inv_id"))){
						...
						relaMap.put("sup_cert_id", entityMap.get("sup_cert_id"));
						relaMap.put("sup_cert_code", entityMap.get("sup_cert_code"));
			...
			if(entityMap.get("supData") != null && !"".equals(entityMap.get("supData"))){
				...
				while (it.hasNext()) {
					...
					if( jsonObj.get("sup_id") != null && !"".equals(jsonObj.get("sup_id"))){
						...
						supMap.put("sup_cert_id", entityMap.get("sup_cert_id"));
						certSupList.add(supMap);
					}
				}
			}
			
			entityMap.put("cert_id", entityMap.get("sup_cert_id"));
			entityMap.put("cert_code", entityMap.get("sup_cert_code"));
			matInvCertMapper.addMatInvCert(entityMap);
			int state = matInvCertSupMapper.addMatInvCertSup(entityMap);  
			if(orderRelaList.size()>0){
				matInvCertRelaMapper.addBatchMatInvCertSupRela(orderRelaList);
			}
			
			if(certSupList.size()>0){
				matCertSupMapper.deleteSup(entityMap);
				matCertSupMapper.addBatchSup(certSupList);
			}
	...
	public String addBatchMatCertSup(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			...
			for(Map<String,Object> item: entityList){
				...
				MatInvCertSup invCert = matInvCertSupMapper.queryMatInvCertSupByCode(item);
				
				if( invCert == null){
					item.put("sup_cert_id", matInvCertSupMapper.queryMatInvCertNextId());
					...
					item.put("sup_cert_id", String.valueOf(invCert.getSup_cert_id()));
					item.put("start_date", String.valueOf(item.get("start_date")).substring(0, 10));
					item.put("end_date", String.valueOf(item.get("end_date")).substring(0, 10));
					updateList.add(item);
					certIds += invCert.getSup_cert_id() +"@"+invCert.getSup_cert_code() + ",";
				}
				...
				if(item.get("supData") != null && !"".equals(item.get("supData"))){
					...
					while (it.hasNext()) {
						...
						if( jsonObj.get("sup_id") != null && !"".equals(jsonObj.get("sup_id"))){
							...
							supMap.put("sup_cert_id", String.valueOf(item.get("sup_cert_id")));
							certSupList.add(supMap);
						}
					}
				}
				...
				if(item.get("inv_id") != null && !"".equals(item.get("inv_id"))){
					...
					invCertMap.put("sup_cert_id", String.valueOf(item.get("sup_cert_id")));
					invCertMap.put("sup_cert_code", String.valueOf(item.get("sup_cert_code")));
					invCertMap.put("sup_cert_inv_name", String.valueOf(item.get("sup_cert_inv_name")));
			...
			if(certList.size() > 0){
				...
				matInvCertSupMapper.addBatchMatInvCertSup(certList);
			}
			
			if(updateList.size() > 0){
				
				matInvCertSupMapper.updateBatchMatInvCertSup(updateList);
			}
			
			if(invCertList.size() > 0 ){
				matInvCertRelaMapper.deleteMatInvCertSupRela(delMap);
				
				matInvCertRelaMapper.addBatchMatInvCertSupRela(invCertList);
			}
			
			if(certSupList.size()>0){
				matCertSupMapper.deleteBatchSup(entityList);
				matCertSupMapper.addBatchSup(certSupList);
			}
				...
				while (it.hasNext()) {
					...
					if( jsonObj.get("inv_id") != null && !"".equals(jsonObj.get("inv_id"))){
						...
						if(!jsonObj.get("inv_id").equals(jsonObj.get("invid"))){
							...
							deleteMap.put("sup_cert_id", entityMap.get("sup_cert_id"));
							deleteMap.put("sup_cert_code", entityMap.get("sup_cert_code"));
							deleteMap.put("sup_cert_inv_name", entityMap.get("sup_cert_inv_name"));
							...
							insertMap.put("sup_cert_id", entityMap.get("sup_cert_id"));
							insertMap.put("sup_cert_code", entityMap.get("sup_cert_code"));
							insertMap.put("sup_cert_inv_name", entityMap.get("sup_cert_inv_name"));
			...
			if(entityMap.get("supData") != null && !"".equals(entityMap.get("supData"))){
				...
				while (it.hasNext()) {
					...
					if( jsonObj.get("sup_id") != null && !"".equals(jsonObj.get("sup_id"))){
						...
						supMap.put("sup_cert_id", entityMap.get("sup_cert_id").toString());
						certSupList.add(supMap);
					}
				}
			}
			
			int state = matInvCertSupMapper.updateMatInvCertSup(entityMap);
			if(insertList.size()>0){
				matInvCertRelaMapper.addBatchMatInvCertSupRela(insertList);
			}
			if(updateList.size()>0){
				matInvCertRelaMapper.deleteBatchMatInvCertSupRela(updateList);
			}
	...
	*/
	@Override
	public String updateBatchMatInvCertSup(List<Map<String,Object>> entityList)throws DataAccessException{
		
		try {
			
		  matInvCertSupMapper.updateBatchMatInvCertSup(entityList);
	...
	*/
    ...
    public String deleteMatInvCertSup(Map<String, Object> entityMap) throws DataAccessException {
	    ...
	    try {
				matInvCertSupMapper.deleteMatInvCertSup(entityMap);
	...
	*/
	@Override
	public String deleteBatchMatInvCertSup(List<Map<String,Object>> entityList)throws DataAccessException{
		
		try {
			
			matCertSupMapper.deleteBatchSup(entityList);
			
			matInvCertRelaMapper.deleteBatchMatInvCertSupRela(entityList);
			
			matInvCertSupMapper.deleteBatchMatInvCertSup(entityList);
	...
	*/
	...
	public String queryMatInvCertSup(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			...
			List<MatInvCertSup> list = matInvCertSupMapper.queryMatInvCertSupNew(entityMap);
			...
			List<MatInvCertSup> list = matInvCertSupMapper.queryMatInvCertSupNew(entityMap, rowBounds);
	...
	*/
	...
	public MatInvCertSup queryMatInvCertSupByCode(Map<String,Object> entityMap)throws DataAccessException{
		return matInvCertSupMapper.queryMatInvCertSupByCode(entityMap);
	}
	...
	*/
	...
	public MatInvCertSup queryMatInvCertSupByUniqueness(Map<String,Object> entityMap) throws DataAccessException{
		return matInvCertSupMapper.queryMatInvCertSupByUniqueness(entityMap);
	}
	/**
	...
	public MatInvCertSup queryMatInvCertSupByCodeNew(Map<String, Object> mapVo) throws DataAccessException{
		return matInvCertSupMapper.queryMatInvCertSupByCodeNew(mapVo);
	}
	/**
	...
	public List<MatInvCertSup> queryMatInvCertSupByID(Map<String, Object> mapVo) throws DataAccessException{
		return matInvCertSupMapper.queryMatInvCertSupByID(mapVo);
	}
	/**
	...
	public int queryDelDate(Map<String, Object> mapVo) throws DataAccessException{
		return matInvCertSupMapper.queryDelDate(mapVo);
	}
	...
	public String queryMatCertInvList(Map<String, Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(matInvCertSupMapper.queryMatCertInvList(entityMap));
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			
			List<Map<String,Object>> list = matInvCertSupMapper.queryMatCertInvList(entityMap, rowBounds);
	...
	public String queryMatInvCertSupDetail(Map<String, Object> entityMap) throws DataAccessException{
		...
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal()==-1){
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(matInvCertRelaMapper.queryMatInvCertSupDetail(entityMap));
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<Map<String,Object>> list = matInvCertRelaMapper.queryMatInvCertSupDetail(entityMap, rowBounds);
			PageInfo page = new PageInfo(list);
	...
	public String queryMatCertInvChoiceInvList(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<Map<String, Object>> list= matInvCertSupMapper.queryMatCertInvChoiceInvList(detailList);
	...
	public String queryMatCerSup(Map<String, Object> entityMap) {
		...
		if (sysPage.getTotal()==-1){
			...
			List<Map<String,Object>> list = (List<Map<String, Object>>) matCertSupMapper.querySup(entityMap);
			...
			List<Map<String,Object>> list = (List<Map<String, Object>>) matCertSupMapper.querySup(entityMap, rowBounds);
	...
	public String supAuditMatInvCertSupBatch(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {	
			//证件审核
			//matInvCertMapper.addBatchMatInvCert(entityMap);
			
			matInvCertSupMapper.supAuditOrUnAuditBatch(entityMap);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public String auditMatInvCertSupBatch(List<Map<String, Object>> entityMap) throws DataAccessException {
		...
		try {	
			//批量审核
			matInvCertSupMapper.auditOrUnAuditBatch(entityMap);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public String importData(Map<String, Object> entityMap)throws DataAccessException{
		...
		try {	
			...
			//证件列表
			List<Map<String, Object>> certRList = matInvCertSupMapper.queryCertListForImport(seachMap);
			...
			//证件类别列表
			List<Map<String, Object>> certTypeList = matInvCertSupMapper.queryCertTypeListForImport(seachMap);
			...
			//生产厂商列表
			List<Map<String, Object>> facList = matInvCertSupMapper.queryFacListForImport(seachMap);
			...
			//材料列表
			List<Map<String, Object>> invList = matInvCertSupMapper.queryInvListForImport(seachMap);
			...
			//供应商列表
			List<Map<String, Object>> supList = matInvCertSupMapper.querySupListForImport(seachMap);
			...
			for(Map<String, List<String>> dataMap : dataList){
				...
				/*if(certRMap.containsKey(certKey)){
					return "{\"warn\":\"" + rowList.get(0) + "，证件号已存在！\",\"state\":\"false\",\"row_cell\":\"" + rowList.get(0) + "\"}";
					
				}*/
				/**校验证件号************end*******************/
				
				if(certRMap.containsKey(certKey)){
					map.put("sup_cert_id", certRMap.get(certKey).get("SUP_CERT_ID"));
				}else{
					if(certMap.containsKey(certKey)){
						map = certMap.get(certKey);
					}else{
						//验证通过存放证件号
						map.put("sup_cert_code", rowList.get(1));
						
						/**校验证件产品名称******begin*****************/
						rowList = dataMap.get("证件产品名称");
						if(rowList != null && rowList.get(1) != null && !"".equals(rowList.get(1))){
							map.put("sup_cert_inv_name", rowList.get(1));
						}else{
							map.put("sup_cert_inv_name", "");
						}
						/**校验证件产品名称******end*******************/
						...
						map.put("sup_cert_id", matInvCertSupMapper.queryMatInvCertNextId());
				...
				if(rowList != null && rowList.get(1) != null && !"".equals(rowList.get(1))){
					...
					if(!invRelaMap.containsKey(invRelaKey)){
						...
						invRMap.put("sup_cert_id", map.get("sup_cert_id"));
						invRMap.put("sup_cert_code", certKey);
				...
				if(rowList != null && rowList.get(1) != null && !"".equals(rowList.get(1))){
					...
					if(!supRelaMap.containsKey(supRelaKey)){
						...
						supRMap.put("sup_cert_id", map.get("sup_cert_id"));
			...
			if(invRelaList.size()>0){
				matInvCertRelaMapper.addBatchMatInvCertSupRela(invRelaList);
			}
			//批量添加证件供应商关系
			if(supRelaList.size()>0){
				matCertSupMapper.addBatchSup(supRelaList);
			}
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
