...
public class DeptServiceImpl implements DeptService {
	...
	public String addDept(Map<String, Object> entityMap) throws DataAccessException {
		...
		mapCheck.put("dict_code", "HOS_DEPT");
		...
		List<Dept> list = deptMapper.queryDeptById(entityMap);
		...
		// 添加编码规则判断
		entityMap.put("proj_code", "HOS_DEPT");
		...
		if (maxNumMap.containsKey(dept_code.length())) {// 编码匹配
			...
			if (dept_level == 1) {
				...
				List<Map<String, Object>> superCode = deptMapper.qureySurp_code(entityMap);
				...
				if(superCode.size() != 0){
					...
					if(superCode.get(0).get("IS_LAST").toString().equals("1")){
						...
						deptMapper.updateIsLast(isLast);
						deptDictMapper.updateIsLast(isLast);
		...
		utilMap.put("field_table", "HOS_DEPT");
		...
		if (entityMap.get("sort_code").equals("系统生成")) {
			...
			int result = deptMapper.addDept(entityMap);
			...
			if (result > 0) {
				...
				entityMap.put("dept_id", deptMapper.queryCurrentSequence());
	...
	public String addBatchDept(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {
			...
			deptMapper.addBatchDept(entityList);
	...
	public String queryDept(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<Dept> list = deptMapper.queryDept(entityMap, rowBounds);
	...
	public Dept queryDeptByCode(Map<String, Object> entityMap) throws DataAccessException {
		return deptMapper.queryDeptByCode(entityMap);
	...
	public String deleteBatchDept(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {
			...
			List<Dept> superCodeMap= new ArrayList<Dept>(); 
			map.put("dict_code", "HOS_DEPT");
			...
			if(entityList!=null && entityList.size()>0){
				for(Map<String, Object> mapVo : entityList){
					...
					storeIdStr+=mapVo.get("dept_id")+",";
						...
						List<Map<String, Object>>	dept=deptMapper.qureySurpcode(map);
						if(dept.size()>0){
			...
			deptDictMapper.deleteBatchDeptDict(entityList);
			 deptMapper.deleteBatchDept(entityList);
		//遍历修改上级是否末级为是
			 for(String key :supermap.keySet()){
				 ...
				 Map<String, Object> hse = supermap.get(key);
					...
					List<Map<String, Object>> dept1 = deptMapper.qureySurpcode(map);
					if (dept1.size() == 0) {
						isLast.put("group_id", SessionManager.getGroupId());
						isLast.put("hos_id", SessionManager.getHosId());
	...
	public String deleteDept(Map<String, Object> entityMap) throws DataAccessException {
		...
		try {
			...
			deptMapper.deleteDept(entityMap);
			return "{\"msg\":\"删除成功.\",\"state\":\"true\"}";
		}
		catch (Exception e) {
	...
	public String updateDept(Map<String, Object> entityMap) throws DataAccessException {
		...
		try {
			List<Dept> list = deptMapper.queryDeptByIdName(entityMap);
            ...
            if(entityMap.get("is_last").toString().equals("1")){
            	List<Dept> listDept=	deptMapper.queryBySuperCode(entityMap);
            	if(listDept.size()>0){
            		return "{\"error\":\"名称：" + entityMap.get("dept_name").toString() + "已存在下级,不允许修改是否末级为是.\"}";
            	}
            }
			deptMapper.updateDept(entityMap);
			
			//拿到修改前的type_code删除掉老数据
			String type_code = entityMap.get("type_code").toString();
			entityMap.put("type_code", entityMap.get("original_type_code").toString());
			accDeptAttrMapper.deleteAccDeptAttr(entityMap);
			
			//拿到新的type_code去做修改
			entityMap.put("type_code", type_code);
			accDeptAttrMapper.addAccDeptAttr(entityMap);
			//判断上级部门类别编码是否一致
			if(entityMap.get("super_code").toString().equals("0")&& !entityMap.get("ol_kind_code").toString().equals(entityMap.get("kind_code").toString()) ){
				...
				if(listDept.size()>0){
					//修改下级部门类别编码
					deptMapper.updateBatchSuperCode(entityMap,listDept);
				}
			}
			//判断上级部门是否停用
            ...
            List<Dept> listDept=	deptMapper.queryBySuperCode(entityMap);
				if(listDept.size()>0){
					//修改是否停用
					deptMapper.updateBatchStop(entityMap,listDept);
				}
			}
			if (entityMap.get("history").equals("true")) {
		    	...
		    	if(entityMap.get("super_code").toString().equals("0")&& !entityMap.get("ol_kind_code").toString().equals(entityMap.get("kind_code").toString()) ){
					List<DeptDict> listDept=	deptDictMapper.queryBySuperCode(entityMap);
					
					if(listDept.size()>0){
						
						deptDictMapper.updateBatchSuperCode(entityMap,listDept);
					}
				}
		    	if(entityMap.get("super_code").toString().equals("0")){
		    		List<DeptDict> listDept=	deptDictMapper.queryBySuperCode(entityMap);
						...
						if(listDept.size()>0){
							//修改是否停用
							deptDictMapper.updateBatchStop(entityMap,listDept);
						}
					}
				deptDictMapper.addDeptDict(entityMap);
				...
				deptDictMapper.updateDeptDict_dept(entityMap);
	...
	public String updateBatchDept(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {
			...
			deptMapper.updateBatchDept(entityList);
	...
	public String updateDeptCode(Map<String, Object> entityMap) throws DataAccessException {
		try {
			deptMapper.updateDeptCode(entityMap);
	...
	public String updateDeptName(Map<String, Object> entityMap) throws DataAccessException {
		try {
			entityMap.put("spell_code", StringTool.toPinyinShouZiMu(entityMap.get("dept_name").toString()));
			entityMap.put("wbx_code", StringTool.toWuBi(entityMap.get("dept_name").toString()));
			deptMapper.updateDeptName(entityMap);
	...
	public String getSuperCode(@RequestParam Map<String, Object> mapVo) {
		...
		if (maxNumMap.containsKey(dept_code.length())) {// 编码匹配
			...
			if (dept_level == 1) {
				...
				List<Map<String, Object>> superCode = deptMapper.qureySurp_code(mapVo);
	...
	public Map<String, Object> queryDeptByCodeName(Map<String, Object> entityMap) {
		return deptMapper.queryDeptByCodeName(entityMap);
	}
	...
	//查询组织结构图
		...
		public String queryDeptOrg(Map<String, Object> map)throws DataAccessException{
			...
			try{
				...
				//查询科室
				List<Map<String,Object>> lastList=deptMapper.queryDeptOrg(map);
