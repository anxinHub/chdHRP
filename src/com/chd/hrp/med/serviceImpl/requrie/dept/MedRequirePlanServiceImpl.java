...
import com.chd.hrp.med.dao.requrie.dept.MedRequirePlanMapper;
import com.chd.hrp.med.entity.MedReqStoreRela;
 ...
 */
@Transactional
@Service("medRequirePlanService")
public class MedRequirePlanServiceImpl implements MedRequirePlanService {

	private static Logger logger = Logger.getLogger(MedRequirePlanServiceImpl.class);
	// 引入DAO操作
	...
	@Resource(name = "medRequirePlanMapper")
	private final MedRequirePlanMapper medRequirePlanMapper = null;
			...
			//try{
				while (it.hasNext()) {
					...
					if( !"".equals(jsonObj.get("inv_id")) && jsonObj.get("inv_id") != null){
						...
						detailMap.put("req_detail_id", medRequireDetailMapper.getNextDetailReqId());
	...
	public String update(Map<String, Object> entityMap) throws DataAccessException {	
		try {
			...
			int state = medRequireMainMapper.update(entityMap);
			if(state > 0){
				//更新明细表
				...
				if(req_detail_ids.length() > 0){
					...
					//首先删除明细表中前台删除的明细数据
					medRequireDetailMapper.delete(deleteMap);
					
				}
				//更新明细表中数据
				if(delUpdateList.size() > 0){
					medRequireDetailMapper.updateBatch(delUpdateList);
				}
				//插入明细表中没有的数据
				if(addList.size()>0){
	...
	public String updateBatch(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {
			medRequireMainMapper.updateBatch(entityList);
	...
	public String delete(Map<String, Object> entityMap) throws DataAccessException {
		try {
			int state = medRequireMainMapper.delete(entityMap);
	...
	public String deleteBatch(List<Map<String, Object>> entityList)	throws DataAccessException {
		try {
			...
			medRequireDetailMapper.deleteBatch(entityList);
			return "{\"msg\":\"删除成功.\",\"state\":\"true\"}";
		} catch (Exception e) {
	...
	public <T> T queryByUniqueness(Map<String, Object> entityMap) throws DataAccessException {
		return (T) medRequirePlanMapper.queryByUniqueness(entityMap);
	}
	...
	public String queryPlan(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			List<?> list = medRequirePlanMapper.queryPlan(entityMap);
			return ChdJson.toJson(list);
		} else {
			RowBounds rowBounds = new RowBounds(sysPage.getPage(),sysPage.getPagesize());
			List<?> list = medRequirePlanMapper.queryPlan(entityMap, rowBounds);
	 ...
	 */
	@Override
	public String updateAbortMedDeptPlan(List<Map<String, Object>> detailList) 	throws DataAccessException {
		
		try {
			int state = medRequirePlanMapper.updateAbortMedDeptPlan(detailList);
			return "{\"msg\":\"中止计划成功.\",\"state\":\"true\"}";
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
			return "{\"error\":\"中止计划失败 数据库异常 请联系管理员! 错误编码  updateAbortMedDeptPlan\"}";
	 ...
	 */
	@Override
	public String updateSubmitMedDeptPlan(List<Map<String, Object>> detailList) throws DataAccessException {
		try {
			medRequirePlanMapper.updateSubmitMedDeptPlan(detailList);
			return "{\"msg\":\"提交成功.\",\"state\":\"true\"}";
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
			return "{\"error\":\"提交失败 数据库异常 请联系管理员! 错误编码  updateSubmitMedDeptPlan\"}";
	 ...
	 */
	@Override
	public String updateUnSubmitMedDeptPlan(List<Map<String, Object>> detailList) throws DataAccessException {
		try {
			//判断是否已生成仓库需求计划
			...
			medRequirePlanMapper.updateUnSubmitMedDeptPlan(detailList);
			return "{\"msg\":\"取消提交成功.\",\"state\":\"true\"}";
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
			return "{\"error\":\"删除失败 数据库异常 请联系管理员! 错误编码  updateCancleSubmitMedDeptPlan\"}";
	 ...
	 */
	@Override
	public String deleteMedDeptPlan(List<Map<String, Object>> detailList) throws DataAccessException {
		try {
			//删除明细表数据
			medRequireDetailMapper.deleteBatch(detailList);
			//删除主表信息
			medRequireMainMapper.deleteBatch(detailList);
			...
			throw new SysException("{\"error\":\"操作失败\"}");
			//return "{\"error\":\"删除失败 数据库异常 请联系管理员! 错误编码  deleteMedDeptPlan\"}";
	...
	public String copyMedDeptPlan(List<Map<String, Object>> entityList) throws DataAccessException {
		try{
			...
			for(Map<String, Object> main : entityList){
				...
				mainMap = medRequireMainMapper.queryByCode(mapVo);
				...
				//查询最大的单号ID
				mainMap.put("req_id", medRequireMainMapper.getNextReqId(mapVo));
				...
				detailOldList = medRequireDetailMapper.queryDetailByID(mapVo);
			...
			if(mainList.size() > 0){
				medRequireMainMapper.addBatch(mainList);
			}
			if(detailList.size() > 0){
				medRequireDetailMapper.addBatch(detailList);
			}
			...
			throw new SysException("{\"error\":\"操作失败\"}");
			//return "{\"error\":\"复制失败 数据库异常 请联系管理员! 错误编码  copyMedDeptPlan\"}";
		}
	}
	/**
	 * 获取需要入库单号
	...
	public String queryDeptExpend(Map<String, Object> entityMap) throws DataAccessException{
		List<?> list = medRequirePlanMapper.queryDeptExpend(entityMap);
		return ChdJson.toJson(list);
	}
	...
	public String queryDetail(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal()==-1){			
			...
			List<?> list = medRequireDetailMapper.queryDetailByCode(entityMap);		
			
			return ChdJson.toJson(list);			
		}else{			
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			
			List<?> list = medRequireDetailMapper.queryDetailByCode(entityMap, rowBounds);	
	...
	public String queryDeptSupport(Map<String, Object> entityMap) throws DataAccessException{
		List<?> list = medRequirePlanMapper.queryDeptSupport(entityMap);
		return ChdJson.toJson(list);
	}
	...
	public String queryStoreSafe(Map<String, Object> entityMap) throws DataAccessException {
		List<?> list = medRequirePlanMapper.queryStoreSafe(entityMap);
		return ChdJson.toJson(list);
	}
	...
	@Override
	public Map<String,Object> printMedRequireMainData(Map<String, Object> map)throws DataAccessException {
		
		try{
			Map<String,Object> reMap=new HashMap<String,Object>();
			WebApplicationContext context = ContextLoader.getCurrentWebApplicationContext();
			MedRequireMainMapper medRequireMainMapper = (MedRequireMainMapper)context.getBean("medRequireMainMapper");
			
			//查询凭证主表
			List<Map<String,Object>> mainList=medRequireMainMapper.queryMedRequireMainByReqCode(map);
					
			//查询凭证明细表
			List<Map<String,Object>> detailList=medRequireMainMapper.queryMedRequireMainDetailByReqCode(map);
