package com.chd.hrp.med.serviceImpl.purchase.collect;
...
import com.chd.hrp.med.dao.purchase.collect.MedPurMainCollectMapper;
 ...
 * @Version: 1.0
 */
@Service("medPurMainCollectService")
public class MedPurMainCollectServiceImpl implements MedPurMainCollectService {
	
	private static Logger logger = Logger.getLogger(MedPurMainCollectServiceImpl.class);
	
	@Resource(name = "medPurMainCollectMapper")
	private MedPurMainCollectMapper medPurMainCollectMapper = null;
	...
	*/
	...
	public String addOrUpdate(Map<String, Object> entityMap) throws DataAccessException {
		...
		medPurMainCollectMapper.update(entityMap);//修改采购主表数据
			
		list.add(entityMap);
		
		if(list.size() > 0 ){
			
			medPurMainCollectMapper.deleteMedPurDetail(list);//删除采购明细数据
		}
			
		if(allDataList.size() > 0 ){//有数据、就添加
				
			medPurMainCollectMapper.addBatchMedPurDetail(allDataList);//添加采购明细数据
		}
			
		if(deletedDataList.size() > 0 ){
				
			medPurMainCollectMapper.updateMedPurMainState(deletedDataList);//还原采购计划单状态到已审核
				
			medPurMainCollectMapper.deleteBatchMedPurRela(deletedDataList);//根据ID删除某个计划的对应关系
		}
			
		if(relaList.size() > 0){
			//更新对应关系
			medPurMainCollectMapper.updateBatchMedPurRela(relaList);//修改采购计划对应关系
	...
	*/
	...
	public String query(Map<String, Object> entityMap) throws DataAccessException {
		...
		if(sysPage.getTotal() == -1){
			...
			List<?> list = medPurMainCollectMapper.query(entityMap);
			...
			List<?> list = medPurMainCollectMapper.query(entityMap, rowBounds);
	...
	*/
	...
	public <T> T queryByCode(Map<String, Object> entityMap) throws DataAccessException {
		...
		return medPurMainCollectMapper.queryByCode(entityMap);
	}
	...
	*/
	...
	public String queryMedPurDetail(Map<String, Object> entityMap) throws DataAccessException {
		...
		if(sysPage.getTotal() == -1){
			...
			List<?> list = medPurMainCollectMapper.queryMedPurDetail(entityMap);
			...
			List<?> list = medPurMainCollectMapper.queryMedPurDetail(entityMap, rowBounds);
	...
	*/
	...
	public String queryMedInvCurAmountDetail(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<?> list = medPurMainCollectMapper.queryMedInvCurAmountDetail(entityMap);
	...
	*/
	...
	public String queryMedPurMainByCollect(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<?> list = medPurMainCollectMapper.queryMedPurMainByCollect(entityMap);
	...
	*/
	public String genByCollectMedPurMain(List<Map<String,Object>> entityMap) throws DataAccessException{
		...
		Map<String,Object> medPurMainMap;//主表map
		List<Map<String,Object>> medPurMainList = new ArrayList<Map<String,Object>>();//主表集合
		
		Map<String,Object> medPurDetailMap;//明细表map
		List<Map<String,Object>> medPurDetailList = new ArrayList<Map<String,Object>>();//明细表集合
		
		Map<String,Object> medPurRelaMap;//对应关系
		List<Map<String,Object>> medPurRelaList = new ArrayList<Map<String,Object>>();//对应关系集合
		...
		List<Map<String,Object>> unitList = (List<Map<String, Object>>) medPurMainCollectMapper.queryMedPurUnit(entityMap);
		
		if(unitList.size() > 0 ){
			
			for(int x = 0 ;x < unitList.size(); x++){
				
				medPurMainMap =defaultValue();//主表数据Map
				
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				//获取主表数据
				Map<String,Object> map = entityMap.get(0);
					
				medPurMainMap.put("group_id", map.get("group_id"));
							
				medPurMainMap.put("hos_id", map.get("hos_id"));
							
				medPurMainMap.put("copy_code", map.get("copy_code"));
							
				medPurMainMap.put("pur_hos_id", unitList.get(x).get("pur_hos_id").toString());
							
				medPurMainMap.put("req_hos_id", unitList.get(x).get("req_hos_id").toString());
							
				medPurMainMap.put("pay_hos_id", unitList.get(x).get("pay_hos_id").toString());
					
				medPurMainMap.put("maker", SessionManager.getUserId());
					
				medPurMainMap.put("make_date", sdf.format(new Date()));
				medPurMainMap.put("checker", SessionManager.getUserId());
				medPurMainMap.put("check_date", medPurMainMap.get("make_date"));
					
				medPurMainMap.put("pur_type", "2");
					
				medPurMainMap.put("is_collect", "1");
					
				medPurMainMap.put("state","2");
				
				String curDate = sdf.format(new Date());
						
				medPurMainMap.put("creat_date", curDate);
				//System.out.println("************* creat_date:"+curDate);	
				medPurMainMap.put("make_year", curDate.substring(0, 4));
						
				medPurMainMap.put("make_month", curDate.substring(5, 7));
				
				int flag =  medPurMainCollectMapper.queryIsExists(medPurMainMap);//查询计划单号是否存在
				
				if(flag > 0){
					
					String maxCode = medPurMainCollectMapper.queryMaxCode(medPurMainMap);
					
					medPurMainMap.put("max_code", (Integer.parseInt(maxCode))+1);
					
					//更新单据号表
					int state = medPurMainCollectMapper.updateMedNoMatch(medPurMainMap);
					
					String code = "00000" + medPurMainMap.get("max_code").toString();
					
					code = code.substring(code.length()-5,code.length());
					
					pur_code = "CG"+medPurMainMap.get("make_year")+medPurMainMap.get("make_month")+code;
					
					medPurMainMap.put("pur_code", pur_code);//计划单号
					
					codeFlag = true;
					
				}else{
				
					pur_code  = "CG"+medPurMainMap.get("make_year")+medPurMainMap.get("make_month")+"00001";
					
					medPurMainMap.put("pur_code", pur_code);
				...
				if(codeFlag == false){
					...
					//更新单据序号表
					medPurMainMap.put("prefixe", "CG");
					medPurMainMap.put("table_code", "M_PUR_MAIN");
					medPurMainMap.put("max_no", 1);
					int state = medPurMainCollectMapper.addMedNoMatch(medPurMainMap);
					
				}
				
				int pur_id = medPurMainCollectMapper.queryMedPurMainPurId();//汇总生成采购单ID
				
				medPurMainMap.put("pur_id", pur_id);
				
				if(pur_ids.length() > 0 ){
					
					medPurMainMap.put("pur_ids", pur_ids.substring(0,pur_ids.length()-1));
				}
				//查询明细数据
				List<Map<String,Object>> list = (List<Map<String, Object>>) medPurMainCollectMapper.queryMedPurMainDetail(medPurMainMap);
				...
				if(list.size() > 0 ){
					...
					for(int i = 0 ;i<list.size();i++){
						...
						if(reDetailMap.get(purDetailStr) == null){
							...
							medPurDetailMap = defaultDetailValue();
							
							medPurDetailMap.put("group_id", list.get(i).get("group_id"));
							
							medPurDetailMap.put("hos_id", list.get(i).get("hos_id"));
							
							medPurDetailMap.put("copy_code", list.get(i).get("copy_code"));
							
							medPurDetailMap.put("pur_id", pur_id);
							medPurDetailMap.put("pur_code", medPurMainMap.get("pur_code"));
							
							medPurDetailMap.put("inv_id", list.get(i).get("inv_id"));
							
							medPurDetailMap.put("inv_no", list.get(i).get("inv_no"));
							
							medPurDetailMap.put("sup_no", list.get(i).get("sup_no"));
							
							medPurDetailMap.put("sup_id", list.get(i).get("sup_id"));
							
							medPurDetailMap.put("price", list.get(i).get("price"));
							
							medPurDetailMap.put("req_amount", list.get(i).get("sum_amount"));
							medPurDetailMap.put("amount",list.get(i).get("sum_amount"));
							//手动筛选药品如果与上一条药品一致则不添加到list中
							if(inv_id != null){
								if(!inv_id.equals(String.valueOf(medPurDetailMap.get("inv_id")))){
									
									inv_id = String.valueOf(medPurDetailMap.get("inv_id"));
									medPurDetailList.add(medPurDetailMap);
								}
							}else{
								inv_id = String.valueOf(medPurDetailMap.get("inv_id"));
								medPurDetailList.add(medPurDetailMap);
						...
						medPurRelaMap = new HashMap<String,Object>();
						
						//添加对应关系
						medPurRelaMap.put("group_id", list.get(i).get("group_id"));
						
						medPurRelaMap.put("hos_id", list.get(i).get("hos_id"));
						
						medPurRelaMap.put("copy_code", list.get(i).get("copy_code"));
						
						medPurRelaMap.put("total_id", pur_id);
						
						medPurRelaMap.put("pur_id", list.get(i).get("pur_id"));
						
						medPurRelaMap.put("inv_id", list.get(i).get("inv_id"));
						
						medPurRelaMap.put("pur_amount", list.get(i).get("amount"));
						
						medPurRelaMap.put("exec_amount", list.get(i).get("sum_amount"));
						
						medPurRelaMap.put("state", 3);//用于修改采购计划为已执行
						
						medPurRelaList.add(medPurRelaMap);
					}
				}
				medPurMainList.add(medPurMainMap);
			}
		}
		
		if(medPurMainList.size() ==0 ){
			
			return "{\"error\":\"汇总失败 主表数据不能为空! 错误编码 genByCollectMedPurMain\"}";
			
		}
		
		if(medPurDetailList.size() ==0 ){
			
			return "{\"error\":\"汇总失败 明细数据不能为空! 错误编码 genByCollectMedPurMain\"}";
			
		}
		
		if(medPurRelaList.size() ==0 ){
			
			return "{\"error\":\"汇总失败 对应关系不存在! 错误编码 genByCollectMedPurMain\"}";
			
		}
		
		medPurMainCollectMapper.addBatch(medPurMainList);//添加主表数据
		
		medPurMainCollectMapper.addBatchMedPurDetail(medPurDetailList);//添加明细表数据
		
		medPurMainCollectMapper.addMedPurRela(medPurRelaList);//添加对应关系
		
		medPurMainCollectMapper.updateBatchPurState(medPurRelaList);//修改采购计划状态为已执行
	...
	public String queryMedPurMainAmountDetail(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<?> list = medPurMainCollectMapper.queryMedPurMainAmountDetail(entityMap);
