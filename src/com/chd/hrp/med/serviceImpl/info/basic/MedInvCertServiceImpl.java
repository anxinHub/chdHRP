...
import com.chd.hrp.med.dao.info.basic.MedCertSupMapper;
import com.chd.hrp.med.dao.info.basic.MedInvCertMapper;
...
@Service("medInvCertService")
public class MedInvCertServiceImpl implements MedInvCertService {

	private static Logger logger = Logger.getLogger(MedInvCertServiceImpl.class);
	//引入DAO操作
	@Resource(name = "medInvCertMapper")
	private final MedInvCertMapper medInvCertMapper = null;
	
	@Resource(name = "medInvCertRelaMapper")
	private final MedInvCertRelaMapper medInvCertRelaMapper = null;
	
	@Resource(name = "medCertSupMapper")
	private final MedCertSupMapper medCertSupMapper = null;
	...
	*/
	...
	public String addMedInvCert(Map<String,Object> entityMap)throws DataAccessException, ParseException{
		...
		List<MedInvCert> list = medInvCertMapper.queryMedInvCertByID(entityMap);
		if (list.size() >0) {
	...
	/*	Boolean flag = compare_date(String.valueOf(entityMap.get("start_date")),String.valueOf(entityMap.get("end_date")));
		...
		try {
			entityMap.put("cert_id", medInvCertMapper.queryMedInvCertNextId());
			...
			int state = medInvCertMapper.addMedInvCert(entityMap);
			if(orderRelaList.size()>0){
				medInvCertRelaMapper.addBatchMedInvCertRela(orderRelaList);
			}
	...
	public String addBatchMedCertSup(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			...
			for(Map<String,Object> item: entityList){
				...
				MedInvCert invCert = medInvCertMapper.queryMedInvCertByCode(item);
				
				if( invCert == null){
					item.put("cert_id", medInvCertMapper.queryMedInvCertNextId());
				...
				//药品材料主页面 修改证件信息后 重新设置 材料证件关系  （只用于 药品材料主页面 修改证件信息）
				if(item.get("inv_id") != null && !"".equals(item.get("inv_id"))){
					...
					inv_id = String.valueOf(item.get("inv_id")) ;//记录材料Id(药品材料材料主页面  修改证件时 删除材料证件 关系用)
			...
			if(certList.size() > 0){
				...
				medInvCertMapper.addBatchMedInvCert(certList);
			}
			
			if(updateList.size() > 0){
				
				medInvCertMapper.updateBatchMedInvCert(updateList);
			}
			
			if(invCertList.size() > 0 ){
				medInvCertRelaMapper.deleteMedInvCertRela(delMap);
				
				medInvCertRelaMapper.addBatchMedInvCertRela(invCertList);
			}
			
			if(certSupList.size()>0){
				medCertSupMapper.deleteBatch(entityList);
				medCertSupMapper.addBatch(certSupList);
			}
			...
			//return "{\"error\":\"添加失败 数据库异常 请联系管理员! 方法 addBatchMedInvCert\"}";
	...
	*/
	...
	public String updateMedInvCert(Map<String,Object> entityMap)throws DataAccessException, ParseException{
		...
		try {
			...
			if(entityMap.get("supData") != null && !"".equals(entityMap.get("supData"))){
				...
				while (it.hasNext()) {
					...
					if( jsonObj.get("sup_id") != null && !"".equals(jsonObj.get("sup_id"))){
						...
						supMap.put("sup_id",  jsonObj.get("sup_id").toString().split(",")[0]);//材料ID
			...
			int state = medInvCertMapper.updateMedInvCert(entityMap);
			if(insertList.size()>0){
				medInvCertRelaMapper.addBatchMedInvCertRela(insertList);
			}
			if(updateList.size()>0){
				medInvCertRelaMapper.deleteBatchMedInvCertRela(updateList);
			}
			
			medCertSupMapper.delete(entityMap);
			if(certSupList.size()>0){
				medCertSupMapper.addBatch(certSupList);
			}
	...
	*/
	@Override
	public String updateBatchMedInvCert(List<Map<String,Object>> entityList)throws DataAccessException{
		
		try {
			
		  medInvCertMapper.updateBatchMedInvCert(entityList);
			...
			return "{\"error\":\"删除失败 数据库异常 请联系管理员! 方法 updateBatchMedInvCert\"}";
	...
	*/
    ...
    public String deleteMedInvCert(Map<String, Object> entityMap) throws DataAccessException {
	    ...
	    try {
				medInvCertMapper.deleteMedInvCert(entityMap);
	...
	*/
	@Override
	public String deleteBatchMedInvCert(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			...
			medInvCertMapper.deleteBatchMedInvCert(entityList);
	...
	*/
	...
	public String queryMedInvCert(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			...
			List<MedInvCert> list = medInvCertMapper.queryMedInvCertNew(entityMap);
			...
			List<MedInvCert> list = medInvCertMapper.queryMedInvCertNew(entityMap, rowBounds);
	...
	*/
	...
	public MedInvCert queryMedInvCertByCode(Map<String,Object> entityMap)throws DataAccessException{
		return medInvCertMapper.queryMedInvCertByCode(entityMap);
	}
	...
	*/
	...
	public MedInvCert queryMedInvCertByUniqueness(Map<String,Object> entityMap) throws DataAccessException{
		return medInvCertMapper.queryMedInvCertByUniqueness(entityMap);
	}
	/**
	...
	public MedInvCert queryMedInvCertByCodeNew(Map<String, Object> mapVo) throws DataAccessException{
		return medInvCertMapper.queryMedInvCertByCodeNew(mapVo);
	}
	/**
	 ...
	 * @throws DataAccessException
	 */
	public List<MedInvCert> queryMedInvCertByID(Map<String, Object> mapVo) throws DataAccessException{
		return medInvCertMapper.queryMedInvCertByID(mapVo);
	}
	/**
	...
	public int queryDelDate(Map<String, Object> mapVo) throws DataAccessException{
		return medInvCertMapper.queryDelDate(mapVo);
	}
	...
	public String queryMedCertInvList(Map<String, Object> entityMap) throws DataAccessException{
		...
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal()==-1){
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(medInvCertMapper.queryMedCertInvList(entityMap));
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			
			List<Map<String,Object>> list = medInvCertMapper.queryMedCertInvList(entityMap, rowBounds);
	...
	public String queryMedInvCertDetail(Map<String, Object> entityMap) throws DataAccessException{
		...
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal()==-1){
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(medInvCertRelaMapper.queryMedInvCertDetail(entityMap));
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<Map<String,Object>> list = medInvCertRelaMapper.queryMedInvCertDetail(entityMap, rowBounds);
			PageInfo page = new PageInfo(list);
	...
	public String queryMedCertInvChoiceInvList(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<Map<String, Object>> list= medInvCertMapper.queryMedCertInvChoiceInvList(detailList);
	 ...
	 * @return
	 */
	public String queryMedCerSup(Map<String, Object> entityMap) {
		...
		if (sysPage.getTotal()==-1){
			...
			List<Map<String,Object>> list = (List<Map<String, Object>>) medCertSupMapper.query(entityMap);
			...
			List<Map<String,Object>> list = (List<Map<String, Object>>) medCertSupMapper.query(entityMap, rowBounds);
	...
	*/
	...
	public String auditMedInvCertBatch(List<Map<String, Object>> entityMap) throws DataAccessException {
		...
		try {	
			//批量审核
			medInvCertMapper.auditOrUnAuditBatch(entityMap);
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	*/
	...
	public String importMedInvCert(Map<String, Object> entityMap) throws DataAccessException {
		...
		try {	
			...
			//证件列表
			List<Map<String, Object>> certRList = medInvCertMapper.queryCertListForImport(seachMap);
			...
			//证件类别列表
			List<Map<String, Object>> certTypeList = medInvCertMapper.queryCertTypeListForImport(seachMap);
			...
			//生产厂商列表
			List<Map<String, Object>> facList = medInvCertMapper.queryFacListForImport(seachMap);
			...
			//材料列表
			List<Map<String, Object>> invList = medInvCertMapper.queryInvListForImport(seachMap);
			...
			//供应商列表
			List<Map<String, Object>> supList = medInvCertMapper.querySupListForImport(seachMap);
			...
			for(Map<String, List<String>> dataMap : dataList){
				...
				if(certRMap.containsKey(certKey)){
					...
					if(certMap.containsKey(certKey)){
						...
						map.put("cert_id", medInvCertMapper.queryMedInvCertNextId());
			...
			if(invRelaList.size()>0){
				medInvCertRelaMapper.addBatchMedInvCertRela(invRelaList);
			}
			//批量添加证件供应商关系
			if(supRelaList.size()>0){
				medCertSupMapper.addBatch(supRelaList);
			}
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
