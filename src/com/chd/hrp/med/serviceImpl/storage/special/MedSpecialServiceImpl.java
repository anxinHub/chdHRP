...
package com.chd.hrp.med.serviceImpl.storage.special;
...
import com.chd.hrp.med.dao.storage.special.MedSpecialAffiOutRelaMapper;
 ...
 * @Version: 1.0
 */
@Service("medSpecialService")
public class MedSpecialServiceImpl implements MedSpecialService {

	private static Logger logger = Logger.getLogger(MedSpecialServiceImpl.class);
	//引入DAO操作
	...
	@Resource(name = "medSpecialAffiOutRelaMapper")
	private final MedSpecialAffiOutRelaMapper medSpecialAffiOutRelaMapper = null;
	...
	*/
	...
	public String add(Map<String,Object> entityMap)throws DataAccessException{
		try {
			...
			//入库日期需在系统启用日期之前
			/*int flag = medCommonMapper.existsInDateCheckBefore(entityMap);
			if(flag == 0){
			...
			if("自动生成".equals(entityMap.get("special_no"))){
				entityMap.put("special_no", medCommonService.getMedNextNo(specialMap));
			}
			//专购品单据ID
			entityMap.put("special_id", medSpecialMapper.queryMedSpecialNextval());
			...
			inMap.put("prefixe", "");
			entityMap.put("in_no", medCommonService.getMedNextNo(inMap));
			...
			//要添加专购品出库主表的Id
			entityMap.put("out_id", medOutMainMapper.queryMedOutMainSeq());
			...
			outMap.put("prefixe", "");
			entityMap.put("out_no", medCommonService.getMedNextNo(outMap));
			...
			if(entityMap.get("detailData") != null && !"".equals(entityMap.get("detailData"))){
					...
					if( !"".equals(jsonObj.get("inv_id")) && jsonObj.get("inv_id") != null){
						...
						if((!ChdJson.validateJSON(String.valueOf(jsonObj.get("is_per_bar"))) || "0".equals(String.valueOf(jsonObj.get("is_per_bar")))) && ("0".equals(String.valueOf(jsonObj.get("is_highvalue"))) || "0".equals(String.valueOf(MyConfig.getSysPara("08010"))))){
							...
							for(int i = 1; i <= Integer.parseInt(detailMap.get("amount").toString()); i++){
								...
								if( i>1){
									barMap.put("detail_id", medSpecialMapper.queryMedSpecialDetailSeq());
								}
				...
				//新增专购品主表数据
				medSpecialMapper.addSpecialMain(entityMap);
				
				//新增专购品入库主表数据
				medSpecialMapper.addInMain(entityMap);
				
				//新增专购品出库主表数据
				medSpecialMapper.addOutMain(entityMap);
				...
				//新增出库资金来源表
				medOutResourceMapper.add(entityMap);
				
				//新增入库单订单关系
				if(orderRelaList.size() > 0){
					medSpecialMapper.insertOrderRela(orderRelaList);
				}
				
				//新增专购品订单关系
				if(orderRelaList.size() > 0){
					medSpecialMapper.addSpecialOrderRelaBatch(orderRelaList);
				}
			}else{
	...
	*/
	...
	public String addBatch(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			...
			return "{\"error\":\"添加失败 数据库异常 请联系管理员! 方法 addBatchMedInitInCommon\"}";
		}
	}
	...
	*/
	...
	public String update(Map<String,Object> entityMap)throws DataAccessException{
		...
		try {
			...
			Map<String,Object> data = medSpecialMapper.queryMedInOutData(entityMap);
			...
			//删除专购品 明细数据
			medSpecialMapper.deleteSpecialDetail(deleteMap);
			//删除入库单明细数据
			medInCommonMapper.deleteMedInDetail(deleteMap);
			//删除出库单明细数据
			medOutDetailMapper.delete(deleteMap);
			//组装明细
			if(entityMap.get("detailData") != null && !"".equals(entityMap.get("detailData"))){
				float money_sum = 0;//记录明细总金额
				//用于查询个体码
				...
				while (it.hasNext()) {
					...
					if( !"".equals(jsonObj.get("inv_id")) && jsonObj.get("inv_id") != null){
						...
						if(!ChdJson.validateJSON(String.valueOf(jsonObj.get("detail_id")))){
							// 专购品明细表 ID
							detailMap.put("detail_id",  medSpecialMapper.queryMedSpecialDetailSeq());
							detailMap.put("in_detail_id", medInCommonMapper.queryMedInDetailSeq());//专购品入库明细表id
							detailMap.put("out_detail_id", medSpecialMapper.queryMedOutDetailSeq());//专购品入库明细表id
							//生成个体码
							if((!ChdJson.validateJSON(String.valueOf(jsonObj.get("is_per_bar"))) || "0".equals(String.valueOf(jsonObj.get("is_per_bar")))) && ("0".equals(String.valueOf(jsonObj.get("is_highvalue"))) || "0".equals(String.valueOf(MyConfig.getSysPara("08010"))))){
				...
				//修改 专购品主表数据
				medSpecialMapper.updateSpecialMain(entityMap);
				//修改 专购品入库主表数据
				medInCommonMapper.updateMedInMain(entityMap);
				//修改 专购品出库主表数据
				medSpecialMapper.updateMedOutMain(entityMap);
				...
				if(detail_ids.length() > 0){
					...
					//删除页面上删掉的  专购品明细记录
					medSpecialMapper.deleteMedSpecialDetailForUpdate(pageMap);
				}*/
					
				//批量添加  专购品明细记录
				if(detailAddList.size() > 0){
					medSpecialMapper.addMedSpecialDetailBatch(detailAddList);
				}
			/*	//修改 专购品明细数据
				if(detailUpdateList.size() > 0){
					medSpecialMapper.updateMedSpecialDetailBatch(detailUpdateList);
				}*/
				
				//批量添加 专购品入库明细数据
				medInCommonMapper.addMedInDetailBatch(addInDetailList);
				
				
				//批量添加 专购品出库明细数据
				medSpecialMapper.addMedOutDetailBatch(addOutDetailList);
				
				entityMap.put("source_money", money_sum);
				//修改资金来源表
				if(entityMap.get("old_money_sum") == null || "".equals(entityMap.get("old_money_sum").toString()) || (Float.parseFloat(entityMap.get("old_money_sum").toString()) != Float.parseFloat(entityMap.get("source_money").toString()))){
					//入库
					medInCommonMapper.updateMedInResource(entityMap);
					//出库
					medSpecialMapper.updateMedOutResource(entityMap);
				}
				
				//删除入库单订单关系表
				medSpecialMapper.deleteOrderRela(entityMap);
				
				//删除 专购品订单关系
				medSpecialMapper.deleteSpecialOrderRela(entityMap);
	...
	*/
	...
	public String delete(Map<String, Object> entityMap) throws DataAccessException {
		...
		try {
			...
			//删除明细
			int state = medInCommonMapper.deleteMedInDetail(entityMap);
			...
			return "{\"error\":\"删除失败 数据库异常 请联系管理员! 方法 deleteMedInitInCommon\"}";
		}	
  }
	...
	*/
	...
	public String deleteBatch(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {	
			...
			//批量删除专购品明细数据
			int  special = medSpecialMapper.deleteSpecialDetailBatch(entityList);
			
			//批量删除专购品入库单明细表
			int state = medInCommonMapper.deleteMedInDetailBatch(entityList);
			
			//批量删除专购品出库单明细表
			int count = medOutDetailMapper.deleteBatch(entityList);
			
			//删除入库资金来源表记录
			medSpecialMapper.deleteMedInSourceBatch(entityList);
			
			//删除出库资金来源表记录
			medSpecialMapper.deleteMedOutSourceBatch(entityList);
			
			//批量删除专购品主数据
			medSpecialMapper.deleteBatch(entityList);
			...
			//删除专购品出入库对应关系
			medSpecialMapper.deleteBatchSpecialRela(entityList);
			
			//删除入库单订单关系
			medSpecialMapper.deleteInOrderRelaBatch(entityList);
			//删除 专购品订单关系
			medSpecialMapper.deleteSpecialOrderRelaBatch(entityList);
			//删除专购品和代销对应关系
			medSpecialAffiOutRelaMapper.deleteMedSpecialAffiOutRelaBatch(entityList);
			return "{\"msg\":\"删除成功.\",\"state\":\"true\"}";
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	//冲账
	@Override
	public String offsetMedSpecialBatch(List<Map<String, Object>> listvo) throws DataAccessException {
		...
		try {
			...
			for(Map<String, Object> m : listvo){
				...
				Map<String, Object> specialMap = medSpecialMapper.queryMedSpecialById(m);
				// 构建冲销后 专购品主表 MED_SPECIAL_MAIN 数据 
				...
				map.put("table_code", "MED_SPECIAL_MAIN");
				map.put("bus_type_code", "48");// 业务类型改为 专购品退货（48）
				map.put("prefixe", "");
				specialMap.put("special_no", medCommonService.getMedNextNo(map));//重新获取专购品主表单据号
				specialMap.put("special_id", medSpecialMapper.queryMedSpecialNextval());//重新获取专购品主表Id
				
				specialList.add(specialMap);
				//查询 对应的 专购品明细数据 
				List<Map<String,Object>> detailList = medSpecialMapper.querySpecialDetail(m);
				...
				for(Map<String,Object> item : detailList){
					...
					item.put("detail_id", medSpecialMapper.queryMedSpecialDetailSeq());//重新获取专购品明细ID
				...
				//根据 ID查询专购品入库单主表 数据
				Map<String, Object> inMainMap = medSpecialMapper.queryInMainDataById(m);
				
				// 构建冲销后 专购品入库主表 MED_IN_MAIN 数据 
				...
				inMainMap.put("in_no", medCommonService.getMedNextNo(inMap));//重新获取专购品入库单 单据号
				inMainMap.put("in_id", medInCommonMapper.queryMedInMainSeq());//重新获取专购品入库单 Id
				
				inList.add(inMainMap);
				
				List<Map<String,Object>> medInDetailList = medSpecialMapper.queryMedInDetailById(m);
				
				//循环 构建冲销后的 专购品入库 明细数据
				for(Map<String,Object> item : medInDetailList){
					item.put("in_id", inMainMap.get("in_id"));// 替换 新的专购品入库单 ID
					item.put("in_no", inMainMap.get("in_no"));// 替换 新的专购品入库单 单据号
					item.put("in_detail_id", medInCommonMapper.queryMedInDetailSeq());//重新获取专购品入库单 明细ID
				...
				//根据 ID查询专购品出库单主表 数据
				Map<String, Object> outMainMap = medSpecialMapper.queryMedOutMainByCode(m);
				...
				outMainMap.put("out_no", medCommonService.getMedNextNo(outMap));//重新获取专购品出库单 单据号
				outMainMap.put("out_id", medOutMainMapper.queryMedOutMainSeq());//重新获取专购品出库单 Id （参数无关紧要）
				
				outList.add(outMainMap);
				
				List<Map<String,Object>> medOutDetailList = medSpecialMapper.queryMedOutDetailById(m);
				
				//循环 构建冲销后的 专购品出库 明细数据
				for(Map<String,Object> item : medOutDetailList){
					item.put("out_id", outMainMap.get("out_id"));// 替换 新的专购品出库单 ID
					item.put("out_no", outMainMap.get("out_no"));// 替换 新的专购品出库单 单据号
					item.put("out_detail_id", medSpecialMapper.queryMedOutDetailSeq());//重新获取专购品出库单 明细ID
				...
				//查询入库资金来源
				Map<String,Object> inResource = medSpecialMapper.queryMedInResource(m);
				...
				//查询出库资金来源
				Map<String,Object> outResource =  medSpecialMapper.queryMedOutResource(m);
				
				//组装  专购品 冲销 出库 资金来源数据
				outResource.put("out_id", outMainMap.get("out_id"));
				if(outResource.containsKey("source_money")){
			...
			//批量新增专购品主表数据
			medSpecialMapper.addMedSpecialMainBatch(specialList);
			//批量新增专购品明细数据
			medSpecialMapper.addMedSpecialDetailBatch(specialDetailList);
			
			// 冲账 批量新增  专购品入库 主表数据
			medInCommonMapper.addMedInMainBatch(inList);
			//冲账 批量新增 专购品入库 明细表数据
			medInCommonMapper.addMedInDetailBatch(inDetailList);
			
			//冲账 批量新增  专购品入库 主表数据
			medSpecialMapper.addOutMainBatch(outList);
			
			//冲账 批量新增 专购品入库 明细表数据
			medSpecialMapper.addMedOutDetailBatch(outDetailList);
			
			//冲账 批量新增 专购品入库 资金来源 数据
			//medSpecialMapper.addInResourceBatch(inResourceList);
			medInCommonMapper.insertMedInResourceBatch(inResourceList);
			//冲账 批量新增 专购品出库 资金来源 数据
			//medSpecialMapper.addOutResourceBatch(outResourceList); 
			medOutResourceMapper.addBatch(outResourceList);
			//冲账 批量新增 专购品出入库对应关系 数据
			medSpecialMapper.addSpecialRelaBatch(relaList);
	...
	public String updateState(List<Map<String, Object>> listVo) throws DataAccessException {
		...
		try {	
			//专购品 入库单批量审核、消审
			medSpecialMapper.updateStateSpecial(listVo);
			
			//专购品 入库单批量审核、消审
			medSpecialMapper.updateStateIn(listVo);
			
			//专购品 出库单批量审核、消审
			medSpecialMapper.updateStateOut(listVo);
			return "{\"msg\":\"操作成功.\",\"state\":\"true\"}";
		}
		catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	//专购品 出入库 确认
	@Override
	public synchronized String confirmMedSpecialBatch(List<Map<String, Object>> listVo) throws DataAccessException {
		...
		try {	
			//判断所选单据是否已结账
			int flag = medSpecialMapper.existsMedSpecialStateForConfrim(listVo);
			if(flag != 0){
			...
			for(Map<String,Object> item : detailList){
				...
				//判断库房是否已结账
				String store_flag = medCommonMapper.queryMedStoreIsAccount(item);
	...
	*/
	...
	public String addOrUpdate(Map<String,Object> entityMap)throws DataAccessException{
		...
		if (medInMain != null) {
			...
			int state = medSpecialMapper.update(entityMap);
		...
		try {
			...
			int state = medSpecialMapper.add(entityMap);
	...
	*/
	@Override
	public String queryMedSpecial(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			List<?> list = medSpecialMapper.queryMedSpecial(entityMap);
			
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<?> list = medSpecialMapper.queryMedSpecial(entityMap, rowBounds);
	...
	*/
	@Override
	public String queryMedSpecialDetailByCode(Map<String,Object> entityMap)throws DataAccessException{

		List<Map<String,Object>> list = medSpecialMapper.queryMedSpecialDetailByCode(entityMap);
	 ...
	 * @param  entityMap<BR>
	 *  参数为要检索的字段
	 * @return MedInitInCommon
	 * @throws DataAccessException
	*/
	
	public  <T>T queryByUniqueness(Map<String,Object> entityMap)throws DataAccessException{
		return medSpecialMapper.queryByUniqueness(entityMap);
	}
	...
	public String queryOrder(Map<String, Object> entityMap) throws DataAccessException {
		...
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal()==-1){
			List<?> list = medSpecialMapper.queryOrder(entityMap);
			
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<?> list = medSpecialMapper.queryOrder(entityMap, rowBounds);
			PageInfo page = new PageInfo(list);
	...
	public String queryOrderDetail(List<Map<String, Object>> entityMap) throws DataAccessException {
		...
		List<?> list = medSpecialMapper.queryOrderDetail(orderMap);
	...
	public String queryAffiOut(Map<String, Object> entityMap) throws DataAccessException {
		...
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal()==-1){
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(medSpecialMapper.queryAffiOut(entityMap));
			
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<Map<String,Object>> list = medSpecialMapper.queryAffiOut(entityMap, rowBounds);
			PageInfo page = new PageInfo(list);
	 ...
	 */
	@Override
	public String queryMedAffiDetail(Map<String, Object> entityMap) throws DataAccessException {
		
		List<?> list = medSpecialMapper.queryMedAffiDetail(entityMap);
	...
	public String queryProtocol(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<?> list = medSpecialMapper.queryProtocol(entityMap);
	...
	public Map<String, Object> queryMedSpecialMainUpdate(Map<String, Object> mapVo) throws DataAccessException{
		...
		return medSpecialMapper.queryMedSpecialMainUpdate(mapVo);
	}
	/**
	...
	public Map<String,Object> queryMedInOutData(Map<String, Object> mapVo) throws DataAccessException{
		return medSpecialMapper.queryMedInOutData(mapVo);
	}
	/**
	...
	public List<Map<String, Object>> querySpecialDetail(Map<String, Object> mapVo) throws DataAccessException{
		return medSpecialMapper.querySpecialDetail(mapVo);
	}
	/**
	 * 查询 专购品出库单序列
	 * @param mapVo
	 ...
	 * @throws DataAccessException
	 */
	public Long queryMedOutMainNextval(Map<String, Object> mapVo) throws DataAccessException{
		return medOutMainMapper.queryMedOutMainSeq();
	}
	/**
	 * 查询 专购品入库单序列
	 * @return
	...
	public Long queryMedInMainSeq() throws DataAccessException{
		return   medInCommonMapper.queryMedInMainSeq();
	}
	 ...
	 */
	@Override
	public String queryMedSpecialBeforeOrNextNo(Map<String, Object> mapVo) throws DataAccessException{
		try {	
			...
			if("-1".equals(mapVo.get("flag").toString())){
				map = medSpecialMapper.queryMedSpecialBefore(mapVo);
				if(map == null){
					return "{\"state\":\"false\"}";
				}
			//下一张
			}else if("1".equals(mapVo.get("flag").toString())){
				map = medSpecialMapper.queryMedSpecialNext(mapVo);
				if(map == null ){
					return "{\"state\":\"false\"}";
				}
			}else{
				return "{\"error\":\"操作失败 页面参数异常 请联系管理员！方法queryMedSpecialBeforeOrNextNo\"}";
			}
			...
			return "{\"error\":\"操作失败 数据库异常 请联系管理员! 方法 queryMedSpecialBeforeOrNextNo\"}";
		}	
	}
	@Override
	...
	public int updateAffiOutState(Map<String, Object> mapVo) throws DataAccessException {
		return medSpecialMapper.updateAffiOutState(mapVo);
	}
	...
	@Override
	public String queryMedSpecialByPrintTemlate(Map<String, Object> entityMap) throws DataAccessException {
		// TODO Auto-generated method stub
		//查询入库主表

		if(entityMap.get("p_num") != null && "1".equals(entityMap.get("p_num").toString())){
			List<Map<String,Object>> map=medSpecialMapper.queryMedSpecialPrintTemlateByMainBatch(entityMap);
			//查询入库明细表
			List<Map<String,Object>> list=medSpecialMapper.queryMedSpecialPrintTemlateByDetail(entityMap);
			
			return superPrintService.getBatchListByPrintTemplateJson(entityMap,map,list);
			
		}else{
			Map<String,Object> map=medSpecialMapper.queryMedSpecialPrintTemlateByMain(entityMap);
			...
			if(entityMap.get("is_collect") != null && "1".equals(entityMap.get("is_collect").toString())){
				...
				list=medSpecialMapper.queryMedSpecialPrintTemlateByDetailCollect(entityMap);
			}else{
				
				list=medSpecialMapper.queryMedSpecialPrintTemlateByDetail(entityMap);
			}
	...
	public String addByAffiOut(Map<String, Object> entityMap) throws DataAccessException {
		try{
			...
			specialCodeMap.put("table_code", "MED_SPECIAL_MAIN");
			...
			inCodeMap.put("table_code", "MED_IN_MAIN");
			...
			outCodeMap.put("table_code", "MED_OUT_MAIN");
			...
			//查询所选主从表信息
			List<Map<String, Object>> spcList = JsonListMapUtil.toListMapLower(medSpecialMapper.queryAffiForAdd(entityMap));
			...
			for(Map<String, Object> map : spcList){
				...
				String store_flag = medCommonMapper.queryMedStoreIsStart(checkMap);
				if(store_flag != null && !"".equals(store_flag)){
					storeMsg.append(store_flag).append("<br/>");
					continue;
				}
				...
				if(spcMain.containsKey(spcMainKey)){
					...
					in_no = medCommonService.getMedNextNo(inCodeMap);
					inMainMap.put("in_no", in_no);
					in_id = medInCommonMapper.queryMedInMainSeq();
					...
					out_no = medCommonService.getMedNextNo(outCodeMap);
					outMainMap.put("out_no", out_no);
					out_id = Long.valueOf(medOutMainMapper.queryMedOutMainSeq());
				...
				if(spcDetail.containsKey(spcDetailKey)){
					...
					detailMap.put("detail_id", String.valueOf(medSpecialMapper.queryMedSpecialDetailSeq()));	//明细ID
					...
					outDetailMap.put("out_detail_id", medSpecialMapper.queryMedOutDetailSeq());
			...
			if(!isContainsSup){
				throw new SysException("{\"error\":\"请选择代销出库药品供应商不能为空\"}");
			}
			...
			if(spcDetailList.size() > 0){
				//插入专购品主表
				medSpecialMapper.addMedSpecialMainBatch(spcMainList);
				//插入专购品明细
				medSpecialMapper.addMedSpecialDetailBatch(spcDetailList);
				//插入入库主表
				...
				//插入出库主表
				medSpecialMapper.addMedOutMainBySpecialBatch(outMainList);
				//插入出库明细表
				medSpecialMapper.addMedOutDetailBySpecialBatch(outDetailList);
				//插入出库资金来源表
				medOutResourceMapper.addBatch(outMainList);
				//插入与代销的对应关系
				medSpecialAffiOutRelaMapper.addMedSpecialAffiOutRelaBatch(affiRelaList);
				//插入与出入库的对应关系
				medSpecialMapper.addMedSpecialRelaBatch(spcRelaList);
			}else{
	...
	//代销生成专购品 查询
	@Override
	public String queryMedAffiSpecial(Map<String, Object> entityMap) throws DataAccessException {
		SysPage sysPage = new SysPage();
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal()==-1){
			List<?> list = medSpecialMapper.queryMedAffiSpecial(entityMap);
			
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<?> list = medSpecialMapper.queryMedAffiSpecial(entityMap, rowBounds);
			PageInfo page = new PageInfo(list);
	...
	*/
	@Override
	public String updateMedSpecialInvoice(Map<String, Object> entityMap) throws DataAccessException {
		try {	
			//更新专购品发票号
			medSpecialMapper.updateMedSpecialInvoice(entityMap);  
			//更新入库单发票号
	 ...
	 */
	@Override
	public String queryMedSpecailInvBatch(Map<String, Object> entityMap) throws DataAccessException {
		SysPage sysPage = new SysPage();
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal()==-1){
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(medSpecialMapper.queryMedSpecailInvBatch(entityMap));
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(medSpecialMapper.queryMedSpecailInvBatch(entityMap, rowBounds));
			PageInfo page = new PageInfo(list);
			return ChdJson.toJson(list, page.getTotal());
		}
	}
	 ...
	 */
	@Override
	public String queryMedSpecailInvDetail(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<Map<String,Object>> list=medSpecialMapper.queryMedSpecailInvDetail(invList);
	...
	public String queryDetails(Map<String, Object> entityMap) throws DataAccessException {
		...
		sysPage = (SysPage) entityMap.get("sysPage");
		if (sysPage.getTotal()==-1){
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(medSpecialMapper.queryMedSpecialDetail(entityMap));
			
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<Map<String,Object>> list = JsonListMapUtil.toListMapLower(medSpecialMapper.queryMedSpecialDetail(entityMap, rowBounds));
			PageInfo page = new PageInfo(list);
	 ...
	 */
	@Override
	public String importMedCommonInsup(Map<String, Object> map,Map<String,Object> entityMap)throws DataAccessException {
		// TODO Auto-generated method stub
		try {
			...
			List<Map<String,Object>> listSpe = medCommonMapper.queryMedInvListSpecial(entityMap);
			...
			for(Map<String, List<String>> item:liData){
				for(String st :item.keySet()){
					...
					List<String> inv_code = item.get("药品编码");
					List<String> inv_name = item.get("药品名称");
					...
					*/
					...
					if(inv_code.get(1) == null){
						return "{\"warn\":\"药品编码为空！\",\"state\":\"false\",\"row_cell\":" +inv_code.get(0) +"\"}";
					}else if(inv_name.get(1) == null){
						return "{\"warn\":\"药品名称为空！\",\"state\":\"false\",\"row_cell\":" +inv_name.get(0) +"\"}";
					...
					for(Map<String,Object> mapSpe :listSpe){//inv_name.get(1).equals(mapSpe.get("inv_name").toString()) && 
						...
						if(inv_code.get(1).equals(invCode)){
							...
							returnMap.put("med_type_id", mapSpe.get("med_type_id"));
							returnMap.put("med_type_name", mapSpe.get("med_type_name"));
							...
							List<HrpMedSelect> listCertId = hrpMedSelectMapper.queryMedInvCertId(entityMap);
							for(HrpMedSelect h :listCertId){
			...
			if(listMap != null && listMap.size() > 0){
				...
				return "{\"warn\":\"药品编码不存在！！\",\"state\":\"false\",\"row_cell\":" + "1" +"\"}";
			}
	...
	@Override
	public Map<String, Object> queryMedSpecialByPrintTemlate1(Map<String, Object> entityMap) throws DataAccessException {
		// TODO Auto-generated method stub
		//查询入库主表

		Map<String,Object> reMap=new HashMap<String,Object>();
		WebApplicationContext context = ContextLoader.getCurrentWebApplicationContext();
		MedSpecialMapper medSpecialMapper = (MedSpecialMapper)context.getBean("medSpecialMapper");
