...
import com.chd.hrp.med.dao.termend.MedFinalChargeMapper;
import com.chd.hrp.med.entity.MedFinalCheck;
import com.chd.hrp.med.service.termend.MedFinalChargeService;
import com.github.pagehelper.PageInfo;

@Service("medFinalChargeService")
public class MedFinalChargeServiceImpl implements MedFinalChargeService {

	private static Logger logger = Logger.getLogger(MedUnconfirmBillServiceImpl.class);
	//引入DAO操作
	@Resource(name = "medFinalChargeMapper")
	private final MedFinalChargeMapper medFinalChargeMapper = null;
	...
	public String query(Map<String, Object> entityMap)
		...
		if (sysPage.getTotal()==-1){
			List<MedFinalCheck> list = (List<MedFinalCheck>)medFinalChargeMapper.query(entityMap);
			
			return ChdJson.toJson(list);
		}else{
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<MedFinalCheck> list = (List<MedFinalCheck>)medFinalChargeMapper.query(entityMap, rowBounds);
	...
	@Override
	public String updateMedFinalCharge(Map<String, Object> entityMap)
			throws DataAccessException {
		try {
			/*---20161121---hsy--由于确认出入库时直接修改期间年月所以去掉该限制*/
			//验证是否存在未确认单据
			/*Integer count=medFinalChargeMapper.existsMedFinalUnconfirm(entityMap);
			...
			int flag = medCommonMapper.existsMedYearMonthCheck(existsMap);
			if(flag == 0){
			...
			if(by_store == 0){
				medFinalChargeMapper.updateMedFinalCharge(entityMap);
			}else{
				medFinalChargeMapper.updateMedFinalChargeStore(entityMap);
			}
	...
	@Override
	public String updateMedFinalInverse(Map<String, Object> entityMap)
			throws DataAccessException {
		try {
			/*---20161121---hsy--由于确认出入库时直接修改期间年月所以去掉该限制*/
			...
			if(by_store == 0){
				medFinalChargeMapper.updateMedFinalInverse(entityMap);
			}else{
				medFinalChargeMapper.updateMedFinalInverseStore(entityMap);
			}
			//System.out.println(entityMap.get("msg").toString());
			String msg = entityMap.get("msg").toString();
	...
	public String queryYearMonthByStoreSet(Map<String, Object> entityMap) throws DataAccessException {
		...
		Map<String, Object> map = medFinalChargeMapper.queryYearMonthByStoreSet(entityMap);
