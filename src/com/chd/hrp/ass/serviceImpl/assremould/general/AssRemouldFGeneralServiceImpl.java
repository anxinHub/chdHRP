...
import com.chd.hrp.ass.dao.assremould.general.AssRemouldFGeneralMapper;
import com.chd.hrp.ass.dao.assremould.general.AssRemouldFdetailGeneralMapper;
import com.chd.hrp.ass.dao.assremould.general.AssRemouldFsourceGeneralMapper;
...
@Service("assRemouldFGeneralService")
public class AssRemouldFGeneralServiceImpl implements AssRemouldFGeneralService {

	private static Logger logger = Logger.getLogger(AssRemouldFGeneralServiceImpl.class);
	//引入DAO操作   主表
	@Resource(name = "assRemouldFGeneralMapper")
	private final AssRemouldFGeneralMapper assRemouldFGeneralMapper = null;
	//明细表
	...
	//资金来源表
	@Resource(name = "assRemouldFsourceGeneralMapper")
	private final AssRemouldFsourceGeneralMapper assRemouldFsourceGeneralMapper = null;
	...
	*/
	...
	public String add(Map<String,Object> entityMap)throws DataAccessException{
		...
		//获取对象050805 资产改造竣工(一般设备)
		AssRemouldFGeneral assRemouldFGeneral = queryByCode(entityMap);

		if (assRemouldFGeneral != null) {
		...
		try {
			...
			int state = assRemouldFGeneralMapper.add(entityMap);
	...
	*/
	...
	public String addBatch(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			...
			assRemouldFGeneralMapper.addBatch(entityList);
	...
	*/
	...
	public String update(Map<String,Object> entityMap)throws DataAccessException{
		...
		try {
		  ...
		  int state = assRemouldFGeneralMapper.update(entityMap);
	...
	*/
	...
	public String updateBatch(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
		  ...
		  assRemouldFGeneralMapper.updateBatch(entityList);
	...
	*/
    ...
    try {
			int state = assRemouldFGeneralMapper.delete(entityMap);
	...
	*/
	...
	public String deleteBatch(List<Map<String,Object>> entityList)throws DataAccessException{
		...
		try {
			assRemouldFsourceGeneralMapper.deleteBatch(entityList);
			assRemouldFdetailGeneralMapper.deleteBatch(entityList);
			assRemouldFGeneralMapper.deleteBatch(entityList);
	...
	*/
	...
	/*	
		...
		List<AssBackGeneral> list = (List<AssBackGeneral>) assRemouldFGeneralMapper.queryExists(mapVo);
		try {
			boolean flag = true;
			boolean flag1 = true;
			//boolean flag2 = true;
			if (list.size() > 0) {
				assRemouldFGeneralMapper.update(entityMap);
			} else {
				if(entityMap.get("create_date") != null && !"".equals(entityMap.get("create_date"))){
					entityMap.put("year", entityMap.get("create_date").toString().substring(0,4));
					entityMap.put("month", entityMap.get("create_date").toString().substring(5,7));
				}
				entityMap.put("bill_table", "ASS_REMOULD_F_GENERAL");
				String fcord_no = assBaseService.getBillNOSeqNo(entityMap);
				entityMap.put("fcord_no", fcord_no);
				assRemouldFGeneralMapper.add(entityMap);
			...
			for (Map<String, Object> detailVo : detail) {
				...
				//查询该卡片是否存在明细数据
				List<Map<String,Object>> detailLiSt= assRemouldFdetailGeneralMapper.queryByFcordNo(detailVo);
					for (Map<String,Object> det : detailLiSt) {
						...
						if (falg == false) {
							assRemouldFsourceGeneralMapper.delete(det);
						}
					}
					List<Map<String,Object>> sourceLiSt=assRemouldFsourceGeneralMapper.queryRemouldSourceByAssCardNo(detailVo);
		...
		/*	if(!flag2){
			...
			*/
			Map<String, Object> vStoreMapp = new HashMap<String, Object>();
			vStoreMapp.put("group_id", entityMap.get("group_id"));
			vStoreMapp.put("hos_id", entityMap.get("hos_id"));
			vStoreMapp.put("copy_code", entityMap.get("copy_code"));
			vStoreMapp.put("fcord_no", entityMap.get("fcord_no"));
			
			assRemouldFsourceGeneralMapper.delete(vStoreMapp);
			assRemouldFdetailGeneralMapper.delete(entityMap);//明细表
			assRemouldFdetailGeneralMapper.addBatch(listVo);//明细表
			if (listSourceVo.size() > 0) {
				assRemouldFsourceGeneralMapper.deleteBatch(listSourceVo);
				assRemouldFsourceGeneralMapper.addBatch(listSourceVo);//资金来源表
	...
	public String collectAssBackGeneral(Map<String, Object> entityMap) {
		...
		assRemouldFGeneralMapper.collectAssBackGeneral(entityMap);
	...
	*/
	...
	public String query(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssRemouldFGeneral> list = (List<AssRemouldFGeneral>)assRemouldFGeneralMapper.query(entityMap);
			...
			List<AssRemouldFGeneral> list = (List<AssRemouldFGeneral>)assRemouldFGeneralMapper.query(entityMap, rowBounds);
	...
	*/
	...
	public <T> T queryByCode(Map<String,Object> entityMap)throws DataAccessException{
		return assRemouldFGeneralMapper.queryByCode(entityMap);
	}
	
	/**
	 * @Description 
	 * 获取050805 资产改造竣工(一般设备)<BR> 
	 * @param  entityMap<BR>
	 *  参数为要检索的字段
	 * @return AssRemouldFGeneral
	 * @throws DataAccessException
	*/
	@Override
	public <T> T queryByUniqueness(Map<String,Object> entityMap)throws DataAccessException{
		return assRemouldFGeneralMapper.queryByUniqueness(entityMap);
	}
	 ...
	 * @param  entityMap<BR>
	 *  参数为要检索的字段
	 * @return List<AssRemouldFGeneral>
	 * @throws DataAccessException
	*/
	@Override
	public List<?> queryExists(Map<String,Object> entityMap)throws DataAccessException{
		return assRemouldFGeneralMapper.queryExists(entityMap);
	}
	@Override
	public String queryAssRemouldFSourceGeneral(Map<String, Object> entityMap) {
		List<Map<String,Object>> ListVo = assRemouldFsourceGeneralMapper.queryAssRemouldFSourceGeneral(entityMap);

		return ChdJson.toJson(ListVo);
	}
	@Override
	public String queryAssRemouldFDetailGeneral(Map<String, Object> mapVo) {
				

		List<Map<String,Object>> ListVo = assRemouldFdetailGeneralMapper.queryAssRemouldFDetailGeneral(mapVo);

		return ChdJson.toJson(ListVo);
	}
	@Override
	public String saveAssRemouldFSourceGeneral(Map<String, Object> mapVo) {
		...
		try {
			for (Map<String, Object> map : detail) {
				...
				Map<String, Object> sourceMap=assRemouldFsourceGeneralMapper.queryRemouldSourceByAssSourceId(map);
				if(sourceMap==null){
					assRemouldFsourceGeneralMapper.add(map);
				}else{
					assRemouldFsourceGeneralMapper.update(map);
				}
			}
			mapVo.put("change_price", change_price.toString());
			assRemouldFdetailGeneralMapper.update(mapVo);
	...
	@Override
	public String deleteAssRemouldFDetailGeneral(List<Map<String, Object>> listVo) {
		assRemouldFsourceGeneralMapper.deleteBatch(listVo);
		assRemouldFdetailGeneralMapper.deleteBatchByCardNo(listVo);
		return "{\"msg\":\"成功.\",\"state\":\"true\"}";
	}
	@Override
	public String deleteAssRemouldFSourceGeneral(List<Map<String, Object>> listVo) {
		Double change_price = 0.0;
		Map<String,Object> mapVo = new HashMap<String,Object>();
		...
		Map<String, Object> detailMap =assRemouldFdetailGeneralMapper.queryByAssCardNoMap(mapVo);
			mapVo.put("change_price", Double.parseDouble(detailMap.get("change_price").toString())-change_price);
		assRemouldFsourceGeneralMapper.deleteBatchBySourceId(listVo);
		assRemouldFdetailGeneralMapper.updatechangePriceByCardNo(mapVo);
	...
	@Override
	public String updateConfirmAssRemouldFGeneral(List<Map<String, Object>> listVo,
			List<Map<String, Object>> listCardVo) {
		
		try {
			assRemouldFGeneralMapper.updateConfirmAssRemouldFGeneral(listVo);
