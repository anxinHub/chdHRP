...
import com.chd.hrp.ass.dao.back.AssBackDetailGeneralMapper;
import com.chd.hrp.ass.dao.back.AssBackGeneralMapper;
import com.chd.hrp.ass.dao.back.AssBackInGeneralMapMapper;
import com.chd.hrp.ass.dao.back.source.AssBackSourceGeneralMapper;
...
import com.chd.hrp.ass.dao.in.AssInDetailGeneralMapper;
...
@Service("assInMainGeneralService") 
public class AssInMainGeneralServiceImpl implements AssInMainGeneralService {    

	private static Logger logger = Logger.getLogger(AssInMainGeneralServiceImpl.class);
	// 引入DAO操作
	@Resource(name = "assInMainGeneralMapper")
	private final AssInMainGeneralMapper assInMainGeneralMapper = null;

	@Resource(name = "assInDetailGeneralMapper")
	private final AssInDetailGeneralMapper assInDetailGeneralMapper = null;
	...
	@Resource(name = "assBackDetailGeneralMapper")
	private final AssBackDetailGeneralMapper assBackDetailGeneralMapper = null;

	@Resource(name = "assBackGeneralMapper")
	private final AssBackGeneralMapper assBackGeneralMapper = null;
	
	@Resource(name = "assBackSourceGeneralMapper")
	private final AssBackSourceGeneralMapper assBackSourceGeneralMapper = null;
	...
	public String deleteBatch(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {
			...
			assInDetailGeneralMapper.deleteBatch(entityList);
			assInMainGeneralMapper.deleteBatch(entityList);
	...
	public String addOrUpdate(Map<String, Object> entityMap) throws DataAccessException {
		...
		if(entityMap.get("invoice_no") != null && !"".equals(entityMap.get("invoice_no"))){
			...
			List<AssInMainGeneral> invList = assInMainGeneralMapper.queryByInvoiceNo(entityMap);
			
			for(AssInMainGeneral assInMainGeneral : invList){
				if(assInMainGeneral.getVen_id() != Integer.parseInt(ven_id)){
					return "{\"warn\":\"当前发票号已存在供应商，不能维护相同发票号不同的供应商! \"}";
				}
			}
		}

		List<AssInMainGeneral> list = (List<AssInMainGeneral>) assInMainGeneralMapper.queryExists(mapVo);
		try {
			if (list.size() > 0) {
				int state = assInMainGeneralMapper.update(entityMap);
			} else {
				if(entityMap.get("create_date") != null && !"".equals(entityMap.get("create_date"))){
					entityMap.put("year", entityMap.get("create_date").toString().substring(0,4));
					entityMap.put("month", entityMap.get("create_date").toString().substring(5,7));
				}
				entityMap.put("bill_table", "ASS_IN_MAIN_GENERAL");
				String ass_in_no = assBaseService.getBillNOSeqNo(entityMap);
				entityMap.put("ass_in_no", ass_in_no);
				int state = assInMainGeneralMapper.add(entityMap);
			...
			assInDetailGeneralMapper.delete(entityMap);
			assInDetailGeneralMapper.addBatch(listVo);
			entityMap.put("in_money", in_money+"");
			assInMainGeneralMapper.updateInMoney(entityMap);
	...
	public String query(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			...
			List<AssInMainGeneral> list = (List<AssInMainGeneral>) assInMainGeneralMapper.query(entityMap);
			...
			List<AssInMainGeneral> list = (List<AssInMainGeneral>) assInMainGeneralMapper.query(entityMap, rowBounds);
	...
	public <T> T queryByCode(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainGeneralMapper.queryByCode(entityMap);
	}
	 ...
	 * @param entityMap<BR>
	 *            参数为要检索的字段
	 * @return AssInMainGeneral
	 * @throws DataAccessException
	 */
	@Override
	public <T> T queryByUniqueness(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainGeneralMapper.queryByUniqueness(entityMap);
	}
	 ...
	 * @param entityMap<BR>
	 *            参数为要检索的字段
	 * @return List<AssInMainGeneral>
	 * @throws DataAccessException
	 */
	@Override
	public List<?> queryExists(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainGeneralMapper.queryExists(entityMap);
	}
	...
	@Override
	public String initAssInCardGeneral(Map<String, Object> mapVo) throws DataAccessException {
		List<Map<String, Object>> listVo = new ArrayList<Map<String, Object>>();
		try {
			String basePath = "ass";
			...
			List<Map<String, Object>> list = assInDetailGeneralMapper.queryByInit(mapVo);
			...
			for(Map<String, Object> mapPrePay :listVo){
				...
				if(prePayStageList.size() > 0){
					if(cur_money - pay_money == 0){
						...
						Integer stage_no = assPayStageGeneralMapper.queryStageGeneralMaxNo(mapPrePay);
						...
						if ((price - (cur_money - pay_money)) > 0) {
							 ...
							 */
							Map<String, Object> payMap = new HashMap<String, Object>();
							Integer stage_no1 = assPayStageGeneralMapper.queryStageGeneralMaxNo(mapPrePay);
							payMap.put("group_id", mapPrePay.get("group_id"));
							payMap.put("hos_id", mapPrePay.get("hos_id"));
							payMap.put("copy_code", mapPrePay.get("copy_code"));
							payMap.put("ass_card_no", mapPrePay.get("ass_card_no"));
							...
							Integer stage_no2 = assPayStageGeneralMapper.queryStageGeneralMaxNo(mapPrePay);
	...
	public String updateAudit(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {
			assInMainGeneralMapper.updateAudit(entityMap);
	...
	public String updateReAudit(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {
			assInMainGeneralMapper.updateReAudit(entityMap);
	...
	public String updateConfirm(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {
			...
			for(Map<String, Object> map : entityMap){
				List<Map<String, Object>> cardList = assCardGeneralMapper.queryByAssInNo(map);//通过单号查询所有卡片
				...
				for(Map<String, Object> card : cardList){
					...
					List<Map<String, Object>> cardSourceList = assResourceGeneralMapper.queryByAssCardNoMap(card);//每一张卡片的资金来源
			...
			if(total_cur_money != 0){
				...
				List<Map<String, Object>> sourceList = assResourceGeneralMapper.queryByAssCardIn(cardMap);
			...
			assInMainGeneralMapper.updateConfirm(entityMap);
	...
	public Map<String, Object> queryAssInGeneralByPrintTemlatePrint(Map<String, Object> entityMap)throws DataAccessException {
		...
		try{
			...
			AssInMainGeneralMapper assInMainGeneralMapper = (AssInMainGeneralMapper)context.getBean("assInMainGeneralMapper");
			 
			//主页面 一般设备 采购入库   批量打印查询
			if("1".equals(String.valueOf(entityMap.get("p_num")))){
				
				//查询 一般设备 采购入库   入库主表数据
				List<Map<String,Object>> map= assInMainGeneralMapper.queryAssInGeneralPrintTemlateByMainBatch(entityMap);
				//查询 一般设备 采购入库    入库明细表数据
				List<Map<String,Object>> list= assInMainGeneralMapper.queryAssInGeneralPrintTemlateByDetail(entityMap);
				...
				//查询 一般设备 采购入库   入库主表数据
				Map<String,Object> map= assInMainGeneralMapper.queryAssInGeneralPrintTemlateByMain(entityMap);
				//查询 一般设备 采购入库    入库明细表数据
				List<Map<String,Object>> list= assInMainGeneralMapper.queryAssInGeneralPrintTemlateByDetail(entityMap);
	...
	public List<String> queryAssInGeneralState(Map<String, Object> mapVo) throws DataAccessException {
		...
		return assInMainGeneralMapper.queryAssInGeneralState(mapVo);
	}
	...
	public String updateAssInMainBillNo(Map<String, Object> entityMap) throws DataAccessException {
		try {
			...
			assInMainGeneralMapper.updateAssInMainBillNo(entityMap);
	...
	public String queryDetails(Map<String, Object> entityMap)
		...
		if (sysPage.getTotal() == -1) {
			...
			List<Map<String, Object>> list = (List<Map<String, Object>>) assInMainGeneralMapper.queryDetails(entityMap);
			...
			List<Map<String, Object>> list = (List<Map<String, Object>>) assInMainGeneralMapper.queryDetails(entityMap, rowBounds);
	...
	public String queryInMap(Map<String, Object> entityMap) {
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssInMainGeneral> list = assInMainGeneralMapper.queryAssBackInMainGenerals(entityMap);
			...
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<AssInMainGeneral> list = assInMainGeneralMapper.queryAssBackInMainGenerals(entityMap, rowBounds);
	...
	@Override
	public String queryAssBackInMainGeneral(Map<String, Object> entityMap) {
		SysPage sysPage = new SysPage();
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssInMainGeneral> list = assInMainGeneralMapper.queryAssBackInMainGeneral(entityMap);
			...
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<AssInMainGeneral> list = assInMainGeneralMapper.queryAssBackInMainGeneral(entityMap, rowBounds);
	...
	public Integer querycountStore(Map<String, Object> vStoreMap) {
		...
		return assInMainGeneralMapper.querycountStore(vStoreMap);
	}

	@Override
	public Integer querycountVen(Map<String, Object> vVenMap) {
		// TODO Auto-generated method stub
		return assInMainGeneralMapper.querycountVen(vVenMap);
	}
	...
	public String queryByInitOut(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssInMainGeneral> list = assInMainGeneralMapper.queryByInitOut(entityMap);
			...
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<AssInMainGeneral> list = assInMainGeneralMapper.queryByInitOut(entityMap, rowBounds);
	...
	public String queryInMainByOutNo(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssInMainGeneral> list = assInMainGeneralMapper.queryInMainByOutNo(entityMap);
			...
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<AssInMainGeneral> list = assInMainGeneralMapper.queryInMainByOutNo(entityMap, rowBounds);
	...
	public String collectAssBackGeneral(Map<String, Object> entityMap) {
		...
		assBackGeneralMapper.collectAssBackGeneral(entityMap);
	...
	@Override
	public String offsetInGeneral(List<Map<String, Object>> listVo)
			throws DataAccessException {
		WebApplicationContext ctx = ContextLoader.getCurrentWebApplicationContext();
		DataSourceTransactionManager transactionManager = (DataSourceTransactionManager) ctx
				.getBean("transactionManager");
		...
		try{
			...
			//获取主表退库数据
			List<Map<String, Object>> mainList = assInMainGeneralMapper.queryAssBackGeneral(inMap);
			//获取从表退库数据
			List<Map<String, Object>> detailList = (List<Map<String, Object>>)assInMainGeneralMapper.queryAssBackDetailByIds(inMap);
			...
			for( Map<String, Object> mainMap: mainList){
				...
				mainMap.put("bill_table", "ASS_BACK_GENERAL");
			...
			for(Map<String, Object> detailMap : detailList){
				...
				//判断手动在退货入一条单据,也冲不了帐 和 重复重复都会呗拦截
 				 List<Map<String,Object>> list1 = (List<Map<String, Object>>) assBackDetailGeneralMapper.queryExists(qcardMap);
					if(list1.size() > 0){
				...
				for(Map<String, Object> card : cardList){
					...
					String results = collectAssBackGeneral(map);
			...
			//关系表
				...
				assBackGeneralMapper.addBatch(insertMainList);
				
				assBackDetailGeneralMapper.addBatch(listVo1);
				assBackInGeneralMapMapper.addBatch(mapListVo);
				if (listSourceVo.size() > 0) {
					assBackSourceGeneralMapper.addBatch(listSourceVo);
				}
				transactionManager.commit(status);
	...
	public String updateIsPrint(List<Map<String, Object>> entityMap) throws DataAccessException {
		try{
			assInMainGeneralMapper.updateIsPrint(entityMap);
	...
	public String assInGenerateBill(List<Map<String, Object>> listVo) throws DataAccessException {
		...
		try {
			...
			for (Map<String, Object> inMap : listVo) {
				// 获取主表数据
				Map<String, Object> main = assInMainGeneralMapper.queryAssGenerateBillGeneral(inMap).get(0);
				// 获取从表数据
				List<Map<String, Object>> detailList = assInMainGeneralMapper.queryAssGenerateBillCard(inMap);
				...
				for (Map<String, Object> detMap : detailList) {
					...
					List<Map<String, Object>> stageList = assInMainGeneralMapper.queryAssGenerateBillStage(detMps);
			...
			assCardGeneralMapper.updateBillState(detailListVo);
