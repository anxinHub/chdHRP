...
@Service("assInMainOtherService") 
public class AssInMainOtherServiceImpl implements AssInMainOtherService {    

	private static Logger logger = Logger.getLogger(AssInMainOtherServiceImpl.class);
	// 引入DAO操作
	...
	public String deleteBatch(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {
			...
			assInMainOtherMapper.deleteBatch(entityList);
	...
	public String addOrUpdate(Map<String, Object> entityMap) throws DataAccessException {
		...
		if(entityMap.get("invoice_no") != null && !"".equals(entityMap.get("invoice_no"))){
			...
			List<AssInMainOther> invList = assInMainOtherMapper.queryByInvoiceNo(entityMap); 
			
			for(AssInMainOther assInMainOther : invList){
				if(assInMainOther.getVen_id() != Integer.parseInt(ven_id)){
					return "{\"warn\":\"当前发票号已存在供应商，不能维护相同发票号不同的供应商! \"}";
				}
			}
		}

		List<AssInMainOther> list = (List<AssInMainOther>) assInMainOtherMapper.queryExists(mapVo);
		try {
			if (list.size() > 0) {
				int state = assInMainOtherMapper.update(entityMap);
			} else {
				if(entityMap.get("create_date") != null && !"".equals(entityMap.get("create_date"))){
					entityMap.put("year", entityMap.get("create_date").toString().substring(0,4));
					entityMap.put("month", entityMap.get("create_date").toString().substring(5,7));
				}
				entityMap.put("bill_table", "ASS_IN_MAIN_OTHER");
				String ass_in_no = assBaseService.getBillNOSeqNo(entityMap);
				entityMap.put("ass_in_no", ass_in_no);
				int state = assInMainOtherMapper.add(entityMap);
			...
			assInDetailOtherMapper.delete(entityMap);
			assInDetailOtherMapper.addBatch(listVo);
			entityMap.put("in_money", in_money+"");
			assInMainOtherMapper.updateInMoney(entityMap);
	...
	public String query(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			...
			List<AssInMainOther> list = (List<AssInMainOther>) assInMainOtherMapper.query(entityMap);
			...
			List<AssInMainOther> list = (List<AssInMainOther>) assInMainOtherMapper.query(entityMap, rowBounds);
	...
	public <T> T queryByCode(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainOtherMapper.queryByCode(entityMap);
	}
	 ...
	 * @param entityMap<BR>
	 *            参数为要检索的字段
	 * @return AssInMainOther
	 * @throws DataAccessException
	 */
	@Override
	public <T> T queryByUniqueness(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainOtherMapper.queryByUniqueness(entityMap);
	}
	 ...
	 * @param entityMap<BR>
	 *            参数为要检索的字段
	 * @return List<AssInMainOther>
	 * @throws DataAccessException
	 */
	@Override
	public List<?> queryExists(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainOtherMapper.queryExists(entityMap);
	}
	...
	@Override
	public String initAssInCardOther(Map<String, Object> mapVo) throws DataAccessException {
		List<Map<String, Object>> listVo = new ArrayList<Map<String, Object>>();
		try {
			String basePath = "ass";
			...
			List<Map<String, Object>> list = assInDetailOtherMapper.queryByInit(mapVo);
			 ...
			 * 资产原值 - 预付余额 - 回冲金额 = 未付金额
			 */
			listVo.get(0).put("naturs_code", "04");
			...
			for(Map<String, Object> mapPrePay :listVo){
				...
				if(prePayStageList.size() > 0){
					if(cur_money - pay_money == 0){
						...
						Integer stage_no = assPayStageOtherMapper.queryStageOtherMaxNo(mapPrePay);
						...
						if ((price - (cur_money - pay_money)) > 0) {
							 ...
							 */
							Map<String, Object> payMap = new HashMap<String, Object>();
							Integer stage_no1 = assPayStageOtherMapper.queryStageOtherMaxNo(mapPrePay);
							payMap.put("group_id", mapPrePay.get("group_id"));
							payMap.put("hos_id", mapPrePay.get("hos_id"));
							payMap.put("copy_code", mapPrePay.get("copy_code"));
							payMap.put("ass_card_no", mapPrePay.get("ass_card_no"));
							...
							Integer stage_no2 = assPayStageOtherMapper.queryStageOtherMaxNo(mapPrePay);
	...
	public String updateAudit(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {
			assInMainOtherMapper.updateAudit(entityMap);
	...
	public String updateReAudit(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {
			assInMainOtherMapper.updateReAudit(entityMap);
	...
	public String updateConfirm(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {
			...
			for(Map<String, Object> map : entityMap){
				List<Map<String, Object>> cardList = assCardOtherMapper.queryByAssInNo(map);//通过单号查询所有卡片
				
				for(Map<String, Object> card : cardList){
					ass_card_nos = ass_card_nos + "'"+card.get("ass_card_no").toString() + "',";
					card.put("naturs_code", "04");
					...
					List<Map<String, Object>> cardSourceList = assResourceOtherMapper.queryByAssCardNoMap(card);//每一张卡片的资金来源
			...
			if(total_cur_money != 0){
				...
				List<Map<String, Object>> sourceList = assResourceOtherMapper.queryByAssCardIn(cardMap);
				
				for(int i = 0;i < sourceList.size(); i ++){
					sourceList.get(i).put("naturs_code", "04");
				}
			...
			assInMainOtherMapper.updateConfirm(entityMap);
	...
	public Map<String, Object> queryAssInOtherByPrintTemlatePrint(Map<String, Object> entityMap)throws DataAccessException {
		...
		try{
			...
			AssInMainOtherMapper assInMainOtherMapper = (AssInMainOtherMapper)context.getBean("assInMainOtherMapper");
			 
			//主页面 批量打印查询
			if("1".equals(String.valueOf(entityMap.get("p_num")))){
				
				//查询 其他固定资产 采购入库 入库主表数据
				List<Map<String,Object>> map= assInMainOtherMapper.queryAssInOtherPrintTemlateByMainBatch(entityMap);
				//查询 其他固定资产 采购入库  入库明细表数据
				List<Map<String,Object>> list= assInMainOtherMapper.queryAssInOtherPrintTemlateByDetail(entityMap);
				...
				//查询 其他固定资产 采购入库 入库主表数据
				Map<String,Object> map= assInMainOtherMapper.queryAssInOtherPrintTemlateByMain(entityMap);
				//查询 其他固定资产 采购入库  入库明细表数据
				List<Map<String,Object>> list= assInMainOtherMapper.queryAssInOtherPrintTemlateByDetail(entityMap);
	...
	public List<String> queryAssInOtherState(Map<String, Object> mapVo) throws DataAccessException {
		...
		return assInMainOtherMapper.queryAssInOtherState(mapVo);
	}
	...
	public String updateAssInMainBillNo(Map<String, Object> entityMap) throws DataAccessException {
		try {
			...
			assInMainOtherMapper.updateAssInMainBillNo(entityMap);
	...
	public String queryDetails(Map<String, Object> entityMap)
		...
		if (sysPage.getTotal() == -1) {
			...
			List<Map<String, Object>> list = (List<Map<String, Object>>) assInMainOtherMapper.queryDetails(entityMap);
			...
			List<Map<String, Object>> list = (List<Map<String, Object>>) assInMainOtherMapper.queryDetails(entityMap, rowBounds);
	...
	public String queryInMap(Map<String, Object> entityMap) {
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssInMainOther> list = assInMainOtherMapper.queryAssBackInMainOthers(entityMap);
			...
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<AssInMainOther> list = assInMainOtherMapper.queryAssBackInMainOthers(entityMap, rowBounds);
	...
	@Override
	public String queryAssBackInMainOther(Map<String, Object> entityMap) {
		SysPage sysPage = new SysPage();
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssInMainOther> list = assInMainOtherMapper.queryAssBackInMainOther(entityMap);
			...
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<AssInMainOther> list = assInMainOtherMapper.queryAssBackInMaiOther(entityMap, rowBounds);
	...
	public Integer querycountStore(Map<String, Object> vStoreMap) {
		...
		return assInMainOtherMapper.querycountStore(vStoreMap);
	}

	@Override
	public Integer querycountVen(Map<String, Object> vVenMap) {
		// TODO Auto-generated method stub
		return assInMainOtherMapper.querycountVen(vVenMap);
	}
	...
	public String queryByInitOut(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssInMainOther> list = assInMainOtherMapper.queryByInitOut(entityMap);
			...
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<AssInMainOther> list = assInMainOtherMapper.queryByInitOut(entityMap, rowBounds);
	...
	public String queryInMainByOutNo(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal()==-1){
			...
			List<AssInMainOther> list = assInMainOtherMapper.queryInMainByOutNo(entityMap);
			...
			RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
			List<AssInMainOther> list = assInMainOtherMapper.queryInMainByOutNo(entityMap, rowBounds);
	...
	@Override
	public String offsetInOther(List<Map<String, Object>> listVo)
			throws DataAccessException {
		WebApplicationContext ctx = ContextLoader.getCurrentWebApplicationContext();
		DataSourceTransactionManager transactionManager = (DataSourceTransactionManager) ctx
				.getBean("transactionManager");
		...
		try{
			...
			//获取主表退库数据
			List<Map<String, Object>> mainList = assInMainOtherMapper.queryAssBackOther(inMap);
			//获取从表退库数据
			List<Map<String, Object>> detailList = (List<Map<String, Object>>)assInMainOtherMapper.queryAssBackDetailByIds(inMap);
			...
			for( Map<String, Object> mainMap: mainList){
				...
				mainMap.put("bill_table", "ASS_BACK_OTHER");
			...
			for(Map<String, Object> detailMap : detailList){
				...
				//判断手动在退货入一条单据,也冲不了帐 和 重复重复都会呗拦截
 				 List<Map<String,Object>> list1 = (List<Map<String, Object>>) assBackDetailOtherMapper.queryExists(qcardMap);
					if(list1.size() > 0){
			...
			//关系表
				...
				assBackOtherMapper.addBatch(insertMainList);
				
				assBackDetailOtherMapper.addBatch(listVo1);
	...
	public String updateIsPrint(List<Map<String, Object>> entityMap) throws DataAccessException {
		try{
			assInMainOtherMapper.updateIsPrint(entityMap);
	...
	public String assInGenerateBill(List<Map<String, Object>> listVo) throws DataAccessException {
		...
		try {
			...
			for (Map<String, Object> inMap : listVo) {
				// 获取主表数据
				Map<String, Object> main = assInMainOtherMapper.queryAssGenerateBillOther(inMap).get(0);
				// 获取从表数据
				List<Map<String, Object>> detailList = assInMainOtherMapper.queryAssGenerateBillCard(inMap);
				...
				for (Map<String, Object> detMap : detailList) {
					...
					List<Map<String, Object>> stageList = assInMainOtherMapper.queryAssGenerateBillStage(detMps);
			...
			assCardOtherMapper.updateBillState(detailListVo);
