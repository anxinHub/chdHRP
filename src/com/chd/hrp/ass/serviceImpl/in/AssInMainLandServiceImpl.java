...
@Service("assInMainLandService")
public class AssInMainLandServiceImpl implements AssInMainLandService {    

	private static Logger logger = Logger.getLogger(AssInMainLandServiceImpl.class);
	// 引入DAO操作
	@Resource(name = "assInMainLandMapper")
	private final AssInMainLandMapper assInMainLandMapper = null;

	@Resource(name = "assInDetailLandMapper")
	private final AssInDetailLandMapper assInDetailLandMapper = null;
	...
	public String deleteBatch(List<Map<String, Object>> entityList) throws DataAccessException {
		...
		try {
			...
			assInDetailLandMapper.deleteBatch(entityList);
			assInMainLandMapper.deleteBatch(entityList);
		...
		try {
			...
			if (cardList.size() > 0) {
				...
				for (Map<String, Object> map : cardList) {
					...
					List<AssFileLand> assFileLandList = (List<AssFileLand>) assFileLandMapper.queryExists(map);
				...
				assCardLandMapper.deleteBatch(cardList);
			}
	...
	public String addOrUpdate(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<AssInMainLand> list = (List<AssInMainLand>) assInMainLandMapper.queryExists(mapVo);
		try {
			if (list.size() > 0) {
				int state = assInMainLandMapper.update(entityMap);
			} else {
				...
				int state = assInMainLandMapper.add(entityMap);
			...
			assInDetailLandMapper.delete(entityMap);
			assInDetailLandMapper.addBatch(listVo);
			entityMap.put("in_money", in_money);
			assInMainLandMapper.updateInMoney(entityMap);
	...
	public String query(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			...
			List<AssInMainLand> list = (List<AssInMainLand>) assInMainLandMapper.query(entityMap);
			...
			List<AssInMainLand> list = (List<AssInMainLand>) assInMainLandMapper.query(entityMap, rowBounds);
	...
	public <T> T queryByCode(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainLandMapper.queryByCode(entityMap);
	}
	 ...
	 * @param entityMap<BR>
	 *            参数为要检索的字段
	 * @return AssInMainLand
	 * @throws DataAccessException
	 */
	@Override
	public <T> T queryByUniqueness(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainLandMapper.queryByUniqueness(entityMap);
	}
	 ...
	 * @param entityMap<BR>
	 *            参数为要检索的字段
	 * @return List<AssInMainLand>
	 * @throws DataAccessException
	 */
	@Override
	public List<?> queryExists(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainLandMapper.queryExists(entityMap);
	}
	...
	@Override
	public String initAssInCardLand(Map<String, Object> mapVo) throws DataAccessException {
		List<Map<String, Object>> listVo = new ArrayList<Map<String, Object>>();
		try {
			String basePath = "ass";
			...
			List<Map<String, Object>> list = assInDetailLandMapper.queryByInit(mapVo);
			 ...
			 * 资产原值 - 预付余额 - 回冲金额 = 未付金额
			 */
			listVo.get(0).put("naturs_code", "01");
			...
			for (Map<String, Object> mapPrePay : listVo) {
				...
				if (prePayStageList.size() > 0) {
					if (cur_money - pay_money == 0) {
						...
						Integer stage_no = assPayStageLandMapper.queryStageLandMaxNo(mapPrePay);
						...
						if ((price - (cur_money - pay_money)) > 0) {
							 ...
							 */
							Map<String, Object> payMap = new HashMap<String, Object>();
							Integer stage_no1 = assPayStageLandMapper.queryStageLandMaxNo(mapPrePay);
							payMap.put("group_id", mapPrePay.get("group_id"));
							payMap.put("hos_id", mapPrePay.get("hos_id"));
							payMap.put("copy_code", mapPrePay.get("copy_code"));
							payMap.put("ass_card_no", mapPrePay.get("ass_card_no"));
							...
							Integer stage_no2 = assPayStageLandMapper.queryStageLandMaxNo(mapPrePay);
					...
					if (cur_money - pay_money == 0) {
						...
						for (Map<String, Object> map : prePaySourceList) {
							Double source_price = Math
									.rint(price * (Double.parseDouble(map.get("cur_money").toString()) / cur_money));
							Double source_cur_money = Math
									.rint(Double.parseDouble(mapPrePay.get("cur_money").toString())
											* (Double.parseDouble(map.get("cur_money").toString()) / cur_money));
							Double source_fore_money = Math.rint(Double.parseDouble(
									mapPrePay.get("fore_money") == null || mapPrePay.get("fore_money").equals("")
											? "0.0" : mapPrePay.get("fore_money").toString())
									* (Double.parseDouble(map.get("cur_money").toString()) / cur_money));
							Double source_depre_money = Math.rint(Double.parseDouble(
									mapPrePay.get("depre_money") == null || mapPrePay.get("depre_money").equals("")
											? "0.0" : mapPrePay.get("depre_money").toString())
									* (Double.parseDouble(map.get("cur_money").toString()) / cur_money));
							Double source_manage_depre_money = Math.rint(Double
									.parseDouble(mapPrePay.get("manage_depre_money") == null
											|| mapPrePay.get("manage_depre_money").equals("") ? "0.0"
													: mapPrePay.get("manage_depre_money").toString())
									* (Double.parseDouble(map.get("cur_money").toString()) / cur_money));
							Map<String, Object> sourceMap = new HashMap<String, Object>();
							sourceMap.put("group_id", mapPrePay.get("group_id"));
							sourceMap.put("hos_id", mapPrePay.get("hos_id"));
							...
							sourceMap.put("pay_money", String.valueOf((price - (cur_money - pay_money)) < 0
									? source_price
									: Double.parseDouble(map.get("cur_money").toString()) - Double.parseDouble(
											map.get("pay_money") == null ? "0" : map.get("pay_money").toString())));
							sourceMap.put("unpay_money",
									String.valueOf((price - (cur_money - pay_money)) < 0 ? "0"
											: source_price - (Double.parseDouble(map.get("cur_money").toString())
													- Double.parseDouble(map.get("pay_money") == null ? "0"
															: map.get("pay_money").toString()))));
							reSourceListVo.add(sourceMap);
							Double pay_money_s = Double
									.parseDouble(map.get("pay_money") == null ? "0" : map.get("pay_money").toString())
									+ (price - (cur_money - pay_money)) < 0 ? source_price
											: Double.parseDouble(map.get("cur_money").toString())
													- Double.parseDouble(map.get("pay_money") == null ? "0"
															: map.get("pay_money").toString());
	...
	public String updateAudit(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {
			assInMainLandMapper.updateAudit(entityMap);
	...
	public String updateReAudit(List<Map<String, Object>> entityMap) throws DataAccessException {
		try {
			assInMainLandMapper.updateReAudit(entityMap);
		...
		try {
			...
			if (total_cur_money != 0) {
				...
				for (int i = 0; i < sourceList.size(); i++) {
					sourceList.get(i).put("naturs_code", "06");
				}

				assPrePayMapper.updateBatchPayMoney(sourceList);
			}

			assInMainLandMapper.updateConfirm(entityMap);
	...
	@Override
	public Map<String,Object> printAssInMainLandData(Map<String, Object> map)throws DataAccessException {
		
		try{
			Map<String,Object> reMap=new HashMap<String,Object>();
			WebApplicationContext context = ContextLoader.getCurrentWebApplicationContext();
			AssInMainLandMapper assInMainLandMapper = (AssInMainLandMapper)context.getBean("assInMainLandMapper");
			
			//查询凭证主表
			List<Map<String,Object>> mainList=assInMainLandMapper.queryAssInMainLandByAssInNo(map);
					
			//查询凭证明细表
			List<Map<String,Object>> detailList=assInMainLandMapper.queryAssInMainLandDetailByAssInNo(map);
	...
	public Integer queryBydept(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainLandMapper.queryBydept(entityMap);
	}

	@Override
	public Integer queryByRdept(Map<String, Object> entityMap) throws DataAccessException {
		return assInMainLandMapper.queryByRdept(entityMap);
	}
	 ...
	 */
	@Override
	public List<String> queryAssInMainLandStates(Map<String, Object> mapVo) throws DataAccessException {
		
		return assInMainLandMapper.queryAssInMainLandStates(mapVo);
	}
	...
	public String queryDetails(Map<String, Object> entityMap)
		...
		if (sysPage.getTotal() == -1) {
			...
			List<Map<String, Object>> list = (List<Map<String, Object>>) assInMainLandMapper.queryDetails(entityMap);
			...
			List<Map<String, Object>> list = (List<Map<String, Object>>) assInMainLandMapper.queryDetails(entityMap, rowBounds);
