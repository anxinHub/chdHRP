...
import com.chd.hrp.acc.dao.autovouch.accpubCost.AccPubAutoVouchMapper;
...
import com.chd.hrp.acc.service.autovouch.accpubCost.AccPubAutoVouchService;
import com.chd.hrp.acc.service.vouch.SuperVouchService;
import com.chd.hrp.sys.entity.BusiQuery;
import com.github.pagehelper.PageInfo;
@Service("accPubAutoVouchService")
public class AccPubAutoVouchServiceImpl implements AccPubAutoVouchService {
	
	private static Logger logger = Logger.getLogger(AccPubAutoVouchServiceImpl.class);

	@Resource(name = "accPubAutoVouchMapper")
	private final AccPubAutoVouchMapper accPubAutoVouchMapper = null;
	...
	public String queryPubAutoVouchHead(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<BusiQuery> list = accPubAutoVouchMapper.queryPubAutoVouchHead(entityMap);
	...
	public String queryPubAutoVouch(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<BusiQuery> busiQueryList = accPubAutoVouchMapper.queryPubAutoVouchHead(entityMap);
	...
	//	String busi_type_code = busiQuery.getBusi_type_code();
		...
		List<Map<String, Object>> list = accPubAutoVouchMapper.queryPubAutoVouch(entityMap, rowBounds);
	...
	public String saveAutoVouch(Map<String, Object> map)throws DataAccessException {
		...
		try{
			...
			viewList=accPubAutoVouchMapper.queryAutoVouchView(map);
			...
			if(initType==1){
				...
				if(map.get("busi_no")!=null && !map.get("busi_no").equals("")){
					...
					reCount=accPubAutoVouchMapper.saveAutoVouchLogTempByNo(map,busiNoArray);
				}
				if(reCount==0){
					return "{\"error\":\"请先查询数据！\"}";
				}
				...
				//按日期生成、按汇总生成
				List<BusiQuery> busiQueryList = accPubAutoVouchMapper.queryPubAutoVouchHead(map);
				if(busiQueryList==null || busiQueryList.size()==0){
				...
				//保存自动凭证日志临时表
				reCount=accPubAutoVouchMapper.saveAutoVouchLogTemp(map);
				if(reCount==0){
					return "{\"error\":\"请先查询数据！\"}";
				}
...
//					}
				...
				if(initType==1){
					...
					//按日期生成、按汇总生成
					where_sql.append(" and "+main_table+".busi_date = '"+yearMonth+"' ");
				...
				metaList=accPubAutoVouchMapper.queryVouchJsonByBusi(map);
	...
	//	String busi_type_code = busiQuery.getBusi_type_code();
		...
		List<Map<String, Object>> list = accPubAutoVouchMapper.queryPubAutoVouch(entityMap);
