...
import com.chd.hrp.acc.dao.autovouch.AccAssAutoVouchMapper;
...
@Service("accAssAutoVouchService")
public class AccAssAutoVouchServiceImpl implements AccAssAutoVouchService {

	private static Logger logger = Logger.getLogger(AccAssAutoVouchServiceImpl.class);
	...
	@Resource(name = "accAssAutoVouchMapper")
	private final AccAssAutoVouchMapper accAssAutoVouchMapper = null;
	...
	public String queryAssAutoVouchHead(Map<String, Object> entityMap) throws DataAccessException {
		List<BusiQuery> list = accAssAutoVouchMapper.queryAssAutoVouchHead(entityMap);
	...
	public String queryAssAutoVouch(Map<String, Object> entityMap) throws DataAccessException {
		List<BusiQuery> busiQueryList = accAssAutoVouchMapper.queryAssAutoVouchHead(entityMap);
		...
		if ("050601".equals(busi_type_code)) {
			// 采购发票只查询暂估凭证
			...
			where_sql += " and v_ass_bill_main.ASS_IN_NO in(select business_no from ACC_BUSI_LOG_050101 where v_ass_bill_main.group_id=group_id and v_ass_bill_main.hos_id=hos_id and v_ass_bill_main.copy_code=copy_code and template_code='002' union select business_no from ACC_BUSI_LOG_050201 where v_ass_bill_main.group_id=group_id and v_ass_bill_main.hos_id=hos_id and v_ass_bill_main.copy_code=copy_code and template_code='002')";
		}
		...
		List<Map<String, Object>> list = accAssAutoVouchMapper.queryAssAutoVouch(entityMap, rowBounds);
	...
	public List<Map<String, Object>> queryAssAutoVouchPrint(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<Map<String, Object>> list = accAssAutoVouchMapper.queryAssAutoVouch(entityMap);
	...
	public String queryVouchJsonByBusi(Map<String, Object> map) throws DataAccessException {
		try {
			...
			viewList = accAssAutoVouchMapper.queryAutoVouchView(map);
			...
			if (initType == 1) {
				...
				if (map.get("busi_no") != null && !map.get("busi_no").equals("")) {
					...
					reCount = accAssAutoVouchMapper.saveAutoVouchLogTempByNo(map, busiNoArray);
				}
				if (reCount == 0) {
					return "{\"error\":\"请先查询数据！\"}";
				}
				...
				// 折旧单独处理
				if (busiTypeCode.equals("050301")) {
					List<BusiQuery> busiQueryList = accAssAutoVouchMapper.queryAssAutoVouchHead(map);
					if (busiQueryList == null || busiQueryList.size() == 0) {
						return "{\"error\":\"没有配置内置数据：SYS_BUSI_QUERY！\"}";
					}

					reCount = accAssAutoVouchMapper.saveAutoVouchDepreLogTemp(map);
					if (reCount == 0) {
						return "{\"error\":\"请先查询数据！\"}";
					}
					...
					reCount = accAssAutoVouchMapper.saveAutoVouchLogTemp(map);
					if (reCount == 0) {
						return "{\"error\":\"请先查询数据！\"}";
					}
				}
			...
			for (Map<String, Object> tempMap : tempList) {
				...
				for (Map<String, Object> view : viewList) {
					...
					is_auto = (view.get("IS_AUTO") == null) ? "0" : view.get("IS_AUTO").toString();
				...
				if (main_table.equals("") || amount_money.equals("")) {
					if (is_auto.equals("1") && sub_type_id.equals("")) {
						throw new SysException(metaCode
								+ "缺少内置数据：字段MAIN_TABLE，字段FIELD_ALIAS【amount_money】，字段FIELD_CODE【SUB_TYPE_ID】！");
					} else {
						throw new SysException(metaCode + "缺少内置数据：字段MAIN_TABLE，字段FIELD_ALIAS【amount_money】");
					}
				}
				...
				if (sub_type_id.equals("")) {
					vouch_sql.append(" and b.acc_year='" + accYear + "' and b.mod_code='" + modCode
							+ "' and b.meta_code='" + metaCode + "' and b.sub_type_id is null ");
				} else {
					vouch_sql.append(" and b.acc_year='" + accYear + "' and b.mod_code='" + modCode
							+ "' and b.meta_code='" + metaCode + "' and b.sub_type_id	=" + sub_type_id + "  ");
				}

				if (busiTypeCode.equals("050101") || busiTypeCode.equals("050201")) {
				...
				if (initType == 1) {
					...
					where_sql.append(" and " + main_table + "." + main_key_filed + " in('"
							+ map.get("busi_no").toString().replace(",", "','") + "') ");
				} else {
					...
					if ("050601".equals(busiTypeCode)) {
						// 采购发票只查询暂估凭证
						where_sql.append(
								" and v_ass_bill_main.ASS_IN_NO in(select business_no from ACC_BUSI_LOG_050101 where v_ass_bill_main.group_id=group_id and v_ass_bill_main.hos_id=hos_id and v_ass_bill_main.copy_code=copy_code and template_code='002' union select business_no from ACC_BUSI_LOG_050201 where v_ass_bill_main.group_id=group_id and v_ass_bill_main.hos_id=hos_id and v_ass_bill_main.copy_code=copy_code and template_code='002')");
					}

					if ("050101".equals(busiTypeCode) || "050201".equals(busiTypeCode)) {
				...
				String selSql = "";
				/*
				 * if (busiTypeCode.equals("050301")) { selSql =
				 * "select b.subj_code,ASS_NATURS,sum(" + amount_money +
				 * ") amount_money from " + main_table + " "; } else {
				 */

				// if("050301".equals(busiTypeCode)){
				// selSql = "select b.subj_code,sum(NOW_DEPRE_AMOUNT)
				// amount_money from ()";
				// }else{
				...
				metaList = accAssAutoVouchMapper.queryVouchJsonByBusi(map);
