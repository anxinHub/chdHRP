...
import com.chd.hrp.acc.dao.autovouch.AccMatAutoVouchMapper;
...
@Service("accMatAutoVouchService")
public class AccMatAutoVouchServiceImpl implements AccMatAutoVouchService {
	
	private static Logger logger = Logger.getLogger(AccMatAutoVouchServiceImpl.class);

	@Resource(name = "accMatAutoVouchMapper")
	private final AccMatAutoVouchMapper accMatAutoVouchMapper = null;
	...
	public String queryMatAutoVouchHead(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<BusiQuery> list = accMatAutoVouchMapper.queryMatAutoVouchHead(entityMap);
	...
	public String queryMatAutoVouch(Map<String, Object> entityMap) throws DataAccessException {
		...
		List<BusiQuery> busiQueryList = accMatAutoVouchMapper.queryMatAutoVouchHead(entityMap);
	...
	//	String busi_type_code = busiQuery.getBusi_type_code();
		...
		List<Map<String, Object>> list = accMatAutoVouchMapper.queryMatAutoVouch(entityMap, rowBounds);
	...
	public String queryVouchJsonByBusi(Map<String, Object> map)throws DataAccessException {
		...
		try{
			...
			viewList=accMatAutoVouchMapper.queryAutoVouchView(map);
			...
			if(initType==1){
				//按单据生成
				busiLogNo=map.get("busi_no").toString();//凭证日志表存的业务编号或ID
				...
				List<BusiQuery> busiQueryList = accMatAutoVouchMapper.queryMatAutoVouchHead(map);
				if(busiQueryList==null || busiQueryList.size()==0){
				...
				busiIdList = accMatAutoVouchMapper.queryMatAutoVouchBusiIdList(map);
			...
			//判断是否生成凭证
			int reCount=accMatAutoVouchMapper.queryAutoVouchIsCreate(map);
			if (reCount>0) {
			...
			for(Map<String, Object> tempMap:tempList){
				...
				metaList=accMatAutoVouchMapper.queryVouchJsonByBusi(map);
				...
				if(metaList!=null && metaList.size()>0){
					...
					for(Map<String, Object> detailMap:metaList){
						...
						if(checkSqlList!=null && checkSqlList.size()>0){
							String checkSelSql="select b.subj_code"+checkSqlList.get(2)+",sum("+amount_money+") amount_money from "+main_table+" ";
							...
							map.put("order_sql", " order by "+checkSqlList.get(2).substring(1, checkSqlList.get(2).length())+" ");
							metaList=accMatAutoVouchMapper.queryVouchCheckJsonByBusi(map);
							if(metaList!=null && metaList.size()>0){
			...
			if(map.get("busi_type_code").toString().equals("040703")&&initType==1){
				
				map.put("busiLogNo",busiLogNo);
				
				map.put("Dsummary",checkMap.get(0).get(0).get("check6_name").toString().split(" ")[1]);
		...
		try{
			...
			if(initType==1){
				...
				if(map.get("busi_no")!=null && !map.get("busi_no").equals("")){
					...
					reCount=accMatAutoVouchMapper.saveAutoVouchLogTempByNo(map,busiNoArray);
				}
				if(reCount==0){
					return "{\"error\":\"请先查询数据！\"}";
				}
				...
				map.put("left_sql", getLeftJoinSql(busiQuery.getMain_table(), busiQuery.getMain_key_field(), busiQuery.getDetail_table(), busiQuery.getWhere_sql()));
				//保存自动凭证日志临时表
				reCount=accMatAutoVouchMapper.saveAutoVouchLogTemp(map);
				if(reCount==0){
					return "{\"error\":\"请先查询数据！\"}";
				}
...
//					}
				...
				vouch_sql.append(" inner join acc_busi_map b on b.group_id="+main_table+".group_id and b.hos_id="+main_table+".hos_id and b.copy_code="+main_table+".copy_code "); 
				//入库退货出库业务增加库房条件
				if(busiTypeCode.startsWith("0401") || busiTypeCode.startsWith("0402") || busiTypeCode.startsWith("0404")){
				...
				if(initType==1){
					...
					//按单据生成，付款单、退款单meta_code=0401现金、银行，按单据生成分录
					if((map.get("busi_type_code").equals("040703") || map.get("busi_type_code").equals("040704")) && metaCode.equals("0401")){
						sel_col=" ,"+business_no+",b.type_id";
						map.put("summary","'['||business_no||']'||"+map.get("summary"));//分录摘要加上单据号
					}
				...
				//过滤发票状态
				if(busiTypeCode.startsWith("0401") && busiTypeCode.startsWith("0404") && map.get("bill_state") != null && !"".equals(map.get("bill_state").toString())){
	...
	//	String busi_type_code = busiQuery.getBusi_type_code();
		...
		List<Map<String, Object>> list = accMatAutoVouchMapper.queryMatAutoVouch(entityMap);
	...
	//检查凭证（所有自动凭证通用）
	@Override
	public String checkAutoVouch(Map<String, Object> map)throws DataAccessException {
		// TODO Auto-generated method stub
		try{
			...
			if(map.get("busi_no")!=null && !"".equals(map.get("busi_no").toString())){
				//处理为字符串的单号
				String busi_no=map.get("busi_no").toString();
				if(busi_type_code.equals("080101") || busi_type_code.equals("080102") || busi_type_code.equals("080110")){
					busi_no="'"+map.get("busi_no").toString().replace(",","','")+"'";
				}
				map.put("busi_no", busi_no);
			}
			
			accMatAutoVouchMapper.saveAutoNote(map);
			
			//查询自动凭证说明表
			int Total=0;
			StringBuilder reJson=new StringBuilder(); 
			List<Map<String,Object>> reList=accMatAutoVouchMapper.querySysBusiNote(map);
			reJson.append("{\"r1\":[");
			
			if(reList!=null && reList.size()>0){
				for(Map<String,Object> m:reList){
					reJson.append("{\"meta_name\":\""+m.get("meta_name")+"\",");
			...
			reList=accMatAutoVouchMapper.queryAutoVouchNote(map);
			reJson.append(",\"r2\":[");
			if(reList!=null && reList.size()>0){
				String metaCode="";
				for(Map<String,Object> m:reList){
					if(metaCode.equals("") || !metaCode.equalsIgnoreCase(m.get("meta_code").toString())){
						reJson.append("{\"flag\":1,\"note\":\""+m.get("table_name")+"\"},");
					}
					reJson.append("{\"flag\":2,\"note\":\""+m.get("note")+"\"},");
					metaCode=m.get("meta_code").toString();
				}
				reJson.setCharAt(reJson.length()-1, ']');
				Total=Total+reList.size();
			}else{
				reJson.append("]");
			}
			
			reJson.append(",\"Total\":"+Total+"}");
