...
import com.chd.hrp.acc.dao.autovouch.accamortize.AccAmortizeAutoVouchMapper;
...
@Service("accAmortizeAutoVouchService")
public class AccAmortizeAutoVouchServiceImpl implements AccAmortizeAutoVouchService {
	...
	@Resource(name = "accAmortizeAutoVouchMapper")
	private final AccAmortizeAutoVouchMapper accAmortizeAutoVouchMapper = null;
	...
	public String queryAmortizeAutoVouchHead(Map<String, Object> entityMap) throws DataAccessException {
		List<BusiQuery> list = accAmortizeAutoVouchMapper.queryAmortizeAutoVouchHead(entityMap);
	...
	public String queryAmortizeAutoVouch(Map<String, Object> entityMap) throws DataAccessException {
		List<BusiQuery> busiQueryList = accAmortizeAutoVouchMapper.queryAmortizeAutoVouchHead(entityMap);
		...
		List<Map<String, Object>> list = accAmortizeAutoVouchMapper.queryAmortizeAutoVouch(entityMap, rowBounds);
		PageInfo page = new PageInfo(list);
		return ChdJson.toJson(list, page.getTotal());
	}

	@Override
	public List<Map<String, Object>> queryAmortizeAutoVouchPrint(Map<String, Object> entityMap)
		...
		List<Map<String, Object>> list = accAmortizeAutoVouchMapper.queryAmortizeAutoVouch(entityMap);
	...
	public String queryVouchJsonByBusi(Map<String, Object> map) throws DataAccessException {
		try {
			...
			viewList = accAmortizeAutoVouchMapper.queryAutoVouchView(map);
			...
			if (initType == 1) {
				...
				if (map.get("busi_no") != null && !map.get("busi_no").equals("")) {
					...
					reCount = accAmortizeAutoVouchMapper.saveAutoVouchLogTempByNo(map, busiNoArray);
				}
				if (reCount == 0) {
					return "{\"error\":\"请先查询数据！\"}";
				}
				...
				List<BusiQuery> busiQueryList = accAmortizeAutoVouchMapper.queryAmortizeAutoVouchHead(map);
				if (busiQueryList == null || busiQueryList.size() == 0) {
				...
				reCount = accAmortizeAutoVouchMapper.saveAutoVouchLogTemp(map);
				if (reCount == 0) {
					return "{\"error\":\"请先查询数据！\"}";
				}
			}
			...
			for (Map<String, Object> tempMap : tempList) {
				...
				for (Map<String, Object> view : viewList) {
					...
					// 取汇总金额
					if (FIELD_ALIAS.equalsIgnoreCase("THIS_AMORTIZED")
							|| FIELD_ALIAS.equalsIgnoreCase("ORIGIN_VALUE")) {
					...
					// 取从表第三层表关联字段
					if (detail_key_filed.equals("") && view.get("ITEM_KEY_FIELD") != null
				...
				if (!detail_key_filed.equals("") && !detail_table.equals("") && !item_table.equals("")) {
					...
					inner_sql2.append(item_table + ".group_id=" + detail_table + ".group_id and " + item_table
							...
							+ ".copy_code ");
					inner_sql2.append(
							" and " + item_table + "." + main_key_filed + "=" + detail_table + "." + main_key_filed);
				}
				...
				vouch_sql.append(" inner join acc_busi_map b on b.group_id=" + main_table + ".group_id and b.hos_id="
						+ main_table + ".hos_id and b.copy_code=" + main_table + ".copy_code and "+ main_table + ".type_code=b.type_id");
				...
				if (map.get("type_code") != null && !map.get("type_code").equals("")) {
					where_sql.append(" and " + main_table + ".type_code = " + map.get("type_code") + "");
				}
				...
				if (initType == 1) {
					...
					where_sql.append(" and to_char(" + busi_date + ",'yyyy-MM-dd') between '" + busiDateB + "' and '"
							+ busiDateE + "' ");
				...
				metaList = accAmortizeAutoVouchMapper.queryVouchJsonByBusi(map);
				...
				if (metaList != null && metaList.size() > 0) {
					...
					for (Map<String, Object> detailMap : metaList) {
						...
						if (detailMap.get("is_check") != null && detailMap.get("is_check").toString().equals("1")) {
							...
							if (detail_table != null && !"".equals(detail_table)) {
						...
						if (detailMap.get("is_cash") != null && detailMap.get("is_cash").toString().equals("1")) {
							...
							cashMap.put("cash_money",
									Float.parseFloat(detailMap.get("credit").toString()) == 0 ? detailMap.get("debit")
											: detailMap.get("credit"));
