...
import com.chd.hrp.acc.dao.InternetBank.cbc.AccCBCPayBorrMapper;
...
@Service("accCBCPayBorrService")
public class AccCBCPayBorrServiceImpl implements AccCBCPayBorrService {

	private static Logger logger = Logger.getLogger(AccCBCPayBorrServiceImpl.class);

	@Resource(name = "accCBCPayBorrMapper")
	private final AccCBCPayBorrMapper accCBCPayBorrMapper = null;
	...
	public String queryAccBorrApply(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			...
			List<Map<String, Object>> list = accCBCPayBorrMapper.queryAccBorrApply(entityMap);
			...
			List<Map<String, Object>> list = accCBCPayBorrMapper.queryAccBorrApply(entityMap, rowBounds);
	...
	public String queryAccBankNetBorr(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			...
			List<Map<String, Object>> list = accCBCPayBorrMapper.queryAccBankNetBorr(entityMap);
			...
			List<Map<String, Object>> list = accCBCPayBorrMapper.queryAccBankNetBorr(entityMap, rowBounds);
...
//				}
//				
//				accCBCPayBorrMapper.addBatchAccBankNetBorr(payList);
//				
//				accCBCPayBorrMapper.addBatchAccBankNetBorrRd(rdList);
//				
...
//				
//				accCBCPayBorrMapper.updateAccBankNetBorr(mapUpdate);
//				
//			}else 
				...
				for (int i = 0; i< paramVoArr.length; i++) {
					...
					accCBCPayBorrMapper.updateBatchAccBankNetBorrRd(updateRdList);
	...
	public String getFSeqNo(Map<String, Object> entityMap) {// 生成指令包序列号
		...
		String fseqNo = accCBCPayBorrMapper.queryMaxFSeqNo(entityMap);// 计算指令包序列号
		...
		if (!"0".equals(fseqNo)) {
			...
			if (fseqNoSub.equals(day)) {
				...
				Long seq_no = Long.parseLong(fseqNo) + 1;

				fseqNo = seq_no.toString();
	...
	public String updateAccBankNetBorr(List<Map<String, Object>> entityMap) throws DataAccessException {
		...
		try {
			...
			for (Map<String, Object> mapVo : entityMap) {
				...
				accCBCPayBorrMapper.updateAccBankNetBorr(map);// 更新主表
				...
				if (rdList != null && rdList.size() > 0) {
					...
					accCBCPayBorrMapper.updateBatchAccBankNetBorrRd(rdList);// 更新明细表
	...
	public String queryAccBankNetBorrRd(Map<String, Object> entityMap) throws DataAccessException {
		...
		if (sysPage.getTotal() == -1) {
			...
			List<Map<String, Object>> list = accCBCPayBorrMapper.queryAccBankNetBorrRd(entityMap);
			...
			List<Map<String, Object>> list = accCBCPayBorrMapper.queryAccBankNetBorrRd(entityMap, rowBounds);
	...
	public double sumTotalAmtByDay(Map<String, Object> entityMap) throws DataAccessException {
		...
		return accCBCPayBorrMapper.sumTotalAmtByDay(entityMap);
