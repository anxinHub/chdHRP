...
import com.chd.hrp.acc.dao.InternetBank.AccBankNetAssPayMapper;
...
@Service("accBankNetAssPayService")
public class AccBankNetAssPayServiceImpl implements AccBankNetAssPayService {

	private static Logger logger = Logger.getLogger(AccBankNetAssPayServiceImpl.class);

	@Resource(name = "accBankNetAssPayMapper")
	private final AccBankNetAssPayMapper accBankNetAssPayMapper = null;
	...
	public String queryAssPayMain(Map<String, Object> entityMap) throws DataAccessException {
		...
		if(sysPage.getTotal() == -1){
			...
			List<Map<String, Object>> list = accBankNetAssPayMapper.queryAssPayMain(entityMap);
			...
			List<Map<String, Object>> list = accBankNetAssPayMapper.queryAssPayMain(entityMap, rowBounds);
	...
	public String queryAccBankNetAssPay(Map<String, Object> entityMap) throws DataAccessException {
		...
		if(sysPage.getTotal() == -1){
			...
			List<Map<String, Object>> list = accBankNetAssPayMapper.queryAccBankNetAssPay(entityMap);
			...
			List<Map<String, Object>> list = accBankNetAssPayMapper.queryAccBankNetAssPay(entityMap, rowBounds);
	...
	public String addAccBankNetAssPay(Map<String, Object> entityMap) throws DataAccessException {
		...
		String iseqno = DateUtil.dateToString(new Date(), "yyyyMMdd");//资产付款支付 只支持单笔支付
...
//				rdMap.put("cash_user", SessionManager.getUserId());rdMap.put("cashe_date", new Date());
				...
				rdMap.put("usecn", "资产款");rdMap.put("ensummary", "");
				...
				accBankNetAssPayMapper.addBatchAccBankNetAssPay(payList);
				
				accBankNetAssPayMapper.addBatchAccBankNetAssPayRd(rdList);
				...
				accBankNetAssPayMapper.updateAccBankNetAssPay(mapUpdate);
				...
				accBankNetAssPayMapper.updateBatchAccBankNetAssPayRd(updateRdList);
	...
	public String getFSeqNo(Map<String, Object> entityMap) {// 生成指令包序列号
		...
		String fseqNo = accBankNetAssPayMapper.queryMaxFSeqNo(entityMap);// 计算指令包序列号
	...
	public String updateAccBankNetAssPay(List<Map<String, Object>> entityMap) throws DataAccessException {
		...
		try {
			...
			for(Map<String, Object> mapVo :entityMap){
				...
				accBankNetAssPayMapper.updateAccBankNetAssPay(map);//更新主表
				...
				if(rdList !=null && rdList.size()>0){
					...
					accBankNetAssPayMapper.updateBatchAccBankNetAssPayRd(rdList);//更新明细表
	...
	public String queryAccBankNetAssPayRd(Map<String, Object> entityMap) throws DataAccessException {
		...
		if(sysPage.getTotal() == -1){
			...
			List<Map<String, Object>> list = accBankNetAssPayMapper.queryAccBankNetAssPayRd(entityMap);
			...
			List<Map<String, Object>> list = accBankNetAssPayMapper.queryAccBankNetAssPayRd(entityMap, rowBounds);
	...
	public double sumTotalAmtByDay(Map<String, Object> entityMap) throws DataAccessException {
		...
		double totalAmt = accBankNetAssPayMapper.sumTotalAmtByDay(entityMap);
	...
	@Override
	public String queryAccBankNetAssPayNum(Map<String, Object> entityMap)
		...
		try {
			...
			String[] paramVoArr = paramVo.split(",");
				...
				for (int i = 0; i< paramVoArr.length; i++) {
					...
					List<Map<String, Object>> rMap= accBankNetAssPayMapper.queryAccBankNetAssPayRd(payMain);
