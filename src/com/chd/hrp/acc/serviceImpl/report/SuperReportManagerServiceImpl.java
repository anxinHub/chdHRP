 ...
 *
 */
@Service("superReportManagerService")
public class SuperReportManagerServiceImpl implements SuperReportManagerService{

	private static Logger logger = Logger.getLogger(SuperReportManagerServiceImpl.class);
...
//		RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
		List<RepInstance> list = superReportManagerMapper.querySuperReportInstanceList(map);
		...
		Map<String, String> paramMap=null;
 		for(String s :param){
 			...
 			if(MyConfig.getSysPara("042").equals("1") && map.get("mod_code")!=null && map.get("mod_code").toString().equals("01") &&
 					paramMap.get("acc_year")!=null && !paramMap.get("acc_year").toString().equals("0000") && !paramMap.get("acc_year").toString().equals("") && 
 		...
 		try{
 			superReportManagerMapper.deleteBatchSuperReportInstance(map.get("group_id").toString(), map.get("hos_id").toString(), map.get("copy_code").toString(), map.get("report_code").toString(), map.get("mod_code").toString(), list);
 		}catch (Exception e) {
	...
	public String querySuperReportDictList(Map<String, Object> map)throws DataAccessException {
		...
		RowBounds rowBounds = new RowBounds(sysPage.getPage(), sysPage.getPagesize());
		List<Map<String, Object>> list = superReportManagerMapper.querySuperReportDictList(map, rowBounds);
	...
	public String saveSuperReportDict(Map<String, Object> map)
		...
		try{
			superReportManagerMapper.querySuperReportDictSqlValidate(dictSql);
		}catch (Exception e) {
			logger.error(e.getMessage(), e);
		...
		try{
			...
			if(map.get("operation").toString().equals("insert")){
				...
				map.remove("field_sort");
	 			...
	 			superReportManagerMapper.insertSuperReportDict(map);
				...
				if (count >0) {
	 			...
	 			superReportManagerMapper.updateSuperReport(map);
	...
	public String deleteBatchSuperReportDict(Map<String, Object> map)
		...
		try{
			List<String> listUse=superReportManagerMapper.querySuperReportDictByUse(map.get("group_id").toString(), map.get("hos_id").toString(), map.get("copy_code").toString(),list);
			String msg="";
			...
			if(msg!=null && !msg.equals("")){
 			...
 			superReportManagerMapper.deleteBatchSuperReportDict(map.get("group_id").toString(), map.get("hos_id").toString(), map.get("copy_code").toString(),list);
 		}catch (Exception e) {
			logger.error(e.getMessage(), e);
			throw new SysException(e.getMessage());
		}
	...
	public String querySuperReportDictByCode(Map<String, Object> map)
		...
		RepDefineDict dict=superReportManagerMapper.querySuperReportDictByCode(map);
		if(dict!=null){
			sb.append("{");
...
//			}
			...
			if(map.get("operation").toString().equals("insert")){
				...
				//添加报表元素
				superReportManagerMapper.insertSuperReportEle(map);
				...
				//修改报表元素
				superReportManagerMapper.updateSuperReportEle(map);
				
				//删除报表元素参数
				superReportManagerMapper.deleteSuperReportElePara(map);
			...
			if(list!=null && list.size()>0){
				superReportManagerMapper.insertSuperReportElePara(list);
			}
...
//			}
	...
	public String deleteSuperReportEle(Map<String, Object> map)throws DataAccessException {
		...
		try{
			...
			//删除报表元素
			superReportManagerMapper.deleteSuperReportEle(map);
			
			
			if(map.get("ele_type").toString().equals("1") || map.get("ele_type").toString().equals("2") || map.get("ele_type").toString().equals("3")){
				//根据报表元素删除存储过程函数视图
				superReportManagerMapper.deleteSuperVouchByProcFunView(map);
			}
	...
	public String querySuperReportSourceAndViews(Map<String, Object> map)
			throws DataAccessException {
		// TODO Auto-generated method stub
		String eleSql=superReportManagerMapper.querySuperReportEleSql(map.get("group_id").toString(),map.get("hos_id").toString(),map.get("copy_code").toString(),map.get("ele_code").toString());
...
//		String reStr="";
//		if(eleSql!=null){
//			List<String> str=superReportManagerMapper.querySuperReportSourceAndViews(eleSql);
...
//					byte[] ele_sql = e.getText().getBytes("UTF-8");//获取存储过程SQL内容
					...
					if(ele_code.equalsIgnoreCase(map.get("ele_code").toString())){
						...
						if(map.get("is_ele") != null && "0".equals(map.get("is_ele").toString())){
							isInit=superReportManagerMapper.updateSuperReportDsSql(map);//更新数据集表中数据
						}else{
							isInit=superReportManagerMapper.updateSuperReportEleSql(map);//更新报表元素表中数据
						}
						
						//非自定义sql需加载到数据库中
						if(map.get("ele_type") == null || !"4".equals(map.get("ele_type").toString())){
							superReportManagerMapper.querySuperReportDictSqlValidate(e.getText());
						}
	...
	@Override
	public String getReportQuery(Map<String, Object> map)
			throws DataAccessException {
		List<Map<String,Object>> list = superReportManagerMapper.getReportQuery(map);
