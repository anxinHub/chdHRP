...
import com.chd.hrp.acc.dao.report.SuperReportEngineMapper;
import com.chd.hrp.acc.dao.report.SuperReportManagerMapper;
 ...
 *
 */
@Service("superReportEngineService")
public class SuperReportEngineServiceImpl implements SuperReportEngineService{

	private static Logger logger = Logger.getLogger(SuperReportEngineServiceImpl.class);
	
	@Resource(name = "superReportEngineMapper")
	private final SuperReportEngineMapper superReportEngineMapper = null;
	
	@Resource(name = "superReportDesignMapper")
	private final SuperReportDesignMapper superReportDesignMapper = null;
	
	@Resource(name = "superReportManagerMapper")
	private final SuperReportManagerMapper superReportManagerMapper = null;
	...
	public String queryAccSuperReportByPerm(Map<String, Object> map) throws DataAccessException {
		...
		list=superReportEngineMapper.queryAccSuperReportByPerm(map);
	...
	public String querySuperReportInstance(Map<String, Object> map)throws DataAccessException {
		...
		try {
			...
			if(map.get("operation").toString().equals("instance")){
				//查询报表实例数据
				RepInstance  repInstance=superReportEngineMapper.querySuperReportInstance(map);
	...
	public String saveSuperReportInstance(Map<String, Object> map)throws DataAccessException {
		...
		//查询报表定义表
		RepDefine repDefine=superReportDesignMapper.querySuperReportByCode(map);		
		...
		try {
			...
			//删除报表实例数据
			superReportEngineMapper.deleteSuperReportInstance(map);
			...
			superReportEngineMapper.insertSuperReportInstance(map);			
	...
	private void superReportInstanceParsing2(Map<String, Object> map,JSONObject sheetObject)throws Exception{
		...
		if(bodyfMap!=null){
			for(Map.Entry<Integer, String> entry : bodyfMap.entrySet()){
				dataList=getDataSet(map,entry.getValue());//需判断数据集类型，目前支持sql与函数
...
//		}
	...
	public String cacuRows(String s,Map<String,String> dsmap){
		...
		for(Map.Entry<String, String> entry : dsmap.entrySet()){
			tmp=entry.getValue();
			if(oldRow-1>Integer.parseInt(tmp.split("_")[2])){
				newRow+=Integer.parseInt(tmp.split("_")[1])-1;
			}
		}
...
//			}
	...
	private String setSQLParameter1(Map<String,Object> map,String sql){
//		if(map.get("acc_year")!=null&&!map.get("acc_year").toString().trim().equals(""))
//			sql=sql.replace("@budg_year", map.get("acc_year").toString());
//			sql=sql.replace("@acc_year", map.get("acc_year").toString());
		if(map.get("ex_para")==null)
			return sql;
		else{
			String ex_para=map.get("ex_para").toString();
			JSONObject json=JSONObject.parseObject(ex_para);
			Set<String> set=json.keySet();
			String key=null;
			String value=null;
			for(String tmps:set){
				value=json.getString(tmps);
				if(value.equals("本集团"))
					value=SessionManager.getGroupId();
				else if(value.equals("本医院"))
					value=SessionManager.getHosId();
				else if(value.equals("本账套"))
					value=SessionManager.getCopyCode();
				key=tmps.substring(0,tmps.indexOf("#"));
				if(key.equals("budg_year")){
					int acc_year=Integer.parseInt(map.get("acc_year").toString());
					value=value.replaceAll("下年度", (acc_year+1)+"");
					value=value.replaceAll("本年度", acc_year+"");
					value=value.replaceAll("上年", (acc_year-1)+"");
					value=value.replaceAll("上两年", (acc_year-2)+"");
				}else if(key.equals("acc_month")){					 
					value=map.get("acc_month").toString();
					logger.debug("已替换期间参数="+value);
				}
	...
	private String setSQLParameter(Map<String,Object> map,String sql){
		List<Map<String,Object>> tl=superReportDataSetMapper.querySuperReportDSparaValue(map);
		String pname="";
		String pvalue="";
		String pjson=null;
		int acc_year=Integer.parseInt(map.get("acc_year").toString());
		...
		for(Map<String,Object> m:tl){
			...
			if(pname.equals("budg_year")){
				pvalue=pvalue.replaceAll("下年度", (acc_year+1)+"");
				pvalue=pvalue.replaceAll("本年度", acc_year+"");
				pvalue=pvalue.replaceAll("上年", (acc_year-1)+"");
				pvalue=pvalue.replaceAll("上两年", (acc_year-2)+"");
			}else{
				if(pname.equals("acc_month"))
					pvalue=map.get("acc_month").toString();
				else{
			...
			if(json.getInteger("data_type")==2)
				pvalue="'"+pvalue+"'";
			sql=sql.replace("@"+pname, pvalue);
		}
		return sql;
	}
	private List<Map<String,Object>> getDataSet(Map<String, Object> map,String dsCode){
		Map<String,Object> mapVo=new HashMap<String,Object>();
		mapVo.put("group_id",SessionManager.getGroupId());
		mapVo.put("hos_id",SessionManager.getHosId());
		mapVo.put("copy_code",SessionManager.getCopyCode());
		mapVo.put("acc_year", map.get("acc_year"));
		mapVo.put("acc_month", map.get("acc_month"));
		mapVo.put("ds_code",dsCode);
		
		List<Map<String,Object>> strListData=null;
		//如果是sql或函数的执行方式不同
		List<Map<String,Object>> l=superReportDataSetMapper.queryDsSql(mapVo);
		...
		if(l!=null&&l.size()==1){
			...
			if(rs.get("DS_TYPE").toString().equals("4")){
				...
				strListData=superReportDataSetMapper.querySql(sql);
			}else if(rs.get("DS_TYPE").toString().equals("1")){//存储过程处理方式
				String execSql=dsCode+"('1','2')";
				strListData=superReportDataSetMapper.querySql("select * from table("+execSql+")");
	...
	private String calculateReportEle(String formula)throws Exception{
		...
		if(eleType.equalsIgnoreCase("RF")){
			...
			List<String> strListData=superReportEngineMapper.querySuperReportInstanceByFun("select "+execSql+" fun from dual");
			logger.debug("执行函数："+"select "+execSql+" fun from dual");
			String resData="";
			...
			procData.put("reVal", "");
			superReportEngineMapper.querySuperReportInstanceByProc(procData);
			...
			if(elePara!=null && !elePara.equals("")){
				...
				//根据元素名称查询参数
				List<RepDefinePara> paraList=superReportDesignMapper.querySuperReportParaByEle(viewMap);
			...
			List<String> strListData=superReportEngineMapper.querySuperReportInstanceByFun(execViewSql);
			...
			if(eleName!=null && !eleName.equals("")){
				...
				String execDeinfeSql=superReportManagerMapper.querySuperReportEleSql(SessionManager.getGroupId(),SessionManager.getHosId(),SessionManager.getCopyCode(),eleName);
				...
				List<String> strListData=superReportEngineMapper.querySuperReportInstanceByFun(execDeinfeSql);
	...
	public String saveSuperReportContent(Map<String, Object> map)throws DataAccessException {
		...
		try {
			superReportEngineMapper.saveSuperReportContent(map);
			return "{\"state\":\"true\"}";
		}
		catch (Exception e) {
		...
		if(Integer.parseInt(accMonth)<14){
			...
			if(cellFormula.indexOf("上期")!=-1){
				...
				if(accMonthInt==0){
					...
					cellFormula=cellFormula.replace("下年度",String.valueOf(Integer.parseInt(accYear)+1));
			...
			for (String sheet : jsonSheet.keySet()) {  
				...
				if(sheetName.equals("") || sheet.equalsIgnoreCase(sheetName)){
					...
					if(cellObject.getString("value")!=null && !cellObject.getString("value").equals("")){
						...
						//logger.debug("$$$$$$"+cellObject.getString("value").trim());
						reVal=(Strings.isBlank(cellObject.getString("value").trim())?"0":cellObject.getString("value").trim()); 
						//logger.debug("------"+reVal);
	...
	public String getSuperReportContent(Map<String, Object> map)throws DataAccessException {
		// TODO Auto-generated method stub
				...
				try {
					...
					//设置打印条件显示					
					List<Map<String,Object>> qlist=superReportDataSetMapper.queryRepQuery(map);
					for(Map<String,Object> tmp:qlist){					
						replaceConditon(jsonSheet,tmp,map.get(tmp.get("PARA_CODE")+"_name"));						
					}
					json.put("sheets", jsonSheet);
					content=json.toJSONString();
	...
	public void replaceConditon(JSONObject sheetJson,Map<String,Object> map,Object o){
		JSONObject sheetObject=null;
		for (String sheet : sheetJson.keySet()) {  
			sheetObject=sheetJson.getJSONObject(sheet);
			break;
		}
		JSONObject tmp=sheetObject.getJSONObject("data");
		tmp=tmp.getJSONObject("dataTable");
		String location=map.get("LOCATION").toString();
		int col=location.split(":")[0].toCharArray()[0]-65;
		JSONObject rowObject=tmp.getJSONObject((Integer.parseInt(location.split(":")[1])-1)+"");
		JSONObject cellObject=rowObject.getJSONObject(col+"");	
		cellObject.put("value", o);
	}
}
