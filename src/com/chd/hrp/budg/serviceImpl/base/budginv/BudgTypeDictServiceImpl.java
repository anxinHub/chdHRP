...
@Service("budgTypeDictService")
public class BudgTypeDictServiceImpl implements BudgTypeDictService {

	private static Logger logger = Logger.getLogger(BudgTypeDictServiceImpl.class);
	//引入DAO操作
	...
	private final BudgTypeService budgTypeService = null;
	
	@Override
	public String queryBudgTypeDictByTree(Map<String, Object> entityMap)
			throws DataAccessException {
		StringBuilder jsonResult = new StringBuilder();
		jsonResult.append("{Rows:[");
		List<BudgTypeDict> list = budgTypeDictMapper.queryBudgTypeDict(entityMap);
		if (list.size()>0) {
			int row = 0;
			for (BudgTypeDict mt : list) {
				if (row == 0) {
					jsonResult.append("{");
				} else {
					jsonResult.append(",{");
	...
	*/
	...
	public String saveBudgTypeDict(Map<String,Object> entityMap)throws DataAccessException{
		...
		//判断名称编码是否重复
		List<Map<String, Object>> mtlist = budgTypeService.queryBudgTypeByCodeName(entityMap);
		if (mtlist.size()>0) {
		...
		try {
	        //更新物资类别
			budgTypeMapper.updateBudgTypeByDict(entityMap);
			if(!"0".equals(super_code)){
		        //更新上级类别is_last为否(0)
			...
			budgTypeMapper.updateBudgTypeSuperIsLastByIds(isLastMap);
			budgTypeDictMapper.updateBudgTypeDictSuperIsLastByIds(isLastMap);
			...
			if("1".equals(entityMap.get("is_save_change"))){
				...
				//更新变更记录数据为现有数据
				budgTypeDictMapper.updateBudgTypeDict(entityMap);
			}
	...
	*/
	...
	public String queryBudgTypeDict(Map<String,Object> entityMap) throws DataAccessException{
		...
		if (sysPage.getTotal()==-1){
			...
			List<BudgTypeDict> list = budgTypeDictMapper.queryBudgTypeDict(entityMap, rowBounds);
	...
	*/
	...
	public BudgTypeDict queryBudgTypeDictById(Map<String,Object> entityMap)throws DataAccessException{
		return budgTypeDictMapper.queryBudgTypeDictById(entityMap);
	}
	...
	*/
	...
	public BudgTypeDict queryBudgTypeDictByUniqueness(Map<String,Object> entityMap)throws DataAccessException{
		return budgTypeDictMapper.queryBudgTypeDictByUniqueness(entityMap);
	}
