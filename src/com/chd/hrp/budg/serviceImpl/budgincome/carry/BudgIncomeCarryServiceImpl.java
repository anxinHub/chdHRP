...
package com.chd.hrp.budg.serviceImpl.budgincome.carry;
...
import com.chd.hrp.budg.dao.budgincome.carry.BudgIncomeCarryMapper;
import com.chd.hrp.budg.dao.budgincome.check.BudgIncomeCheckMapper;
import com.chd.hrp.budg.dao.budgincome.elseincome.BudgElseIncomeCheckMapper;
...
@Service("budgIncomeCarryService")
public class BudgIncomeCarryServiceImpl implements BudgIncomeCarryService,Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger.getLogger(BudgIncomeCarryServiceImpl.class);
	//引入DAO操作
	@Resource(name = "budgCarryMapper")
	private final BudgCarryMapper budgCarryMapper = null;
	
	@Resource(name = "budgIncomeCarryMapper")
	private final BudgIncomeCarryMapper budgIncomeCarryMapper = null;
	...
	@Resource(name = "budgElseIncomeCheckMapper")
	private final BudgElseIncomeCheckMapper budgElseIncomeCheckMapper = null;
	...
	public String queryYearMonth(Map<String, Object> entityMap) throws DataAccessException {
		...
		//1.查询预算结账表数据:收入预算已结账数据
		entityMap.put("income_flag", 1);
		List<Map<String, Object>> budgCarryList = budgCarryMapper.queryBudgCarry(entityMap);
		
		//2.查询当前模块启用年度
		entityMap.put("mod_code", "0202");//收入预算模块编码
		cur_year_month = budgCarryMapper.queryYearMonthStart(entityMap);//获取收入预算启用年月
		//2.1未启用,返回空
	...
	public String chargeBudgIncomeCarry(Map<String,Object> entityMap) throws DataAccessException {
		...
		try {
			...
			//3.1判断当前年度医疗收入预算是否存在未审批下达数据(BUDG_MED_INCOME_CHECK)
			List<Map<String,Object>> medIncomeCheckList = (List<Map<String,Object>>)budgIncomeCheckMapper.query(entityMap);
			if(medIncomeCheckList.size() > 0){
				return "{\"warn\":\"医疗收入预算未下达,不可结转 \"}";
			}
			
			//3.2判断当前年度其他收入预算是否存在未审批下达数据(BUDG_ELSE_INCOME_CHECK)
			List<Map<String,Object>> elseIncomeCheckList = (List<Map<String,Object>>)budgElseIncomeCheckMapper.query(entityMap);
			if(elseIncomeCheckList.size() > 0){
				return "{\"warn\":\"其他收入预算未下达,不可结转 \"}";
			}
			...
			budgCarryMap.put("income_flag", "1");
			budgCarryMap.put("income_user", SessionManager.getUserCode());
			budgCarryMap.put("income_date",DateUtil.getCurrenDate());
			...
			if("12".equals(String.valueOf(entityMap.get("month")))){
				...
				if(budgCarry == null){
					...
					entityMap.put("income_flag", "1");
					entityMap.put("income_user",SessionManager.getUserCode());
					entityMap.put("income_date",DateUtil.getCurrenDate());
			...
			//4.查询医院月份收入预算
			List<Map<String, Object>> hosMonthList =budgMedIncomeHosMonthMapper.queryBudgMedIncomeHosMonthByYearMonth(entityMap);
			...
			for(Map<String,Object> obj : hosMonthList){
				...
				if(carried_next_month < 0){
					return "{\"warn\":\"医院月份医疗收入预算:本月结转值小于0,不能结转 \"}";
				}
				...
				hosMonthCurMap.put("subj_code", obj.get("subj_code"));
				...
				hosMonthPreMap.put("subj_code", obj.get("subj_code"));
				hosMonthPreMap.put("last_month_carried", carried_next_month);
				hosMonthUpdateList.add(hosMonthPreMap);
			}
			
			//5.查询科室月份业务预算
			List<Map<String,Object>> deptMonthList  = budgMedIncomeDeptMonthMapper.queryBudgMedIncomeDeptMonthByYearMonth(entityMap);
			...
			//组装本月要修改下月结转的数据
			//组装下月要修改上月结转的数据
			for(Map<String,Object> obj : deptMonthList){
				if("0".equals(obj.get("is_carried"))){//如果指标结转类型为否
					continue;
				}
				...
				if(carried_next_month < 0){
					return "{\"warn\":\"科室月份医疗收入预算:本月结转值小于0,不能结转 \"}";
				}
				
				//修改当前月数据的下月结转值Map
				Map<String,Object> deptMonthCurMap = new HashMap<String,Object>();
				deptMonthCurMap.put("group_id", obj.get("group_id"));
				deptMonthCurMap.put("hos_id", obj.get("hos_id"));
				deptMonthCurMap.put("copy_code", obj.get("copy_code"));
				deptMonthCurMap.put("year", obj.get("year"));
				deptMonthCurMap.put("month", obj.get("month"));
				deptMonthCurMap.put("dept_id", obj.get("dept_id"));
				deptMonthCurMap.put("subj_code", obj.get("subj_code"));
				...
				deptMonthPreMap.put("group_id", obj.get("group_id"));
				deptMonthPreMap.put("hos_id", obj.get("hos_id"));
				deptMonthPreMap.put("copy_code", obj.get("copy_code"));
				deptMonthPreMap.put("year", next_year_month.substring(0, 4));
				deptMonthPreMap.put("month", next_year_month.substring(4, 6));
				deptMonthPreMap.put("dept_id", obj.get("dept_id"));
				deptMonthPreMap.put("subj_code", obj.get("subj_code"));
			...
			//查询其它收入预算
			List<Map<String,Object>> elseIncomeList = budgElseIncomeMapper.queryBudgElseIncomeByYearMonth(entityMap);
			List<Map<String,Object>> elseIncomeUpdateList = new ArrayList<Map<String,Object>>();
			
			
			//5.1 处理其它收入数据
			//组装本月要修改下月结转的数据
			//组装下月要修改上月结转的数据
			for(Map<String,Object> obj : elseIncomeList){
				if("0".equals(obj.get("is_carried"))){//如果指标结转类型为否
					continue;
				}
				...
				if(carried_next_month < 0){
					return "{\"warn\":\"其它收入预算:本月结转值小于0,不能结转 \"}";
				}
				
				//修改当前月数据的下月结转值Map
				Map<String,Object> elseIncomeCurMap = new HashMap<String,Object>();
				elseIncomeCurMap.put("group_id", obj.get("group_id"));
				elseIncomeCurMap.put("hos_id", obj.get("hos_id"));
				elseIncomeCurMap.put("copy_code", obj.get("copy_code"));
				elseIncomeCurMap.put("year", obj.get("year"));
				elseIncomeCurMap.put("month", obj.get("month"));
				elseIncomeCurMap.put("subj_code", obj.get("subj_code"));
				elseIncomeCurMap.put("carried_next_month", carried_next_month);
				
				elseIncomeUpdateList.add(elseIncomeCurMap);
				
				//修改下月数据的上月结转值Map
				Map<String,Object> elseIncomePreMap = new HashMap<String,Object>();
				elseIncomePreMap.put("group_id", obj.get("group_id"));
				elseIncomePreMap.put("hos_id", obj.get("hos_id"));
				elseIncomePreMap.put("copy_code", obj.get("copy_code"));
				elseIncomePreMap.put("year", next_year_month.substring(0, 4));
				elseIncomePreMap.put("month", next_year_month.substring(4, 6));
				elseIncomePreMap.put("subj_code", obj.get("subj_code"));
				elseIncomePreMap.put("last_month_carried", carried_next_month);
				elseIncomeUpdateList.add(elseIncomePreMap);
			}
			
			
			//6.修改医院医疗收入预算：本月的下月结转值和下月的上月结转值
			if(hosMonthUpdateList.size() > 0){
				budgMedIncomeHosMonthMapper.updateBatchBudgMedIncomeHosMonthValue(hosMonthUpdateList);
			}
			
			
			//7.修改科室医疗收入预算：本月的下月结转值和下月的上月结转值
			if(deptMonthUpdateList.size() > 0){
				budgMedIncomeDeptMonthMapper.updateBatchBudgMedIncomeDeptMonthValue(deptMonthUpdateList);
			}
			
			//8.修改其它收入预算：本月的下月结转值和下月的上月结转值
			if(elseIncomeUpdateList.size() > 0){
				budgElseIncomeMapper.updateBatchBudgElseIncomeValue(elseIncomeUpdateList);
			}
	...
	public String reChargeBudgIncomeCarry(Map<String, Object> entityMap) throws DataAccessException {
		...
		try {
			...
			//1.查询当前模块启用年度
			/*entityMap.put("mod_code", "0202");//采购预算模块编码
			...
			if(String.valueOf(entityMap.get("pre_year_month")).equals(start_year_month)){
				return "{\"warn\":\"当前年月为收入预算模块启用年月,不能反结  \"}";
			}*/
			...
			List<Map<String,Object>> deptMonthUpdateList = new ArrayList<Map<String,Object>>();//修改科室收入预算List
			List<Map<String,Object>> HosMonthUpdateList = new ArrayList<Map<String,Object>>();//修改医院收入预算List
			List<Map<String,Object>> elseIncomeUpdateList = new ArrayList<Map<String,Object>>();//修改其它收入预算List
			...
			elseIncomeUpdateList.add(NextYearMonthMap);
			...
			elseIncomeUpdateList.add(CurYearMonthMap);
			...
			//6.修改本月结账标记为0,结转日期为空,结账人为空
			
			entityMap.put("income_flag", "0");
			entityMap.put("income_user","");
			entityMap.put("income_date","");
			budgCarryMapper.updateBudgCarry(entityMap);
