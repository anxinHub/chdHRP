...
/**
 * @Title.
 * @Description. QRCode 二维码处理器
 * @Copyright: Copyright (c) 2016年8月9日 下午6:20:22
...
public final class QRCode {
	 ...
	 * @return QRCode 处理器
	 */
	public static QRCode NEW(final String content, BarcodeFormat bf) {
		QRCode qrcode = new QRCode();
		QRCodeFormat format = QRCodeFormat.NEW();
		qrcode.format = format;
		qrcode.qrcodeImage = toQRCode(content, format, bf);
		return qrcode;
	}
	 ...
	 * @return QRCode 处理器
	 */
	public static QRCode NEW(final String content, BarcodeFormat bf, int width, int height) {
		QRCode qrcode = new QRCode();
		QRCodeFormat format = QRCodeFormat.NEW();
		qrcode.format = format;
		qrcode.qrcodeImage = toQRCode(content, format, bf, width, height);
		return qrcode;
	}
	 ...
	 * @return QRCode 的图像对象
	 */
	public static BufferedImage toQRCode(String content, QRCodeFormat format, BarcodeFormat bf) {
		if (format == null) {
			format = QRCodeFormat.NEW();
		}
	 ...
	 * @return QRCode 的图像对象
	 */
	public static BufferedImage toQRCode(String content, QRCodeFormat format, BarcodeFormat bf, int prmwidth, int prmheight) {
		if (format == null) {
			format = QRCodeFormat.NEW();
		}
		...
		BitMatrix matrix = null;
		try {
			matrix = encode(content, bf, prmwidth, prmheight, format.getHints());
	...
	public static void main(String[] args) {
		QRCode.NEW("123456789").toFile("d:/123.png");

		String qrcode1 = QRCode.from("d:/123.png");
		QRCode.NEW("2222222CHD", BarcodeFormat.QR_CODE).toFile("d:/1234567.png");
		QRCode.NEW("2222222CH", BarcodeFormat.PDF_417, 200, 100).toFile("d:/12345678.png");
		String qrcode3 = QRCode.from("d:/1234567.png");
		String qrcode2 = QRCode.from("d:/12345678.png");
